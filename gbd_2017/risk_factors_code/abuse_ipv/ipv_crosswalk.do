// Prep and crosswalk IPV dataset for Dismod
...
// Bring in newest population envelope 
	qui do "`shared_functions_dir'/get_demographics.ado"
	qui do "`shared_functions_dir'/get_population.ado"
	get_demographics, gbd_team(cov) clear
...
// Load population data
	...
	// Prepare  proportion ever in a relationship variable for population-weighting
		//gen total_ever = mean * pop

		forvalues i= 0/999 { 
			gen total_ever_`i' = draw_`i' * pop
		}

		//label variable total_ever "Total that have ever been in a relationship"

	// Collapse to population weight
		collapse (sum) pop total_ever_*, by(location_id year age_start age_end sex)
...
if `crosswalk' == 1 {
	...
	//			physvio = ever experienced physical partner violence
	//			sexvio = ever experienced sexual partner violence
	//			sexphys = ever experienced sexual or physical violence by partner
	//			sexphcur = current (12-month) sexual or physical violence by partner
	//			severe_sex = severe violence including sexual violence
...
** *********************************************************************************
	// Bring in raw exposure prevalence
		qui do "`shared_functions_dir'/get_epi_data.ado"
		get_epi_data, bundle_id(3371) clear // get data from raw IPV bundle
	...
	// Create age groups that correspond to regression results above
		...
		replace agegrp = "15_49" if age_start >=14 & age_end <=49 // this will be written over again if age group is smaller
	...
	// Generate a dummy to indicate where data is for the "current" period (last 1 to 2 years)
		gen curr = (cv_recall_1yr == 1)
		gen ptype = "prev"
		replace ptype = ptype + "_sev" if cv_case_severe == 1
		replace ptype = ptype + "_curr" if curr == 1
		replace ptype = ptype + "_sex" if cv_sexual_ipv == 1 & cv_phys_ipv==0
		replace ptype = ptype + "_phys" if cv_phys_ipv  == 1 & cv_sexual_ipv==0
		replace ptype = "prev_GS" if ptype == "prev" | strpos(case_name, "cross")>0 // add possibility that case name inidcates a crosswalk has already been applied.
	...
	// Merge on fraction ever_sex
		...
		// will need to change each GBD iteration
		local estimation_years 1990 1995 2000 2005 2010 2017
		foreach i in `estimation_years' {
			replace dismod_year = `i' if abs(`i' - year < 3)
		}

		replace dismod_year = 1990 if year < 1990
		replace dismod_year = 2010 if year==2013 // to deal with gap between 2010 and 2017
		replace dismod_year = 2017 if year==2014 // to deal with gap between 2010 and 2017

		drop year
		rename dismod_year year

		merge m:1 location_id year agegrp using `eversexfinal', keep(3) nogen force
...
// Adjust for fraction who have had sex
	...
	// Create 1,000 draws based on mean and standard error to multiply by 1,000 draws from DisMod model 
		...
		// First fill out those data points that just have effective_sample_size
...
// Make a variable for original case name
	replace case_name = substr(ptype, 6, .)
...
// Drop extraneous data points
	...
	** If a study has breakdown by age, exclude the all age data points
	egen sourcenum = group(nid location_id year_start year_end sex case_name recall_type)
	...
	gen data_status = "excluded" if agedif == maxagedif & sum != 1 & minagedif != maxagedif
		...
		//egen sourcenum = group(nid location_id year_start year_end sex)
	...
	** If a study has ipv_any for an age group exclude other data points that were crosswalked
	egen sourcenum = group(nid location_id year_start year_end sex age_start age_end)
	...
	drop sourcenum goldstd sum
		...
		replace data_status = "excluded" if location_name == "Ukraine" & year_start == 1999 & age_start == 17 & age_end == 44
		replace data_status = "" if location_name == "Turkey" & year_start == 2014 // shouldn't be dropped but caught in above logic
...
// Create an indicator for whether a point was crosswalked. If data is gold standard (case_name = ipv_ever_any) it is not crosswalked, in all other cases it is o
	replace cv_physcrosswalk = case_name != "ipv_ever_any" & regexm(case_name, "phys")
	replace cv_sexcrosswalk = case_name != "ipv_ever_any" & regexm(case_name, "sex")
...
// Drop excluded data -- it will still stay in raw bundle but we can remove it from the prepped bundle
drop if data_status=="excluded"

// MARK OUTLIERS

	// Mark Korea outliers: These data points are very high and are from poor quality studies
...
// Save clean and crosswalked dataset 
	export excel "FILEPATH", sheet("Data") firstrow(variables) replace
