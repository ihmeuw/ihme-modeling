...
##      11/10/2017: the function will now return a list of length==2, with the rmse results and the data w/ covariates
########################################################################################################################################################
#            ##### test_prior()  Function ######
#
#
#       #### Purpose: test out of sample RMSE of all possible linear prior models for STGPR
#                      MUST BE RUN ON CLUSTER
#
#       #### Notes: -One job is submitted per model to proj_custom_models.  Use count_mods=T (the default) to find how many models you need!
#                   -updated on 6/2/2017 to use data_id instead of run_id, and will create holdouts using the get_kos() function by USERNAME
#
#
#
################ Arguments ##############################################################################################################################
...
#     ######### REQUIRED #########
#
#           -data_id: An STGPR data_id that corresponds to data uploaded to the STGPR database
#               ex:   data_id=3906
#
...
#                           Default is TRUE; the idea is you should know how many models you are going to test before testing them
...
#                           Default is "aic"- this is so that models won't break if no holdouts are run.
...
#                           Default is "lmer"
#                           If random effects specified, modtype must be set to "lmer", the default
#                           If no random effects specified, modtype must be set to "lm"
#                           Haven't thoroughly tested models of type 'lm'..watch for bugs!
#               ex: modtype="lm"
#
...
#               ex:   custom_covs=list(c("sodium_age_spec", paste0(j, "temp/syadgir/sodium_age_specific_filled.csv")), c("asdf_covariate", "J:/asdf/path.csv"))
#
...
#                           Must be in R syntax format, eg "cov1:cov2"
...
#
#          -ko_prop: The proportion of data to hold out for each KO. Default is 0.25
#                           Knockouts are created by replicating the age-year missingness pattern of one location in another
#
#           -kos: Number of knockouts to perform on the data set.   Default is 5
#
...
#                           Default is T.  This is to avoid models breaking if age group is specified as a fixed effect
#
...
#                           Default is F.  CODEm does not do this, which is why the default is F
...
#                           Default is TRUE
#                           This may help with problems with holding out data that is too related to the data that isn't being held out
#                           This will decrease the stability of the true proportion of data points being held out (ie it will be generally be higher than the proportion requested by ko_prop)
#
#           -seed: Numeric, seed to set for knockouts.
#                           Default is 32594 (Simon's birth date-don't forget it!)
#
...
#                           Default is 2.  Run a linear model on your data if you think this may not be appropriate
#               ex: slots_per_job=1
#
#           -forms_per_job: Integer, number of formulas to tesst per job to request.
#                           Default is 10. These run relatively quickly. If you are testing many covariates (more formulas), run more forms per job
#               ex: forms_per_job=45
#
################# Output ###############################################################################################################################
#
...
restrict_violations<-function(rmses, covs, prior_sign=NULL, p_value=.05){
  ...
  invisible(
    lapply(covs, function(x){
      ...
      temp<-data.table(lowers, uppers)
      temp[, insig:=ifelse(data.table::between(0, lowers, uppers), 1, 0)]
      temp[is.na(insig), insig:=0]
  ...
  test_prior<-function(data_id, cov_list, data_transform, username,
                       ...
                       ko_prop=0.25, kos=5, remove_subnats=T, no_new_ages=T, only_data_locs=F, drop_nids=T, seed=32594, location_set_id=22,
                       proj="proj_crossval", slots_per_job=2, forms_per_job){
  ...
  output_folder<-paste0("FILEPATH/data_", data_id, "_", date, "/")
  ...
  ##USERNAME: need to set wd fo rstgpr functions to get read in.. 
  wd_orig<-getwd()
  setwd(stgpr_central) ##USERNAME: updated this line 6/27/2018 to the new stgpr repo
  ...
  source(paste0("FILEPATH/linear_oos/make_ko.R"))
  ...
  ##USERNAME: this system is new--pull in a data_id from STGPR database, create own knockouts
  ...
  data_and_names<-bind_covariates(data, cov_list=cov_list, custom_cov_list=custom_covs)
  data<-data_and_names[[1]] ##USERNAME: this is the data w/ bound covariate estimates
  cov_list<-data_and_names[[2]] ##USERNAME: these are the covariate_name_shorts
  ...
  for(sexchar in sex_list){
    ...
    if(length(unique(data.s$age_group_id))==1){
      if(unique(data.s$age_group_id)==22  | unique(data.s$age_group_id)==27){
        ...
        stop("Ask USERNAME about incorporating custom age group")
      }
    }else{
      by_age<-1
    }
  ...
  ##USERNAME: check to make sure banned pairs are valid names
  ...
  ##USERNAME: set up banned set logic
  ...
  for(sexchar in sex_list){
    ...
    ##USERNAME: setup number of jobs to submit
    n <- length(forms)
    ...
    if(F){
      ...
      ##USERNAME: get start and end forms
      date<-gsub("-", "_", Sys.Date())
      start <- seq[id ==i, start]
      end <- seq[id == i, end]

      command <- paste0("qsub -pe multi_slot ", slots_per_job, " -P ", proj, " -N ", paste0("oos_", sexchar, "_", i),
                        " -o /FILEPATH/", username, "/output -e /FILEPATH/", username, "/errors", " FILEPATH/linear_oos/shell.sh ",
                        start, " ", end, " ",
                        sexchar, " ",  data_id, " ", date, " ", data_transform, " ", modtype, " ", kos)
      system(command)
