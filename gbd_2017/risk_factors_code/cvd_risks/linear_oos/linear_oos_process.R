...
##            -process script for test_prior.R script
##
#######################################
...
for(index in start:end){ ##USERNAME: loop over start to end

  output<-paste0(output_folder, "form_", sexchar, "_", index, ".csv") ##USERNAME: save the output of each model here
  ...
  #form<-as.character(forms[[i]])[2]  ##USERNAME: this was for dredge save system

  #form<-formula(paste(paste0(data_transform,"(data)"), forms[index]), sep="~")  ##USERNAME: this is for new formula save system
  ...
  ##USERNAME: should be same as used in STGPR.  Allows lm or lmer
  ...
  rm(mod) ##USERNAME: clean up the model a bit
