...
##USERNAME:data paths
old_data_path<-paste0("USERNAME/2015_full_lit.csv", na.strings= "")
new_data_path<-paste0("FILEPATH/")
micro_folder<-paste0("FILEPATH/", me, "/")

##USERNAME:utility paths
unit_conversion_path <-paste0("FILEPATH/unit_conversion.csv") ##USERNAME:table with conversion factors for things like g/mL to mmol/L
standard_units_path <-paste0("FILEPATH/standard_units.csv")  ##USERNAME: table with the desired measure for each me_name
central<-paste0("FILEPATH/")



output<-paste0("FILEPATH/", me, "_to_sd/to_sd_", date, ".csv")
plot_output<-paste0("FILEPATH/", me, "_sd_data_", date, ".pdf")
...
#####USERNAME: some of Patty's functions
...
df<-new_data ##USERNAME: old data should all be in there
...
## Sex: Code id ##USERNAME: the tolower() function converts uppercase characters to lowercase
...
## Convert  ##USERNAME: this conversion only works on means, not prevalences.  need to figure something out for prevalences....
vars <- c("mean", "sd")
...
df[, cv_lit:=1]
#df[, year_id.t:=round(year_start+year_end/2)]
...
locs <- get_location_metadata(location_set_id=22)[, .(location_id, ihme_loc_id, super_region_name, region_name, level)]  
