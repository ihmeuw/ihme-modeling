...
# load packages
pacman::p_load(data.table, gridExtra, ggplot2, lme4, magrittr, parallel, stringr, zoo, readxl,ini)
...
if (Sys.info()["sysname"] != "Linux") { #linux has trouble reading the odbc file due to dependancy
  ...
  dt <- file.path(out.dir, 'carex_prepped.fst') %>% read.fst(as.data.table=TRUE,old_format = T)
  carc.dt <- file.path(out.dir, 'carex_carc_dt.fst') %>% read.fst(as.data.table=TRUE,old_format = T)
