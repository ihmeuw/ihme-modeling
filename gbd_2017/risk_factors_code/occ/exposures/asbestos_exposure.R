# ---HEADER-------------------------------------------------------------------------------------------------------------
# Purpose: Calculate AIR for a given country, and mesothelioma PAFs
#********************************************************************************************************************************
...
# load packages
# pacman::p_load(data.table, fst, gridExtra, ggplot2, lme4, magrittr, parallel, stringr, zoo)
pacman::p_load(data.table, gridExtra, ggplot2, lme4, magrittr, parallel, stringr, zoo)
library(fst, lib.loc = file.path(h_root,"FILEPATH"))
...
gbd.years <- c(seq(1990,2005,5),2007,2010,2015,2016,2017) #removing unnecessary years, takes too long
gbd.years <- seq(1990,2017)
...
meso <- get_draws(gbd_id_type='cause_id',
                  ...
                  year_id=c(year_start:year_end), age_group_id=relevant.ages,
                  measure_id=1, status='best', num_workers=(cores.provided-1))
...
dt[, measure_id := NULL] #now a proportional measure (% of pop exposed to occ asbestos)
...
paf.dir <- file.path(draw.dir, 'FILEPATH', me.name, output.version)
