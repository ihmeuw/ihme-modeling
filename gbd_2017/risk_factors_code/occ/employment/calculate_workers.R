...
if (Sys.info()["sysname"] == "Linux") {
  ...
  if (length(arg)==0) {
    ...
    arg <- c("361", #location
             "9", #output version
             "1000", #draws
             "0", #whether to calculate occ/ind workers
             "5") #cores
  }
  ...
  arg <- c("151", #location
           "7", #output version
           "1000", #draws
           "1", #whether to calculate occ/ind workers
           "5") #cores

}

# load packages
# pacman::p_load(data.table, gridExtra, ggplot2, lme4, magrittr, parallel, stringr, zoo, fst, readxl)
...
squeezeR <- function(models,
  ...
  #now save the workers draw values, then save the summary vals for review
  write.fst(dt[, -c(draw.cols, prop.cols, 'run_id', 'measure_id'), with=F],
...
if (calc.occind == 0){ ## writing output for total employment to be used in 2_prep_ind.R
  ...
  write.fst(dt[, -c(worker.cols, 'run_id', 'measure_id'), with=F],
          path=fileName(draw.dir, country, 'emp', '_prop.fst'))
  write.fst(dt[, -c(draw.cols, 'run_id', 'measure_id'), with=F],
...
if (calc.occind == 1){ ## writing industry and occupation-specific workers to be used by 2_prep_inj.R etc
  ...
  models <- models[me_name != "occ_occ_major_0_undef" & me_name != "occ_occ_major_X_undef"]
