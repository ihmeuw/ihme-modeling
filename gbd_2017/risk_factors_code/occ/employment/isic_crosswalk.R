...
# load packages
library(Cairo, lib.loc = file.path(h_root,"FILEPATH"))
pacman::p_load(data.table, gridExtra, ggplot2, lme4, magrittr, parallel, stringr)
library(xlsx, lib.loc = file.path(h_root,"FILEPATH"))
...
write.csv(collapse, file.path(out.dir, "ISIC_2_TO_3_LVL_1_CW.csv"),row.names=F)
...
write.csv(collapse, file.path(out.dir, "ISIC_4_TO_3_LVL_1_CW.csv"),row.names=F)
