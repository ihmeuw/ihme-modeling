...
bully_data_anx <- bully_data_anx[Low.or.high!=".", .(author=paste(First.Author, Publication.year), Adjusted,
...
bully_data_anx[, `:=` (cases_total=exposed_cases+nonexposed_cases, exposed_proportion = exposed_sample_size/parameter_sample_size)]


bully_data_anx[, `:=` (arr = or_2_rr(value, cases_total/parameter_sample_size, exposed_sample_size/parameter_sample_size),
...
bully_data_anx[, arr_se := (log(arr_high) - log(arr_low)) / (qnorm(0.975,0, 1)*2)]
bully_data_anx[author=="Schoon 1997", `:=` (arr = value, arr_low=lower, arr_high = upper,  arr_se = sd)]
bully_data_anx[author=="Schoon 1997", `:=` (arr = value, arr_se = sd)]
bully_data_anx[is.na(arr), `:=` (missing_rr=1, arr=value)]
bully_data_anx[is.na(arr_se), arr_se := ifelse(is.na(sd), (log(upper) - log(lower))/(qnorm(0.975, 0, 1)*2), sd)]

## create ratios per pair
for(i in unique(bully_data_anx$threshold_pairs)){
  arr_lowthres <- bully_data_anx[threshold_pairs==i & Low.or.high=="L", arr]
  arr_lowthres_se <- bully_data_anx[threshold_pairs==i & Low.or.high=="L", arr_se]
  bully_data_anx[threshold_pairs==i & Low.or.high=="H", `:=` (ratio = arr / arr_lowthres,
                                                          ratio_se = sqrt(((arr^2)/(arr_lowthres^2)) *  ((arr_se^2)/(arr^2) + (arr_lowthres_se^2)/(arr_lowthres^2))))]
}
bully_data_anx[, `:=` (yi = log(ratio), vi = ratio_se^2)]
meta_rr <- rma(measure="RR", yi=yi, vi=vi, slab=author, data=bully_data_anx, method="DL")

crosswalk <- exp(meta_rr$beta[1])
crosswalk_se <- (exp(meta_rr$ci.ub) - exp(meta_rr$ci.lb))/(qnorm(0.975,0, 1)*2)
# Crosswalk     = 1.310574
# Crosswalk se  = 0.06983203
