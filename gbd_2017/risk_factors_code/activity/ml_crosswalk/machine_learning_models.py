...
# mpl.use("Agg")  # make sure matplot lib is working non-interactively by switching it to a different backend.
...
class MachineLearningModel(object):
    ...
    def train(self, feature_cols, target_col):
        """
        Splits the data and trains the machine learning algorithm. Updates the relevant class properties.

        :param feature_cols:
        :param target_col:
        """

        # self.oh_matched_data = self.oh_matched_data[self.oh_matched_data[target_col.lower() + '_data'].notnull()]
    ...
    def fit_and_save(self, model):
        ...
        # pickle.dump(self.model, open(file_path + '.sav', 'wb'))

    def predict(self, new_df, save_dir, unseen=None, draw_number=None):
        """
        For use on new data. Unseen means never introduced into the program previously on this run.
...
class RandomForestModel(MachineLearningModel):
    ...
    def train(self, feature_cols, target_col):
        """
        Train model and generate relevant metrics
...
class XGBoostModel(MachineLearningModel):
    ...
    def train(self, feature_cols, target_col):
        ...
        #    cross_validation.train_test_split(self.features, self.target, test_size=0.3,
        #                                      random_state=42)
