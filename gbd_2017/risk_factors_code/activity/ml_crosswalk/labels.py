...
# TODO: find where to put these global variables. Should I only read from the config file in one module?
...
class SharedLabels:
    ...
    def super_regions(self, as_dict=False):
        ...
        # super_region_df.super_region_id = super_region_df.super_region_id.astype('category')
...
def get_save_dir(topic, estimand, estimator=None, version=None):
    ...
    root_dir = drives().j + config[topic]['head_dir'] + 'ml_crosswalk/'
    ...
    save_dir = root_dir + 'version_{}'.format(version.replace(" ", "_")) + 'predicting_' + estimand.lower() + '/'
