...
def predict_data_two_step(topic, algorithm, version, estimand_data, first_estimator, second_estimator, estimand,
    ...
    # new_graphing_df = first_set_tools.data_obj.build_test_graphing_dataset(rf=first_set_tools.rf, df=new_predictions)
...
def plot_predictions_vs_estimator_data(topic, version, sups, year_start, year_end, estimator,
    ...
    # maxes = predictions[['{}'.format(estimator_col),'rf_prediction','me_name']].groupby(by='me_name').max()
    # maxes = maxes[['{}'.format(estimator_col),'rf_prediction']].max(axis=1)
...
def make_st_gpr_plots(topic, version, sups, estimand, loc_id, me, total, algorithm):
    ...
    if len(to_graph) == 0:
        return  
    # if to_graph is None:
    #    raise ValueError("You have nulls in your graphing dataset when dealing with \
    #        location_id {} and me_name {}.".format(loc_id, me))
...
class Metrics(object):
    ...
    def print_facet_plots(self):
        ...
        # if linear==True:
        #    plt.savefig(file_path+'linear_pred_vs_actual.png')

        for ax, me_name in zip(g.axes.flat, metric_text):
            ax.plot((0, maxes.value.max()), (0, maxes.value.max()), c=".2", ls="-")
            # plt.setp(ax.texts, text='')
            # ax.text(200, 400, 'Pearson correlation = {}'.format(pearson_vals), ha='center', fontsize=9)
