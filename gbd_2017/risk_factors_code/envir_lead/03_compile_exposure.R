...
## Purpose: Compile Exposure Estimates
############################################################################################################
...
jpath <- file.path("FILEPATH")

#***************************************************************************************************************

# Read in compiled data set used for lead exposure in GBD 2013 
df13 <- fread(file.path("FILEPATH"), header = T)
df13 <- df13[order(nid)]
...
# Drop other studies with underlying data that was re-extracted in the 2016 systematic review and KNHANES extraction,
# and generally fix any mis-extracted or duplicated data
...
# rename and create new variables to better overlap with the format of the 2016 extraction sheet
...
# add1 asks whether it is measured in geometric mean (the gold standard) which corresponds to a cv_mean_type of 0 in gbd2016
...
# read in the 2016 extraction data
df16 <- fread(file.path("FILEPATH"), header = T)
...
NHANES1 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES1, "bll_mean", "mean")
NHANES2 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES2, "bll_mean", "mean")
NHANES3 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES3, "bll_mean", "mean")
NHANES4 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES4, "bll_mean", "mean")
NHANES5 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES5, "bll_mean", "mean")
NHANES6 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES6, "bll_mean", "mean")
NHANES7 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES7, "bll_mean", "mean")
NHANES8 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES8, "bll_mean", "mean")
NHANES9 <- fread(file.path("FILEPATH"), header = T)
setnames(NHANES9, "bll_mean", "mean")
KNHANES1 <- fread(file.path("FILEPATH"), header = T)
setnames(KNHANES1, "bll_mean", "mean")
KNHANES2 <- fread(file.path("FILEPATH"), header = T)
setnames(KNHANES2, "bll_mean", "mean")
KNHANES3 <- fread(file.path("FILEPATH"), header = T)
setnames(KNHANES3, "bll_mean", "mean")
KNHANES4 <- fread(file.path("FILEPATH"), header = T)
setnames(KNHANES4, "bll_mean", "mean")
KNHANES5 <- fread(file.path("FILEPATH"), header = T)
setnames(KNHANES5, "bll_mean", "mean")
KNHANES6 <- fread(file.path("FILEPATH"), header = T)
setnames(KNHANES6, "bll_mean", "mean")
DEUENV <- fread(file.path("FILEPATH"), header = T)
setnames(DEUENV, "bll_mean", "mean")
DELHIDHS <- fread(file.path("FILEPATH"), header = T)
setnames(DELHIDHS, "bll_mean", "mean")
MUMBAIDHS <- fread(file.path("FILEPATH"), header = T)
setnames(MUMBAIDHS, "bll_mean", "mean")
UZBDHS <- fread(file.path("FILEPATH"), header = T)
...
write.csv(dfdata, file.path("FILEPATH"),row.names = F)
...
write.csv(dfdata[cv_mean_type == 0,], file.path("FILEPATH"),row.names=F)
