...
## Purpose: Generate Breastfeeding Indicators and Scale to ABF
########################################################################################################################
...
# set arguments 
input_dir <- paste0("FILEPATH")       #where extracted CSVs currently reside
output_dir <- paste0("FILEPATH")      #where you want to output prepped CSVs
...
for (file in files) {
  ...
  if (("age_month" %in% names(data))) {
    ...
    if ("food_24h" %in% names(data) & "liq_24h" %in% names(data)) {
      #data[food_24h == 0, food_24h := "N"]
      #data[food_24h != "N" & !is.na(food_24h), food_24h := "Y"]  
      #data[liq_24h == "0", liq_24h := "N"]
      #data[liq_24h != "N" & !is.na(liq_24h), liq_24h := "Y"]  
...
write.csv(missing, paste0(output_dir,"FILE"))
