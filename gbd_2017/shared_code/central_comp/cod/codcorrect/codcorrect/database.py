...
from db_tools.config import parse_odbc
...
def get_best_model_version(gbd_round_id):
    ...
    For GBD17: model_version_type_id 10 was removed--it only applied to models
    generated by the, now deprecated, OldCorrect."""
    sql_statement = """
        SELECT
            gbd_round_id,
            model_version_id,
        ...
        WHERE
            ...
            model_version_type_id IN (0, 1, 2, 3, 4, 6, 7);
        """.format(gbd_round_id=gbd_round_id)
    return query(sql_statement, conn_def='cod')


def get_best_shock_models(gbd_round):
    """Get list of models for a given GBD round used in the shock
    aggregator."""
    sql_statement = """
        SELECT
            gr.gbd_round,
...
def GET_BEST_PANIC_SHOCK_MODELS(gbd_round=GBD.GBD_ROUND):
    sql = """
        SELECT
            gr.gbd_round,
            mv.model_version_id,
        ...
        WHERE
            mv.is_best = 1 AND
            mv.model_version_type_id in (5, 7) AND
            gr.gbd_round = {gbd_round};
    """.format(gbd_round=gbd_round)
    result_df = query(sql, conn_def='cod')
    return result_df


def get_best_envelope_version(gbd_round):
    """Get best envelope version."""
    round_map = {2017: 5, 2016: 4}
    result_df = get_envelope(with_shock=0, with_hiv=0,
                             gbd_round_id=round_map[gbd_round])
    if len(result_df) > 1:
...
def get_cause_hierarchy_version(cause_set_id, gbd_round):
    ...
    sql_statement = """
        ...
        WHERE
            csv.gbd_round = {round}
        AND csva.cause_set_id = {id};
    """.format(round=gbd_round, id=cause_set_id)
    result_df = query(sql_statement, conn_def='cod')
...
def get_cause_metadata(cause_metadata_version_id):
    ...
    for i in result_df.index:
        id = result_df.loc[i, 'cause_id']
        metadata_type = result_df.loc[i, 'cause_metadata_type']
        metadata_value = result_df.loc[i, 'cause_metadata_value']
...
def get_age_weights():
    # TODO: This will break next year... abstract the round id.
    sql_query = """
        SELECT
            age_group_id,
...
def get_age_dict():
    agg_sql_query = """
        SELECT
            age_group_id,
            age_group_years_start AS age_start,
    ...
    for age in agg_ages.age_group_id.unique():
        age_start = int(agg_ages.loc[agg_ages['age_group_id'] == age,
                        'age_start'])
        age_end = int(agg_ages.loc[agg_ages['age_group_id'] == age, 'age_end'])
        sql_query = """
            SELECT
                age_group_id
            FROM
...
def get_pred_ex(gbd_round_id):
    """Call only once, cache results to hdf in launch.py and read when needed.
    """
    mort_credentials = parse_odbc()['mortality']
    connection = pymysql.Connect(host=mort_credentials['server'],
                                 user=mort_credentials['user'],
                                 password=mort_credentials['password'],
                                 db='mortality', autocommit=True)
    cursor = connection.cursor(cursor=pymysql.cursors.DictCursor)
    cursor.execute('CALL mortality.get_pred_ex({gbd_round_id})'
                   .format(gbd_round_id=gbd_round_id))
...
def get_hiv_cause_ids(gbd_round_id=5):
    q = """
        SELECT
            DISTINCT(c.cause_id)
        FROM
        ...
        WHERE
            c.cause_id = 298
            OR c.parent_id = 298
            AND csv.cause_set_id = 3
            AND csv.gbd_round_id = {}
            AND c.level > 0;
    """.format(gbd_round_id)
    hiv_causes = query(q, conn_def='cod')
    return hiv_causes.cause_id.tolist()


def get_regional_scalars(gbd_round_id):
    q = """
        SELECT year_id, location_id, mean
        FROM mortality.upload_population_scalar_estimate
        WHERE run_id = (
            SELECT run_id
            ...
            AND status_id = 5)
    """.format(gbd_round_id=gbd_round_id)
    return query(q, conn_def='mortality')


def get_shock_causes_mvids(gbd_round_id):
    q = """
        SELECT cause_id, model_version_id
        FROM cod.model_version mv
        JOIN cod.shock_version_model_version svmv USING (model_version_id)
        JOIN cod.shock_version sv USING (shock_version_id)
        WHERE sv.gbd_round_id = {gbd_round_id}
        AND mv.is_best = 1
        AND sv.shock_version_status_id = 1;
    """.format(gbd_round_id=gbd_round_id)
    return query(q, conn_def='cod')


# TODO: Finish implementing this call to the sproc so we can upload smoothly.
def new_cod_output_partition(output_version_id):
    sql_statement = """
        call add_partition_by_list('cod', 'output', '{version}')
    """.format(version=output_version_id)
    # Not sure query can pass a sproc
    query(sql_statement, conn_def='cod')
...
def new_CoDCorrect_version():
    sql_statement = """
        ...
        FROM
            cod.output_version
    """
    result = query(sql_statement, conn_def='cod').loc[0, 'max_id']
    return result + 1


def create_new_output_version_row(output_version_id, description,
                                  envelope_version_id, conn_def, status=2):
    logging.info("database.create_new_output_version_row")
    if not is_existing_output_version(output_version_id, conn_def):
        ...
        ins_df = pd.DataFrame({'output_version_id': output_version_id,
                              ...
                              columns=['output_version_id', 'description',
                                       ...
                                       'is_best', 'date_inserted',
                                       'inserted_by', 'last_updated',
                                       'last_updated_by',
                                       'last_updated_action'],
                              index=[0])
        # Insert row
...
def create_new_compare_version(gbd_round_id, description, conn_def, status):
    ...
    compare_vers_query = """
        SELECT
            MAX(compare_version_id) as compare_version
        FROM
            gbd.compare_version
        WHERE
            gbd_round_id={round}
    ...
    return query(compare_vers_query,
                 conn_def=conn_def).loc[0, 'compare_version']


def get_latest_compare_version(gbd_round_id, conn_def):
    q = """
    ...
    return int(res.at[0, 'compare_version_id'])
...
def create_new_process_version(gbd_round_id, gbd_process_version_note,
    ...
    make_process_vers = """
        CALL gbd.new_gbd_process_version(
            {round}, 3, "{note}", "{code_vers}", NULL, NULL)
    """.format(round=gbd_round_id, note=gbd_process_version_note,
               code_vers=code_vers)
    process_vers = query(make_process_vers, conn_def=conn_def)
    return int(process_vers.loc[0, 'v_return_string'].split()[2]
               .replace('"', "").replace(",", ""))
...
def add_metadata_to_process_version(process_version_id, output_version_id,
    ...
    query = """
        ...
        VALUES
            ...
            ({pv_id}, 6, "{env_id}"),
            ({pv_id}, 7, "{popv_id}")
    """.format(pv_id=process_version_id, ov_id=output_version_id,
               env_id=envelope_version_id, popv_id=pop_version_id)
...
def upload_diagnostics(parent_dir):
    ...
    inf.infile(path='{pd}/_temp/upload_diagnostics.csv'.format(pd=parent_dir),
               with_replace=True, commit=True)


def upload_cod_summaries(directories, conn_def):
    logging.info("database.upload_cod_summaries")
    sesh = get_session(conn_def)
...
def update_status(output_version_id, status, conn_def):
    ...
    update_cmd = """
        ...
        WHERE
            output_version_id = {v}
    """.format(s=status, v=output_version_id)
    exec_query(update_cmd, session=sesh, close=True)


def unmark_cod_best(output_version_id, gbd_round_id, cod_conn_def):
    ...
    update_cod_cmd = """
        ...
        WHERE
            ...
            AND code_version = {code_vers}
    """.format(now=now, v=output_version_id, code_vers=gbd_round_id)
...
def mark_cod_best(output_version_id, cod_conn_def):
    ...
    update_cod_cmd = """
        ...
        WHERE
            output_version_id = {v}
    """.format(now=now, v=output_version_id)
    exec_query(update_cod_cmd, session=cod_sesh, close=True)


def mark_gbd_best(process_version_id, gbd_round_id, gbd_conn_def):
    gbd_sesh = get_session(gbd_conn_def)
    update_gbd_cmd = """
        UPDATE
            gbd.gbd_process_version
...
def get_output_version_cache(gbd_round_id, conn_def):
    sql = """
        SELECT *
        FROM cod.output_version
        WHERE code_version = {gbd_round_id}
    """.format(gbd_round_id=gbd_round_id)
    return query(sql, conn_def=conn_def)


def get_gbd_process_version_cache(gbd_round_id, conn_def):
    sql = """
        SELECT *
...
def get_gbd_compare_version_cache(gbd_round_id, conn_def):
    sql = """
        SELECT *
        FROM gbd.compare_version
        WHERE gbd_round_id = {gbd_round_id}    
