...
class Uploader(object):
    ...
    def activate_process_version(self, process_version_id, session):
        query = ('UPDATE gbd.gbd_process_version '
                 'SET gbd_process_version_status_id = 1 '
                 'WHERE gbd_process_version_id = {}').format(process_version_id)
        exec_query(query, session=session, close=True)
