...
from get_draws.api import get_draws as draws
from adding_machine.summarizers import get_estimates

logging.basicConfig(level=logging.DEBUG)
logger = logging.getLogger("maternal_custom.calculate_mmr")
...
class MMR(object):
    def __init__(self, cause_id, year_id, out_dir):
        ...
        self.aggregated_age_group_ids = {24: list(range(8, 15)), 169: list(range(7, 16)),
                                         162: list(range(8, 10))}
    ...
    def pull_codcorrect_draws(self):
        ...
        codcorrect_df = draws('cause_id', self.cause_id,
        ...
        codcorrect_df = codcorrect_df.ix[
            codcorrect_df.age_group_id.isin(
                list(self.age_group_ids) + list(self.aggregated_age_group_ids.keys()))]
    ...
    def calculate_live_births(self, pop, asfr):
        logger.info("Calculating live births")
    ...
    def save_mmr(self, mmr_draws, mmr_summaries):
        logger.info("Saving MMR draws to " + self.out_dir + r'FILEPATH'.format(
                         self.year_id[0], self.cause_id[0]))
        mmr_draws.to_hdf(self.out_dir + r'FILEPATH'.format(
                         self.year_id[0], self.cause_id[0]),
        ...
        logger.info("Saving MMR summaries to " + self.out_dir + r'FILEPATH'
                             .format(self.year_id[0], self.cause_id[0]))
        mmr_summaries.to_csv(self.out_dir + r'FILEPATH'
                             .format(self.year_id[0], self.cause_id[0]),
...
class AggregateLocations(object):
    ...
    def aggregate(self):
        ...
        for lsid in self.location_set_ids:
            ...
            for level in list(range(max_level, 0, -1)):
                print("Level:", level)
    ...
    def pull_hierarchy(self, location_set_id):
        logger.info("Pulling location hierarchy for {}".format(location_set_id))
    ...
    def get_region_locs(self, loc_df):
        logger.info("Getting region locations")
        return (loc_df[loc_df["location_type_id"] == 6]
                ['location_id'].tolist())

    def load_regional_scalars(self):
        logger.info("Loading regional scalars")
        call = '''
            SELECT 
                *
            FROM
            ...
            WHERE
                run_id = (SELECT 
                          ...
                          WHERE
                            process_id = 23 AND gbd_round_id = 5
                                AND status_id = 5)
               '''
        df = query(call, conn_def='database')
        df = df[['location_id', 'year_id', 'mean']]
        df.rename(columns={'mean': 'scaling_factor'}, inplace=True)
        return df

    def get_most_detailed(self, loc_df):
        logger.info("Getting most detailed locations only.")
        return (loc_df.ix[loc_df['most_detailed'] == 1]['location_id']
                .drop_duplicates().tolist())

    def check_missing_locations(self, df, loc_df, most_detailed):
        logger.info("Checking missing locations.")
    ...
    def keep_aggregates_only(self, aggregated_df, location_set_id):
        logger.info("Keeping aggregate locations only.")
