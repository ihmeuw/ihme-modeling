...
jobname, env_model_vers, source_id, target_id, out_dir = sys.argv[1:6]
...
if 'timing' in jobname:
    env = get_draws(gbd_id=366, gbd_id_type='cause_id', source='codcorrect',
                measure_id=[1], sex_id=[2], location_id=locations)
else:
...
if pull_results:
    ...
    cfs = get_draws(gbd_id=source_id, gbd_id_type='modelable_entity_id', source='epi',
                measure_id=[18], sex_id=[2])
else:
    logger.info("Pulling results from draw files.")
    dfs = []
    for year in list(range(1980, 2018)):
        df = pd.read_hdf(os.path.join(cluster_dir, str(source_id), '{}_2.h5'.format(year)), key='draws')
        dfs.append(df)
    cfs = pd.concat(dfs)
...
if "timing" in jobname:
    ...
    final_deaths.to_hdf('%s/FILEPATH.h5' % out_dir,
                        'draws', format='table', mode='w',
                        data_columns=['measure_id', 'location_id', 'year_id',
                                      'age_group_id', 'sex_id'])
else:
    final_deaths['cause_id'] = target_id
    final_deaths.to_hdf('%s/FILEPATH%s.h5' % (out_dir, target_id),
                        key='draws', data_columns=['location_id', 'year_id', 'age_group_id', 'sex_id', 'cause_id', 'measure_id'],
                        format='table', mode='w')
