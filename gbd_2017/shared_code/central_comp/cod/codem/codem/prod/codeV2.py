...
logger = logging.getLogger("codem.codeV2")
...
parser.add_argument('db_connection', help='db server to connect to',
                    type=str)
parser.add_argument('gbd_round_id', help='the gbd round id to use',
                    type=int)
...
model_version_id = int(args.model_version_id)
...
codem_model = Ensemble(model_version_id, db_connection, gbd_round_id=gbd_round_id, debug_mode=debug_mode,
...
try:
    ...
    print("Will not dump dill because it's too big.")
