...
class Ensemble:
    ...
    def __init__(self, model_version_id, db_connection, gbd_round_id=5, debug_mode=False,
        ...
        self.mod_inputs = Inputs(model_version_id, db_connection,
                                 update=log_results, debug_mode=debug_mode, gbd_round_id=gbd_round_id)
        self.log_results = log_results
        self.base_dir = "FILEPATH"
        self.model_dir = self.base_dir.format(
            self.mod_inputs.acause, model_version_id)
        rSource = "FILEPATH".format(model_version_id)
    ...
    def move_run_location(self):
        ...
        if not os.path.exists(self.model_dir):
            ...
            subprocess.call(
                "chmod 777 -R {}".format('/'.join(self.model_dir.split("/"))), shell=True)
    ...
    def adjust_input_data(self, save=True):
        ...
        self.mod_inputs.data_frame["ln_rate"][(
            self.mod_inputs.data_frame["cf"].isnull())] = np.NaN
        self.mod_inputs.data_frame["lt_cf"][(
            self.mod_inputs.data_frame["cf"].isnull())] = np.NaN
        ...
        if save:
            pd.concat([self.mod_inputs.data_frame,
                      ...
                      axis=1).to_csv("FILEPATH.csv", index=False)
            self.mod_inputs.priors.to_csv("FILEPATH.csv", index=False)
    ...
    def covariate_selection(self):
        ...
        if not os.path.isfile("FILEPATH.txt"):
            if self.old_covariates_mvid is None:
                ...
                subprocess.call("FILEPATH " +
                                self.rSource + "FILEPATH.R " + str(self.model_version_id) +
                                " " + self.db_connection + "URL" + " " +
                                self.model_dir, shell=True)
            else:
                cov_file_path = self.base_dir.format(self.mod_inputs.acause,
                                                     self.old_covariates_mvid) + '/FILEPATH.txt'
                print cov_file_path
        ...
        self.mod_inputs.submodel_covariates = space.import_cv_vars(
            "FILEPATH.txt")
    ...
    def run_knockouts(self):
        ...
        # write the knockouts cause we are still rpy2less
        pd.concat(self.mod_inputs.knockouts, axis=1).to_csv("FILEPATH.csv",
                                                            index=False)
    ...
    def run_linear_model_builds(self):
        ...
        self.json_dict = space.import_json("FILEPATH.txt")
    ...
    def read_linear_models(self):
        ...
        """
        json_dict = space.import_json("FILEPATH.txt")
    ...
    def write_results(self):
        ...
        self.mod_inputs.submodel_rmse = {x: y for x, y in zip(
            self.linear_models.submodel_ids, self.linear_models.RMSE)}
        self.mod_inputs.submodel_rmse.update(
            {x: y for x, y in zip(self.st_models.submodel_ids, self.st_models.RMSE)})
    ...
    def cleanup_code_base(self):
        ...
        subprocess.call(
            "rm -rf FILEPATH{}".format(self.model_version_id), shell=True)
    ...
    def aggregate_results(self):
        ...
        merge_data += [
            i for i in self.mod_inputs.data_frame.columns if i.startswith("draw_")]
    ...
    def reload_data(self, gbd_round_id):
        ...
        st_model_file = os.path.join(self.model_dir, "FILEPATH.txt")
        json_dict = space.import_json(st_model_file)
        draw_data = pd.read_hdf(
            self.model_dir + "FILEPATH".format(sexn), "data")
        self.mod_inputs.data_frame, self.mod_inputs.covariates, self.mod_inputs.priors = \
            Q.getCodemInputData(
                self.model_version_id, self.db_connection, gbd_round_id, outlier_save=False)
        ...
        draw_data = draw_data.rename(
            columns={"age_group_id": "age", "year_id": "year", "sex_id": "sex"})
        self.mod_inputs.data_frame = pd.merge(
            self.mod_inputs.data_frame, draw_data, how='inner')
    ...
    def subset_data(self, submodels):
        ...
        if not isinstance(submodels, list):
            submodels = [submodels]
        id_cols = [
            c for c in self.mod_inputs.data_frame.columns if not c.startswith("draw_")]
        inv_map = {}
    ...
    def transform_draws(df, model_space):
        ...
        """
        assert model_space in [
            "death", "cf", "rate"], 'models_space must be either "death", "cf", or "rate"'
        assert len(np.setdiff1d(["cf", "envelope", "pop"], df.columns)
                   ) == 0, 'df must contain "cf", "envelope" & "pop"'
        draws = [d for d in df.columns if d.startswith("draw_")]
        if model_space == "death":
            df["y"] = df["cf"] * df["envelope"]
        elif model_space == "cf":
            df[draws] = df[draws].values / \
                df["envelope"].values[..., np.newaxis]
            df["y"] = df["cf"]
        else:
            df[draws] = df[draws].values / \
                df["pop"].values[..., np.newaxis] * 100000
    ...
    def plot_location(self, location_id, age_group_id, model_space="death", submodels=None):
        ...
        if len(draws) >= 20:
            ...
            plt.fill_between(sub_df.year, sub_df.lower_bound,
                             sub_df.upper_bound, alpha=.2)
        plt.xlabel("Year")
        plt.ylabel(model_space)
        plt.title("location_id: {l}, age_group_id: {a}".format(
            l=location_id, a=age_group_id))
