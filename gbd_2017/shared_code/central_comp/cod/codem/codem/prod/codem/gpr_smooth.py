...
def find_variance_type(df, ko):
    ...
    for location in locations:
        for age in age_groups:
            ...
            all_data = df2[df2.filter(
                regex="%*_train").values.flatten()].source_type.count()
            non_representative_count = sum(
                df2[df2.filter(regex="%*_train").values.flatten()].national == 0)
            if all_data == representative_count >= 10:
                df2.loc[:, "variance_type"] = "high"
            elif non_representative_count > 0 and non_representative_count != all_data:
                df2.loc[df2.national == 0, "variance_type"] = "low"
...
def calculate_nsv(df, ko, simple_ln, simple_lt, residuals, variance):
    ...
    train_var = filter(lambda x: re.search(
        r"ko[0-9]+_train", x), ko.columns)[0]
    df2["ln_rate_nsv"] = 0.
    df2["lt_cf_nsv"] = 0.
...
def age_group_nsv(age_df, res_df, simple_df):
    train_var = filter(lambda x: re.search(
        r"ko[0-9]+_train", x), age_df.columns)[0]
    sweet = age_df[(age_df.variance_type == "high")
                   & (age_df[train_var])].index
    not_sweet = age_df[(age_df.variance_type != "high") &
                       (age_df[train_var])].index
    medium = age_df[(age_df.variance_type == "medium") &
                    (age_df[train_var])].index
    low = age_df[(age_df.variance_type == "low") &
                 (age_df[train_var])].index
    amplitude = {"high": mad(res_df.loc[sweet].as_matrix(), axis=0) * 1.4826,
                 "medium": mad(res_df.loc[not_sweet].as_matrix(), axis=0) * 1.4826}
    amplitude["low"] = amplitude["medium"]
    nsv = {"high": (mad(simple_df.loc[sweet].as_matrix(), axis=0) * 1.4826)**2,
           "medium": (mad(simple_df.loc[medium].as_matrix(), axis=0) * 1.4826)**2,
           "low": (mad(simple_df.loc[low].as_matrix(), axis=0) * 1.4826)**2}
...
def age_group_gpr(ca_df, age_amplitude, preds, response_list, scale):
    train_var = filter(lambda x: re.search(
        r"ko[0-9]+_train", x), ca_df.columns)[0]
...
def median_amplitude(amp):
    ...
    '''
    amplitudes = amp.copy()
    for data_type in ["medium", "high", "low"]:
        needs_replace = []
        valid_data = np.array([])
        for age in amplitudes.keys():
            if np.any(np.isnan(amplitudes[age][data_type])) or np.any(amplitudes[age][data_type] == 0):
                ...
                valid_data = np.append(
                    valid_data, amplitudes[age][data_type], 0)
        if len(valid_data) == 0:
            continue
        valid_data = np.reshape(
            valid_data, (len(amplitudes[age][data_type]), -1))
        valid_data = np.percentile(valid_data, 50, axis=1)
        for age in needs_replace:
            amplitudes[age][data_type] = valid_data
    if not all([check_amplitude_array_validity(amplitudes[a]["medium"]) for a in amplitudes.keys()]):
        for a in amplitudes.keys():
            amplitudes[a]["medium"] = amplitudes[a]["high"]
    return amplitudes
