...
class WarningLog:
    ...
    def add_warning(self, warning):
        ...
        self.text += "{count}. {message}\n".format(
            count=self.count, message=warning)
    ...
    def log_missingness(self, data_frame, covariates):
        ...
        for m in missing.keys():
            if missing[m] != 0:
                self.add_warning(message.format(
                    na_count=missing[m], variable=m))
    ...
    def log_bad_cf(self, data_frame, var):
        ...
        bad_value_count = ((data_frame[var] == 1)
                           | (data_frame[var] == 0)).sum()
        self.add_warning(message.format(
            bad_cf_count=bad_value_count, variable=var))
