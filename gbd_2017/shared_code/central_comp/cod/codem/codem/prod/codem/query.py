...
def outlierSPROC(model_version_id, gbd_round_id, connection):
    """
    ...
    DB = 'DATABASE'.format(creds=creds,
    ...
    try:
        ...
        if re.search("No outlier found for this model version id", str(msg)):
            print msg
        elif re.search("already exists in outlier_history table", str(msg)):
            print msg
        else:
            raise e
    finally:
...
def codQuery(cause_id, sex, start_year, start_age, end_age,
             location_set_version_id, db_connection, model_version_id, gbd_round_id, outlier_save):
    '''
...
def locQuery(location_set_version_id, db_connection):
    ...
    df["path_to_top_parent"] = \
        df["path_to_top_parent"].map(
            lambda x: ",".join((x[2:]).split(",")[:3]))
...
def data_variance(df, response):
    ...
    def f(i): return np.random.normal(cf[i], cf_sd[i], 100) * (env[i] / pop[i])
    if response == "lt_cf":
        def f(i): return np.random.normal(cf[i], cf_sd[i], 100)
...
def queryCodData(cause_id, sex, start_year, start_age, end_age, regionsExclude,
                 location_set_version_id, gbd_round, db_connection, model_version_id, gbd_round_id, outlier_save):
    """
    :param cause_id: int
    ...
    :param outlier_save: bool
        save model outliers in outlier history table
    :return: data frame
...
def covMetaData(model_version_id, db_connection):
    ...
    '''
    df = db_connect.query(QS.metaQueryStr.format(
        mvid=model_version_id), db_connection)
    models = df.covariate_model_id.values.tolist()
    names = db_connect.query(QS.covNameQueryStr.format(
        ", ".join(str(x) for x in models)), 'DATABASE')
...
def createAgeDF(db_connection):
    ...
    ageDF2 = ageDF.copy(deep=True)
    ageDF['age'] = 22
    ageDF2['age'] = 27
    return pd.concat([ageDF, ageDF2], ignore_index=True)
...
def getCVDF(covID, trans, lag, offset, sex, location_set_version_id, db_connection):
    '''
...
def getCovData(model_version_id, location_set_version_id, db_connection):
    ...
    df = getCVDF(covs.covariate_model_id[0], covs.transform_type_short[0],
                 covs.lag[0], covs.offset[0], sex, location_set_version_id, db_connection)
    for i in range(1, len(covs)):
        dfTemp = getCVDF(covs.covariate_model_id[i],
                         covs.transform_type_short[i], covs.lag[i],
                         covs.offset[i], sex, location_set_version_id, db_connection)
...
def getCodemInputData(model_version_id, db_connection, gbd_round_id, outlier_save):
    '''
    integer -> (Pandas data frame, Pandas data frame)
    ...
    :param outlier_save: save outliers to outlier history table? Don't want to do this when we are just
                         reloading an old CODEm Ensemble object
    '''
...
def write_model_output(df_true, model_version_id, sex_id, db_connection):
    ...
    c = ["mean_cf", "lower_cf", "upper_cf",
         "year", "age", "sex", "location_id"]
    df = df[c]
...
def submodel_covs(submodel_version_id, db_connection):
    ...
    call = '''
        ...
        '''.format(submodel_version_id)
    covs = db_connect.query(call, db_connection).ids.values.tolist()
    df = db_connect.query(covNameQueryStr.format(", ".join(str(x) for x in covs)),
                          'modeling-covariates-db')
...
def model_version_type_id(model_version_id, db_connection):
    ...
    m_type_id = db_connect.query(call, db_connection)[
        "model_version_type_id"][0]
    return m_type_id
