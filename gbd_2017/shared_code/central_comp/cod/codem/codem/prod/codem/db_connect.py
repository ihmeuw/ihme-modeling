...
logger = logging.getLogger("codem.db_connect")
...
def query(call, connection, creds=None):
    ...
    if creds is None:
        creds = read_creds()
    DB = 'FILEPATH//{creds}@{connection}.URL:PORT'.\
...
def write_df_to_sql(df, db, table, connection, creds=None, return_key=False):
    ...
    if creds is None:
        creds = read_creds()
    DB = "FILEPATH//{creds}@{connection}.URL:PORT/{db}".\
...
def countPartition(db, table, versionID, db_connection):
    ...
    :db_connection (str): server to connect to, e.g. 'DATABASE'
...
def increase_partitions(db_connection, db, table):
    ...
    in GBD 2016 and GBD 2017. This has only been used for the TABLE table.

    :db_connection (str): server to connect to, e.g. 'DATABASE'
    :db (str): database, e.g. 'cod'
    :table (str): table in database, e.g. 'TABLE'
