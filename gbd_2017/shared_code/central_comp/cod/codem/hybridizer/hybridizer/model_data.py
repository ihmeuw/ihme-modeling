...
class ModelData(object):
    ...
    def aggregate_locations(self):
        ...
        data = data.ix[data['location_id'].isin(
            self.get_estimated_locations())]
        ...
        for level in xrange(max_level, 0, -1):
            ...
            data = pd.concat([self.format_draws(data), temp]
    ...
    def generate_all_ages(self):
        ...
        data = data.ix[data['age_group_id'].map(
            lambda x: x in AGES_DISAGGREGATED)]
    ...
    def generate_summaries(self):
        ...
        self.data_summaries = self.data_summaries[self.index_columns +
                                                  self.data_columns]
        self.data_summaries['mean_cf'] = self.data_summaries[self.data_columns].mean(
            axis=1)
...
if __name__ == "__main__":
    log_dir = 'FILEPATH'.\
        format(acause=cause_data[hybrid_model_properties['cause_id']]['acause'],
               model_version_id=model_version_id)
    l.setup_logging(log_dir, 'hybridizer1')
