...
class Model:
    def __init__(self, d, response):
        ...
        self.ran_eff = {k: dic2df(d["ran_eff"][k])
                        for k in d["ran_eff"].keys()}
        self.random_intercepts = self.ran_eff.keys()
    ...
    def adjust_predictions(self, vector, linear_floor, df_sub):
        ...
        if self.response == "ln_rate":
            ...
            x = np.log((linear_floor / 100000.) /
                       (df_sub["envelope"] / df_sub["pop"]))
            floor = x / (1 - x)
