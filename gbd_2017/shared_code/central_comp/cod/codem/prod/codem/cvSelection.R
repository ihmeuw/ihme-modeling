...
output_file <- file(paste0(model_dir, "/FILEPATH"), open="wt")
sink(output_file)
...
mci_to_cmvi <- function(model_covariate_id){
    ...
    con <- dbConnect(MySQL(), user='codem', password=password, dbname="cod",
                     host=DB)
...
holdout_number <- function(model_version_id){
    ...
    con <- dbConnect(MySQL(), user='codem', password=password,
                     host=DB, dbname="shared")
    res <- dbSendQuery(con, call)
    df <- fetch(res)
    dbDisconnect(dbListConnections(MySQL())[[1]])
    df[1,1]
