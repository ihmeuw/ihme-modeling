...
from cascade_ode.cv import enforce_min_cv
...
class Cascade(object):
    ...
    def __init__(
        ...
        Args:
            ...
            version(str, 'strUser'): One of 'strUser' or 'strUserB'. 
        ...
        demo = Demographics()
        self.loctree = loctree(location_set_version_id=location_set_version_id,
...
class Cascade_loc(object):
    ...
    def __init__(self, loc, sex, year, cascade, parent_loc=None, reimport=True,
        ...
        self.in_dir = os.path.join(self.cascade.root_dir, 'locations', str(
            loc), 'inputs', sex_label, str(self.year))
        self.out_dir = os.path.join(self.cascade.root_dir, 'locations', str(
            loc), 'outputs', sex_label, str(self.year))
    ...
    def shift_priors(self, data, adj_data):
        ...
        for ig in integrands:
            ...
            prior_func = interpolate.InterpolatedUnivariateSpline(
                ...
                k=1,
                ext=3)
            ig_data['spline'] = ig_data.age_mid.apply(lambda x: prior_func(x))

            fit_mask = ig_prior.meas_value != 0
            spline_mask = ig_data.spline != 0

            data_over_prior = (
                    ig_data.adjust_median[spline_mask]
                    /
                    ig_data.spline[spline_mask])

            scale_factor = data_over_prior.mean()
            ig_prior.loc[fit_mask, 'meas_value'] = (
                    ig_prior.meas_value[fit_mask] * scale_factor)
            shifted_priors = shifted_priors.append(ig_prior)
    ...
    def gen_data(self, location_id, sex_id, drop_emr=False):
        ...
        for integrand in integrands_to_average:
            if (self.cascade.model_version_meta.add_csmr_cause.notnull(
            ).all() and
                    self.cascade.single_param is None and
                    'mtspecific' not in list(exclude_params)):
        ...
        if self.loclvl == 'world':
            ...
            if self.cascade.single_param is None:
                ...
                subdata_mtall = subdata_mtall.groupby(
                    group_cols).mean().reset_index()
                subdata_mtall['meas_stdev'] = inf
                subdata_mtall.ix[subdata_mtall.integrand ==
                                 'mtall', 'atom'] = 'none'
                subdata_mtall.ix[subdata_mtall.integrand ==
                                 'mtall', 'subreg'] = 'none'
                subdata_mtall.ix[subdata_mtall.integrand ==
    ...
    def draw(self):
        ...
        for col in draw_cols:
            clipped = draws.ix[draws.measure_id.isin(
...
def drill_to(model_version_id, location_id, sex_id, year_id, gbd_round_id=5):
    '''
    Run dismod down to the given location id. This function assumes
    the global dismod model has already run.
    ...
    casc = Cascade(
        model_version_id, reimport=False,
        root_dir=os.path.expanduser(settings['cascade_ode_out_dir']))
    year_split_lvl = casc.model_version_meta.fix_year.values[0] - 1
    lt = casc.loctree

    drill_ids = [node.id for node in reversed(
        lt.get_node_by_id(location_id).ancestors())]
    drill_ids.append(location_id)

    parent_child_pairs = [
        (drill_ids[i - 1], drill_ids[i]) for i in range(1, len(drill_ids))]

    cascade_locs = []
    for parent_id, child_id in parent_child_pairs:
        if parent_id == 1:
            # global case, model has already run
            parent = Cascade_loc(parent_id, 0, 2000, casc, reimport=False)
        else:
            parent = Cascade_loc(parent_id, sex_dict[sex_id], year_id, casc,
                                 reimport=False)

        this_lvl = lt.get_nodelvl_by_id(child_id)
        if this_lvl >= (year_split_lvl - 1):
            # don't run full timespan
            child = Cascade_loc(
                child_id, sex_dict[sex_id], year_id, casc, parent_loc=parent)
        else:
            # run full timespan
            child = Cascade_loc(
                child_id, sex_dict[sex_id], year_id, casc, parent_loc=parent,
        ...
        child.run_dismod()
        child.summarize_posterior()
        child.draw()
        child.predict()
        cascade_locs.append(child)
