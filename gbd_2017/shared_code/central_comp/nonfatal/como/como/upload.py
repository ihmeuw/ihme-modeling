...
class LoadNonfatal(object):
    ...
    def make_partitions(self, year_type):
        ...
        for component in self.components:
            ...
            for loc in self.location_id:
                q = "CALL gbd.add_partition('gbd','{}',{})".format(table, loc)
                sesh.execute(q)
                sesh.commit()
        sesh.close()
    ...
    def load_component_location(self, component, location_id, year_type):
        ...
        if year_type == "single_year":
            indir_glob = "{}/summaries/{}/*/{}/{}/*.csv".format(
                self.como_version.como_dir, component, year_type, location_id)
        else:
            indir_glob = "{}/summaries/{}/*/{}/{}.csv".format(
                self.como_version.como_dir, component, year_type, location_id)
...
def run_upload(como_version, location_id=[], n_processes=25):
