...
class SourceSinkFactory(object):
    ...
    def get_params_by_component(self, component, source_type="draws"):
        if source_type not in ["draws", "inputs"]:
            raise ValueError("'source_type' must be either 'draws' or 'inputs'"
                             "received {}".format(source_type))
        dim = self.como_version.nonfatal_dimensions
        if component != "sexual":
            dim = dim.get_dimension_by_component(component, 5)
        else:
            dim = dim.get_dimension_by_component("cause", 5)
        ...
        if self.redis_host:
            io_queue = IOQueue()
            io_queue.connect(host=self.redis_host)
            params["io_queue"] = io_queue
        return params
    ...
    def get_sequela_modelable_entity_source(self, modelable_entity_id,
        ...
        # make a draw source from parameters
