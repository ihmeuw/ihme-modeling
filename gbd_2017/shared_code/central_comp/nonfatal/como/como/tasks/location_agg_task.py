...
class LocationAggTaskFactory(object):
    ...
    def get_task(self, component, year_id, sex_id, measure_id,
        ...
        agg_locs = [node for node in loc_tree.node_ids
        ...
        for location_id in [node.id for node in loc_tree.leaves()]:
            if measure_id == 6:
                upstream_name = IncidenceTaskFactory.get_task_name(
                    location_id, sex_id)
            elif measure_id == 5 and component in ["sequela", "injuries",
                                                   "impairment"]:
...
class LocationAggTask(object):
    ...
    def __init__(self, como_version, measure_id, year_id, sex_id,
        ...
        self._ss_factory = SourceSinkFactory(como_version, redis_host)
