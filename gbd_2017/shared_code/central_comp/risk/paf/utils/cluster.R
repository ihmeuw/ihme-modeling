...
retry_qsub <- function(job_name, script, params, out_dir, slots, arguments = NULL,
    ...
    while (tries != 0 & nrow(params) != 0) {
        ...
        for (i in 1:nrow(params)) {
            ...
            if (loc_only) {
                file <- paste0(out_dir, params[i, location_id], ".csv")
            } else {
                file <- paste0(out_dir, params[i, location_id], "_", params[i, sex_id], ".csv")
            }
            if (file.exists(file)) {
                fini <- c(fini, i)
            }
        ...
        if (is.null(fini)) {
            ...
            stop("All jobs failed. Check out your logs and submit a help ticket!")
        }
        tries <- tries - 1
        params <- params[-fini, ]
