# pull exposures
get_exp <- function(rei_id, location_id, year_id, sex_id, gbd_round_id, n_draws) {
    ...
    if (rei_id %in% c(142, 96)) exp <- exp[measure_id == 5]
    if (rei_id  == 341) exp <- exp[measure_id %in% c(5,18), ]
    if (rei_id == 122) exp <- exp[modelable_entity_id == 2436]
...
# pull exposure sd
get_exp_sd <- function(rei_id, location_id, year_id, sex_id, gbd_round_id, n_draws) {
    if (rei_id == 95) {
...
get_tmrel <- function(rei_id, location_id, year_id, sex_id, gbd_round_id, n_draws,
                      tmrel_lower, tmrel_upper, ages) {
    ...
    if (inherits(try(tmrel <- get_draws(gbd_id_type = "rei_id", gbd_id = rei_id,
                                              location_id = location_id, year_id = year_id,
                                              sex_id = sex_id, gbd_round_id = gbd_round_id,
                                              source = "tmrel"), silent = TRUE), "try-error")) {
...
# pull RRs
get_rr <- function(rei_id, location_id, year_ids, sex_id, gbd_round_id, n_draws) {
    if (length(location_id) > 1)
        stop("Can only pull RRs for one location at a time")
    rr <- get_draws(gbd_id_type = "rei_id", gbd_id = rei_id, location_id = location_id,
                    sex_id = sex_id, gbd_round_id = gbd_round_id, source = "rr")
    ...
    # most rrs don't vary by year so if the user requests more years than are returned,
