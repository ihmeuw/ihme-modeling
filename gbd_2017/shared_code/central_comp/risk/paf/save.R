save_paf <- function(dt, rid, rei, n_draws, out_dir) {
    ...
    if (pafs_above_1 != 0)
        stop("Some PAF draws (", pafs_above_1, ") in data ",
                "are greater than 1.")
...
save_results_epi <- function(cluster_proj, log_dir, rei, me_id, year_id, measure_id, description, input_dir) {
    system(paste0("qsub -P ", cluster_proj, " -pe multi_slot 40",
                  " -e ", log_dir, "/errors -o ", log_dir, "/output",
                  " -cwd -N save_results_epi_", rei,
                  " utils/python.sh FILEPATH gbd_env",
                  " python utils/epi_save_results.py",
                  " --me_id ", me_id,
...
save_results_risk <- function(cluster_proj, log_dir, rei, me_id, year_id, measure_id,
    ...
    system(paste0("qsub -P ", cluster_proj, " -pe multi_slot 40",
                  ...
                  " utils/python.sh FILEPATH gbd_env",
                  " python utils/risk_save_results.py",
                  " --me_id ", me_id,
                  " --year_id \"", paste(year_id, collapse=" "),
