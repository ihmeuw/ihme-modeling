...
# pull RRs
get_rr <- function(rei_id, location_id, year_id, sex_id, gbd_round_id, n_draws) {
    if (length(location_id) > 1)
        stop("Can only pull RRs for one location at a time")
    rr <- get_draws(gbd_id_type = rep("rei_id", length(rei_id)), gbd_id = rei_id, location_id = location_id,
