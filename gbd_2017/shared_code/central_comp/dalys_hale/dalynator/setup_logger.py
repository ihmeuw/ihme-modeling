...
def create_logger_from_file(logger_name, path="logging.yaml",
    ...
    if os.path.exists(final_path):
        ...
        logging.getLogger(logger_name).info(
            "Creating log config from file {} for name".format(final_path,
                                                               logger_name))
    else:
        # fall back if it can't find the file
        logging.basicConfig(level=default_level)
        logging.getLogger(logger_name).info(
            "Creating basicConfig logger, could not find config file {}"
            .format(final_path))

    logger = logging.getLogger(logger_name)
    _add_aggregator_logger(logger)
    return logger
...
def create_logger_in_memory(name, level, log_file_name):
    ...
    # AND the root logger too - a call to basicConfig sets handlers on the
    # un-named root logger.
    ...
    msg = ("'{}' logger created in memory at {}, the run begins, "
           "release the Unicorns...".format(name, log_file_name))
    logger.info(msg)
    print(msg)
...
def _add_aggregator_logger(master_logger):
    """And make sure that the aggregator logger is also initialized as
    per dalynator logger"""
...
def _add_jobmon_client_logger(master_logger):
    ...
    master_logger.info("Setting jobmon log handlers to same as {}"
                       .format(master_logger.name))
