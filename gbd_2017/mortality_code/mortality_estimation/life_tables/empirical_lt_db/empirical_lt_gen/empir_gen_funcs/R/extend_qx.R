...
extend_qx <- function(empir_lt, hmd_qx_results, by_vars) {
    ...
    ## Start should be the last age that you actually have data for
    ...
    for(seq_age in seq(60, 95, 5)) {
        hmd_qx_results[start == seq_age, logit_qx_start_par := get(paste0("pari_logitqx", seq_age))]
    }

    hmd_qx_results[is.na(ind_age_par) & start == target_age, ind_age_par := 0] # Set regression parameter to 0 if it's the starting age group
