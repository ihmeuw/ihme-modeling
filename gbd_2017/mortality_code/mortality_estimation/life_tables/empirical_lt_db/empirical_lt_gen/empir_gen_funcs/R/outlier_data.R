...
outlier_data <- function(dt, outlier_sheet, completeness_outliers, run_id_5q0_estimate, run_id_45q15_estimate, drop_outliers = T) {
    ...
    completeness_outliers[, outlier_source := "Completeness cutoffs"]
    ...
    outliers_5q0[, outlier_source := "5q0 outlier"]

    outliers_45q15 <- unique(outliers_45q15[outlier == 1, .SD, .SDcols = unique_data_ids])
    outliers_45q15[, outlier_source := "45q15 outlier"]
    ...
    ## Subset outlier sheet to places where apply_outliering == 1, meaning that this is a current outliering that we should do
    ...
    if(nrow(outlier_sheet) > 0) {
        ...
        if(nrow(continuous_outliers) > 0) {
            ...
            continuous_outliers[, outlier_source := "Manual outliering: continuous"]
        ...
        outlier_sheet[, outlier_source := "Manual outliering: specific"]
    ...
    final_outliers <- unique(final_outliers[, .SD, .SDcols = final_unique_ids])
    final_outliers[, outlier := 1]

    dt <- merge(dt, final_outliers, by = final_unique_ids, all.x = T)

    nrow_drops <- nrow(unique(dt[outlier == 1, .SD, .SDcols = final_unique_ids]))
    message("Dropping ", nrow_drops, " lifetables from empirical death data")

    if(drop_outliers == T) {
        dt <- dt[outlier != 1 | is.na(outlier)]
        dt[, c("outlier") := NULL]
    }
