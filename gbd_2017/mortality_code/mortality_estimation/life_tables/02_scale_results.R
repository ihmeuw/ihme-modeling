...
## Set up settings
  ...
  if (Sys.info()[1]=="Windows") {
    ...
    current_year <- as.integer(commandArgs(trailingOnly = T)[2])
    vers_age_sex <- as.integer(commandArgs(trailingOnly = T)[3])
  ...
  source(paste0("mltgeneration/R/recalc_ax.R"))
...
gen_ci_lt <- function(lt, id_vars) {
  lt <- copy(lt) # To avoid modifying the underlying lifetable itself
  ...
  ## Generate 45q15
  lt_45q15 <- lt[age >= 15 & age <= 55, list(qx = 1 - prod(px)), by = id_vars]
  
  ## Generate 5q0
  lt_5q0 <- lt[age >= 0 & age <= 1, list(qx = 1 - prod(px)), by = id_vars]
