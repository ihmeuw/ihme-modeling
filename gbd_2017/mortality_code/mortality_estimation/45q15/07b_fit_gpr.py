...
cc = sys.argv[2]
sexes = ['male','female']
hiv_uncert = int(sys.argv[3])
hsim = int(sys.argv[4])

output_dir = "FILEPATH"
output_gpr_dir = "FILEPATH"
...
for iter in iters:
	for ss in sexes:
		...
		data = pl.csv2rec(parameter_file, missing='NA')

		best_scale = float(data['scale'][(data['best']==1) & (data['ihme_loc_id']==cc) & (data['sex']==ss)][0])
		best_amp2 = float(data['amp2x'][(data['best']==1) & (data['ihme_loc_id']==cc) & (data['sex']==ss)][0]) * data_mse
		...
		if (hiv_uncert == int(1)):
			...
			if (hitercount == 50):
				drawlist_file = "FILEPATH"
				drawlist = pl.csv2rec(drawlist_file, missing='NA')
				...
				sim_file = "FILEPATH"
				pl.rec2csv(all_sim_comp, sim_file)
			elif (hitercount == 1):
