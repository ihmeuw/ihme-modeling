...
library(foreign); library(plyr); library(reshape); library(data.table); library(haven); library(assertable); library(readr)
...
# LDI_version <- get_best_covariate_version(covariate_id = 57, gbd_round_id = gbd_round_id)
LDI <- as.data.frame(fread("FILEPATH"))
LDI <- LDI[, c("location_id", "year_id", "mean_value")]
names(LDI)[names(LDI)=="year_id"] <- "year"
names(LDI)[names(LDI)=="mean_value"] <- "LDI_id"

#3/2/17 creating old andhra pradesh
...
# edu_version <- get_best_covariate_version(covariate_name_short = "education_yrs_pc", gbd_round_id = gbd_round_id)
edu <- as.data.frame(fread("FILEPATH"))
...
merge_completeness <- function(input, gbd_year, ddm_version_id) {
  ...
  output[is.na(vr_complete), vr_complete := F] # mark all other data points not from DDM as vr_incomplete
  return(output)
}
...
raw <- get_mort_outputs(model_name = "45q15",
                        model_type = "data",
                        run_id = data_run_id,
                        outlier_run_id = version_id,
                        location_set_id = 82)

raw <- raw[outlier == 0,]

# TEMPORARILY FILTER OUT ALL CHINA CDC VR DATA
...
raw <- merge_completeness(raw, gbd_year = gbd_year, ddm_version_id = ddm_version_id)
...
#12/28/16 - getting rid of weird data points with year = NA
...
# exposure for certain method/source types cannot be 0 (some exceptions: sibs)
write.csv(data.table(raw[!(method_id %in% c(11, 5)), ]), paste0(output_dir, "/data/test_exposure.csv"), row.names = F)
...
# Force India SRS, China DSP, Botswana, Honduras, Suriname (SUR), Algeria (DZA), Philippines (PHL) to be incomplete
...
# GBD 2017 keeping certain locations at their 2016 type for choosing parameters in spacetime
...
# workarounds for locations introduced in gbd 2017 that don't have parameters
...
# data validation
# prevents the regression from creating -Infs and causing 07a to break
...
# Covariates and 45q15 variables can't be Inf, for covariates that would mean population was 0 or something else was wrong with them
...
write_csv(location_parameter_types, "FILEPATH")
