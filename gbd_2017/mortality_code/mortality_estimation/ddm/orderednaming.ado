...
if (_N > 0) {
	** Drop if no 0 age group
	...
	preserve
	keep if numof0to == keepit0to
	if ("`type'" == "pop"){
		keep ihme_loc_id year sex source_type pop_source pop_nid underlying_pop_nid
	}
	if ("`type'" == "deaths"){
		keep ihme_loc_id year sex source_type deaths_source deaths_nid deaths_underlying_nid
	}
	gen note = "No values for <1 year age group"
	export delim using "FILEPATH/missing_zero_value_`iso3'.csv", replace
	restore
	...
	gen note = "No `type' counts"
	export delim using "FILEPATH/no_values_`iso3'.csv", replace
	restore
	keep if keepit == 1	
...
// If you still have observations after the drops above, then proceed. Else...
if (_N > 0) { 

	capture: drop DATUMUNK DATUMTOT
	...
	gen note = "Discontinuity in age group patterns"
	export delim using "FILEPATH/dropped_`iso3'.csv", replace
	restore
	tab dropit
	drop if dropit == 1
	drop dropit openend
	...
	lookfor precise_year
	return list
	if("`r(varlist)'" == "") {
		merge 1:1 ihme_loc_id year sex `type'_source source_type using "`data'", keepusing(DATUMUNK)
		tab _merge
		drop if _merge == 2
		drop _merge
	}
	else {
		merge 1:1 ihme_loc_id year sex precise_year `type'_source source_type using "`data'", keepusing(DATUMUNK)
		tab _merge
		drop if _merge == 2
		drop _merge
	}

	replace DATUMUNK = 0 if DATUMUNK == .

	lookfor DATUM
	return list
	foreach var of varlist `r(varlist)' {
		replace `var' = `var' + DATUMUNK*(`var'/DATUMTOT)
	}

	drop DATUMUNK DATUMTOT
