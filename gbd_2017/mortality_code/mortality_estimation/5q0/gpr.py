...
def pmodel_func(x, pyear, pmort):
                ...
                #return pl.interp(x,all[:,1],all[:,0])
...
def logit(p):
	#return np.log((p)/(1-p))/np.log(10)
	return np.log((p)/(1-p))

def inv_logit(p):
	#return 1/(1+np.exp(-(np.log(10)*p)))
	return 1/(1+np.exp(-p))
...
def gpmodel(ihme_loc_id,region_name,year,log10_mort,log10_var,category,pyear,pmort,mse,scale,amp2x,predictionyears):
	...
	#create vectors of all years/5q0 values in dataset (this is even more astonishingly inefficient that the previous loop)
	for acat in allcategory:
		for ucat in pl.unique(category):
			if acat == ucat:
