...
# assign zaf rapid mortality report to vr (per Haidong 1/10/14)  Changed back 3/1/14
data['vr'] = 0
...
## For multiple VR systems, I'm not changing this (yet) 5-13-13 - cal
## 3-1-14 changing for multiple vr systems - cal
# Logic is as follows:
#   Loop through each country
#   For each country, loop through each "VR" source (VR == 1)
#   If the country only has VR, label category as VR only
#   If the country has VR and non-VR, check if there is overlap
#   If there is no overlap between the VR source and the non-VR source, label vr_no_overlap
#   If there is overlap,
    ...
    for src in vr_sources:
        ...
        if len(other_years) == 0:
            ...
            if bool(other_year_range & vr_year_range):
                ...
                result = sm.OLS(np.log10(regression_data['q5']), regression_data[['year', 'vr', '_intercept']] ).fit()
...
# VR-SSA from South Africa is a no-overlap VR point. Assign to VR-biased for now, per Haidong
data.loc[(data['ihme_loc_id'] == "ZAF") & (data['nid'] == 106649), 'category'] = "vr_biased"

# VR is not complete in KGZ pre-2007
...
# Manually mark the VR in Iran unbiased for post 2015
