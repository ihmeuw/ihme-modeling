...
apply_mac <- function(dir, grouping.categories, all.women, subset=NULL, filetag=NULL, uncertainty=FALSE, n.sims=1, fund.uncert=FALSE, codedir = "FILEPATH" ) {
...
# Load MAC model parameters
  startdir <- getwd()
  setwd("FILEPATH")
  source("FILEPATH/02a. functions - RE model with uncertainty.r")
  

# load the model parameters

  ## output.mac
  output.mac.varnames <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.mac/output.mac.varnames.csv", sep=""),stringsAsFactors=FALSE)
  output.mac.fe <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.mac/output.mac.fe.csv", sep=""),stringsAsFactors=FALSE)
  output.mac.iso_re<- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.mac/output.mac.iso_re.csv", sep=""),stringsAsFactors=FALSE)
  output.mac.reg_re <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.mac/output.mac.reg_re.csv", sep=""),stringsAsFactors=FALSE)
  output.mac.errors <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.mac/output.mac.errors.csv", sep=""),stringsAsFactors=FALSE)
  
  ## output.mac.reftime
  output.mac.reftime.varnames <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.mac.reftime/output.mac.reftime.varnames.csv", sep=""))
  output.mac.reftime.fe <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.mac.reftime/output.mac.reftime.fe.csv", sep=""))
