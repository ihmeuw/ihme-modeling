...
# Load MAP model parameters
  ...
  load(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/distributions - age.rdata", sep=""))
  setwd("FILEPATH")
    
# bring in the coefficients for output.map

  output.map.varnames <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.map/output.map.varnames.csv", sep=""),stringsAsFactors=FALSE)
  output.map.fe <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.map/output.map.fe.csv", sep=""), stringsAsFactors=FALSE)
  output.map.iso_re<- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.map/output.map.iso_re.csv", sep=""), stringsAsFactors=FALSE)
  output.map.reg_re <- read.csv(paste("FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.map/output.map.reg_re.csv", sep=""), stringsAsFactors=FALSE)
  output.map.errors <- read.csv(paste("/FILEPATH", if (all.women) "All women" else "Ever-married women", "/fitted_models/output.map/output.map.errors.csv", sep=""), stringsAsFactors=FALSE)
...
# Load MAP data
  ...
  for(x in 1:length(data)) data[[x]] <- data[[x]][!is.na(data[[x]]$gbdregion),] 
