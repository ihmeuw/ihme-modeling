...
// Obtain list of all countries to combine: 
	quietly { 
	clear 
	local all_files: dir "FILEPATH/`sex'/" files "*raw_`sex'.dta", respectcase
	gen filename = "" 
	...
	if(`restart' == 1) {
		...
		foreach fileheader of local fileheaders {
			preserve
			capture use "FILEPATH/`fileheader' - `sex' - `period'.dta", clear
			foreach country in local drop_me {
				drop if country == "`country'"
			}
			saveold "FILEPATH/`fileheader' - `sex' - `period'.dta", replace
			restore
		}
	}
	...
	foreach country of local countries {
		...
		foreach survey of local surveys {
			...
			use "FILEPATH/`sex'/`survey'_`sex'", clear
			...
			if ("`sex'" == "both") { // find the range of the data 
				...
				preserve
				use "FILEPATH/both/`survey'_both", clear
				replace year = 1900 + (year-1)/12		
				summ year
				local max_year_real = r(max)
				local min_year_real = r(min)
			...
			if ("`sex'" == "both") { 
				...
				preserve
				use "FILEPATH/drop_years.dta", clear
		...
		if (`neonatal' == 2) { 
			...
			sort country year
			saveold "FILEPATH/q5 - `sex' - `period'.dta", replace
			restore
			** edit source variable
			tempfile all_data
			save `all_data', replace
			use "FILEPATH/q5 - `sex' - `period'.dta", clear
			...
			if("`sex'"=="both" & `save_GBD' == 1) {
				...
				drop source2
				order iso3 source
				cd "FILEPATH/direct 5q0 for GBD - survey"
