...
// Loop through files to create survey-specific datasets 
	...
	foreach svy of local surveys {
		...
		use "FILEPATH/`sex'/`svy'_raw_`sex'.dta", clear
	...
	if (`neonatal' == 2) { 
		...
		saveold "FILEPATH/`sex'/`svy'_`sex'.dta", replace
	}
...
// Loop through countries to create country-specific datasets 
	...
	foreach country of local countries {
		...
		forvalues i=1(1)`max' { 
			noi di "       svy `i' of `max'"
			local count = `count' + 1 
			use "FILEPATH/`sex'/`svy`i''_`sex'.dta", clear 
			replace psu = (`i'*10^6) + psu // make psu unique across surveys
			if (`count' > 1) append using `temp'
			save `temp', replace
		} 
		saveold "FILEPATH/`sex'/`country'_`sex'.dta", replace
	}
