...
resid_space_time <- function(data, region=NULL, lambda=.6, zeta=.99, min_year=1950, max_year=2017, print=T, use_super_regs=F, sex_mod=F, sex_input=NULL, age_input=NULL, params=NULL) {
  ...
  super_regs <- get_locations(gbd_year=2017)
    ...
    other.resids <- (sum(!in.country)>0)
      ...
      for (yy in min_year:max_year) {
        ...
        combined <- linear*data_density[cc] + constant*(1-data_density[cc])
          # the weight = w[count] part of the following doesn't work for >1 region; I'm guessing it doesn't matter
