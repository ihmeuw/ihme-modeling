...
# Get the 5q0 output directory
output_5q0_dir <- "FILEPATH"

# Get the data density file
data_density_file <- "FILEPATH"

# Get the DDM output directory
...
## Reading in files
d <- data.table(read.dta("FILEPATH")) ## data file
d <- d[!is.na(q_u5)]
births <- fread("FILEPATH")
child <- fread("FILEPATH")
child <- child[estimate_stage_id == 3, ]
vr_list <- fread("FILEPATH")


location_data <- data.table(get_locations(level="estimate", gbd_year = 2017))

compiled_sex_stage_1_file <- "FILEPATH"
s1 <- fread(compiled_sex_stage_1_file)
...
for_gpr <- copy(st_data) ## save the empirical sex ratio data for later gpr use
...
write.csv(s2_est, "FILEPATH", row.names=F)
