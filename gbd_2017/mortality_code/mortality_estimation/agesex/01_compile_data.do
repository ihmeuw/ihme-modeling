...
** *************************
    ...
    ** Get births
    // Get estimates from shared function
    import delimited "FILEPATH", clear
...
** Compile CBH data
** *************************
    local surveys: dir "FILEPATH", respectcase
    ...
    ** For new CBH preps, use automated CBH tracker to bring in sources
    import delimited "FILEPATH", clear
...
** prep COD VR file
    use "FILEPATH", clear
    gen subdiv = ""  // Don't use this column anymore but the script expects it

    // convert long age_group_id format to wide deaths (who) format:
    ** deaths will hold the number of deaths
    ** deaths_age will represent WHO age group
    gen deaths_age = .
    ...
    ** NOTE "deaths" column is the number of deaths, the column "deaths_age" holds a WHO age group identifier
    reshape wide deaths, i(NID source location_id iso3 year sex subdiv) j(deaths_age)

    collapse (sum) deaths*, by(iso3 location_id year source subdiv sex)
    tempfile cod_vr_col
    save `cod_vr_col'

    keep iso3 location_id sex subdiv source year deaths3 deaths91 deaths93 deaths94
    ...
    gen deaths2 = deaths91 + deaths93 + deaths94
...
** load demographics VR data
    ...
    ** merge better
    ...
    //replace source_type = "VR2" if ihme_loc_id == "KOR" & year == 2010
...
** Label exclusions
    label define exclude 0 "FILEPATH"
...
** Mark data types
** *************************
    gen age_type = "FILEPATH" if q_enn == . & q_lnn == . & q_pnn == .
    replace age_type = "FILEPATH" if q_enn == . & q_lnn == . & q_nn != . & q_pnn != .
    replace age_type = "FILEPATH" if q_enn != . & q_lnn !=. & q_pnn != .
    count if age_type == ""
    assert `r(N)' == 0
    ** assert q_inf != . & q_ch != . if age_type == "FILEPATH"
    assert q_nn != . & q_pnn != . & q_ch != . if age_type == "FILEPATH"
    ** assert q_enn != . & q_lnn != . & q_pnn != . & q_ch != . if age_type == "FILEPATH"
...
** *************************
    ...
    keep if comp >= .95
    // why would the comp variable be dropped here?
    drop comp
    replace year = year + .5
    tempfile adult_comp
    ...
    ** Drop all ENN sources for ALB
    replace exclude_enn = 8 if ihme_loc_id == "ALB"
    ** Drop all LNN sources for BHS
