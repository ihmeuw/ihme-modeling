...
library(data.table)
library(openxlsx, lib.loc = paste0(root, "FILEPATH"))

## OBJECTS
temp_dir <- paste0(root, "FILEPATH")
functions_dir <- paste0(root, "FILEPATH")
age_table_code <- paste0(temp_dir, "age_table.R")
date <- Sys.Date()
date <- gsub("-", "_", date)
...
age_split <- function(bundle, gbd_id, acause, age, region_pattern, location_pattern_id){
  ...
  write.xlsx(total, paste0(root, "FILEPATH", "age_split.xlsx"), 
...
age_sex_split <- function(bundle, acause){
  ...
  ## SAVE DATA
  write.xlsx(total, paste0(root, "FILEPATH","age_sex_split.xlsx"), 
