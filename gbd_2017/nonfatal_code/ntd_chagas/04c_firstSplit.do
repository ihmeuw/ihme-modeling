...
\******************************************************************************/
    ...
    * PULL IN DRAWS FROM DISMOD MODELS FOR INCIDENCE AND PREVALENVCE *
      if `endemic'==0 {
	    use FILEPATH/migrantPrev_`location'.dta, clear
	    ...
	    get_draws, gbd_id_type(modelable_entity_id) gbd_id(`meid_master') source(ADDRESS) location_id(`location') age_group_id(`ages') measure_id(5) status(best) clear
        rename draw_* prev_*
		}
	  ...
	  joinby age_group_id sex_id using FILEPATH/chronicPr_`location'.dta
	...
	* PERFORM DRAW-LEVEL CALCULATIONS  *
	 ...
	 foreach meid of local meids {
       export delimited location_id year_id sex_id age_group_id measure_id modelable_entity_id draw_* using FILEPATH/`location'.csv if modelable_entity_id==`meid', replace
	   }
  ...
  get_draws, gbd_id_type(modelable_entity_id) gbd_id(`meid_master') source(ADDRESS) location_id(`location') age_group_id(`ages') measure_id(6) status(best) clear
	...
	export delimited location_id year_id sex_id age_group_id modelable_entity_id measure_id draw_* using FILEPATH/`location'.csv, replace
