...
df <- as.data.table(read.dta13("FILEPATH"))
...
## issue with 1990 10to14 data. We replace the row with 1990 5to9 data
...
df <- do.call("rbind", list(df, df_1995, df_2000, df_2017))
...
rural <- get_location_metadata(location_set_id = 35, gbd_round_id = 5)
...
write.csv(df, paste0(output_directory, my_worm, "_prop.csv"), row.names = F)
