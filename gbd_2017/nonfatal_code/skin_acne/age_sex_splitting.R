...
age_table_code <- paste0(temp_dir, FILEPATH) ## SWICH TO WHERE YOU KEEP THIS CODE
date <- Sys.Date()
date <- gsub("-", "_", date)
...
all_age <- as.data.table(read.xlsx(paste0(j_root, FILEPATH)))
region_pattern <- T
...
age_split <- function(bundle, gbd_id, acause, age, region_pattern, location_pattern_id){
  ...
  all_age <- all_age[measure %in% c("prevalence", "incidence"),] #get prevalence and incidence only
  ...
  all_age <- all_age[(age_end-age_start)>20,] #gap 
  all_age <- all_age[!mean ==0, ] ##don't split points with zero prevalence
  all_age[, sex_id := sex] #assiging this sex_id column the value in the sex column 
  all_age[sex_id=="Both", sex_id :=3]
  all_age[sex_id=="Female", sex_id := 2]
  all_age[sex_id=="Male", sex_id :=1]
  all_age[, sex_id := as.integer(sex_id)]
  all_age[measure == "prevalence", measure_id := 5] #assigning this measure_id column the measure id 5 for prevalence
  all_age[measure == "incidence", measure_id := 6]
  all_age[, year_id := year_start] ##so that can merge on year later, creating year_id column and assigning it the value in year start
  ...
  if (region_pattern == T){
    ...
    split <- merge(split, super_region, by = "location_id") #adding super_region_id to data
  ...
  age_pattern <- as.data.table(get_draws(gbd_id_type = "modelable_entity_id", gbd_id = 2274, 
                                         measure_id = c(5, 6), location_id = locations, source = "epi",
                                         version_id = 261632, sex_id = c(1,2), gbd_round_id = 5,
  ...
  write.xlsx(total, paste0(j_root, FILEPATH))
  
  write.xlsx(total, paste0(j_root, "WORK/12_bundle/", acause, "/", bundle, "/03_review/02_upload/", date, "_age_split.xlsx"), 
             sheetName = "extraction") 
}


df <- as.data.table(read.xlsx(paste0(temp_dir, "Acne/pre_age_sex.xlsx")))
...
age_sex_split <- function(bundle, acause){
  ...
  ## SAVE DATA
  write.xlsx(total, paste0(FILEPATH), 
