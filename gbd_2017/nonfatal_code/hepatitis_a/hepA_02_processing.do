// Purpose: Pull seroprevalence from DisMod, and convert to incidence, then
//	        split by severity	
// Input:   age_mid.csv
...
\******************************************************************************/
    ...
    * PULL IN DRAWS FROM DISMOD MODELS FOR ANTI-HAV PREVALENVCE *
      interpolate, gbd_id_type(modelable_entity_id) gbd_id(`meid') location_id(`location') age_group_id(`ages') measure_id(5) source("epi") status(best) reporting_year_start(1980) reporting_year_end(2017) clear
      rename draw_* prev_*
      ...
      * load table of age_group_ids and corresponding age_mids
      import delimited using "`j'/FILEPATH/age_mid.csv", clear
