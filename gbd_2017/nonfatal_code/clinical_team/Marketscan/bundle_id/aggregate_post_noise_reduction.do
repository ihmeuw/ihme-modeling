...
import delimited "$prefix/FILEPATH") varn(1) case(preserve) clear
...
if `bundle_id' == 79 | `bundle_id' == 646 | `bundle_id' == 74 | `bundle_id' == 75 | `bundle_id' == 423 | `bundle_id' == 77 | `bundle_id' == 422 | `bundle_id' == 667 | `bundle_id' == 76 {
	preserve
		...
		quiet run "$prefix/FILEPATH"
		get_covariate_estimates, covariate_id(13) sex_id("2") year_id("2000 2010 2012") clear
...
if `inp_only' == 1 {
	...
	order source_type
	export excel "`out_dir'/FILEPATH", firstrow(var) sheet("extraction") replace
...
if `inp_only' == 0 {
	...
	export excel "`out_dir'/FILEPATH") replace
	cap mkdir "$prefix/FILEPATH"
	saveold "$prefix/FILEPATH", replace
}
