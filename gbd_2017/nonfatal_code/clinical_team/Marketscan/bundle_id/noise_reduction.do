...
if "`resubmit'" != "yes" {
    log using "`cause_log_dir'/FILEPATH", replace
}    
else if "`resubmit'" == "yes" log using "`cause_log_dir'/FILEPATH", replace
...
** Get age sex restrictions
    clear
    set obs 1
    gen acause = "imp_or_env"
    gen male = 1
    gen female = 1
    gen yll_age_start = 0
    gen yll_age_end = 125
    tempfile Marketscan_placeholder
    save `Marketscan_placeholder', replace
    do "/FILEPATH"
    if strmatch("$data_name", "_Marketscan*") {
        ...
        replace yll_age_end = 125 if acause == "gyne_other"
        // merging on acause NAME so make sure they match
        replace acause = "digest_gastritis" if acause == "digest_gastrititis"
        // this appears to have been added to cod cause hierarchy since last run
    }
    keep acause male female yll_age_*
    append using "/FILEPATH"
    append using `Marketscan_placeholder'
    ...
    **  re-enforce age restrictions
        ...
        ** fix Marketscan ages
        if strmatch("$data_name", "_Marketscan*") {
            replace yll_age_start = 0 if yll_age_start < 1
            replace yll_age_end = 150 if yll_age_end == 80
        }
    ...
    ** Identify population
        if !strmatch("$data_name", "_Marketscan*") merge m:1 iso3 using "$temp_dir/FILEPATH", keep(1 3) nogen
    ...
    ** throw modelable_entity_id back on acause for Marketscan
    if strmatch("$data_name", "_Marketscan*") {
        replace acause = acause + "#" + string(modelable_entity_id)
