...
cap mkdir "`output_dir'FILEPATH"
log using "`output_dir'FILEPATH", replace name(log)
...
// use a call to shared cause Db to get acause name for bundles
quiet run "$prefix/FILEPATH"
	create_connection_string, server("server") database("db")
...
quiet run "$prefix/FILEPATH"
	create_connection_string, server(SERVER) database(DATABASE)
	local conn_string = r(conn_string)
	odbc load, exec("QUERY") `conn_string' clear
	tempfile bundle_ids
...
foreach prev_cat in all all_inp {
	...
	foreach dataset in ccae2000 ccae2010 ccae2012 mdcr2000 mdcr2010 mdcr2012 {
		...
		local files: dir "`temp_folder'/FILEPATH"
		foreach file of local files {
			append using "`temp_folder'/FILEPATH"
		}
		...
		preserve
			...
			odbc load, exec("QUERY") `conn_string' clear
				//merge m:1 cause_metadata_type_id using `meta_ids', keep(3) nogenquiet
...
drop if age_start == 64 // overlap when enrollees switch coverage
