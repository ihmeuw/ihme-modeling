...
//  Local for the directory where all the COD data are stored
    global in_dir "/FILEPATH"

// Local for the output directory
    global out_dir "$in_dir/FILEPATH"
        
// program directory
    global prog_dir "$repo_dir/FILEPATH"
...
// prepare function to get iso3s that are prepped subnationally
    quietly run "$prog_dir/FILEPATH"
// get subnat iso3s
    get_subnational_iso3s, location_set_id(`location_set_id') gbd_round_id(`gbd_round_id')
    local subnat_iso3s `r(subnat_iso3s)'
...
// Refresh temp directory
    ...
    capture mkdir "$temp_dir/FILEPATH"
    !rm -rf "$temp_dir/FILEPATH"
    capture mkdir "$temp_dir/FILEPATH"
    
// read in data
    if `test_single_cause' == 1 {
        use "$out_dir/FILEPATH", clear
    }
    else {
        use "$out_dir/FILEPATH", clear
    }
...
// BECAUSE OF LOW SAMPLE SIZE OR LOW DEATH COUNTS WE HAVE A GREAT LEVEL OF STOCHASTIC VOLATILITY FOR SOME CAUSE-DEMOGRAPHICS. THIS CAUSES HUGE FLEXUATIONS IN THE TREND WHICH LEADS TO
    ...
    // For the China data-series we need to prep them together (plus Hong Kong and Macau)
        if inlist("$source", "China_1991_2002", "China_2004_2012") {
            if "$source" == "China_1991_2002" local check_files "$in_dir/FILEPATH"
            if "$source" == "China_2004_2012" local check_files "$in_dir/FILEPATH"
            local check_files "`check_files' $in_dir/FILEPATH"
        ...
        if (`VR'/`r(N)'==1) | inlist("$source", "Cancer_Registry", "Other_Maternal", "Mexico_BIRMM", "Maternal_report", "SUSENAS", "China_MMS_1996_2005", "China_MMS_2006_2012", "China_Child_1996_2012", "China_1991_2002") | strmatch("$source", "ICD*") {
            preserve
                ...
                ** save population of national level for subnat iso3 too 
                ...
                append using `nats'
                save "$temp_dir/FILEPATH", replace
            restore
        }
    ...
    // For the VR data-series we need to prep them together (get populations of the countries in this source prior to doing so)
        ...
        foreach source of local sources {
            capture confirm file "$in_dir/FILEPATH"
            local round = 1
            while _rc == 601 {
                local time = `round' * 5
    ...
    // Run noise reduction
            ...
            ** store a macro of the national iso3s that got added from append
            ...
            foreach cause of local causes {
                // Keep only the cause we are interested in
                    ...
                    if `r(N)'>6 {
                        // Submit job
                            !qsub -pe multi_slot 4 -l mem_free=8g -N "CoD_11_${source}_`cause'" "$prog_dir/FILEPATH" "$source $timestamp $username `cause'"
                         // Submit check
                             sleep 1000
                    }
                    else if `r(N)' <= 6 {
                        preserve
                            ...
                            save "$temp_dir/FILEPATH", replace
                            capture saveold "$temp_dir/FILEPATH", replace
                        restore
                    }
            }
        ...
        // Loop through and append regression results back together
            ...
            foreach cause of local causes {
                // Check for file
                    capture confirm file "$temp_dir/FILEPATH"
                    if _rc == 0 {
                        display "FOUND!"
                    }
                    while _rc != 0 {
                    ...
                    capture append using "$temp_dir/FILEPATH"
                    if _rc != 0 {
                        sleep 15000
                        append using "$temp_dir/FILEPATH"
                    }
            }
        ...
        // Prepare metadata and covariates
            preserve
                ...
                ** Site-specific PfPR
                ...
                save `sspfpr', replace
                ** National PfPR
                odbc load, exec("QUERY") dsn(prodcov) clear
                replace location_id = . if type == "admin0"
                replace iso3 = substr(iso3,1,3)
                drop type
                tempfile natpfpr
                ...
                ** Malaria geographic groupings
                insheet using "$h/FILEPATH", comma names clear
                keep iso3 group
                duplicates drop
                    // *********************************************************************
            ...
            foreach acause of local acauses  {
                ...
                foreach group of local `acause'_groups {
                    ...
                    ** Submit noise reduction job
                    !qsub -P proj_hospital -pe multi_slot 4 -l mem_free=8g -N "CoD_11va_${source}_`acause'_`group'" "$prog_dir/FILEPATH" "$source $temp_dir $timestamp `acause' `group'"
                    restore
                }
            }
