...
def get_data(inp_only):
    maps = pd.read_csv("{FILEPATH}/clean_map.csv")

    root = '{FILEPATH}/'

    if inp_only:
        inc = pd.read_stata(root + 'TWN_NATIONAL_HEALTH_INSURANCE_CLAIMS_2016_IP105I_Y2018M05D10.DTA')
        prev = pd.read_stata(root + 'TWN_NATIONAL_HEALTH_INSURANCE_CLAIMS_2016_IP105P_Y2018M05D10.DTA')
    else:
        inc = pd.read_stata(root + 'TWN_NATIONAL_HEALTH_INSURANCE_CLAIMS_2016_OIP105I_Y2018M05D10.DTA')
        prev = pd.read_stata(root + 'TWN_NATIONAL_HEALTH_INSURANCE_CLAIMS_2016_OIP105P_Y2018M05D10.DTA')
...
def extract_age(df):
    ...
    df.rename(columns={0: 'age_start', 1: 'age_end', 2: 'single'}, inplace=True)
...
def get_sample(df):
    ...
    # pop = get_population(year_id = df.year_id.unique().tolist(), location_id=8,
...
def prep_final_formatting(df, nid_dict):
    """
    the marketscan process in stata outputs 2 files, one to actually noise reduce and one which
    I think just sits there until we're about to write then it gets merged on
    """
...
def apply_bundle_restrictions(df, col_to_restrict, drop_restricted=True):
    ...
    # takes like 9 minutes to run from the print statement above... why?
...
if __name__ == "__main__":
    ...
    inp_only = False # Run twice, once with True the next with False
    prev, inc, bundles, maps = get_data(inp_only)
    ...
    # what to do with stuff not in the map? drop for now
    ...
    df.to_stata(path + "{FILEPATH}/{}_all_twn_bundles.dta".format(today), 
               write_index=False)

    for each in df.acause.unique():
        tmp = df[df.acause == each].copy()
        bundle_id = tmp.acause.str.split("#").iloc[0][1]
        tmp.to_stata(path + "intermediate/{}.dta".format(bundle_id), write_index=False)
    print("Finished writing intermediate data, beginning on formatted...")
    # write df2 to the other place
    df2.to_stata(path + "{FILEPATH}/{}_all_twn_bundles.dta".format(today), write_index=False)
