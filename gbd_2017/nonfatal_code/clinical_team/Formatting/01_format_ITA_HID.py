...
def read_raw_data(write_data=False):
    ...
    for year in years:
        ...
        if year == 2007:
            ...
            fpath = "FILEPATH/gbd_2007.sas7bdat"
            df = pd.read_sas(fpath)
        else:
            # read from stata
            fpath = r"FILEPATH/"\
                    "gbd_{}.dta".format(year)
        ...
        try:
            ideal_keep = ['sesso', 'eta', 'reg_ric', 'regric', 'gg_deg',
    ...
    if write_data:
        ...
        write_hosp_file(df, "/FILEPATH/ITA_all_years.H5")
    return df_list
...
for year in years:
    ...
    # start = time.time()
    if year == 2007:
        fpath = "/FILEPATH/gbd 2005-2016 sas7bdat files/gbd_2007.sas7bdat"
        df = pd.read_sas(fpath)
    else:
        # read from stata
        fpath = r"FILEPATH/"\
    ...
    df_wide.to_stata(r"FILEPATH/ITA_HID_{}.dta".format(year))
    write_path = r"FILEPATH/ITA_HID_{}.H5".format(year)
...
print(df_agg.shape)
# # Saving the file
write_path = "FILEPATH/formatted_ITA_HID.H5"
