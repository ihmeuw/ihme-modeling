	...
	** ****************************************************************
		...
		** ****************************************************************
			...
			// Temp folder
				...
				local temp_folder "{FILEPATH}/`data_name'"
...
** **************************************************************************
	...
	// Create jobs to split data sets into temporary files which can then be aggregated
		foreach n of numlist `year_start'/`year_end' {
			...
			foreach file of local hcup_core`n' {
				...
				// Submit Jobs
					!qsub -pe multi_slot 4 -l mem_free=8G -o FILEPATH -e FILEPATH -N "HOSPITAL_00_`data_source'_`file'" "$prefix{FILEPATH}/01_format_`data_name'_individual.do" "`hcup_file_pathway' `hcup_save_pathway' `file'"
			}
		}
	...
	// Append all datasets
		...
		foreach n of numlist `year_start'/`year_end' {
			...
			foreach file of local hcup_core`n' {
				...
				capture append using "`temp_folder'/`file'", force
				if _rc == 601 {
					sleep 60000
	...
	// SAVE
		compress
		save "`output_folder'/formatted_USA_HCUP_SID_04.dta", replace
		save "`archive_folder'/formatted_USA_HCUP_SID_04_`today'.dta", replace
