...
df = pd.read_hdf(r"FILEPATH/" +
                r"PHIL_CLAIMS_CONCAT.H5", key='df')
print("Data read in")
...
if sys.version[0] == '3':
   df.cc_list = df.cc_list.map(lambda x: list(filter(None, x)))  # py 3
if sys.version[0] == '2':
   df.cc_list = df.cc_list.map(lambda x: filter(None, x))  # py 2
...
for df in df_list:
    ...
    wide.to_stata(r"FILEPATH/PHL_HICC_{}.dta".format(split_int))
    del wide
...
# Saving the file
write_path = "FILEPATH/formatted_PHL_HICC.H5"
