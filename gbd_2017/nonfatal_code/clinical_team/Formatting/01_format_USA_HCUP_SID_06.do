	...
	// Create jobs to split data sets into temporary files which can then be aggregated
		foreach n of numlist `year_start'/`year_end' {
			...
			foreach file of local hcup_core`n' {
				...
				// Submit Jobs
					!qsub -pe multi_slot 4 -l mem_free=8G -o FILEPATH -e FILEPATH -N "/FILEPATH/01_format_`data_name'_individual.do" "`hcup_file_pathway' `hcup_save_pathway' `file'"
			}
		}
...
// drop unneeded cols
	...
	// SAVE
		compress
		save "`output_folder'/formatted_USA_HCUP_SID_06.dta", replace
		save "`archive_folder'/formatted_USA_HCUP_SID_06_`today'.dta", replace
