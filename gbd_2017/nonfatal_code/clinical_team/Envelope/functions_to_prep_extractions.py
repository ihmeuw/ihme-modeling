...
## Purpose: formula was taken from docs
## calc standard erros
class calc_se(object):
    ...
    def calc_se(self):
        ...
        for i in['lower', 'upper', 'mean', 'standard_error', 'sample_size', 'age_start', 'age_end']:
            ...
            self.df[i] = map(np.float, self.df[i])
...
## TODO: Make it more extensible and use p.Map to increase speed... I was having some issues with it before
class compile_survey_data(object):
    ...
    def compile_survey_data(self,path,
        ...
        def read_in_data(input_file):
            ...
            col = int_stuff.keys()
            int_stuff = pd.DataFrame(zip(*[int_stuff.values()]*len(temp))).T
        ...
        file_type='num_visit.csv'
        num_visit= num_visit.append(map(read_in_data, files))
        ...
        fraction= fraction.append(map(read_in_data, files))
        ...
        fraction['mean']= 1- np.array(map(np.exp,-rate*fraction['recall_adj'].values))
...
class overlap(object):
    ...
    def gen_overlap(self,window, df1, df2):
        ...
        for i in data.keys():
            temp = data[i]
            temp['year_start'] = map(np.float, temp['year_start'])
            ...
            for j in ['age_start', 'age_end',  'location_id', 'year_round']:
                temp[j] = map(np.float, temp[j])
            temp['sex'] = map(str, temp['sex'])
            data[i]=temp.copy()
        df1 = data['df1']
        df2 = data['df2']
...
class interp(object):
    ...
    def __my_interp__(self, x):
        ...
        new_mean = map(np.log, x['mean'])
        f_mean =interp1d(x['year_id'], new_mean)
        mean_val = map(np.exp, f_mean(np.arange(1990,2017)))

        new_se = map(np.log, x['se'])
        f_se =interp1d(x['year_id'], new_se)
        se_val = map(np.exp, f_se(np.arange(1990,2017)))
...
class compile_cw_fraction_to_avg(object):
    ...
    def compile_data(self, path):
        ...
        ## read in the data
        p= Pool(5)## this seems like the right number.....
        collect=pd.DataFrame()
        files = [ x for x in os.listdir(path) if 'num_visit_ever.csv' in x]

        collect= collect.append(map(read_in_data, files))
