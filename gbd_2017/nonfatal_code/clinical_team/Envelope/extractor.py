...
## could probably be done with os.path.norm pr something like that
def change_paths(path, new_root="j", old_root= "J"):
    new_path= os.path.normpath(path )
    new_path =new_path.replace("\\", "/")
    new_path =new_path.replace(old_root,new_root)
    return new_path
...
class extractor(object):
    ...
    def merge_data_4(self):
        ...
        self.all_data.columns = map(str, self.all_data.columns)
        ...
        for i in np.arange(1, len(self.reader['filepaths'])):
            ...
            self.survey[str(i)].columns = map(str, self.survey[str(i)].columns)
    ...
    def __remove_na__(self,  varname, b,miss_opt=None, yes_opt=None, no_opt =None, ret=True):
        ...
        self.all_data[varname] = map(str, self.all_data[varname].values)
        ...
        try:
            ...
            self.all_data[varname] = map(np.float, self.all_data[varname].values)

            if miss_opt is not None:
                float_miss_opt = [x for x in miss_opt if ('nan'==x) | (digit_check_2(x)) ]
                miss_opt =map(np.float, float_miss_opt)
                self.all_data.loc[self.all_data[varname].isin(miss_opt),varname] = -99567
            if yes_opt is not None:yes_opt = map(np.float, yes_opt )
            if no_opt is not None: no_opt =map(np.float, no_opt)
    ...
    def __yes_no__(self, reader_var):
        ...
        for t in np.arange( len(self.reader[b])):
            ...
            if len(self.reader[b])==1:
                ...
                ## Fill with option
                if b+'_no' in self.reader.keys(): no_opt = self.reader[b+'_no']
                else:no_opt =[np.nan]
                if b+'_miss' in self.reader.keys(): miss_opt = self.reader[b+'_miss']
                else:miss_opt =[np.nan]
                if b+'_yes' in self.reader.keys(): yes_opt = self.reader[b+'_yes']
                else: yes_opt =[np.nan]


            if len(self.reader[b])>1:
                ## Fill with option
                if b+'_no' in self.reader.keys(): no_opt = self.reader[b+'_no'][t]
                else:no_opt =[np.nan]
                if b+'_miss' in self.reader.keys(): miss_opt = self.reader[b+'_miss'][t]
                else:miss_opt =[np.nan]
                if b+'_yes' in self.reader.keys(): yes_opt = self.reader[b+'_yes'][t]
                else: yes_opt =[np.nan]
    ...
    def __cont__(self, reader_val):
        ...
        for t in np.arange(len(self.reader[b])):
            varname = self.reader[b][t]
            # convert to stirng
            if b+'_miss' in self.reader.keys():
    ...
    def get_gender_5(self):
        ...
        if 'nan' not in map(str,self.reader['sex']):
...
#
            ...
            self.all_data[self.reader['age_birth_yr'][0]] = map(np.float, self.all_data[self.reader['age_birth_yr'][0]])
        ...
        if 'nan' not in map(str, self.reader['age_yr']):
    ...
    def get_utilization_7(self):
        ...
        if 'nan' not in map(str, self.reader['num_visit']):
            self.__cont__('num_visit')

            self.collect.loc[self.collect.num_visit>40, 'num_visit'] =np.nan


        if 'nan' not in map(str, self.reader['num_bed_days']):
            self.__cont__('num_bed_days')

        ## heck gated
        if 'nan' not in map(str, self.reader['util_var_gated']):
        ...
        if 'nan' not in  map(str, self.reader['util_var']):
    ...
    def get_surveyset_9(self):
        ...
        # step #1
        if 'nan' in map(str, self.reader['pweight']):
            self.collect['pweight'] =1
        else:
            if len(self.collect) < 200000: 
                self.__cont__('pweight')
            else:
                self.collect['pweight'] = map(np.float,  self.all_data[self.reader['pweight'][0]].values)
        ...
        for i in ['psu', 'strata']:
            if ('nan' not in map(str, self.reader[i])) & (len(self.reader[i])>1):
                ...
                self.reader[i] = map(str ,[i+'_combine'])

        # step #2
        if 'nan' in map(str,self.reader['psu']):
        ...
        # step #3
        if 'nan' in map(str, self.reader['strata']):
