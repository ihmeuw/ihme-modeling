...
def get_age_groups():
    ...
    age_groups = db_tools.ezfuncs.query(QUERY)
    return age_groups
...
def prep_age_aggregate_to_detail_map(age_group_table, good_age_group_ids):
    ...
    detail_age_map = pd.concat(age_maps, ignore_index=True)
...
# make shared function
def get_age_weights(distribution_set_version_id=15, level_of_analysis='cause_id'):
    """Get age weights from the database"""

    if level_of_analysis == 'nonfatal_cause_name':
        # this assumes that if you want to age/sex split at nonfatal_cause_name
...
def separate_detailed_from_aggregate(df, good_age_group_ids,
    ...
    Args:
        ...
        unsplittable_ages (list):
            list of ages that can't be split
        unsplittable_sexes (list):
            list of sexes that can't be split
...
def prep_child_to_available_parent_map(cause_set_id, gbd_round_id,
    ...
    "available": nonfatal_cause_name/cause_id are present in the weights
    dataframe

    Arguments:
        cause_set_id (int): from table in the database
        gbd_round_id (int): from table in the database
...
def prep_cause_to_weight_cause_map(cause_set_id, gbd_round_id, weight_causes,
    ...
    if level_of_analysis == 'cause_id':
        ...
        weight_cause_map = pd.DataFrame({'nonfatal_cause_name': weight_causes,
                                         'weight_nonfatal_cause_name': weight_causes})
...
def prep_split_df(df, pop_df, weight_df, age_detail_map,
    ...
    # now add the cause id that will be merged on age weights
    ...
    # for the nonfatal_cause_name level, weight_df has column named
    # 'nonfatal_cause_name'
...
def calculate_detail_val(df, id_cols, fix_gbd2016_mistake=True,
    ...
    if fix_gbd2016_mistake:
        ...
        # restricted in the aggregate group.
...
def split_age_sex(df, id_cols, value_column='deaths', fix_gbd2016_mistake=True,
                  gbd_round_id=5, cause_set_id=4,
    ...
    Arguments:
        ...
        value_column (str): must be a column in df that contains values to be
            split
        fix_gbd2016_mistake (bool): team to keep
    ...
    if level_of_analysis == 'nonfatal_cause_name':
        ...
        good_age_group_ids.append(28)  # hosp team uses 0-1 years old
    ...
    weight_df = get_age_weights(level_of_analysis=level_of_analysis)
...
def read_data(source, stage, timestamp='most_recent', refresh_id=None):
    ...
    if timestamp == 'most_recent':
        df = pd.read_csv(
            "FILEPATH/"
            "{}.csv".format(source, stage)
        )
    else:
        df = pd.read_csv(
            "FILEPATH/{}_{}.csv".format(source, stage, timestamp)
        )
    return df


def example_test_run_cod():
    mapped_df = read_data("FILEPATH")
    ...
    # then define the identifying columns of the dataframe
    ...
    # no assertion means deaths were same and id cols preserved

    agesplit_df = read_data("FILEPATH")
...
def example_test_run_hospital():
    df = pd.read_csv("FILEPATH/"
                     r"sample_data_for_age_splitting_NFC.csv")
    
    id_cols = ['source', 'nid', 'nonfatal_cause_name',
               'year_id', 'age_group_id', 'sex_id', 'location_id']
    t = split_age_sex(df, id_cols, value_column='val',
                      level_of_analysis='nonfatal_cause_name',
                      fix_gbd2016_mistake=False)
    # no assertion means all tests passed
    print(
        "It worked! Orig value sum ({}) == New value sum ({})".format(
            df.val.sum(),
            t.val.sum()
        )
    )

    t.to_csv(r"FILEPATH/test_split.csv", index=False)
