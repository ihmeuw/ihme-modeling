...
// Pull in draws for excess mortality and incidence
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(`meid') measure_id(6 9) location_id(`location') age_group_id(`ages') status(best) source(epi) clear
...
// Merge and transform data
		use `cfr', clear
		merge 1:1 age_group_id location_id year_id sex_id using `incidence', keep(3) nogen
      ...
      drop draw_* incidence_* chronic_incidence_* cf_*
      drop measure_id metric_id model_version_id modelable_entity_id
