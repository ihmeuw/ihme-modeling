// Location: do "$j/WORK/03_cod/01_database/04_explore/2014_03_11_mi_ihd_ratio/gen_mi_ihd_ratio_2015.do"
// PREP STATA
    clear
    set more off
    if c(os) == "Unix" {
...
** *********************************************************************
    ...
    foreach system of local filelist {
        ...
        keep if cause == "MI"
         ...
         tempfile raw
         save `raw'
         // Read in pre redistributed IHD data, make long and load in MI data
