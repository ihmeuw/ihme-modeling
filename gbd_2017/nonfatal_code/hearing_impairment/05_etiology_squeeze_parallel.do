		...
		*local meids "1282 2311 2918 2920 2922 2924"
		...
		foreach meid in `meids' {
			...
			if inlist(`meid', `meids_best') {
					...
					if `meid' == 2311 get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(`meid') measure_id(18) location_id(`location_id') clear
					//Load rest as prevalence 
					else get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(`meid') measure_id(5) location_id(`location_id') clear
			   		}

			//If no best model, load zeros 
			if !inlist(`meid', `meids_best') {
			   		noisily di in red "NO MODEL MARKED BEST FOR MEID `meid' - `name' - LOADING ZEROES"
					if `cluster' == 1 get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(2406) measure_id(5) location_id(`location_id') clear
					forval i = 0/999 {
...
//Loop over every year and sex for given location 
	...
	use FILEPATH, clear 
	keep age_group_id adjusted* // Others were kept for diagnostics
	tempfile envelope
	...
	*** OTITIS  (coming in already as prevalence)
		// Chronic otitis media only leads to mild and moderate hearing loss 
				...
				// Mild and moderate split proportionally as per Fria 1985 Figure 2 (FILEPATH)
	...
	*** MENINGIITS 
		// menigitis is coming in as 26+ db rather than 35+ db estimates.  crosswalk using nhanes data
			**STRATEGY (updated 1/20/2016) : we crosswalk 26+ to 35+, then apply the remainder to 20-34. 
							...
							** keep *_0_* *_0
							...
							forvalues draw=0/999 {
								...
								foreach etiology in otitis cong_other other meng_pneumo meng_hib meng_meningo meng_other {
									...
									replace `etiology'_sev`sev'_draw_`draw' = adjusted_`draw'_`sev' * proportion_`etiology' if age_group_id <= 8 // under 20 
