...
*** SET UP LOCALS WITH MODELABLE ENTITY IDS ***  
	...
	local cod_typh_male	  427235
	local cod_typh_female 428708
	local cod_para_male   427244
	local cod_para_female 427247
...
*** PULL IN DRAWS FROM DISMOD MODELS FOR OVERALL INCIDENCE, PR TYPHOID & PR PARATYPHOID ***
	foreach x in inc typh para {
		if "`x'" == "inc" {
			get_draws, gbd_id_type(modelable_entity_id) gbd_id(``x'_meid') location_id(`location') source(epi) status(best) gbd_round_id(5) clear
			}
...
*** PERFORM DRAW-LEVEL CALCULATIONS TO SPLIT TYPHOID & PARATYPHOID, & CALCULATE MRs ***
	...
	forvalues i = 0/999 {
		quietly {
			...
			replace typh_`i' = inc_`i' * prtyph_`i'
...
*** EXPORT DEATHS IF USING DATA RICH CODEM MODEL ***
		...
		foreach x in typh para {
			get_draws, gbd_id_type(cause_id) gbd_id(``x'_cid') source(codem)  version_id(`cod_`x'_male')  location_id(`location') age_group_id(`ages') year_id(`codYears') measure_id(1) clear
			save `codAppend', replace
			
			get_draws, gbd_id_type(cause_id) gbd_id(``x'_cid') source(codem)  version_id(`cod_`x'_female') location_id(`location') age_group_id(`ages') year_id(`codYears') measure_id(1) clear
...
*** CALCULATE PREVALENCE ***	
	foreach x in typh para {
		forvalues i = 0/999 {
			quietly replace `x'_`i' = `x'_`i' * (rbeta(v_`x', w_`x') * range_`x' + min_`x') / 365.25  if measure_id==5 
			}
		}
