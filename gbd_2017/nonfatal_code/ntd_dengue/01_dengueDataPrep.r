...
## There shouldn't be duplicates, but there are. I am going to take the last one...
...
MexData <- read.dta13(sprintf("FILEPATH/mexSubnational_1990_2015.dta",prefix))
...
IndData <- read.dta13(sprintf("FILEPATH/indiaSubnational2016_clean.dta",prefix))
...
####
#### Grabbing MAX of those with multiple
####
...
df3<-get_draws(gbd_id_type="cause_id",gbd_id=357,location_id=ULocs, year_id=UYears, sex_id = 1:2, source=ADDRESS, status="best", gbd_round_id = 5, num_workers = 30)
...
write.csv(DENVData, sprintf("FILEPATH/CleanDENVData.csv",prefix))


DENVData <- read.csv(sprintf("FILEPATH/CleanDENVData.csv",prefix))


eEF <- read.dta13(sprintf("FILEPATH/empiricalExpansionFactors.dta",prefix))
eTRR <- read.dta13(sprintf("FILEPATH/dengueTrendRR.dta",prefix))
eID <- read.dta13(sprintf("FILEPATH/efInflatorDraws.dta",prefix))
...
write.csv(FullData, sprintf("FILEPATH/CleanMatchedDENVData.csv",prefix))
...
locationsDENV <- FullData[FullData$denge_prob > 0,]

mod0 <- lm(score ~ denge_prob, data = locationsDENV)

tmpscore <- locationsDENV$score
...
AgeDist <- read.dta13(sprintf("FILEPATH/ageDistribution.dta",prefix))

ef_toan <- read.csv(sprintf("FILEPATH/ef_toan.csv", prefix))
...
mod <- lmer(meansqrtdeath ~ year_id + (1|region_name), data=locationsDENV)

FullData$LinTrend[which(FullData$denge_prob > 0)] <- predict(mod)

locationsDENV <- FullData[FullData$denge_prob > 0.0001 & !(FullData$denge_prob > 0 & FullData$cases == 0),]
locationsDENV <- locationsDENV[-2214,]

locationsDENV$logLinTrend <- log(locationsDENV$LinTrend)
locationsDENV$logscore <- log(locationsDENV$score+1)

mod2 <- lmer(logmeanM ~ logLinTrend * logscore  + (1|location_id), offset = log(sampleM), data = locationsDENV)

locations <- unique(locationsDENV$location_id)


AllDeath <- get_draws(gbd_id_type="cause_id",gbd_id=357,location_id=locations, year_id=1990:2017, sex_id = 1:2, source=ADDRESS, status="best", gbd_round_id = 5, num_workers = 30)
...
AllPop <- get_population(age_group_id = 22, location_id = locations, year_id = 1990:2017, sex_id = 3)
...
mod <- lmer(meansqrtdeath ~ year_id + (1|region_name), data=locationsDENV)
# mod <- gam(meansqrtdeath ~ s(year_id, k=4) + s(region_name, bs ="re"), data=locationsDENV)


AllOut$PredTrend <- predict(mod, newdata = data.frame(year_id = AllOut$year_id, region_name = AllOut$region_name))

FinalData <- merge(AllOut, FullPRData, by = c("year_id","location_id"))

FinalData$logLinTrend <- log(FinalData$PredTrend)
FinalData$logscore <- log(FinalData$PredScore+1)
FinalData$sampleM <- FinalData$population

mod2 <- lmer(logmeanM ~ logLinTrend * logscore  + (1|location_id), offset = log(sampleM), data = locationsDENV)

FinalData$predout <- exp(predict(mod2, newdata=FinalData))

hm <- which(FinalData$location_id == 43916)

plot(FinalData$predout[hm], type='p')

write.csv(FinalData, sprintf("FILEPATH/FinalData.csv",prefix))
