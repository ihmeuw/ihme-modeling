...
if (HAQI_or_HSA == "HSA") {
  ...
  covariates <- get_covariate_estimates(covariate_id=208, year_id=1980:year_end, location_id=pop_locs, status="best")[, .(location_id, year_id, mean_value, model_version_id)]
  setnames(covariates, "mean_value", "health")
  
} else if (HAQI_or_HSA =="HAQI") {
  
  # covariate: HAQ - covariate_id=1099, covariate_name_short="haqi"
  covariates <- get_covariate_estimates(covariate_id=1099, year_id=1980:year_end, location_id=pop_locs, status="best")[, .(location_id, year_id, mean_value, model_version_id)]
  setnames(covariates, "mean_value", "health")
...
if (WRITE_FILES == "yes") {
  write.csv(pred_death_save, file.path(j.version.dir, paste0("01_death_predictions.csv")), row.names=FALSE)
}
#*********************************************************************************************************************** 
...
### save results
if (WRITE_FILES == "yes") {
  write.csv(split_deaths_hyb, file.path(j.version.dir, "02_death_predictions_hybridized.csv"), row.names=FALSE)
}
#***********************************************************************************************************************
...
# save model version
cat(paste0("Varicella DisMod model (me_id 14369) for prevalence and incidence - model version ", unique(var_prev$model_version_id)), 
    file=file.path(j.version.dir.logs, "input_model_version_ids.txt"), sep="\n", append=TRUE)
...
### drop age over 90
predictions_prev_save <- predictions_prev_save[!age_group_id %in% c(32, 235), ]
