...
* BOILERPLATE *
  ...
  capture log close
  log using FILEPATH/impairment.txt, replace
...
* PREP DATA FOR MODELLING *  
  ...
  *merge geographic restrictions endemic list
  
  save "FILEPATH/prep1.dta", replace
  
  import excel "FILEPATH/TRAgeo_restrictions2017.xlsx", firstrow clear
  
  merge 1:m location_id using "FILEPATH/prep1.dta"
  ...
  *remove saudi subnationals
  ...
  bysort location_id: egen anyNonZero = max(casesImpairment>0 & !missing(casesImpairment))
  ...
  generate meanImpairment = casesImpairment / sampleImpairment
  generate logitMeanImpairment = logit(meanImpairment + 0.0000001)
...
* RUN FULL MODEL TO EXTRACT COEFFICIENT ON SEX * 
  mepoisson meanImpairment ageS* pca1 sexOrdered || super_region_id: || region_id: || country_id:
  local sexBeta = _b[sexOrdered]

  
* RUN PREDICTION MODEL *  
  quietly mepoisson meanImpairment ageS* pca1 || super_region_id: || region_id: || country_id:
...
*COLLAPSE INTO 5-YEAR WINDOWS 
  ...
  save "FILEPATH/trachomaImpairmentFullDraws2017_full.dta", replace
