...
* PULL AGE GROUP METADATA *
  odbc load, exec("SELECT age_group_id, age_group_years_start AS age_start, age_group_years_end AS age_end FROM age_group WHERE age_group_id IN (`=subinstr("`ages'", " ", ",", .)')") dsn(ADDRESS) clear
  save `pop'
...
* BRING IN INCOME DATA *    
  odbc load, exec("SELECT location_id AS country_id, location_metadata_value FROM location_metadata WHERE location_metadata_type_id = 12") dsn(ADDRESS) clear
...
merge 1:1 location_id year_id using FILEPATH\prAtRiskTrachomaModelled.dta, gen(parCovarMerge) //keep(3) nogenerate
...
\******************************************************************************/
  ...
  merge m:1 location_id year_id using `covariates', keep(1 3) gen(covarMerge) //assert(3) 
