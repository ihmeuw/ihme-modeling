...
import excel "FILEPATH/trachomaElimination.xlsx", sheet("Sheet1") cellrange(B1:D37) firstrow clear
save `elim', replace
...
* PULL POPULATION DATA *
  ...
  get_population, age_group_id(22) sex_id(3) location_id(-1) location_set_id(8) year_id(-1) clear
  keep if inrange(year_id, 1980, 2016)
  
  merge m:1 location_id using `locations', keep(3) nogenerate
  save `pop', replace

import delimited "FILEPATH/data-verbose.csv", bindquote(strict) clear 
...
drop if inlist(iso3, "AFG", "CAF", "COD", "GIN")
...
use "FILEPATH/prAtRiskTrachoma.dta", clear
...
expand 2 if year_id>=2011, gen(newObs)
replace year_id = year_id+3 if newObs==1
drop newObs
...
save FILEPATH/trachomaPrAtRisk.dta, replace
*/
use  FILEPATH/trachomaPrAtRisk.dta, clear
...
local gom3 BFA BRA CIV CMR EGY ETH GNB LAO MWI SDN SSD YEM
...
replace final = maSmoothB if inlist(iso3, "BDI", "DZA", "ERI", "IRQ", "KEN", "KHM", "MEX", "MLI") | inlist(iso3, "MMR", "MRT", "MWI", "NER", "NGA", "NPL", "SEN", "SLB", "TZA") | inlist(iso3,"VNM", "UGA", "AUS")
...
foreach i of local is {
  twoway (scatteri 0 2020) (scatter prAtRiskTrachoma year_id) (scatter prToModel year_id)(line maSmooth year_id) (line maSmoothB year_id)  ///
  (line gom3Smooth year_id) (line final year_id) if iso3=="`i'", xline(2015) name(`i', replace)
  }
*/
...
keep if inrange(year_id, 1980, 2015)
...
foreach iso of local snIsos {  
  ...
  bysort year_id location_id (iso3): generate keep=_n==1
  keep if keep==1
  drop keep
  ...
  save FILEPATH/prAtRiskTrachomaModelled.dta, replace
  export delimited using FILEPATH/prAtRiskTrachomaModelled.csv, replace
