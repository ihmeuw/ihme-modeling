...
def wait_dismod(me1, me2=None, me3=None):
    # get status_ids of model_versions passed in
    
    if ((me2 is not None) & (me3 is not None)):
	    q = ('SELECT modelable_entity_id, model_version_id, '
                  ...
                  'IN(%s,%s,%s) AND gbd_round_id = 5 AND '
                  'model_version_status_id=1' %(me1,me2,me3))
    elif ((me2 is not None) & (me3 is None)):
        q = ('SELECT modelable_entity_id, model_version_id, '
				  'model_version_status_id, date_inserted FROM '
                  'epi.model_version WHERE modelable_entity_id '
                  'IN(%s,%s) AND gbd_round_id = 5 AND '
                  'model_version_status_id=1' %(me1,me2))
    else:
        q = ('SELECT modelable_entity_id, model_version_id, '
				  'model_version_status_id, date_inserted FROM '
                  'epi.model_version WHERE modelable_entity_id '
                  '= %s AND gbd_round_id = 5 AND '
                  'model_version_status_id=1' % me1)
    while True:
        status = query(q, conn_def="epi")
        df1 = status[status.modelable_entity_id == me1]
        df1 = df1.sort_values(by=['date_inserted'], ascending=False).reset_index(drop=True)
        mv1 = df1.loc[0, 'model_version_id']
        if me2 is not None:
            df2 = status[status.modelable_entity_id == me2]
            df2 = df2.sort_values(by=['date_inserted'], ascending=False).reset_index(drop=True)
            mv2 = df2.loc[0, 'model_version_id']
        if me3 is not None:
            df3 = status[status.modelable_entity_id == me3]
            df3 = df3.sort_values(by=['date_inserted'], ascending=False).reset_index(drop=True)
            mv3 = df3.loc[0, 'model_version_id']            
        break
    if ((me2 is not None) & (me3 is not None)):
        return mv1, mv2, mv3
    elif ((me2 is not None) & (me3 is None)):
...
def get_locations():
    ...
    locations_df = get_location_metadata(location_set_id=35, gbd_round_id=5)  
