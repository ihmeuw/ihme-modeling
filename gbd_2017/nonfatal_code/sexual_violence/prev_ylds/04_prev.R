...
ages <- get_demographics(gbd_team = "epi", gbd_round_id = 5)$age_group_id
ages <- c(ages[1:22], 33, 235)

orig <- get_draws("modelable_entity_id", 10470, source = "epi", 
                  measure_id = 5, location_id = location_id, year_id = year_id, 
                  sex_id = sex_id, status = "best")
