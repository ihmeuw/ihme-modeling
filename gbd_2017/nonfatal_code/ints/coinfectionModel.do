...
import excel "FILEPATH.xlsm", sheet("extraction") firstrow clear
...
get_population, location_id(`locations') age_group_id(-1) sex_id(1 2) year_id(-1) clear
save `pop'


foreach covar in 49 863 486 {
	get_covariate_estimates, covariate_id(`covar') location_id(`locations') clear
...
get_covariate_estimates, covariate_id(881) location_id(`locations') clear
	local name `=covariate_name_short[1]'
...
get_covariate_estimates, covariate_id(486) location_id(-1) clear
...
replace  ageMid = 3 if age_group_id<5  
...
compress
save ""FILEPATH/hivRRs.dta", replace
