...
process_draws <- function (parent_draws, severe_id, mod_id, mild_id) {

  ## Pull in draws
  cat("\tGetting severe draws\n")
  severe_draws <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = severe_id, source = "ADDRESS",
                            location_id = my_loc, status = "best")

  cat("\tGetting moderate draws\n")
  mod_draws    <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = mod_id, source = "ADDRESS",
                            location_id = my_loc, status = "best")

  cat("\tGetting mild draws\n")
  mild_draws   <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = mild_id, source = "ADDRESS",
                            location_id = my_loc, status = "best")
  ...
  for (my_draw in grep("draw", names(severe_draws), value = T)){
    if (severe_id %in% c(2956, 2957, 2958)) severe_draws[, (my_draw) := get(my_draw) * (8/33)]
  ...
  for (my_draw in grep("draw", names(mod_draws), value = T)){
    if (mod_id %in% c(2956, 2957, 2958)) mod_draws[, (my_draw) := get(my_draw) * (8/33)]
    setnames(mod_draws, old = my_draw, new = paste0("mod_", strsplit(my_draw, "_")[[1]][2]))
  }

  for (my_draw in grep("draw", names(mild_draws), value = T)){
    if (mild_id %in% c(2956, 2957, 2958)) mild_draws[, (my_draw) := get(my_draw) * (8/33)]
    setnames(mild_draws, old = my_draw, new = paste0("mild_", strsplit(my_draw, "_")[[1]][2]))
  }

  ## Merge everything together
  cat("Now merging together data and running subtractions to compute exclusivity draws.\n")
...
process_itch_draws <- function(parent_draws, severe_id, mod_id){
...
negative_check <- function(dt){
  ...
  if (length(anynegs) != 0){
    ...
    write.csv(dt, paste0(output_directory, "errors/", my_loc,".csv"), row.names = F)
...
restrict_loc <- get_model_results("ADDRESS", gbd_id = 1494, measure_id = 5, age_group_id = 27, year_id = 2017,
...
if (my_loc %in% restrict_loc){
  ...
  my_draws <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = 1494, source = "ADDRESS",
                        location_id = my_loc, status = "best")
...
if (!(my_loc %in% restrict_loc)){
  ...
  my_parent <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = 1494, source = "ADDRESS",
                         location_id = my_loc, status = "best")

  my_parent[, c("metric_id", "modelable_entity_id", "model_version_id") := NULL]
  my_parent <- process_draws(my_parent, 2515, 1496, 1495) ## subtract skin diseases
  my_parent <- process_draws(my_parent, 2956, 2958, 2957) ## subtract vision diseases
  my_parent <- process_itch_draws(my_parent, 2621, 2620)  ## subtract itch diseases

  ## Done !
  cat("Draws complete!\n")
  write.csv(my_parent, paste0(output_directory, my_loc, ".csv"), row.names = F)
