...
syphilis  <- readr::read_csv(paste0(out_dir, "EX/syphilis_draws.csv"))
haq       <- readr::read_csv(paste0(out_dir, "EX/", location_id, "_haq.csv"))
...
latent_to_tert <- 0.275 # from Blencowes save lives tool paper

# Note: using latent seroreversion duration for tertiary because we don't have 
...
syphilis_durations %>% 
  ...
  readr::write_csv(paste0(out_dir, "EX/", location_id, "_syphilis.csv"))
