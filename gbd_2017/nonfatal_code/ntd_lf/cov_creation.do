...
// *********************************************************************************************************************************************************************
	...
	import delimited "FILEPATH/lf_exclusions_010517.csv", clear
	tempfile lf_restrictions
	save `lf_restrictions', replace

	import excel "FILEPATH/pct_databank_lf_2017.xlsx", firstrow clear
	...
	// fixing different types of MDA for India and Bangladesh
	...
	export delimited "FILEPATH/cum_mda_treated_lf_2017.csv", replace
...
// *********************************************************************************************************************************************************************
	...
	/*
	import delimited "FILEPATH/geo_restrict_lf.csv", clear
	local year 1980
	forvalues i = 12/48 {
...
import delimited "FILEPATH/lf_gr_map.csv", clear
// rename columns
local year = 1990
forvalues i = 4/30 {
	rename v`i' year_`year'
...
save `inclusions', replace
// save it
export delimited "FILEPATH/geographic_exclusions.csv", replace
*/
keep if include == 1
...
// pull in WHO pop at risk numbers
	...
	drop if location_name == "Cook Islands" | location_name == "French Polynesia" | location_name == "New Caledonia" | location_name == "Niue" | location_name == "Palau" | location_name == "Tuvalu" | location_name == "United Republic of Tanzania (Zanzibar)" | location_name == "Wallis and Futuna"
	...
	get_population, location_id(`pop_locs') year_id(1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017) age_group_id(22) clear
	...
	// create 2017 estimates from 2016
	expand 2 if year_id == 2016, gen(new)
	...
	keep if inlist(year_id, 1990, 1995, 2000, 2005, 2010, 2017)
	keep if include == 1
	// pass on parent proportion to children
	preserve
	keep if inlist(ihme_loc_id, "BRA", "IND", "IDN", "CHN", "KEN", "ETH")
	rename prop_at_risk parent
	tempfile parents
	save `parents', replace
	restore
	drop if inlist(ihme_loc_id, "BRA", "IND", "IDN", "CHN", "KEN", "ETH")
	replace ihme_loc_id = substr(ihme_loc_id, 1, 3)
...
// pull population at risk numbers from WHO for schisto
	...
	replace ihme_loc_id = substr(ihme_loc_id, 1, 3)
	rename year_id year // to match pct data column names
	...
	merge 1:m year ihme_loc_id using `pops', nogen keep(2 3) // keeping 2 keeps the ones that don't have population estimates
	...
	// put in year 2016 with same proportion at risk
	...
	// replace population at risk with 0s with .001 (1%)
...
** want to first get subnationals that are endemic
** then aggregate to the national level
** then divide by pct population at risk numbers

	get_location_metadata, version_id(149) clear // this gets the covariate location list
...
// schisto cummulative treated according to WHO PCT databank
    ...
    preserve // sets template to 2006 and up
      keep location_id
      duplicates drop location_id, force
      expand 36
    ...
    bysort location_id (year): replace cov_national = cov_national[_n-1] if year == `gbd_year' // making an assumption that national coverage was the same in 2016 as it was in 2015
