...
// include "FILEPATH/01_prev_sequela_parallel.do"
...
// By locationpull draw files and correct prevalence of infection and morbidity for effect of mass treatment and scale to national level
  ...
  ** Pull PAR adjusted mf prevalence draws ** USERNAME: just changed this to the correct meid
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(10519) source(ADDRESS) measure_id(5) location_id(`location_id') year_id(`year_ids') sex_id(`sex_ids') clear
  ...
  ** We assume that the age distribution in population at risk is the same as in national population, and therefore use the national population at risk figures to calculate
  ** crude prevalence
  ...
  ** squeeze dismod age pattern draws into envelope draws
  get_draws, gbd_id_type(modelable_entity_id) gbd_id(10994) measure_id(5) location_id(`location_id') year_id(`year_ids') sex_id(`sex_ids') source(ADDRESS) clear
  ** clean up
  keep *_id draw_*
  ...
  ** merge in population data
  ...
  preserve
  import delimited "`in_dir'/prop_pop_at_risk_2017.csv", clear
  tempfile par
  save `par', replace
  restore
  ...
  ** pull hydrocele
  ** use 2000 estimates for later years
  foreach year in 2005 2010 2017 {
    use `temp_2000'
    drop prop_at_risk
...
**********************************************************************************************
  ...
  ** squeeze dismod age pattern draws into envelope draws
  get_draws, gbd_id_type(modelable_entity_id) gbd_id(10993) measure_id(5) location_id(`location_id') year_id(`year_id') sex_id(`sex_id') source(ADDRESS) clear
  ** clean up
  keep *_id draw_*
  quietly drop if age_group_id == 22 | age_group_id == 27 | age_group_id == 33
