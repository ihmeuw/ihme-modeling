...
*/
	...
	** pull in lymphedema draws
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(1492) source(ADDRESS) measure_id(5) location_id(`location') status(best) sex_id(1 2) clear

	** for lymphedema, 95% of patients have 4 episodes per year, 4 days each
	forvalues i = 0/999 {
	...
	** pull in hydrocele draws
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(1493) source(ADDRESS) measure_id(5) location_id(`location') status(best) sex_id(1 2) clear

	** for hydrocele, 70% of patients have 2 episodes per year, 4 days each
	forvalues i = 0/999 {
