...
### Adapted from code for headache frequency and duration 
###########################################################
...
beta_dis <- function(dt){
  ...
  #for all columns in the list of columns defined above as draws (so draw_0...draw_999), multiple the proportion in the draw column by the mean frequency for that category, to get frequency of symptoms in that category weighted by proportion of total cases that are in that category
...
reformat <- function(dt){
  ...
  dt <- copy(long_dt)
  sum_vars <- c("nid", "sex", "age_start", "age_end", "case_name") #effectively, in this data set, this is the same as by_vars, since only one parameter is included, but if in future, I have "Severity" data, too, could separately summarize draws of Frequency and draws of Severity for the same NID-sex-agegroup-casename combination
