...
"value_in" files. This step is parallelized from the ODE solver parent screipt by etiology and outcome (8 jobs)
"""
...
def read_data(etiology, outcome, loc, year, sex):
	"""Reads in data generated for data_in."""
	df = pd.read_csv(os.path.join(in_dir, etiology, str(loc), "{etiology}_{outcome}_{loc}_{year}_{sex}.csv".format(
		etiology = etiology,
		outcome = outcome, 
		loc = loc,
		year = year, 
...
def get_values():
	...
	model_version = db.get_best_model_versions(
	    ...
	    ids = me_id,
	    status = "best")["model_version_id"].iloc[0].astype(str)
	filepath = os.path.join(
	    "/share/epi/panda_cascade/prod",
	    model_version,
	    "full/value.csv")
...
def main(in_dir, out_dir):
	...
	for loc in dems['location_id']:
	    ...
	    for year in dems['year_id']: 
	        for sex in dems['sex_id']: 
	            df = read_data(etiology, outcome, loc, year, sex)
	            data.append(df)
	...
	filepath = os.path.join(folder, "value_in_{etiology}_{outcome}.csv".format(
		etiology = etiology,
		outcome = outcome))
	...
	if not os.path.exists(checks):
	    os.makedirs(checks)
	np.savetxt(os.path.join(checks,"finished_{}_{}.txt".format(etiology, outcome)), finished)
