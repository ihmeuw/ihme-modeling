...
// Description:	Mutliply outcome fractions by meningitis_etiology incidence draws
// Number of output files: 62880
// *********************************************************************************************************************************************************************
// *********************************************************************************************************************************************************************
// Specific settings
...
// WRITE CODE HERE
	...
	foreach location of local locations {
		...
		! qsub -P proj_custom_models -N "`job_name'" -pe multi_slot `slots' -l mem_free=`mem' -o "FILEPATH" -e "FILEPATH" "`shell_file'" "`code_dir'/`step_num'_parallel.do" ///
		...
		if `i_last_step' {
			...
			if `num_last_steps' > 1 {
				foreach i of local last_steps {
					...
					local dir = subinstr(`"`dir'"',`"""',"",.)
					cap confirm file "`root_j_dir'/03_steps/finished.txt"
