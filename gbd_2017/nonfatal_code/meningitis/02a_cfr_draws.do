...
// Description:	Create case fatality rate draws from DisMod output (calculations for duration have been adjusted, changed code for new .ado files and syntax)
...
// automatically set locals when running from 00_master
	...
	// directory for standard code files. This directory is for shared functions and may change every year
	adopath + "FILEPATH"
	// shell file
...
// Calculating case fatality rate from remission and excess mortality rates
	...
	foreach location of local locations {
		...
		//fix this so that any user can use it.
		! qsub -P proj_custom_models -N "`job_name'" -pe multi_slot `slots' -l mem_free=`mem' "FILEPATH" ///
