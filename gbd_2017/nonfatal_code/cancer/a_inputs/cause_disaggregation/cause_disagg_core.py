...
from a_inputs.a_mortality_incidence.cause_disaggregation import (
...
def map_remaining_garbage(df, data_type_id):
    ...
    '''
    # Set the output to drop the gbd_cause column. Final cause information will 
    #   be stored in acause
    ...
    #   NOTE: some ICD codes will be present in the acause column. These ICD 
    #       codes are what will be redistributed as garbage
    ...
    # Update transition causes (causes that are in flux or are often 
    #       inaccurately entered into custom maps)
...
def remap_coding(df, data_type_id):
    ''' Updates coding_system where ICD9_detail data are present within the uid,
        then replaces ICD10 garbage mapping with ICD9_detail codes where 
        necessary
    '''
    def update_coding_system(group):
        ''' If a certain year/sex/registry combination has at least one ICD9
            code, the coding system of every entry in the group is renamed to 
            ICD9_detail (even if an individual observation is actually ICD10). 
        '''
    ...
    df = df.groupby(by=['registry_index', 'sex', 'year_start', 'year_end'])\
