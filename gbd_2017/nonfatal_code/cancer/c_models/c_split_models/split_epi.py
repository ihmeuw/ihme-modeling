...
def split_estimates(modelable_entity_id, cnf_model_run_id):
    ...
    work_dir = get_work_dir(parent_me, cnf_model_run_id)
    utils.ensure_dir(work_dir)
    children_mes, proportion_mes, me_tag = get_me_info(modelable_entity_id, parent_me)
    ...
    split_epi_model(source_meid=parent_me,
                    ...
                    split_meas_ids=measures,
                    output_dir=work_dir)
    print("split data saved to " + work_dir)


def save_splits(modelable_entity_id, cnf_model_run_id):
    ...
    children_mes, skip_mes, me_tag = get_me_info(modelable_entity_id, parent_me)
    measures = get_measures(me_tag)
    save_worker = utils.get_path('save_epi_worker', process='cancer_model')
    # Generate a list of arguments (one for each child me)
    description = "{}_run_{}".format(me_tag, cnf_model_run_id)
    ...
    for cm in children_mes:           
        save_arg_list += [save_args_template.format(meid=cm,
                            meas=" ".join([str(m) for m in measures]),
                            desc=description,
                            input_dir="{}/{}".format(work_dir,cm),
                            cnf_rid=cnf_model_run_id) ]
    # Start jobs                            
...
def main():
    ''' Uses arguments to manage split and save of meid
    '''
    parent_meid = int(sys.argv[1])
    cnf_model_run_id = int(sys.argv[2])
    split_estimates(parent_meid, cnf_model_run_id)
    save_splits(parent_meid, cnf_model_run_id)
