##############################################
## Description: Generates lambda values through an nLx value produced from  
##  lifetable (nLx produced using ID variables directly from get_life_table output)
## Note: Since nLx represents person-years lived between age x and x + n, nLx 
##      age groups can be added to calculate the total nLx
##############################################
...
mortality_funcs <- paste0(FILEPATH, "/Project/Mortality/shared/functions")
...
convert_lifetable <- function(dt, id_vars = get_uid_cols()) {
    ...
    reshape_formula <- as.formula(paste0(paste(id_vars, collapse = " + "),
                                                 " ~ lt_parameter"))
    dt <- data.table::dcast(data = dt, 
                            formula = reshape_formula, 
                            value.var = "mean")
...
generate_nLx <- function(gbd_round_num=5) {
    ...
    mx_ax_qx <- get_life_table(location_set_id = 35, status = 'best',
                    ...
                    gbd_round_id = gbd_round_num, with_shock = 1, with_hiv = 1)
...
upload_notes <- paste("current 'best' version for gbd_round_id", gbd_round_id)
cdb.append_to_table(table_name='cnf_lambda_version',
                    new_data=data.frame(date_updated=c(today), 
