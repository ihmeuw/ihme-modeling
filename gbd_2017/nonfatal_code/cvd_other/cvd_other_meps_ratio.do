...
// We want to use the new MEPS data since we're not sure how the MEPS data in GBD2010 was generated
// We want to make sure that we aren't overcounting multiple patient visits, so we use MEPS instead of Hosp data
// The downside to MEPS is greater uncertainty
...
// This dataset should include all relevant observations, as well as all participants who didn't have any visits (since they will be kep in the merges that are there in the hospital inpatient merging)
...
// Other options for getting uncertainty include bootstrapping, but that was too complicated for this
// Still looking for a PSU variable?
clear all
...
// Talk to Mehrdad for more details on the rationale behind it (or me I guess)
sort mprev_
drop n
...
save `tmp_dir'/meps_draws/meps_draws, replace
...
// We use this US-specific ratio (yes, I know...) to calculate cvd_other estimates for all other countries, using hf_prev estimates
...
merge 1:1 location_id year_id sex_id age_group_id n using `tmp_dir'/meps_draws/meps_draws.dta, keep(3) // Only matching to the USA 2005 data
sum mprev_
...
save `tmp_dir'/meps_draws/ratio_merge, replace
reshape wide coef_, i(sex_id age_group_id) j(n)
save `tmp_dir'/meps_draws/ratio_merge_wide, replace
