...
// Get draws for overall heart failure due to other
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(9575) source(epi) measure_id(5) location_id(`location') status(best) clear
...
// Save Other CVD (excluding HF deaths)
	...
	outsheet age_group_id sex_id year_id draw_* using  "FILEPATH", comma replace
log close
