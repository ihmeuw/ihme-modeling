...
// Define locals
	...
	local log_fil "`input_dir'/goiter_cretin_regress.smcl"
...
// Prep the dismod output file
   ...
   get_model_results, gbd_team("epi") gbd_id("1609") gbd_round_id("5") measure_id("5") location_set_id("35") clear 
...
// merge on regions
  ...
  // Prep the goiter file which we use as an indicator for countries with less than 20% total goiter 
    ...
    // Take the highest value in each country since we don't have a complete time series for each country
...
// Keep ages 1-4
    ...
    sort iso3 year_id
    save "`save_dir'/cretin_prev.dta", replace
...
export excel using "`input_dir'/cretin_prepped.xlsx", firstrow(variables) sheet("extraction") nolabel replace

get_epi_data, bundle_id(410) clear
keep seq
export excel using "`input_dir'/zeroed.xlsx", firstrow(variables) sheet("extraction") nolabel replace

upload_epi_data, bundle_id(410) filepath("`input_dir'/zeroed.xlsx") clear

upload_epi_data, bundle_id(410) filepath("`input_dir'/cretin_prepped.xlsx") clear
