...
*** PULL AGE GROUP DATA ***
	get_demographics, gbd_team(ADDRESS) clear
	odbc load, exec("SELECT age_group_id, age_group_years_start AS age_start, age_group_years_end AS age_end FROM age_group WHERE age_group_id IN (`=subinstr("`r(age_group_ids)'", " ", ",", .)')") `ADDRESS' clear
	save `ageMeta'
...
*** PULL THE DRAWS FROM THE INCDIENCE AGE/SEX CURVE MODEL (DISMOD USED ONLY FOR AGE-SEX SPLIT) ***
	get_draws, gbd_id_field(modelable_entity_id) gbd_id(1461) source(ADDRESS) location_ids(`location') measure_ids(6) age_group_ids(`ages') model_version_id(`ageModel') clear
...
*** EXPORT INCIDENCE DRAWS **
	...
	generate modelable_entity_id = 1461
	generate measure_id = 6
	export delimited using `outDir'/1461/6_`location'.csv, replace
...
\******************************************************************************/
	...
	*** EXPORT ZERO PREVALENCE DRAWS IF THIS IS A NON-ENDEMIC LOCATION ***
	 sum endemic
	 if `r(mean)'== 0 {
		export delimited using `outDir'/1461/5_`location'.csv, replace
		}
	...
	*** ESTIMATE PREVALENCE FROM INCIDENCE IF THIS IS AN ENDEMIC LOCATION ***
		...
		* Reshape from wide back to long (reshape command was VERY slow here -- this is a faster way to do it) *
		forvalues y = 1990 / 2016 {
			preserve
