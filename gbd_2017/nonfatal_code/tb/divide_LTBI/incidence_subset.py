...
def subset_incidence_only(in_df,split_col='measure_combined',
                          split_val='incidence',location_col='location_id'):
    ...
    combined_df = pd.concat([prev_all,inc_dense_only],ignore_index=True)
    return combined_df
...
def combined_transform(in_df,num_mean,num_se,denom_mean,denom_se,
                       split_col="measure_combined",split_val="incidence",
...
def LTBI_data_adjust(df):
    ...
    df = df.drop(['standard_error','mean_allforms','lower_allforms','upper_allforms',
             'mean_latent','lower_latent','upper_latent','measure_latent',
             'se_allforms','se_latent',
             'measure_id','model_version_id'], axis=1)
    df['lower']=''
    df['upper']=''
    df = df.rename(columns={'mean_combined':'mean','measure_allforms':'measure',
                       'se_combined':'standard_error'})
    print('(1)mean max, (2)mean min, (3)std.error max, (4)std.error min. If any are less than 0 or greater than 1, you have a problem')
    print(df['mean'].max(),df['mean'].min(),df['standard_error'].max(),
...
def join_to_latent(in_df,model_version_id):
    ...
    if 'year_id' not in in_df:
        ...
        in_df = df_round(in_df,round_col_name='year_id',input_col='years_average',base=5)
    

    # Join latent to in_df
    join_on = ['year_id','location_id','sex_id','age_group_id']
    join_df = in_df.merge(latent_df,on=join_on,how='inner',
    ...
    join_df['se_allforms'] = join_df.apply(lambda x: get_new_se(x['standard_error'],x['lower_allforms'],x['upper_allforms']),axis=1)
...
def join_to_latent_custom_ages(in_df,filepath):
...
def backcalculate_allforms_se(num_new,num_mean,denom_mean,denom_se):
    ...
    if num_mean != 0:
        # Formula for calculating the combined SE
        new_se = np.sqrt((num_new**2)*(denom_mean**2)-((num_mean**2)*(denom_se**2)/(denom_mean**2)))
    return new_se
