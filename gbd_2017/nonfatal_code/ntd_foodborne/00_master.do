...
// Load original parent models
	odbc load, exec("SELECT modelable_entity_id, modelable_entity_name, cause_id, model_version_id FROM ADDRESS.model_version JOIN ADDRESS.modelable_entity USING (modelable_entity_id) JOIN ADDRESS.modelable_entity_cause USING (modelable_entity_id) WHERE model_version_status_id = 1 AND cause_id = 364 AND gbd_round_id = 5 AND modelable_entity_name LIKE '% food-borne % countries' OR model_version_status_id = 1 AND cause_id = 364 AND gbd_round_id = 4 AND modelable_entity_name LIKE 'Symptomatic %'") `ADDRESS' clear
...
// Save draws with limited country lists
	if `country_limit' == 1 {
		...
		foreach group of local groupings {
			...
			capture mkdir "FILEPATH"
			! qsub -P proj_custom_models -N "`group'_country_exclusions" -pe multi_slot 4 -l mem_free=8 "`shell_file'" "`code_dir'/01_country_limit.do" "`prog_dir' `tmp_dir' `parent_me' `limit_me' `model'"
			local ++ a
			sleep 100
		}
		...
		** Make age draws
		...
		foreach g_name of local grouping_names {
			preserve
				** Heavy
				if "`g_name'" != "cerebral" {
					odbc load, exec("SELECT modelable_entity_id, modelable_entity_name, cause_id, model_version_id FROM ADDRESS.model_version JOIN ADDRESS.modelable_entity USING (modelable_entity_id) JOIN ADDRESS.modelable_entity_cause USING (modelable_entity_id) WHERE model_version_status_id = 1 AND cause_id = 364 AND modelable_entity_name LIKE 'Heavy `g_name'%'") `ADDRESS' clear
				}
				else if "`g_name'" == "cerebral" {
					odbc load, exec("SELECT modelable_entity_id, modelable_entity_name, cause_id, model_version_id FROM ADDRESS.model_version JOIN ADDRESS.modelable_entity USING (modelable_entity_id) JOIN ADDRESS.modelable_entity_cause USING (modelable_entity_id) WHERE model_version_status_id = 1 AND cause_id = 364 AND modelable_entity_name LIKE 'Cerebral paragonimiasis'") `ADDRESS' clear
					}
				...
				** Asymp
				if "`g_name'" != "cerebral" {
					odbc load, exec("SELECT modelable_entity_id, modelable_entity_name, cause_id, model_version_id FROM ADDRESS.model_version JOIN ADDRESS.modelable_entity USING (modelable_entity_id) JOIN ADDRESS.modelable_entity_cause USING (modelable_entity_id) WHERE model_version_status_id = 1 AND cause_id = 364 AND modelable_entity_name LIKE 'Asymptomatic `g_name'%'") `ADDRESS' clear
				}
				else if "`g_name'" == "cerebral" {
				...
				** Parent
				if "`g_name'" != "cerebral" {
					odbc load, exec("SELECT modelable_entity_id AS parent_id, modelable_entity_name AS name, model_version_id AS parent_model FROM ADDRESS.model_version JOIN ADDRESS.modelable_entity USING (modelable_entity_id) JOIN ADDRESS.modelable_entity_cause USING (modelable_entity_id) WHERE model_version_status_id = 1 AND cause_id = 364 AND gbd_round_id = 4 AND modelable_entity_name LIKE 'Symptomatic %`g_name'%'") `ADDRESS' clear
				}
				else if "`g_name'" == "cerebral" {
					odbc load, exec("SELECT modelable_entity_id AS parent_id, modelable_entity_name AS name, model_version_id AS parent_model FROM ADDRESS.model_version JOIN ADDRESS.modelable_entity USING (modelable_entity_id) JOIN ADDRESS.modelable_entity_cause USING (modelable_entity_id) WHERE model_version_status_id = 1 AND cause_id = 364 AND gbd_round_id = 4 AND modelable_entity_name LIKE 'Symptomatic respiratory paragonimiasis%'") `ADDRESS' clear
				}
				gen child_id = `id'
...
// Upload
	// run save_results
	*qui run "FILEPATH/save_results_epi.ado"
	...
	if `upload_parent' == 1 {
		...
		foreach fbt_group of local fbt_groups {
			quietly {
				...
				preserve
					save_results_epi, modelable_entity_id(`parent_upload') db_env("prod") input_file_pattern({location_id}.csv) description("country limit applied to model `model'") input_dir("FILEPATH") mark_best(TRUE) clear
				restore
		...
		foreach me_upload of local upload_ids {
			preserve
				quietly {
					...
					save_results_epi, modelable_entity_id(`me_upload') db_env("prod") input_file_pattern({location_id}.csv) description("high intensity proportion split from model `parent_id'") input_dir("FILEPATH") mark_best(TRUE) clear
					noisily di "UPLOADED -> " c(current_time)
				}
			restore
		}
		...
		foreach me_upload of local upload_ids {
			preserve
				quietly {
					...
					noisily di "ID: `me_upload'..."
					save_results_epi, modelable_entity_id(`me_upload') db_env("prod") input_file_pattern({location_id}.csv) description("asymptomatic remainder from model `parent_id'") input_dir("FILEPATH") mark_best(TRUE) clear
					noisily di "UPLOADED -> " c(current_time)
				}
			restore
		}
