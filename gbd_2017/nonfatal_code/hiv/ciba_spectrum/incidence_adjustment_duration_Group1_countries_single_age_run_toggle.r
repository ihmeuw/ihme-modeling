...
# Adjusts incidence from a Spectrum run using cohort data
...
###
tmp.inc.data <- fread(paste0(stage1.loc,""))
...
# Calculate deaths with Spectrum population
...
duration.path <- paste0("")

# Get data from duration Spectrum run
duration.data <- fread(paste0(duration.path, ""))
...
write.csv(out.data, paste0(out.dir_run, ""), row.names=F)
