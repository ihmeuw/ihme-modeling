...
## Purpose: Split Spectrum under-5 prev, inc, etc. to NN granular and 1-4 age groups, extract prevalence in pregnancy
...
out_dir <- paste0("",spec_name)
dir.create(out_dir, recursive = T, showWarnings = F)
out_dir_death <- paste0("",spec_name)
dir.create(out_dir_death, recursive = T, showWarnings = F)
out_dir_birth <- paste0("",spec_name, '/')
...
spec_draw <- data.table(fread(paste0(spec_dir,"")))
...
pop <- get_population( age_group_id = -1, location_id = loc_id, year_id = -1, sex_id = c(1,2), location_set_id = 79, status = "recent")[, 
                          .(location_id, sex_id, year_id, age_group_id, population)]
setnames(pop, c("year_id", "population"), c("year", "gbd_pop"))
birth_pop <- get_population(age_group_id=164, location_id=loc_id, year_id=1970:2017, sex_id=1:2)
...
write.csv(birth_dt[,.(year, sex_id, run_num, age_group_id, birth_prev_rate, birth_prev_count)], paste0(out_dir_birth, iso3, ''), row.names = F)
...
write.csv(spec_combined[,list(sex_id,year_id,age_group_id,run_num,hiv_deaths,non_hiv_deaths, non_hiv_deaths_prop)],paste0(out_dir_death,""),row.names=F)

spec_combined <- merge(spec_combined,draw_map,by="run_num")



write.csv(spec_combined[,list(sex_id,year_id,run_num,hiv_deaths, non_hiv_deaths, new_hiv,hiv_births,suscept_pop,total_births,pop_neg,pop_lt200,pop_200to350,pop_gt350,pop_art,age_group_id)],paste0(out_dir,""),row.names=F)
