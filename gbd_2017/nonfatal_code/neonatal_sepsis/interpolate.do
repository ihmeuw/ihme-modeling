...
get_draws, gbd_id_type(modelable_entity_id) gbd_id(`copula_me') source(epi) measure_id(5) location_id(`location_id') year_id(`year_id') age_group_id(2 3 4) sex_id(`sex_id') gbd_round_id(5) status(best) clear
	...
	rename prevalence age_group_id_4
	tempfile age_group_id_4
	save `age_group_id_4', replace
	restore
	...
	drop _merge
	merge 1:1 sort_order using `age_group_id_4'
...
/////////////////////
	...
	replace age = (7 + ((27-7)/2)) / 365 if age_group_id == 3 
 
	replace age = (28 + ((365-27)/2)) / 365  if age_group_id == 4 
 
	replace age = (((7 + ((27-7)/2)) + (28 + ((365-27)/2))) / 2) / 365 if age_group_id == 99 
