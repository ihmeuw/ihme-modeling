...
##############################################################
def queryToDF(query, host = DATABASE, db = '', user = USER, 
			pwd = PASSWORD, select = True):
	if db == '':
		conn_string = FILEPATH.format(user = user, 
			pwd = pwd, host = host)
	else:
		conn_string = FILEPATH.format(user = user, 
...
def get_days_cfr_dfs(birth_prev, cfr, acause):
	data_dir = FILEPATH % (parent_dir, acause)
	cfr_df = pd.read_csv(FILEPATH % (data_dir, acause, cfr))
	cfr_df = cfr_df.rename(columns = {'draw_1000' : 'draw_0'})
	cfr_df = cfr_df.loc[cfr_df['year'].isin([1990, 1995, 2000, 2005, 2010, 2017])]
	days_df = pd.read_stata(FILEPATH % (data_dir, birth_prev))
...
def get_mild_modsev_dfs(mild_prop, modsev_prop, acause):
	...
	mild_df = pd.read_csv(FILEPATH % (data_dir, acause, mild_prop))
	mild_df = mild_df.rename(columns = {'draw_1000' : 'draw_0'})
	mild_df = mild_df.loc[mild_df['year'].isin([1990, 1995, 2000, 2005, 2010, 2017])]
	mild_df = mild_df.set_index(col_list)
	mild_df = mild_df.sortlevel()
	modsev_df = pd.read_csv(FILEPATH % (data_dir, acause, modsev_prop))
	modsev_df = modsev_df.rename(columns = {'draw_1000' : 'draw_0'})
	modsev_df = modsev_df.loc[modsev_df['year'].isin([1990, 1995, 2000, 2005, 2010, 2017])]
...
def get_other_modsev_dfs(birth_prev, acause):
	...
	cfr_df = cfr_df.rename(columns = {'draw_1000' : 'draw_0'})
	cfr_df = cfr_df.loc[cfr_df['year'].isin([1990, 1995, 2000, 2005, 2010, 2017])]
	at_birth_df = pd.read_stata(FILEPATH % (data_dir, birth_prev))
	...
	oh_six_df = pd.read_stata(FILEPATH % (data_dir, birth_prev))
	...
	seven_df = pd.read_stata(FILEPATH % (data_dir, birth_prev))
...
def merge_most_detailed(df):
	query = '''
		...
		FROM FILEPATH
		WHERE location_set_version_id = 298
		AND location_set_id = 35
	'''
	detailed = queryToDF(query, host = FILEPATH)
...
def save_scale(acause_dict, acause, birth_prev):
	for prop_type in ['mild_count', 'modsev_count']:
		...
		test_outdir = FILEPATH % (parent_dir, acause, birth_prev)
		prev_df.to_csv(FILEPATH % (test_outdir, prop_type))
