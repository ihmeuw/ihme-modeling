...
file.path(j_root, "FILEPATH/age_sex_split.R") %>% source
#***********************************************************************************************************************
...
prep_case_notifs <- function(cause="Pertussis") {
  ...
  case_notif <- read_excel(file.path(j_root, "FILEPATH", "incidence_series.xls"), sheet=cause) %>% data.table
...
prep_regression <- function(...) {
  ...
  write.csv(historical, file.path(j.version.dir.inputs, paste0("incidence_data_inputs.csv")), row.names=FALSE)
...
### save results
if (WRITE_FILES == "yes") {
  write.csv(rescaled_cases, file.path(j.version.dir, "02_case_predictions_rescaled.csv"), row.names=FALSE)
}
#*********************************************************************************************************************** 
...
### save split draws
if (WRITE_FILES == "yes") {
  write.csv(split_cases, file.path(j.version.dir, "03_case_predictions_split.csv"), row.names=FALSE)
}
#*********************************************************************************************************************** 
...
prep_covariates <- function(...) {
  ...
  hsa <- get_covariate_estimates(covariate_id=208, year_id=1980:year_end, location_id=pop_locs, model_version_id=18665)
  ...
  mal_cov <- get_covariate_estimates(covariate_id=66, year_id=1980:year_end, location_id=pop_locs)
  mal_cov[, ln_mal := log(mean_value)]
  # save model version
  cat(paste0("Covariate malnutrition 66 (CoD) - model version ", unique(mal_cov$model_version_id)), file=file.path(j.version.dir.logs, "input_model_version_ids.txt"), sep="\n", append=TRUE)
  
  # healthcare access and quality index, covariate_id=1099, covariate_name_short="haqi"
  haqi <- get_covariate_estimates(covariate_id=1099, year_id=1980:year_end, location_id=pop_locs, model_version_id=19760)
...
if (WRITE_FILES == "yes") {
  write.csv(CFR_draws_save, file.path(j.version.dir, paste0("04_cfr_model_draws.csv")), row.names=FALSE)
}
#***********************************************************************************************************************
...
if (WRITE_FILES == "yes") {
  write.csv(predictions_deaths_save, file.path(j.version.dir, "05_death_predictions_from_model.csv"), row.names=FALSE)
}
#***********************************************************************************************************************
...
### save results
if (WRITE_FILES == "yes") {
  write.csv(rescaled_deaths, file.path(j.version.dir, "06_death_predictions_rescaled.csv"), row.names=FALSE)
}
#***********************************************************************************************************************
...
if (WRITE_FILES == "yes") {
  write.csv(split_deaths_save, file.path(j.version.dir, "07_death_predictions_split.csv"), row.names=FALSE)
}
#***********************************************************************************************************************
