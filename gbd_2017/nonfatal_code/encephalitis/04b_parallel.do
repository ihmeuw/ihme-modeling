...
// Author:		
// Last updated:	11:36 AM 6/30/2014
// Description:	Parallelization of 04b_outcome_prev_womort

// *********************************************************************************************************************************************************************
// *********************************************************************************************************************************************************************
// LOAD SETTINGS FROM STEP CODE (NO NEED TO EDIT THIS SECTION)
// EMGOLD2 NEED TO EDIT THIS TOP SECTION
	...
	foreach year of local years {
		foreach sex of local sexes {
			...
			** Pull in the outcome file
			...
			//these age groups are now different. 5-20, 30, 31, 32, 235
			...
			foreach n of numlist 235{
				qui expand 5 if age_group_id == `n'
			}
			
			//qui expand 20 if age_group_id == `n' & `n' == 21 // expanding last age group to 100, but unequally weights this... figure out how to deal with it
			...
			** Do math based on what theo sent:
			forvalues i = 0/999 {
				...
				** do a calculation that chris/theo devised:
				forvalues seqnum = 1/99 { // WOULD HAVE TO CHANGE THIS NUMBER IF CHANGE EXPANSION YEARS OF LAST AGE GROUP
			...
			** return to the original age group ids
			...
			// I don't think I want this part here, I'll collate all the outputs at the end
			if "`group'" == "_vision" { // THIS NEEDS TO BE VISION, NOT HEARING
				cap mkdir "FILEPATH"
				cap mkdir "FILEPATH"
				drop measure_id modelable_entity_id grouping
				// these are saved using save_results sn parent file
				outsheet using "FILEPATH", comma replace // measure id for prevalence
			}
			else {
