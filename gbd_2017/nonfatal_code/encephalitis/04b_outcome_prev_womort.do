...
// Author:		
// Last updated:	11:32 AM 6/30/2014
...
// WRITE CODE HERE
	...
	// save results (no need to parallelize, it's only 2 submission, only for vision and epilepsy)
	run "FILEPATH"
	
	save_results_epi, input_dir("FILEPATH") input_file_pattern("{measure_id}_{location_id}_{year_id}_{sex_id}.csv") modelable_entity_id(`_vision_`functional'') ///
	description("Custom `functional' `date' unsqueezed - custom code results") measure_id("5") sex_id("1 2") mark_best(True) clear
	
	run "FILEPATH"
	save_results_epi, input_dir("FILEPATH") input_file_pattern("{location_id}.csv") modelable_entity_id("2821") ///
	description("Custom `functional' `date' unsqueezed - custom code results") measure_id("5") sex_id("1 2") mark_best(True) clear
