...
functions <- paste0(j_root, FILEPATH)
all_output <- FILEPATH
dir.create(paste0(all_output, date))
output <- paste0(all_output, date, "/")
csv_input <- paste0(j_root, FILEAPTH)
lib <- FILEPATH
draws <- paste0("draw_", 0:999)
splits_bundle <- 1346

##Load packages
pacman::p_load(data.table, ggplot2, lme4, arm, boot, haven, readr)
...
source(paste0(functions, "get_epi_data.R"))

##User functions
format_data <- function(dt){
  dt[, year_id := floor((year_start + year_end)/2)]
  dt <- dt[group_review == 1 | is.na(group_review)] ##no studies group reviewed out
  ...
  dt <- unique(dt, by = c("nid", "year_id", "location_id")) ##sum cases and sample size over nid and year so only one data point per both sex all age
...
locations <- get_location_metadata(location_set_id = 9) ##get epi locations
locations <- locations[!location_id==1, .(location_id, location_name, parent_id, super_region_name, most_detailed)]##get rid of global keep necessary variables
...
under_5 <- get_outputs(topic = "cause", age_group_id = 1, sex_id = 3, year_id = 2016,
                                     metric_id = 3, location_id = "all", location_set_id = 9, gbd_round_id = 4)
...
## get epilepsy deaths
epilepsy <- get_outputs(topic = "cause", age_group_id = 27, sex_id = 3, year_id = 2016, location_id = "all",
                        metric_id = 3, location_set_id =9, gbd_round_id = 4, cause_id = 545)
...
##Get proportion data
print("getting data")
splits_data <- get_epi_data(bundle_id = splits_bundle)
...
##generate a frame for the predictions, all most detailed countries years 1980-2016
