		...
		if _rc {
			...
			** di in red "location is `location'" // for tests
			...
			local mem = `slots' * 2
				...
				! qsub -P proj_custom_models -N "`job_name'" -pe multi_slot `slots' -l mem_free=`mem' "`shell_file'" "`code_dir'/`step_num'_exclusions_parallel.do" "`date' `step_num' `step_name' `location' `code_dir' `in_dir' `out_dir' `tmp_dir' `root_tmp_dir' `root_j_dir'"
				sleep 100
	...
	foreach location of local keep_locs {
		** submit job
		...
		if _rc {
			...
			! qsub -P proj_custom_models -N "`job_name'" -pe multi_slot `slots' -l mem_free=`mem' "`shell_file'" "`code_dir'/`step_num'_split_parallel.do" ///
...
**********************************************************************************************************************
	...
	foreach meid of local meids {
		di "saving `meid'"
		save_results_epi, modelable_entity_id(`meid') mark_best(TRUE) db_env("prod") input_file_pattern({location_id}.csv) description("Upload `date'") input_dir(FILEPATH/`meid') measure_id(5) clear
