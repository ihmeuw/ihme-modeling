...
in_dir <- ('FILEPATH')
out_dir <- paste0(in_dir, 'FILEPATH', opt_type)

# .libPaths('FILEPATH')
.libPaths('FILEPATH')
library(seegSDM)
library(gbm)
source(paste0(in_dir, '/econiche_central/calcStats.R'))
# source(paste0(in_dir, '/run_optimizerPy.R'))
...
dat_all <- read.csv(paste0(in_dir, 'FILEPATH/dat_all.csv'))
...
run_optimizerPy(funcs.file_path <- paste0(in_dir, 'FILEPATH'),
                funcs.file <- 'optimizers.py ',
                bounds.file_path <- paste0(in_dir, 'FILEPATH'),
...
par <- read.csv(paste0(out_dir, 'FILEPATH/best_pars_', jobnum, '.csv'))
...
save(stats_test, file = paste0(out_dir,'FILEPATH/stats_test_', jobnum,'.Rdata'))
save(stats_train, file = paste0(out_dir,'FILEPATH/stats_test_', jobnum,'.Rdata'))
...
model.list <- list(model = model,
                   ...
                   pred = pred.raster,
                   coords = coords)
save(model.list, file = paste0(out_dir,'FILEPATH/model_', jobnum,'.Rdata'))
...
save(stats, file = paste0(out_dir,'FILEPATH/stats_', jobnum,'.Rdata'))
preds <- writeRaster(pred.raster,
                     paste0(out_dir, 'FILEPATH/preds_', jobnum, '.tif'),
                     format = 'GTiff')
