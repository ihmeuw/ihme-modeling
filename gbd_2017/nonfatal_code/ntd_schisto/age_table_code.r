...
###          Formulas for se and splitting come from spreadsheet in
###           FILEPATH
...
###           and age_end.  This lives in FILEPATH or the YLD's shared repo
...
### Caveats: Does not propogate uncertainty from every step in the process
...
request_num <- 55679
  acause <- "ntd_schisto"
gbd_id <- 10537 ##put me_id that you are age_splitting for
study_covariates <- #c("cv_diag_elisa", "cv_diag_blood", "cv_diag_other") ##put study covariates whose values you want to maintain/keep the same
##whatever ages you want/have results for (from Dismod)
output_file <- "age_split_new_best" ##name your output file
...
##Load data
all_age <- as.data.table(read.xlsx(paste0(FILEPATH), sheet = 1)) ##get all age data
...
all_age <- all_age[measure %in% c("proportion"),]
...
all_age[measure == "proportion" & is.na(sample_size), sample_size := mean(1-mean)/standard_error^2]
...
age_pattern <- as.data.table(get_draws(gbd_id_field = "modelable_entity_id", gbd_id = gbd_id,
                                       measure_ids = c(5, 6), location_ids = locations, source = ADDRESS,
