...
dat_all <- read.csv('FILEPATH/dat_all_dist-100km.csv')
covs <- brick('FILEPATH/schisto_covs1.grd')
covs <- dropLayer(covs, c(6, 11:13))

# make lists of all files in output directories
path <- 'FILEPATH'
model_files <- list.files(path = paste0(path, 'FILEPATH/'), full.names = TRUE)
model_list <- c()
stats_files <- list.files(path = paste0(path, 'FILEPATH/'), full.names = TRUE)
stats_list <- c()
preds_files <- list.files(path = paste0(path, 'FILEPATH/'), full.names = TRUE)
...
writeRaster(preds_sry,
            file = paste0(path, 'oncho_preds.tif'),
            format = 'GTiff',
            overwrite=T)
...
write.csv(relinf, file = paste0(path, 'relinf.csv'))
...
writeRaster(preds_sry$mean, file = paste0(path, 'prediction_map.tif'), format = 'GTiff')
writeRaster(preds_sry$uncertainty, file = paste0(path, 'uncertainty_map.tif'), format = 'GTiff')
...
png(paste0(path, 'effects-grid.png'),
    width = 5000,
    height = 3000,
    pointsize = 60)
...
png(paste0(path, 'raster-grid.png'),
    width = 5000,
    height = 3000,
    pointsize = 60)
layout(matrix(c(1:9),3,byrow=T))
...
png(paste0(path, 'hists-grid.png'),
    width = 5000,
    height = 3000,
    pointsize = 60)
layout(matrix(c(1:9),3,byrow=T))
