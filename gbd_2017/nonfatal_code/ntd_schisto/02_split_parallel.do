...
**********************************************************************************************************************
	import delimited "`in_dir'/species_specific_exclusions.csv", clear
	...
	** if there isn't coinfection, intestinal or urinary schistosomiasis
	if `split' == 0 {
		if haematobium == "" {
			local urinary = 1
		}
...
** Morbidity
**********************************************************************************************************************
** set parameters for translating infection (x) to morbidity (y): y = (a + bx^c)/(1+bx^c) ** USERNAME changed
...
**********************************************************************************************************************
	** get draws
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(1465) measure_id(18) location_id(1) year_id(2000) source(ADDRESS) status(best) clear
	...
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(1466) measure_id(18) location_id(1) year_id(2000) source(ADDRESS) status(best) clear
	replace location_id = `location'
	drop year_id
	tempfile stage2_draws
	quietly save `stage2_draws', replace

	get_draws, gbd_id_type(modelable_entity_id) gbd_id(1467) measure_id(18) location_id(1) year_id(2000) source(ADDRESS) status(best) clear
	replace location_id = `location'
	drop year_id
...
** we do not decrease acute symptoms with respect to MDA because that is covered in the dismod model

/*

use ``seq'_draws', clear
forvalues year 2005 2010 {
	preserve
	keep if year_id == `year'
	drop year_id
	forvalues i = 0/999 {
	rename seq_`i' lag_`i'
	}
	tempfile year_`year'
...
merge m:1 sex_id age_group_id using `year_2005', nogen assert(3)
replace seq_`i' = lag_`i'[_n+1] * .9^5 + (seq_`i' - lag_`i'[n+1]) if year_id == 2010
drop lag_*
merge m:1 sex_id age_group_id using `year_2010', nogen assert(3)
replace seq_`i' = lag_`i'[_n+2] * .9^11 + (seq_`i' - lag_`i'[n+2]) if year_id == 2016
drop lag_*
