...
// run glems to find ratio of total cases that are haematobium vs. mansoni
		import delimited "FILEPATH/merged_species_05292017.csv", clear
	...
	// methods of moments to get total cases and sample size (really only want cases -- alpha1)
	// this is alpha1, don't need alpha2
	...
	// run generalized linear model on region -- ideally this would be a mixed effects model with random effects on location_id, fixed effects on region -->
	...
	// take the mean across regions to deal with rounded numbers
		...
		// try random effects on study
		// encode(field_citation_value), gen(cit)
		// meglm cases_m i.reg_id, family(binomial sample_size_m) || cit: //
...
// wait for jobs to finish before passing execution back to main step file
	...
	*qui run "FILEPATH/save_results.do"
	...
	save_results_epi, modelable_entity_id(2965) mark_best(TRUE) db_env("prod") input_file_pattern({location_id}.csv) description("New baseline model, PAR adjustment `date'") input_dir(`tmp_dir'/draws/2965) measure_id(5) clear
	save_results_epi, modelable_entity_id(2966) mark_best(TRUE) db_env("prod") input_file_pattern({location_id}.csv) description("New baseline model, PAR adjustment `date'") input_dir(`tmp_dir'/draws/2966) measure_id(5) clear
