...
*** PULL START AND END YEARS FOR ALL MODELLED AGE GROUPS ***	
	odbc load, exec("SELECT age_group_id, age_group_years_start AS age_start, age_group_years_end AS age_end FROM age_group WHERE age_group_id IN (`age_groups')") ADDRESS clear
	save `ages'

	
*** PULL IN AGE-SPECIFIC DATA ***	
	import excel using "FILEPATH/BundleXXX.xlsx", firstrow clear
