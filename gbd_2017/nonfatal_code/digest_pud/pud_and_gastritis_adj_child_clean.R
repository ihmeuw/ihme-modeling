...
## list of ME (modelable entity) defs 
pud_total <- 9759
pud_chronic <- 19693
pud_acute <- 19692
pud_complic <- 19691 
...
adj_chronic_pud <- 20398
adj_acute_pud <- 20399
adj_complic_pud <- 20400

#
gastritis_total <- 9761
gastritis_chronic  <- 19697
...
# adjust chronic, acute, complic to total envelope
## get draws for total, chronic, acute, complic
...
unadj_chronic_draws <- get_draws(gbd_id_type="modelable_entity_id", gbd_id=get(paste0(cause,"_chronic")), source="epi", location_id=location, measure_id=measure, age_group_id=age_groups)
unadj_acute_draws <- get_draws(gbd_id_type="modelable_entity_id", gbd_id=get(paste0(cause,"_acute")), source="epi", location_id=location, measure_id=measure, age_group_id=age_groups)
unadj_complic_draws <- get_draws(gbd_id_type="modelable_entity_id", gbd_id=get(paste0(cause,"_complic")), source="epi", location_id=location, measure_id=measure, age_group_id=age_groups)

## make copies of the draws to apply any/all downstream adjustments
adj_chronic_draws <- copy(unadj_chronic_draws)
...
## this is syntax for a test to make sure the draws manipulated from a copy still have same identifiers
...
## write draws to files so that code to apply anemia proportions can use them, all locations for a given cause and child model need to go to a single, unique folder
