####################################################################################################
## Prep draws and estimates of age-specific fertility rate (asfr) by location id. Uses
...
interpolate_asfr <- function(asfr, rake_to_tfr = T, rake_to_asfr = F, log_space = T) {
  ...
  # # append on age groups so that 15 and 49 ages don't have below zero values
  # asfr <- add_zero_ages(asfr, ages_needed = 0:terminal_age)
  
  # move age groups to half way in between age group start and age group end
  ...
  # okay now get rid of the extra 0s
  ...
  if(rake_to_asfr == T){
    ...
    ## expand the scalars to be able to merge on by single year ages (should probably make a function that does this going forward)
    temp_scalar <- list()
...
# scalars -- for diagnostic purposes
