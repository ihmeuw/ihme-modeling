...
## Set start and end options
  ...
  run_comment <- "agewt_dc" # Name this whatever parameter combination or whatever that you are testing
  run_date <- Sys.Date()
...
## Prepare location-specific data for GPR
if (start <= 5 & end >=5) {
  qsub("st_05",paste0(code_dir,"/05_graph_results.R"), slots = 4, submit = !test, proj="proj_hiv", log = T)
}
