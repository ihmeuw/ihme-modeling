...
inc.data <- data.table()
for (tmp.loc in loc) {
	tmp.inc.data <- fread(paste0(tmp.loc,""))
...
inc.ratio.dir <- paste0("")	
file.list <- list.files(inc.ratio.dir)
loc.list <- gsub("","", file.list)
...
alloc.col(inc.ratio.dt, 1003)
for (loc.select in unique(source.table$loc)) {
	# loc.select <- unique(source.table$loc)[1]
	### read in ratio
	inc.ratio.data <- fread(paste0(inc.ratio.dir, loc.select, ""))
...
write.csv(inc.adjut.data, paste0(out.dir_run, loc,""), row.names=F)

out.dir_ratio <- paste0("")
dir.create(out.dir_ratio, showWarnings=FALSE, recursive=TRUE)
write.csv(inc.ratio.dt, paste0(out.dir_ratio, ""), row.names=F)

## save it to current_run folder
out.dir <- paste0("")
dir.create(out.dir, showWarnings=FALSE, recursive=TRUE)
write.csv(inc.adjut.data, paste0(out.dir, ""), row.names=F)
