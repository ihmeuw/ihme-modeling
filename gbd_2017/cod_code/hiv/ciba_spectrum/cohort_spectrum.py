...
# Epidemic Type
inputEpidemicTypeData = beers.openCSV(AIMfilepath + '')
...
# Epidemic Start Year
try:
    inputEpidemicStartYearData = beers.openCSV('')
...
## Demographic Inputs
# Migration
inputMigrationData = beers.openCSV(demProjFilePath + "")
# Population
inputPopData = beers.openCSV(demProjFilePath + "")
# Total fertility rate (TFR)
inputTFRdata = beers.openCSV(demProjFilePath +  "")
# Age-specific fertility rate (ASFR)
inputASFRdata = beers.openCSV(demProjFilePath + "")
# HIV-free mortality
inputSRdata = beers.openCSV('')
# Sex ratio at birth (SRB)
inputSRBdata = beers.openCSV(demProjFilePath  + "")

## Transition parameters
# Off-ART Mortality
inputNoARTMortality = beers.openCSV(demProjFilePath + "")
# On-ART Mortality
inputOnARTMortality = beers.openCSV(demProjFilePath + "")
# CD4 progression
inputProgressionParameters = beers.openCSV(demProjFilePath + "")

# On-ART Mortality scalar
inputOnARTScalar = pd.read_csv("")

# Child Cotrimoxazole effect
inputChildCTXeffect = beers.openCSV(AIMfilepath + "")

## Treatment parameters
# Adult ART Eligibility
inputAdultARTeligibility = beers.openCSV(demProjFilePath +  "")
# Adult ART Coverage
inputAdultARTCoverage = beers.openCSV(demProjFilePath + "")
# Child ART Eligibility
inputChildARTelgibility = beers.openCSV(demProjFilePath + "")
# Child ART Coverage
inputChildARTCoverage = beers.openCSV(demProjFilePath + '')
# Prevention of Mother to Child Transmission (PMTCT)
inputPMTCT = beers.openCSV(demProjFilePath + '')
# TFR reduction
inputTFRreduction = beers.openCSV(demProjFilePath + "")
# Postnatal Dropout
inputPostnatalDropout = beers.openCSV(demProjFilePath + '')
# Percent Breastfeeding
inputPercentBF = beers.openCSV(demProjFilePath + '')
# Special eligibility populations
inputEligiblePops = beers.openCSV(AIMfilepath + "")

## Incidence and Prevalence    
# Incidence Hazard Percent
inputEPPinc = beers.openCSV(demProjFilePath + "")
if stage == 'stage_2':
    inputAdjInc = beers.openCSV("")
if group[0] == '2' and stage == 'stage_1':
    inputAdjInc = beers.openCSV('')
# Prevalence Rate Percent
inputEPPprev = beers.openCSV(demProjFilePath + "")
# Sex ratio of incidence (by epidemic type)
if group[0] == '1':
    if parent != 'IND':
        inputIncSexRatio = beers.openCSV("")
    else:
        inputIncSexRatio = beers.openCSV("")
    sexRatioAdj = 1
else:
    inputIncSexRatio = beers.openCSV(AIMfilepath + '')
    ...
    try:
        inputSexRatioAdj = beers.openCSV(AIMfilepath + "")
...
inputIncAgeDist = beers.openCSV(AIMfilepath + "")
# CD4 distrubtion at infection
inputInitCD4dist = beers.openCSV(AIMfilepath + "")
# Mother to Child Transmission Rates
inputMTCtransRates= beers.openCSV(AIMfilepath + "")
# Child CD4 distrubtion at infection for counts of eligibility
inputChildCD4countDist = beers.openCSV(AIMfilepath + '')
# Child CD4 distrubtion at infection for percent eligible
inputChildCD4percentDist = beers.openCSV(AIMfilepath + '')

childHIVMortalityOffART = pd.read_csv(demProjFilePath + "", index_col = False)
childHIVMortalityOnART = pd.read_csv(demProjFilePath + "", index_col = False)
childProgressionParameters = pd.read_csv(demProjFilePath + "", index_col = False)
CD4PercAtInf = pd.read_csv(demProjFilePath + "", index_col = False)
childARTDist = pd.read_csv(demProjFilePath + "", index_col = False)
...
for run_id in xrange(indiv_run_nums):
    ...
    # \splitting sex ratio of incidence based on prevalence surveys for generalized epidemics
    ...
    #child incidence adjustment scalars from CIBA                   
    if stage == 'stage_2' and parent != 'IND':
        childIncAdjPD = pd.read_csv("")
        ...
        childIncAdj = {}
        for year in range(minYear, 2018):
            childIncAdj[year] = {}
            for age in range(0,15):
    ...
    #     t = minYear + 1
        ...
        for sex in sexes:
            for age in xrange(childMaxAge + 1, maxAge + 1):
                ARTneed = 0
                for c in noARTCD4states:
                     ## Adding next eligibility group up 
                    if c == 'LT50CD4':
                        lowerEligGroup = 'LT50CD4'
                    else:
                        lowerEligGroup = noARTCD4states[noARTCD4states.index(c) -1]
                ...
                # Get the number of eligible pregnant women 
            ...
            sumART = 0
        #capping coverage at 90% of the total population living with HIV
        ...
        ## use CIBA to only inform age split, prevalence surveys informing sex split
        if stage == "stage_2" and parent == "IND":
            for sex in sexes:
                pred_inc = 0
                pop_neg = 0
            ...
            for age in range(0, childMaxAge):
                ...
                for i in range(minYear, t + 1):
                    for c in CD4States:
                        ...
                        #consider changing these variable names later - took these directly from DPPROJ.PAS
...
if detailed_output:
    ...
    cov_dir = result_path + '' + ISO + '/'
    if not os.path.isdir(cov_dir):
            os.makedirs(cov_dir)
    appended_cov_data.to_csv(cov_dir + str(run_num) + '')

    # Write deaths data to new directory
    deaths_dir = result_path + ''
    if not os.path.isdir(deaths_dir):
            os.makedirs(deaths_dir)
    appended_deaths_data.to_csv(deaths_dir + str(run_num) + '')
