...
pdf(paste0(out_dir, "EX/", location_id, "_results.pdf"))
...
emr <- readr::read_csv(paste0(out_dir, "/EX/excess_mortality_rates_by_stage.csv")) %>% 
...
cod_proportions <- readr::read_csv(paste0(out_dir, "EX/global_cod_ratios.csv"))
...
#   this is because we're splitting into older congenital deaths as well
...
# needs to be wide for save_results
...
# once all of these have saved for all locs, upload with save_results_cod()
...
save_by_year_sex <- function(df, year, location = location_id) {
  ...
  readr::write_csv(subset_male, paste0(out_dir, "EX/", location, "_", year, "_1.csv"))
  readr::write_csv(subset_female, paste0(out_dir, "EX/", location, "_", year, "_2.csv"))
}
