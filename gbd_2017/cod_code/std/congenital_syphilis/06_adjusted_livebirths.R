...
pdf(paste0(out_dir, "EX/", location_id, "_diagnostics.pdf"))
...
anc_visits <- read_csv(paste0(out_dir, "EX/", location_id, ".csv"),
                       col_types = cols(
...
# can read in data here, but df is saved in environment
# unadjusted_births <- readr::read_csv(paste0(out_dir, location_id, ".csv"))
...
# create groups of early/late untreated, inadeqautely treated and adeqautely treated

message(paste(Sys.time(), "Creating test * treat proportions"))

test  <- readr::read_csv(paste0(out_dir, "EX/test_",  location_id, ".csv")) %>% 
  select(location_id, year_id, draw_num, test = draw)

treat <- readr::read_csv(paste0(out_dir, "EX/treat_", location_id, ".csv")) %>% 
...
fetal_death_rate <- readr::read_csv(paste0(out_dir, "/EX/excess_mortality_rates_by_stage.csv")) %>%
...
adjusted_live_births <- left_join(treatment_groups, fetal_death_rate, by = "draw_num") %>% 
  ...
  select(location_id, year_id, draw_num, adj_untreated_early:adj_inadequate_late, adequate)
...
message(paste0(Sys.time(), " Saving adjusted birth results in ", out_dir, location_id, "_adjusted.csv"))

readr::write_csv(adjusted_live_births, paste0(out_dir, "EX/", "_adjusted.csv"))
