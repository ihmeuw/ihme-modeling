...
#          into early and late. Right now plan is to use split from Japanese notification data
...
#              Currently based off 70%/30% early/late split from Ingraham 1950
...
neonatal_mean <- 0.0876
neonatal_lower <- 0.0446
neonatal_upper <- 0.1426
neonatal_std_error <- (neonatal_upper - neonatal_lower) / (2 * 1.96)

# Inadequately treated neonatal deaths from meta-analysis
inadeq_neonatal_mean  <- 0.0302
inadeq_neonatal_lower <- 0.0
inadeq_neonatal_upper <- 0.0929
...
inadeq_fetal_mean  <- 0.1013
inadeq_fetal_lower <- 0.0277
inadeq_fetal_upper <- 0.2079
...
#   makes use of anonymous function (since it's simple here) with sample size (n) and mean (calculated earlier)
#   then set the names column names
...
readr::write_csv(trimmed_draws, paste0(out_dir, "/EX/excess_mortality_rates_by_stage.csv"))
