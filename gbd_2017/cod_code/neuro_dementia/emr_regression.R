...
library(dbplyr, paste0(j_root, FILEPATH))
library(openxlsx, paste0(j_root, FILEPATH))
...
cod_model_m <- 396329
dismod_model <- 292544
locations_regression <- c(434, 102, 93, 101, 89, 4749) ##Finland, Scotland, US, Sweden, Canada, Netherlands, England 
locations_replace <- c(locations_regression, 79) ##Add back Finland
...
dem_ages <- dems$age_group_id[dems$age_group_id >= 13] ## only need 40 and older
...
age_weights[new_age %in% 13:16, weight := sum(weight)][new_age %in% 13:16, new_age := 1]
...
all_draws <- all_draws[, new_age := age_group_id] ##create new column same as the old one
all_draws <- all_draws[age_group_id<=16, new_age := 1] ##new age group is one for 40-59
...
coef[, se := round(se, 3)]
write.csv(coef, paste0(doc_dir, "coef_", date, ".csv"), row.names = F)
...
write.csv(predict_doc, paste0(doc_dir, "predict_ci_", date, ".csv"), row.names = F)
...
## UPLOADER READY
age_row_1 <- data.table(age_group_id = 1, age_start = 40, age_end = 59)
...
## OUTPUT AND UPLOAD
write.xlsx(all_emr, paste0(upload_dir, "new_emr_data", date, ".xlsx"), sheetName = "extraction")
upload_epi_data(bundle_id = bid, filepath = paste0(upload_dir, "new_emr_data", date, ".xlsx"))
