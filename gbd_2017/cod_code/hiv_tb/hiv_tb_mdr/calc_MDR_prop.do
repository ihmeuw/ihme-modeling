** Description: calculate proportions of HIVTB-MDR and TB no-HIV MDR
...
clear all
adopath + "FILEPATH"
get_model_results, gbd_team("epi") gbd_id(9806) location_set_version_id(269) clear
...
// get population
		...
		clear all
		adopath + "FILEPATH"
		get_population, location_id("-1") year_id("-1") sex_id("1 2") age_group_id("-1") location_set_version_id(269) clear
		tempfile pop_all
...
get_model_results, gbd_team("epi") gbd_id(1176) location_set_version_id(269) clear
keep if measure_id==6
keep if (age_group_id>=2 & age_group_id<=20) | (age_group_id>=30 & age_group_id<=32) | age_group_id==235
duplicates drop location_id year_id age_group_id sex_id measure_id, force
drop lower upper
...
get_model_results, gbd_team("epi") gbd_id(9969) location_set_version_id(269) clear
keep if measure_id==6
keep if (age_group_id>=2 & age_group_id<=20) | (age_group_id>=30 & age_group_id<=32) | age_group_id==235
duplicates drop location_id year_id age_group_id sex_id measure_id, force
drop lower upper
...
// merge the files and interpolate
...
// calculate MDR proportions by HIV status
