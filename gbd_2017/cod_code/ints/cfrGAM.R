...
# PULL IN SDI & AGE GROUP METADATA, & CREATE PREDCITION DATA FRAME #  
  ...
  # Get SDI
  sdi <- data.frame(get_covariate_estimates(covariate_id=881))
  ...
  ageMeta <- sqlQuery(ADDRESS, paste("SELECT age_group_id, age_group_years_start, age_group_years_end FROM age_group WHERE age_group_id IN (", ages, ")"))
  ...
  # Create prediction data frame to compute a simultaneous interval for a penalised spline (see https://www.fromthebottomoftheheap.net/2016/12/15/simultaneous-interval-revisited/)
...
# PULL IN CFR DATA AND CLEAN FOR MODELLING #  
  cfr <- read.csv("FILEPATH/cfr_working.csv")
...
###############################################################################################################################################################   
 ...
 write.table(rnorm(n=nDraws, mean=0, sd=crit/qnorm(0.975)), file = "FILEPATH/cfrDrawDeviations.txt", eol = " ", quote = FALSE, row.names = FALSE, col.names = FALSE)

 out <- forPred[,c("location_id", "year_id", "age_group_id", "estPrHiv", "logitPred", "logitPredSeSm")]
 
 write.csv(forPred[,c("location_id", "year_id", "age_group_id", "estPrHiv", "logitPred", "logitPredSeSm")], file = ""FILEPATH/cfrEstimates.csv", row.names = FALSE)
