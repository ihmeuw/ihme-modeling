...
*** PULL IN DRAWS FROM DISMOD INCIDENCE MODEL ***
	local drawLoc = `location'
	
	get_draws, gbd_id_type(modelable_entity_id) gbd_id(`modelMeid') location_id(`drawLoc') age_group_id(`ages') source(epi) status(best) gbd_round_id(5) clear
...
*** PULL IN HIV PREVALENCE DRAWS ***
	get_covariate_estimates, covariate_id(49) location_id(`location') year_id(`codYears') age_group_id(`ages') clear
...
*** MERGE IN CASE FATALITY DATA *** 
	file open dev using "`rootDir'/temp/cfrDrawDeviations.txt", read
	file read dev dev
	tokenize `dev'
	
	merge 1:1 location_id year_id age_group_id sex_id using `rootDir'/temp/cfr_`location'.dta, assert(3) nogenerate
...
*** EXPORT DEATHS ***
	...
	outsheet location_id year_id sex_id age_group_id cause_id measure_id draw_* using `rootDir'/death/`location'.csv, comma replace
	drop draw_*
	
	rename mrHiv_* draw_* 
	outsheet location_id year_id sex_id age_group_id cause_id measure_id draw_* using `rootDir'/death_hiv/`location'.csv, comma replace
	drop draw_*
	
	rename incNoHiv_* draw_* 
	outsheet location_id year_id sex_id age_group_id cause_id measure_id draw_* using `rootDir'/inc_nohiv/`location'.csv, comma replace
	drop draw_*
	
	rename incHiv_* draw_* 
	outsheet location_id year_id sex_id age_group_id cause_id measure_id draw_* using `rootDir'/inc_hiv/`location'.csv, comma replace
	drop draw_*
...
*** SPLIT OUT INCIDENCE AND CALCULATE PREVALENCE ***
	...
	foreach var of varlist draw_* {
		local duration = (rbeta(`v', `w') * `range' + `min') / 365.25
...
*** EXPORT SEQUELA INCIDENCE DRAWS ***
	...
	foreach state of local states {
		...
		outsheet location_id year_id sex_id age_group_id modelable_entity_id measure_id draw_* using `rootDir'/`state'/`location'.csv if state=="`state'", comma replace
		}
