...
// Save checkpoint? //
// save "$j/temp/ctroeger/Cholera/master.dta", replace
...
use age_group_id location_id year_id sex_id cf_`iter' using "FILEPATH/cholera_cf_draws.dta", clear
	...
	table region_name year_id if inlist(year_id, 1990, 1995, 2000, 2005, 2010, 2017), c(sum n_diarrhea sum d_diarrhea sum s_ncases  sum s_ndeaths) row
