...
4) Same as for 3, but with prevalence estimates added as well:
   build_cod_dataset 360, covariate_ids(142 208 118 109) ///
   get_model_results(gbd_team(ADDRESS) model_version_id(95825)) ///
   saveto("FILEPATH/cod360.dta")
...
*** CREATE CONNECTION STRING TO SHARED DATABASE ***
	...
	create_connection_string, server(ADDRESS) database(ADDRESS)
	local ADDRESS = r(conn_string)
...
*** BRING IN LIST OF COD ESTIMATION AGE GROUPS ***
	...
	odbc load, exec("SELECT age_group_id FROM age_group_set_list WHERE age_group_set_id = 12") `ADDRESS' clear
	save `age'

*** BRING IN AGE GROUP DETAILS ***
	odbc load, exec("SELECT age_group_id, age_group_name, age_group_years_start, age_group_years_end FROM age_group") `ADDRESS' clear
...
*** BRING IN AGE WEIGHTS ***
	odbc load, exec("SELECT age_group_id, age_group_weight_value FROM age_group_weight WHERE gbd_round_id = 4") `ADDRESS' clear
...
if "`get_model_results'" != "" {
  ...
  * PULL IN MODEL RESULTS & CLEAN UP A COUPLE VARIABLES ***
	...
	* Get the modelable entity name to label the variable produced by the resolver *
	...
	odbc load, exec("SELECT modelable_entity_name FROM ADDRESS.model_version JOIN ADDRESS.modelable_entity USING (modelable_entity_id) WHERE model_version_id = `mvid'") `ADDRESS' clear
...
if "`covariate_ids'" != "" {
  ...
  * LOAD IN COVARIATE META-DATA TO DETERMINE NAME & AGE/SEX SPECIFICITY *
	odbc load, exec("SELECT covariate_id, covariate_name_short, covariate_name, by_age, by_sex, dichotomous FROM covariate WHERE covariate_id IN (`=subinstr(trim(itrim("`covariate_ids'")), " ", ",", .)')") `ADDRESS' clear
