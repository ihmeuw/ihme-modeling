'''
Description: Applies redistribution packages to the input data to redistribute 
    "garbage codes"
'''
...
def evaluate_cause_restrictions(cause_map, proportion_metadata, proportion_ids):
    ...
    full_cause_map = pd.merge(
        full_cause_map, proportion_metadata, on='proportion_id'
    ...
    return(pd.merge(full_cause_map, pd.DataFrame(maps), on='id'
                    ).ix[:, ['proportion_id', 'cause', 'eval']
                    ].reset_index(drop=True))


def read_json(file_path):
    ''' loads json file and returns as dictionary
    '''
...
def extract_package(package, cause_map):
    ''' Extracts useful package data and returns it
    '''
    fmt_package = {}
    ...
    for tg in package['target_groups']:
        ...
        fmt_package['target_groups'][str(c)] = {
            'target_codes':
                list((set(target_codes)-set(except_codes))
...
def get_packages(input_data, package_folder, cause_map):
    ...
    packages = []
    pkg_json = read_json(package_folder+'/'+rdp+'.json')
    for rdp in package_list:
        package = extract_package(pkg_json, cause_map)
        garbage_present = input_codes.isin(list(package['garbage_codes'])
                         ).tolist()
...
def prep_data(data, proportion_ids, residual_cause='cc_code'):
    ''' Adds required columns and groups data in preparation for RDP
    '''
...
def find_weight_groups(package, proportion_metadata):
    ...
    for wg in package['weight_groups']:
        ...
        try:
            ...
            meta['eval'] = meta.eval(resources)
        weights.append(meta.ix[meta['eval']==True, 
                        ['proportion_id', 'weight_group']])
    return(pd.concat(weights))


def create_target_df(package):
    ''' Returns a dataframe containing the targets required by the package
    '''
    targets = []
    for tg in package['target_groups']:
        temp = pd.DataFrame(
...
def create_proportion_list(data, weight_groups, targets, package):
    ''' Returns a list of the target groups for which proportions are required
    '''
    proportion_list = []
    ...
    for tg in package['target_groups']:
        temp = data.ix[data['cause'].isin(package['target_groups'
                        ][tg]['target_codes']),
...
def create_weight_df(package):
    ''' Returns a dataframe containing the weights and target groups in the 
            package
    '''
    weights = []
    for tg in package['target_groups']:
...
def get_proportions(data, proportion_metadata, package, cause_map_evaluated, 
    ...
    ''' 
    This function generates proportions based on the frequencies in the data 
...
def redistribute_garbage(data, proportions, package):
    ''' Returns the data after applying proportions according to the package
            instructions
    '''
    diagnostics = []
    ...
    output_data = output_data.ix[:, ['proportion_id', 'cause', 'freq']
                        ].reset_index(drop=True)
    # Create diagnostics
...
def test_package_application(input_df, current_df):
    ''' validates that package application did not create a significant 
            difference from the input data
    '''
    current_df.loc[:,'freq'].fillna(value=0, inplace=True)
    diff = current_df['freq'].sum() - input_df['freq'].sum()
    if abs(diff) > 1:
        message = "Difference after package application is too large ({})".format(diff)
...
def apply_packages(prepped_data, proportion_metadata, evaluated_cause_map,
    ...
    ''' Redistributes the prepped_data according to each of the packages
    '''
    ...
    for package in packages:
        ...
        proportions = get_proportions(
            data, proportion_metadata, package, evaluated_cause_map, 
...
def run(input_data, PACKAGE_MAP, TEMP_FOLDER):
    ...
    #
    output_cols =  ['registry_index', 'year_start','year_end', 'year_id', 
                        'sex', 'coding_system','split_group', 'age', 'cause', 
                        'freq']
    proportion_uids = ['dev_status', 'super_region', 'region', 
                    'subnational_level1', 'subnational_level2', 'country',
                    'location_id', 'registry_index', 'year_start', 'year_end', 
                    'year_id', 'sex', 'coding_system', 'split_group', 'age']
    residual_cause = 'ZZZ'
    ...
    prepped_data, proportion_metadata = prep_data(input_data, proportion_uids, 
                                                    residual_cause=residual_cause)
