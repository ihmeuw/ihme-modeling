...
//
// Purpose:	Standardize causes to standardize format
//
** ******************************************************************
** Standardize cause and cause_name	
** ******************************************************************
// Ensure correct data format for cause and cause name (type may have been 
//      converted if all values were empty upon save)
    capture tostring cause, replace
    capture tostring cause_name, replace

// set script location for formatting icd10_codes. global is set by 
//      set_registry_common.do
    local format_ICD10_codes = "$standardize_ICD10_codes"

** *************************************************************************
** Correct Coding Systems
** *************************************************************************
// mark rdptarget codes (codes used to assist redistribution packages that 
...
// Re-assign entries to "CUSTOM" if incorrectly categorized
    // Assign "CUSTOM" to ICD10 codes with alphabetic characters other than 
    //      "C" or "D"
        gen cause_test = subinstr(subinstr(cause, "C", "", .), "D", "", .)
        #delim ;
        gen fails_test = 1 if regexm(cause_test, "[a-zA-Z]") & 
            inlist(coding_system, "ICD10", "ICD9_detail", "NPCR") & 
            rdp_target != 1;
        #delim cr
    ...
    // Assign "CUSTOM" if cause is blank (data_check.do should have verified
    //       that either cause or cause_name exist)
        #delim ;
        replace coding_system = "CUSTOM" 
            if inlist(coding_system, "ICD10", "ICD9_detail", "NPCR") 
            & cause == "";
    // Remove "_cancer"
        replace cause_name = subinstr(cause_name, "_cancer", "", .);
        replace cause_name = substr(cause_name, 1, length(cause_name) -1) 
            if inlist(substr(cause_name, -1, 1), ","  , "_");
        #delim cr
...
** *************************************************************************
** Remove Special Characters, Standardize Spacing and Commas 
** *************************************************************************
// // Format entries
...
// cause_name
    ...
    replace cause_name = subinstr(cause_name, "_", " ", .) 
        if coding_system != "GBD" & rdp_target != 1;
    #delim cr
...
// Remove cause_name from defined coding systems. Remove cause from CUSTOM/GBD 
//      coding systems
    #delim ;
    replace cause_name = cause 
        if !inlist(coding_system, "ICD10", "ICD9_detail", "NPCR") 
        & cause_name == "";
    replace cause = "" 
        if !inlist(coding_system, "ICD10", "ICD9_detail", "NPCR");
...
if `r(N)' > 0 {
    ...
    quietly foreach 1 of numlist 0/9 {
        foreach 2 of numlist 0/9 {
            #delim ;
            replace cause = subinstr(cause, 
                "C`1'`2'-D", "C`1'`2'-C96, D00-D", .);
            foreach 3 of numlist 0/9 {
                replace cause = subinstr(cause, 
                    "C`1'`2'.`3'-D", 
                    "C`1'`2'.`3'-C96.9, D00.0-D", .);
    ...
    count if pos_D > 0
        ...
        #delim ;
        replace cause1 = substr(cause1, 1, length(cause1) -1) 
            if substr(cause1, -1, 1) == "D";  
        replace cause1 = substr(cause1, 1, length(cause1) -1) 
            if substr(cause1, -1, 1) == ",";
...
if `r(N)' > 0 {
    ...
    // Add decimals to 4 and 5 digit codes 
    quietly foreach 1 of numlist 0/9 {
        foreach 2 of numlist 0/9 {
            foreach 3 of numlist 0/9 {
