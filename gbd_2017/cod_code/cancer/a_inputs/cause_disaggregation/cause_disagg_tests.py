...
How to use: import in cause_disaggregation
'''
...
def test_mapped_codes(df):
    ...
    if len(tmp.index) > 0:
        tmp.to_csv('<temp_folder>/causes_mapped_more_than_one_code.csv')
        raise AssertionError("ERROR: some disaggregated causes are mapped to" 
            "more than one code. Please correct")
    else:
        return df
...
def check_calc_errors(df, metric, pre_total, dropped_data_total):
    ...
    df.to_csv('<temp_folder>/inf_total.csv')
...
def check_miss_cause(df):
    ...
    assert len(df[missing_causeval]) == 0, (
        "ERROR: cannot continue with missing cause infomation. Error in "
            "cause_disaggregation suspected")
    return(None)
