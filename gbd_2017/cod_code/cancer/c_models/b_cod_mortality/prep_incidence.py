...
from b_staging import staging_functions
from registry_pipeline import df_manipulate
from a_inputs.a_mortality_incidence import mi_dataset
...
def _add_location_id(df):
    ''' Adds location_id to the data by registry, specially handling exception
            registries
    '''
...
def format_incidence_input(df):
    ...
    # add year_id column
    df = df_manipulate.add_year_id(df).rename(
...
def update_populations(df):
    ...
    df.loc[df['pop'].notnull() & df['rate'].notnull(),
...
def run():
    ...
    pydo.do_stata(drop_and_check, arg_list=None)
