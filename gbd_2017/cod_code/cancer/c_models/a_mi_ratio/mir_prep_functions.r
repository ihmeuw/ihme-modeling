...
mir_prep.get_uploaderInfo <- function(){
    ## Returns the uid for the person running the model input script
    ##
    if (!exists("mir_prep.__uploaderinfo")) {
        uInfo <- as.character(Sys.info()["user"]) 
        if (is.numeric(uInfo)){
            stop("error: please enter a valid uwnetid")
        }
        mir_prep.__uploaderinfo <<- uInfo
    }
    return(mir_prep.__uploaderinfo)
}
...
mir_prep.get_minCasesMap <- function(which_type){
    ...
    if (!exists("mir_prep.__minCasesMap")) {
        ...
        #       the missing requirement witht the general minimum_cases_requirement
...
mir_prep.aggData <- function(input_df, var_to_aggregate){
    ...
    # create lists of data to be asggregated or kept
    ...
    # For any rows could not combine because of differing NIDs,
...
mir_prep.restrictByConfigSettings <- function(mi_df, config_type) {
    ...
    acceptable_mi = (mi_df$mi_ratio <= max_mi_accepted &
                    mi_df$mi_ratio > 0 &
                    mi_df$deaths > 1 &
                    mi_df$cases > 0)
    mi_df <- mi_df[acceptable_mi ,]
    # Restrict by country_id
    location_tags <- mir.load_country_ids()
    drop_countries_input = strsplit(as.character(input_config$drop_countries), ",") 
    drop_countries = as.list(as.numeric(unlist(drop_countries_input)))
    mi_df <- merge(mi_df, location_tags, all.x=TRUE)
    mi_df <- subset(mi_df, !(mi_df$country_id %in% drop_countries), ) 
...
mir_prep.restrictByHAQ <- function(mi_df){
    ## Drops all data for quintile 1-4 that are > quartile 3 by haq quintile, 
    ##      age, sex_id, cancer, then collapse to max value by haq_quintile, 
    ##      age, sex_id, cancer
    ##
    ...
    ## Load HAQ values
    input_config <- mir.get_inputConfig()
    haq_input_number <- input_config$haq_input_number
    haq <- mir.get_HAQValues()
    mi_df <- merge(mi_df, 
                   haq, 
...
mir_prep.generateCaps <- function(type, mir_model_version_id, input_data, pctile) {
    ...
    mir_prep.test_generateCaps(newMapOfCaps=rbind(these_caps, existing_record), 
 ...
 mir_prep.addVariance <-function(df, run_logit_models){
    ## calculates variance using either the delta method
    ##
 ...
 mir_prep.addSTGPR_requirements <- function(df, is_logit_model=true){
...
mir_prep.updateOutliers <- function(new_mir_input, me_name) {
    ## updates any missing or generic nid entries with whatever nid is in the c
    ##      urrent input
    ##    note: this process is intended to update nids when an nid was 
    ##          previously unassigned. because a change of assigned nid indicates
    ##          a change in the data, nids should not be updated if they have 
    ...
    if (nrow(to_update) > 0) {
        ...
        archive_folder <- get_path("mi_model", base="workspace")
...
mir_prep.test_inputConfig <- function(df){
    ...
    required_columns = c('mir_model_version_id', 'mir_model_input_version',
                         'max_mi_input_accepted', 'offset', 'upper_cap_percentile', 
                        'lower_cap_percentile', 'drop_countries')
...
mir_prep.test_generateCaps <- function(newMapOfCaps, capType, 
    ...
    if (length(unique(newMapOfCaps$mir_model_version_id)) < 
            length(unique(existingCapMap$mir_model_version_id))) {
    ...
    for (c in required_columns) {
        ...
        if (!all(existing_values %in% new_values)) {
            ...
            if (results['dropping data']=="") {
                ...
                results['dropping data'] = c(results['dropping data'], 
                                                this_deletion_message)
            }
        }
    }
