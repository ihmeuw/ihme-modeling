# -*- coding: utf-8 -*-
'''
Description: Functions used during the staging process that add various tags
    to the data
How to Use: See individual functions
'''
...
def homogenize_pop(df, 
            uid_cols=['registry_index', 'year_id', 'sex_id', 'age_group_id']):
    ''' Returns the dataset with it's population homogenized by the uid_cols.
...
def add_population(df, data_type_id=2):
    ''' Returns the dataset with missing population replaced by IHME estimates 
            where allowed
    '''
    ...
    ihme_pop = pop.load_raw_ihme_pop(add_pop.location_id.unique())
