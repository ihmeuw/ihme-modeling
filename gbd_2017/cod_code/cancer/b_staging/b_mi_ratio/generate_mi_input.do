...
// Set folders
    ...
    local inc_input = "$mi_datasets_staging_storage/GBD${gbd_iteration}/appended_p7_inc.csv"
    local mor_input = "$mi_datasets_staging_storage/GBD${gbd_iteration}/appended_p7_mor.csv"
...
**   Purpose: generates merge_name variable to enable merges. merge_name 
**      is equal to the dataset name from the first character to the year 
**      designation.
...
program add_merge_name
    // generate merge_name
        ...
        #delim ;
        replace locationOfYear = strpos(dataset_name, "_2") 
            if (locationOfYear == 0) 
            | (strpos(dataset_name, "_2") < strpos(dataset_name, "_1") 
            & strpos(dataset_name, "_2") != 0);
        #delim cr
    ...
    // check for duplicates
        ...
        if r(N) > 0 {
            ...
            noisily di "All data from the same dataset must have same naming schema."
            noisily di "Ensure no data redundancy from a single dataset " 
            noisily di "(by registry_index, sex, acause, and year_id)."
            BREAK
        }
        drop duplicate
...
// // Get INC data and Reformat to enable merge
    import delimited "`inc_input'", clear
    ...
    rename sex sex_id
    rename nid NID_inc
    keep registry_index year_start year_end sex_id acause dataset_id dataset_name age cases pop NID
    ...
    keep cases* pop* dataset* location_id registry_index acause sex_id year* NID
...
// // Get MOR data
    import delimited "`mor_input'", clear
    ...
    rename nid NID_mor
    keep if regexm(acause, "neo_")
    drop deaths_gc deaths_pct_gc // variables not constant within i() variables
    ...
    drop tmp_var
    destring location_id, replace
    keep deaths* pop* dataset* location_id registry_index acause sex_id year* NID
    collapse(sum) deaths* (mean) pop*, by(dataset* location_id registry_index acause sex_id year* NID) fast
...
// Merge data from the same dataset
    merge 1:1 merge_name acause location_id registry_index sex year_id using `incData'
...
// Keep relevant variables
    keep dataset* location_id registry_index year* sex_id acause dataType cases* deaths* pop* NID*
...
**      Keep subnational data if present. Then keep remaining national 
**          data if present. (see KeepNational above)
** *****************************************************************************
// // Keep National registries over non-national registries where possible
    KeepNational
    compress
    save "$temp_folder/first_MI_drop_tempfile.dta", replace

** ****************************************************************
** PART 5: Merge Incidence-only and Mortality-only Data by registry_index-year 
**      where possible
** ****************************************************************
// // Separate INC-only: check for redundancy and rename identifiers
    ...
    //      population because it should reflect the mean population over the 
    //      period
    ...
    // Drop registry_index-cause-years that are missing all data
        ...
        count if to_drop == 1
        if r(N) {
            local dropreason = "incidence dataset contained no incidence for this location-sex-year-cause "
            gen dropReason = "`dropreason'" if to_drop == 1
...
// // Merge INC-only/MOR-only and keep only matching registries
    ...
    // Record drop reason, then drop un-merged data
        ...
        if r(N) {
            ...
            gen dataType = .
            record_and_drop "Part 4" "$staging_name"
        }
        else drop to_drop
...
// Combine NIDs so that there is a single entry for each location_id year_id 
//      sex_id an acause
