# qsub to create fatal estimates for GBD2017 VL - makes cause id = 347 "Leishmaniasis"
...
i<-arg1
  ...
  if(i %in% presence_locations){
    ...
    for(j in required_years){
      ...
      upload_file$cause_id<-rep(347, nrow(upload_file))
    ...
    for(j in required_years){
#temp file to make a dim(output) 46, 1000
      ...
      write.csv(upload_file,
                paste0(prefix, "FILEAPTH", i,"_",j,".csv"))
    }
  }
