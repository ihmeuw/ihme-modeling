...
// Get vaccine coverage estimates //
	...
	get_covariate_estimates, covariate_id(210) clear
	drop age_group_id
...
// little equation for combining stds //
	...
	save `vtype', replace
	// Helpful to save this file at this point for referral // 
	save "FILEPATH/pneumo_vaccine_covs.dta", replace

// Import the PCV data from the literature review //
	import delimited "FILEPATH/lri_pneumo_data_2017.csv", clear
...
// We decided that VE all invasive is equivalent in modeling to VE VT invasive disease //	
...
/// Use a uniform distribution from 0.3 to 1 ///
	...
	gen dummy = 1
	forval j = 1/1000 {
		gen uniform_adj_`j' = 0.3 + (1-0.3) * runiform()
	}
