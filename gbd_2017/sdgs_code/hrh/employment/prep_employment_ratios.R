...
#now see which countries need full age/sex splitting
...
sex.splits.list <- splits.list[splits.list %ni% c(age.splits.list, full.splits.list)] %>% sort #which are left?
...
clean.splits <- splits[data < 1 & abs(data_diff) < .5,
                       -c('sex', 'age', 'age_short.x', 'age_short.y', 'run_id',
                      'age_group_name', 'age_group_name_short', 'data_diff',
                      names(splits)[names(splits) %like% 'ratio' | names(splits) %like% 'og']),
                 with=FALSE]
clean.splits[, cv_subgeo := 1] #give less weight to split points
clean.ref <- ref[, -c('sex', 'age','age_short',
...
#output
write.csv(dt, file=file.path(out.dir, "occ_employment_ratio.csv"),row.names = F)
#***********************************************************************************************************************
