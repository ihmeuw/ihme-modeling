...
## pull relative risks
## bmi rrs
rr_draws <- get_draws(gbd_id_field = c('rei_id'), gbd_round_id = c('4'), draw_type = c('rr'), gbd_id = c(108), source = c('risk'), location_ids = c(paste0(L)))
...
## pull mediated shift
m_shift <- import("/share/gbd/WORK/05_risk/02_models/02_results/diet_ssb_mediated/rr/2/rr_G.csv")
m_shift <- m_shift %>% dplyr::mutate(group=ifelse(parameter=="less:25",1,2))

m_long <- merged.stack(m_shift,var.stubs=c("rr_"),sep="var.stubs",keep.all=T)
setnames(m_long,".time_1","draw")
m_long <- m_long %>% dplyr::select(draw,group,rr_)
m_long <- dcast(m_long,("draw ~ group"),value.var=c("rr_"),sep="")
setnames(m_long,"1","shift_less_25")
setnames(m_long,"2","shift_over_25")
m_long <- sapply(m_long, as.numeric)
m_long <- data.table(m_long)

## pull overweight draws
...
over_long <- merged.stack(over_draws,var.stubs=c("draw_"),sep="var.stubs",keep.all=T)
setnames(over_long,".time_1","draw")
setnames(over_long,"draw_","prop_over")
over_long <- sapply(over_long, as.numeric)
over_long <- data.table(over_long)
setkey(over_long, location_id, year_id, age_group_id, sex_id, draw)
...
exp_long <- left_join(exp_long,over_long)
exp_long <- sapply(exp_long, as.numeric)
exp_long <- data.table(exp_long)
exp_long <- exp_long[draw_>0]
exp_long <- exp_long[prop_over>0]
setkey(exp_long, location_id, year_id, age_group_id, sex_id, ID, draw)
...
calcPAFbyrow <- function(jjj,xx,yy,tm,rs,inv,cp,sex,age,iixx,shift) {
  ...
  ## BMI RRs are in 5 kg/m^2 dose response unit-space
  rrval = rrval^(shift/5)
...
calcbyrow <- function(jj) {
  tryCatch({
    ...
    ## shift in overweight
    over <- as.numeric(exp_long[jj,"prop_over"])
    shift_over_25 <- as.numeric(exp_long[jj,"shift_over_25"])
    shift_under_25 <- as.numeric(exp_long[jj,"shift_less_25"])
    shift = over * shift_over_25  + (1 - over) * shift_under_25
    ...
    pafs = data.table(location_id=LL,year_id=YY,sex_id=SS,age_group_id=AA,draw=ix,PAFOUT,mean=Mval,sd=Spred,prop_over=over)
    return(pafs)
