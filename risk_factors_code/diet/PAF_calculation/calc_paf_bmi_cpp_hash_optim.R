...
## bmi draws
bmi_draws <- get_draws(gbd_id_field = c('modelable_entity_id'), gbd_round_id = c('4'), gbd_id = c('2548'), source = c('risk'), location_ids = c(paste0(L)), status=c('best'), year_ids=c(comp_dem$year_ids))
bmi_ay <- get_draws(gbd_id_field = c('modelable_entity_id'), gbd_round_id = c('4'), gbd_id = c('2548'), source = c('risk'), location_ids = c(paste0(L)), status=c('best'), year_ids=c(ay))
bmi_draws <- bind_rows(bmi_draws,bmi_ay)
bmi_draws <- bmi_draws %>% dplyr::filter(model_version_id==max(model_version_id, na.rm = TRUE))
bmi_draws <- bmi_draws %>% dplyr::select(location_id,year_id,age_group_id,sex_id,starts_with("draw_"))
bmi_draws <- bmi_draws[!duplicated(bmi_draws), ]
names(bmi_draws)[grep("draw_", names(bmi_draws))] <- paste("bmi", names(bmi_draws)[grep("draw_", names(bmi_draws))], sep="_")

## obese draws
obese_draws <- get_draws(gbd_id_field = c('modelable_entity_id'), gbd_round_id = c('4'), gbd_id = c('9364'), source = c('risk'), location_ids = c(paste0(L)), status=c('best'), year_ids=c(comp_dem$year_ids))
obese_ay <- get_draws(gbd_id_field = c('modelable_entity_id'), gbd_round_id = c('4'), gbd_id = c('9364'), source = c('risk'), location_ids = c(paste0(L)), status=c('best'), year_ids=c(ay))
obese_draws <- bind_rows(obese_draws,obese_ay)
obese_draws <- obese_draws %>% dplyr::filter(model_version_id==max(model_version_id, na.rm = TRUE))
obese_draws <- obese_draws %>% dplyr::select(location_id,year_id,age_group_id,sex_id,starts_with("draw_"))
obese_draws <- obese_draws[!duplicated(obese_draws), ]
names(obese_draws)[grep("draw_", names(obese_draws))] <- paste("obese", names(obese_draws)[grep("draw_", names(obese_draws))], sep="_")

## overweight draws
over_draws <- get_draws(gbd_id_field = c('modelable_entity_id'), gbd_round_id = c('4'), gbd_id = c('9363'), source = c('risk'), location_ids = c(paste0(L)), status=c('best'), year_ids=c(comp_dem$year_ids))
over_ay <- get_draws(gbd_id_field = c('modelable_entity_id'), gbd_round_id = c('4'), gbd_id = c('9363'), source = c('risk'), location_ids = c(paste0(L)), status=c('best'), year_ids=c(ay))
over_draws <- bind_rows(over_draws,over_ay)
over_draws <- over_draws %>% dplyr::filter(model_version_id==max(model_version_id, na.rm = TRUE))
over_draws <- over_draws %>% dplyr::select(location_id,year_id,age_group_id,sex_id,starts_with("draw_"))
over_draws <- over_draws[!duplicated(over_draws), ]
names(over_draws)[grep("draw_", names(over_draws))] <- paste("over", names(over_draws)[grep("draw_", names(over_draws))], sep="_")

exp_draws <- left_join(bmi_draws,obese_draws)
exp_draws <- left_join(exp_draws,over_draws)
...
## read in weights
WEIGHTS <- fread(paste0(j_drive,"/WORK/05_risk/ensemble/weights/",risk_map$risk[1],".csv"))

## weights are now universal across space-time-location
w <- WEIGHTS %>% dplyr::ungroup() %>% dplyr::select(-year_id,-sex_id,-age_group_id,-location_id) %>% dplyr::slice(1)
...
## filter out continuous
rr_draws <- rr_draws %>% dplyr::filter(parameter=="per unit")
...
exp_long <- merged.stack(exp_draws,var.stubs=c("bmi_draw_","over_draw_","obese_draw_"),sep="var.stubs",keep.all=T)
setnames(exp_long,".time_1","draw")
exp_long <- sapply(exp_long, as.numeric)
exp_long <- data.table(exp_long)
exp_long <- exp_long[bmi_draw_>0]
...
Rcpp::sourceCpp(paste0(j_drive,"/temp/",user,"/GBD_2016/bmi/integ_bmi.cpp"))

calc_bmi_shape <- function(b,over,obese,weights,mean) {
  tryCatch({
    fx <- NULL
...
calcbyrow <- function(jj) {
  ...
  Mval = as.numeric(exp_long[jj,"bmi_draw_"])
  Over = as.numeric(exp_long[jj,"over_draw_"])
  Obese = as.numeric(exp_long[jj,"obese_draw_"])
  ...
  for(p in seq(1,10,by=2)) {
    SOPT <- nlminb(start=mean/p,objective = calc_bmi_shape,over=Over,obese=Obese,weights=w,mean=Mval,lower=mean*.01,upper=mean*1.5,control=list(iter.max=3,eval.max=3))
  ...
  D <- NULL
  D <- get_edensity(w,Mval,Spred,10,50)
  base <- D$x
  denn <- D$fx
  if(INV==1) {
    (cap <- D$XMIN)
  ...
  pafs = data.table(location_id=LL,year_id=YY,sex_id=SS,age_group_id=AA,draw=ix,PAFOUT,bmi_draw=Mval,sd=Spred,sd_obj=Spred_obj,obese=Obese,overweight=Over)
