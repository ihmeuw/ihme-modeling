...
## PUFA
pufa <- exp_draws %>% dplyr::filter(modelable_entity_id==2436)
## Sat fat
sf <- exp_draws %>% dplyr::filter(modelable_entity_id==2439)
pufa <- pufa %>% dplyr::filter(model_version_id==max(pufa$model_version_id, na.rm = TRUE))
sf <- sf %>% dplyr::filter(model_version_id==max(sf$model_version_id, na.rm = TRUE))
pufa <- pufa %>% dplyr::select(location_id,year_id,age_group_id,sex_id,starts_with("draw_"))
sf <- sf %>% dplyr::select(location_id,year_id,age_group_id,sex_id,starts_with("draw_"))
pufa <- pufa[!duplicated(pufa), ]
sf <- sf[!duplicated(sf), ]
## PUFA TMREL = .12 - [SFA - .07] if SFA is above 7%
for(i in 0:999) {
  sf[,paste0('shift_',i)]=sf[,paste0("draw_",i)] - .07
}
sf <- sf %>% dplyr::select(-starts_with("draw_"))
exp_draws <- inner_join(sf,pufa)
...
exp_long <- merged.stack(exp_draws,var.stubs=c("draw_","shift_"),sep="var.stubs",keep.all=T)
...
calcbyrow <- function(jj) {
  tryCatch({
    ...
    TMREL = as.numeric(exp_long[jj,"draw_"])
    SF = as.numeric(exp_long[jj,"shift_"])
    if(SF>=0) TMREL = .12 - SF
