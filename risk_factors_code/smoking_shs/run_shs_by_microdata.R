...
file.path(j_root, "FILEPATH/get_model_results.R") %>% source
...
### by hh microdata
  ...
  data <- fread(file.path(home, "01_extracted", hh))
  ...
  # individual counts as a zero in that house (you can't expose yourself to SHS)
  ...
  ### add up number of OTHER members of household (NOT including the respondent)
  data[, hh_size := rowSums(data[, age_sex_cols, with=FALSE])]
  # call out survey if strange household sizes
  if (quantile(data$hh_size, 0.95) <= 1) { fwrite(data, file.path(j_root, "FILEPATH", "outliers", paste0("HHs_too_small_", hh)), row.names=FALSE) }
  if (max(data$hh_size) > 100) { fwrite(data, file.path(j_root, "FILEPATH", "outliers", paste0("HHs_too_large_", hh)), row.names=FALSE) }
  ...
  fwrite(hh_size, file.path(j_root, "FILEPATH", "hh_size", hh), row.names=FALSE)
  ...
  ### read in primary smoking prevalence, updating files from current best model run as necessary
  prev_mean <- fread(file.path(j_root, "FILEPATH", paste0(unique(data$location_id), "_mean.csv")))
  prev_draws <- fread(file.path(j_root, "FILEPATH", paste0(unique(data$location_id), "_draws.csv")))
  # merge together mean and draws
  prev <- merge(prev_mean, prev_draws, by=c("location_id", "year_id", "age_group_id", "sex_id"))
  # randomly select 100 draws (vs. 1000 for computational efficiency)
  set.seed(031194)
  ...
  # multiply shs exposure probability by the probability that that person is a smoker
  exposure[, shs := shs_exp * (1 - ind_prev)]
  
  ### set all older age groups to same age_group_id for now
  ...
  write.csv(exposure, file.path(home, "02_shs_calculated", hh), row.names=FALSE)
#***********************************************************************************************************************