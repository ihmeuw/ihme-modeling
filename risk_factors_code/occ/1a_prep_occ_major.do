...
// Function library	
	...
	// crosswalking
	    include "`code_dir'/_lib/01_0_crosswalk_functions.do"
...
lowess exp_unadj year_id, by(location_id sex_id isco_code) gen(lowess_hat) nograph    

	gen residual = lowess_hat - exp_unadj
	gen sd = .

	foreach year of numlist 1970/2015 {
		bysort location_id isco_code sex_id: egen temp = sd(residual) if inrange(year_id, `year'-5, `year'+5)
		replace sd = temp if year_id == `year'
		drop temp
	}	

	foreach year of numlist 1970/1975 {
		bysort location_id isco_code sex_id: egen temp = sd(residual) if inrange(year_id, 1970, `year'+10-(`year'-1970))
		replace sd = temp if year_id == `year'
		drop temp
	}

	foreach year of numlist 2010/2015 {
		bysort location_id isco_code sex_id: egen temp = sd(residual) if inrange(year_id, `year'-10+(2015-`year'), 2015)
		replace sd = temp if year_id == `year'
		drop temp
	}
...
gen sample_size = .
rename isco_version isco_version
