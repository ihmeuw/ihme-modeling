...
# load packages
pacman::p_load(Cairo, data.table, gridExtra, ggplot2, lme4, magrittr, parallel, stringr, xlsx)
...
dt <- file.path(data.dir, 'ISIC_Rev_2-ISIC_Rev_3.txt') %>% fread
...
fwrite(collapse, file=file.path(out.dir, "ISIC_2_TO_3_LVL_1_CW.csv"))

#***********************************************************************************************************************

# ---ISICv3.1 to ISICv4-------------------------------------------------------------------------------------------------
#read in the ISICv3.1->4 concordance table and analyze the flows between major groups
dt <- file.path(data.dir, 'ISIC_Rev_3_1_ISIC_Rev_4.txt') %>% fread
...
fwrite(collapse, file=file.path(out.dir, "ISIC_4_TO_3_LVL_1_CW.csv"))

#***********************************************************************************************************************

# ---RIVER--------------------------------------------------------------------------------------------------------------
#create riverplot
river <- makeRivPlot(all_2_3, all_3_4, "major_2", "major_3", "major_4")

#print the plot object and save as a bitmap
bmp(file.path(graph.dir, "isic_riverplot.bmp"), #toggle if only showing final PM2.5 for GBD2013
    width = 16,
    height = 12,
    units = 'in',
...
title(main='Visualizing Concordance Between Major Groups of ISIC Rev. #2, 3, 4') #add title
