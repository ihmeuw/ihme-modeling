...
HHBUDGETYEM <- read.csv(FILEPATH, stringsAsFactors = FALSE) #some issues w/ weight distribution
...
## SUBSET TO VARIABLES THAT WE WANT
...
HAZ_chart_months <- read.csv(paste0(gbd_dir,"growth_standards/HAZ_0_60_months.csv"), header = TRUE, sep =",")
WHZ_chart_months <- read.csv(paste0(gbd_dir,"growth_standards/WHZ_0_60_months.csv"), header = TRUE, sep =",")
WAZ_chart_months <- read.csv(paste0(gbd_dir,"growth_standards/WAZ_0_60_months.csv"), header = TRUE, sep =",")

HAZ_chart_weeks <- read.csv(paste0(gbd_dir,"growth_standards/HAZ_0_13_weeks.csv"), header = TRUE, sep =",")
WAZ_chart_weeks <- read.csv(paste0(gbd_dir,"growth_standards/WAZ_0_13_weeks.csv"), header = TRUE, sep =",")
...
# drop if HAZ is blank -- shouldn't be any that are blank at this point 
...
# identify locations for file saving -- needs to be within each section to avoid exporting blank csvs (for locations that's don't have data for this section)
...
write.csv(all_data_HAZ_1, file = paste0(save_dir,"/HAZ/",location,"_HAZ.csv"), row.names = FALSE)
}
...
vars_to_keep <- c("nid", "psu", "survey_name", "ihme_loc_id", "year_start", "year_end", "pweight", "strata", "sex", "age_wks",
                "birth_weight", "child_weight", "child_height",
                #"point", "latitude", "longitude", "uncertain_point", "buffer", "admin_1", "WAZ", "age_year", "survey_module", "WAZ_b1", "WAZ_b2", "WAZ_b3", "region")
...
write.csv(all_data_WAZ_1, file = paste0(save_dir,"/WAZ/",location,"_WAZ.csv"), row.names = FALSE)
}
...
write.csv(all_data_WHZ_1, file = paste0(save_dir,"/WHZ/",location,"_WHZ.csv"), row.names = FALSE)
}
...
for (reg in subset_reg){
  ...
  write.csv(data1, file = paste0(save_dir, "/",reg,"_all_data_HAZ.csv"), row.names = FALSE)
}
...
for (reg in subset_reg){
  ...
  write.csv(data1, file = paste0(save_dir, "/",reg,"_all_data_WAZ.csv"), row.names = FALSE)
}
...
for (reg in subset_reg){
  ...
  write.csv(data1, file = paste0(save_dir, "/",reg,"_all_data_WHZ.csv"), row.names = FALSE)
}
