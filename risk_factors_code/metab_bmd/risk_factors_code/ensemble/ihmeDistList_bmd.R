...
#  
#  restrict distribution choices: exclude exponential, inverse gamma and log logistics for bmd
#
#############################################################
set.seed(17895)
#######################################################################
...
gammaOBJ = list(
  ...
  mv2par = function(mn,vr){try(list(shape = mn^2/vr,rate = mn/vr),silent=T)}, 
  
  dF = function(x,p){try(dgamma(x, shape=p[1], rate=p[2]),silent=T)}, 
...
normOBJ = list(
  ...
  dF = function(x,p){try(dnorm(x, mean=p[1], sd=p[2]),silent=T)},
...
mgammaOBJ = list(
  ...
  initF = function(D){try(list(shape = (XMAX-mean(D))^2/var(D), rate = (XMAX-mean(D))/var(D)),silent=T)}, 
  
  mv2par = function(mn,vr,XMIN,XMAX){try(list(shape = (XMAX-mn)^2/vr, rate = (XMAX-mn)/vr, XMIN = XMIN, XMAX = XMAX),silent=T)}, 
  
  dF = function(x,p,XMIN,XMAX){try(dmgamma(x, shape=p[1], rate=p[2], XMIN=p[3], XMAX=p[4]),silent=T)}, 
...
lnormOBJ = list(
  ...
  dF = function(x,p){try(dlnorm(x, meanlog=p[1], sdlog=p[2]),silent=T)},
...
expOBJ = list(
  ...
  mv2par = function(mn,vr){try(list(rate=1/mn),silent=T)}, 
  
  dF = function(x,p){try(dexp(x, rate = p),silent=T)}, 
...
weibullOBJ = list(
  ...
  dF = function(x,p){try(dweibull(x, shape=p[1], scale=p[2]),silent=T)}, 
...
invgammaOBJ = list(
  ...
  dF = function(x,p){try(dinvgamma(x, shape=p[1], scale=p[2]),silent=T)},
...
llogisOBJ = list(
  ...
  dF = function(x,p){try(dllogis(x, shape=p[1], scale=p[2]),silent=T)}, 
...
invweibullOBJ = list(
  ...
  dF = function(x,p){try(dinvweibull(x, shape=p[1], scale=p[2]),silent=T)}, 
...
gumbelOBJ = list(
  ...
  dF = function(x,p){try(dgumbel(x, alpha=p[1], scale=p[2]),silent=T)}, 
...
betasrOBJ = list(
  ...
  initF = function(D){betasr_mv2p(mean(D), var(D), XMIN(D), XMAX(D))}, 
  
  dF = function(x,p){try(dbetasr(x, shape1=p[1], shape2=p[2], XMIN=p[3], XMAX=p[4]),silent=T)},
...
glnormOBJ = list(
  ...
  dF = function(x,p){try(dglnorm(x, mu = p[1], alpha = p[2], beta = p[3]),silent=T)}, 
