...
insheet using "FILEPATH/`region_id'/hiv_rr_$nn.csv", clear
...
drop if _m == 1
replace kid_CDR = 0 if _merge==3 & ihme!="THA"
replace adult_CDR = 0 if _merge==3 & ihme!="THA"
drop _merge
...
duplicates drop ihme sex year, force
keep ihme year c_v5q0 c_v45q15 sex region_name super_region_name ciso
...
tempfile tdata
save `tdata'
keep ihme sex year c_v5q0 c_v45q15 *name* ciso
duplicates drop ihme sex year, force
...
merge 1:1 ihme sex year using `hivfree'


drop if _merge==1
replace match=100 if match==.
...
qui forvalues y=1/$macs {
	...
	local early=years[`t',3]
		...
		if substr("`cc'",1,3)=="BRA" {
            keep if substr(ihme, 1,3)=="BRA"
        }  
		...
		***
		...
		if "`cc'"=="BRA" {
            replace weights=weights/5 if lag!=0
        }  
...
save `smstan',replace
saveold "FILEPATH/standard_lt_`region_id'_`sex'_sim_$nn.dta", replace
...
keep   ihme year c_v45q15 sex age c_v5q0 c_pqx sqx
gen lx=1
sort  ihme sex year age
bysort  ihme sex year: replace lx=lx[_n-1]*(1-c_pqx[_n-1]) if _n>1
sort  ihme sex year age
...
sort  ihme sex year age
reshape wide  c_pqx sqx, i( ihme sex year) j(age)
save `tpqx'
...
reshape long c_pqx sqx qx_adj, i(ihme sex year) j(age)    
keep   ihme year c_v45q15 sex c_v5q0 age qx_adj c_pqx sqx
tempfile tallouts
save `tallouts'
...
keep  ihme sex year age sqx c_pqx c_v5q0 c_v45q15 qx_adj
gen lx=1
...
cap lab drop agelbl
keep  ihme sex year age c_v5q0 c_v45q15 qx_adj sqx c_pqx
...
use "FILEPATH/entry_$nn.dta",clear
	...
	keep if _m == 3
	drop _m
    keep ihme sex year v45q15 v5q0
    ...
    keep   ihme year v45q15 sex age v5q0 c_v* mx_nohiv ax *mxdiff secsim hivrr  nn
    ...
    keep   ihme year v45q15 sex age v5q0 c_v* mx_nohiv ax *mxdiff secsim hivrr nn mx
    keep if age<15
    save `sec5over'                
    ...
    keep   ihme year v45q15 sex v5q0 age qx_adj secsim nn mx
    tempfile secouts                    
    save `secouts'
...
assert qx != .
assert qx > 0
save "FILEPATH/LT_sim_nohiv_`region_id'_`sex'_$nn.dta",replace
use `simouts',clear
compress
assert qx != .
assert qx > 0
save "FILEPATH/LT_sim_withhiv_withsim_`region_id'_`sex'_$nn.dta",replace
...
**read in the HMD location indicator
merge m:1 ihme using "$datadir/hmdcountries.dta"
drop if _merge==2
drop _merge
...
bysort ihme secsim sex year: replace hiv_mx=hiv_mx[20]*`mxold' if _n==21 & hmd!=1
bysort ihme secsim sex year: replace nohiv_mx=nohiv_mx[20]*`mxold' if _n==21 & hmd!=1
...
append using `u5s'
save "FILEPATH/envelope_`region_id'_`sex'_sim_$nn.dta",replace
