...
** **********************
	...
	drop if adj45q15 == . // Subnationals that don't compute for whatever reason -- we drop here
	drop adj45q15_*
    cap drop svy
	tempfile sibs
	save `sibs', replace 
...
** **********************
	...
	//replace exclude = 1 if substr(ihme_loc_id, 1, 3) == "BRA" & year == 2015 & source_type == "VR"
   ...
   // replace exclude = 1 if ihme_loc_id == "GAB" & source_type == "SIBLING_HISTORIES" & sex== "male" & year > 1997 & year < 2012 & adj45q15 < 0.35  
	...
	replace exclude = 1 if ihme_loc_id == "IRN" & source_type == "VR" & year >= 2004 & year < 2015 // VR past this point was Tehran-only
	...
	replace exclude = 1 if ihme_loc_id == "PER" & source_type == "SIBLING_HISTORIES" & (source_date=="2013" | source_date=="2014") // exclude because we think too low 3/9/16
	...
	//replace exclude = 1 if ihme_loc_id == "PSE" & source_type == "VR" & year >= 2008 
	...
	// replace exclude = 1 if ihme_loc_id == "USA" & floor(year) == 2014  
