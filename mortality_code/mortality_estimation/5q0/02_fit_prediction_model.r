...
if (Sys.info()[1] == "Linux"){
    ...
    source("FILEPATH/get_spacetime_loc_hierarchy.R")
}else{
    root <- "JFILEPATH"
    hivsims <- F
    user <- "USERNAME"
...
source("/helper_functions/choose_reference_categories.r")
source("FILEPATH/space_time.r")
spacetime_parameters = read.csv("FILEPATH/selected_parameters.txt")

data <- read.csv("data/prediction_input_data.txt", stringsAsFactors=F)
...
isos <- setDT(read.csv(paste0("FILEPATH/locations", gbd_year,".csv"), stringsAsFactors=F))

# kenya provinces are run in the first stage of the model for the gbd 2016 round, but aren't present in the gbd 2015 location hierarchy
if (gbd_year == 2015) {
    locations_2016 <- setDT(read.csv("FILEPATH/locations2016.csv", stringsAsFactors = F))
    kenya_locations <- locations_2016[parent_id == 180,]
    rm(locations_2016)
    isos <- rbindlist(list(isos, kenya_locations), use.names = T)
}
...
# save first stage model
  save(model, file="FILEPATH/first_stage_regressions_GBD2015.rdata")
  save(model, file=paste("FILEPATH/first_stage_regression_GBD2015_", Sys.Date() ,".rdata", sep=""))
...
#to_correct means that a point will be corrected in the vr step
  nbs.ind <- ((data$data == 1 & !data$to_correct) | data$vr_no_overlap==1)
...
#RUS & POL - Soviet correction for VR
  ...
  #Poland is ok because early VR set as reference, and even complete VR gets adjusted ^
  #if the adjustment isn't downward (Can't do that for Russia because of the Census)
      ...
      #Countries for which we want to estimate different VR bias for multiple different VR systems    
          ...
          for (sys in vr.systems){
              if(length(data$vr[data$ihme_loc_id==ihme_loc_id & data$index==1 & data$source == sys]) >= 5) {
                ...
                # still need to fill in missing on the tails of the in-sample vr data
...
stopifnot(length(unique(data$ihme_loc_id))==all_loc_len)
  ...
  #mark data from countries with surveys in addition to VR
...
# get subnationals - we only want to calculate mse from nationals
  ...
  subnats <- read.csv("FILEPATH/locations.csv", stringsAsFactors = F)
...
write.csv(data, "FILEPATH/prediction_model_results_all_stages_GBD2015.txt", row.names=F)
write.csv(data, paste("FILEPATH/prediction_model_results_all_stages_GBD2015_", Sys.Date(), ".txt", sep=""),row.names=F)