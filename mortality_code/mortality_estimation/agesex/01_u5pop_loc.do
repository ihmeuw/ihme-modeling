...
import delimited "FILEPATH/qx_`loc_id'.csv", clear
keep if age_group_id <=5
gen age = "q_ch" if age_group_id == 5
replace age = "q_u5" if age_group_id == 1
replace age = "q_enn" if age_group_id == 2
replace age = "q_lnn" if age_group_id == 3
replace age = "q_pnn" if age_group_id == 4
assert age != ""
drop age_group_id
...
drop sex_id
rename draw sim
reshape wide qx, i(ihme_loc_id sim sex year) j(age, string)
rename qx* *
tempfile post_1970_qx
save `post_1970_qx'

use `data', clear
append using `post_1970_qx'
save `data', replace
...
*** 1.3 adjust age specific values to match 4q1 entry
...
saveold "FILEPATH/shock_u5_pop_sims_`ihme_loc_id'.dta", replace
...
saveold "FILEPATH/shock_u5_pop_mean_`ihme_loc_id'.dta", replace
