...
// full locations for GBD 2015
	qui do "`prefix'FILEPATH" 
	...
	// file to map Xiso3 codes to ihme_loc_id
	odbc load, exec("SELECT ihme_loc_id, local_id AS iso3 FROM shared.location_hierarchy_history WHERE local_id like 'X%' AND location_set_version_id = 4") dsn(shared) clear
	tempfile xcodes
	save `xcodes', replace		
	// CoD produces USA estimates by county, but we need by state, so map counties to states
	odbc load, exec("SELECT location_id, parent_id, level, location_name, ihme_loc_id FROM shared.location_hierarchy_history where location_set_version_id in (27,28)") dsn(shared) clear
	duplicates drop
	tempfile county
	save `county', replace
...
// do you want to keep the NID?
local dropnid = "no"

// if you want WHO VR vs. COD VR comparison graphs, change this to "yes"
local whocomp = "no"
local whocompfile = "who_vr/data/USABLE_ALL_AGE_DEATHS_VR_WHO_1950-2011.dta"
...
local archivedcompfile = "cod_vr/data/archive/USABLE_ALL_AGE_DEATHS_VR_WHO_GLOBAL_vICD7-10_ 7 Mar 2016.dta"
	// merge on correct location codes
...
// creating a date-specific local for data archiving purposes 
	local date = c(current_date)
	
// put graphs in a new folder with a date on it
cap mkdir "../../../04_outputs/03_diagnostics/who_vr_updates/difference_graphs_`date'"	

// make sure to save a version in our current file structure system, too
global outputdir "`prefix'FILEPATH"
global graphsdir "`prefix'FILEPATH`date'"


use "`prefix'FILEPATH", clear // to use when VR dedup succeeds, AT 1/3/2017
...
// for subnational entities, need to have correct iso3s.  
	gen gbd_country_iso3 = iso3 if location_id != .
		...
		// new data points marked as GBD, but subdiv says they're northern ireland and scotland
		replace ihme_loc_id = "GBR_433" if subdiv == "Northern Ireland"
		replace location_id = 433 if subdiv == "Northern Ireland"
		replace ihme_loc_id = "GBR_434" if subdiv == "Scotland"
		replace location_id = 434 if subdiv == "Scotland"
...
// collapse across subdivisions
	...
	sort ihme_loc_id year sex deaths1
	// duplicates in MAR, QAT, and USA_566
	// usa_566, two points for 2003, one has very few deaths
	drop if dup == 1 & source == "US_NCHS_counties_ICD10" & child_split == 1 & year == 2003
	drop dup
	duplicates tag ihme_loc_id sex year source, gen(dup)
...
// get rid of the extra observations
...
// do not want to use the CHN CDC data because we already use it in another part of our system
...
// do not want to use the Middle East Maternal data source; Diego only had maternal deaths in the source, and had to fill the rest with the envelope
...
// All of the following points represent pooled data that COD has choosen to pool to generate more realiable CF -- this happens early in their process 
...
** Drop if the source is ICD9_BTL in certain countries -- deaths are really low -- cause fractions are usable, so we will use WHO instead
...
** 3/9/2015 drop sources where the cause fractions are fine for CoD purposes but the death numbers are off for mortality
	...
	** drop if ihme_loc_id == "RUS" & inlist(NID, 133401, 133402, 152425) & (year >=1999 & year <=2013)
...
** 3/11/2015: drop CoD sources where the death numbers seem unlikely
	...
	** UK regions huge spike in deaths in 1995 -- cut these in half for now until the CoD team can look at this
...
// check that none of the variables are missing any death data		
	foreach var of varlist deaths1-deaths94 {
		count if `var' == .
...
// format deaths into standard morality data structure
	...
	cap drop DATUM95to99
		** 95to99 was generated from some SWE observations that only have under1 data -- drop it
...
// more general formatting 
	...
	keep if COUNTRY == "RUS"
	tempfile russia_add
	save `russia_add', replace
	
	use "cod_vr/data/USABLE_ALL_AGE_DEATHS_VR_WHO_GLOBAL_vICD7-10.dta", clear
	drop if COUNTRY == "RUS"
	append using `russia_add'
		saveold "cod_vr/data/USABLE_ALL_AGE_DEATHS_VR_WHO_GLOBAL_vICD7-10.dta", replace
		saveold "$outputdir/USABLE_VR_WHO_GLOBAL_vICD7-10.dta", replace
	saveold "cod_vr/data/archive/USABLE_ALL_AGE_DEATHS_VR_WHO_GLOBAL_vICD7-10_`date'.dta", replace
		saveold "$outputdir/USABLE_VR_WHO_GLOBAL_vICD7-10_`date'.dta", replace
	}
...
// graphing code
	cap quietly do "../../../../../Usable/Tools/ADO/pdfmaker.do"
	cap quietly do "../../../../../Usable/Tools/ADO/pdfmaker_acrobat10.do"
	cap quietly do "../../../../../Usable/Tools/ADO/pdfmaker_acrobat11.do"
...
if "`newcurrent'" == "yes" {
	...
	merge 1:1 COUNTRY YEAR SEX using `archivedcompfile'
		// want to keep all data (there are some that are only in the master data, but those would be the new points -- so keep those)
		qui count if _m==1
