// Save compartmental model mortality and progression parameters as inputs for SPECTRUM model
...
foreach a of local ages {
	foreach t of local types {
		...
		insheet using "`comp_dir'/output/`a'_sample_`t'.csv", clear comma names
...
**************************************************************
// Need to deal with draws that didn't converge.  Following code will set to missing the entire draw that didn't converge (ie, it didn't converge for mortality, progression, or both) to missing, and then re-sample from an entire draw (ie, both mortality and progression parameters).  Output needs to ascertain that there is one draw labeled 1...1000 for every age/bin combination
...
if `visuals' == 1 {
	...
	preserve
		...
		duplicates drop
		outsheet using "`comp_dir'/output/`date'/re_age_specific_parameter_bounds.csv", replace comma names
	restore
...
preserve
	...
	duplicates drop
	sort type age cd4
	outsheet using "`comp_dir'/output/`date'/coeff_var_table.csv", replace comma names
	
restore


**************************************************************
** SAVE FOR TO USE IN SPECTRUM
**************************************************************		
preserve
...
// Copy files to the proper locations
	...
	foreach country in `isos' {
		di "`country'"
		! cp "`comp_dir'/output/progression_compiled.csv" "`progression_path'/`country'_progression_par_draws.csv"
		! cp "`comp_dir'/output/mortality_compiled.csv" "`mortality_path'/`country'_mortality_par_draws.csv"
	}
	...
	// Check that all file-saving jobs have finished before proceeding
		...
		while `ccc' == 0 {
			...
			foreach i of local isos {
				capture confirm file "`progression_path'/`i'_progression_par_draws.csv"
				if !_rc local ++filecount

				capture confirm file "`mortality_path'/`i'_mortality_par_draws.csv"
				if !_rc local ++filecount
			}
