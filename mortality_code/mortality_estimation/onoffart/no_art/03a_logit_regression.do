...
**************************************************************
use "`data_dir'/regression/input/hiv_specific_mort.dta", clear
...
// Convert to cumulative probabilities - THIS ACCOUNTS FOR THE FRAME-SHIFT - SHOULD FIX EARLIER ON IN THE MORTALITY SUBTRACTION
...
if `save_draws_graph' == 1 {
	outsheet using "`data_dir'/regression/output/logit_hazard_draws_re_study.csv", replace comma names
}
...
foreach a of local ages {
	...
	drop surv_prob0
	outsheet using "`data_dir'/compartmental_model/input/`a'.csv", replace comma names
	restore
...
preserve
	...
	// Save for graphing in R
	outsheet using "`data_dir'/regression/output/draw_uncertainty.csv", replace comma names
restore
