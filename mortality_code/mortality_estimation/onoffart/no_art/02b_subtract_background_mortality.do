...
// Exact age interval - this means that the age interval in the data matches or is very close to one of the age-intervals in  life tables (ie, 5-year).  There's no point to averaging surrounding age interval mortality rates
generate exact_age_interval = 1 if inlist(age_group, "25_29", "21_23")

// Age-specific - use midpoint age, then the surrounding two five-year age groups from  life tables
drop midpoint_age
...
**************************************************************
**PREP HIV-FREE MORTALITY DATA 
**************************************************************
// Merge on HIV FREE crude death rates for all ages data
...
drop if inlist(age_group_name,"All Ages","Neonatal") | inlist(age_group_id,2,3,4)
...
// IF Some population numbers are missing, especially in the older age groups in lower income countries.  We'll set missings to zeroes here, as all we're doing is computing weighted averages so it won't affect the final results.  We'll also set the corresponding mortality rates to zero.
...
**************************************************************
**ADJUST KAPLAN-MEIER MORTALITY BY SUBTRACTING HIV-FREE MORTALITY RATES
**************************************************************
// All ages mortality
...
// Wider age interval than 5-year rates
...
**SAVE
**************************************************************
drop if inlist(pubmed_id, 1111, 2222, 3333)
...
if `visuals' == 1 {
	...
	// Compare UNAIDS weibull and pooled ALPHA/ZAF mienrs cohort survival adjustments
