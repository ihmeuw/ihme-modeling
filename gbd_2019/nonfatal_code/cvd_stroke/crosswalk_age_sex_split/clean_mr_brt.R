...
source("FILEPATH") ## helper functions from USERNAME
...
## USERNAME's function
##USERNAME: identify rows where mean is contained inside adjusted SE What is saved when you make a "commit"?
find_mrbrt_overlaps<-function(model_obj, adjust_zcovs=T){
  ...
  #trimmed_data[, adjusted_data:=adjusted_data + (counter_preds$Y_mean)] ## don't adjust data
...
get_model_version_ids <- function(meid) {
  ...
  for (bid in meid){
    if (is.null(string)==T){
      ...
      string<-paste0(string,"QUERY",bid)
    }
  }
  query<- paste0("QUERY",string)
  query<-gsub("\n","",query)
