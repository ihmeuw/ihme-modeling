...
##---------------------------------------------------------------------------------------
## An example if populations were desired for the denominator weights

## We need to get all age populations and aggregate to the desired age groups ##
# source("filepath/get_population.R")
# pop_ages <- get_population(age_group_id = age_info$age_group_id, year_id = 1985:2019, sex_id=3, location_id="all", decomp_step="step1")
# pop_ages$number <- pop_ages$population
# denom_weights <- pop_ages
...
#                                                                                                                            ifelse(age_group_id < 18, "Group 5", "Group 6"))))))
...
age_split_mrbrt_weights <- function(df, denominator_weights, output_dir, bounded=T, title="", global_merge = T){
  ...
  ## adding data as dummy anchors to constrain the curve at older age groups
...
##----------------------------------------------------------------------------------------------------
  ...
  # Reassign age_start and age_end with the
  # original ages at the boundaries so that the new age split data do
  # not go outside of the original age ranges. So, for example,
  # a data point 18 to 90 would be split now into 20-40, 40-60, 60-80, 80-99.
  # Changing it so that it is split into
  # 18-40, 40-60, 60-80, 80-90. 
