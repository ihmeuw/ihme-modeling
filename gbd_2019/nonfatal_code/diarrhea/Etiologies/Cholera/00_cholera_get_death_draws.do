...
foreach y in `year' {
	di "`y'"
	get_draws, gbd_id_type(cause_id) source(codcorrect) gbd_id(302) year_id(`y') location_id(`locs') version_id(94) gbd_round_id(6) decomp_step("step4") clear
	append using `draws'
	save `draws', replace
}

save "/filepath/death_draws_codcorrect.dta", replace
