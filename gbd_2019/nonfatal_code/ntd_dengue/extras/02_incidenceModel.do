...
*collapse (mean) draw_* population, by(location_id location_name yearWindow is_estimate) fast
...
*egen casesMean = rowmean(draw_*)
*  tabstat casesMean if is_estimate==1, by(year_id) stat(n mean sum)
...
save_results, modelable_entity_id(ADDRESS3) description("Post-dengue fatigue (using `spaceVar' & `timeVar')") in_dir("FILEPATH") file_pattern("FILEPATH") mark_best(yes)  env("prod")
