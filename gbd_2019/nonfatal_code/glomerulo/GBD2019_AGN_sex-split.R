...
# INPUT DATA -------------------------------------------------------------
##Merging literature data with GBD2019 step 2 clinical data
dt1 <- fread("FILEPATH FOR LITERATURE DATA")
dt2 <- fread("FILEPATH FOR ADMIN DATA")

dt2$cv_literature[dt2$cv_literature == 0 | is.na(dt2$cv_literature)] <-0
dt2 <- subset(dt2, clinical_data_type=="claims" | clinical_data_type=="inpatient")

dt2$cv_subpopulation	 <-0
dt2$cv_biopsy	 <-0
dt2$cv_serum	 <-0 
dt2$cv_diag_exam	 <-0
dt2$cv_chart_review	 <-0
dt2$cv_post_streptococcal	 <-0
dt2$cv_literature <-0

dt2 = within(dt2, {cv_ms2000 = ifelse((year_start==2000 & clinical_data_type=="claims" & location_name!="Taiwan"), 1, 0)})
dt2 = within(dt2, {cv_marketscan = ifelse((year_start!=2000 & clinical_data_type=="claims" & location_name!="Taiwan"), 1, 0)})
dt2 = within(dt2, {cv_hospital = ifelse( clinical_data_type=="inpatient", 1, 0)})
...
appended <- rbind(dt1, dt2,fill=TRUE)
...
model_name <- paste0("acute_pancreatitis_sexsplit_", date)
