...
df<-get_bundle_data(bundle_id = bid, decomp_step = dstep, export = T, gbd_round_id = 6)

# Now read in the cleaned data (after manual review to drop old outliers, age-split data, etc. that we do not want)
library("openxlsx")
...
model_name <- paste0("amphetamines_sexsplit_", date)
...
# Note that NSDUH for ages 35-99 has very small SS / cases especially at state-level; wide age bin; causing problems with age pattern.
# Dropping NSDUH and the few Iran studies for the purposes of fitting the sex ratios (restricting age range to be <= 25 years consistent with age-splitting threshold)
...
sex_model <- run_mr_brt(
  ...
  data = mrbrt_sex_dt[((age_end - age_start) <= 25) & nid != 294223],
  mean_var = "log_ratio",
  se_var = "log_se",
...
ggplot(data = traindata, aes(x = midage, y = ratio)) + geom_hline(yintercept = 1) + geom_point(aes(size = 1/ratio_se, color = trimmed), alpha = .4) + geom_pointrange(data = preds, aes(ymin = Y_mean_lo, ymax = Y_mean_hi, y = Y_mean, x = ((X_lower + X_upper)/2))) +
  geom_errorbarh(data = preds, aes(xmin = X_lower, xmax = X_upper, x = ((X_lower + X_upper)/2), y = Y_mean), height = .001) + labs(x = "Age", y = "Female:Male Sex Ratio", title = "Amphetamine Sex Split") + theme_bw() + theme(text=element_text(size = 20))
