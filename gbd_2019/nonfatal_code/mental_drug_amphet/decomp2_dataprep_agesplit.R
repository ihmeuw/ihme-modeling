...
final_split<-final_split[nid==299931 & ihme_loc_id=="GBR_4624", drop:=1]
final_split<-final_split[is.na(drop)]
final_split<-final_split[, drop:=NULL]
...
bundle_version<-save_bundle_version(bundle_id = 157, decomp_step = "step2", include_clinical = F)
save_crosswalk_version(bundle_version_id = bundle_version$bundle_version_id,
                       data_filepath = "FILEPATH",
                       description = "Sex-Split, Crosswalked, and Age-Split Data using vid 399761 SR pattern")
