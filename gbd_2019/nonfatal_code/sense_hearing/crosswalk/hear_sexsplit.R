...
get_closest_age <- function(i, var, start = T, dt){
  age <- dt[i, get(var)] ###explain this function
  if (start == T){
...
split_data <- function(dt, model, newdata){
  ...
  pops[age_group_years_end == 125, age_group_years_end := 99]  ###same age group concept I don't understand
...
graph_predictions <- function(dt){
  ...
  graph_dt <- graph_dt[!is.na(mean),] #rerun without this line when grants singularity image works
