...
library(msm) 
library(ggplot2)
source("FILEPATH") #has all shared functions, just specify gbd round  ##should I build a folder like this, why was that useful #yes
source("FILEPATH") #add to/figure out why this isnt in function lib (probably need to specific gbd round id?)
...
dt_prep <-copy(micro)
dt_bert <- data.table(dt_prep[, grep("prev", names(dt_prep)), with = FALSE]) #thx stackoverflow
...
#dt_bert <- unique(dt_bert[  ,by =.(sex,age)]) #will instead unique this in the parallelization portion for each unique alt to ref
...
merge_dt <- function(map) {
  #use extract_db once fixed
  extract <- data.table(read.xlsx(paste0(j, "FILEPATH"))) #get raw data
  alts <-map #merge w/ xwalk map 2017 (xwalk map 2019 will have to wait sis)
...
#SPLIT INTO BUNDLES BY MEID #do this #l8r b/c you need results to show to theo
