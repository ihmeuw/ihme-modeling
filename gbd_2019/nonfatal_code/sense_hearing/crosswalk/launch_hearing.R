...
apply_adj <- function(){
    ...
    #get bundle dt
    # print(paste0("Getting category ", names(sep[z]), " from the merged, raw datatable & writing to file"))
    # bundle_dt <- sep[[z]]
    # write.xlsx(x = bundle_dt, file = paste0(hxwalk_temp,"FILEPATH",names(sep[z]) ,"FILEPATH"))
    #
    # dt <- data.table(copy(bundle_dt))
    #bid <- 378

    #dt <- data.table(read.xlsx(paste0(j_work, "imp_hearing/", bid, "FILEPATH",bid,"FILEPATH")))
    #identify points to adjust (those that are not GBD thresholds)

    #ref_thresh <- names(sep[z]) #diff for each bundle
    dt <- copy(mgd_ext)
