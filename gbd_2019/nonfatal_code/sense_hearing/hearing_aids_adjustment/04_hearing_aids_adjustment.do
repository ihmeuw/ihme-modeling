...
// LOAD SETTINGS FROM MASTER CODE (NO NEED TO EDIT THIS SECTION)
	...
	//If running locally, manually set locals (User: Commented out are unnecessary for running locally...if needed refer to full list below)
	if `cluster' == 0 {
...
//Clear previous check files
	...
	// PARALLELIZE BY LOCATION (year and sex are looped over in individual jobs)
			//Set locals of location to loop over 
				...
				//local location_ids 53569  //uncomment the above when running for all locations 
			...
			quietly {
				foreach location_id in `location_ids' {
							...
							!qsub -N "`jobname'" -e "FILEPATH" -o "FILEPATH" -l m_mem_free=10G -l fthread=5 -l h_rt=02:00:00 -l archive=TRUE -q all.q -P "proj_custom_models" "FILEPATH" "FILEPATH" "`root_j_dir' `root_tmp_dir' `date' `step_num' `step_name' `code_dir' `location_id'"	
...
/*
	...
	// write check file to indicate step has finished
		file open finished using "`out_dir'/FILEPATH", replace write
		file close finished
