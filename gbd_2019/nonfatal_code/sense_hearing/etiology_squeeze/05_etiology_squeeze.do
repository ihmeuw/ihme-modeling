...
// LOAD SETTINGS FROM MASTER CODE (NO NEED TO EDIT THIS SECTION)
	...
	if `cluster' == 0 {
		...
		local in_dir "FILEPATH"  // the files in this directory have not been modified since July, what is this for?
	...
	**Define directories 
		// directory for external inputs 
			...
			//commenting out directory creation because using previous date to pull inputs from 04, for step 1 will create directory for new date 
	...
	// write log if running in parallel and log is not already open lhaile: commenting this section out, because it uses output directory Aislyn used for previous step, this section is also commented out in parallel script
	/*
	cap mkdir "FILEPATH"
	cap mkdir "FILEPATH"
	...
	// PARALLELIZE BY LOCATION (year and sex are looped over in individual jobs)
			...
			quietly {
				foreach location_id in `location_ids' {
							...
							!qsub -N "`jobname'" -e "FILEPATH" -o "FILEPATH" -l m_mem_free=10G -l fthread=7 -l h_rt=05:00:00 -l archive=TRUE -q all.q -P "proj_yld" "FILEPATH" "FILEPATH" "`root_j_dir' `root_tmp_dir' `date' `step_num' `step_name' `code_dir' `location_id'"	
							//local ++ n 
							}
						}
