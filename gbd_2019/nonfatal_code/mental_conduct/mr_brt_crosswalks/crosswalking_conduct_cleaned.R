...
bundle_id <- 168
acause <-"mental_conduct"
covariates <- c("cv_estimate_cd_odd_ddnos")
uses_csmr <- F
test_sex_by_super_region <- F
crosswalk_pairs <- '/FILEPATH.csv'
age_sex_split_estimates <- "FILEPATH.xlsx"
need_to_age_split <- F
need_save_bundle_version <- 6761  # Set as true to save bundle version, otherwise specify bundle version here
sex_ratio_by_age <- F
...
### No study level covariates in GBD2019
...
review_sheet_final[,cv_single_informant := NULL]
...
save_crosswalk_version(v_id, crosswalk_save_file, description = "Sex-ratio & crosswalks applied #1")
