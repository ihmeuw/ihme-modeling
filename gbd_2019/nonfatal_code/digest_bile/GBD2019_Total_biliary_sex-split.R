...
# PREP INPUT DATA --------------------------------------------------------
##Merging literature data with clinical data
dt1 <- fread(paste0(dem_dir, "FILEPATH_LITERATURE_DATA"))
dt1$cv_ms2000 <-0
dt1$cv_marketscan <-0
dt1$cv_hospital <-0
dt1 <- subset(dt1, cv_exc_cholecystectomy==0)

dt2 <- fread(paste0(dem_dir, "FILEPATH_ADMIN_DATA"))
dt2$cv_literature[dt2$cv_literature == 0 | is.na(dt2$cv_literature)] <-0
dt2 <- subset(dt2, cv_literature!=1)
dt2$cv_exc_cholecystectomy <-0
dt2$cv_ultrasonology <- 0
...
write.csv(appended, "FILEPATH", row.names = F)
