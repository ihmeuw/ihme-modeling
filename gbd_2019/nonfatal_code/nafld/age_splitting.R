...
a_cause <- OBJECT
name <- OBJECT
bundle_version_id <- OBJECT ## this is the meid for iterative or wherever age split Dismod was run (id trach low - 23905, trach blind - 24393) (VERSION # trachoma low - 390887, trachoma blind - 404969; both iterative)
ver_id <- OBJECT ## Dismod version used for age split
step <- OBJECT #step for Dismod version used for age split
...
# GET FUNCTIONS -----------------------------------------------------------
functions_dir <- paste0(j_root, "temp/central_comp/libraries/current/r/")
...
source("/home/j/temp/central_comp/libraries/2017_archive/r/get_draws.R")
...
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = id, 
                           ...
                           version_id = ver_id, sex_id = c(1,2), gbd_round_id = 5, 
...
final_dt <- final_dt[group_review != 0, ]
