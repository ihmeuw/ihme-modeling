...
if(calculate_prevalences==T){
  for(me in c(upload_mes)){
    ...
    invisible(lapply(c("tx", "sympt", "asympt"), function(x){
      temp_folder<-paste0(output_folder, "/", me, "/", x, "_draws_cvd/")
    ...
    for(year in c(seq(from=1990, to=2010, by=5), 2015, 2017, 2019)){
      for(sex in c(1, 2)){
        ...
        command <- paste0("qsub -l m_mem_free=4G -l fthread=2 -P ", proj," -q all.q -N ", paste0("sympt_", me, "_", year, "_", sex),
                          #" /share/code/cvd/cvd_nrvd/model/severity_splits/shell.sh ",
  ...
  for(me in upload_mes){
    folder<-paste0(output_folder, "/", me, "/sympt_draws_cvd/")
    ...
    if(length(miss)>0){
      ...
      if(!all(file.exists(paste0(output_folder, "/", me, "/sympt_draws/", miss)))){stop("Resubmission jobs broke!")}
...
if(save_upload==T){
  for(me in c(upload_mes)){
    sympt_me<-ifelse(me=="aort", 19671, 19672)
    asympt_me<-ifelse(me=="aort", 19386, 19387)
    tx_me<-ifelse(me=="aort", 19578, 19582)
    
    
    folder<-paste0(output_folder, "/", me, "/tx_draws_cvd/")
    ...
    folder<-paste0(output_folder, "/", me, "/asympt_draws_cvd/") 
    save_results_epi(input_dir=folder, input_file_pattern="{year_id}_{sex_id}_{measure_id}.csv",
