## Severe, acute, uncomplicated gastritis/duodenitis; re-preparing GBD 2017 data with GBD 2019 data-preparation methods
...
agd_dt <- get_raw_data(3201, 'step2', bundle_version = 0)
#Bundle version ID: 7955

## Store bundle columns for later
bundle_columns <- names(agd_dt)
...
get_cases_sample_size(agd_dt)
get_se(agd_dt)

## Add variables for use in data-prep that will later need to be dropped to pass Uploader validations
agd_dt <- get_definitions(agd_dt)
agd_dt <- merge(agd_dt, loc_dt[ , c("location_id", "super_region_id", "region_id")], by = "location_id")

## Prep raw data for match-finding, crosswalking and transformation
agd_findmatch <- copy(agd_dt)

agd_findmatch <- subnat_to_nat(agd_findmatch)
agd_findmatch <- calc_year(agd_findmatch)

scatter_bydef(agd_findmatch, upper=0.01)

## Find matches
age_dts <- get_age_combos(agd_findmatch)
agd_findmatch <- age_dts[[1]]
age_match_dt <- age_dts[[2]] 

pairs <- combn(agd_findmatch[, unique(definition)], 2)
#combn function generates all combinations of the elements of vector x (in this case, a vector of the unique values found in the definition column of the data.table agd_findmatch) taken m (in this case, 2) at a time

matches <- rbindlist(lapply(1:dim(pairs)[2], function(x) get_matches(n = x, pair_dt = agd_findmatch)))
matches[ , nid_comb:=paste0(nid, nid2)]

write.xlsx(matches, paste0(out_path, "/3201_matches_", date, ".xlsx"), col.names=TRUE)
...
agd_fit <- run_mr_brt(
  output_dir = out_path,
  model_label = paste0("agd_xwmodel_", date),
...
agd_transform <- agd_dt[definition!="reference", ]
if ("crosswalk_parent_seq" %in% names(agd_transform)) {
  agd_transform[is.na(crosswalk_parent_seq), `:=` (crosswalk_parent_seq = seq, seq = NA)]
  agd_transform[!is.na(crosswalk_parent_seq), seq = NA] 
} else {
  agd_transform[, `:=` (crosswalk_parent_seq = seq, seq = NA)]
}

# Predict crosswalk for training data
agd_trainingprediction <- unique(predict_xw(agd_fit, "logit_dif"), by = cv_alts)
# Predict crosswalk for raw data
agd_predictnew <- unique(predict_xw(agd_fit, "logit_dif", agd_transform), by = cv_alts)

# Transform data that need and can be crosswalked
agd_transform <- transform_altdt(agd_transform, agd_predictnew, "logit_dif")

# Bind reference data, crosswalked data; make scatter-plot
agd_dt2 <- rbind(agd_transform, agd_dt[definition=="reference", ], fill=TRUE)
scatter_bydef(agd_dt2, raw = FALSE, upper = 0.01)
# Now drop data that could not be crosswalked or that were group-reviewed out; none for this bundle, so will just make a copy
agd_modeling <- copy(agd_dt2)
...
agd_outliered <- auto_outlier(agd_modeling)
scatter_markout(agd_outliered, upper = 0.01)

#This is where you would bind back in other measures that were dropped before crosswalking if bundle had any
...
agd_final <- agd_outliered[, ..columns_keep]

#Write the data to an Excel file in order to upload it  
upload_path <- paste0(j, "FILEPATH/3201_", date, ".xlsx")
write.xlsx(agd_final, upload_path, col.names=TRUE, sheetName = "extraction")

#Then add a description and upload
description <- "Same approach as XWV 875, but re-pulled clinical data for corrected SGP claims and fixed my automated outliering"
compgd_upload_xw <- save_crosswalk_version(bundle_version_id=7955, data_filepath=upload_path, description = description)