...
df <- readxl::read_xlsx("FILEPATH", sheet = 'FILEPATH')
...
readxl::excel_sheets('FILEPATH')
dat_original <- readxl::read_xlsx('FILEPATH', sheet = 'extraction')
#NOT READING THE UNDERLYING NID


#as sex splitting already took care of this 
...
dat_original_mansoni<- subset(dat_original, case_name=="S mansoni" | case_name=="S intercalatum"| case_name=="S mekongi")
...
length(which(dat_original_mansoni$case_diagnostics == "kk1")) #8069
length(which(dat_original_mansoni$case_diagnostics == "kk2")) #147
length(which(dat_original_mansoni$case_diagnostics == "kk3")) #45
length(which(dat_original_mansoni$case_diagnostics == "cca")) #1
length(which(dat_original_mansoni$case_diagnostics == "pcr")) #5
length(which(dat_original_mansoni$case_diagnostics == "elisa")) #6
length(which(dat_original_mansoni$case_diagnostics == "sed")) #105
length(which(dat_original_mansoni$case_diagnostics == "fec")) #51

#making dummy variables (turning long to wide format)
dat_original_mansoni <- dat_original_mansoni %>%
  #filter(!cases == 0) %>%
  mutate(
...
cov_names <- c("elisa","kk1","kk2", "cca", "pcr", "sed", "fec") 
...
tmp_orig <- as.data.frame(dat_original_mansoni) %>%
  .[, orig_vars] %>%
...
#mean =1 is missing standard error, lower CI and high CI. 
#mean =0 is missing standard error and higher CI. Below are calculations to deal with these missing numbers:
...
write.csv(final_data_mansoni_kk3_ref, file="final_data_mansoni_kk3_ref_decomp_step2.csv")

#final_data_mansoni_kk3_ref needs to be binded in R with hema and japonicum after running those two scripts 

final_crosswalked_data <- rbind(final_data_mansoni_kk3_ref, final_data_japon, final_data_hema)

write.csv(final_crosswalked_data, file="FILEPATH")
...
###appending the different specie files together####
