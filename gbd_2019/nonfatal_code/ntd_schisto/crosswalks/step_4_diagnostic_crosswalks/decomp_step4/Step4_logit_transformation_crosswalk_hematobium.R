...
dx_data_h <- readxl::read_xlsx("FILEPATH", sheet = 'heama')

openxlsx::write.xlsx(dx_data_h, file = 'FILEPATH')

hema_dx_data <- readxl::read_xlsx("FILEPATH")
...
data_sex_adj_hema_for_dx<- subset(final_sex_split_data, case_name=="S haematobium")


data_sex_adj_hema_for_dx<- subset(data_sex_adj_hema_for_dx, sample_size!=cases)
#1417


#will term mean and standard error as pre-adjusted so we can compare the adjusted with these 
colnames(data_sex_adj_hema_for_dx)[colnames(data_sex_adj_hema_for_dx)=="mean"] <- "pre_adjusted_mean"
colnames(data_sex_adj_hema_for_dx)[colnames(data_sex_adj_hema_for_dx)=="standard_error"] <- "pre_adjusted_standard_error"
colnames(data_sex_adj_hema_for_dx)[colnames(data_sex_adj_hema_for_dx)=="upper"] <- "pre_adjusted_upper"
colnames(data_sex_adj_hema_for_dx)[colnames(data_sex_adj_hema_for_dx)=="lower"] <- "pre_adjusted_lower"


data_sex_adj_hema_for_dx$pre_adjusted_mean[data_sex_adj_hema_for_dx$cases==0] <- 0

unique(data_sex_adj_hema_for_dx$case_diagnostics)
#"filt" "sed"  "cen"  "pcr"  "dip" 
#no more cca


data_sex_adj_hema_for_dx <- data_sex_adj_hema_for_dx %>%
  mutate(
...
tmp_orig <- as.data.frame(data_sex_adj_hema_for_dx) %>%
  .[, orig_vars] %>%
...
#mean =1 is missing standard error and lower and higher CI. Below are calculations to deal with these missing numbers:
...
colnames(final_data_hema)[colnames(final_data_hema)=="lo_adjusted"] <- "lower"
colnames(final_data_hema)[colnames(final_data_hema)=="hi_adjusted"] <- "upper"
