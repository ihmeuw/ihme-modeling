...
dx_data_j <- readxl::read_xlsx("FILEPATH", sheet = 'japonicum')

#saving japonicum sheet it in ihme/ntds
openxlsx::write.xlsx(dx_data_j, file = 'FILEPATH')

japon_dx_data <- readxl::read_xlsx("FILEPATH")
...
data_sex_adj_japon_for_dx<- subset(final_sex_split_data, case_name=="S japonicum")

data_sex_adj_japon_for_dx<- subset(data_sex_adj_japon_for_dx, sample_size!=cases)
#

#keep the means, SE, CIs cases, sample size from sex splits

#will term mean and standard error as pre-adjusted so we can compare the adjusted with these 
colnames(data_sex_adj_japon_for_dx)[colnames(data_sex_adj_japon_for_dx)=="mean"] <- "pre_adjusted_mean"
colnames(data_sex_adj_japon_for_dx)[colnames(data_sex_adj_japon_for_dx)=="standard_error"] <- "pre_adjusted_standard_error"
colnames(data_sex_adj_japon_for_dx)[colnames(data_sex_adj_japon_for_dx)=="upper"] <- "pre_adjusted_upper"
colnames(data_sex_adj_japon_for_dx)[colnames(data_sex_adj_japon_for_dx)=="lower"] <- "pre_adjusted_lower"


data_sex_adj_japon_for_dx$pre_adjusted_mean[data_sex_adj_japon_for_dx$cases==0] <- 0

unique(data_sex_adj_japon_for_dx$case_diagnostics)
#"kk1"   "kk2"   "kk3"   "iha"   "elisa" 

data_sex_adj_japon_for_dx <- data_sex_adj_japon_for_dx %>%
  #filter(!mean_new == 0) %>%
  mutate(
...
tmp_orig <- as.data.frame(data_sex_adj_japon_for_dx) %>%
  .[, orig_vars] %>%
...
colnames(final_data_japon)[colnames(final_data_japon)=="lo_adjusted"] <- "lower"
colnames(final_data_japon)[colnames(final_data_japon)=="hi_adjusted"] <- "upper"

write.csv(final_data_japon, file="FILEPATH")

#final_data<- read.csv("FILEPATH")
#plots - final_data 
library(ggplot2)
