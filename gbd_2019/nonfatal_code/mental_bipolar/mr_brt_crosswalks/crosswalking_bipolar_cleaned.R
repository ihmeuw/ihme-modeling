...
bundle_id <- 161
acause <-"mental_bipolar"
covariates <- c("cv_recall_lifetime", "cv_bipolar_recall_point")
uses_csmr <- F
test_sex_by_super_region <- F
crosswalk_pairs <- 'FILEPATH.csv'
age_sex_split_estimates <- "FILEPATH.xlsx"
need_to_age_split <- F
need_save_bundle_version <- 8381 # Set as true to save bundle version, otherwise specify bundle version here
sex_ratio_by_age <- F
...
sex_ratios[, mc_mid_age := mid_age - mean_mid_age]
...
sex_ratios[, prev_age := cv_prevalence * mc_mid_age]
sex_ratios[, mtstand_age := cv_mtstandard * mc_mid_age]
...
sex_ratios[, cv_64 := ifelse(super_region_id == 64, 1, 0)]
sex_ratios[, cv_103 := ifelse(super_region_id == 103, 1, 0)]
# Will make 4, 137, and 158 the reference
sex_locations <- c(64)
...
review_sheet[seq %in% c(sex_parents, age_parents) & is_outlier == 1,] 
...
review_sheet_final[is.na(cv_recall_lifetime), cv_recall_lifetime := 0] 
...
write.csv(review_sheet_final, crosswalk_save_file, row.names = F, na = "")

##### Upload crosswalked dataset to database -----------------------------------------------------------------------

save_crosswalk_version(v_id, crosswalk_save_file, description = "Parent_seq + mean = 0 corrections")
