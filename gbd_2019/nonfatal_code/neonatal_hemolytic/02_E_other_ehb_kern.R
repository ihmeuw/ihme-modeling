...
rh <- fread(file = 'FILEPATH/rh_disease_ehb_all_draws.csv')
g6pd <- fread(file = 'FILEPATH/g6pd_ehb_all_draws.csv')
preterm <- fread(file = 'FILEPATH/preterm_ehb_all_draws.csv')

#restrict Rh data down to just DisMod estimation years and specified sexes
rh <- rh[year %in% c(1990,1995,2000,2005,2010,2015,2017,2019)]
rh <- rh[sex %in% c(1,2)]
...
kern_long <- melt.data.table(kern_scalars, id.vars = names(kern_scalars)[!grepl("draw", names(kern_scalars))], 
                             measure.vars = patterns("draw"),
                             variable.name = 'draw.id')
setnames(kern_long, 'value', 'kern_scalar')

#merge on location, year and draw_id
prev_long <- merge(other_long, kern_long, by = c('location_id','year','draw.id'), all.x = TRUE)

prev_long[, kern_prev := value.ehb * kern_scalar]
...
write.csv(ehb_prev_wide, file = 'FILEPATH/other_ehb_all_draws.csv',
          row.names = FALSE)
write.dta(ehb_prev_wide, file = 'FILEPATH/other_ehb_all_draws.dta')
...
write.csv(kern_prev_wide, file = 'FILEPATH/other_kernicterus_all_draws.csv',
          row.names = FALSE)
write.dta(kern_prev_wide, file = 'FILEPATH/other_kernicterus_all_draws.dta')
...
write.csv(ehb_prev_wide, file = 'FILEPATH/other_ehb_summary_stats.csv',
          row.names = FALSE)
write.dta(ehb_prev_wide, file = 'FILEPATH/other_ehb_summary_stats.dta')
...
write.csv(kern_prev_wide, file = 'FILEPATH/other_kernicterus_summary_stats.csv',
          row.names = FALSE)
write.dta(kern_prev_wide, file = 'FILEPATH/other_kernicterus_summary_stats.dta')
