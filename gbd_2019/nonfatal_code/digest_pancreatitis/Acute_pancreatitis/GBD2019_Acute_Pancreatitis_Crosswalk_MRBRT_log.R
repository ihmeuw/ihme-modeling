...
cause_path <- "Acute_pancreatitis"  #SPECIFY FILEPATH
...
location_match <- "exact"
year_range <-0       #SPECIFY YEAR RANGE FOR BETWEEN STUDY COMPARISON
covariate_name <- "cv_stringent"   #set covariate of interest for crosswalk (alt)


#CREATE COVARIATES AND TAG REFERENCE vs. ALTERNATIVE
df <- as.data.table(read.csv("FILEPATH"))
df$cv_stringent <- ifelse((df$cv_chart_review==1  & df$cv_faap==0 & df$cv_excludes_aonc==0) ,1,0)
df = within(df, {cv_hospital = ifelse( clinical_data_type=="inpatient" | location_name=="Taiwan", 1, 0)})
...
#CREATE FUNCTION TO ORDER COLUMNS OF CROSSWALK CSV FILES CONSISTENTLY AND DROP UN-NEEDED COLUMNS
  ...
  reorder_columns <- function(datasheet){
    ...
    template_cols <- c("location_match", "age_start_match", "age_end_match", "sex", "log_ratio", "delta_log_se", 
                       "cv_marketscan.y", "cv_ms2000.y", "cv_stringent.y",
...
#SET YOUR REFERENCE IF ALREADY NOT DONE
  ...
  dat_original = within(dat_original, {cv_stringent = ifelse( df$cv_chart_review==1  & df$cv_faap==0 & df$cv_excludes_aonc==0 ,1,0)})
  ...
  cov_names <- c("cv_ms2000", "cv_marketscan", "cv_stringent") # can be a vector of names 
...
#    NOTE: ratio must be specified as alternative/reference
  tmp_metareg <- read.csv(master_xwalk) %>%
    mutate(cv_ms2000 = if_else(cv_ms2000.y == reference_value, 1, 0, 0), cv_marketscan = if_else(cv_marketscan.y == reference_value, 1, 0, 0), cv_stringent = if_else(cv_stringent.y == reference_value, 1, 0, 0))
...
#STANDARDIZE VARIABLE NAMES IN THE ORIGINAL EXTRACTED DATASHEET
  ...
  tmp_orig$cv_ms2000[is.na(tmp_orig$cv_stringent)] <-0
...
#########################################################################################################################################
  ...
  covariate_name <- "_within"    #set covariate of interest for crosswalk (alt), used for the model name
...
#########################################################################################################################################
  ...
  setnames(predicted, c("X_cv_ms2000", "X_cv_marketscan", "X_cv_stringent"), c("cv_ms2000", "cv_marketscan", "cv_stringent"))
