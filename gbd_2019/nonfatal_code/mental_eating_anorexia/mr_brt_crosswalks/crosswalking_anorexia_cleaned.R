...
covariates <- c("cv_icd", "cv_lay_interviewer", "cv_recall_1yr", "cv_diag_clinical")
uses_csmr <- T
test_sex_by_super_region <- F
crosswalk_pairs <- '' # no crosswalks for Anorexia
age_sex_split_estimates <- "FILEPATH.xlsx"
need_to_age_split <- F
need_save_bundle_version <- 3422 # Set as true to save bundle version, otherwise specify bundle version here
sex_ratio_by_age <- F
...
sex_ratio <- sex_ratio[measure != "incidence"]
...
age_sex_split[, `:=` (study_covariate = "sex", sex_parent = NULL, seq = NULL)] # originally was seq = NA but somehow was duplicate column seq
age_sex_split[, `:=` (seq = NA)] #
