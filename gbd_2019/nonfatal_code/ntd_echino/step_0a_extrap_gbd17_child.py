...
draws = get_draws(source="epi",
                  ...
                  measure_id=5,
                  gbd_round_id=5)
draws.to_hdf(f'{draws_dir}/{meid}_gbd_17/all_draws.h5', key='draws', mode='w')
"""
import numpy as np
import pandas as pd
import scipy as sp
...
def linear_extrap(df):
    """
    Extend 2015-2017 to 2019 via a linear extrap on mean / CI
    """
    ...
    # transform each year mean/CI
    for year in [2015, 2017]:
        year_df = df[df.year_id == year].copy().reset_index(drop=True)
        year_df[f"mean_{year}"] = year_df[draw_cols].mean(axis=1)
        year_df[f"upper_{year}"] = year_df[draw_cols].quantile(0.95, axis=1)
        year_df[f"lower_{year}"] = year_df[draw_cols].quantile(0.05, axis=1)
        df_19 = df_19.merge(year_df[["age_group_id", "sex_id", "measure_id", f"mean_{year}", f"upper_{year}", f"lower_{year}"]], on=["age_group_id", "sex_id", "measure_id"], how="left")

    # Regress linear model per demographic.
    df_19["mean_2019"] = df_19.apply(lambda x: _predict_lm_2019( [x["mean_2015"], x["mean_2017"]]), axis=1 )
    df_19["lower_2019"] = df_19.apply(lambda x: _predict_lm_2019( [x["lower_2015"], x["lower_2017"]]), axis=1 )
    df_19["upper_2019"] = df_19.apply(lambda x: _predict_lm_2019( [x["upper_2015"], x["upper_2017"]]), axis=1 )

    # Expand draws as normal distribution / exponentiate to normal space
    df_19[draw_cols] = df_19.apply(lambda x: pd.Series( np.random.normal( x["mean_2019"],  abs(x["upper_2019"] - x["lower_2019"]) * 1.05, (1000) ) ), axis=1)
