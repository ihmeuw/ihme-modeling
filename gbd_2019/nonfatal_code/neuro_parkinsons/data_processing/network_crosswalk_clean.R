...
## I WAS MAPPING SUBNATS TO NATS BUT THEO TOLD ME TO STOP SO ITS COMMENTED OUT (NVMD I CONVINCED HIM THIS IS BETTER)
...
get_matches <- function(n, pair_dt, year_span = 10, age_span = 5){
  ...
  keep_vars <- c("nid", "location_id", "sex", "year_start", "year_end", "measure", "age_start", "age_end", "clinical_data_type",
  ...
  ## EXACT LOCS FOR MARKETSCAN
  mkscn_sub <- copy(matched[clinical_data_type == "claims" | clinical_data_type2 == "claims"])
  mkscn_sub <- mkscn_sub[location_id == location_id2]
  matched <- rbind(matched[clinical_data_type == "" & clinical_data_type2 == ""], mkscn_sub)
  ...
  ## AGE ROUNDING IF NOT MARKETSCAN
  unmatched$age_start <- unlist(parallel::mclapply(1:nrow(unmatched), function(i) get_closest_age(i, var = "age_start", dt = unmatched), mc.cores = 9))
  unmatched$age_end <- unlist(parallel::mclapply(1:nrow(unmatched), function(i) get_closest_age(i, var = "age_end", start = F, dt = unmatched), mc.cores = 9))
  unmatched$age_start2 <- unlist(parallel::mclapply(1:nrow(unmatched), function(i) get_closest_age(i, var = "age_start2", dt = unmatched), mc.cores = 9))
  unmatched$age_end2 <- unlist(parallel::mclapply(1:nrow(unmatched), function(i) get_closest_age(i, var = "age_end2", start = F, dt = unmatched), mc.cores = 9))
...
create_differences <- function(ratio_dt){
  ...
  ## GET RID OF DATA WHERE EITHER MEANS ARE 0
  dt <- dt[!(mean == 0) & !(mean2 == 0)]
...
park_dt <- predict_sex$final
park_dt <- park_dt[exclude_xwalk == 0 | is.na(exclude_xwalk)] ## GET RID OF EXCLUDE CROSSWALK == 1
park_dt[cv_nursing_home_excl == 0.5, cv_nursing_home_excl := 1] ## MODERATE SEARCHING IS STILL SUBOPTIMAL
park_dt <- aggregate_marketscan(park_dt)
park_dt <- get_cases_sample_size(park_dt)
park_dt <- get_se(park_dt)
park_dt <- calculate_cases_fromse(park_dt)
defs <- get_definitions(park_dt)
park_dt <- defs[[1]]
cvs <- defs[[2]]
park_dt <- subnat_to_nat(park_dt)
park_dt <- calc_year(park_dt)
age_dts <- get_age_combos(park_dt)
park_dt <- age_dts[[1]]
age_match_dt <- age_dts[[2]] ## HELPFUL FOR VETTING
pairs <- combn(park_dt[, unique(definition)], 2)
message("finding matches")
matches <- rbindlist(lapply(1:dim(pairs)[2], function(x) get_matches(n = x, pair_dt = park_dt)))
...
model_name <- paste0("parkinson_network_", date)

create_covs <- function(x){
  if (x %in% c("nursing_home_excl", "UKPD_Brain_Bank")){
    return(cov_info(x, "X", uprior_ub = 0))
  } else {
    return(cov_info(x, "X"))
  }
}
...
if (file.exists(paste0("FILEPATH"))){
  park_model <- readr::read_rds(paste0("FILEPATH"))
} else {
  park_model <- run_mr_brt(
  ...
  readr::write_rds(park_model, paste0("FILEPATH"))
}

park_predicts <- get_preds(park_model, mrbrt_vars)

forrestplot_graphs <- graph_combos(model = park_model, predictions = park_predicts)

# PREDICTION --------------------------------------------------------------

message("adjusting data")
adjust_dt <- predict_sex$final
adjust_preds <- get_preds_adjustment(raw_data = adjust_dt, model = park_model)
...
data_plot <- prediction_plot(adjusted$vetting_dt)
ggsave(data_plot, filename = paste0(mrbrt_dir, "FILEPATH"), width = 12)
