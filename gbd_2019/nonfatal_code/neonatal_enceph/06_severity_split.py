...
def get_days_cfr_dfs(birth_prev, cfr, acause):
    ...
    cfr_df = pd.read_csv('%s/cfr_28_days/%s_%s_draws.csv' % (data_dir, acause, cfr))
...
def get_mild_modsev_dfs(mild_prop, modsev_prop, acause):
    ...
    mild_df = pd.read_csv('%s/sev_prop_28_days/%s_%s_draws.csv' % (data_dir, acause, mild_prop))
    ...
    modsev_df = pd.read_csv('%s/sev_prop_28_days/%s_%s_draws.csv' % (data_dir, acause, modsev_prop))
...
## - Takes the CFR and subtracts it from 1, to get the survival rate 
## - days_df is the prev at 28 days
############################################################## 
...
def rescale_prev(cause_dict):
    ...
    if len(too_big_rows) == 0:
        ...
        scale_factor = old_div(squeeze_dict['total_symp_count'], summed)
...
def save_scale(acause_dict, acause, birth_prev):
    for prop_type in ['mild_count', 'modsev_count']:
        ...
        prev_df.to_csv('%s/sev_split_prev/%s/%s_scaled_check_py3.csv' % (test_outdir, birth_prev, prop_type))
...
def save_prev(acause_dict, acause, birth_prev):
    for prop_type in ['mild_prev', '0_0_modsev_prev', '0_7_modsev_prev', '7_28_modsev_prev', '28_28_modsev_prev', 'asymp_prev']:
        ...
        prev_df = prev_df[draw_cols]
        test_outdir = '%s/%s' % (parent_dir, acause)
        prev_df.to_csv('%s/sev_split_prev/%s/%s_final_prev_py3.csv' % (test_outdir, birth_prev, prop_type))
