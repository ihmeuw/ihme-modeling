...
covariates <- c("cv_lay_interviewer")
uses_csmr <- F
...
decomp_2_bundle_version <- 3845
need_save_bundle_version <- 14606
sex_ratio_by_age <- T
...
crosswalk_save_file <- paste0(crosswalk_save_folder, "crosswalk_", Sys.Date(), "fixed_decomp4.xlsx")
write.xlsx(review_sheet_final, crosswalk_save_file, sheetName = "extraction")

##### Upload crosswalked dataset to database -----------------------------------------------------------------------

save_crosswalk_version(v_id, '/FILEPATH.xlsx', description = "_Decomp 4 new data")
