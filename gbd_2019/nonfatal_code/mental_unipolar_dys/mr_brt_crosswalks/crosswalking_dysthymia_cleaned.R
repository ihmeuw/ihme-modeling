...
need_to_age_split <- F
need_save_bundle_version <- "BUNDLE VERSION" # Set as true to save bundle version, otherwise specify bundle version here
sex_ratio_by_age <- T
...
outlier_agesexsplit <- review_sheet[seq %in% c(sex_parents, age_parents) & is_outlier == 1, seq] # just checking if any are outliered
...
predicted <- predicted[cv_recall_1yr == 0 & cv_symptom_scales == 0 & cv_whs == 0,]
crosswalk_reporting <- copy(predicted) # for reporting later
covariates <- "d_lay_interviewer"
review_sheet_final[, `unnamed:.3` := NULL]
...
review_sheet_final[specificity == "", specificity := "sex"]
...
save_crosswalk_version(v_id, crosswalk_save_file, description = "Sex ratio by age - xwalk parent seq corrected")
