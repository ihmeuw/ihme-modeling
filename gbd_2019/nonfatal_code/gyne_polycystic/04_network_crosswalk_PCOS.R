...
sex_names <- fread("/FILEPATH/sex_names.csv")
...
all_split <- all_split[, c('nid', 'location_id', 'location_name', 'sex_id', 'age', "year_id", 'mean', 'standard_error', 'sample_size', cv), with = F]
...
if(length(match_split) == 2){
  ...
  if(match_type == 'between_study'){
    matched_merge <- merge(match_split[['1']], match_split[['0']],  by = c( 'age', 'sex_id'), all.x = TRUE, allow.cartesian=TRUE)
  }
...
fib_network <- rbind(all df with possible matches)
...
fib_network <- fib_network %>%
  mutate(cv_clinical = if_else(dx_1 == "clinical", 1, 0)) %>%
         ...
         mutate(cv_Rotterdam =  if_else(dx_1 == "Rotterdam", 1,0)) %>%
         mutate(cv_aes =  if_else(dx_1 == "aes", 1,0))
...
#dummy k =  -1  if k is the denominator
  ...
  fib_network$cv_Rotterdam[(fib_network$dx_2 == "Rotterdam")] <- -1
  fib_network$cv_aes[(fib_network$dx_2 == "aes")] <- -1

View(fib_network)  
  
write.xlsx(fib_network, 
                     file = paste0(save_dir, "network_prep/02_input_mrbrt_network/", bun_id, "_net_prep_allmatches.xlsx"),
                     row.names = FALSE)
...
matched_merge <- filter(matched_merge, dx_1 >0.005)
matched_merge <- filter(matched_merge, dx_2 >0.005)

dat_metareg <- as.data.frame(matched_merge)
extra_vars <- c("cv_clinical", "cv_self_report", "cv_Rotterdam", "cv_aes")
...
mod_lab <- paste0(bundle,  "_", 'network_logit_rerun_allmatches')
...
# -- read in data frame
#dat_original <- fread(paste0(save_dir, "network_prep/00_clinical_crosswalked/", bun_id, "__", "for_modeling.csv"))
bun_id <- 201
dat_original <- fread(paste0("FILEPATH", bun_id, "_for_modeling_s4.csv"))

dat_original <- as.data.table(dat_original)

dat_original <- dat_original %>%
  mutate(cv_clinical = ifelse(clinical_data_type == "literature", 0, 1)) %>%
  mutate(cv_rotterdam = 0) %>%
  mutate(cv_aes = 0) %>%
  mutate(cv_nih1990  = 0) %>%
...
dat_original[cv_nih1990  == 1, reference := 1]
dat_original[, age := round((age_start + age_end)/2)]
dat_original[, dx := "NA"]
dat_original[cv_self_report == 1, dx := "self_report"]
dat_original[cv_rotterdam == 1,  dx := "Rotterdam"]
dat_original[cv_aes == 1,  dx := "aes"]
...
cov_names <- c("cv_clinical", "cv_self_report", "cv_aes", "cv_rotterdam")
...
write.csv(final_data,
          file = paste0("/FILEPATH/", bun_id, "_adjusted_s4.csv"),
          row.names = FALSE)

## Delete duplicative mean and se rows for modeling
final_data$is_outlier[(final_data$mean == 0 & final_data$cv_clinical == 1)] <- 1 
final_data$is_outlier[(final_data$cv_marketscan == 1 & final_data$year_start == 2000)] <- 1 
...
for_modeling$is_outlier[(for_modeling$age_start >= 90)] <- 1 
for_modeling$is_outlier[(for_modeling$mean == 0 & for_modeling$cv_inpatient == 1)] <- 1 


write.csv(for_modeling,
          file = paste0("/FILEPATH/", bun_id, "_for_modeling_s4.csv"),
          row.names = FALSE)

################

for_modeling <- for_modeling %>%
  select(-(age:hi_adjusted))
...
write.xlsx(to_upload,
          file = paste0("/ihme/mnch/crosswalks/gyne/network_prep/05_to_upload/step4/", bun_id, "_to_upload_s4.xlsx"),
