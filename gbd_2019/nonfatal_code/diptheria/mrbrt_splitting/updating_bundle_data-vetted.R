#----HEADER-------------------------------------------------------------------------------------------------------------
# Author:  "USERNAME"
# Purpose: Integrating GBD 2019 Diphtheria CFR systematic review with existing bundle data
# Run:     
#***********************************************************************************************************************
...
model_label <- "additional_2019_outliering"  
AGG         <- FALSE  
acause      <- "diptheria"
bundle_id   <- 407  
old_step    <- "step4" 
...
### objects
input_dir    <- paste0(j_root, "/"FILEPATH"/diptheria/"FILEPATH"")
version_dir  <- paste0(j_root, "/"FILEPATH"/diptheria/"FILEPATH"versions")
adj_data_dir <- paste0(j_root, "/"FILEPATH"/diptheria/407/03_review/01_download/")
save_dir     <- paste0(j_root, "/"FILEPATH"/diptheria/407/01_input_data/02_crosswalk/")
brt_out_dir  <- paste0(j_root, "/"FILEPATH"/diptheria/"FILEPATH"")
...
### pull current bundle (not bundle version or crosswalk version)
...
### read in exported Step 2 bundle (GBD 2019; file name specific to a manually-cleaned Step 2 download)
step2 <- read_excel(""FILEPATH"diptheria_407_2019-09-12_manual_cleaned.xlsx") %>% data.table

### read in new Step 4 data 
step4 <- read_excel(""FILEPATH"Diphtheria_CFR2019_extraction_step4clean.xlsm") %>% data.table
step4[sex=="both", sex := "Both"]
step4[urbanicity_type=="mixed/both", urbanicity_type := "Mixed/both"]

### append together
updated <- rbind(step2, step4, fill=TRUE)

### clean columns in preparation for upload 
updated[is.na(group_review), `:=` (group=1, specificity="total", group_review=1)]  
updated[is.na(is_outlier), is_outlier := 0]
updated[, seq := ""]
updated[, seq_parent := ""]
updated[nid==279013 & location_name=="Nigeria", `:=` (location_name="Borno", location_id=25325)]  
updated[sample_size <= 5, is_outlier := 1]

# first save combined
openxlsx::write.xlsx(updated, file=path_to_bundle_data, sheetName="extraction", row.names=FALSE, col.names=TRUE)

# subset to the original extractions and save
updated_i <- updated[!is.na(mean) & !is.na(lower) & !is.na(upper)]

# subset to new extractions (i.e. only have cases/sample_size) and save
updated_ii <- updated[is.na(mean) & is.na(lower) & is.na(upper) & !is.na(cases) & !is.na(sample_size)]
updated_ii <- updated_ii[cases > 0]
...
### upload updated_i bundle (i.e. new or re-extracted) to GBD 2019 Step 4
...
sex_adj_dt[, seq := NA]
# group, specificity, and group_review either have to be all filled in or not filled in at all
...
input_data_ss_ws_as <- apply_age_split(data = input_data_ws_ss,
                                       ...
                                       write_out = TRUE,
                                       out_file = file.path(bundle_dir, acause, bundle_id, ""FILEPATH"", paste0(acause, bundle_id, date, "_ss_ws_as_data.xlsx")))
...
# define file path for saving
path_to_final_crosswalk_data <- file.path(bundle_dir, acause, bundle_id, ""FILEPATH"", paste0(acause, bundle_id, date, "_final_crosswalk_data_ss_as_step4.xlsx"))
...
# 'save' call
bundle_metadata_step4 <- fread(file = file.path(version_dir, paste0("bv_metadata_step4.csv")))
crosswalk_description <- "Final crosswalk data, sex-split and age-split -- Step 4"
crosswalk_metadata <- save_crosswalk_version(bundle_version_id = bundle_metadata_step4$bundle_version_id[1], data_filepath = path_to_final_crosswalk_data,
                                             description = crosswalk_description)
fwrite(crosswalk_metadata, file = paste0(input_dir, "final_crosswalk_metadata_step4.csv"), row.names = F)  ## UGH this didn't save??
#***********************************************************************************************************************
...
### useful objects
xw_v_id <- 10010
path_to_edited_xwv <- file.path(bundle_dir, acause, bundle_id, ""FILEPATH"", paste0(acause, bundle_id, date, "_edited_final_crosswalk_data_ss_as_step4.xlsx"))
note <- "NOTE"
...
xw_edited <- xwv[nid==133385 & location_name=="Turkey" & year_start==2001, is_outlier := 1]
...
#########
######### If only adjusting existing data!!!
#########

# get data
xw_v_id <- 11285  # pulled from EpiViz...
path_to_edited_xwv <- file.path(bundle_dir, acause, bundle_id, ""FILEPATH"", paste0(acause, bundle_id, date, "_edited_final_crosswalk_data_ss_as_step4_new_outliers.xlsx"))
note <- "New un-outliering and outliering E,C Europe and NAf"
...
# outliering 9/28:
diph[nid==416122, is_outlier := 1]  
diph[nid==139060, is_outlier := 1]  
...
# outliering 10/01:
diph[nid==279013, is_outlier := 1]  
diph[nid==416131, is_outlier := 1]  

# make 'crosswalk_parent_seq' the 'seq'
diph[, crosswalk_parent_seq := seq]

# re-save as .xlsx file which will point to in 'save' call
write.xlsx(diph, file=path_to_edited_xwv, sheetName="extraction", row.names=FALSE, col.names=TRUE)
