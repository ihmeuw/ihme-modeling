...
source("FILEPATH") ## USERNAMES'S helper functions 
...
match_mr_brt <- function(data,
  ...
  for (n in 1:ncol(comb)) {
    ...
    ## - it's likely we update the way we do this matching.
...
## USERNAME's function
##USERNAME: identify rows where mean is contained inside adjusted SEWhat is saved when you make a "commit"?
find_mrbrt_overlaps<-function(model_obj, adjust_zcovs=T){

  ##USERNAME: pull in data
  ...
  ##USERNAME: calculate upper/lower
