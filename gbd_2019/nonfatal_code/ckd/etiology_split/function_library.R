...
pred<-function(pred_df, coef_df, stage_name, etio_name, cov_dt){
  ...
  # Predict by multiplying your predictor matrix by the draws of your regression coefs
  ...
  # Bind you predcitor matrix to your predictions to make sure everything is labeled
...
process_draws<-function(pred_dt){
  ...
  # Drop original draws and denoms -- rename props as draws
...
format_draws<-function(pred_dt, dm_correction, dm_1_me, dm_2_me, loc_id=NULL, proportion_dir, decompstep){
  ...
  if (dm_correction==T){
    # Diabetes prevalence correction -- try to make prevalence of ckd due to DM1 less than prev DM1 in each location/year/age/sex
    if (loc_id!=102){
      ...
      # temporary fix: use best DM models from GBD2019 for GBD2020 because the new GBD2020 DM models aren't finalized yet and have issues in Brazil as of 9/25/20 . . .
      fix <- 1
      loc_id2 <- loc_id
      ...
      message("merging dm draws with predictions. correcting precitions")
...
plot_predictions<-function(dir, pred_dt, plot_version, cohort){
  ...
  print(gg)
  dev.off()
  message("done plotting. review these before running the prediction code to make sure they look ok --
        don't waste cluster resources by generating dumb predictions")

}

# format_draws_backfill<-function(pred_dt, dm_correction, dm_1_me, dm_2_me, loc_id=NULL, proportion_dir,gbd_round_id){
#   message("THIS IS JANKY FOR DECOMP STEP 1 DONT USE THIS FUNCTION OTHERWISE")
#   # source get_draws from gbd_2017
#   source_shared_functions("get_draws",gbd_rnd=gbd_round_id)
#   
#   # Fix all the metadata stuff
#   pred_dt[,sex_id:=ifelse(sex_id==0,1,2)]
#   pred_dt[,measure_id:=18]
...
#   # Create a data.table of all the id combos for which we need results (given by get_demographics)
#   message("getting demographics to save for")
#   demographics<-get_demographics("epi",gbd_round_id = gbd_round_id)
#     # add 2015 and 2019
#     demographics$year_id<-sort(append(demographics$year_id,values = c(2015,2019)))
#   demographics[["location_id"]]<-NULL
#   template<-as.data.table(expand.grid(demographics))
#   pred_dt<-merge(pred_dt,template,by=c("age_group_id","sex_id"),allow.cartesian = T)
#   
#   # No DM results for new WHO locs 
#   problem_locs <- c(320, 416, 393, 396, 367, 369, 374, 380, 413)
#   
#   if (dm_correction==T&!(loc_id%in%problem_locs)){
#     # Diabetes prevalence correction -- try to make prevalence of ckd due to DM1 less than prev DM1 in each location/year/age/sex
#     if (loc_id!=102){
#       message("beginning diabetes correction process")
#       # Set draw names
...
#                           sex_id = c(1,2), status = "best",gbd_round_id = gbd_round_id)
#       # Melt draws 
#       message("processing dm draws")
#       id_vars<-c("age_group_id","sex_id","year_id","location_id","modelable_entity_id")
#       dm_draws_long<-melt(dm_draws[,-c("model_version_id","measure_id","metric_id"),with=F],id.vars = id_vars,variable.name = "draw",
#                           value.name = "prev")
#       # Fill 2015 and 2019 with 2017 values
#       fill_dt<-copy(dm_draws_long[year_id==2017])
#       fill_dt_2019<-copy(fill_dt)
#       fill_dt_2019[,year_id:=2019]
#       fill_dt_2015<-copy(fill_dt)
#       fill_dt_2015[,year_id:=2015]
#       dm_draws_long<-rbindlist(list(dm_draws_long,fill_dt_2015,fill_dt_2019),use.names = T)
#       # Set etiology name
#       dm_draws_long[,etio:=ifelse(modelable_entity_id==dm_1_me,"dm1","dm2")]
#       dm_draws_long[,modelable_entity_id:=NULL]
#       # Cast wide by DM type 
...
#   
#   # Subnat fix 
#   new_subnats<-c(51,86,165,16,214)
#   if (loc_id %in% new_subnats){
#     source_shared_functions("get_location_metadata")
#     loc_dt<-get_location_metadata(22)
#     child_locs<-loc_dt[parent_id==loc_id,location_id]
#     
#     for (cl in child_locs){
#       # Add location_id
#       cl_dt<-copy(pred_dt)
#       cl_dt[,location_id:=cl]
#       
#       # Write files for each stage/etio/location
#       message(paste("writing files to", proportion_dir))
#       keep<-c(names(cl_dt)[grepl("draw_",names(cl_dt))],names(cl_dt)[grepl("_id",names(cl_dt))])
#       for (s in unique(cl_dt[,stage])){
#         for (e in unique(cl_dt[,etio])){
#           dt<-copy(cl_dt[stage==s&etio==e,])
#           dt<-dt[,keep,with=F]
#           dir<-paste0(proportion_dir,s,"/",e,"/")
#           write.csv(dt,paste0(dir,cl,".csv"),row.names = F)
#           message(paste("wrote", dir,cl,".csv"))
#         }
#       }
#       
#     }
