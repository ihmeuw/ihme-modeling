...
#'
#' @Implementation: This steps template is very similar to the GBD2013 template, but it doesn't make assumptions about where files should be stored. In 2013, all results were a 
#'                  function of the "functional" and "gbd" arguments. Since functional_groups no longer 
...
model_custom <- function(code_dir, out_dir, tmp_dir, date, steps, in_dir, ds, new_cluster) {
  ...
  # Load steps template
  step_sheet <- read.xlsx(file.path(code_dir,"encephalitis_steps.xlsx"), sheet="steps")
