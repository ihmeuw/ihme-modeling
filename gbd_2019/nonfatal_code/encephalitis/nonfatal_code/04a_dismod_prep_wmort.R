...
epilepsy_encephalitis <- 2821
long_modsev_encephalitis <- 2815
# ------------------------------------------------------------------------------
...
cp.mort.dt <- read_excel(path = file.path(in_dir, paste0(step_num, '_', step_name), "CP Mortality update for GBD2013.xls"), sheet = "SMR values for neonatal")
setDT(cp.mort.dt)
cp.mort.dt <- cp.mort.dt[Cause =="NE"] ## neonatal encephalopathy
cp.mort.dt <- cp.mort.dt[, parameter := NULL]
setnames(cp.mort.dt, c("UL", "LL"), c("upper", "lower"))
cp.mort.dt$modelable_entity_id <- numeric()
cp.mort.dt$modelable_entity_name <- ''
cp.mort.dt$sex_id <- numeric()
cp.mort.dt$year_id <- numeric()
cp.mort.dt$location_id <- numeric()
cp.mort.dt$measure <- "mtexcess"

saveRDS(cp.mort.dt, file.path(out_dir, "02_temp", "03_data", "long_modsev_smr.rds"))
# ------------------------------------------------------------------------------
