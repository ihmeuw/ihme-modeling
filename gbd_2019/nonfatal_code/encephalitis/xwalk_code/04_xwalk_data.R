...
#' @note uses GBD 2019 population for sample sizes

rm(list=ls())

pacman::p_load(openxlsx, parallel, pbapply, boot)
...
ds <- 'step4'   # decomp step
bv_id <- 18875  # bundle version id () includes literature and clinical data
...
sex_model_name <- "2020_01_03_encephalitis_sexsplit"

ms2000_xwalk_model_name <- "2020_01_14_claims_2000_old_mrbrt_params_age_spline_CF3"
ms_xwalk_model_name <- "2020_01_14_claims_2010_2016_age_spline_CF3"
surv_xwalk_model_name <- "2020_01_14_surveillance_null_CF3"

# Specify the column names for the reference and alternative study covariates
ref_name  <- c("cv_inpatient")
alt_names <- c("cv_surveillance", "cv_marketscan_data", 
...
# SOURCE FUNCTIONS --------------------------------------------------------
k <- # filepath
source(paste0(k, "get_population.R"))
source(paste0(k, "get_age_metadata.R"))
source(paste0(k, "validate_input_sheet.R"))
source(paste0(k, "upload_bundle_data.R"))
...
fit_ms2000 <- readRDS(paste0(model_objects_dir, ms2000_xwalk_model_name, ".RDS")) # there was no new marketscan 2000 claims data
...
fwrite(ms_2000_model_summary_dt, paste0(model_objects_dir,"step4_", "MS_2000_predict_summary.csv"))
...
fwrite(ms_data_model_summary_dt, paste0(model_objects_dir,"step4_", "MS_claims_predict_summary.csv"))
...
fwrite(surv_model_summary_dt, paste0(model_objects_dir,"step4_", "surveillance_claims_NULL_predict_summary.csv"))
...
fwrite(xwalk_final_dt, paste0(mrbrt_dir, date, "step4_xwalk", ".csv"))
...
global_cases_dt <- fread(age_pattern_path) # pulls in age pattern csv
...
write.xlsx(final_age_split_dt, file = xwalk_out_path, sheetName = "extraction")

result <- save_crosswalk_version(bundle_version_id = bv_id, 
                                 data_filepath = xwalk_out_path, 
                                 description = "USERNAME decomp exempt xwalk CF3 incl all data")
