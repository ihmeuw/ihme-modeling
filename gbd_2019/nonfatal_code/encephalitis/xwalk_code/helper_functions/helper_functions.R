...
# CHECK FOR NON-DISMOD LOCATIONS
find_nondismod_locs <- function(raw_dt) {
  ...
  if (nrow(nondismod_locs) > 0) {
    print(paste0("The following non-DisMod locations exist: ", unique(nondismod_locs$location_name)))
  } else {
...
get_study_cov <- function(raw_dt) {
  ...
  #'       currently have separate identifiers for different claims data. 
  ...
  additional_inpatient_nids <- c(104918, 104919, 135150, 135151, 114858)
  # add january 2020 additional NIDs that appear to be inpatient
  
  dt[nid %in% additional_inpatient_nids, cv_inpatient := 1]
...
j <- # filepath
get_additional_xwalk_data <- function() {
  dt <- fread(paste0(j, "format_encephalitis_incidence_USERNAME_June192019.csv"))
  return(dt)
}
...
adjust_row <- function(row) {
  #' @description adjust_row using this 
  #' 
  #' @param row data.table. a single row data table to be split
  #' @return row data.table. a single row data table to be split
  #' @details more details
  #' @note notes about function
  #' 
  #' 
...
plot_mean_mean_adj <- function(plot_dt) {
  ...
  g <- ggplot(plot_dt, aes(x = mean, y = mean_adj)) + geom_point(aes(color = def)) +
    ...
    theme_bw() + xlab("Mean") + ylab("Adjusted mean") + ggtitle("Encephalitis incidence xwalk")
  saveRDS(g, paste0(model_plot_dir, date, "_encephalitis_mean_vs_mean_adj_plot_object.RDS"))
  ...
  print(g2)
  dev.off()
  print(paste0("Plot saved in", model_plot_dir, date, "_encephalitis_mean_vs_mean_adj_plot_object.pdf"))
}
...
get_row_population <- function(i, ds) {
  ...
  #'              from GBD 2019 decomp steps for each row's demographics
...
split_data <- function(dt, model) {
  #' sex split data
  #' 
  #' @description apply sex ratio to Both Sex data 
  #' 
...
split_denominator <- function(raw_dt, age_dt) {
  ...
  pop <- get_population(age_group_id = row_age_group_ids, location_id = row_location_id, year_id = row_year_ids, sex_id = row_sex_id, 
                        gbd_round_id = 6, decomp_step = 'step2')
  ...
  raw_dt$match_key <- 1
  pop$match_key <- 1
  age_split_dt <- merge(raw_dt, pop[, .(age_group_id, age_split_sample_size, match_key)], by ="match_key", allow.cartesian = T)
...
split_numerator <- function(raw_dt, pattern_dt) {
  ...
  if (all(c(2,3,4) %in% row_age_group_ids)) row_age_group_ids <- c(0, row_age_group_ids)
