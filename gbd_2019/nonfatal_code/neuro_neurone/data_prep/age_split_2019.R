...
# GET OBJECTS -------------------------------------------------------------
b_id <- 347  #this is bundle ID
a_cause <- "MOD_ENV_"


id <- 2566 ## 
ver_id <- 365867 ## Dismod version used for age split
...
expand_age <- function(small_dt, age_dt = ages){
  ...
  dt <- dt[!drop<1,] ##comment this out if you want to retain all age splits regardless of small case #
...
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = id, ## USING 2010 AGE PATTERN BECAUSE LIKELY HAVE MORE DATA FOR 2010
                           measure_id = 5, location_id = locs, source = "epi", ##Measure ID 5= prev, 6=incidence, 18=proportion
                           version_id = ver_id, sex_id = c(1,2), gbd_round_id = 6, decomp_step = step, #can replace version_id with status = "best" or "latest"
