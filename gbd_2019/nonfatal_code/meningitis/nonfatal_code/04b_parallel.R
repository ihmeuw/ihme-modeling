...
for (y in years) {
  for (s in sexes) {
    for (g in groups) {
      ...
      incid.dt <- incid.dt[, lapply(.SD, sum), 
                           by=.(modelable_entity_id, measure_id, grouping, location_id, year_id, age_group_id, sex_id), 
      ...
      incid.dt[time == 1, paste0("prev_", 0:999) := lapply(.SD, function(x) x * 1/2), 
               .SDcols=paste0("draw_", 0:999)]
      ...
      incid.dt[, paste0("exact_", 0:999):= lapply(0:999, function(x){
        calculate_exact(get(paste0("exact_",x)), get(paste0("draw_",x)))
      })]
      incid.dt[, paste0("prev_", 0:999) := lapply(0:999, function(x) { 
        get(paste0("exact_", x)) + get(paste0("draw_", x)) * 0.5 * (1 - get(paste0("exact_", x)))
      })]
      ...
      incid.dt <- incid.dt[, lapply(.SD, mean), 
                           by=.(modelable_entity_id, measure_id, grouping, location_id, year_id, age_group_id, sex_id), 
                           .SDcols=paste0("draw_", 0:999)]
      ...
      if (g == '_vision' || g == '_hearing') {
        write_dir <- file.path(tmp_dir, "03_outputs", "01_draws", g, "_unsqueezed")
        dir.create(write_dir, showWarnings = F, recursive = T)
        col.remove <-  c("measure_id", "modelable_entity_id", "grouping")
        incid.dt[, (col.remove):= NULL]
        fwrite(incid.dt, file.path(write_dir, paste0("5_", location, "_", y, "_", s, ".csv")))
      } else {
        saveRDS(incid.dt, file.path(tmp_dir, '03_outputs', '01_draws', paste0(g, '_', location, '_', y, '_', s, '.rds')))
      }
    }
  }
}
