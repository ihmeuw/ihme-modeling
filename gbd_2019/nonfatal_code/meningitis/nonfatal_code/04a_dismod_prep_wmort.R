...
grouping <- c("long_modsev", "epilepsy")
...
cols.remove <- c("parameter", "cause")
cp.mort.dt[, (cols.remove):= NULL]
setnames(cp.mort.dt, c("ul", "ll"), c("upper", "lower"))
cp.mort.dt$modelable_entity_id <- ''
cp.mort.dt$modelable_entity_name <- ''
cp.mort.dt$sex_id <- ''
cp.mort.dt$year_id <- ''
cp.mort.dt$location_id <- ''
cp.mort.dt$measure <- "mtexcess"
cp.mort.dt$etiology <- ''
saveRDS(cp.mort.dt, file=file.path(out_dir, "02_temp", "03_data", "long_modsev_smr.rds"))

## erase and make directory for finished checks
dir.create(file.path(tmp_dir, "02_temp", "03_data", "checks"), showWarnings = F)
files <- list.files(file.path(tmp_dir, "02_temp", "01_code", "checks"))
finished.files <- grep("finished.*.txt", files, value = T)
for (f in finished.files) {
  file.remove(file.path(tmp_dir, "02_temp", "01_code", "checks", f), showWarnings=F)
}
