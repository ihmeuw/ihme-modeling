...
def get_births(cfr_df):
    ...
    births_df = births_df.drop(
        ['model_version_id', 'covariate_id', 'covariate_name_short', 'location_name', 'age_group_id', 'age_group_name',
         'lower_value', 'upper_value', 'sex'], axis=1)
...
def get_days_cfr_dfs(birth_prev, cfr, acause):
    data_dir = '%s/%s' % (parent_dir, acause)

    cfr_df = pd.read_csv('FILEPATH/%s_%s_draws.csv' % (data_dir, acause, cfr))
    cfr_df = cfr_df.rename(columns={'draw_1000': 'draw_0'})
    cfr_df = cfr_df.loc[cfr_df['year'].isin([1990, 1995, 2000, 2005, 2010, 2015, 2017, 2019])]
    days_df = pd.read_stata('FILEPATH/all_draws.dta' % (data_dir, birth_prev))
...
def get_mild_modsev_dfs(mild_prop, modsev_prop, acause):
    ...
    mild_df = pd.read_csv('FILEPATH/%s_%s_draws.csv' % (data_dir, acause, mild_prop))
    ...
    modsev_df = pd.read_csv('FILEPATH/%s_%s_draws.csv' % (data_dir, acause, modsev_prop))
...
def get_other_modsev_dfs(birth_prev, acause):
    ...
    at_birth_df = pd.read_stata('FILEPATH/all_draws.dta' % (data_dir, birth_prev))
    ...
    oh_six_df = pd.read_stata('FILEPATH/all_draws.dta' % (data_dir, birth_prev))
    ...
    seven_df = pd.read_stata('FILEPATH/all_draws.dta' % (data_dir, birth_prev))
...
def save_scale(acause_dict, acause, birth_prev):
    for prop_type in ['mild_count', 'modsev_count']:
        ...
        test_outdir = '%s/%s' % (parent_dir, acause)
        prev_df.to_csv('FILEPATH/%s_scaled_check.csv' % (test_outdir, birth_prev, prop_type))
...
def final_prev(acause_dict, births_df):
    for prop_type, prop_prev in zip(['mild_count', 'modsev_count', 'total_count'],
                                    ['mild_prev', '28_28_modsev_prev', 'total_prev']):
...
def calc_asymp(acause_dict):
    acause_dict['asymp_prev'] = (acause_dict['total_prev'] - acause_dict['mild_prev']) - acause_dict[
        '28_28_modsev_prev']
    return acause_dict
...
def save_prev(acause_dict, acause, birth_prev):
    for prop_type in ['mild_prev', '0_0_modsev_prev', '0_7_modsev_prev', '7_28_modsev_prev', '28_28_modsev_prev',
                      'asymp_prev']:
        prev_df = acause_dict[prop_type]
        prev_df = prev_df[draw_cols]
        test_outdir = '%s/%s' % (parent_dir, acause)
        prev_df.to_csv('FILEPATH/%s_final_prev.csv' % (test_outdir, birth_prev, prop_type))
