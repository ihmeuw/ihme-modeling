...
get_split_data <- function(){
  dt <- fread(paste0(dem_dir, cause_name, bundle_version, ".csv"))
  dt <- dt[group_review==1 | is.na(group_review), ]
  #dt <- fread(paste0(dem_dir, cause_name, "age_sex.csv"))
  return(dt)
}
...
find_sex_match <- function(dt){
  ...
  match_vars <- c("nid", "age_start", "age_end", "location_id", "measure", "year_start", "year_end",  # "effective_sample_size",
...
model_name <- paste0(cause_name, "_sexsplit_w_age", date)
...
#split_data <- function(dt, model){
  ...
  nosplit_dt <- tosplit_dt[sex %in% c("Male", "Female")] #| (sex == "Both")]
  ...
  #Run fourth line if pulling in pre-run MR-BRT model
...
pdf(paste0(dem_dir, cause_name, "_sex_split_w_age_graph.pdf"))
graph_predictions(predict_sex$graph)
dev.off()

write.csv(total_dt, paste0(dem_dir, cause_name, "_", covariate_name, ".csv"), row.names = F)
