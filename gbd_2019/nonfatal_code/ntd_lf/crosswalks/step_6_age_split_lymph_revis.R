...
col_order <- function(dt){
  epi_order <- fread(paste0(repo_dir, "FILEPATH"), header = F)
...
get_pop_structure <- function(locs, years, age_groups){
  populations <- get_population(location_id = locs, year_id = years,decomp_step = "iterative",
...
## EXAMPLE
#put your input file here (xlsx). 
dt <- as.data.table(read.csv(paste0(h_root, "FILEPATH")))
#IF THIS IS A CROSSWALKED DATASET, MAKE SURE CASES REFLECT ADJUSTED PREVALENCE !:
#note in my data the mean is not missing, so here I recalculate it and round up:
...
write.xlsx(final_split, paste0(h_root, "FILEPATH"))
write.csv(final_split, paste0(h_root, "FILEPATH"))
