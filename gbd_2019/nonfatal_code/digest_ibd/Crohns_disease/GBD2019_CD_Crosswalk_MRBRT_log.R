...
# INPUT DATA -------------------------------------------------------------
df2 <- as.data.table(read.csv("FILEPATH FOR ADMIN DATA"))
df1 <- as.data.table(read.csv("FILEPATH FOR LITERATURE DATA"))

df2 <- subset(df2, clinical_data_type=="inpatient" | clinical_data_type=="claims")
df2$cv_inpatient[df2$clinical_data_type=="inpatient" | df2$location_name =="Taiwan"] <- 1
df2$cv_literature <- 0
df2$cv_self_report <- 0
df2$cv_chart_review <- 0
df2$cv_dx_exam <-0

df2$cv_ms2000[df2$clinical_data_type=="claims" & df2$year_start==2000 & df2$location_name!="Taiwan"] <- 1
df2$cv_marketscan[df2$clinical_data_type=="claims" & df2$year_start!=2000 & df2$location_name!="Taiwan"] <- 1
df2$cv_dx_admin_data <- ifelse((df2$clinical_data_type=="inpatient" | df2$location_name=="Taiwan"),1,0)

df1$cv_ms2000 <- 0
df1$cv_marketscan <-0
df1$cv_literature <-1

df_master <- rbind(df1, df2, fill=TRUE )
df_master$sample_size <- as.numeric(as.character(df_master$sample_size))
df_master$mean <- as.numeric(as.character(df_master$mean))

write.csv(df_master, paste0("FILEPATH"), row.names = F)

df_inc <- subset(df_master, measure=="incidence")
df_prev <- subset(df_master, measure=="prevalence")
...
###Alt 1 (only alternative): International criteria
...
cause_path <- "CD"  #SPECIFY FILEPATH
cause_name <- "digest_ibd" #SPECIFY CAUSE NAME


#CREATE COVARIATES AND TAG REFERENCE vs. ALTERNATIVE
df_inc = within(df_inc, {cv_chart_review = ifelse(cv_chart_review==1 | cv_diag_exam==1, 1, 0)})
...
dat_original = within(dat_original, {cv_chart_review = ifelse(cv_chart_review==1 | cv_diag_exam==1, 1, 0)})
...
###Alt 3: International criteria data
...
#######################################################################################################################
# Lastly, adjust Alt 3: Within-study matches
location_match <- "exact"
year_range <-0       #SPECIFY YEAR RANGE FOR BETWEEN STUDY COMPARISON
covariate_name <- "cv_chart_review"   #set covariate of interest for crosswalk (alt)
...
# -- specify how to identify reference studies
# -- identify mean, standard error and covariate variables
  dat_original <- as.data.table(df_prev)
...
#SET YOUR REFERENCE IF ALREADY NOT DONE
  ...
  dat_original = within(dat_original, {cv_hospital = ifelse( clinical_data_type=="inpatient" | location_name=="Taiwan", 1, 0)})
  dat_original$cv_chart_review <- ifelse((dat_original$cv_chart_review==1 | dat_original$cv_diag_exam==1),1,0)
...
#########################################################################################################################################
  ...
  cov_names <- c("cv_ms2000", "cv_marketscan", "cv_chart_review") 
  predicted <- subset(predicted, !( cv_marketscan==1 & cv_stringent==1)) 
  predicted <- subset(predicted, !( cv_ms2000==1 & cv_stringent==1)) 
  predicted <- subset(predicted, !( cv_marketscan==1 & cv_stringent==1)) 
  predicted <- subset(predicted, !( cv_marketscan==1 & cv_ms2000==1)) 
  predicted <- subset(predicted, !( cv_ms2000==1 & cv_marketscan==1 & cv_stringent==1)) 
  predicted <- subset(predicted, !( cv_ms2000==0 & cv_marketscan==0 & cv_stringent==0)) 
  
  review_sheet_final <- merge(tmp_orig, predicted, by=cov_names)
  total_dt_prev_ref<-subset(review_sheet_final,( cv_ms2000==0 & cv_marketscan==0 & cv_chart_review==0) )
  ...
  review_sheet_final[, `:=` (log_mean = log_mean - Y_mean, log_se = sqrt(log_se^2 + Y_se^2))]
  review_sheet_final[, `:=` (mean = exp(log_mean), standard_error = deltamethod(~exp(x1), log_mean, log_se^2)), by = c("log_mean", "log_se")]
  review_sheet_final[, `:=` (cases = NA, lower = NA, upper = NA)]
  ...
  review_sheet_final_prev <- rbind.fill(review_sheet_final, review_sheet_final_inc)
