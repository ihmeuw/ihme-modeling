####################################################
## Description: Crosswalk Data Processing - Osteo HIP
####################################################
...
hip <- as.data.table(get_bundle_version(9143 , export = F, transform = T))
hip <- hip[!(field_citation_value %like% "Truven Health" & clinical_data_type == ""),]
hip <- hip[clinical_data_type != "inpatient"]
hip[field_citation_value %like% "Truven Health" & year_start == 2000, cv_marketscan_all_2000 := 1]
hip[field_citation_value %like% "Truven Health" & year_start != 2000, cv_marketscan := 1]
hip[field_citation_value %like% "Taiwan National Health Insurance", cv_taiwan_claims_data := 1]
hip[, "crosswalk_parent_seq" := as.numeric(NA)]

hip <- hip[is_outlier != 1 | is.na(is_outlier)]

hip[, case_def := ifelse(!is.na(cv_marketscan) & cv_marketscan == 1, "marketscan", NA)]
hip[, case_def := ifelse(!is.na(cv_marketscan_all_2000) & cv_marketscan_all_2000 == 1, "marketscan00", case_def)]
hip[, case_def := ifelse(!is.na(cv_taiwan_claims_data) & cv_taiwan_claims_data == 1, "taiwan_claims", case_def)]
hip[, case_def := ifelse(!is.na(cv_dx_selfreported_pain) & cv_dx_selfreported_pain == 1, "sympt_no_xray", case_def)]
hip[, case_def := ifelse((!is.na(cv_dx_radiographic_only) & cv_dx_radiographic_only == 1) | (!is.na(cv_dx_symptomatic_only) & cv_dx_symptomatic_only == 1), "xray_maybe_sympt", case_def)]
hip[, case_def := ifelse(!is.na(cv_dx_oa_selfreport) & cv_dx_oa_selfreport == 1, "selfrep_physdx", case_def)]
hip[, case_def := ifelse(is.na(case_def), "GBD", case_def)]

# APPEND REGION, SUPER REGION, ETC
hip <- merge(hip, loc_data, by = "location_id")
hip$country <- substr(hip$ihme_loc_id.y, 0, 3)
hip[, location_name.x := NULL]
hip[, ihme_loc_id.x := NULL]
setnames(hip, "location_name.y", "location_name")
setnames(hip, "ihme_loc_id.y", "ihme_loc_id")

# FILL OUT MEAN/CASES/SAMPLE SIZE
# CALCULATE STD ERROR BASED ON UPLOADER FORMULAS
hip <- get_cases_sample_size(hip)
hip <- get_se(hip)

# SEX SPLIT
test <- find_sex_match(hip)
test2 <- calc_sex_ratios(test)

model_name <- paste0("oa_hip_sexsplit_", date)
...
hip <- as.data.table(copy(test))
original_w_cd <- as.data.table(copy(hip))

claims_data <- hip[clinical_data_type == "claims" & location_name != "Taiwan"]
hip <- hip[cv_marketscan != 1 | is.na(cv_marketscan)]
hip <- hip[cv_marketscan_all_2000 != 1 | is.na(cv_marketscan_all_2000)]
claims_data <- aggregate_marketscan(claims_data)
claims_data <- claims_data[location_name == "United States" | location_name == "Taiwan"]
claims_data[location_name == "United States", location_name := "United States of America"]
hip <- rbind(hip, claims_data)
hip <- get_cases_sample_size(hip)
hip <- get_se(hip)

hip <- hip[, c("nid", "sex", "year_start", "year_end", "age_start", "age_end", "location_id", "location_name", "ihme_loc_id", "country", "region_name", "super_region_name", "measure", "mean", "lower", "upper", "standard_error", "case_def")]

# CREATE CD MATCHES BY DEMOGRAPHICS
hip[, demographics := paste2(sex, country, measure, sep = "_")]
hip[, demographics := paste2(sex, location_name, measure, sep = "_")]

hip_cds <- list(unique(hip$case_def))

hip[, year_mean := (year_start + year_end)/2]
hip[, age_mean := (age_start + age_end)/2]

hip$case_def <- as.factor(hip$case_def)
hip <- hip[order(case_def)]

list <- lapply(unique(hip$case_def), function(i) {
  subset(hip, case_def == i)
})
...
hip_matched <- copy(paired_GBD_marketscan)
hip_matched <- hip_matched[0,]

for (i in 1:length(pairss)) {
  hip_matched <- rbind(hip_matched, get(pairss[i]))
}

hip_matched <- hip_matched[abs(year_mean.denom - year_mean.num) < 11]
hip_matched <- hip_matched[abs(age_start.denom - age_start.num) < 6 & abs(age_end.denom - age_end.num) <6]

remaining_cd <- unique(c(unique(as.character(hip_matched$case_def.num)), unique(as.character(hip_matched$case_def.denom))))

# CALCULATE RATIO AND STANDARD ERROR

hip_matched <- hip_matched %>%
  mutate(
...
hip_matched <- as.data.table(hip_matched)
hip_matched[, id := paste0(nid.num, " (", country.num, ": ", sex.num, " ", age_start.num, "-", age_end.num, ") - ", nid.denom, " (", country.denom, ": ", sex.denom, " ", age_start.denom, "-", age_end.denom, ")")]
hip_matched[, id_var := paste0(nid.num, ":", nid.denom)]
...
id_var <- c("id","id_var", "ref", "alt")
metareg_vars2 <- c(id_var, metareg_vars, extra_vars)
...
fit1 <- run_mr_brt(
  output_dir = "FILEPATH",
  model_label = "crosswalk_network_oa_hip",
  data = tmp_metareg,
  mean_var = "ratio_log",
  se_var = "ratio_se_log",
  covs = cov_list,
...
########## AGE SPLITTING
source(paste0("FILEPATH", "ge_split_fns.R"))
