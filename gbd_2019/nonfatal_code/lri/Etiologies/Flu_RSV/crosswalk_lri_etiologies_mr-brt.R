...
##################################################################################################
  ...
  denom_weights$age_dummy <- with(denom_weights, ifelse(age_group_id < 5, "Group 1", ifelse(age_group_id ==5, "Group 2",
                                                                                            ifelse(age_group_id < 9, "Group 3", 
                                                                                                   ifelse(age_group_id < 13, "Group 4",
...
##---------------------------------------------------------------
  ...
  age_df <- age_split_mrbrt_weights(df, denominator_weights, output_dir=paste0("filepath"), bounded=T, title=name)
...
####################################################################################################
    ...
    g <- ggplot(df, aes(x=factor(cv_inpatient), y=mean, col=factor(cv_inpatient))) + 
      ...
      geom_point(alpha=0.5) + xlab("Inpatient") + 
...
# Subset outliers and data extracted only for crosswalks (group = 99)
   cv_inpatient <- bundle_crosswalk_collapse(df, covariate_name="cv_inpatient",
  ...
  ## produce approximation of a funnel plot
    ...
    f <- ggplot(mod_data, aes(x=log_ratio, y=delta_log_se, col=factor(outlier))) + geom_point(size=3) + geom_vline(xintercept=preds$Y_mean) + scale_y_reverse("Standard error") +
      theme_bw() + scale_x_continuous("Log ratio") + scale_color_manual("", labels=c("Used","Trimmed"), values=c("#0066CC","#CC0000")) + ggtitle(paste0(name, " inpatient ratio")) +
  ...
  ## Create essentially a forest plot
    ...
    f <- ggplot(mod_data[mod_data$outlier==0,], aes(ymax=log_ratio + delta_log_se*1.96, ymin=log_ratio - delta_log_se*1.96)) + geom_point(aes(y=log_ratio, x=label)) + 
      geom_errorbar(aes(x=label), width=0) +
          theme_bw() + ylab("Log ratio") + xlab("") + coord_flip() + ggtitle(paste0(name, " inpatient ratio (",round(preds$ratio,3),")")) +
...
####################################################################################################
  ...
  cv_elisa <- bundle_crosswalk_collapse(elisa_df[elisa_df$is_outlier==0,], 
      ...
      write.csv(cv_elisa, "filepath", row.names=F)
  ...
  ## produce approximation of a funnel plot
    mod_data <- fit1$train_data
    mod_data$outlier <- abs(mod_data$w - 1)
    f <- ggplot(mod_data, aes(x=log_ratio, y=delta_log_se, col=factor(outlier))) + 
      geom_point(size=3) + geom_vline(xintercept=preds$Y_mean) + scale_y_reverse("Standard error") +
      theme_bw() + scale_x_continuous("Log ratio") + 
  ...
  ## Create essentially a forest plot
    ...
    f <- ggplot(mod_data[mod_data$outlier==0,], aes(ymax=log_ratio + delta_log_se*1.96, ymin=log_ratio - delta_log_se*1.96)) + 
      geom_point(aes(y=log_ratio, x=label)) + geom_errorbar(aes(x=label), width=0) +
      theme_bw() + ylab("Log ratio") + xlab("") + coord_flip() + 
      ggtitle(paste0(name, " ELISA (",round(preds$ratio,3),")")) +
      geom_hline(yintercept=0) + geom_hline(yintercept=preds$Y_mean, col="purple") +
      geom_rect(data=preds, aes(ymin=Y_mean_lo, ymax=Y_mean_hi, xmin=0, xmax=length(mod_data$label[mod_data$outlier==0])+1), alpha=0.1, fill="purple")
    print(f)
  ...
  ## Make a diagnostic ggplot ##
    df$adjustment_type <- ifelse(df$cv_inpatient==1 & df$cv_diag_elisa==1, "Inpatient and ELISA", 
