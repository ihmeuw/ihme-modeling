...
bundle_id <- 152
acause <-"mental_schizo"
covariates <- c("cv_lay_interviewer", "cv_recall_1yr", "cv_registry_records")
uses_csmr <- F
test_sex_by_super_region <- F
crosswalk_pairs <- '' # no crosswalks for Schizophrenia
age_sex_split_estimates <- "FILEPATH.xlsx"
need_to_age_split <- F
need_save_bundle_version <- 3839 # Set as true to save bundle version, otherwise specify bundle version here
sex_ratio_by_age <- F
...
review_sheet_final[is.na(specificity) & study_covariate == "sex", specificity := "sex"]
