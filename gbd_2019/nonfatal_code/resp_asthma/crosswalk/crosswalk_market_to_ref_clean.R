...
dt$cv_marketscan <- ifelse((dt$clinical_data_type == "claims" & dt$location_name!= "Taiwan"), 1, 0)

dt$is_alt <- dt$cv_marketscan
dt$is_reference <- ifelse(dt$cv_marketscan==0,1,0)

# Aggregate marketscan to national level
...
dt_market_agg <- aggregate_marketscan(dt)
dt_market_agg <- dt_market_agg[dt_market_agg$location_name=="United States" & dt_market_agg$cv_marketscan==1, ]
dt_no_market <- dt[dt$cv_marketscan!=1, ]
dt <- rbind(dt_no_market, dt_market_agg)

# Get rid of extremely small values (round to 0 if below e-7)
dt_new$mean[dt_new$mean < 0.0000001] <- 0
...
actual_data$cv_marketscan <- ifelse((actual_data$clinical_data_type=="claims" & actual_data$location_name!="Taiwan" & actual_data$cv_marketscan_all_2000!=1),1,0)
...
gg_scatter <- ggplot(review_sheet_final, aes(x=mean_orig, y=mean, color=cv_marketscan)) + geom_point() +
