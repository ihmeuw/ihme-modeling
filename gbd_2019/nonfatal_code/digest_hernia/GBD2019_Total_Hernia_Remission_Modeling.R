...
int3=lm(mean~sex_binary * age_start, data=dt_proc , singular.ok = FALSE) # this indicates that sex and age do not interact with one another
...
reg3=lm(mean~ 0+ haqi_mean + sex_binary + age_start, data=dt_proc )
reg4=lm(mean~ 0+ haqi_mean + sex_binary + age_start + sex_binary*age_start, data=dt_proc )
reg5=lm(mean~ 0+ haqi_mean + sex_binary + age1 + age2 +age3 + age4 + age5, data=dt_proc)
reg6=lm(mean~ 0+ haqi_mean + sex_binary + age1 + age2 +age3 + age4 + age5 + age3*sex_binary + age4*sex_binary , data=dt_proc)
...
predictions <- predict(reg2, interval="confidence", newdata = meta_dt)
