...
##
## Author: USERNAME
## Purpose: Master script that contains the 2019 MR-BRT crosswalk process. Often run from launch_crosswalk.R
...
master_mrbrt_crosswalk <- function(
  ...
  ## If we don't have a "reference" column, making one so we can figure out what we need to crosswalk
  if (!sex_split_only) {
  ...
  if (nrow(data[sex=="Both",]) > 0) {
    ...
    ## Give a heads up about how many sex-specific data points you have to crosswalk.
    ...
    ## Fix column types to avoid data.table warning messages
