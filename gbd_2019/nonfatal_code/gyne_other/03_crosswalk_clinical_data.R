...
apply_mrbrt <- function(df,folder){ 
  ...
  # write-out
  message(paste0("Crosswalk ratios applied! Writing out to FILEPATH/bundle_id_",format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".xlsx"))
  write.xlsx(data,paste0("FILEPATH/crosswalked_bundle_id_",format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".xlsx"),sheetName="extraction")
  return(data)
}
