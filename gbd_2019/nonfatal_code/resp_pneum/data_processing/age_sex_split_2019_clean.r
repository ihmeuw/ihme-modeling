...
### Based on age_sex_split.R by USERNAME
###             and modified by USERNAME
### Must have group review specificity set to "sex"
...
cause_path <- "FILEPATH"
cause_name <- "resp_" # change name based on cause
date <- gsub("-", "_", Sys.Date())

## Get bundle version and save this for use in sex split script, then add parent_seq column, run age-sex split and save this for applying sex split
...
write.csv(bundle_version_dt, paste0("FILEPATH"), row.names = F)
...
age_sex_split <- function(dt){
  ...
  dt_split2[, split := length(specificity[specificity == "age,sex"]), by = list(nid, group, measure, year_start, year_end)]
  ...
  dt_split2 <- dt_split[dt_split$nid %in% both_nids, ]
  has_male <- dt_split2[dt_split2$sex=="Male"]
  has_male <- has_male$nid
  has_male <- unique(has_male)
  dt_split3 <- dt_split2[dt_split2$nid %in% has_male,]
  dt_split <- dt_split3
...
write.csv(split_dt, paste0("FILEPATH"), row.names = F)
