  ...
  for(me in c(upload_mes)){
    ...
    for(year in c(seq(from=1990, to=2010, by=5), 2015, 2017, 2019)){
      for(sex in c(1, 2)){
        ...
        command <- paste0("qsub -l m_mem_free=VALUE -l fthread=VALUE -N ", paste0("sympt_", me, "_", year, "_", sex),
...
if(save_upload==T){
  for(me in c(upload_mes)){
    ...
    ##sy: save both prevalence and incidence to asymptomatic
