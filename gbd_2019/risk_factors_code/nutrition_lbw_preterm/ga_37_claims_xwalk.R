...
trim_percent = 0.05
model.label = paste0("preterm_xw_outlier_MS2000_NUMYR_10_GS_other_X_claims_TRIM_", trim_percent)
dir.create(file.path(diag_dir, model.label))

ga_37 <- fread(file.path(input_dir, "all_data", paste0(bun_id, ".csv")))

ga_37 <- ga_37[measure == "prevalence"]
ga_37 <- ga_37[age_start ==0 & age_end == 0]
...
saveRDS(ga_37, file.path(ratio_dir, me, "preadjusted_data.rds"))

num_yrs = 10
ga_37[, mid_year := ( year_start + year_end ) / 2]
ga_37[, mid_year_cat := cut(mid_year, breaks = seq(1990, 2020, num_yrs))]
...
matches <- make_matches(data = ga_37, ratio.num = "cv_claims", ratio.denom = "cv_other", direct_to_reference = T)
...
ms2000 <- ga_37[field_citation_value %like% "United States MarketScan Claims and Medicare Data - 2000", temp_index]
...
saveRDS(fit, file.path(ratio_dir, me, "cv_claims_ratio.rds"))
