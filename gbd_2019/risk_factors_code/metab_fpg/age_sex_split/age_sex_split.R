...
age_table_code <- paste0(repo_dir, "age_table.R") ## SWICH TO WHERE YOU KEEP THIS CODE
date <- Sys.Date()
date <- gsub("-", "_", date)
...
age_split <- function(bundle, gbd_id, acause, age, region_pattern, location_pattern_id){
  ...
  write.xlsx(total, paste0(j_root, "filepath", acause, "/", bundle, "filepath", date, "_age_split.xlsx"),
...
age_sex_split <- function(bundle, acause){
  ...
  ## SAVE DATA
  write.xlsx(total, paste0(j_root, "/WORK/12_bundle/", acause, "/", bundle, "/03_review/02_upload/", date, "_age_sex_split.xlsx"),
