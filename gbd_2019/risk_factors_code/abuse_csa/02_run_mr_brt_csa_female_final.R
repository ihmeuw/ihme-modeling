...
dataset <- fread("FILEPATH/female_csa_prepped_mrbrt_aug1.csv")
...
outdir <- paste0("FILEPATH/csa_Female_", model)
...
fit1 <- run_mr_brt(
  ...
  model_label = paste0("csa_Female_", model),    #Change as needed
  data = dataset,
  mean_var = "logit_diff",
...
pred1[, `:=` (keep = d_intercourse + d_perp3 + d_contact_noncontact)] #Change as needed
