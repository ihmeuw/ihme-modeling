...
dataset <- fread("FILEPATH/csa_dep.csv")
output_dir <- "FILEPATH"
...
fit1 <- run_mr_brt(
  ...
  covs = list(
    ...
    cov_info("cv_symptom_scale", "X"),
             ...
             #r_linear = TRUE,
             #l_linear = FALSE),
    cov_info("cv_confounding_uncontrolled", "X")
    #cov_info("cv_case_cohort", "X")
    #cov_info("cv_subpopulation", "X"),
    #cov_info("cv_male", "X"),
    #cov_info("cv_female", "X"),
    #cov_info("cv_both_sex", "X")
...
df_pred1 <- expand.grid(cv_symptom_scale = c(0,1), cv_confounding_uncontrolled = c(0, 1, 2))
...
pred1[, `:=` (keep = cv_symptom_scale + cv_confounding_uncontrolled)]
pred1[, `:=` (x_cov = ifelse(cv_symptom_scale == 1, "symptom_scale",
                             ifelse(cv_confounding_uncontrolled == 1, "cv_confounding_uncontrolled",
