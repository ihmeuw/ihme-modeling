...
template<class Type>
Type scaled_logit(Type x, Type upper_bound, Type lower_bound) {
  return logit((x - lower_bound) / (upper_bound - lower_bound));
}
template<class Type>
Type inv_scaled_logit(Type x, Type upper_bound, Type lower_bound) {
  return ((invlogit(x) * (upper_bound - lower_bound)) + lower_bound);
}
...
template<class Type>
array<Type> invlogit_array(array<Type> logit_array, Type upper_bound, Type lower_bound) {
  array<Type> a(logit_array.rows(), logit_array.cols());
  for (int i = 0; i < logit_array.rows(); i++) {
    for (int j = 0; j < logit_array.cols(); j++) {
      a(i, j) = inv_scaled_logit(logit_array(i, j), upper_bound, lower_bound);
    }
  }
  return a;
}
...
template<class Type>
matrix<Type> calculate_half_number_migrants(matrix<Type> pop, array<Type> migration, int age_int, bool mig_prop) {
  int n_ages = pop.rows();
  ...
  for (int a = 0; a < n_ages; a++) {
    if (mig_prop) {
      ...
      half_net_migrants_cohort.row(a + 1) = migration.transpose().col(a + 1) * Type(age_int) * Type(0.5);
    }
  }
...
array<Type> ccmpp(array<Type> n0_female, array<Type> n0_male,
                  ...
                  int row_fert_start, int row_fert_end, bool female, bool mig_prop) {
  ...
  for (int y = 0; y < n_periods; y++) {
    ...
    matrix<Type> half_net_migrants_cohort_male = calculate_half_number_migrants(curr_pop_male, g_male.col(y), age_int, mig_prop);
    matrix<Type> half_net_migrants_cohort_female = calculate_half_number_migrants(curr_pop_female, g_female.col(y), age_int, mig_prop);
    ...
    if (!mig_prop) {
      half_new_born_migrants_male = g_male(0, y) * 0.5;
      half_new_born_migrants_female = g_female(0, y) * 0.5;
    }
