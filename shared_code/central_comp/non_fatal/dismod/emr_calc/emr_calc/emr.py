...
import pandas as pd
import numpy as np
from db_tools import loaders, ezfuncs, config
from emr_calc import data, envs
...
class ComputeDismodEMR(ComputeEMR):
    ...
    def __init__(self, model_version_id, envr="prod"):
        ...
        config.DBConfig(
            ...
            odbc_filepath=os.path.join(envs.Environment.get_code_root(),
                                       ".odbc.ini"))
    ...
    def set_data_acmr(self, id_template_df):
        try:
            self._data_acmr = data.DataACMR(id_template_df)
        except data.NoNonZeroValues:
    ...
    def compute_dismod_emr(self, remission_df=None, csmr_type="cod"):
        ...
        # pull in adjusted data and associated metadata
    ...
    def upload_emr(self):
        ...
        infiler.upload_stagedir(
            table="t3_model_version_emr", schema="epi", session=session,
            commit=True)
...
def dismod_emr(model_version_id, envr="prod", remission_df=None,
               csmr_type="cod"):
    ...
    """
    compute_emr = ComputeDismodEMR(model_version_id, envr=envr)
