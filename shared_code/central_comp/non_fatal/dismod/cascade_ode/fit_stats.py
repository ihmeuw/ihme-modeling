...
def read_data_pred_file(fdef):
    ...
    try:
        ...
        logging.exception('Error reading pred file: {}'.format(f))
        df = pd.DataFrame()
    return df
...
def calc_rmses(df):
    ...
    # when computing RMSE
    thisdf = df[df.adjust_median > 0]
    rmses = thisdf.groupby(['integrand','cv_id','hold_out']).apply(lambda x:
        np.sqrt(
...
def calc_mean_errors(df):
    ...
    # when computing RMSE
    thisdf = df[df.adjust_median > 0]
    means = thisdf.groupby(['integrand','cv_id','hold_out']).apply(lambda x:
            np.mean((np.log(x['adjust_median'])-np.log(x['pred_median']))**2))
...
def calc_coverage(df):
    ...
    df.ix[unadj_bool, 'adjust_lower'] = (
            df.ix[unadj_bool, 'meas_value'] - 1.96 *
            df.ix[unadj_bool, 'meas_stdev'])
    df.ix[unadj_bool, 'adjust_upper'] = (
            df.ix[unadj_bool, 'meas_value'] + 1.96 *
            df.ix[unadj_bool, 'meas_stdev'])
...
def write_fit_stats(mvid, outdir, joutdir):
    ...
    fs_df.ix[fs_df.hold_out==0, 'fit_stat_id'] = 1
    fs_df.ix[fs_df.hold_out==1, 'fit_stat_id'] = 2
    fs_df.rename(columns={'rmse': 'fit_stat_value'}, inplace=True)

    cov_df = cov_df[cov_df.covered==True]
    ...
    cov_df.ix[cov_df.hold_out==0, 'fit_stat_id'] = 3
    cov_df.ix[cov_df.hold_out==1, 'fit_stat_id'] = 4
