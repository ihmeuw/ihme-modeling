...
def check_upload(output_version_id, upload_cod_summary_status,
                 upload_gbd_summary_status):
    log_files = []
    if upload_cod_summary_status:
        log_files = (
            log_files + [os.path.join('FILEPATH', 'logs', 'FILEPATH.txt')])
    if upload_gbd_summary_status:
        log_files = ([
            log_files + (os.path.join('FILEPATH', 'logs', 'FILEPATH.txt'))
            for measure in [1, 4] for change in ["True", "False"]])
    for f in log_files:
        try:
            assert 'All done' in open(f).read(), (
                "Can't mark best. {} upload job is not finished".format(f))
        except IOError:
            raise "Can't mark best. {} upload job has no log file".format(f)
