...
import numpy as np
from PV import rmse_out_map, trend_out_map
...
def weight(ranked_array, psi, cutoff):
    '''
...
def psi_weights(space_err, lin_err, psi, cutoff):
    ...
    psi_values = weight(ranks, psi, cutoff)
    M = len(psi_values) / 2
...
def psi_range(space_pred, lin_pred, space_err, lin_err, psi_values, cutoff):
    ...
    values in psi range.
    '''
    psi_pred = np.array([psi_predict(space_pred, lin_pred, space_err, lin_err,
                                      x, cutoff) for x in psi_values])
    return psi_pred.T
...
def ensemble_all(space_models, linear_models, psi_values, cutoff):
    ...
    p = mp.Pool(20)
    ensemble_predictions = np.array(p.map(psi_map, inputs))
    p.close()
    p.join()
