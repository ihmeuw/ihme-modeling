...
class MetricConverter(ComputationElement):
    ...
    def convert_to_percentages(self, df,
                               index_cols=['location_id', 'year_id',
                                           'age_group_id', 'sex_id',
                                           'measure_id', 'metric_id'],
                                value_cols=DEFAULT_DRAW_COLS):
        """ Convert DataFrame in number-space to percent-of-total space.
         """
    ...
    def aggregate_population(df):
        ...
        df = AgeAggregator(df, ['draw_0'], index_cols,
                           age_groups={22: (0, 200)}).get_data_frame()
        # Remove ASR
    ...
    def convert_to_rates(self, df, value_cols=DEFAULT_DRAW_COLS):
        ...
        # Change metric to percent space
    ...
    def get_data_frame(self):
        df = self.df
        number_df = df.loc[df['metric_id']==gbd.metrics.NUMBER].copy(deep=True)
        ...
        if self.to_rate:
            ...
            # Remove rates from input DataFrame
            df = df.loc[(df['metric_id']!=gbd.metrics.RATE)|
                       ((df['age_group_id']==27)&
                        (df['metric_id']==gbd.metrics.RATE))]
        if self.to_percent:
            # Convert data to percent space
            percent_df = self.convert_to_percentages(
                number_df, value_cols=[c for c in df if c.startswith('draw_')])
            new_df.append(percent_df)
            # Remove percentages from input DataFrame
