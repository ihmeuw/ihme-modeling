...
class SexAggregator(ComputationElement):
    """ sex aggregation by location, age, year, cause, set sex_id to 3 (gbd.sex.BOTH)
    """
    ...
    def get_data_frame(self):
        ...
        group_index_cols = ['location_id', 'year_id', 'age_group_id']

        self.add_index_column(group_index_cols, 'cause_id', pre_sex_df)
        self.add_index_column(group_index_cols, 'sequela_id', pre_sex_df)
        self.add_index_column(group_index_cols, 'rei_id', pre_sex_df)
        self.add_index_column(group_index_cols, 'measure_id', pre_sex_df)

        sex_df = to_sum_df.groupby(group_index_cols).sum()
