...
class AggregateCauses(ComputationElement):
    ...
    def aggregate(self):
        ...
        level = max_depth - 1
        while level >= 0:
            logger.info("  aggregating level {l} of {md}".format(
                l=level, md=max_depth))
            aggs = []
            for cause in self.cause_tree.level_n_descendants(level):
                child_ids = [c.id for c in cause.children]
                if len(child_ids) > 0:
                    agg = data.loc[data['cause_id'].isin(child_ids)].copy(deep=True)
                    if not agg.empty:
                        agg['cause_id'] = cause.id
                        agg = agg.groupby(self.index_columns).sum().reset_index()
                        aggs.append(agg)
            if aggs:
                aggs = pd.concat(aggs)
                data = pd.concat([data, aggs])
            level -= 1
        data = data.groupby(self.index_columns).sum().reset_index()
        return data

    def get_data_frame(self):
        logger.info("BEGIN aggregate_causes")
        data = self.aggregate()
        logger.info("END aggregate_causes")
        return data
