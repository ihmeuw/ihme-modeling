// *********************************************************************************************************************************************************************
// *********************************************************************************************************************************************************************
// Purpose:		Prep, submit, and then aggregate data for regressions
** **************************************************************************
** CONFIGURATION
** **************************************************************************
	** ****************************************************************
	...
	** ****************************************************************
		...
		capture log close
		log using "`log_folder'/predict__master.log", replace
	...
	** ****************************************************************		
		...
		// Get schema data			
			// Get full
				capture use "`input_folder'/_schemes/_scheme_`scheme'.dta", clear
				if _rc {
					sleep 5000
					use "`input_folder'/_schemes/_scheme_`scheme'.dta", clear
				}
				preserve
...
** ****************************************************************
	// Get prepped data
		capture use "/ihme/cod/prep/RDP_regressions/ICD_cod/_input_data/`input_data_name'.dta", clear
		if _rc {
			sleep 5000
			use "/ihme/cod/prep/RDP_regressions/ICD_cod/_input_data/`input_data_name'.dta", clear
		}
		** use "$prefix/WORK/03_cod/01_database/02_programs/redistribution/regression_proportions/_testing/`input_data_name'.dta", clear
	...
	// Save deaths by garbage/target for each region
		preserve
			...
			save "`temp_folder'/data/__garbage_target_data.dta", replace
		restore
	...
	* // Tag analysis groups where there is no garbage or a single target
	* 	// No garbage
	* 		gen temp = 0
	* 		replace temp = 1 if group_cause == 0
	* 		egen has_garbage = max(temp), by(group_location group_year group_sex group_age)
	// Move garbage code cause fraction into its own column
		gen temp = 0
	...
	* // Drop male/female only restrictions -- DID THIS WHEN SEXES WERE POOLED AND FIXED EFFECT WAS USED
	* 	** Drop female cause targets for males
	* 	drop if sex == 1 & (inlist(acause,"neo_breast_cancer","neo_cervical_cancer","neo_ovarian_cancer","neo_uterine_cancer") | index(acause,"maternal"))
	* 	** Drop male cause targets for females
	* 	drop if sex == 2 & inlist(acause,"neo_prostate_cancer","neo_testicular_cancer")
	...
	// Loop through different location, year, sex, age, and cause groups
		// Create analytical group ids
			...
			save "`temp_folder'/data/_all_input_data.dta", replace
		...
		preserve
			foreach group__id of local analysis_groups {
				...
				capture rm "`temp_folder'/data/output_`group__id'.dta"
				save "`temp_folder'/data/input_`group__id'.dta", replace
				!/usr/local/bin/SGE/bin/lx24-amd64/qsub  -P proj_codprep -pe multi_slot 5 -l mem_free=10g -N "RDP_regress_`package'_`scheme'_`group__id'" "`code_folder'/shellstata.sh" "`code_folder'/02_generate_proportions_worker.do" "`username' `scheme' `package' `input_data_name' `group__id' `today'"
				restore, preserve
			}
		restore
** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
		// Check for completion
			...
			foreach group__id of local analysis_groups {
				...
				local checkfile "`temp_folder'/data/output_`group__id'.dta"
	...
	// Format
		...
		// Load person years
			merge m:1 country using "/ihme/cod/prep/RDP_regressions/ICD_cod/_input_data/`input_data_name'_person_years.dta", assert(1 3) nogen
			replace person_years = 0 if person_years == .
		// Suppress all random effects for Andean Latin America
		...
		// Produce uncertainty and calculate random effect using significant levels and person-years
			...
			* foreach var of varlist beta* omega* {
			* 	replace `var' = -1*`var'
			* }
	...
	// Save
		save "`temp_folder'/02_predicted_`package'.dta", replace
