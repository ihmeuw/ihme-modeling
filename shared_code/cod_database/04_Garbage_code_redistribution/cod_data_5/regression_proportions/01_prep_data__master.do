...
** **************************************************************************
	...
	** ****************************************************************
		...
		capture log close
		log using "`log_folder'/prep__master.log", replace
	...
	** ****************************************************************
		// Pull location hierarchy
			// Get data from hierarchy
				odbc load, exec("SELECT * FROM shared.location_hierarchy_history WHERE location_set_version_id = `location_set_version'") strConnection clear
			...
			// Save
				tempfile location_hierarchy
				save `location_hierarchy', replace
				save "`output_folder'/_location_hierarchy.dta", replace

				
** ****************************************************************
** RUN PROGRAM
** ****************************************************************
	// Standardize ICD10 & ICD9_detail data
	if "`input_data_name'" == "ICD_cod" {
		// Submit jobs for prep
			insheet using "$prefix/WORK/03_cod/01_database/02_programs/prep/documentation/shared_maps.csv", comma names clear
			foreach codebook_name in ICD9_detail ICD10 {
				...
				foreach input_source in `codebook_name' ``codebook_name'_sources' { //''
					capture confirm file "/ihme/cod/prep/RDP_regressions/`input_data_name'/_input_data/`input_source'.dta"
					if _rc {
						display "Submitting job for `input_source'"
						!/usr/local/bin/SGE/bin/lx24-amd64/qsub -pe multi_slot 10 -l mem_free=20g -P "proj_codprep" -N "RDP_prep_`input_source'" "`code_folder'/shellstata.sh" "`code_folder'/01_prep_data_ICD_cod.do" "`input_data_name' `input_source' `codebook_name' `username'"
					}
				}
			}
		...
		// Append results together
			...
			foreach codebook_name in ICD9_detail ICD10 {
				foreach input_source in `codebook_name' ``codebook_name'_sources' { //''
					...
					display "/ihme/cod/prep/RDP_regressions/`input_data_name'/_input_data/`input_source'.dta"
					local checkfile "/ihme/cod/prep/RDP_regressions/`input_data_name'/_input_data/`input_source'.dta"
	...
	// Save
		compress
		save "`output_folder'/`input_data_name'.dta", replace
		save "`temp_folder'/`input_data_name'.dta", replace
	...
	// Get person-years
		...
		save "`output_folder'/`input_data_name'_person_years.dta", replace
		save "`temp_folder'/`input_data_name'_person_years.dta", replace
