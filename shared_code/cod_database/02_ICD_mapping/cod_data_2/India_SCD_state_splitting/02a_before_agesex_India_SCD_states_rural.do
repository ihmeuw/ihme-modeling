** ************************************************************************************************* **				
** Purpose: Format and map the results from India model					
** ************************************************************************************************* **
...
** save iss model results
	** only run it if there are new results for the model
	local new_model_results = 1
	if `new_model_results'==1 {
		!bash "$j/WORK/03_cod/01_database/03_datasets/`data_name'/code/save_scd_model_results.sh"
		** give bash file time to delete the old deaths_split_scd file
		sleep 1000
		capture confirm file "$j/WORK/03_cod/01_database/03_datasets/`data_name'/explore/deaths_split_scd.csv"
		while _rc {
			sleep 3000
			capture confirm file "$j/WORK/03_cod/01_database/03_datasets/`data_name'/explore/deaths_split_scd.csv"
		}
...
** get a map of cause_names
	use "$j/WORK/03_cod/01_database/03_datasets/`data_name'/data/intermediate/01_mapped.dta", clear
	keep if acause=="_gc"
...
** import modeled data from SCD model
	import delimited using "$j/WORK/03_cod/01_database/03_datasets/`data_name'/explore/deaths_split_scd.csv", clear

** misc cleaning
	drop v1

** use all cause as cc_code for the four states that have no data with a full cause list
	replace cause = "cc_code" if cause=="all" & inlist(state, "Arunachal PrUSER", "Jammu and Kashmir", "Nagaland", "West Bengal")
	drop if cause=="all"
...
** get the location_ids back
	...
	tempfile preloc
	save `preloc', replace
	odbc load, exec("SELECT location_id, location_ascii_name as location_name FROM shared.location WHERE path_to_top_parent LIKE '%,163,%'") clear strConnection
	tempfile locs
	save `locs', replace
...
** fix age to be like cod age groups and reshape
...
** fix identifiers
	...
	** no year-specific NIDs for these years for some reason
	replace NID = 173685 if NID==.
...
** save to 02a before agesex file
	save "$j/WORK/03_cod/01_database/03_datasets/`data_name'/data/intermediate/_archive/02a_before_agesex_`timestamp'.dta", replace
	save "$j/WORK/03_cod/01_database/03_datasets/`data_name'/data/intermediate/02a_before_agesex.dta", replace
