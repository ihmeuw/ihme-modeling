...
def prepTrainingData():
    ...
    # make sure we didn't lose any of the raw data from adding populations (will fail if there are locations in )
...
weights_dir = "{prefix}/WORK/03_cod/01_database/02_programs/urban_rural_splitting/weights".format(prefix=prefix)
archive_path = "{weights_dir}/_archive/urban_rural_weights_{ts}.csv".format(weights_dir=weights_dir, ts=time_string)
location_cause_weights.to_csv(archive_path, index=False)
location_cause_weights.to_csv("{weights_dir}/urban_rural_weights.csv".format(weights_dir=weights_dir), index=False)

# some rough comparison tools
#old_weights = pd.read_csv("{weights_dir}/_archive/urban_rural_weights_03_23_15_15_39.csv".format(weights_dir=weights_dir))
#old_weights = old_weights.rename(columns={'state':'location_name', 'weight':'old_weight'})
#comp = pd.merge(old_weights, location_cause_weights, on=['location_name','location_id_urban','acause','urbanicity','location_id_state'], how='inner')
#pd.pivot_table(comp, index='acause', columns='urbanicity', values=['old_weight','weight'], aggfunc=np.mean).to_csv('{weight_dir}/comparison_after_pops.csv'.format(weight_dir=weights_dir))
