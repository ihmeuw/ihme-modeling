...
** PREP DATA FOR WEIGHTS
	 use "$j/WORK/03_cod/01_database/03_datasets/India_MCCD_states_ICD10/explore/states_splitting/inputs/States_MCCD_ICD10.dta" if regexm(cause, "^(`n_chapter')|(`e_chapter')"), clear
	...
	** get SRS region mapping
	merge m:1 location_id using "$j/WORK/03_cod/01_database/03_datasets/India_SCD_states_rural/data/raw/SCD_SRS_region_mapping.dta", keep(1 3) keepusing(srs_region) nogen
	** Combine the six union territories into the 'South' region
...
** CREATE AGE/SEX/YEAR AND REGIOUSERGE/SEX/YEAR WEIGHTS, APPLY REGION WHERE POSSIBLE
	...
	** Make special weights for Punjab 1980 since  sex in undefined and has no E codes
...
** FORMAT AND EXPORT RESULTS FOR VISUALIZING
	...
	** export results
	export delimited using "$j/WORK/03_cod/01_database/03_datasets/`source'/explore/injury_weights_mccd.csv", replace
