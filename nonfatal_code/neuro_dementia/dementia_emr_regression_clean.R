...
### Purpose: EMR Regression for Dementia
###########################################################
...
##pull locations
locations <- c(102, 93, 385, 79) ##getting info for US, Sweden, Puerto Rico, Finland

##define objects
ages <- c(13:20, 30:32, 235)
sexes <- c(1,2)
meid <- 11641
cid <- 543
cod.model.f <- 355949
...
##collapse age_group 13,14,15,16 (40-59) because not a lot of people have Alzheimer's that young
...
model <- lm(mean_logemr ~ age_group1 + age_group17 + age_group18 + age_group19 + age_group20 + age_group30 +
              age_group31 + age_group32 + sex_id1 + sex_id2, data= all.draws)
...
us.sub <- as.list(locations[parent_id==102 & !location_id==385, .(location_id)]) ##list of us subnationals and is not puerto rico
swed.sub <- as.list(locations[parent_id==93, .(location_id)]) ##list of sweden subnationals
...
##get us and sweden emr data
us.data <- as.vector(emr.data[location_id==102, .(mean_emr, upper_emr, lower_emr)])
swed.data <- as.vector(emr.data[location_id==93, .(mean_emr, upper_emr, lower_emr)])

##replace values with true values for subnationals of countries used in the regression
all.emr <- all.emr[location_id %in% us.sub$location_id, c("mean_emr", "upper_emr", "lower_emr") := us.data]
all.emr <- all.emr[location_id %in% swed.sub$location_id, c("mean_emr", "upper_emr", "lower_emr") := swed.data]

##get age start and age end
age.groups <- as.data.table(get_ids("age_group")) ##get age group id's 
age.groups <- age.groups[age_group_id %in% c(1, 17:20, 30:32, 235), ] ##only the id's we want (we don't really want id 1, but will set this to match "new_age 1" manually)
...
age.groups <- age.groups[age_group_id==1, age_start := 40] ##set age group 1 manually
...
write.csv(all.emr, paste0(j.root, output.dir, ".csv"), row.names = F)
