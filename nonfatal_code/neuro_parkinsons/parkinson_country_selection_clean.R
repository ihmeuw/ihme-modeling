...
### Purpose: CSMR and Prevalence by location and sex- Parkinson's Disease 
###########################################################
...
age_weights <- age_weights[age_group_id %in% c(9:20, 30:32, 235),]
...
##pull prevalence
prevalence.f <- as.data.table(get_model_results(gbd_team = "epi", model_version_id = 96479, 
                                                measure_id = 5, year_id = 2016, sex_id = 2, age_group_id = 27))
prevalence.m <- as.data.table(get_model_results(gbd_team = "epi", model_version_id = 96479, 
                                                measure_id = 5, year_id = 2016, sex_id = 1, age_group_id = 27))
setnames(prevalence.f, "mean", "as_prev")
setnames(prevalence.m,"mean","as_prev")
prevalence.m <- prevalence.m[,.(age_group_id, sex_id, location_id, as_prev)]
prevalence.f <- prevalence.f[,.(age_group_id, sex_id, location_id, as_prev)]

##pull csmr
csmr.m <- as.data.table(get_model_results(gbd_team = "cod", model_version_id = 92759, 
                                          age_group_id = c(2:20, 30:32, 235), year_id = 2016))
csmr.f <- as.data.table(get_model_results(gbd_team = "cod", model_version_id = 92762,
                                          age_group_id = c(2:20, 30:32, 235), year_id = 2016))
