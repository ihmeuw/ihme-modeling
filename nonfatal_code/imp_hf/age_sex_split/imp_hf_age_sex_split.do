...
egen both_case_total = sum(cases) if sex=="Both" & group_review==1 & measure=="prevalence", by(group)
egen both_sample_total = sum(sample_size) if sex=="Both" & group_review==1 & measure=="prevalence", by(group)

egen male_case_total = sum(cases) if sex=="Male" & group_review==1 & measure=="prevalence", by(group)
egen male_cases = max(male_case_total), by(group)
egen male_sample_total = sum(sample_size) if sex=="Male" & group_review==1 & measure=="prevalence", by(group)
egen male_sample = max(male_sample_total), by(group)

egen female_case_total = sum(cases) if sex=="Female" & group_review==1 & measure=="prevalence", by(group)
egen female_cases = max(female_case_total), by(group)
egen female_sample_total = sum(sample_size) if sex=="Female" & group_review==1 & measure=="prevalence", by(group)
...
expand 2 if sex=="Both" & group_review==1 & measure=="prevalence", generate(males)
replace sex="Male" if males==1

expand 2 if sex=="Both" & group_review==1 & measure=="prevalence", generate(females)
