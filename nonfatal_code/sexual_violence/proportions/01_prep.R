...
## Date: March 15, 2017
## Purpose: CHILD SCRIPT TO PREP DATA FOR INJURIES OR MENTAL HEALTH ANALYSIS
##########################################################################
...
# get n-code ICD9 map
  ncode_map <- paste0(code_dir, "FILEPATH.csv") %>% fread
  ...
  seq_map <- paste0(code_dir, "FILEPATH.csv") %>% fread
