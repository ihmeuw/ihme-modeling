"""
PURPOSE: This script runs an ODE solver to transition from incidence to prevalence
    for a given E-N combination and platform.
AUTHOR: USERNAME
DATE: DATE
...
Arguments:
    ...
    Arg 2: string sex
        female or male
    Arg 3: binary has_mort
        1 if sequela has mortality, 0 if not.
    Arg 4: binary has_prev0
        1 if you are specifying an initial prevalence, 0 if not.
    Arg 5: string ages_dir
        ...
        Temporarily, you may use this directory:
    Arg 6: string code_dir
        Path to folder containg python package that has various necessary functions.
    Arg 7: string pop_s_path
        filepath to csv that contains cleaned single-year-age-group 
        populations (to create this file use 
    Arg 8: string pop_grp_path
    ...
    Arg 9: string out_dir
        folder where you wish to output prevalences
    Arg 10: string inc_dir
        folder that contains incidence inputs. Note that this folder must contain 
        files named =ANYTHING_[iso3]_[year]_[sex].csv. Each file must have the 
        following columns: age, draw0-draw999.
    Optional Arg: string mort_dir (only include if has_mort == 1)
        optional argument to directory containing excess mortality inputs.
        Note that this folder must contain 
        files named ANYTHING_[iso3]_[year]_[sex].csv. Each file must have the 
        following columns: age, draw0-draw999.
    Optional Arg: string prev0_path (only include if has_prev0 == 1)
        optional argument that specifies path to initial prevalence
        Note that this file must be named ANYTHING_[iso3]_[year]_[sex].csv. It 
        must have the following columns: age, draw0-draw999.
        Note that if both has_mort and has_prev0, prev0_path must FOLLOW mort_dir 
        in the string of arguments
...
def progress_one_year(interp_module,df,pop_df,drawnum,has_inc=True,has_mort=True,inc_df=pd.DataFrame(),mort_df=pd.DataFrame()):
    ...
    # collapse the age groups that are now off (i.e. the under1 age groups that 
    # are now between age 1 and 2 and the terminal age group)
...
def aggregate_data(dir,iso3,sex):
    ...
    print "Files for aggregate_data:"
    print files
    result = pd.DataFrame()
    print "Result in aggregate_data:"
    print result
    for i in files:
        df = pd.read_csv(i)
...
def get_pop_total(pop_s,pop_grp):
    """Take under1 age groups and add on their pop to the single age population.
    This creates a DF that has under 1 pops + single year age group pops above 1."""
