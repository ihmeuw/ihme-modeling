...
* DEFINE LOCALS AND DIRECTORIES *
  ...
  local  sensData FILEPATH/sensitivity_of_blood_cultures_typhoid_diag.dta
...
* RUN THE METAANLYSIS (THIS REQUIRES THAT YOU INSTALL METAN) *
...
* Find the mean, confidence limits and standard error from the 1,000 draws
  ...
  replace note_modeler = "Multiplied by adjustment factor (~1.8) to account for poor diagnostic sensitivity" + note_modeler 
...
* EXPORT FILE *
  ...
  export excel using `1'_adjusted.xlsx, sheet("extraction") sheetreplace firstrow(variables)
