...
// Description:	Replace wrong sequela_ids & sequela_names with the correct ones and do age/sex-splitting
...
// download and load lit data (this will grab the latest changes made to the database including marking outliers)
	...
	// Replace wrong sequela_ids & sequela_names with the correct ones
	replace sequela_id=1282 if nid==85056 | nid==85057 | nid==85058
	replace sequela_name="Chronic ear pain due to otitis media cases" if nid==85056 | nid==85057 | nid==85058
	replace grouping="chronic" if nid==85056 | nid==85057 | nid==85058
	
	// exclude three data points which are not AOM but entered as AOM
	
	replace data_status="excluded" if nid==103421 & sequela_id==1281
	replace issues="not AOM" if nid==103421 & sequela_id==1281
	...
	// age-sex split data points given by age and by sex separately (example given below)
		...
		gen agesex_split_group = 1 if nid == 85058 & iso3=="AUS" & grouping=="chronic" & data_status=="" 
