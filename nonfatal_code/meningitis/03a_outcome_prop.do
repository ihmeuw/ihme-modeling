...
// major_mort0 = major visual disturbance, major_mort1 = major cognitive difficulties, major motor deficit, or major multiple impairments, seizure is self explanatory
	preserve
	drop in 1 // dropping labels
	levelsof outcome, local(variab)
	drop in 7 // drops major multiple impairments
	drop *se *sd *median 
...
** creating draws for major_all and minor_all
	...
	foreach etiology of local etiologies {
		foreach sev of local severity {
			keep outcome `etiology'_mean `etiology'_se 
...
** Calculate ratio draws of minor against major 
	foreach etiology of local etiologies {
		...
		rename v_* w_*
		merge 1:1 modelable_entity_id using "`tmp_dir'/02_temp/03_data/`etiology'_major.dta", nogen
		forvalues i = 0/999 {
		...
		rename w_* v_*
		save "`tmp_dir'/02_temp/03_data/`etiology'_minor_ratio.dta", replace // ratio of minor/major proportion (never is greater than 1)
		clear
	}
