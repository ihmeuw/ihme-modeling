...
// Upload hearing file to DisMod so that the outputs can be used for hearing/vision envelop squeeze (for _vision, do not use 'unsqueezed' sequelae anymore to avoid confusuion in central squeeze)
	...
	foreach cause of local etiologies {
		levelsof grouping if acause == "`cause'", local(grouping) // hearing and vision
		foreach group of local grouping {
			levelsof healthstate if acause == "`cause'" & grouping == "`group'", local(healthstate) // only ever _unsqueezed
			foreach state of local healthstate {

				local job_name "save_results_`c'"
				di "submitting `job_name'"
				local slots = 12
				local mem = `slots' * 2
				! qsub -P proj_custom_models -N "`job_name'" -pe multi_slot `slots' -l mem_free=`mem' "`shell_file'" "`code_dir'/04b_save_results.do" ///
