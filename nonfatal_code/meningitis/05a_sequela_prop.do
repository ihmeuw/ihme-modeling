			...
			if `conduct' == 1 {
				...
				forvalues b = 0/5 {
					if present_`b' == 1 {
						...
						clear
						//creating a beta distribution for each of the sequelae
				...
				//appending the sequelae for a group together
				forvalues c = 0/5 {
					capture append using `state_`c''
					capture erase `state_`c''
				}
				//the labels are currently from 1-1000. This changes it to 0-999
				forvalues d = 1/1000 {
					local e = `d' - 1
					qui rename v`d' v_`e'
				}
				...
				save "`tmp_dir'/03_outputs/01_draws/`etiology'_`group'.dta", replace
				clear
			}
			clear
		}
