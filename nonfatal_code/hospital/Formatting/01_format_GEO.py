...
#####################################################
df = pd.read_excel(root + "FILEPATH")
...
# df.to_csv(root + r"FILEPATH", index=False, encoding='utf-8')
...
df['age_group_unit'] = 1
df['source'] = 'GEO_COL'  # col for collaborator
...
# drop nulls in sex_id
df = df.dropna(subset=['sex_id'])

# drop nulls in age
df = df.dropna(subset=['age'])

# drop nulls in dx_1, there are only 3
df = df.dropna(subset=['dx_1'])

# drop nulls in outcome_id, there are only 2
df = df.dropna(subset=['outcome_id'])

# Columns contain only 1 optimized data type
# NOTE age is not included
int_cols = ['sex_id', 'outcome_id', 'year_start', 'year_end', 'nid',
            'code_system_id', 'age_group_unit', 'representative_id',
            'metric_id']
...
if df[str_cols].isnull().any().any():
    warnings.warn("There are Nulls in one of the columns {}"
                  " These nulls will be converted to the string 'nan'".format(str_cols))
...
# Age has some strings in it
df['age'] = pd.to_numeric(df['age'], errors='coerce', downcast='integer')
df = df.dropna(subset=['age']) # have to do this again because of coerction
...
# outcome_id has 4 things in it at the moment
# data dictionary (2nd sheet) says:
# 1 Home or Self Care
...
outcome_dict = {"1": 'discharge',
                ...
                "3": 'discharge',
                "4": 'death'}
df['outcome_id'] = df['outcome_id'].replace(outcome_dict)

# dx_2 is only external causes, so we want to make those the primary diagnosis
# where possible
# df.loc[nature_condition & cause_condition, ['dx_1', 'dx_2']] = df.loc[nature_condition & cause_condition, ['dx_2', 'dx_1']].values
df.loc[df.dx_2.notnull(), ['dx_1', 'dx_2']] = df.loc[df.dx_2.notnull(), ['dx_2', 'dx_1']].values
