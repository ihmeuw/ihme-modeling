...
* RUN PREDICTION MODEL *  
  mepoisson meanImpairment ageS* pca1 || super_region_id: || region_id: || country_id:
...
* SAVE RESULTS * 
  capture rm FILEPATH/all_draws.h5
  
  local inpath = "FILEPATH/full.csv"
  local outpath = "FILEPATH/full.h5"
  !FILEPATH FILEPATH/to_hdf.py "`inpath'" "`outpath'"

run FILEPATH/save_results.do
save_results, modelable_entity_id(1500) description("Proportion of low vision due to trachoma (custom mixed effects)") in_dir("FILEPATH") file_pattern("full.h5") h5_tablename(draws)  env("prod")
