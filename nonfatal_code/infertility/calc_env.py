...
class SexEnvelopeMkr(object):
    ...
    def __init__(self, male_prop_id, female_prop_id, exp_id, env_id, year_id):
        ...
        dim = draws.SquareImport.dUSERt_idx_dmnsns
        dim["year_id"] = year_id
        dim["measure_id"] = [{MEASURE ID}]
        self.base_importer = draws.SquareImport(idx_dmnsns=dim)

        # import and scale sex proportions
        self.male_prop = self.import_sex_prop(male_prop_id, {SEX ID})
        self.female_prop = self.import_sex_prop(female_prop_id, {SEX ID})
        self.scale_props()
    ...
    def import_exposure(self, modelable_entity_id):
        ...
        # get import shape
        gopher_idx_dmnsns = broadcast_idx_dmnsns.copy()
        gopher_idx_dmnsns["sex_id"] = [{SEX ID}]
...
def calculate_envs(male_prop_id, female_prop_id, exp_id, env_id, male_env_id,
    ...
    male_sex_env.to_hdf(
        ...
        key="data", format="table", data_columns=male_keys)
    ...
    female_sex_env.to_hdf(
        ...
        key="data", format="table", data_columns=female_keys)
