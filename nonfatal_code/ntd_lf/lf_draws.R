...
source("FILEPATH") # load when_mda_draw
...
for(j in 1:length(mda_data$location_name)) {
  maxAge <- 99
  number_requiring <- mda_data$requiring[j]
  number_treated <- mda_data$treated[j]
...
format_mda_data <- data.frame(mean = mda_data$mean_draws,
                              ...
                              age_start = rep(0, length(mda_data$location_id)),
                              age_end = rep(99, length(mda_data$location_id)),
...
remission3$year_start <- rep(2003, length(unique(vec_locations))) 
...
write.csv(epi_uploader, paste0("FILEPATH"),row.names = FALSE)                          
