...
// LOAD SETTINGS FROM MASTER CODE (NO NEED TO EDIT THIS SECTION)
	...
	if "`1'" != "" {
		...
		// base directory on J 
		local root_j_dir "$prefix/WORK/12_bundle/ntd_ebola/gbd2016"
		// base directory on share
		local root_tmp_dir "/share/scratch/users/USER/ntds/ntd_ebola"
		// timestamp of current run (i.e. 2014_01_17)
		...
		local last_steps ""
		// directory where the code lives
		local code_dir "`root_j_dir'/01_code/dev"
	}
	...
	// set adopath
	adopath + "$prefix/temp/central_comp/libraries/current/stata"
...
Outline:
	...
	4) Get prevalence of post-Ebola chronic fatigue syndrome
		- Apply duration provided by Theo
*/
...
// Get parameters
	...
	** Chronic duration
	**
	import excel using "`in_dir'/long term ebola with ersatz for uncertainty GBD2016.xlsx", firstrow sheet("ersatz") clear
...
** *********************************
// Get deaths
	use "$prefix/WORK/03_cod/01_database/03_datasets/_Shock/data/raw/ebola_cis_redistributed.dta" if year >= 1989 & year <= 2016, clear
	merge m:1 location_id using "`tmp_dir'/locs.dta", keepusing(location_name) nogen keep(3)
	// replace location_name = "South Africa" if location_name == "South Africa (ex-Gabon)" // from old code
	// sex is now in "male" and "female" format
...
// Convert deaths to cases, save 2014 and 2015 to add to other dataset (only deaths for most recent epidemic)
	...
	preserve
		...
		** THIS IS A TEMPORARY FIX FOR LACKING THE CORRECT AGE GROUPS
		** missing Guinea and Liberia for 2014 age_group_id 235 males
		...
		drop new
		** missing: 2015 ages 31 (male) and 32 (both) for Guinea
		...
		drop new count
		** missing: Liberia 2015 male (4-10 12-20 30-32 235) female (30-32 235)
...
// Get chronic fatigue syndrome
	...
	keep location_name year_id
	duplicates drop
	save "`tmp_dir'/chronic_location_years.dta", replace
	
** *********************************
// By location, check file for endemicity, save 0s if not
	cap mkdir "`tmp_dir'/logs"
	foreach location_id of local location_ids {
		!qsub -N "Ebola_`location_id'" -P proj_custom_models -pe multi_slot 4 "`shell_file'" "`code_dir'/`step_num'_`step_name'_parallel.do" "`location_id' `tmp_dir'"
...
// Check for full set of results
	...
	local location_ids `r(location_ids)'
	foreach location_id of local location_ids {
		capture confirm file "`tmp_dir'/final/9669/`location_id'.csv"
...
// Upload
  ...
  forval x = 1991/2016 {
	local saveyears `saveyears' `x'
 	...
 	// run save_results
	qui run "$prefix/temp/central_comp/libraries/current/save_results.do"
	save_results, modelable_entity_id(9668) description("Incidence and prevalence of acute Ebola episode") env("prod") years(`saveyears') in_dir("`tmp_dir'/final/9668") file_pattern({location_id}.csv) mark_best("yes")

    save_results, modelable_entity_id(9669) description("Incidence and prevalence of chronic fatigue following acute Ebola episode") years(`saveyears') env("prod") in_dir("`tmp_dir'/final/9669") file_pattern({location_id}.csv) mark_best("yes")
