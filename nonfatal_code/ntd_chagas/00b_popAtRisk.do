...
\********************************************************************************/  
  ...
  * RUN & EXECUTE 'GRABLOCALS' FUNCTION THAT PASSES IN NEEDED LOCALS *
	run "FILEPATH/grabLocals.ado"
	grabLocals
	
  * PUT RETURNED RESULTS FROM 'GRABLOCALS' INTO STANDARD LOCALS *	
	foreach i in dirCustomChagas dirCodemChagas { 
		local `i' `r(`i')'
		}
		
	local nYears = 37 // number of years between 1980 and most recent year of current GBD
...
\********************************************************************************/ 
  ...
  * PULL IN AND APPEND POPULATION AT RISK DATA *
	import delimited using "`dirCodemChagas'/chagas dismod input_v3.csv", clear
	tempfile appendTemp
	save `appendTemp'

	import delimited using "`dirCodemChagas'/chagas dismod input.csv", clear
  ...
  * BRING IN CHAGAS ENDEMCITY DATA & CREATE AN OBSERVATION FOR EACH YEAR AND COUNTRY *
	use "`dirCustomChagas'/data/chagasEndemicity.dta", clear
	keep if chagas==1
...
\********************************************************************************/
  ...
  * PULL IN LIST OF NEIGHBORING COUNTRIES *	
	preserve
	use "`dirCustomChagas'/data/cntryNeighbors.dta", clear
  ...
  * LOOP THROUGH COUNTRIES WITH 1 YEAR OF DATA, GET & APPLY MEAN TREND OF NEIGHBORS *
	foreach single of local singles {
		...
		* Create estimate based on that country's 1 data point and neigbors' trends *
...
\********************************************************************************/
  ...
  * BRING IN FULL LIST OF COUNTRIES *
...
\********************************************************************************/
  ...
  * SET THE PR AT RISK TO ZERO FOR NON-ENDEMIC COUNTRIES *
	...
	rename year year_id
	save "`dirCustomChagas'/data/chagasPopulationAtRisk.dta", replace
	...
	export delimited using FILEPATH/prAtRiskCovar.csv, replace
