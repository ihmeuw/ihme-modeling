...
*** BOILERPLATE ***
	...
	adopath + FILEPATHa
	run FILEPATHa/get_population.ado	
	run FILEPATHa/get_demographics.ado
	run FILEPATHa/get_outputs.ado
	run FILEPATHa/get_draws.ado

	tempfile master prev migration pop endemic endemicDraws usa nonusa subnats
...
*** GET DEMOGRAPHICS AND POPULATION ESTIMATES ***	
	...
	get_population, location_id(`locations') age_group_id(`ages' 22) sex_id(`sexes' 3) year_id(-1) clear
...
*** PULL IN CHAGAS MORTALITY ESTIMATES & SUM BY LOCATION ***  
	get_outputs, topic("cause") location_id("all") year_id("all") cause_id(346) gbd_round_id(4) clear version(latest)  
...
*** BRING IN MIGRATION DATA ***
	use FILEPATH/migrationData.dta, clear
...
*** BRING IN POPULATION AT RISK DATASET ***  
	...
	merge m:1 iso3 year_id using FILEPATH/chagasPopulationAtRisk.dta, assert(2 3) keep(3) nogenerate
	drop iso3
...
*** BREAK OUT BY AGE AND SEX ***  
	cross using FILEPATH/immigrantsByAgeSex.dta
...
*** COMBINE PREVALENCE AND MIGRATION DATA AND CACULATE NUBMER OF CHAGAS INFECTED IMMIGRANTS *** 
	merge m:1 source_location_id year_id sex_id age_group_id using FILEPATH/endemicPrevDraws.dta, assert(3) nogenerate
...
*** BREAKDOWN US NUMBERS BY STATE ***
	...
	joinby mergeRegion using FILEPATH/immigrantsByState.dta
	...
	save FILEPATH/latinAmericanMigrants.dta, replace
