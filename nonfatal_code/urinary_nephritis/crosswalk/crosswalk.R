...
# process MS # 
  ...
  ms[,c("cases_total","sample_size_total"):=lapply(.SD,sum),.SDcols=c("cases","sample_size"),
     by=c("age_start","age_end","year_start","year_end","sex")]
  
  ms_locs<-ms[,location_id]%>%unique
...
# process hospital # 
  # only keep hospital data from where we have MS data # 
  hosp<-hosp[location_id%in%ms_locs]
...
# adjust input data # 
dt_adj<-read.xlsx(paste0(data_dir,"request_73604.xlsx"))%>%as.data.table()
dt_adj<-merge(dt_adj[cv_hospital==1],compare[,.(age_start,age_end,ratio,ratio_se,sex)],by=c("age_start","age_end","sex"),all.x=T)
