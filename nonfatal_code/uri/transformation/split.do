...
// Description:	age/sex-splitting, ESS-splitting
...
// run transformations, organized by logical groupings
	...
	// split denominators by age/sex distribution observed in the general population for each given country/year (example given below)
		...
		gen ess_split_group = 1 if nid==116804 & data_status == ""
		esssplit ${functional}, date(${date}) tmpdir("ADDRESS")

	// age-sex split data points given by age and by sex separately 
		// mark each group of datapoints to split with a unique group number: 1, 2, 3, etc.
		gen agesex_split_group = 1 if nid == 123456 & !inlist(data_status, "outlier", "excluded")
		gen agesex_split_group = 2 if nid == 524334 & !inlist(data_status, "outlier", "excluded")
