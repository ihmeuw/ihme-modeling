...
// Purpose: Update to use get_population central function. For now, this only works on the cluster.
	...
	drop if age == 1
	gen age_group_id = .
	// recode anything under 1 to Perinatal (28-365 days), per USERNAME (USERNAME 04/14/2016)
	...
	//  3.15.2016: Apply correction factors for reporting vs actual ebola deaths, provided by USERNAME . These correction factors not applied to non-West African 2014 ebola epidemic deaths, assumed these deaths are considered accurate. 3 countries are USA, Mali and Nigeria
	gen tag = 0
