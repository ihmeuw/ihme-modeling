...
** ******************************************************************
    ...
    // set script location for formatting icd10_codes
        local format_ICD10_codes = "$standardize_ICD10_codes"

    ** *************************************************************************************
    ** Correct Coding Systems
    ** *************************************************************************************
    // mark rdp target codes (codes used to assist redistribution packages that should not be found in the data)
    ...
    // Re-assign entries to "CUSTOM" if incorrectly categorized
        ...
        // Assign "CUSTOM" if cause is blank (data_check.do should have verified that either cause or cause_name exist)
            replace coding_system = "CUSTOM" if inlist(coding_system, "ICD10", "ICD9_detail", "NPCR") & cause == ""
        // Remove "_cancer"
    ...
    // Re-assign CUSTOM causes to GBD if in the GBD cause list
        ...
        if r(N) {
            // get gbd causes
                ...
                import delimited "$parameters_folder/custom_cancer_map.csv", clear varnames(1)
    ...
    ** **************************************************************************************
    ...
    // cause_name
        ...
        // remove special characters
            replace cause_name = subinstr(cause_name,"�","c",.)
            replace cause_name = subinstr(cause_name,"�","ae",.)
            replace cause_name = subinstr(cause_name,"�","oe",.)
            replace cause_name = subinstr(cause_name,"�","n",.)
            foreach letter in � � � � �{
    ...
    // add remaining formatting
        foreach var in cause cause_name {
            ...
            // Reformat capitalization
                ...
                replace `var' = strproper(`var') if coding_system != "GBD" & rdp_target != 1
                replace `var' = itrim(`var')
                replace `var' = trim(`var')
        }
    ...
    // Remove cause_name from defined coding systems. Remove cause from CUSTOM/GBD coding systems
        replace cause_name = cause if !inlist(coding_system, "ICD10", "ICD9_detail", "NPCR") & cause_name == ""
        replace cause = "" if !inlist(coding_system, "ICD10", "ICD9_detail", "NPCR")
        replace cause_name = "" if inlist(coding_system, "ICD10", "ICD9_detail", "NPCR")
