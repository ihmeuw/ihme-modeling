...
random: Does your model have random effects?  If yes, then specify "random(yes)".  DUSERts to "no"
...
mark_best: Do you want this model automatically marked best upon upload?  If so, specify "mark_best(yes)". DUSERt is no.
...
*** PROCESS SYNTAX ***
	...
	else if "`mark_best'" != "no" & "`mark_best'" != "yes" {
		di "Allowable values for option mark_best are yes & no.  DUSERting to no."
		local mark_best no
		}
	...
	if "`random'" == "" {
		...
		di "Allowable values for option random are yes & no.  DUSERting to no."
		local random no
		}
...
*** CREATE OUTPUT DIRECTORIES ***
	...
	local codeDir FILEPATH/customCod
...
*** CYCLE THROUGH LOCATIONS AND SUBMIT BASH FILES TO PROCESS DRAWS ***
	...
	if "`endemic'"!="" {
		...
		foreach location of local locations {
			...
			! qsub -P `project' -pe multi_slot 8 -N customCod_`cause_id'_`location' "FILEPATH/submit_prediction_processor.sh" "`cause_id'" "`location'" "`link'" "`min_age'" "`random'" "`=subinstr("`saving_cause_ids'", " ", "_", .)'" "`modelDir'" "0" "`inflator_draw_stub'"
			}	
		}
	...
	while `complete'==0 {
	  ...
	  if `r(N)'==0 {
		...
		foreach saving_cause_id of local saving_cause_ids {
		  ...
		  save_results, cause_id(`saving_cause_id') mark_best(no) description("`description'") in_dir(`modelDir'/`saving_cause_id'/draws)
		  }
		}
