...
** **************************************************************************
	...
	keep location_id year_id ln_ldi
	tempfile ln_ldi
	save `ln_ldi', replace
	
	// get health system access capped covariat
	get_covariate_estimates, covariate_id(208) clear
	rename mean_value health_system_access2
	keep location_id year_id health_system_access2
	tempfile hsa2
	save `hsa2', replace
	
	// import dataset
	import delimited using "`out_dir'/inputs/anc_syphilis_test_formatted.csv", clear
	drop if mean == .
	replace mean=0.001 if mean==0
	replace mean=0.999 if mean==1
	gen lt_mean = logit(mean)
	keep location_id year_id lt_mean
	
	// merge onto template and add covariates
	joinby year_id location_id using `template', unmatched(both)
	merge m:1 location_id year_id using `ln_ldi', nogen keep(3)
...
** **************************************************************************
	...
	// run model
	xtmixed lt_mean ln_ldi health_system_access2 || region_id:
	...
	forvalues i = 0/999 {
		...
		gen anc_syph_test_`i' = invlogit(xb_`i' + re_`i') if !mi(re_`i')
		replace anc_syph_test_`i' = invlogit(xb_`i') if mi(re_`i')
	...
	// save results
	export delimited using "`out_dir'/models/anc_syph_test.csv", replace
	save "`out_dir'/models/anc_syph_test.dta", replace
