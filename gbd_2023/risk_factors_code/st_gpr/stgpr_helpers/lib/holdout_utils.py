...
These functions need some serious love, and I think the method itself is sketchy too. But
it's been this way for years and changing it has never been a priority.
"""
...
def _add_holdouts_to_dataframe(
    ...
    holdouts_df[holdout_cols] = holdouts_df[holdout_cols].fillna(
        stgpr_schema.Holdout.USED_IN_MODEL.value
    )
    return holdouts_df
...
def _add_holdout(
    ...
    ko_data[holdout_column] = stgpr_schema.Holdout.HELD_OUT.value
    ko_data = ko_data[
...
def _get_holdout_data_frame(
    ...
    partitioned[var][partitioned[var].isnull()] = stgpr_schema.Holdout.USED_IN_MODEL.value
    partitioned[var][
        partitioned[columns.VAL].isnull()
    ] = stgpr_schema.Holdout.USED_IN_MODEL.value
