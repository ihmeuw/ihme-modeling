...
def read_parameters_from_config(
    path_to_config: str, model_index_id: Optional[int]
) -> pd.DataFrame:
    ...
    Returns:
        Model parameters, read into a DataFrame which still needs to be processed before
            modeling.
    ...
    return pd.read_csv(path_to_config, engine="python", encoding="utf-8-sig").pipe(
        lambda df: _get_parameter_set_at_index(df, model_index_id)
    )
...
def process_parameters(params_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    params_dict = (
        params_df.pipe(_validate_required_parameters_exist)
    ...
    params_dict[parameters.DENSITY_CUTOFFS] = _update_density_cutoffs(
        params_dict[parameters.DENSITY_CUTOFFS]
    )
    return params_dict
...
def _assign_default_parameter_values(config: pd.DataFrame) -> pd.DataFrame:
    ...
    for param_name, default_value in parameters.DEFAULT_PARAMETERS.items():
        if param_name not in config or config[param_name].isna().any():
            ...
            logging.info(
                f"{param_name} not found in config. Using default value {flattened_value}"
            )
