...
def validate_stgpr_version_not_deleted(stgpr_version_id: int, session: orm.Session) -> None:
    """Validates that an ST-GPR version ID was not deleted.

    Assumes ST-GPR version exists.
    ...
    Raises:
        ValueError: if ST-GPR version ID was deleted.
    """
    if db_stgpr.stgpr_version_was_deleted(stgpr_version_id, session):
        raise exc.ModelWasDeleted(f"ST-GPR version {stgpr_version_id} was deleted.")
