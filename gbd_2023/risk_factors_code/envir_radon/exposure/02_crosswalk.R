...
rerun_crosswalk <- F
xw_version <- 1 
description <- "saving data to iterative for GBD2020"
...
source(file.path(central_lib,"current/r/get_bundle_version.R"))
source(file.path(central_lib,"current/r/save_crosswalk_version.R"))
source(file.path(central_lib,"current/r/get_crosswalk_version.R"))

#mr-brt
source(file.path(j_root,"FILEPATH/mr_brt_functions.R"))
...
#bundle_dir
out_path <- file.path(j_root,"FILEPATH",bundle,"FILEPATH/crosswalk.xlsx")

# Save bundle version & prep dataset---------------------------------------

bundle_version <- save_bundle_version(bundle_id= bundle, decomp_step=decomp, gbd_round_id=7)

bundle <- get_bundle_version(bundle_version$bundle_version_id) #only applying XW to new GBD2020 data

dt <- bundle[is_outlier==0]
...
dt[is.na(method) & !is.na(GM) & other_error_description=="Range", method:="GM & Range"] # sample size?
dt[is.na(method) & !is.na(AM) & other_error_description=="Range", method:="AM & Range"] # sample size?
dt[is.na(method) & !is.na(AM) & other_error_description=="Maximum value", method:="AM & Max"] # sample size?
...
# 6. GM/AM and Range # Using approximation of range/4=sd in log space. This could be improved!
...
# uses formula for lm and the assumption that log(max) will be about 2 standard deviations away from lm
...
# rows with lm < 0. Drop for now #OTHER SOLUTIONS??--6 rows in 1 NID in E India & 2 in serbia
...
#pdf for plots
pdf(paste0(home_dir,"crosswalk/xw_adjustment_plots_",Sys.Date(),".pdf"))
...
if(rerun_crosswalk){
  ...
  saveRDS(fit1, file.path(home_dir,"crosswalk",xw_version,"fit_object.RDS"))
  
} else {
  fit1 <- readRDS(file.path(home_dir,"crosswalk",xw_version,"fit_object.RDS"))
}
...
# merge corrected and crosswalked data on
bundle[site_memo=="",site_memo:=NA]
bundle[,site_memo:=gsub("\"\"","\"",site_memo)]
bundle[,site_memo:=gsub("\n","\r\n",site_memo)]

xw <- merge(bundle,dt_out[,.(nid,location_id,site_memo,AM,ASD,GM,Median,method,lm,lsd,lvar_mean,lse)],all.x=T,all.y=F)

xw[,crosswalk_parent_seq:=seq]
xw[,group_review:=NA]

write.xlsx(xw,out_path,row.names=F,sheetName="extraction")
crosswalk_version <- save_crosswalk_version(bundle_version$bundle_version_id,out_path)
# Note (05/04/2020): tried to save a xw version, but encountered this error
# Saving crosswalk versions for custom bundles is not supported at this time

write.csv(dt_out,paste0(home_dir,"/crosswalk/crosswalked_results_",decomp,"_",date,".csv"),row.names=F)


# write to log to save version ids
crosswalk_version[,date:=Sys.Date() %>% as.character]
crosswalk_version[,description:=description]
crosswalk_version[,bundle_version_id:=bundle_version$bundle_version_id]

log <- fread(paste0(home_dir,"/bundle/log.csv"))
log_out <- rbind(log,crosswalk_version,fill=T,use.names=T)

write.csv(log_out,paste0(home_dir,"/bundle/log.csv"),row.names=F)

# Save bundle version & XW version to ST-GPR shape------------------------

stgpr_bundle <- 8186
stgpr_outpath <- "FILEPATH/stgpr_crosswalk.xlsx"

# save bundle version
bundle_version <- save_bundle_version(bundle_id= stgpr_bundle, decomp_step=decomp, gbd_round_id=7)

# save XW version
stgpr <- read.xlsx(out_path) %>% as.data.table
...
stgpr[,unit_value_as_published:=1]

write.xlsx(stgpr,stgpr_outpath,row.names=F,sheetName="extraction")

crosswalk_version <- save_crosswalk_version(bundle_version$bundle_version_id,stgpr_outpath)
