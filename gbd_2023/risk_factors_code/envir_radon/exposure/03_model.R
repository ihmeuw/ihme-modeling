...
desc <- "GBD2020 annual save & extrapolation to 2022"

# Directories -------------------------------------------------------------

home_dir <- ("/ihme/erf/GBD2020/envir_radon/exp")
...
#get model results
source(file.path(central_lib,"current/r/get_model_results.R"))
#save results epi
...
std_locs <- get_location_metadata(location_set_id=101, gbd_round_id=7)

#for reproducibility
set.seed(143)

# Open PDF to save plots
pdf(paste0(home_dir,"/model/diagnostic_graphs_",decomp,"_",date,".pdf"),height=8.5,width=11)


# Read in data ------------------------------------------------------------


dt <- fread(paste0(home_dir,"/crosswalk/crosswalked_results_",decomp,"_",crosswalk_date,".csv"))
...
mean_temp <- get_covariate_estimates(covariate_id=71,decomp_step="iterative", gbd_round_id=7)
...
save(mod1,file=paste0(home_dir,"/model/model_1_std_locs_",decomp,"_",date,".Rdata"))
...
# save version of this model
save(mod2,file=paste0(home_dir,"/model/model_2_resid_",decomp,"_",date,".Rdata"))
...
# save LM draws
write.csv(draws_radon,file=paste0(home_dir,"/model/draws_model_results_lm_",decomp,"_",date,".csv"),row.names=F)

# drop pred1 and resid columns
...
#save LM summaries
write.csv(exp[,.(location_id,exp_lower,exp_mean,exp_median,exp_upper,conf_int)],file=paste0(home_dir,"/model/summary_model_results_lm_",decomp,"_",date,".csv"),row.names=F)
...
# save version of this model
save(mod_sd,file=paste0(home_dir,"/model/model_sd_",decomp,"_",date,".Rdata"))
...
# save draws lsd
write.csv(draws_radon,file=paste0(home_dir,"/model/draws_model_results_lsd_",decomp,"_",date,".csv"),row.names=F)
...
# save summary lsd
write.csv(sd[,.(location_id,sd_lower,sd_mean,sd_median,sd_upper,sd_conf_int)],file=paste0(home_dir,"/model/summary_model_results_lsd_",decomp,"_",date,".csv"),row.names=F)
...
# save draws am & asd
write.csv(draws_radon[,c("location_id",paste0("draw_am_",draw_nums_gbd),paste0("draw_asd_",draw_nums_gbd)), with = F],file=paste0(home_dir,"/results/draws/am_asd_",decomp,"_",date,".csv"),row.names=F)
...
# save summary am,asd
write.csv(exp,file=paste0(home_dir,"/results/summary/am_asd_",decomp,"_",date,".csv"),row.names=F)
...
save <- function(meas){
  ...
  save_dir <- paste0(home_dir,"/upload/",meas,"_",decomp,"_all_draws_",date,"/")
  dir.create(save_dir)
...
# Save exposure
if(save_exposure){
  save_results_epi(input_dir=paste0(home_dir,"/upload/am_", decomp, "_all_draws_", date),
                 input_file_pattern=paste0("{year_id}.csv"),
                 year_id=years,
...
# Save sd
if(save_sd){
  save_results_epi(input_dir=paste0(home_dir,"/upload/asd_", decomp, "_all_draws_", date),
                 input_file_pattern=paste0("{year_id}.csv"),
                 year_id=years,
