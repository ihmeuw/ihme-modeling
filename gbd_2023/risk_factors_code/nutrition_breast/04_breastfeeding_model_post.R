...
abfrate <- log_file[me_name=="abfrate"]$run_id
...
save_stgpr(abfrate)
save_stgpr(ebfrate0to5)
save_stgpr(predbfrate0to5)
save_stgpr(partbfrate0to5)
...
saveRDS(non_exc_wide, paste0(data_dir,"/all_cats_0to5.rds"))
...
saveRDS(disc6_11,paste0(data_dir,"/all_cats_6to11.rds"))

# Any Breastfeeding 12-23 Months
abf12_23 <- abf[age_group_id==238]
...
saveRDS(disc12_23,paste0(data_dir,"/all_cats_12to23.rds"))
...
non_exc_wide <- dcast.data.table(non_exc, as.formula(paste0(paste(ids2, collapse=" + "), " ~ variable")), 
                                 value.var=paste0("exp_cat",1:4))
saveRDS(non_exc_wide, paste0(data_dir,"/bf_exclusive_exposure.rds"))
...
# Write out and save
saveRDS(disc_bf, paste0(data_dir,"/bf_discontinued_exposure.rds"))
...
for (cat in cats){ 
  message(paste0("writing out for cat ",cat))
...
# Save Results!
save_epi(input_dir=data_dir,cat="exp_cat1",me_id=9598,step=decomp_step,bundle_id=log_file[me_name=="abfrate",bundle_id],crosswalk_id=log_file[me_name=="abfrate",crosswalk_version_id]) #No Breastfeeding
...
# Save Results!
save_epi(input_dir=data_dir,cat="disc_exp_cat1",me_id=8907,step=decomp_step,bundle_id=log_file[me_name=="abfrate",bundle_id],crosswalk_id=log_file[me_name=="abfrate",crosswalk_version_id]) #Discontinued Breastfeeding
