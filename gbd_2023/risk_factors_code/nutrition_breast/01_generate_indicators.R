...
for (file in files) {
  ...
  data <- load.file(file.path(paste0(input_dir,file)))
  if (file %like% ".dta") file <- gsub(".dta",".csv",file)
  ...
  } else if ((unique(data$survey_name) %like% "DHS" | unique(data$survey_name) %like% "MIS") & "qb21" %in% names(data)){
    data <- data[qb21=="respondent"|qb21=="Respondent"|qb21=="yes"|qb21=="Yes"]  
  } else if ((unique(data$survey_name) %like% "DHS" | unique(data$survey_name) %like% "MIS") & "q202" %in% names(data)){
    data <- data[q218=="respondent"|q218=="Respondent"|q218=="yes"|q218=="Yes"]  
  }
  if ("caseid" %in% names(data)) data <- data[!duplicated(caseid)]
  ...
  if (("age_month" %in% names(data))) {
    ...
    data[, dhs_recode := ifelse(min(bf_dur, na.rm=T) < 0, 1, 0), by=c("nid")]
      ...
      data[bf_dur == -3 & dhs_recode == 1, bf_dur := NA] 
    ...
    if ("food_24h" %in% names(data) & "liq_24h" %in% names(data)) {
      ...
      message(paste(c("Generating indicators:", indics), collapse=" "))
        ...
        if("ebf" %in% indics) {
          ...
          ebf[hasvars == 0, value := bf_excl] #use bf_excl as a 2nd option 
        }
        # predbf 
        if("predbf" %in% indics) {
          #keep only rows with age_month under 6 months 
        ...
        if("bf" %in% indics) {
          ...
          bf[, askstill := ifelse(length(na.omit(bf_still)) > 0, 1, 0), by = c("nid")] 
          bf <- bf[askstill == 1,]
          bf[, value := bf_still]
