...
all_causes <- all_causes[! cause_id %in% 411] 
...
causes <- causes[causes != 411] 

if(gamma_param_draws) {
  cause_temp <- data.table(cause_id = causes)
  cause_temp <- merge(cause_temp, cause_ids[,.(cause_id, acause)], all.x = T)
  cause_temp[cause_id == 1116, acause := "neo_breast_premenopause"] 
  cause_temp[cause_id == 1117, acause := "neo_breast_postmenopause"] 
...
print("Applying age attenuation to IHD, ischemic stroke, intracerebral hemorrhage, subarachnoid hemorrhage, and diabetes.")
