...
if(save_dir %like% "FILEPATH"){
  ...
  df <- fread(paste0(save_dir, "FILEPATH/step3_signal_data.csv"))
  mod2 <- py_load_object(filename = paste0(save_dir, "FILEPATH/step2_signal_model.pkl"), pickle = "dill")
  all_covs <- readRDS(paste0(save_dir, "FILEPATH/step3_signal_covs.RDS"))
...
if(fit_model){
  ...
  print(paste0("Saving model to ", save_dir, "FILEPATH/step4_signal_model.pkl"))
  py_save_object(model, filename = paste0(save_dir, "FILEPATH/step4_signal_model.pkl"), pickle = "dill")
  print("success")
  
} else {
  
  model <- py_load_object(paste0(save_dir, "FILEPATH/step4_signal_model.pkl"), pickle = "dill")
