...
if(save_dir %like% "FILEPATH"){
  df <- fread(paste0(save_dir, "FILEPATH/step1_signal_data.csv"))
  df[, a_mid := (a_0 + a_1)/2][, b_mid := (b_0 + b_1)/2][, linear := b_mid - a_mid]
  df_meta <- fread(paste0(save_dir, "FILEPATH/step1_data.csv"))
  mod2 <- py_load_object(filename = paste0(save_dir, "FILEPATH/step2_signal_model.pkl"), pickle = "dill")
  covs <- readRDS(paste0(save_dir, "FILEPATH/step3_candidate_covs.RDS"))
}
...
# Just for cause ID 543
if(cause_num == 543 & length(unique(df_meta$custom_latelife_bmi)) != 1) {
  
  bias_and_alpha_cols <- c(bias_and_alpha_cols, "custom_latelife_bmi")
  bias_cols <- c(bias_cols, "custom_latelife_bmi")
  
}

# Drop cov columns that are not unique
if(cause_num == 543) {
  for(cov in bias_and_alpha_cols) {
    if(length(unique(df_meta[, get(cov)])) == 1) {
      bias_and_alpha_cols <- bias_and_alpha_cols[bias_and_alpha_cols != cov]
      bias_cols <- bias_cols[bias_cols != cov]
    }
  }
}
...
if(fit_model){
  ...
  saveRDS(selected_covs,
          file = paste0(save_dir, "FILEPATH/step3_signal_covs.RDS"))
  fwrite(df_merge,
         file = paste0(save_dir, "FILEPATH/step3_signal_data.csv"))
