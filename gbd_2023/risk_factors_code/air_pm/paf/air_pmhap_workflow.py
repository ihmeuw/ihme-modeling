...
def tool_air_pmhap_paf():
    ...
    # define filepaths
    script_path = "FILEPATH" # "/homes/ekmarsh/repos/air_pollution/air/paf/"
    home_dir = "FILEPATH" # "/ihme/erf/GBD2020/"
    ...
    out_bwga_dir = f"/ihme/erf/GBD2022/air_pmhap/rr/bwga/draws/{output_version}/"
    ...
    for year_id in years:
        for location_id in locations:
            ...
            if f"{location_id}_{year_id}.csv" not in bwga_files:
                ...
                #     size = os.path.getsize(f"/ihme/erf/GBD2020/air_pm/exp/gridded/{exposure_grid_version}/draws/{location_id}_2020.fst")
                # else:
                #     size = os.path.getsize(f"/ihme/erf/GBD2020/air_pm/exp/gridded/{exposure_grid_version}/draws/{location_id}_{year_id}.fst")

                # size = os.path.getsize(f"/ihme/erf/GBD2022/air_pm/exp/gridded/{exposure_grid_version}/draws/{location_id}_{year_id}.fst")
                size = os.path.getsize(f"/ihme/erf/GBD2022/air_pm/exp/gridded/{exposure_grid_version}/draws_final/{location_id}_{year_id}.fst")
    ...
    wfr = workflow.run(resume=True)
    # print(f"Workflow {workflow.workflow_id} completed with status {status}.") # status isn't defined anywhere... -nhashmeh
