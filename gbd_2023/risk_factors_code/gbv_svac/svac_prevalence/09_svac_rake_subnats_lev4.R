...
file.list <- paste0(root, "FILEPATH/", risk, '/', run_id, '/', version, '/', c(l, as.character(unique(locs[level==4 & parent_id==l]$location_id))), '.csv')
...
for(c.loc in unique(raked$location_id)){
  print(c.loc)
  write.csv(raked[location_id == c.loc], paste0(root, "FILEPATH/", risk, '/', run_id,"/", version, "/", c.loc,".csv") )
}
