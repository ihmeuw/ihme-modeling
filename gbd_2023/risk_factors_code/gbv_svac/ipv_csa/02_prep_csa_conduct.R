...
# 
# Author: USERNAME
# Purpose: Clean & prepare CSA - Conduct Disorder RR data for modeling
#
####################################################################################################################################################################
...
#conduct data
conduct <- data[cause %like% 'conduct disorder']

conduct[!is.na(most_adjusted_mean), `:=` (mean=most_adjusted_mean, lower=most_adjusted_lower, upper=most_adjusted_upper)]
conduct[is.na(most_adjusted_mean), `:=` (mean=raw_mean, lower=raw_lower, upper=raw_upper)]

#start to create subsets by exposure type
cm_conduct <- conduct[exposure_timing=='Child maltreatment']
length(unique(cm_conduct$Study_ID))

#get csa
cm_conduct[, .(length(unique(Study_ID))), by=.(violence_type)]
csa_conduct <- cm_conduct[violence_type == 'sexual']
length(unique(csa_conduct$Study_ID))

study_info <- fread(paste0(study_info_dir, ro_pair, '_annotated.csv'))
csa_conduct <- merge(csa_conduct, study_info[, .(`Covidence_#`, duplicate_study)], all.x=T)

csa_conduct <- csa_conduct[!(Study_ID=='nelson 2002' & sex!='Combined Male and Female')]

model_input <- copy(csa_conduct[duplicate_study==0])
...
write.csv(model_input, paste0(mrbrt_out, 'abuse_csa-mental_conduct.csv'), row.names=F)
...
write.csv(table_output, paste0(table_out, 'csa_conduct_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
...
write.csv(bc_output, paste0(table_out, 'csa_conduct_biascovs_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
