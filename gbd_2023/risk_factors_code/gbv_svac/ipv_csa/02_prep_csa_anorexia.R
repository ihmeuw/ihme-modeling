...
# 
# Author: USERNAME
# Purpose: Clean & prepare CSA - Anorexia Nervosa RR data for modeling
#
####################################################################################################################################################################
...
#for studies where multiple possible data points, select those that should be used in model
...
#bind on additional study-info and covariate information
...
#fix missing extraction information manually
csa_eating_disorders[Study_ID=='talmon 2021', Uncertainty_type:='confidence interval']

#choose modeling point for Kendler 2000
csa_eating_disorders[Study_ID %in% c('kendler 2000') & exposed_level!='any exposure', use_model:=0]

model_input <- copy(csa_eating_disorders[use_model==1])
model_input <- model_input[cause=='anorexia nervosa']
...
write.csv(model_input, paste0(mrbrt_out, 'abuse_csa-mental_eating_anorexia.csv'), row.names=F)
...
write.csv(table_output, paste0(table_out, 'csa_anorexia_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
...
write.csv(bc_output, paste0(table_out, 'csa_anorexia_biascovs_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
