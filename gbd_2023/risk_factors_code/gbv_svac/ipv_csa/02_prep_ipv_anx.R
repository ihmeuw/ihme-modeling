...
# 
# Author: USERNAME
# Purpose: Clean & prepare IPV - Anxiety Disorders RR data for modeling
#
####################################################################################################################################################################
...
anx <- anx[Study_ID!='daundasekara 2022'] #exclude study duplicative with Suglia 2011
anx <- anx[Study_ID!='vos 2006'] #exclude study with self-reported AD diagnosis and also that is effectively XC
anx <- anx[Study_ID!='dichter 2017'] #duplicative with Makaroun 2020
anx <- anx[Study_ID!='chandan 2020'] #duplicative with cprd analyses

#start to create subsets by exposure type
ipv_anx <- anx[perpetrator_type == 'partner' | perpetrator_type=='partner; former partner']
length(unique(ipv_anx$Study_ID))

anx_study_info <- fread(paste0(study_info_dir, 'ipv_anx_info_annotated.csv'))
ipv_anx <- merge(ipv_anx, anx_study_info[, .(Study_ID, `Covidence_#`, exposure_timing, cv_symptom_scale, subpopulation, cv_subpop,
                                             `control for outcome at baseline`, cv_baseline_uncontrolled)], 
...
#make some fixes
ipv_anx[Study_ID=='brown 2020' & exposure_recall_type=='past year', exposure_recall_type:='past year at 10-year follow-up']
ipv_anx[Study_ID=='ouellet-morin 2015' & outcome_definition %like% 't3', `:=` (fup_years=7, exposure_recall_type='exposed at t1 and/or t2, outcome at t3')]
ipv_anx[Study_ID=='ouellet-morin 2015' & !outcome_definition %like% 't3', `:=` (fup_years=5, exposure_recall_type='exposed at t1 and/or t2, outcome at t2')]
ipv_anx[Study_ID=='suglia 2011', exposure_recall_type:=exposed_level]
ipv_anx[Study_ID=='suglia 2011', exposed_level:='any exposure']
ipv_anx[Study_ID=='ahmadabadi 2020' & violence_type=='emotional', violence_type:='psychological']

#select best data points
ipv_anx[, use_model:=1]
ipv_anx[Study_ID=='chowdhary 2008' & exposure_recall_type!='lifetime', use_model:=0]
ipv_anx[Study_ID=='brown 2020' & exposure_recall_type!='past year at 10-year follow-up', use_model:=0]
ipv_anx[Study_ID=='suglia 2011' & exposure_recall_type!='ipv prior to 12-month follow-up', use_model:=0]
ipv_anx[Study_ID=='suglia 2011' & outcome_type=='prevalence', use_model:=0]
ipv_anx[Study_ID=='ouellet-morin 2015' & exposed_level!='any exposure', use_model:=0]
ipv_anx[Study_ID=='ouellet-morin 2015' & !outcome_definition %like% 't3', use_model:=0]
ipv_anx[Study_ID=='ehrensaft 2006' & is.na(most_adjusted_upper), most_adjusted_upper:=most_adjusted_other_uncertainty_value]

model_input <- ipv_anx[use_model==1 & sex=='Female']
model_input <- model_input[!(Study_ID %in% unique(ipv_anx[violence_type=='physical; sexual' & violence_type_combination=='and/or']$Study_ID) & violence_type!='physical; sexual')]
...
model_input <- model_input[!(Study_ID %in% c('brown 2020', 'makaroun 2020') & cause!='anxiety disorders')]

unique(model_input[, .(Study_ID, violence_type, cause)])

#final covariates 
model_input[, cv_aggregate_outcomedef:=ifelse(cause %in% c('depressive disorders, anxiety disorders'), 1, 0)]
model_input[, cv_component_outcomedef:=ifelse(cause %in% c('post-traumatic stress disorder', 'generalized anxiety disorder'), 1, 0)]
model_input[, cv_aggregate_expdef:=0]
model_input[violence_type %in% c('violent relationship', 'physical; sexual; psychological', 'physical; psychological'), cv_aggregate_expdef:=1]
...
#scale se's
model_input[Study_ID %in% c('ahmadabadi 2020'),
            ln_rr_se:=ln_rr_se*sqrt(2)]

# #remove aggregate exp defs
...
#remove females only covariate
model_input[, bc_females_only:=NULL]

#get list of covariates meeting minimum value criteria
...
write.csv(model_input, paste0(mrbrt_out, 'abuse_ipv_exp-mental_anxiety.csv'), row.names=F)
...
write.csv(table_output, paste0(table_out, 'ipv_anx_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
