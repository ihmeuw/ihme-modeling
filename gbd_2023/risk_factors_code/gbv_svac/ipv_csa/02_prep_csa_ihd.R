...
# 
# Author: USERNAME
# Purpose: Clean & prepare CSA - Ischemic Heart Disease RR data for modeling
#
####################################################################################################################################################################
...
#ihd data
ihd <- data[cause %like% 'myocardial infarc' | outcome_name %like% 'myocardial infarc' | outcome_name %like% 'heart attack' | cause %like% 'ischemi' | outcome_name %like% 'ischemi']

ihd[!is.na(most_adjusted_mean), `:=` (mean=most_adjusted_mean, lower=most_adjusted_lower, upper=most_adjusted_upper)]
ihd[is.na(most_adjusted_mean), `:=` (mean=raw_mean, lower=raw_lower, upper=raw_upper)]

#start to create subsets by exposure type
cm_ihd <- ihd[exposure_timing=='Child maltreatment']
length(unique(cm_ihd$Study_ID))

#get csa
cm_ihd[, .(length(unique(Study_ID))), by=.(violence_type)]
csa_ihd <- cm_ihd[violence_type == 'sexual']
length(unique(csa_ihd$Study_ID))

study_info <- fread(paste0(study_info_dir, ro_pair, '_annotated.csv'))
csa_ihd <- merge(csa_ihd, study_info[, .(`Covidence_#`, bc_subpop, duplicate_study, bc_mi_only)], all.x=T)

model_input <- copy(csa_ihd[duplicate_study==0])
...
write.csv(model_input, paste0(mrbrt_out, 'abuse_csa-cvd_ihd.csv'), row.names=F)
...
write.csv(table_output, paste0(table_out, 'csa_ihd_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
...
write.csv(bc_output, paste0(table_out, 'csa_ihd_biascovs_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
