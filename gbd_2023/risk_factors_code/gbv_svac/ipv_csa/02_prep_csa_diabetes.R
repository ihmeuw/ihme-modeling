...
# 
# Author: USERNAME
# Purpose: Clean & prepare CSA - Diabetes RR data for modeling
#
####################################################################################################################################################################
...
#diabetes data
diabetes <- data[cause %like% 'diabetes' & !cause %like% 'gest']

diabetes[!is.na(most_adjusted_mean), `:=` (mean=most_adjusted_mean, lower=most_adjusted_lower, upper=most_adjusted_upper)]
diabetes[is.na(most_adjusted_mean), `:=` (mean=raw_mean, lower=raw_lower, upper=raw_upper)]

#start to create subsets by exposure type
cm_diab <- diabetes[exposure_timing=='Child maltreatment']
length(unique(cm_diab$Study_ID))

#get csa
cm_diab[, .(length(unique(Study_ID))), by=.(violence_type)]
csa_diab <- cm_diab[violence_type == 'sexual']
csa_diab <- csa_diab[Study_ID!='mason 2013']
length(unique(csa_diab$Study_ID))

#exclude duplicative using BRFSS
csa_diab <- csa_diab[Study_ID!='campbell 2016']

study_info <- fread(paste0(study_info_dir, ro_pair, '_annotated.csv'))
csa_diab <- merge(csa_diab, study_info[, .(`Covidence_#`, bc_subpop,
                                         bc_selfreported, bc_type_nonsp, bc_prevalent_diabetes)], all.x=T)
csa_diab[Study_ID=='duncan 2015', cause:='diabetes mellitus']

csa_diab <- csa_diab[!(Study_ID=='rich-edwards 2010' & subgroup_analysis=='yes')]

model_input <- copy(csa_diab)
...
write.csv(model_input, paste0(mrbrt_out, 'abuse_csa-diabetes.csv'), row.names=F)
...
write.csv(table_output, paste0(table_out, 'csa_diabetes_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
...
write.csv(bc_output, paste0(table_out, 'csa_diabetes_biascovs_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
