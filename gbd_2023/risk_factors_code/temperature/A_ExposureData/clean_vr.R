...
clean_vr = function(df, paths, varnames, iso3) {
  ...
  if (iso3 == "CHN") {
    ...
    df <- merge(df, plink, by = 'province', all.x = T)
  ...
  big <- merge(df, icdmap, by = 'value', all.x = T)

  # where icd codes failed to link to gbd cause, remove decimal precision and re-merge to icd map
  big[big$cause_id == 743 | is.na(big$cause_id) == T, value := gsub("\\..*$", "", value)]
  big <- big[, 'cause_id' := NULL]
  big <- merge(big, icdmap, by = 'value', all.x = T)
