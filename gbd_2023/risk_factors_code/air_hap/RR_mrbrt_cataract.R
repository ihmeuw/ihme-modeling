# Purpose: Fit MR-BRT models to HAP cataract RR data
...
model.version <- 9 # adding Fisher Info boost
...
df <- openxlsx::read.xlsx(paste0(in_dir, "/extraction_ier_hap_GBD2019.xlsm"), sheet="extraction", startRow=3, colNames=F, skipEmptyRows = T, skipEmptyCols = F) %>% as.data.table
names(df) <- openxlsx::read.xlsx(paste0(in_dir, "/extraction_ier_hap_GBD2019.xlsm"), sheet="extraction", colNames=T, skipEmptyCols = F, rows=1) %>% names
...
# No exposures are reported for the studies in PM2.5 (because all the studies use solid fuels as their exposure)
# We're running a simple meta-analysis that doesn't use any PM2.5 exposure as a predictor because we're assuming
# they all have the same exposure.
df[, linear_exp := 1]
...
# only fit intercept model
# then re-select covariates
# then fit model again
...
if (run_covs){
  ...
  # save covs for later
  if(save_covs){
    saveRDS(covs, paste0(out_dir, "covs_031620.RDS"))
  }
}

if(load_covs){
  covs <- readRDS(paste0(out_dir, "covs_11102020.RDS"))
}
...
# -------------------------  Get evidence score  ------------------------------------------
# get score for inclusion/exclusion (from USERNAME)

betas <- samples[[1]]
inclusion <- quantile(betas[, 1], 0.05)  # this first row is for the slope
# the fifth percentile is 0.6542019, so it should be included
