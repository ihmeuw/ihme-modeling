...
pacman::p_load(data.table, gridExtra, ggplot2, lme4, magrittr, parallel, stringr, zoo, openxlsx, fst)
...
location_set_id <- 22
all.ages <- c(2:3,6:20,30:32,34,235,238,388:389)
...
ageWrapper <- function(this.age,
  ...
  #loop through all the years and keep rows relevant to our cohort
  ...
  cohortBuilder <- function(this.year,
    ...
    if (cohort.age.start < 0) stop("age start is zero")
...
#some subnationals do not have developed status in db
#climb the hierarchy to fill in
if(is.na(dev.status)) {
...
ages[age_group_id == 238, age_group_name := "12-23 months"]
...
age.map[age_group_id == 2, c("age_group_years_start","age_group_years_end") := list(0,6/365.25)]
age.map[age_group_id == 3, c("age_group_years_start","age_group_years_end") := list(7/365.25,27/365.25)]
age.map[age_group_id == 238, c("age_group_years_start","age_group_years_end") := list(1,23/12)]
age.map[age_group_id == 388, c("age_group_years_start","age_group_years_end") := list(1/12,5/12)]
age.map[age_group_id == 389, c("age_group_years_start","age_group_years_end") := list(6/12,11/12)]
age.map[age_group_id %in% c(8:20,30:32,235), age_mid := ceiling((age_group_years_start + age_group_years_end)/2)]
age.map[age_group_id %in% c(6:20,30:32,34,235), age_group_years_end := age_group_years_end + 1]

## then get pop
# read in population using central function
pops <- get_population(location_set_version_id=location_set_version_id,
                       ...
                       age_group_id = all.ages, gbd_round_id = gbd_round_id, decomp_step = decomp_step)
#***********************************************************************************************************************
...
#mclapply error drops some years when running at high cores. if failed, keep reducing the cores until you
...
current[ccode=='PAH', carcinogen := 'pah']
