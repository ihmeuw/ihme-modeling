...
# load packages
pacman::p_load(data.table, gridExtra, ggplot2, lme4, magrittr, parallel, stringr, openxlsx, zoo, fst)
...
dt[me_name == "occ_occ_major_5_sales", (worker.cols) := lapply(.SD, function(x) x/2), .SDcols = worker.cols]
...
dt[isco_code==6, exposure_me := 'agriculture'] #note this is skilled agriculture
...
# for any year-age-sex combos that have an exposure that sums to > 1, need to squeeze so that it equals 1
exp.sum.cols <- paste0("exp_sum_",0:999)
dt[, (exp.sum.cols) := lapply(.SD, sum), .SDcols = exp.cols, by = c("year_id","sex_id","age_group_id")] # exposure sums for all 1000 draws
for (x in exp.sum.cols) {
  exp.col <- gsub("_sum","",x)
  dt[get(x) > 1, paste0(exp.col) := get(exp.col)/sum(get(exp.col)), by = c("year_id","sex_id","age_group_id")] # if the exposure sums to > 1, squeeze draw to 1
}
dt[, (exp.sum.cols) := NULL]
