...
source("/FILEPATH/wipe_db.R")
source("/FILEPATH/get_model_documentation.R")
...
bundles_toggle <- T
version_toggle <- F
xwalk_description <- "Step 2, adding new data for GBD 2020"
...
bundles <- fread("/FILEPATH/occ_me_ids.csv")[grepl("occ_occ_",me_name)]
...
file.path(ubcov.function.dir, "db_utilities.r") %>% source
...
#Armed forces also does not need to be split
...
saveData <- function(cat, dt) {
  ...
  if (decomp_step %in% c("step4","iterative")) {
    ...
    if (this.entity == "occ_occ_major_7_craft") bvid <- 4484
