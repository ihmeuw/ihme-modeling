...
#merge on levels
locs <- get_location_metadata(gbd_round_id = 7, location_set_id = 22, decomp_step = decomp_step)
...
recent_runs <- read.xlsx("/FILEPATH/stgpr_run_log.xlsx") %>% data.table
...
dt <- fread(file.path(data.dir, best.emp$run_id, 'draws_temp_0', paste0(country, '.csv')))
setkeyv(dt, c('location_id', 'year_id', 'age_group_id', 'sex_id'))

#coastal prop covariate only goes back to 1980, need to duplicate for missing years
