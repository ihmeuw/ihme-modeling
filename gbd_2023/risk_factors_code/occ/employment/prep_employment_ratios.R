...
## read in microdata
microdata_reg <- fread(file.path(dir,paste0("/emp_regular_",microdata_version,".csv")))
microdata_cens <- fread(file.path(dir,paste0("/emp_census_",microdata_version,".csv")))
...
locations <- get_location_metadata(gbd_round_id = gbd_round_id, location_set_id = 22, decomp_step = decomp_step)
...
## some variance is too small. If recalculated variance from above helps, use that. Otherwise recalculate
...
# NEW ILO TABULATIONS -----------------------------------------------------
new <- fread(file.path(dir,"/emp_ilo_byagesex_02_26_20.csv"))[!is.na(obs_value),list(ihme_loc_id = ref_area,country = ref_area.label,year_id = time,survey_id = paste0(source.label," - ",time),sex_id = ifelse(grepl("M",sex),1,ifelse(grepl("F",sex),2,3)),age = classif1.label,data = obs_value/100,note1 = note_classif.label,note2 = note_source.label)]
...
new <- new[,age := gsub(".*: *","", age)] #annotated command: gsub(paste0(".*",  # match anything (.) 0 or more times (*)
#            ":",   # match a colon
#            " *"), # match a space 0 or more times
#     "", age)
...
gold_ids <- new[sex_id != 3,all(seq(8,18) %in% unique(age_group_id)),by=.(survey_id,sex_id)]
gold_ids <- gold_ids[V1 == T,all(c(1,2) %in% unique(sex_id)),by=survey_id][V1 == T,survey_id]
...
# OLD ILO TABULATIONS -----------------------------------------------------
old <- fread(file.path(dir,"/emp_ilo_byagesex_old.csv"))[!is.na(obs_value),list(ihme_loc_id = ref_area,country = ref_area.label,year_id = time,survey_id = paste0(ref_area," - ",source.label," - ",time),sex_id = ifelse(grepl("M",sex),1,ifelse(grepl("F",sex),2,3)),age = classif1.label,data = obs_value/100,note1 = note_classif.label,note2 = notes_source.label)]
...
# WRITE OUTPUTS -----------------------------------------------------------
if (do_split) {
  write.csv(df_both, file.path(dir,"/emp_microbothILO_final.csv"),row.names=F)
} else {
  write.csv(df_both,file.path(dir,"/emp_microbothILO_agesexspecific.csv"),row.names=F)
  write.csv(new_split,file.path(dir,"/emp_newILO_tobesplit.csv"),row.names=F)
  write.csv(old_split,file.path(dir,"/emp_oldILO_tobesplit.csv"),row.names=F)
}
...
if (generate_covs) {
  ...
  ## religion_muslim_prop, adapting so don't have to run model by sex. 
  ## Replacing male values with 0's since it isn't as predicitve
  source("/FILEPATH/get_covariate_estimates.R")
  muslim_prop <- get_covariate_estimates(covariate_id = 1218, gbd_round_id = 7, decomp_step = "step2")
  muslim_prop[, sex_id := 2]
  fake_df <- copy(muslim_prop)
  fake_df[, sex_id := 1]
  fake_df[, mean_value := 0]
  muslim_prop <- rbind(muslim_prop, fake_df)

  ## proportion of total population
  pops <- get_population(location_id = "-1",age_group_id = c(22,microdata[,unique(age_group_id)]),location_set_id = 22,
                         year_id = seq(1980,2022),sex_id = c(1,2),gbd_round_id = gbd_round_id,decomp_step = decomp_step)
  ...
  gov_exp <- fread("/FILEPATH/scenario_stats.csv")[scenario == 0 & year < 2023,list(ihme_loc_id = iso3,year_id = year,gov_exp = mean)]
  ...
  custom_covs <- merge(custom_covs, muslim_prop[,.(location_id, year_id, sex_id, mean_value)], by = c("location_id", "year_id", "sex_id"))
  setnames(custom_covs,c("pop_perc", "mean_value", "gov_exp"),c("cv_pop_perc", "cv_religion_muslim_prop","cv_gov_exp"))
  

  custom_covs[,cv_pop_perc := round(cv_pop_perc,3)]
  custom_covs[,cv_gov_exp := round(cv_gov_exp,3)]
  write.csv(custom_covs,file.path(dir,"emp_custom_covs.csv"),row.names=F)
}
