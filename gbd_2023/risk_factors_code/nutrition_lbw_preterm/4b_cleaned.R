...
createMeanPrevList <- function(meids, location_ids, sex_ids, year_ids, age_group_ids, num_draws, decomp_step, gbd_round_id){
  
  # Get draws for all meids and reshape long
  draws <- lapply(meids, function(meid){
    
    draws <- get_draws(gbd_id_type = 'modelable_entity_id', source = "epi",
                       gbd_id = meid,
                       location_id = location_ids, sex_id = sex_ids, year_id = year_ids, age_group_id = age_group_ids,
                       decomp_step = decomp_step, gbd_round_id = gbd_round_id)
...
get_ensemble_density <- function(weights, mean, sd, XMIN, XMAX){
  #' This function is an adapatation of get_edensity.R (check out original code at: FILEPATH/edensity.R)
  ...
  for(z in 1:length(weights)){
    ...
    # dlist[[distr]] subsets to the functions associated with a distribution (e.g., lnorm, invgamma, exp)
    # mv2par is a function associated with each distribution that tells you how to convert from m*ean and v*ariance to the distribution par*ameters (mv2par)
    # Some distribution require an XMIN and XMAX to estimate the distribution parameters. LENGTH is the number of arguments needed for mv2par (either 2 parameters: mean and variance
    # or 4 parameters: mean, variance, XMIN, and XMAX)
    ...
    # If you're are running interactively, you at this point can do plot(x, fxj) to see what the individual distribution looks like
    # plot(x, fxj)
