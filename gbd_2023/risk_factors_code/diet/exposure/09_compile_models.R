################################################################################
## DESCRIPTION ##  Compile sales and fao models from stgpr & make whole grains ratio 
## INPUTS ##    need a list of run_ids for the sales and fao models
#################################################################################
...
models_compile_version <- args[1]
gbd_round <- args[2]

#####
stgpr_runids <- fread(sprintf("FILEPATH", gbd_round))
stgpr_runids <- stgpr_runids[include_fao_sales_compiler=="yes" & me_name!="hvo_sales_g_adj",.(me_name, run_id, sales_model)]  # dropping modeled transfat data
compiled_data_file <- paste0("FILEPATH")
...
compile_models <- function(runids){
  ...
  #add non-modeled transfat sales data
  transfat_sales_data <- fread(sprintf("FILEPATH/ready_to_model/hvo_sales_g_unadj.csv", gbd_round, models_compile_version))
  transfat_sales_data[, c("duplicate", "sample_size", "me_name", "cv", "sd_resid", "ihme_loc_id", "location_name", "is_outlier", "measure"):=NULL]
  setnames(transfat_sales_data, "data", "gpr_mean")
  transfat_sales_data[, back_se:=sqrt(variance)]
  
  compiled_estimates[, back_se:=(gpr_upper-gpr_lower)/(2*1.96)]
  compiled_estimates[, variance:=(back_se)^2]
  
  compiled_estimates <- rbind(compiled_estimates, transfat_sales_data, fill=TRUE)
  
  compiled_estimates[ grep("sale", gbd_cause), cv_sales_data:=1]
  compiled_estimates[ !(grep("sale", gbd_cause)), cv_fao:=1]
  
  
  #grains need to go here
...
make_and_add_whole_grains <- function(models_compile_version, gbd_round, just_append = FALSE){
  ...
  ##########################
  compiled_data_file <- paste0("FILEPATH",gbd_round,"/modeling/st_gpr/fao/modeled_results/FAO_sales_clean_compiled_",models_compile_version, ".csv" )
  ...
  whole_grains_file <- paste0("FILEPATH",gbd_round,"/modeling/st_gpr/fao/modeled_results/whole_grains/whole_grains_", models_compile_version, ".csv")
  if(!just_append){
  ...
  code <- paste0(code_dir, "diet_exposure/code/active_code/exposure/fao_data/099_make_fao_whole_grains.do")
  arg_list <- list(version = models_compile_version, gbd_round = gbd_round)
  qsub(jobname=jobname, code, pass_argparse=arg_list, mem=2, cores=1, wallclock= "00:30:00", proj="proj_covariates", queue = "all", submit=F, 
  shell = paste0(code_dir,"diet_exposure/code/active_code/stata_shell.sh"), archive_node = TRUE)
  message("pausing for 3 minutes to let job finish")
  Sys.sleep(180)   #pause for 3 minutes
  
  #check if qsub worked

  if( !file.exists(whole_grains_file)){ stop(" Check log files and rerun.")}
  }
  if(just_append){if( !file.exists(whole_grains_file)){ stop("There is no whole grains data to append!")}}
