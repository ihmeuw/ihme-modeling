######################################################################################################################################################################################################################################
#
# Purpose: Logit rake level 5 and 6 subnats to raked level 4 parent subnats
#
#####################################################################################################################################################################################################################################
# rm(list=ls())
...
file.list <- paste0(root, "FILEPATH", run_id, '/', version, '/', c(as.character(unique(locs[parent_id==l]$location_id))), '.csv')
file.list %>% lapply(read_draw) %>% rbindlist() -> unraked_data
print('data loaded')

#clean up
unraked_data[, c('version', 'upper', 'lower'):=NULL]
if ('V1' %in% colnames(unraked_data)) {
  unraked_data$V1 <- NULL
}
...
lvl4 <- fread(paste0(root, "FILEPATH", run_id, '/', version, '/', l, '.csv'))
...
pops <- get_population(release_id = release, age_group_id = 'all', location_id = 'all', sex_id = 2, year_id = 'all') #only pull pop for these lvl3+4 locs; makes faster
pops$run_id <- NULL
pops$sex_id <- NULL
...
#save by location
dir.create(paste0(root,"FILEPATH",run_id,"/", version, "/"))
