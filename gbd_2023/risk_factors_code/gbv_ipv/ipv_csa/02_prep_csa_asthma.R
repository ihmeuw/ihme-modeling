...
# 
# Author: USERNAME
# Purpose: Clean & prepare CSA - Asthma RR data for modeling
#
####################################################################################################################################################################
...
#asthma data
asthma <- data[cause %like% 'asthm']

asthma[!is.na(most_adjusted_mean), `:=` (mean=most_adjusted_mean, lower=most_adjusted_lower, upper=most_adjusted_upper)]
asthma[is.na(most_adjusted_mean), `:=` (mean=raw_mean, lower=raw_lower, upper=raw_upper)]

#start to create subsets by exposure type
cm_asthma <- asthma[exposure_timing=='Child maltreatment']
length(unique(cm_asthma$Study_ID))

#get csa
cm_asthma[, .(length(unique(Study_ID))), by=.(violence_type)]
csa_asthma <- cm_asthma[violence_type == 'sexual' | Study_ID=='coogan 2013']
length(unique(csa_asthma$Study_ID))

study_info <- fread(paste0(study_info_dir, ro_pair, '_annotated.csv'))
csa_asthma <- merge(csa_asthma, study_info[, .(`Covidence_#`, bc_subpop, bc_regular_symptoms)], all.x=T)

csa_asthma <- csa_asthma[!(Study_ID=='coogan 2013' & !violence_type %in% c('sexual', 'physical; sexual'))]
csa_asthma <- csa_asthma[!(Study_ID=='coogan 2013' & !violence_type_combination %in% c('only', 'and'))]
csa_asthma <- csa_asthma[!(Study_ID=='coogan 2013' & subgroup_analysis=='yes')]
csa_asthma <- csa_asthma[!(Study_ID=='coogan 2013' & exposure_age_upper==18)]

csa_asthma <- csa_asthma[!(Study_ID=='han 2022' & sex!='Combined Male and Female')]

model_input <- copy(csa_asthma)
model_input[Study_ID=='kascakova 2022', bc_confounding_uncontrolled:=1]
...
write.csv(model_input, paste0(mrbrt_out, 'abuse_csa-asthma.csv'), row.names=F)
...
write.csv(table_output, paste0(table_out, 'csa_asthma_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
...
write.csv(bc_output, paste0(table_out, 'csa_asthma_biascovs_',gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
