...
education_run_id <- 216400 
rei_id <- 363
me_id <- 18740 
cause_ids <- c(568, 571)
location <- args[1]
years <- c(1990:2024)
ages <- c(6:20, 30:32, 235) # ages younger than 5 have 0 exposure and are manually added at the end
release <- 16  
...
rr_draws <- fread("/FILEPATH/rr_mrt_brt_GBD2020.csv")
rr_draws[, `:=` (rr = exp(rr), mean_rr = exp(mean_rr))]
rr_draws[rr == 0, rr := mean_rr]
rr_draws <- rr_draws[mean_rr > 1,]
rr_draws[, mean_rr := NULL]

### Load persistence estimates from meta-regression ####
persistence_draws <- fread("/FILEPATH/persistence_draws.csv")
...
#### Pull Exposure ####
if(me_id == 18740){
  exposure <- interpolate(gbd_id_type = "modelable_entity_id", gbd_id = me_id, measure_id = 18, source = "epi", location_id = location, reporting_year_start=min(years), reporting_year_end=max(years), age_group_id = ages, sex_id = c(1,2), status = "best", release_id = release)
} else {
  exposure <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = me_id, measure_id = 18, source = "epi", location_id = location, year_id = years, age_group_id = ages, sex_id = c(1,2), status = "best", release_id = release)
}
#### Assume 1990 prevalence back to 1920 ####
for(y in c(1920:1989)){
...
population <- get_population(age_group_id = ages, location_id=location, year_id=c(1950:2024), sex_id=c(1, 2), release_id = release)
for(y in c(1920:1949)){
...
in_school_5to9[, `:=` (age_group_start = 5, prevalence = prevalence * 4/5)]
...
in_school_6to10 <- in_school[age_group_start==6, ] 
in_school_7to11 <- copy(in_school[age_group_start==6, ])
in_school_7to11[, age_group_start := 7]
...
in_school[prop_in_school>1, prop_in_school := 1] 
...
for(sex in c(1, 2)){
  for(year in years){
    ...
    for(age_start in unique(exposure$age_group_start)){
      ...
      exposure_age[prevalence<0, prevalence := 0]
      exposure_age[, prev_sum_check := sum(prevalence), by = c("draw", "sex_id")]
      exposure_age[prev_sum_check > 1, prevalence := prevalence / prev_sum_check]
      exposure_age[, prev_sum_check := NULL]

      # Merge with RRs from meta-regression
      exposure_age <- merge(exposure_age, rr_draws, by=c("time", "draw", "sex_id"))
      ...
      # Change PAFs < 0 to 0 based on advice from NAME and NAME
    ...
    # Re-add age_group_ids under 5 years old and set estimates to 0
    for(infant_age in c(2:3, 388:389, 238, 34)){
    ...
    if(location == 179){
      pafs_byage[, location_id := 60908]
      write.csv(pafs_byage, paste0("/FILEPATH/paf_yld_60908_", year, "_", sex, ".csv"), row.names=F)
      pafs_byage[, location_id := 94364]
      write.csv(pafs_byage, paste0("/FILEPATH/paf_yld_94364_", year, "_", sex, ".csv"), row.names=F)
      pafs_byage[, location_id := 95069]
      write.csv(pafs_byage, paste0("/FILEPATH/paf_yld_95069_", year, "_", sex, ".csv"), row.names=F)
    } else {
      write.csv(pafs_byage, paste0("/FILEPATH/paf_yld_", location, "_", year, "_", sex, ".csv"), row.names=F)
    }
