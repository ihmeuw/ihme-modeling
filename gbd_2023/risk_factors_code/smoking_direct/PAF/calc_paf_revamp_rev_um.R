...
ref<-fread(FILEPATH, "paf_calc_ref.csv")
...
file_names <- list.files(paste0(output_path,"/sim_histories/py/sex_", s, "/", l, "/"),full.names = T)
...
path_current<-gsub("peripheral_artery_disease","peripheral_artery_disease_um",ref$path_current[ref$cause_id==c])
...
if (exp_def == "prevalence") {
  ...
  for (a_orig in ages) {
    ...
    # Is this still necessary for current RR? 
...
# this path cannot be created??
dir.create(paste0(FILEPATH,"pafs_ss_annual_um/"), showWarnings = F)
save_paf(dt = out, rid=99, rei="smoking_direct", n_draws=1000, out_dir=paste0(FILEPATH,"/pafs_ss_annual_um/"))
