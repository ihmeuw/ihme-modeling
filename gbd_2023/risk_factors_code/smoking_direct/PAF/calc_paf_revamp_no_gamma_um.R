###############################################################################################################
## Purpose: Calculate Smoking Unmediated PAFs for PAD 
###############################################################################################################
...
parameters <- fread(paste0(FILEPATH, "array_paf_no_gamma.csv"))
...
drawnum <- parameters[task_id, draw_num] %>% as.integer
...
drawvars<-c(paste0("draw_", 0:(drawnum-1))) 
age_map <- get_age_metadata(19)

ages_og<-c(11:20, 30, 31, 32, 235)
# this is the file for all the RR path, need to update the path for current RR.
ref<-fread(paste0(FILEPATH, "paf_calc_ref_no_gamma.csv"))
...
load(paste0(FILEPATH,"/sim_histories/cess/sex_", s, "/", l, ".RData"))
load(paste0(FILEPATH,"/sim_histories/amt/sex_", s, "/", l, ".RData"))

file_names <- list.files(paste0(FILEPATH,"/sim_histories/py/sex_", s, "/", l, "/"),full.names = T)
...
# keep only selected draws
current_prev<-current_prev[, c(idvars,drawvars), with=F]
...
# keep only selected draws
former_prev<-former_prev[, c(idvars,drawvars), with=F]
...
if (F){
  load(paste0(FILEPATH, "/py/sex_",s,"/",l,".RData"))
  
  current_prev_id_old <- 43997
  
  current_prev_old<-fread(paste0(FILEPATH, "smoking_prev_current_any_",current_prev_id_old, "/", l, ".csv"))
  ...
  former_prev_old<-fread(paste0(FILEPATH, "/smoking_prev_former_any_",former_prev_id_old, "/", l, ".csv"))
...
#########################################
## Calculate the Unmediated PAFs for PAD
#########################################

# Initialize the outframe
out<-NULL
c <- 502 # cause ID for PAD

message(paste0("Calculating Unmedidated PAFs for PAD, Cause ID: ", c))
# paths of unmdidated RR draws for current smokers and unmdidated RR draws for former smokers
path_current<-paste0(FILEPATH, "/peripheral_artery_disease_um.csv")
path_former <- paste0(FILEPATH, "/pad_former_um.csv")
...
if (exp_def == "prevalence") {
  rr_current<-fread(paste0(path_current))[cause_id==c] %>% .[draw < drawnum]
  ...
  rr_current <- fread(paste0(path_current)) %>% .[draw < drawnum]
  rr_former <- fread(paste0(path_former)) %>% .[draw < drawnum]
  ...
  for (a_orig in ages) {
    ...
    for (y_orig in years) {
      ...
      new_max_temp<-data.table(new_max=current_smoker_integral, draw=0:(drawnum-1))
      ...
      # Format the output
      pafs_temp<-data.table(cause_id = c, location_id = l, year_id = y_orig, age_group_id = a_orig, sex_id = s, draw = 0:(drawnum-1), paf = pafs_temp)
...
dir.create(paste0(FILEPATH,"pafs_ss_annual_um/"), showWarnings = F)
save_paf(dt = out, rid=99, rei="smoking_direct", n_draws=drawnum, out_dir=paste0(paf_path,"/pafs_ss_annual_um/"))
message("paf saved")
...
dir.create(paste0(FILEPATH,"/paf_summaries_ss_annual_um/"), showWarnings = F)
write.csv(out, paste0(FILEPATH,"/paf_summaries_ss_annual_um/", l, "_", s, ".csv"), na = "", row.names = F)
message("Finished!")
