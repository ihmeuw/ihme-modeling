...
# Author: 
# Date: 
# Purpose: Perform NO2 evidence scoring using MSCM guidelines:)
#          
...
df <- read.csv(paste0("FILEPATH")) %>% as.data.table


# left over from when we used the spline approach; now we're just using this for plotting
...
# only use this for covariate selection
...
make_plot <- function(dfpred, altvar, predmean, predlo, predhi){
  p <- ggplot()+
    # plot the datapoints
    geom_errorbarh(data = df, aes(xmin = NO2_conc_ref, xmax = NO2_conc_alt, y = log_mean_rescaled, alpha = 1/log_se_weighted), color = "black") +
    ...
    # # add a line to mark the predicted effect size for 5 ppb
    # geom_hline(aes(yintercept=effect_size),linetype="solid",color="red")+
    # geom_hline(aes(yintercept=effect_size_lo),linetype="dashed",color="red")+
    # geom_hline(aes(yintercept=effect_size_hi),linetype="dashed",color="red")+
...
lin_cov_model = mr$LinearCovModel(
  alt_cov=c("b_0","b_1"), # these are equivalent so you don't have to repeat these twice (can just use NO2_conc_ref & NO2_conc_alt)
  ref_cov=c("a_0","a_1"),
  use_re=T,
  use_spline=F,
  use_re_mid_point=T
...
if (run_covs){
  ...
  # NOTE: for GBD 2020, we did not include exposure timeframe covs (too many covariates)
  candidate_covs <- list("cv_exp_subpopulation","cv_exp_exposure_population","cv_exp_PM_controlled","cv_exp_outcome_selfreport",
                         "cv_exp_confounding_uncontrolled_1","cv_exp_confounding_uncontrolled_2","cv_exp_selection_bias_1","cv_exp_selection_bias_2",
                         # "cv_exp_exp_prenatal","cv_exp_exp_lifetime","cv_exp_exp_birth","cv_exp_exp_yo_incidence",
                         "cv_exp_duration_fup")
  ...
  # covs <- c(covs,"a_1","b_1") # this is leftover from when we used the spline approach
...
model <- mr$MRBRT(
  data = data,
  cov_models = list(lin_cov_model,
    mr$LinearCovModel(covs[1],use_re=F,prior_beta_gaussian=array(c(0, 0.1*beta_sd))),
    mr$LinearCovModel(covs[2],use_re=F,prior_beta_gaussian=array(c(0, 0.1*beta_sd))),
    mr$LinearCovModel(covs[3],use_re=F,prior_beta_gaussian=array(c(0, 0.1*beta_sd)))),
  inlier_pct = 0.9
  )
...
if (num_fill > 0) {
  ...
  model_fill <- mr$MRBRT(data, cov_models=model$cov_models)
  # model_fill <- mrbrt002::MRBRT(data, cov_models=model$cov_models)
...
pdf(paste0("FILEPATH"), width = 11,height = 7)
...
# -------------------------Get p-val for inclusion/exclusion-------------------------------
beta <- model$beta_soln[1]
beta_sd

get_pval <- function(beta, beta_sd, one_sided = FALSE) {
  zscore <- abs(beta/beta_sd)
  if (one_sided) return(1 - pnorm(zscore))
  if (!one_sided) return((1 - pnorm(zscore))*2)
}

get_pval(beta,beta_sd)
