...
unique(df$custom_domain)
df[,custom_leisure_pa := ifelse(custom_domain %in% c("sports", "recreation"), 1, 0)]
df[,custom_met_conversion := ifelse(custom_unit %like% "MET", 0, 1)]
...
bias_cols <- bias_cols[!bias_cols %in% c("custom_domain", "exposure_idv_score", "exposure_obj_score", "outcomes_blind_score", "exposure_mult_score",
                                         "control_rct_other_score", "reverse_causality_score", "selection_bias_score",
                                         "custom_incidence", "custom_mortality", "custom_male", "custom_female", "custom_unit", "custom_domain")]
...
if(fit_model){
  ...
  py_save_object(final_model, paste0(save_dir, "models/FILEPATH"), pickle = "dill")
  
} else {
  final_model <- py_load_object(paste0(save_dir, "models/FILEPATH"), pickle = "dill")
}
