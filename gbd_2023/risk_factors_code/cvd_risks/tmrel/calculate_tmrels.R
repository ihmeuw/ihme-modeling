...
mrbrt_version <- "final_model"
risk_grep <-"risk"
protective_data <- T
ro_addendum <- ""
mrbrt_dir <- paste0(out_dir, mrbrt_version,"/")
save_dir <- paste0(out_dir, "/results/")
exclude_ro_pair <- c()
note <- ""
sex_id <- 3
...
pairs <- gsub(".pkl","",list.files(paste0(mrbrt_dir, "/FILEPATH/"), pattern = ".pkl"))
...
rr_data <- rbindlist(lapply(causes$pair, function(p){
  ...
  input_data <- readRDS(paste0(mrbrt_dir, "/FILEPATH/", p, ".RDS"))
...
rr_draws <- rbindlist(lapply(causes$pair, function(p){
  ...
  tmrel <- df_final_pred[mean==min(mean), exposure]
    ...
    #y_draws <- cbind(df_final_pred[,.(a_0, b_0)], y_draws)
    y_draws[, ro := p]
