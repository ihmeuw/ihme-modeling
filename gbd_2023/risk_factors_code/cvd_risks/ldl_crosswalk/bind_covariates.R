...
bind_covariates<-function(df, cov_list, custom_cov_list=NULL, release_id){
  ...
  if(!"location_id"  %in% names(df) & "ihme_loc_id"  %in% names(df)){
    message("You're missing location_id; adding using location_set_id 22")
  ...
  for(i in 1:length(cov_list)){
    ...
    if(cov_list[i] %in% cust_covs){ # get covariate from custom location
      ...
      message(paste("Getting", cov_list[i], "from covariate database", 'using ID', cid))
  ...
  if(length(multi_age_covs)>0 | length(multi_agesex_covs)>0){
    if(!"age_group_id" %in% names(df) | all(df[["age_group_id"]]==22)){
      ...
      if("age_start" %in% names(df) & "age_end" %in% names(df)){
        ...
        # expand df for each necessary row--this is from age sex splitting function
