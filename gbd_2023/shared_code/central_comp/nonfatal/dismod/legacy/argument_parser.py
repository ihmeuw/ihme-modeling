...
METHOD_NAMES = [
    Methods.DISABLE_NON_STANDARD_LOCATIONS,
...
ARGUMENTS = dict(
    pdb=dict(
        ...
        action="store_true",
        help=(
            "Open a debugger when there is an exception. "
    ...
    config_file=dict(
        ...
        type=Path,
        help=(
            "The argument to this option is the full path to a "
    ...
    verbose=dict(
        ...
        default=0,
        help=(
            "Increase logging verbosity. Different parts of the code may "
    ...
    quiet=dict(
        ...
        default=0,
        help=(
            "Decrease logging verbosity. Different parts of the code may "
            "have different logging verbosity, and each -q decreases "
            "verbosity."
        ),
    ),
    disable_nonstandard_locations=dict(
        flags=[f"--{Methods.DISABLE_NON_STANDARD_LOCATIONS.replace('_', '-')}"],
        dest=Methods.DISABLE_NON_STANDARD_LOCATIONS,
        default=None,
        action="store_true",
        help=("Turns off extraction and application of betas for standard locations."),
    ),
    no_disable_nonstandard_locations=dict(
        flags=[f"--no-{Methods.DISABLE_NON_STANDARD_LOCATIONS.replace('_', '-')}"],
        dest=Methods.DISABLE_NON_STANDARD_LOCATIONS,
        default=None,
        action="store_false",
        help=("Turns on extraction and application of betas for standard locations."),
    ),
...
def cascade_parser(description: str = "") -> ArgumentParser:
    """
    ...
    Args:
        description: This shows up on the command line as the program
            description text.
    Return:
...
def inverse_parser(args):
    ...
    for name, argument in ARGUMENTS.items():
        ...
        if action == "count":
            ...
            raise RuntimeError(f"Unknown argument action, {action} for {name}.")
    return command_line
