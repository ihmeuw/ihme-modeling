...
from db_tools import config, loaders
from db_tools_core import get_session, query_2_df

from cascade_ode.legacy.emr import data, envs
...
class ComputeEMR(object):
    ...
    def compute_emr_from_prev(self):
        if self.can_compute_from_prev():
            ...
            # compute se
            emr_from_prev["se"] = emr_from_prev["mean"] * np.sqrt(
                (
            ...
            raise InsufficientInputs(
                "compute_emr_from_prev requires data_prev and data_csmr to" "be set"
            )
    ...
    def compute_emr_from_inci(self):
        ...
        if self.can_compute_from_inci():
            ...
            # compute se
            emr_from_inci["se"] = emr_from_inci["mean"] * np.sqrt(
                (
                ...
                ** 2
                + (self.data_remis.se / self.data_remis.mean) ** 2
            )
...
class ComputeDismodEMR(ComputeEMR):
    def __init__(self, model_version_id, release_id, envr="prod"):
        # setup global environment
        assert envr in ["prod", "dev"], "envr must be 'prod' or 'dev'"
        envs.Environment.set_environment(envr)
        config.DBConfig(
            load_base_defs=True, load_odbc_defs=True, odbc_filepath=settings["odbc_filepath"]
        )
    ...
    def set_data_csmr(self, id_template_df, csmr_type="cod"):
        ...
        if csmr_type == "cod":
            try:
                self._data_csmr = data.DataCoDCSMR(self.model_version_id, id_template_df)
            except data.NoNonZeroValues:
                self._data_csmr = None
        else:
            try:
                self._data_csmr = data.DataCustomCSMR(self.model_version_id, id_template_df)
            except data.NoNonZeroValues:
                self._data_csmr = None

    def set_data_prev(self, adj_data, adj_meta):
        try:
            self._data_prev = data.DataModelDataAdjPrevalence(adj_data, adj_meta)
        except data.NoNonZeroValues:
            self._data_prev = None

    def set_data_inci(self, adj_data, adj_meta):
        try:
            self._data_inci = data.DataModelDataAdjIncidence(adj_data, adj_meta)
        except data.NoNonZeroValues:
    ...
    def set_data_acmr(self, id_template_df):
        try:
            self._data_acmr = data.DataACMR(id_template_df, self.release_id)
        except data.NoNonZeroValues:
    ...
    def compute_dismod_emr(self, remission_df=None, csmr_type="cod"):
        ...
        # build template
        id_template_df = data.adj_data_template(
            self.adj_meta.df.reset_index(), self.model_version_id
        )
        id_template_df = id_template_df[
            ["input_data_key", "location_id", "year_id", "age_group_id", "sex_id"]
        ]
        ...
        try:
            ...
            raise InsufficientInputs("The model requires one of incidence and prevalence")

        # check if we computed anything
        if emr.empty:
            raise NoEMRCalculated("No emr could be calculated from the provided parameters")
        else:
            self.emr = emr

    def format_emr_for_upload(self):
        df = self.emr.merge(self.adj_meta.df, left_index=True, right_index=True)
        df = df.reset_index()
        df = df.rename(
            columns={
                "input_data_key": "model_version_dismod_id",
                "nid": "source_nid",
                "se": "standard_error",
        ...
        df["measure_id"] = 9
        df["lower"] = df.apply(
            lambda x: data.lower_from_se(x["mean"], x["standard_error"]), axis=1
        )
        df["upper"] = df.apply(
            lambda x: data.upper_from_se(x["mean"], x["standard_error"], "rate"), axis=1
        )
        self.emr = df

    def upload_emr(self):
        session = get_session(envs.Environment.get_odbc_key())
        table = "t3_model_version_emr"
        schema = "epi"

        should_upload = _verify_no_data_present(table, schema, self.model_version_id, session)
        ...
        infiler.upload_stagedir(table=table, schema=schema, session=session, commit=True)
...
def _verify_no_data_present(table, schema, model_version_id, session):
    ...
    count = query_2_df(
        qry, session=session, parameters={"model_version_id": model_version_id}
    )["count"].iat[0]
    return count == 0
...
def dismod_emr(
    ...
    Args:
        ...
        release_id (int): release_id for the provided model_version
    ...
    """
    compute_emr = ComputeDismodEMR(model_version_id, release_id, envr=envr)
    compute_emr.compute_dismod_emr(remission_df=remission_df, csmr_type=csmr_type)
    compute_emr.format_emr_for_upload()
    if not no_upload:
        compute_emr.upload_emr()
        LOGGER.info("Uploaded EMR")
    else:
        LOGGER.info("Not uploading EMR as requested by no_upload flag.")
