...
import cascade_ode.legacy.upload  # noqa: F401
...
def mock_functions_in_module(module_name):
    ...
    Args:
        ...
        so ``cascade_ode.legacy.upload``.
    """
    try:
        module_to_mock = sys.modules[module_name]
    except KeyError:
        raise ValueError(
            f"Cannot mock functions in {module_name} because it is not yet imported."
        )
    func_names = [
        name
        for (name, value) in getmembers(module_to_mock)
        if isfunction(value) and getmodule(value) == module_to_mock
    ]
    LOGGER.info(f"mocking functions in {module_name}: {', '.join(func_names)}")
...
def setup_io_patches(no_upload):
    ...
    """
    if no_upload:
        mock_functions_in_module("cascade_ode.legacy.upload")
