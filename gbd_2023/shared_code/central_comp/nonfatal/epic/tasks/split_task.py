...
from epic.lib.logic import sev_split
...
def parse_arguments() -> argparse.Namespace:
    """Parse standard sev split task arguments."""
...
class SevSplitTaskFactory(object):
    """Standard severity split task creation class."""
    ...
    def __init__(self, task_registry: Dict[str, Task], tool: Tool) -> None:
        ...
        self.task_template = self.tool.get_task_template(
            template_name=DAG.Tasks.SPLIT,
            command_template=(
                "{python} {script} --split_version_id {split_version_id} "
                "--output_dir {output_dir} --release_id {release_id} "
                "--year_id {year_id} "
                "--n_draws {n_draws} --annual {annual}"
            ),
            node_args=[
                Params.SPLIT_VERSION_ID,
                Params.OUTPUT_DIR,
                Params.RELEASE_ID,
                Params.YEAR_ID,
                Params.N_DRAWS,
    ...
    def get_task(
        ...
        """Returns an EPIC standard sev split task given task args."""
        # make task
        upstream_tasks = get_upstreams(node, process_graph, self.task_registry)
        node_args = {
            Params.SPLIT_VERSION_ID: str(split_version_id),
...
if __name__ == "__main__":
    """Entry point to call sev_split.SevSplitter from sev_split.py"""
