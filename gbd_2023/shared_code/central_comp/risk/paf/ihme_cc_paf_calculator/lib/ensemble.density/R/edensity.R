...
get_edensity <- function(weights, mean, sd, .min, .max) {
    ...
    buildDENlist <- function(jjj) {
        ...
        if (LENGTH == 4) {
            EST <- try(
                unlist(dlist[paste0(distn)][[1]]$mv2par(M_, (S_^2),
                    XMIN = XMIN,
                    XMAX = XMAX
                )),
                silent = T
...
# Wrapper around get_edensity that computes the edensity for all draws in one demographic.
get_edensity_for_draws <- function(weights, mean, sd, ...) {
    weights <- as.data.frame(t(weights))
    edensities <- lapply(names(mean), function(draw) {
        get_edensity(weights, mean[[draw]], sd[[draw]], ...)
    })
    names(edensities) <- names(mean)
    return(edensities)
}
