...
def location_depth(full: pd.DataFrame, train: pd.DataFrame) -> np.ndarray:
    """
    ...
    """
    D = {d: train[d].unique() for d in ["level_1", "level_2", "level_3", "location_id"]}
    level = np.array(
        [
            (
                (full.location_id != full.level_3)
                & (full.location_id.map(lambda x: x in D["location_id"]))
            ).values
        ]
        + [full[d].map(lambda x: x in D[d]).values for d in ["level_3", "level_2", "level_1"]]
    ).astype(np.int8)
    return level.sum(axis=0)


def calc_xi_vec(
    depth: int, zeta_space_smooth: float, zeta_space_smooth_nodata: float
) -> np.ndarray:
    """
    ...
    """
    xi = zeta_space_smooth ** (1 - int(depth < 3)) * zeta_space_smooth_nodata ** (
        int(depth < 3)
    )
...
def calculate_xi_matrix(
    full: pd.DataFrame,
    train: pd.DataFrame,
    zeta_space_smooth: float,
    zeta_space_smooth_nodata: float,
) -> np.ndarray:
    """
    ...
    non_rep_loc = train[train.is_representative != 1].location_id.unique()
    non_rep = full.location_id.map(lambda x: x in non_rep_loc)  # noqa: F841
    non_rep_vec = EV("zeta_space_smooth * (non_rep - non_rep * zeta_space_smooth)")
    # keep track of the places that only have no rep data to give them the full location weight
    ...
    base[only_non_rep.values & (depths == 3), 0] = base[
        only_non_rep.values & (depths == 3), :
    ][:, [0, 2]].sum(axis=1)
    base[only_non_rep.values & (depths == 3), 2] = 0
    base[only_non_rep.values & (depths == 4), 0] = base[
        only_non_rep.values & (depths == 4), :
    ][:, [0, 1]].sum(axis=1)
    base[only_non_rep.values & (depths == 4), 1] = 0
    return base


def calculate_lambda_array(
    full: pd.DataFrame,
    train: pd.DataFrame,
    lambda_time_smooth: float,
    lambda_time_smooth_nodata: float,
) -> np.ndarray:
    """
    Given two data frames ("full", "train") where train is a subset of full used
    ...
    lam = have_country_data.map(
        lambda x: lambda_time_smooth ** (x) * lambda_time_smooth_nodata ** (1 - x)
    )
    return lam.values
...
def makeS(full: pd.DataFrame, train: pd.DataFrame, var: str) -> np.ndarray:
    """
...
def timeW(
    ...
    """
    ageS = makeS(full_sub, train_sub, "ageC").astype(  # noqa: F841, make stride of age values
        "float32"
    )
    i_age = full_sub.ageC.values.astype("float32")  # noqa: F841, age vector
    age_weight_function = "1 / exp(omega_age_smooth * abs(ageS - i_age))"

    yearS = makeS(full_sub, train_sub, "year_id").astype(  # noqa: F841, stride of year values
        "float32"
    )
    i_year = full_sub.year_id.values.astype("float32")  # noqa: F841, year vector
    lambda_time = calculate_lambda_array(  # noqa: F841, assign lambda
        full_sub, train_sub, lambda_time_smooth, lambda_time_smooth_nodata
    ).astype("float32")
    if time_weight_method == "tricubic":
        aMax = np.maximum(  # noqa: F841, argMax vector
            EV("abs(i_year - year_start)"), EV("abs(year_end - i_year)")
        ).astype("float32")
...
def matCRS(full_sub, train_sub):  # noqa: F841
    """
    ...
    """
    sub_nat_S = makeS(full_sub, train_sub, "location_id")  # noqa: F841
    country_S = makeS(full_sub, train_sub, "level_3")  # noqa: F841
    region_S = makeS(full_sub, train_sub, "level_2")  # noqa: F841
    sub_nat_V = full_sub.location_id.values  # noqa: F841
    country_V = full_sub.level_3.values  # noqa: F841
    region_V = full_sub.level_2.values  # noqa: F841
    has_sub_nat_V = (train_sub.level_3 != train_sub.location_id).values.astype(  # noqa: F841
        np.int8
    )
    not_representitive_V = (0**train_sub.is_representative.values).astype(  # noqa: F841
        np.int8
    )
...
def weight_matrix(valid_positions, xi_vector, weight_matrix):
    ...
    sum_of_weights[sum_of_weights == 0.0] = 1.0
...
def spacetime(
    ...
    year_start = np.min(df.year_id)
    year_end = np.max(df.year_id)
    Wat = timeW(
        full_sub,
        train_sub,
        omega_age_smooth,
    ...
    xi_mat = calculate_xi_matrix(
        full_sub, train_sub, zeta_space_smooth, zeta_space_smooth_nodata
    ).astype("float32")
    ...
    account_missing[account_missing == 0.0] = 1.0
