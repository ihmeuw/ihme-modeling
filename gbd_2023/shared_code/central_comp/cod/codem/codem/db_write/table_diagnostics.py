...
def validation_table(
    submodel_summary_df: pd.DataFrame, model_pv: Dict[str, int]
) -> pd.DataFrame:
    """
    ...
    df = QS.validation_table.format(
        ...
        mean_error_in_ensemble=model_pv["pv_mean_error_in"],
        mean_error_out_ensemble=model_pv["pv_mean_error_out"],
        ...
        coverage_out_sub=submodel_summary_df[submodel_summary_df["rank"] == 1][
            "coverage_out"
        ][0],
        mean_error_out_sub=submodel_summary_df[submodel_summary_df["rank"] == 1][
            "mean_error_out"
        ][0],
    )
    df = df.replace("\n", "")
...
def get_submodel_summary(model_version_id: int, conn_def: str) -> pd.DataFrame:
...
def get_submodel_summary_df(
    model_version_id: int,
    submodel_covariates: Dict[str, Dict[int, List[str]]],
    submodel_rmse: Dict[int, int],
    submodel_trend: Dict[int, int],
    submodel_mean_error: Dict[int, int],
    submodel_coverage: Dict[int, int],
    conn_def: str,
) -> pd.DataFrame:
    """
    Create submodel-specific summary table (1 row for each submodel) mostly with predictive
    validity metrics.

    :return df: data frame with submodel-specific summaries
    """
    logger.info("Making submodel summary data frame.")
    df = get_submodel_summary(model_version_id, conn_def)
    ...
    df["mean_error_out"] = df.submodel_version_id.map(lambda x: submodel_mean_error[x])
    df["coverage_out"] = df.submodel_version_id.map(lambda x: submodel_coverage[x])
...
def get_covariate_summary_df(submodel_summary_df, draw_id: List[int]) -> pd.DataFrame:
    """
...
def get_model_type_df(covariate_summary_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    df = (
        df[["Type", "Dependent_Variable", "Number", "Draws"]]
        .groupby(["Type", "Dependent_Variable"])
        .sum()
    )
    df = df.reset_index()
    df.columns = ["Type", "Dependent Variable", "Number of Models", "Number of Draws"]
    return df


def get_submodel_betas_df(linear_models_json, st_models_json, all_data_holdout):
    """
    ...
    main_json = {"mixed": linear_models_json, "spacetime": st_models_json}

    keys = sorted(list(linear_models_json.keys()))
    covar_list = [
        [
            sb.get_fe_and_var(
                d=main_json, key=key, mtype=mtype, all_data_holdout=all_data_holdout
            )
            for key in keys
        ]
...
def get_all_submodel_info_df(
    data_frame: pd.DataFrame,
    covariate_df: pd.DataFrame,
    covariate_summary_df: pd.DataFrame,
    submodel_betas_df: pd.DataFrame,
    lm_dict: Dict[str, str],
    all_data_holdout: str,
    include_counts: bool,
) -> pd.DataFrame:
    """
    IMPORTANT: Merge the submodel metadata with predictive validity/n_draws/rank onto the
    covariate dataframe. USES the keys from the submodel. The sorted values are key --
    the keys are sorted and the submodel table is sorted, and they line up row by row.
    ...
    for mtype in ["count", "lt_cf", "ln_rate"]:
        df.loc[df["model"].str.contains(mtype), "mtype"] = mtype

    df["standard_beta"] = 0.0
    for index, row in df.iterrows():
        standard = sb.standardize_beta(
            covariate=row["covariate"],
            outcome=row["Dependent_Variable"],
            value=row["beta"],
            rat_dict=ratio_dict,
        )
        df.at[index, "standard_beta"] = standard
    return df


def create_weighted_beta_column(merged_submodels_df):
    """
    Create a data frame with a row for each covariate with weighted betas and
    standardized betas.
    ...
    df = df.loc[df.n_draws > 0]
    df = (
        df.groupby(df.covariate)
        .apply(sb.weighted, ["beta", "standard_beta"], weight="n_draws")
        .reset_index()
    )
...
def get_covlist(d, all_data_holdout):
    ...
    :param d: dictionary of linear model or spacetime model (they have the same covariate
        lists in aggregate)
...
def get_submodel_covariate_draws(
    json, mtype, key, response, n_draws, rat_dict, scale, all_data_holdout
):
    """
    Makes a data frame for all covariates in ONE submodel with (n_draws * scale) draws of
    the betas (Also standardizes the betas)

    :param json: (dict) main json dictionary -- both mixed and spacetime models put together
    ...
    :param rat_dict: (dict) dictionary of ratios of standard deviations of outcome / covariate
        by model type and covariate combination
    :param scale: (int) how much to scale up the draws by -- will take (n_draws * scale)
        number of draws
    :param all_data_holdout: the holdout for all of the data
    :return df: data frame
    """
...
def get_beta_draws(
    ...
    main_json = {"mixed": lm_dict, "spacetime": st_dict}
...
def create_covariate_table(covariate_summary_df, priors, submodel_weighted_betas_df):
    """
    Create table with one row per covariate with covariate metadata and also the standardized
    betas/weighted betas.

    At the end it also creates "relative" betas. These are to show the relative influence of
    the betas in the final ensemble because effectively, in submodels where this covariate
    doesn't exist, it should have a value of 0 with weight sum(n_draws) for those submodels.
    Note: This is not taken into account with the weighted average because it is only a
    weighted average across submodels that have that covariate.
    ...
    info = priors
    info = info.merge(draws, how="right", on="name").merge(submodels, how="right", on="name")
    info = info[
        [
            "name",
            "lag",
            "transform_type_short",
            "level",
            "direction",
            "offset",
            "n_draws",
    ...
    info = info.sort_values(["n_draws", "n_submodels"], ascending=False).reset_index(
        drop=True
    )
