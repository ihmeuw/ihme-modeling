...
def create_global_table(global_df, filepath):
    ...
    df["lower_deaths"] = df[columns].quantile(0.025, axis=1)
    df["upper_deaths"] = df[columns].quantile(0.975, axis=1)
    plt.figure(figsize=(16, 12))
    plt.plot(df.year_id.values, df.mean_deaths.values, label="Mean Death Prediction")
    plt.fill_between(
        df.year_id.values,
        df.lower_deaths.values,
        df.upper_deaths.values,
        alpha=0.3,
        color="blue",
...
def plot_comparisons(df, name, varname, filepath, sharex=False, sharey=False):
    """
    Plot the kernel densities of covariate distributions in the same seaborn FacetGrid.
    Outputs the plot to the model directory.

    :param df: input data frame with draws
    :param name: name of the plot
    :param varname: name of the variable to put in the kernel density
        (one of "beta", "standard_beta")
    ...
    g.map(sns.distplot, varname, hist=False, kde_kws={"shade": True}).add_legend(
        title="Model Type"
    )
...
def create_draw_plots(beta_draws, cause_name, date, model_version_id, sex, filepath):
    """
...
def create_covar_ndraws_plot(
    covariate_summary_df, cause, date, model_version_id, sex, filepath
):
...
def create_covar_submodel_plot(
    covariate_summary_df, cause, date, model_version_id, sex, filepath
):
