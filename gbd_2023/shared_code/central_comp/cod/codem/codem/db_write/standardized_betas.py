...
def remove_unnecessary_coefficients(var_list):
    ...
    """
    age_columns = [x for x in list(set(var_list)) if re.search("^age[0-9]", x)]
    good_vars = [
        x for x in list(set(var_list)) if x not in age_columns and x != "(Intercept)"
    ]
    return good_vars
...
def get_fe_and_var(d, key, mtype, all_data_holdout):
    ...
    :return fix_eff: data frame with covariate fixed effects and standard error for a given
        submodel.
    """
...
def calc_sd(df, covariate_df, vartype, name):
    ...
    if vartype == "covariate":
        ...
        data = df.copy()
        if name == "rate":
            var = np.log(data["cf"] * data["envelope"] / data["population"])
        else:
...
def get_sd_ratio(df, covariate_df, covariate, outcome):
    ...
    """
    cov_sd = calc_sd(df=df, covariate_df=covariate_df, vartype="covariate", name=covariate)
    out_sd = calc_sd(df=df, covariate_df=covariate_df, vartype="outcome", name=outcome)
    ratio = cov_sd / out_sd
    return ratio


def make_ratio_dict(
    df: pd.DataFrame, covariate_df: pd.DataFrame, covariates: List[str], include_counts: bool
) -> Dict[str, float]:
    """
    Makes a dictionary of all of the standard deviation ratios for the list of covariates,
    by model type.
    ...
    for mtype in ["rate", "cf"]:
        ...
        for cov in covariates:
            covdict[cov] = get_sd_ratio(
                df=df, covariate_df=covariate_df, covariate=cov, outcome=mtype
            )
        rdict[mtype] = covdict
    if include_counts:
        rdict["count"] = rdict["rate"]
    return rdict
...
def standardize_beta(covariate, outcome, value, rat_dict):
    ...
    :param rat_dict: (dict) dictionary with keys for model type and covariate with values
        for std. ratios
...
def make_draw(fix_eff, vcov):
    ...
    """
    return (
        np.dot(np.linalg.cholesky(vcov), np.random.normal(size=len(vcov)))
        + fix_eff.values.ravel()
    )


def make_ndraws_df(n_draws, fix_eff, vcov, response, rat_dict, scale):
    """
    Make n draws from the beta mean + variance-covariance matrix. Also standardizes them
    ...
    df["standard_beta"] = df.apply(
        lambda x: standardize_beta(
            covariate=x["covariate"], outcome=response, value=x["beta"], rat_dict=rat_dict
        ),
        axis=1,
    )
    return df
