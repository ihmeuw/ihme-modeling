...
class PV(ModelTask):
    ...
    def calculate_submodel_pv(self, input_object_name, output_object_prefix):
        self.pickled_outputs[output_object_prefix + "_pv"] = self.pickled_inputs[
            input_object_name
        ]
        ...
        self.pickled_outputs[output_object_prefix + "_pv"].uniform_predictions(
            self.model_metadata.data_frame, self.pickled_inputs["response_list"]
        )

        logger.info("Mean error out-of-sample.")
        self.pickled_outputs[output_object_prefix + "_pv"].mean_error_out(
            self.model_metadata.data_frame, self.model_metadata.ko_data
        )
        ...
        logger.info("Coverage out-of-sample.")
        self.pickled_outputs[output_object_prefix + "_pv"].coverage_out(
            self.model_metadata.data_frame, self.model_metadata.ko_data, output_object_prefix
        )
        self.pickled_outputs[output_object_prefix + "_mean_error_all"] = self.pickled_outputs[
            output_object_prefix + "_pv"
        ].mean_error_all
        self.pickled_outputs[output_object_prefix + "_rmse_all"] = self.pickled_outputs[
            output_object_prefix + "_pv"
        ].RMSE_all
        self.pickled_outputs[output_object_prefix + "_trend_all"] = self.pickled_outputs[
            output_object_prefix + "_pv"
        ].trend_all

        self.pickled_outputs[output_object_prefix + "_coverage_all"] = self.pickled_outputs[
            output_object_prefix + "_pv"
        ].coverage_all
