...
class LinearModelBuilds(ModelTask):
        ...
        if not self.check_file_output():
            ...
            linear_mod = LinMod(
                ...
                more_args=[
                    ...
                    self.model_metadata.model_parameters["include_counts"],
                ],
            )
            ...
            if exit_status:
                ...
                self.alerts.alert(
                    "There was an error in linear model builds. Submit a ticket please!"
                )
                ...
                while not self.check_file_output():
                    ...
                    if i > 10:
                        ...
                        self.alerts.alert(
                            "Cannot find linear model files. Please submit a ticket!"
                        )
                        raise RuntimeError("Cannot find linear model -- file system problem?")
        else:
            logger.info("Skipping linear model builds")
            self.alerts.alert(
                "Skipping: linear model builds has already happened for this model."
            )
            pass
        if not self.check_file_output():
            logger.info("Linear model builds has failed.")
            self.alerts.alert(
                "Your model has failed to build the linear models in R. "
    ...
    def save_response_list(self):
        logger.info(
            "Generating the response (outcome variable) list that will be used "
        ...
        self.pickled_outputs["linmod_type_list"] = linmod_type_list
