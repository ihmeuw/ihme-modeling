...
from dalynator.lib.utils import get_index_draw_columns, draw_column_list
from dalynator.aggregate_causes import AggregateCauses
from dalynator.lib.risk_attr_burden_to_paf import risk_attr_burden_to_paf
...
def aggregate_sexes_global_wrapper(df):
    ...
    df = aggregate_sexes(
            ...
            pop_merge_columns=MPGlobals.data_container.pop_merge_columns,
            include_pre_df=False)
    MPGlobals.logger.debug(f"sex_aggregation complete, df shape {df.shape}.")
    return df
...
def aggregate_dimensions(df, index_cols=None):
    ...
    # Create single draw column to aggregate
    df['draw_0'] = 1.0
    draw_cols = ['draw_0']
...
def aggregate_summaries(meas_df):
    ...
    uploadable results
    """
    index_cols, draw_cols = get_index_draw_columns(meas_df)
    MPGlobals.logger.info("AS index columns {}".format(repr(index_cols)))

    szdr = partial(summarize_draws, index_cols=index_cols)
    agga = partial(aggregate_ages)
    bcp = partial(back_calc_pafs, n_draws=MPGlobals.data_container.n_draws)

    # Number space summaries
    base_summs = szdr(meas_df)
    bs_summs = szdr(aggregate_sexes_global_wrapper(meas_df))
    aa_ss_summs = szdr(agga(meas_df))
    aa_bs_summs = szdr(agga(aggregate_sexes_global_wrapper(meas_df)))

    # Back-calculated paf summaries
    MPGlobals.logger.info("Begin bcp(meas_df)")
    bcp_summs = szdr(bcp(meas_df))
    MPGlobals.logger.info("Begin bcp(aggregate_sexes_global_wrapper(meas_df))")
    bcp_bs_summs = szdr(bcp(aggregate_sexes_global_wrapper(meas_df)))
    MPGlobals.logger.info("Begin bcp(agga(meas_df))")
    bcp_aa_ss_summs = szdr(bcp(agga(meas_df)))  # includes age std
    MPGlobals.logger.info("Begin bcp(agga(aggregate_sexes_global_wrapper(meas_df)))")
    bcp_aa_bs_summs = szdr(bcp(agga(aggregate_sexes_global_wrapper(meas_df))))  # includes age std

    # Rate space summaries
    MPGlobals.logger.info(f"Start converting to rates, time = {time.time()}")
    meas_df = convert_number_to_rate(
            df=meas_df,
            pop_df=MPGlobals.data_container["pop"],
            include_pre_df=False,
            merge_columns=MPGlobals.data_container.pop_merge_columns,
    )
    MPGlobals.logger.info(f"Converting to rates complete, time = {time.time()}")
    rate_summs = szdr(meas_df)
    rbs_summs = szdr(aggregate_sexes_global_wrapper(meas_df))
    raa_ss_summs = szdr(agga(meas_df))
    raa_bs_summs = szdr(agga(aggregate_sexes_global_wrapper(meas_df)))
...
def burdenate(key):
    """Apply pafs, write draws, and summarize burden for the given 'key'.
    """
    ...
    index_cols, draw_cols = get_index_draw_columns(meas_paf_df)
    if agg_causes:
...
def compute_dalys(ylds_df, ylls_df):
    ...
    n_draws = MPGlobals.data_container.n_draws
    index_cols, draw_cols = get_index_draw_columns(ylds_df)
...
def get_summ_filename(draw_dir, risk_type, location_id, year_id, measure_id):
    ...
    fn = ("{dd}/upload/{m}/single_year/"
          "upload_{fl}_{loc}_{y}.csv".format(dd=draw_dir, fl=file_label,
...
def run_pipeline_burdenator(args):
    """
    Run the burdenator most-detailed pipeline. Will raise ValueError if input
    ...
    # Get detailed ages
    MPGlobals.most_detailed_age_groups = data_container["age_spans"]["age_group_id"]

    # Fetch PAF input from RF team
    logger.info("start apply PAFs, time = {}".format(time.time()))
    draw_columns = draw_column_list(n_draws=args.n_draws)
