...
from dalynator.tasks.run_pipeline_burdenator_most_detailed import \
    aggregate_ages, aggregate_sexes_global_wrapper, back_calc_pafs, MPGlobals
...
def compute_aggregates(df, n_draws, tool_name):
    ...
    df = pd.concat([df, aggregate_sexes_global_wrapper(df)], sort=True)
    df = pd.concat([df, agga(df)], sort=True)

    if "burdenator" in tool_name:
        # Calculate rate space
        rate_df = convert_number_to_rate(
            df=df, pop_df=MPGlobals.data_container["pop"], include_pre_df=False)
        ...
        rate_df = convert_number_to_rate(
            df=df, pop_df=MPGlobals.data_container["pop"], include_pre_df=True)
        df = convert_number_to_percent(df=rate_df, include_pre_df=True)
    return df
...
class PctChangePipeline(object):
    ...
    def instantiate_data_containers(self, cache_dir=None):
        ...
        self.data_container_start = DataContainer(
            ...
            age_group_ids=self.args.age_group_ids,
    ...
    def compute_pct_change(self, df=None):
        ...
        self.index_cols, self.draw_cols = get_index_draw_columns(df)
