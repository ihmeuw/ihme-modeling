...
preserve
    ** Get all the iso3s from Southeast asia (location_id 9 is the region Southeast Asia)
    odbc load, exec("SELECT DISTINCT map_id as iso3 FROM shared.location_hierarchy_history WHERE path_to_top_parent LIKE '%,9,%' AND location_set_version_id=`lsvid'") dsn(ADDRESS) clear
