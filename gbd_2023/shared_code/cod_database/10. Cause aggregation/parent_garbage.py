...
from cod_prep.utils import CodSchema, report_duplicates
...
class ParentMappedAggregatedGarbageAdder(CodProcess):
    ...
    def get_computed_dataframe(self, df):
        ...
        df = self.add_parentmapped_garbage_to_agg_df(df, gbg_df, id_cols)
    ...
    def get_parentmapped_garbage(self, df, id_cols):
        ...
        value_to_package_id = self.package_map.set_index("value", verify_integrity=True)[
            "package_id"
        ]
    ...
    def assemble_parentmapped_garbage_for_agg_df(self, id_cols):
        ...
        df = convert_to_cause_fractions(
            df, deaths_cols=["deaths_raw", "deaths_corr", "deaths_rd", "deaths"]
        )

        if len(df) > 0:
            cause_aggregator = CauseAggregator(df, self.cause_hierarchy, self.source, self.data_type_id)
    ...
    def add_parentmapped_garbage_to_agg_df(agg_df, gbg_df, id_cols):
        ...
        df.loc[df["cf_raw_gbgparnt"] > 0, "cf_raw"] = df["cf_raw"] + df["cf_raw_gbgparnt"]
        df.loc[df["cf_corr_gbgparnt"] > 0, "cf_corr"] = df["cf_corr"] + df["cf_corr_gbgparnt"]
        return df
