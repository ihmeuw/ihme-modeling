...
from cod_prep.downloaders.population import add_envelope, add_population
...
class NonZeroFloorer(CodProcess):
    ...
    def get_computed_dataframe(self, pop_df, env_df, cause_hierarchy, data_type_id):
        ...
        if self.track_flooring:
            self.df.fillna(
                value={"cf_final_pre_floor": self.df["cf_final"], "floor_flag": 0},
                inplace=True,
            )
        return self.df
    ...
    def replace_cf(self, check_cf_col):
        ...
        if (check_cf_col == "cf_final") and self.track_flooring:
            ...
            self.df.loc[cf_over_0 & cf_less_than_floor, "floor_flag"] = 1
        self.df.loc[cf_over_0 & cf_less_than_floor, check_cf_col] = self.df["floor"]
    ...
    def convert_nonzero_mad(self, df, cmdf):
        ...
        df = df.rename(columns={"year": "year_id", "sex": "sex_id", "age": "age_group_id"})
        age_to_id_map = {
            1: 5,
            5: 6,
            10: 7,
        ...
        new_cause_ages = pd.read_csv(
            self.conf.get_resource("nonzero_floor_new_age_restrictions")
        ...
        for year_id in range(1980, self.conf.get_id("year_end")):
            if year_id not in df.columns:
                df[year_id] = np.NaN
        df = (
            df.sort_values(axis="columns", by="year_id")
    ...
    def check_nonzero_floors(self, df):
        assert df.notnull().values.all()
        df = df.astype({"cause_id": int, "age_group_id": int, "sex_id": int, "floor": float})
    ...
    def drop_non_observed_cause_age_sexes(self, data_type_id):
        ...
        self.df = self.df.merge(
            drop_file, on=["cause_id", "age_group_id", "sex_id"], how="left", indicator=True
        )
        if data_type_id == 9:
    ...
    def merge_nonzero_floor_info(self, cmdf):
        ...
        if self.df.floor.isnull().any():
            self.df = self.df.groupby(["sex_id", "cause_id"]).apply(self.fill_na_floors)
        if self.df.floor.isnull().any():
