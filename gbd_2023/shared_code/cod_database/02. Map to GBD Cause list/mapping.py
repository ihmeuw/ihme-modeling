...
class BridgeMapper(CodProcess):
    ...
    def validate_bridge_map(self, map_df, cause_meta_df):
        ...
        ) == set(), "The acauses and bridge codes in the bridge map are not mutually exclusive."
    ...
    def needs_bridging(self, has_verbal_autopsy):
        ...
        Note: There are some special sources that do not use any of the bridge mapped
        code systems. However, NAME still wishes to have them bridge mapped.
        """
    ...
    def get_file_name(self, has_verbal_autopsy):
        ...
        source_to_sheet = {
            ...
            "Iran_NAME_special_ICD10": "Iran_NAME_special_ICD10",
            "Iran_2011": "Iran_NAME_special_ICD10",
            "ETH_DHIS2": "DHIS2",
        }
    ...
    def redistribute_zz_bridge_codes(self, df, map_df, schema: CodSchema):
        ...
        for zz_code in zz_code_targets:
            ...
            print_log_message(
                "Found ZZ code: {}, deaths: {}".format(
                    zz_code, df.loc[to_redistribute, "deaths"].sum()
                )
            )
...
class GBDCauseMapper(CodProcess):
    ...
    def special_cause_reassignment(self, df, code_system_id):
        ...
        There are instances where NAME (or another PI) has good reason to
        ...
        if (df["source"] == "India_SRS_states_report").any() and (code_system_id == 1):
            ...
            for package_id in custom_grbg.package_id.unique():
                ...
                assert len(gp_df) != 0, "Found 0 codes for package {}".format(package_id)
                gp_dfs.append(gp_df)
    ...
    def check_NAMEs_map(self):
        """Check the new map against NAME's map per NAME's request."""
