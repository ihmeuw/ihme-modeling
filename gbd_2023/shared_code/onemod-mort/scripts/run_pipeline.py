...
def create_tasks(
    ...
    if stage in ["preprocess", "fit_global", "fit_national", "fit_location"]:
    ...
    if stage in ["prepare_handoff"]:
        ...
        args["node_args"] = dict(
            dirpath=[
                f"{name}/predictions"
                for name in ["global_model", "national_model", "location_model"]
            ]
        )
        action_tasks = create_action_tasks(**args)
        tasks.extend(action_tasks)
    ...
    if stage == "create_figures":
        action_tasks = create_action_tasks(**default_args)
        tasks.extend(action_tasks)
        upstream_tasks_0 = action_tasks

        args = default_args.copy()
        args["action"] = "create_coef_figures"
        action_tasks = create_action_tasks(**args)
        tasks.extend(action_tasks)
        ...
        action_tasks = create_action_tasks(**args)
        tasks.extend(action_tasks)
        upstream_tasks_2 = action_tasks

        args = default_args.copy()
        args["action"] = "combine_figures"
        args["upstream_tasks"] = (
            upstream_tasks_0 + upstream_tasks_1 + upstream_tasks_2
        )
        action_tasks = create_action_tasks(**args)
        tasks.extend(action_tasks)
        upstream_tasks = action_tasks
