...
def iterate_through_df_and_split_nationals_by_population(all_vars):
    ...
    for index, row in df.iterrows():
        ...
        if row['most_detailed'] == 0:
            ...
            assert abs(event_df.best.sum() - national_deaths) < 1, "{},{},{},{}".format(parent_loc_id,
                                                                                        year_id, national_deaths, event_df.best.sum())
...
def run_subnational_splitting(df, source):
    original_death_count = df.copy()['best'].sum()
    df['high'] = df['high'].astype("float")
    df['low'] = df['low'].astype("float")
    df['location_id'] = df['location_id'].apply(lambda x: int(x))
    all_locations = get_location_metadata(location_set_id=21, release_id=16)
    ...
    pop = get_population(location_id=-1, release_id=16,
                         year_id=-1, location_set_id=21)
    count = 0
    while has_nationals:
        count += 1
    ...
    df.to_csv(FILEPATH.format(source))
