...
from shock_prep.etl.apply_location_mapping import apply_map_hierarchy as amh
...
def _assign_dest_col(df):
    ...
    df = df.loc[:, ['source_event_id', 'source_col', 'has_side_b', 'sides_equal']]
    df = df.drop_duplicates()
    df['sides_equal_bool'] = df['sides_equal'] == True
    
    df = df.assign(dest_col=lambda d:
                np.where(~d.has_side_b | d.sides_equal_bool,
                         'location_id',
                         d.source_col.map({
                             'location_id': 'side_a',
                             'side_a': 'side_a',
