...
// Modified:	INDIVIDUAL_NAME INDIVIDUAL_NAME  INDIVIDUAL_NAME 
...
global cancer_repo = REDACTED
global code_prefix = REDACTED
global cancer_storage = REDACTED
global registry_input_storage = REDACTED
global registry_input_code = REDACTED
global prep_process_storage = REDACTED
global common_cancer_data = REDACTED

// Full file paths for readability
// registry_input_code  = REDACTED
// prep_process_storage = REDACTED
// common_cancer_data   = REDACTED
// Input folder
local registry_inputs = REDACTED
// Output folders
local temp_folder = REDACTED

local code_folder = REDACTED
...
// load ihme_population
	use "$common_cancer_data/populations.dta", clear 
...
// // Keep only "gold standard" data
	...
	// save
		...
		// save "`temp_folder'/good_format.dta", replace
		// testing to see if we can save the file and inspect it

** ********************
** Enforce restrictions
** ********************
	// use "FILEPATH", clear 
	//  # testing to see if we can read in the file we saved above
...
** ********************
	...
	// Create "average_cancer"
		preserve
			drop if inlist(gbd_cause, "neo_nmsc", "neo_nmsc_bcc", "neo_nmsc_scc") 
		...
		preserve
			drop if inlist(gbd_cause, "neo_nmsc", "neo_nmsc_bcc", "neo_nmsc_scc") 
			collapse (mean) cases*, by(sex age gbd_cause) 
...
// Finalize and Save
	...
	gen release_id = $current_release_id
	gen refresh = $current_refresh
	gen is_best = 1 
	gen last_updated = REDCTED
	gen last_updated_by = "USERNAME" 
	gen last_update_action = "inserted"
	gen date_inserted = REDCTED
	gen inserted_by = "USERNAME" 
	gen prep_incidence_rates_id = .
	order prep_incidence_rates_id sex_id acause age rate release_id refresh is_best last_updated last_updated_by last_update_action date_inserted inserted_by
