...
def load_cause_rates(data_type_id):
    ...
    if data_type_id == 2:
        ...
        cause_map = cause_map[cause_map['is_best'].eq(1) & 
                                cause_map['release_id'].eq(release_id) &
...
def add_cause_rates(dataset, uid_cols, ds_instance):
    ...
    merge_df = merge_df.loc[~merge_df['acause'].eq('nan'),] 
    ...
    if prep_step == 5:
REDACTED
        # mapping leukemia garbage codes to using leukemia_other rates
