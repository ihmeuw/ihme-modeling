...
def run_nonzero_floor():   
    ...
    df.to_csv(utils.get_path(process='cod_mortality', key='nonzero_floor_outputs'))
    return 
...
def run_noise_reduction_prior(): 
    ...
    subprocess.call(['/ihme/singularity-images/rstudio/shells/execRscript.sh', '-i', '/ihme/singularity-images/rstudio/ihme_rstudio_4201.img', '-s', script_path], shell=False)
...
def run_noise_reduction_create_metrics(): 
    ...
    # this calls cancer_estimation/c_models/b_cod_mortality/noise_reduction_posterior.r
    subprocess.call(['/ihme/singularity-images/rstudio/shells/execRscript.sh', '-i', '/ihme/singularity-images/rstudio/ihme_rstudio_4201.img', '-s', script_path], shell=False)
...
def load_nids() -> pd.DataFrame: 
    def _attach_registry_index(nid_df : pd.DataFrame) -> pd.DataFrame: 
        ...
        assert all(fin['_merge'].eq('both')), 'In _attach_registry_index, not all registries could be merged into the nid dataframe.'
        del fin['_merge']
        return fin 
    ...
    def _apply_IND_corrections(in_df : pd.DataFrame) -> pd.DataFrame: 
        ''' Append additional IND registries to dataframe. This is due to splitting of India registries that occurs and 
            post-split registry_index values do not exist.
        '''
        df = pd.DataFrame()
    ...
    nid_df.drop_duplicates(subset=['registry_index','queue_id','data_type_id','year_start','year_end'], inplace=True)
...
def format_bundle(df : pd.DataFrame) -> pd.DataFrame: 
    ...
    def _attach_queue_id(df : pd.DataFrame) -> pd.DataFrame:
        ...
        assert all(fin_df['_merge'].eq('both')), "In '_attach_queue_id', not all rows in 'bundle_input' were matched in the 'dataset' table."
        del fin_df['_merge']
        return fin_df 
    ...
    # attach any other required columns that are required
...
REDACTED
        bundle_input = pd.read_csv('FILEPATHGBD2020/c_models/b_cod_mortality/decomp3_refresh2/prepped_incidence.csv') 
    ...
    cod.run_cod_upload('testcod', utils.get_path(key='cod_upload', process='cod_mortality'))
