...
def project_to_special_locations(df):
    ...
    '''
    print("INFO: project_to_special_locations has not been implemented")
    return(df)
...
def supplement_national_estimates(df):
    ...
    natnl_est = df.loc[for_natnl_projections, :].copy()
    natnl_est.loc[:, 'location_id'] = natnl_est['country_id']
...
def project_ihme_location_estimates(df):
    ...
    ihme_pop = pop.load_raw_ihme_pop(list(df.location_id.unique()))
    ihme_pop.rename(columns={'population':'pop'}, inplace=True)
    final_df = df.merge(ihme_pop, how="left")
