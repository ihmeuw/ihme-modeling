...
temp_dir = f"FILEPATHGBD2022/b_staging/c_cod_mortality/{today}/"
utils.ensure_dir(temp_dir)

SKIP_DROP_AND_CHECK = False
...
def _add_location_id(df):
    df.to_csv(f"{temp_dir}/start_of_add_location_id.csv")
...
def remove_blank_registry_index_rows(df):
    return df[df['registry_index'] != ""]
...
if __name__ == "__main__":
    import sys 
    # sys.argv[1] is the rid of the run of the mortality pipeline
    run(int(sys.argv[1]))
