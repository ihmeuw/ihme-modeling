...
def refine_by_cc_code(df):
    ''' Generates a 'cc_code' (measure of the remaining difference between cancer
            mortality and all-cause mortality) and drops data that are not
    ...
    # remove child causes
    causes_w_subtypes = "neo_liver_|neo_leukemia_|neo_eye_|neo_lymphoma_"
    deaths_df = df.loc[~df['acause'].str.contains(causes_w_subtypes, regex = True), :]
    ...
    output = pd.concat([output, cc_df])
    df = modeled_locations.add_sdi_quintile(df, delete_existing=True)
    print("Final entries with cc_code attached: {}".format(len(output)))
    output.drop_duplicates(get_uid_cols(), inplace=True)
...
def run():
    ...
    input_file = utils.get_path("nonzero_floor_outputs", process="cod_mortality")
