...
print("REMINDER: this script must be submitted on FAIR cluster")
library(here)
library(data.table)
if (!exists("code_repo"))  {
...
run_mir_worker.register_model <- function(mir_model_version_id, model_index_id, 
    ...
    #source(file.path(central_root, 'FILEPATH/register.R'))
...
run_mir_worker.launch_model <- function(run_id){
    ...
    source("FILEPATH/public.R")
    #source(file.path(central_root, 'FILEPATH/sendoff.R'))
