...
# Library loading needs changed to set up for RStudio session running
# To run from the terminal, remove the lib.loc from the packages as they exist in cancer environment
library(here)
library(blob)
library(RSQLite)
...
cur_release_id <- get_gbd_parameter("current_release_id")

####################################
# Helper Functions                 #
####################################

grab_caps <- function(mir_version, include_VR = F, combine_age_types = T, mir_version_peds=NULL, mir_version_all_ages=NULL){
  ...
  if (combine_age_types){
    all_causes_peds <- list.files(paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_", mir_version_peds), full.names = T)
    all_causes_all_ages <- list.files(paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_", mir_version_all_ages), full.names = T)
    all_causes <- c(all_causes_peds,all_causes_all_ages )
  } else{
  ...
  if (include_VR) {
   all_causes <- all_causes[grepl("neo", all_causes)]
    ...
    all_causes <- all_causes[grepl("neo", all_causes)&!grepl("_VR", all_causes)]
    }
...
get_negative_binomial_coef <- function(cause_name, transform_mir = T, 
                                       ...
                                       combine_age_types = F, 
                                       mir_version_peds = NULL, 
                                       mir_version_all_ages = NULL){
  ...
  haqi_quintile <- get_covariate_estimates(covariate_id = 1099, age_group_id = "all", location_id = "all", 
                                           year_id = "all", sex_id = "all", release_id = cur_release_id)
  ...
  if(combine_data_types & st_gpr_input){
    if(combine_age_types){
      message("pulling data from both peds only and all ages")
      data_peds <- lapply(cause_name, function(x){
        path = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_peds,"/",  
                      x, ".csv")
      ...
      data_peds <- rbindlist(data_peds, fill = T)
      data_all_ages <- lapply(cause_name, function(x){
        path = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_all_ages,"/",  
                      x, ".csv")
      ...
      data_all_ages <- rbindlist(data_all_ages, fill = T)
      data <- rbind(data_peds, data_all_ages)
    }else{
    ...
    if(combine_age_types){
      message("pulling data from both peds only and all ages")
      path_peds = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_peds,"/",  
                    cause_name, ".csv")
      data_peds <- fread(path_peds)
      print(path_peds)
      path_all_ages = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_all_ages,"/",  
                         cause_name, ".csv")
      data_all_ages <- fread(path_all_ages)
      print(path_all_ages)
      data <- rbind(data_peds, data_all_ages)
    }else{
      path = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version,"/",  
  ...
  #SET UP CAUSE SPECIFIC DATA RESTRICTIONS

  if (cause_name %in% c()) {

  } else message("No data restrictions.")
  
  # OPTIONAL AGGREGATION OF AGE VARIABLES
  # if we want to aggregate age variables to the configurations used in GBD 2019
  if(!regular_categorical_age){
      if (cause_name=="neo_neuro"){
        ...
        # neo eye rb is 0-4 as reference to 5-9 (new upper age restriction 2020)
        data[, age_group_id := ifelse(age_group_id > 5, 2, 1)]
      } else if(cause_name == "neo_liver_hbl"){
        # neo liver hbl is 0-4 ref to 5-9 (new upper age restriction 2020) 
        data[, age_group_id := ifelse(age_group_id > 5, 2, 1)]
      ...
      } else if(cause_name %in% c("neo_lymphoma_burkitt", "neo_bone", "neo_eye", "neo_eye_other", "neo_lymphoma_other", "neo_tissue_sarcoma", "neo_ben_brain")){
        # these causes aggregated to the bins: 0-19, 20-39, 40-59, 60-79, 80+
  ...
  # as we have above assigned haqi quintiles to 'haq' if using quintiles this works in either case
  ...
  print(our_formula)
  # huge dispersion parameter with negative binomial > 30000
  ...
  # get location_names 
  locs <- get_location_metadata(location_set_id = 35, release_id = cur_release_id)
...
get_nb_config <- function(acause_name = NULL,
  ...
  locs <- get_location_metadata(location_set_id = 22, release_id = cur_release_id)
  ...
  setDT(ages)
  ages <- ages[release_id == cur_release_id & acause == acause_name & is_active == 1]
  age_start <- ages$yll_age_start #age_start <- ages$yll_age_start[2] this was giving error bc nothing exists for second var
  ...
  if(!regular_categorical_age&!use_continuous_age){
    ...
    } else if (acause_name %in% c("neo_eye","neo_eye_other", "neo_ben_brain")){
      total_data[, age_id_1 := ifelse(age_group_id<3, 1, 0)]
      total_data[, age_id_2 := ifelse(age_group_id>=3&age_group_id<5, 1, 0)]
      total_data[, age_id_3 := ifelse(age_group_id>=5&age_group_id<9, 1, 0)]
...
generate_mir_draws <- function(config_rows= NULL, cause_name = NULL, draw_coeffs = NULL, 
                               ...
                               combine_age_types = NULL, mir_version_peds = NULL, mir_version_all_ages = NULL){
  ...
  computeMIR <- function(config_row, input_coeffs, cause_name){
    if(use_continuous_age){
      ...
      if(!(cause_name %in% c("neo_eye","neo_eye_rb", "neo_liver_hbl","neo_eye_other","neo_lymphoma_burkitt", "neo_ben_brain"))){
        #there are 5 age categories for these causes, including neuroblastoma which has 5 bins but different from others
        results <- apply(input_coeffs, 1, function(x){
      ...
      } else if(cause_name%in% c("neo_ben_brain")){
        results <- apply(input_coeffs, 1, function(x){
          exp((x['(Intercept)'] + (x['age_id_2'] * config_row['age_id_2']) + (x['age_id_3'] * config_row['age_id_3']) + 
                 (x['age_id_4'] * config_row['age_id_4']) +  
                 (x['haq'] * config_row['haq'])))
        ...
        #there are two age categories for these causes
        results <- apply(input_coeffs, 1, function(x){
  ...
  # merging in caps
  caps <- grab_caps(mir_version = mir_version, combine_age_types = combine_age_types, mir_version_peds = mir_version_peds, mir_version_all_ages = mir_version_all_ages)
...
parallel_draw_launch <- function(cause_name, mir_version = mir_version, draw_count = 0, new_data="", 
                                 add_year = F , regular_categorical_age =T, use_continuous_age=F, 
                                 use_haqi_estimates = T, combine_data_types = F,
                                 run_description = "", combine_age_types = F, mir_version_peds = NULL, mir_version_all_ages = NULL){
  ...
  data_results <- get_negative_binomial_coef(cause_name = cause_name, transform_mir = F,
                                            ...
                                            combine_age_types = combine_age_types,
                                            mir_version_peds = mir_version_peds, 
                                            mir_version_all_ages = mir_version_all_ages)
  ...
  all_results <- mclapply(total_config, FUN = function(x){
   generate_mir_draws(config_rows = x, draw_coeffs = draw_coeffs,
                      ...
                      run_description = run_description, combine_age_types = combine_age_types, mir_version_peds = mir_version_peds, mir_version_all_ages = mir_version_all_ages)
  }, mc.cores = detectCores() - 2)
  ...
  if(st_gpr_input){
    if(combine_age_types){
      input_set_peds = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_peds,"/", 
                         cause_name, ".csv")
      input_set_all_ages = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_all_ages,"/", 
                              cause_name, ".csv")
      input_set <- list(input_set_peds, input_set_all_ages)
      if(combine_data_types){
        input_set_VR_peds = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_peds,"/", 
                              cause_name, "_VR.csv")
        input_set_VR_all_ages = paste0(get_path(key = "mir_inputs", process = "mir_model"), "/model_version_",mir_version_all_ages,"/", 
                              cause_name, "_VR.csv")
        input_set_VR <- list(input_set_VR_peds, input_set_VR_all_ages)
      } else{
...
if (!interactive()) {
  cause_name1 <- commandArgs()[8] # options: see list of new causes in launcher script
  cause_name2 <- commandArgs()[9] # options: see list of new causes_VR in launcher script
  mir_version <- commandArgs()[10] # options: see mir_model_version table mir_version_ids
  draw_count <- commandArgs()[11] # options: [0, 100, 1000]
  combine_age_types <- commandArgs()[12]
  mir_version_peds <- commandArgs()[13]
  mir_version_all_ages <- commandArgs()[14]
  description <- commandArgs()[15]
  
  #for debugging 
  print("printing params to help with debugging")
  print(paste0("cause_name1 = ", cause_name1)) 
  print(paste0("cause_name2 = ", cause_name2)) 
  print(paste0("mir_version = ", mir_version)) 
  print(paste0("draw_count = ", draw_count)) 
  print(paste0("combine_age_types = ", combine_age_types)) 
  print(paste0("mir_version_peds = ", mir_version_peds)) 
  print(paste0("mir_version_all_ages = ", mir_version_all_ages)) 
  ...
  parallel_draw_launch(list(cause_name1, cause_name2), mir_version, as.numeric(draw_count), 
                       ...
                       combine_data_types = combine_data_types, new_data = "",
                       run_description = description, 
                       combine_age_types = combine_age_types, mir_version_peds = mir_version_peds, mir_version_all_ages = mir_version_all_ages)
