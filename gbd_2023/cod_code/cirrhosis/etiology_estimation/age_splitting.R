...
pacman::p_load(data.table, openxlsx, ggplot2, magrittr, dplyr, plyr)
...
orig_dt <- as.data.table(read.xlsx(FILEPATH)
...
expand_age <- function(small_dt, age_dt = ages){
  ...
  dt[, drop := cases/n.age] ##drop the data points if cases/n.age is less than 1 (as per USERNAME)
...
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = id, ## USING 2010 AGE PATTERN BECAUSE LIKELY HAVE MORE DATA FOR 2010
                           measure_id = c(5,6,18), location_id = locs, source = "epi", ##Measure ID 5= prev, 6=incidence, 18=proportion
                           version_id = ver_id, sex_id = c(1,2), gbd_round_id = 6, decomp_step = "step4", #can replace version_id with status = "best" or "latest"
  ...
  se <- copy(age_1)
  se <- se[age_group_id==5, .(measure_id, sex_id, se_dismod, location_id)] ##just use standard error from 1-4 age group (as per USERNAME)
