...
meid <- 11641 ## SET TO EITHER ADJUSTED (ID) OR UNADJUSTED PREVALENCE (ID), or iterative post-mortality)
rr_mrbrt_model <- "FILEPATH"
ar_mrbrt_model <- "FILEPATH"
prop_file_noage <- "FILENAME"
...
age_metadata <- get_age_metadata(12, gbd_round_id=7)
...
get_mrbrt_data <- function(folder, sex = T){
  dt <- fread(paste0(mrbrt_dir, folder, "FILEPATH"))
...
excess_death <- function(id, dt){
  prev <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = id, source = "epi", measure_id = 5,
                    ...
                    gbd_round_id = 7, age_group_id = full_age_dt[, age_group_id], decomp_step = "iterative")
...
get_counts <- function(dt, draw_cols){
  pops <- get_population(age_group_id = full_age_dt[, unique(age_group_id)], location_id = location,
                         year_id = c(seq(1990, 2015, by = 5), 2017, 2019, 2020, 2021, 2022), sex_id = c(1, 2), 
                         decomp_step = 'step2', gbd_round_id=7)
