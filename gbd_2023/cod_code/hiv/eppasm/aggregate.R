...
code.dir <- paste0("FILEPATH", user, "/hiv_gbd2019/")
...
if(length(args) > 0) {
  ...
  parent <- "ZAF"
  run.name = "art_mort_test_1"
  spec.run.name <-"art_mort_test_1"
  ncores <- 2
}
...
single.age.dir <- paste0('FILEPATH', spec.run.name,'/detailed_deaths/')
...
if(parent == "IND_44538") {
  age.map <- fread(paste0(root, "temp/USERNAME/maps/age_map.csv"))[age %in% unique(out.dt$age), .(age_group_id, age)]
