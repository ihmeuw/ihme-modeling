...
code.dir <- paste0("FILEPATH/hiv_gbd2019/")
...
source(paste0(root, "FILEPATH/get_proc_version.R"))
...
in.dir <- paste0('FILEPATH/',run.folder,'/compiled/stage_1')
setwd(in.dir)
...
if (group_infor %in% c("1A", "1B")) {
  ...
  cod.data.raw <- fread(paste0("FILEPATH", ens.version, ".csv"))  ## use the up-to-date ensemble data file
  ...
  cod.data <- fread('FILEPATH/spectrum_gpr_results.csv')  ## update to time
  

  if(grepl('CHN', loc)){
    cod.data <- fread('FILEPATH/spectrum_gpr_results_20190723_china_2019scalars.csv') 
  }
...
duration.path <- paste0("FILEPATH", run.folder, "/compiled/stage_1/")
...
out.dir_run <- paste0("FILEPATH/", run.folder, "_adj/")
