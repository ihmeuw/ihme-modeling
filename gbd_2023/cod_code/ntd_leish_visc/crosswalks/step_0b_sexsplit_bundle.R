...
#' [Sex Crosswalk] this pulls from bundle 
fit1 <- readRDS(paste0(crosswalks_dir, "FILEPATH.rds"))

age_split_bundle <- get_bundle_version(ADDRESS)
age_split_ss <- apply_sex_crosswalk(mr_brt_fit_obj =  fit1, all_data = age_split_bundle, release_id = ID)
...
agesplit_out_file <- paste0(crosswalks_dir, "FILEPATH.xlsx")
 
openxlsx::write.xlsx(age_split_ss, agesplit_out_file,  sheetName = "extraction")
agesplit_description <- "binom sex split, only age-specific points"
