# Purpose: create cfr model inputs (VL - fatal)
# Notes: in gbd 2019, 2017 cfrs used (new subnationals pull from parent); custom subnationals south sudan 1990-1994, africa, europe
#
### ======================= BOILERPLATE ======================= ###
rm(list = ls())
...
# cfr = cod data / mean estimated incidence data

#load in vl death data
vl_deaths <- get_cod_data(cause_id=ADDRESS, release_id = release_id)
...
# some parents do not have estimated incidence \
no_incidence_file <- c()
yes_incidence_file  <- c()

for (loc in loc_ids){
  if (paste0(loc, ".csv") %in% list.files(paste0(draws_dir, "A"))){
...
for (loc_id in yes_incidence_file){
  incidence_draws <- fread(paste0(draws_dir, "A/", loc_id, ".csv"))
...
cat("\nGetting Population\n")
population   <- get_population(location_id = loc_ids, age_group_id = "all", year_id = year_ids, sex_id = c(1,2), release_id = release_id)
