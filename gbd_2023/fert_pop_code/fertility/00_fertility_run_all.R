...
clone_dir <- paste0("FILEPATH/Fert_Pipeline_", workflow_args)
...
# Overwrite some args
retries <- 3
submission_project_name <- "proj_fertilitypop"
command_template_add_ons <- ""
shell_path_r <- gsub("PYTHONPATH= ", "", shell_path_r)

# Set parent versions
pop_est_id <- "Population estimate run id" # baseline
pop_syr_id <- "Single year population estimate run id" # baseline
ddm_est_id <- as.integer(
  get_versions(
    "ddm estimate",
    run_status = "best",
    gbd_year = get_gbd_year(previous_round_id)
  )[, run_id]
)
...
version1 <- version_id
version2 <- "bested run id current year" 
version3 <- "bested run id previous year" 
...
if (!dir.exists(clone_dir)) {
  ...
  git_clone(clone_dir, stash_repos = c("fp/fertility_modeling" = branch))
  
  fs::file_copy(
    paste0("FILEPATH/00_fertility_run_all.R"),
...
code_dir <- paste0(clone_dir, "FILEPATH")
base_dir <- paste0("FILEPATH", version_id)
graph_dir <- paste0(gsub("modeling", "visuals", base_dir))
...
resume_file <- fs::path(clone_dir, "resume_file.csv")
...
readr::write_csv(model_locs, fs::path(base_dir, "/FILEPATH/model_locs.csv"))
...
wf_tool <- set_default_tool_resources(
  ...
  resources = list(
    ...
    "stdout" = fs::path("FILEPATH", "output"),
    "stderr" = fs::path("FILEPATH", "errors")
  )
)
...
for(loop in 1:2) {
  ...
  inputs_template <- task_template(
    ...
    node_args = list("version_id", "gbd_year", "year_start", "year_end", "loop",
                     ...
                     "pop_syr_id", "ddm_est_id", "birth_est_id", "code_dir", "release_id")
  )
  ...
  for(part in 1:end_part) {
    ...
    if (loop == 1 & part == 2) {
      ...
      if (gen_new) {
        ...
        split_version <- mortdb::gen_new_version(
          "asfr",
          "data",
...
if (wfr != "D") {
  ...
  jobmon_errors <- get_jobmon_errors(
    wf$workflow_id,
    odbc_section = "jobmon3"
  )
  
  readr::write_csv(jobmon_errors, fs::path(base_dir, "jobmon_errors.csv"))
