...
source(paste0(code_dir, '/FILEPATH/space_time.R'))
...
data <- assertable::import_files(list.files(input_dir, pattern = paste0('age_', model_age, '_data.csv'), full.names = T), FUN=fread)
pred <- assertable::import_files(list.files(stage1_dir, pattern = paste0('age_', model_age, '_stage1_results.csv'), full.names = T), 
                                 FUN=fread)
...
## Weight VR years with fewer than 100 births lower (decision made before GBD 2019, likely to account for variability of low counts)
...
custom_params <- fread(paste0('FILEPATH/age_', model_age, '_params.csv'))
...
for (i in 1:nrow(params)){
  st_pred <- space_time_beta(st_data, loc = params$ihme_loc_id[i], beta = params$beta[i], zeta = params$zeta[i], min_year = year_start,
                             max_year = year_end, loc_map_path = paste0(loop1_dir, 'loc_map.csv'), use_super_reg = params$use_super_reg[i],
...
readr::write_csv(pred, paste0(output_dir, 'age_', model_age, '_stage2_results.csv'))
...
## Get bounds
bounds <- fread(paste0(loop1_dir, 'logit_bounds.csv'))[age == model_age]
...
readr::write_csv(gpr_input, paste0(output_dir, 'age_', model_age, '_gpr_input.csv'))
