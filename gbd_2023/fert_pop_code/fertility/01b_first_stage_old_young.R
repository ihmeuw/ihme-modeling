...
if (model_age == 20) {
  ...
  model_locs <- fread(paste0(loop1_dir, '/model_locs.csv'))[,ihme_loc_id]
}
...
if (model_age == 10) {
  ...
  assertable::check_files(paste0('gpr_', all_locs$ihme_loc_id, '_45.csv'), 
                          folder = gpr_dir, continual = F, display_pct = 0)
  asfr_adj <- assertable::import_files(list.files(gpr_dir, full.names = T, 
                                                  pattern = '.*45.csv'), FUN = fread)
  setnames(asfr_adj, 'mean', 'asfr_adj')
  asfr_adj[, year_id := floor(year)]
  data <- merge(data, asfr_adj[,.(year_id, ihme_loc_id, asfr_adj)],
                by=c('year_id', 'ihme_loc_id'))
...
readr::write_csv(data, paste0(input_dir, 'age_', model_age, '_data.csv'))
...
readr::write_csv(pred, paste0(output_dir, '../FILEPATH/age_', model_age, '_stage1_results.csv'))
...
readr::write_csv(params, paste0("FILEPATH", 'age_', model_age, '_params.csv'))
...
## Creating data variance - TEMPORARY
