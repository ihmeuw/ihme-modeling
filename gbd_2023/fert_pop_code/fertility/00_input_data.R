...
if (interactive()){
  ...
  loop <- 1
  decomp_step <- 'step3'
  split_version <- 'recent'
  pop_est_id <- "Population estimate run id"
  pop_syr_id <- "Single year population estimate run id"
  ddm_est_id <- "DDM estimate run id"
  birth_est_id <- "Birth estimate run id"
} else {
  ...
  parser$add_argument('--release_id', type = 'integer')
...
if (loop == 1) {
  ...
  # remove any duplicates from retries in the split version upload
...
if (loop == 1) {
  ...
  bounds <- fread(paste0(loop1_dir, 'logit_bounds.csv'))
...
readr::write_csv(logit_bounds, paste0(input_dir, '/logit_bounds.csv'))
...
if (loop == 1) {
  fem_edu <- get_covariate_estimates(release_id = release_id, covariate_id = 33, 
...
if (loop == 1) {
  readr::write_csv(input_data, paste0(input_dir, 'input_data.csv'))
  readr::write_csv(input_pop, paste0(input_dir, 'input_pop.csv'))
  readr::write_csv(input_pop_single, paste0(input_dir, 'input_pop_single.csv'))
  readr::write_csv(input_sbh, paste0(input_dir, 'input_sbh.csv'))
  readr::write_csv(input_tb, paste0(input_dir, 'input_tb.csv'))
  readr::write_csv(fem_edu, paste0(input_dir, 'fem_edu.csv'))
  readr::write_csv(loc_map, paste0(input_dir, 'loc_map.csv'))
  readr::write_csv(age_map, paste0(input_dir, 'age_map.csv'))
  readr::write_csv(sbh_locs, paste0(input_dir, 'sbh_locs.csv'))
  readr::write_csv(tb_locs, paste0(input_dir, 'tb_locs.csv'))
} else {
  l1_input_data <- fread(paste0(loop1_dir, 'input_data.csv'))
