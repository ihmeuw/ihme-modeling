...
historic_locs <- data.table(readRDS(paste0(hfc_dir, 'mpidr_reg.RDS')))
pop <- fread(paste0(input_dir, "input_pop.csv"))
input_data <- fread(paste0(input_dir, 'input_data.csv'))

loop1 <- assertable::import_files(
  list.files(
    gpr_dir, 
    full.names = T, 
    pattern = paste0(
      '(', 
      paste(current_locs, collapse = '|'),
      ')', 
      '_[0-9]{2}.csv'
    )
  ), 
...
## subset to historical locations-- for England and Wales
...
readr::write_csv(upload_tb, paste0(output_dir, loc, '_input_for_upload.csv'))
upload_results(paste0(output_dir, loc, '_input_for_upload.csv'), 'asfr', 'data', run_id = split_version, enforce_new = F, 
               send_slack = F)
