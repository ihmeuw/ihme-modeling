...
getVrOutput = function() {
  ...
  csv_files = list.files(path = paste0(j_dir, "FILEPATH"),
                         pattern = "usable",
                         full.names = T,
                         recursive = F)

  # open and combine all csv files
  vr_data = rbindlist(
...
getLitOutput = function() {
  ...
  index <- purrr::map_lgl(bundle_data, ~ all(is.na(.)))
  bundle_data <- bundle_data[, !index]
  bundle_data <- as.data.table(bundle_data)

  bundle_data[nid %in% c(146854, 256163), ":=" (mean = NA, lower = NA, upper = NA)]
...
getUbCovOutput = function() {
  ...
  sbhfiles <- list.files(paste0('FILEPATH/_dhs'), pattern = '.dta', full.names = T, recursive = T)
