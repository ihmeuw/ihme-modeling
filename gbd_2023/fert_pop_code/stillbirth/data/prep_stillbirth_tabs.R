...
# Read in user1 extractions
tabsdf1 <- readxl::read_xlsx("FILEPATH")
tabsdf1 <- as.data.table(tabsdf1[-1, ])
tabsdf1 <- tabsdf1[!is.na(nid)]

# Read in user2 extractions
tabsdf2 <- readxl::read_xlsx("FILEPATH")
tabsdf2 <- as.data.table(tabsdf2[-1, ])
tabsdf2 <- tabsdf2[!is.na(nid)]
tabsdf2 <- tabsdf2[!(!is.na(cases) & cases == "S/D")]

# Read in user3 extractions
tabsdf3 <- readxl::read_xlsx("FILEPATH")
tabsdf3 <- as.data.table(tabsdf3[-1, ])
tabsdf3 <- tabsdf3[!is.na(nid)]
...
# Combine extractions
tabsdf <- rbind(tabsdf1, tabsdf3, tabsdf2, coded_vr, fill = T)
...
if (nrow(cut_rows) > 0) readr::write_csv(cut_rows, paste0(j_dir, "/FILEPATH/rows_stillbirths_cases_not_equal.csv"))
...
if (nrow(cut_rows2) > 0) {
  readr::write_csv(cut_rows2, paste0(j_dir, "FILEPATH/rows_w_stillbirths_only.csv"))
}
...
# Subset to optimal variable set
# add underlying_nid if it gets used
...
sex_list <- as.data.table(
  sex_specific %>% group_by(ihme_loc_id, nid, year_id) %>%
...
if (nrow(cut_rows3) > 0) {
  readr::write_csv(cut_rows3, paste0(j_dir, "FILEPATH/rows_missing_male_or_female.csv"))
}
...
sex_specific_aggr_stillbirths <- aggregate(
  stillbirths ~ nid + data_type + location_id + ihme_loc_id + year_id + year + age_group_id + definition + sb_inclusion_ga + sb_inclusion_bw + sb_inclusion_preference,
  data = sex_specific[!is.na(stillbirths)],
  sum
)
sex_specific_aggr_livebirths <- aggregate(
  livebirths ~ nid + data_type + location_id + ihme_loc_id + year_id + year + age_group_id + definition + sb_inclusion_ga + sb_inclusion_bw + sb_inclusion_preference,
  sex_specific[!is.na(livebirths)],
  sum
)
sex_specific_aggr_total_births <- aggregate(
  total_births ~ nid + data_type + location_id + ihme_loc_id + year_id + year + age_group_id + definition + sb_inclusion_ga + sb_inclusion_bw + sb_inclusion_preference,
  sex_specific[!is.na(total_births)],
  sum
)

sex_specific_aggr <- merge(
  sex_specific_aggr_stillbirths,
  sex_specific_aggr_livebirths,
  by = c(
    "nid", "data_type", "location_id", "ihme_loc_id", "year_id", "year", "age_group_id",
    "definition", "sb_inclusion_ga", "sb_inclusion_bw", "sb_inclusion_preference"
  )
)
sex_specific_aggr <- merge(
  sex_specific_aggr,
  sex_specific_aggr_total_births,
  by = c(
    "nid", "data_type", "location_id", "ihme_loc_id", "year_id", "year", "age_group_id",
    "definition", "sb_inclusion_ga", "sb_inclusion_bw", "sb_inclusion_preference"
  )
...
tabsdf_sub <- tabsdf_sub[!(location_id %in% 44643:44792)]

# Final checks
assertable::assert_values(
  tabsdf_sub,
  colnames = names(tabsdf_sub[, c("nid", "data_type", "location_id", "ihme_loc_id", "year_id", "sex_id")]),
  test = "not_na"
)
...
readr::write_csv(tabsdf_sub, paste0(j_dir, "FILEPATH/stillbirth_tabs_extractions_FINAL.csv"))
readr::write_csv(tabsdf_sub, paste0(j_dir, "FILEPATH/stillbirth_tabs_extractions_", Sys.Date(), ".csv"))
