...
prep_ko <- function(data, location_set_id = 22, start_year = 1980, end_year = 2022, by_sex = 1, by_age = 1, release_id = 16) {
  ...
  ## by_sex = 0 if data includes multiple sexes
  ...
  ## square dataset
  # start and end years are unnecessary
  # by_sex = 0 if sex_id includes both 1 and 2 or is 3
  ...
  ## get locations regardless of data inclusion
...
get_kos <- function(square_data, no_miss_data, all_locs, data_locs, kos = 5, prop_to_hold = .30,
  ...
  for(ko in 1:kos) {
    ...
    # test1 == T: data was missing for that age-year in loc2 but not in loc1
    # test1 == NA: data was present for both loc1 and loc2 for that age year
    ...
    ## while loop drops data_ids until reaching specified proportion
    ...
    ## check all age group ids present: only a problem if factor(age_group_id) is in the model, it will break trying to predict new levels of a fixed effect
    if (no_new_ages == T) {
      ...
      if (length(all_held_ages > 1)) {
        ...
        ## reintegrate data points into the training dataset
