...
gpr_files <- list.files(path = paste0(estimate_dir, version_estimate, "/FILEPATH"))
scaled_files <- list.files(path = paste0(estimate_dir, version_estimate, "/FILEPATH"))

parent_locs <- fread(paste0(estimate_dir, version_estimate, "/FILEPATH/parent_locs.csv"))
...
gpr_files <- paste(paste0(estimate_dir, version_estimate, "/FILEPATH/"), gpr_files, sep = "")
scaled_files <- paste(paste0(estimate_dir, version_estimate, "/FILEPATH/"), scaled_files, sep = "")
...
## Merge on location_id and drop ihme_loc_id
...
readr::write_csv(
  agg_draws,
  paste0(estimate_dir, version_estimate, "/FILEPATH/all_location_stillbirth_draws_", main_std_def, ".csv")
)
...
readr::write_csv(
  summary,
  paste0(estimate_dir, version_estimate, "/FILEPATH/all_location_stillbirth_results_", main_std_def, ".csv")
)
...
for (y in 1:nrow(years_pc)) {
  ...
  readr::write_csv(
    pc_summary,
    paste0(estimate_dir, version_estimate, "/FILEPATH/sbr_pc_summary_", pc_start, "_", pc_end, "_", main_std_def, ".csv")
  )
  ...
  readr::write_csv(
    pc_summary,
    paste0(estimate_dir, version_estimate, "/FILEPATH/sb_pc_summary_", pc_start, "_", pc_end, "_", main_std_def, ".csv")
  )
...
readr::write_csv(
  arocsum,
  paste0(estimate_dir, version_estimate, "/FILEPATH/sbr_aroc_summary_", main_std_def, ".csv")
)
...
unscaled_files <- list.files(
  path = paste0(estimate_dir, version_estimate, "/FILEPATH"),
  full.names = TRUE
)
...
## Calculate stillbirth rate and total
if (model == "SBR/NMR") {
