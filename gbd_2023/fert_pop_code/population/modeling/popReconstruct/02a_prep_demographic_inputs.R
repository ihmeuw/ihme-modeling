...
source(paste0(code_dir, "functions/split_nzl_migration.R"))
...
lt[, nSx := nLx / shift(nLx, type = "lag"), by = setdiff(lt_id_vars, "age")]
lt[age == 0, nSx := nLx / lx]

survival_data <- lt[age < lt_max_age, c(..lt_id_vars, "nSx")]
survival_data[, age := age - 1]

survival_data_95 <- lt[age >= lt_max_age - 1, c(..lt_id_vars, "Tx")]
survival_data_95[, nSx := Tx / shift(Tx, type = "lag"), by = setdiff(lt_id_vars, "age")]
survival_data_95[, Tx := NULL]
setnafill(survival_data_95, "nocb", cols = "nSx")

survival_data <- rbind(survival_data, survival_data_95)
setnames(survival_data, c("age", "nSx"), c("age_group_years_start", "mean"))
...
# When survival ratio at onemod boundary is > 1, set it to average of adjacent age groups
smooth_survival_boundary <- function(dt, age_boundary) {

  dt_bad_boundary <- unique(dt[
    age_group_years_start == age_boundary & mean > 1,
    .(age_group_years_start = c(age_boundary - 1, age_boundary + 1)),
    by = .(location_id, year_id, sex_id)
  ])

  survival_smoothed_boundary <-
    dt[
      dt_bad_boundary,
      on = .(location_id, year_id, sex_id, age_group_years_start)
    ][
      j = .(age_group_years_start = age_boundary, mean = mean(mean)),
      by = .(location_id, year_id, sex_id)
    ]

}

survival_smoothed_boundary <- smooth_survival_boundary(survival_data, 84)

survival_data[survival_smoothed_boundary, mean := i.mean, on = .(location_id, year_id, sex_id, age_group_years_start)]
...
if (nrow(migration_data) > 0) {
  ...
  locs_nzl <- location_hierarchy[ihme_loc_id %like% "NZL", location_id]
  migration_pop_locations <- c(unique(migration_data$location_id), locs_nzl)
  gbd_pop_previous <- gbd_pop_previous[
    location_id %in% migration_pop_locations,
    list(location_id, year_id, sex_id, age_group_id, pop = population)
  ]
  ...
  # Split NZL national 5-year age group migration data into single year
  # Maori, non Maori, and national migration data.
  dt_mig_nzl_split <- split_nzl_migration(
    migration_data[location_id == 72],
    gbd_pop_previous[
      location_id %in% locs_nzl &
        year_id %in% unique(migration_data[location_id == 72, year_id])
    ],
    migration_age_groups_needed
  )

  migration_data <- rbind(
    migration_data[location_id != 72],
    dt_mig_nzl_split
  )
...
migration_data[
  location_id %in% location_hierarchy[ihme_loc_id %like% "USA", location_id] &
    year_id >= 2000,
  year_id := year_id - 1
]

# add on years that we don't have values for and assign value to be zero
migration_data_all <- CJ(location_id = location_hierarchy[is_estimate == 1, location_id], year_id = years[c(-length(years))],
...
usa_mig_prop <- 600000 / gbd_pop_previous[location_id == 102 & year_id == 1999, sum(pop)]
migration_data[location_id == 102 & year_id <= 1999,  missing_value := usa_mig_prop]
...
# Ensure migration data is only prepared up to the year before the end year of estimation
migration_data <- migration_data[year_id < year_end]
...
readr::write_csv(asfr_data, paste0(output_dir, "/inputs/asfr.csv"))
readr::write_csv(srb_data, paste0(output_dir, "/inputs/srb.csv"))
readr::write_csv(survival_data, paste0(output_dir, "/inputs/survival.csv"))
readr::write_csv(migration_data, paste0(output_dir, "/inputs/migration.csv"))
