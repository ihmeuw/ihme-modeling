...
settings <- list(
  ...
  use_onemod_mortality = FALSE,
  use_onemod_pop = FALSE,
  copy_previous_drop_age = FALSE, # Must be FALSE or numeric run ID

  queue = "all",
  submission_project_name = "proj_popreconstruction",
  default_resubmission_attempts = 3,
  r_shell_singularity_version = "3501",
  r_shell_submit_singularity = "4222",
  ...
  modeling_hierarchy = ifelse(gbd_year == 2017, "mortality", "population"),
  reporting_hierarchies = list("sdi", "who", "world bank", "eu",
                               "commonwealth",
                               "world bank income levels", "oecd", "g20",
                               "african union", "nordic", 13, 130, 131,
                               132, 136, 138, 143, 147),
  ...
  location_specific_settings_dir = paste0("FILEPATH"),
  census_specific_settings_dir = paste0("FILEPATH"),

  # uncertainty inputs
  cv_rmse_dir = "FILEPATH",

  # output directories
  output_dir = paste0("FILEPATH", ifelse(test, "test/", ""), pop_reporting_vid, "/"),
  sandbox_dir = paste0("FILEPATH", ifelse(test, "test/", ""), pop_reporting_vid, "/"),

  shared_functions_dir = "FILEPATH"
)
source(paste0(settings$shared_functions_dir, "get_covariate_estimates.R"))
settings$sdi_mv_id <- unique(get_covariate_estimates(covariate_id = 881, location_id = 1, year_id = gbd_year, release_id = release_id)$model_version_id)
...
all_hierarchies[location_set_name == "136" & level <= 1 & location_name %like% "Sahel", is_estimate := 0]
all_hierarchies[location_set_name == "african union" & level <= 1, is_estimate := 0]
...
for (loc_id in unique(all_hierarchies[, location_id])) {
  ...
  if (nrow(all_hierarchies[location_set_name == "population" & location_id == loc_id & is_estimate == 1]) == 1) {
