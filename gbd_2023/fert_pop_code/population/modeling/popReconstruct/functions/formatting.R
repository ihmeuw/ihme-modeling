...
average_adjacent_year_rates <- function(data, id_vars) {
  ...
  data <- data[, mean := ((mean + next_year_value) / 2), by = id_vars]
  data[, next_year_value := NULL]
  return(data)
}
...
format_value_matrix <- function(data, use_age_int, use_terminal_age, id_vars, baseline = F) {
  ...
  if (nrow(census_specific_settings[!is.na(census_specific_sd_scalar)]) > 0) {
    census_specific_sd_scalars <- census_specific_settings[!is.na(census_specific_sd_scalar), list(year_id, census_specific_sd_scalar)]
    data <- merge(data, census_specific_sd_scalars, all.x = T, by = c("year_id"))
    data[!is.na(census_specific_sd_scalar), sd := sd * as.numeric(census_specific_sd_scalar)]
