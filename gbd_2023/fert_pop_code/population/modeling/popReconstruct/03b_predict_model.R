...
if (!copy_results) {
  ...
  error[, pct_error := ((estimate - data) / data) * 100] # this has been changed GBD2019 from estimate in the denominator to data
  ...
  readr::write_csv(posterior_net_migration, paste0(output_dir, loc_id, "/outputs/model_fit/net_migration_posterior_drop", drop_above_age, ".csv"))
  readr::write_csv(prior_net_migration, paste0(output_dir, loc_id, "/outputs/model_fit/net_migration_prior_drop", drop_above_age, ".csv"))

  # save absolute and percent errors
  readr::write_csv(error, paste0(output_dir, loc_id, "/outputs/model_fit/errors_drop", drop_above_age, ".csv"))

  # save output derived counts of population, births, migrants, deaths
  readr::write_csv(prior, paste0(output_dir, loc_id, "/outputs/model_fit/counts_prior_drop", drop_above_age, ".csv"))
  readr::write_csv(counts, paste0(output_dir, loc_id, "/outputs/model_fit/counts_drop", drop_above_age, ".csv"))
} else {
