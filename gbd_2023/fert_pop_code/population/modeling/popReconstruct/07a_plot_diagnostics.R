...
prior_total_migration <- prior_migration[measure_id == 55 & sex_id == 3 & age_group_id == 22]
prior_migration_proportion <- prior_migration[measure_id == 77 & sex_id %in% 1:2 & age_group_id %in% age_groups[(plot_migration), age_group_id]]
...
posterior_total_migration <- migration[measure_id == 55 & sex_id == 3 & age_group_id == 22]
posterior_migration_proportion <- migration[measure_id == 77 & sex_id %in% 1:2 & age_group_id %in% age_groups[(plot_migration), age_group_id]]
...
if (!is.na(migration_current_round_run_id)) {
  ...
  gbd_total_migration_current <- gbd_migration_current[measure_id == 55 & age_group_id == 22,
                                                       list(sex_id = 3, source = paste0("GBD", gbd_year, "_v", pop_current_round_run_id), mean = sum(mean)),
                                                       by = c("location_id", "year_id", "age_group_id")]
  gbd_migration_proportion_current <- gbd_migration_current[measure_id == 77 & sex_id %in% 1:2 & age_group_id %in% age_groups[(plot_migration), age_group_id],
...
if (nrow(gbd_migration_previous) > 0) { # new locations don't exist in the previous GBD round
  if (gbd_year_previous == 2017) {
    ...
    aggregate_migrants <- function(migrants, population) {
      ...
      net_migrants_reporting[, measure_id := 55] # measure id 55 is for continuous migration counts
      net_migration_proportion_reporting[, measure_id := 77] # measure id 77 is for migration proportions
    ...
    gbd_migration_previous <- aggregate_migrants(gbd_migration_previous[measure_id == 55, list(location_id, year_id, sex_id, age_group_id, mean)],
                                                 gbd_pop_previous[, list(location_id, year_id, sex_id, age_group_id, mean = population)])
  }
  gbd_total_migration_previous <- gbd_migration_previous[measure_id == 55 & age_group_id == 22 & sex_id == 3,
                                                         list(location_id, year_id, sex_id, age_group_id, measure_id, source = paste0("GBD", gbd_year_previous), mean)]
  gbd_migration_proportion_previous <- gbd_migration_previous[measure_id == 77 & sex_id %in% 1:2 & age_group_id %in% age_groups[(plot_migration), age_group_id],
