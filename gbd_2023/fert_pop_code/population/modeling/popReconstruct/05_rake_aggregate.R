...
draw <- task_map[task_id == SLURM_ARRAY_TASK_ID, draw]
...
# additional aggregation for new reporting categories
if(nrow(location_hierarchy[location_set_name != "population"]) > 0) {
  
  # find missing special aggregates
  missing_locs <- setdiff(location_hierarchy$location_id, unique(pop_draws$location_id))
  missing_loc_sets <- unique(location_hierarchy[location_id %in% missing_locs]$location_set_name)
  
  other_pop_draws <- data.table()
  for(x in missing_loc_sets) {
    
    other_pop_draws_temp <- mortcore::agg_results(
      pop_draws, 
      id_vars = c("location_id", "year_id", "sex_id", "age_group_id", "draw"),
      value_vars = "value", 
      location_set_id = as.numeric(x), 
      gbd_year = gbd_year,
      agg_hierarchy = T, 
      loc_scalars = T
    )
    other_pop_draws <- rbind(other_pop_draws_temp, other_pop_draws)
    
  }
  
  pop_draws <- rbind(pop_draws, other_pop_draws)
  pop_draws <- unique(pop_draws)
...
readr::write_csv(data.table(test = 1), paste0(output_dir, "/raking_draws/confirm_raking_aggregation_completion_", draw, ".csv"))
