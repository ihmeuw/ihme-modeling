...
pop_fit[age_groups, age_start := i.age_group_years_start, on = "age_group_id"]
pop_fit[, age_start_abr := age_start - age_start %% 5]
pop_fit[age_start < 5, age_start_abr := age_start]

dt_onemod <-
  fs::path("FILEPATH") |>
  fs::path("FILEPATH") |>
  arrow::open_dataset(format = "arrow") |>
  dplyr::filter(location_id == loc_id, pop > 0) |>
  dplyr::collect() |>
  setDT()
...
pop_fit[
  dt_onemod,
  pop_onemod := i.pop,
  on = .(location_id, year_id, sex_id, age_start_abr = age_start)
]

pop_fit[
  j = mean_new := pop_onemod * (mean / sum(mean)),
  by = .(location_id, year_id, sex_id, age_start_abr)
]

pop_fit[!is.na(mean_new), mean := mean_new]
pop_fit[, c("age_start", "age_start_abr", "pop_onemod", "mean_new") := NULL]
...
# Assert that cv rmse predictions exist for all needed year lags
missing_year_lags <- pop_draws[is.na(pred_rmse), years_to_closest_census]
if (length(missing_year_lags) > 0) {
  stop(paste0(
    "RMSE predictions are missing for the year lags: ",
    paste0(missing_year_lags, collapse = ", ")
  ))
}
...
save_outputs(pop_reporting_draws,
             ...
             id_vars = c("location_id", "year_id", "sex_id", "age_group_id"), by_var = "draw", value_var = "value")
readr::write_csv(data.table(test = 1), paste0(output_dir, loc_id, "/outputs/confirm_create_ui_completion.csv"))
