...
library(ini, lib.loc = "FILEPATH") # this is needed for mortcore since we are using the lbd singularity
library(assertable, lib.loc = "FILEPATH") # this is needed for mortcore since we are using the lbd singularity
...
# suggested add by liming
Sys.setenv(CPLUS_INCLUDE_PATH ='/opt/intel/mkl/include')
...
copy_results <- task_map[task_id == SLURM_ARRAY_TASK_ID, as.logical(copy_results)]
...
if (!copy_results) {
  ...
  extract_mean <- function(mean_vector, variable_name, years, ages = NULL) {
    ...
    if (!is.null(ages)) var_mean[, age_group_years_start := rep(ages, times = nrow(var_mean) / length(ages))]
    return(var_mean)
  }
  ...
  s_mean <- merge(
    s_mean, 
    survival_cohort_age_groups_needed, 
    by = "age_group_years_start",
    all.x = TRUE
  )
  ...
  g_mean <- merge(
    g_mean, 
    migration_cohort_age_groups_needed, 
    by = "age_group_years_start",
    all.x = TRUE
  )
  ...
  if (!fix_fertility) readr::write_csv(f_mean, paste0(output_dir, loc_id, "/outputs/model_fit/asfr_drop", drop_above_age, ".csv"))
  if (!fix_survival) readr::write_csv(s_mean, paste0(output_dir, loc_id, "/outputs/model_fit/survival_ratio_drop", drop_above_age, ".csv"))
  if (!fix_srb) readr::write_csv(srb_mean, paste0(output_dir, loc_id, "/outputs/model_fit/srb_drop", drop_above_age, ".csv"))
  if (!fix_baseline) readr::write_csv(n0_mean, paste0(output_dir, loc_id, "/outputs/model_fit/baseline_pop_drop", drop_above_age, ".csv"))
  if (!fix_migration) readr::write_csv(g_mean, paste0(output_dir, loc_id, "/outputs/model_fit/migration_proportion_drop", drop_above_age, ".csv"))
