...
# Handle PHL National Capital Region (NCR), which gained area from Rizal 1975,
# leading to both split and non-split data for NCR
dt_phl_ncr <- subnational_data[
  location_id == 53614 & year_id == 1960,
  .(mean = sum(mean), split = TRUE),
  by = c(census_id_vars, "sex_id", "age_group_id")
]

subnational_data <- rbindlist(
  list(subnational_data[!(location_id == 53614 & year_id == 1960)], dt_phl_ncr),
  use.names = TRUE,
  fill = FALSE
)

# Check that no other censuses have both split and non-split values
dup_locations <- subnational_data[, uniqueN(split), by = census_id_vars][V1 != 1]
stopifnot("Location year has both split and non-split values" = nrow(dup_locations) == 0)

# Scale 2014 split Crimea and Sevastopol to total populations
# (total population not included in input data)
dt_ukr_sub_totals <- subnational_data[
  location_id %in% c(44934, 44939) & year_id == 2014 & (split),
  .(pop_sum = sum(mean)),
  by = .(location_id, year_id, sex_id, nid, split)
]

dt_ukr_sub_totals[location_id == 44934 & sex_id == 1, pop_total := 868579]
dt_ukr_sub_totals[location_id == 44934 & sex_id == 2, pop_total := 1022886]
dt_ukr_sub_totals[location_id == 44939 & sex_id == 1, pop_total := 181735]
dt_ukr_sub_totals[location_id == 44939 & sex_id == 2, pop_total := 211569]
dt_ukr_sub_totals[, scalar := pop_total / pop_sum]

subnational_data[
  dt_ukr_sub_totals,
  mean := mean * i.scalar,
  on = .(location_id, year_id, sex_id, nid, split)
]
...
readr::write_csv(census_data, paste0(output_dir, "/outputs/03_split_historical_subnationals.csv"))
