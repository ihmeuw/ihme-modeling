...
settings <- list(
  ...
  r_shell_singularity_version = "4222",
  ...
  use_raw_net_pes_adjustment_locs = c("AUS", "USA", "NZL", "IND", "COL", "BGR", "CRI"),
  ...
  output_dir = paste0("FILEPATH", ifelse(test, "test/", ""), pop_processing_vid, "/"),
  sandbox_dir = paste0("FILEPATH", ifelse(test, "test/", ""), pop_processing_vid, "/"),

  shared_functions_dir = "FILEPATH"
)
source(paste0(settings$shared_functions_dir, "get_covariate_estimates.R"))
settings$sdi_mv_id <- unique(get_covariate_estimates(covariate_id = 881, release_id = release_id, location_id = 1, year_id = gbd_year)$model_version_id)
...
hierarchy_name <- ifelse(gbd_year == 2017, "mortality", "population") # mortality hierarchy was used for 2017 since the population hierarchy was created late and incorrectly
...
readr::write_csv(location_hierarchy, paste0(output_dir, "/inputs/location_hierarchy.csv"))
