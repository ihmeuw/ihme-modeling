...
smooth_survival_boundary <- function(dt, age_boundary) {
  ...
  dt_bad_boundary <- unique(dt[
    age == age_boundary & value > 1,
    .(age = c(age_boundary - 1, age_boundary + 1)),
  ...
  survival_smoothed_boundary <-
    dt[
      ...
      j = .(age = age_boundary, value = mean(value)),
      by = .(location_id, year_id, sex_id)
    ]

}

survival_smoothed_boundary <- smooth_survival_boundary(survival, 84)

survival[survival_smoothed_boundary, value := i.value, on = .(location_id, year_id, sex_id, age)]
...
# Use later year nLx in place of 1950 value for some locations
missing_triangle_lifetables_update <- missing_triangle_lifetables[
  (location_id == 16 & year_id == 1960) |
    (location_id == 38 & year_id == 1956)
]

missing_triangle_lifetables_update[, year_id := 1950]

census_data_backwards_ccmpp[
  missing_triangle_lifetables_update,
  nLx := i.nLx,
  on = c("location_id", "year_id", "sex_id", "age_group_years_start")
]
...
readr::write_csv(baseline_censuses, paste0(output_dir, "/diagnostics/backwards_ccmpp_results.csv"))
...
readr::write_csv(census_data, paste0(output_dir, "/outputs/09_generate_baseline.csv"))
