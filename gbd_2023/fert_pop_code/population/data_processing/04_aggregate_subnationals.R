...
# 2020 UKR census data point was only conducted in Crimea and Sevastopol so not a national census
final_subnational_data <- final_subnational_data[!(location_id %in% get_location_id("UKR", location_hierarchy) & year_id == 2021)]
...
# use detailed national IDN 2020 census data
final_subnational_data[aggregate == F & location_id %in% get_location_id("IDN", location_hierarchy) & year_id == 2020, outlier_type := "not outliered"]
final_subnational_data[aggregate == T & location_id %in% get_location_id("IDN", location_hierarchy) & year_id == 2020, outlier_type := "duplicated"]
...
# use aggregate CHN_44533 data rather than DYB data
...
readr::write_csv(census_data, paste0(output_dir, "/outputs/04_aggregate_subnationals.csv"))
...
readr::write_csv(subnational_aggregate_check, paste0(output_dir, "/diagnostics/subnational_aggregates.csv"))
