...
data_run_id <- data_45q15_version
} else {
  gbd_year <-
  end_year <-
  version_id <-
...
LDI <- as.data.frame(fread(paste0("FILEPATH")))
...
LDI <- merge(LDI, codes, by = c("location_id"), all=FALSE)

edu <- as.data.frame(fread(paste0("FILEPATH")))
...
# Load covid infections
covid_inf_pc <- as.data.frame(fread(paste0("FILEPATH")))
covid_inf_pc <- covid_inf_pc[, c("location_id", "year_id", "mean_value")]
names(covid_inf_pc)[names(covid_inf_pc)=="year_id"] <- "year"
names(covid_inf_pc)[names(covid_inf_pc)=="mean_value"] <- "covid_inf_pc"

# creating old andhra pradesh
pop_ap <- population[location_id %in% c("4841", "4871")]
setnames(pop_ap, c("year_id"), c("year"))
pop_ap <- as.data.table(pop_ap[, list(pop = sum(population)), by = c("location_id", "year")])
covid_inf_pc_ap <- as.data.table(covid_inf_pc[covid_inf_pc$location_id %in% c(4841, 4871), ])
covid_inf_pc_ap <- merge(covid_inf_pc_ap, pop_ap, by=c("location_id", "year"))
covid_inf_pc_ap[, covid_inf_pc := covid_inf_pc * pop]
setkey(covid_inf_pc_ap, year)
covid_inf_pc_ap <- covid_inf_pc_ap[, .(covid_inf_pc = sum(covid_inf_pc), pop = sum(pop)), by = key(covid_inf_pc_ap)]
covid_inf_pc_ap[, covid_inf_pc := covid_inf_pc/pop]
covid_inf_pc_ap[, location_id := 44849]
covid_inf_pc_ap[, pop:= NULL]
covid_inf_pc <- rbind(covid_inf_pc, covid_inf_pc_ap)


# Get 5q0 estimates
m5q0_estimates <- fread(paste0("FILEPATH"))
...
# merge together
fwrite(edu, paste0("FILEPATH"), row.names = F)
fwrite(hiv, paste0("FILEPATH"), row.names = F)
fwrite(LDI, paste0("FILEPATH"), row.names = F)
fwrite(covid_inf_pc, paste0("FILEPATH"), row.names = F)
fwrite(m5q0_estimates, paste0("FILEPATH"), row.names = F)
codmod <- merge(edu, hiv, by=c("location_id", "year", "sex"), all.x = T)
codmod <- merge(codmod, LDI, by=c("location_id", "ihme_loc_id", "year"))
codmod <- merge(codmod, covid_inf_pc, by=c("location_id", "year"), all.x = T)
...
codmod <- ddply(codmod, c("ihme_loc_id", "year", "sex"),
                function(x) {
                  ...
                  data.frame(LDI_id = x$LDI_id[1],
                             ...
                             covid_inf_pc = ifelse(is.na(x$covid_inf_pc[1]), 0, x$covid_inf_pc[1])
                  )
                }
)
assert_values(data = codmod, colnames = colnames(codmod), test = "not_na")
assert_values(data = codmod, colnames = c("LDI_id", "hiv", "mean_yrs_educ", "m5q0", "covid_inf_pc"), test = "gte", test_val = 0)
...
columns_to_remove <- c("run_id", "method_name", "viz_year", "sex_id",
                       "underlying_nid", "age_group_id", "outlier")
...
## duplicate data in countries with only one data point
single <- table(ihme_loc_id = raw$ihme_loc_id, sex = raw$sex)
single <- melt(single)
...
raw <- data.table(raw)
raw[adj45q15 >=1, adj45q15 := 0.99]

raw <- raw[!is.na(raw$year), ]
n_raw <- nrow(raw)

raw <- raw[location_id != 44849L]

# exposure for certain method/source types cannot be 0
...
raw <- raw[order(raw$ihme_loc_id, raw$year, raw$sex, raw$category), c("location_id", "ihme_loc_id", "year", "sex", "mort", "source_type", "category", "vr", "nid", "exposure", "adjust.sd", "obs45q15")]
...
data <- data[order(ihme_loc_id, sex, year),
             list(location_id, location_name, super_region_name, region_name,
                  parent_id, ihme_loc_id, sex, year, LDI_id, mean_yrs_educ, covid_inf_pc,
                  m5q0, adjust.sd, hiv, mort, exposure, obs45q15, source_type, type,
                  category, vr, nid, data)]

assert_values(data = data, colnames = c("mort", "obs45q15", "LDI_id", "mean_yrs_educ", "hiv", "covid_inf_pc", "m5q0"), test = "not_inf")

fwrite(data, paste0("FILEPATH"))
...
fwrite(mean_adult_hiv_deduplicated, paste0("FILEPATH"))

location_parameter_types <- unique(data[, list(location_id, ihme_loc_id, location_name, type, sex)])
fwrite(location_parameter_types, paste0("FILEPATH"))
