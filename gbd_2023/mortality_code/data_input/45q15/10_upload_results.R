...
if(!interactive()) {
  ...
  version_id <-
  data_45q15_version <-
  ddm_estimate_version <-
  estimate_5q0_version <-
...
setnames(results, c("med_stage1", "med_stage2", "mort_med", "mort_lower", "mort_upper"), c("mean1", "mean2", "mean3", "lower3", "upper3"))
...
id_vars <- list(location_id = unique(locations$location_id)[locations$location_id != 6], year_id = c(1950:end_year), sex_id = c(1:2), estimate_stage_id = c(1:3))
...
upload_results(filepath = paste0("FILEPATH"), model_name = "45q15", model_type = "estimate", run_id = version_id)
