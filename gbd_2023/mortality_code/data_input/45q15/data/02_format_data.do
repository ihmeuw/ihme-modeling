...
preserve
keep if filename == "FILEPATH" | outlier == 1
tempfile others
save `others', replace
restore
drop if filename == "FILEPATH" | outlier == 1

// Only source the non-outliered, non-VR data
tempfile data
save `data', replace
	
// Make a map of location_ids and ihme_loc_ids because reshape has too many variables later
preserve
...
import excel using "FILEPATH", firstrow clear
	...
	** rename variables
	...
	// Drop others again for more NID fixes
	preserve
	keep if filename == "FILEPATH" | outlier == 1
	save `others', replace
	restore
	...
	// Do more fixes for missing NIDs
	...
	merge m:1 year_id using `sus_nids', assert(2 4) keep(4) keepusing(nid) nogen update replace		
	save `sus_nids', replace
	use `no_nids', clear
	merge 1:1 year_id location_id sex_id age_group_id method_id source_type_id viz_year adjustment using `sus_nids', assert(1 4) nogen update replace		// Update master set
	...
	sort location_id year_id sex_id method_id source_type_id nid underlying_nid adjustment outlier
	quietly by location_id year_id sex_id method_id source_type_id nid underlying_nid adjustment outlier:  gen dup = cond(_N==1,0,_n)
