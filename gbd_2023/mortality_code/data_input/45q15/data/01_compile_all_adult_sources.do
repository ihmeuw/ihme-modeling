...
** **********************
	...
	replace shock = 1 if ihme_loc_id == "COG" & floor(year) == 1997 
...
** **********************
  ...
  replace exclude = 1 if ihme_loc_id == "IRQ" & source_type == "VR" & year == 2008
	...
	replace exclude = 1 if ihme_loc_id == "SAU_44544" & source_type == "VR" & year == 2000
	...
	** KEN subnational DHS
	...
	// Mark all unadjusted datapoints as excluded for now
...
** **********************
	...
	** fill in/replace calcualted exposures for SRS/DSP 
	replace exposure = correction*natl_pop if inlist(source_type, "SRS", "DSP")

	** replace calculated exposures with corrected nat'l populations for sub-national sources from DYB 
	replace exposure = correction*natl_pop if regexm(deaths_source, "DYB") & inlist(source_type, "SURVEY", "UNKNOWN")

	** fill in exposures for sources that don't already have an exposure
	...
	// outlier India DSS and Household points after 2000
