...
# requires mortdb package, loaded in main script
...
resid_space_time <- function(data, gbd_year, region=NULL, lambda=.6, zeta=.99, min_year=1950, max_year=NULL, print=T, use_super_regs=F, sex_mod=F, sex_input=NULL, age_input=NULL, params=NULL) {
  ...
  super_regs <- get_locations(gbd_year=gbd_year)
  super_regs <- super_regs[, c("ihme_loc_id", "super_region_name")]
    ...
    for (cc in countries) {
      ...
      if(use_super_regs!=F){
        if(rr %in% use_super_regs){
          if(!"super_region_name" %in% names(data)) data <- merge(data, super_regs, all.x=T, by="ihme_loc_id")
      ...
      for (yy in min_year:max_year) {
        ...
        if(nrow(model.data) == 0){
          # happens when no data in super region
          preds[[count]] <- data.frame(ihme_loc_id = cc, year = (yy+0.5), pred.2.resid = 0, weight = 1, keep = 1, stringsAsFactors = F)
        ...
        } # close if/else when we have country or super region data
