...
DoExclusions <- function(data, add.data, order, births, ddm_run_id) {
  if (order == 1) {
    ...
    data$iso3.sourcedate <- paste(data$iso3, data$source.date, sep=" ")	# creates a combined field for country and year of census
  ...
  else if (order == 267) {   # DHS in-depth
    # drop PHL indepth 1993
    add.data <- add.data[add.data$iso3 != "PHL",]
  }
  else if (order == 299) {    # INDIA: SRS 1990-1997
    add.data<-subset(add.data,t>1995)
  }
  else if (order == 301) { # BANGLADESH: SRS 2001-2003
  ...
  else if (order == 314) {   # BANGLADESH: SRS
    ...
    if (file.exists(paste0("FILEPATH"))) {
      vr_split <- setDT(read_dta(paste0("FILEPATH")))
      vr <- rbind(vr, vr_split, fill = TRUE)
    }
...
UpdateVr <- function(vr, pop, births, data) {
  ...
  vr <- vr[!(grepl("IND", ihme_loc_id) &
               ...
               (year >= 2008 & year <= 2019)), ]
  vr <- vr[deaths_nid != 449948] # cont. removing IND SRS for 2018 (national and states)
  ...
  vr <- vr[sex == "both" & year>=1950, .SD,
           .SDcols = c("ihme_loc_id","country","year","deaths_source","source_type", "deaths_nid", "deaths_underlying_nid",
                       paste("DATUM",seq(0,4),"to",seq(0,4),sep="FILEPATH"),"DATUM1to4","DATUM0to4")]
  ...
  # take out Bangladesh SRS data points
...
dirty_scrub <- function(data) {
  ...
  scrub[data$ihme.loc.id == "MDV" & tolower(data$source) == "ministry of health vital statistics - vital registration" & data$data.age == "old" & data$t!="1995" & data$t!="1997" & data$t!="1998" & data$t!="1999"] <- T
  ...
  scrub[data$ihme.loc.id == "COD" & data$source == "average of east and west "] <- T
  ...
  scrub[data$ihme.loc.id == "PNG" & data$source == "DHS Preliminary report " & data$source.date == 2006] <- T
...
kill <- function(data) {
  old_locs <- fread(
    paste0(get_path("FILEPATH"),"FILEPATH"),
    stringsAsFactors = F)
  ...
  for (i in 1:length(unique(data$ihme.loc.id))) {
    ...
    if( sum(indir) != 0 ) {
      ...
      for (j in 1:length(i.survs)) {
        ...
        else kill0 <- c(kill0,point1,point2)
      }
    }
  }
  ...
  kill[data$ihme.loc.id == "IDN" & data$source == "SUPAS" & data$in.direct == "direct"] <- T
  ...
  #Exceptions are DSP, SRS, India, and PAK_demographic_survey points
  ...
  #outlier VR in Bolivia 2000-2004
  ...
  # Outlier PNAD before 2005 in the Amazon regions of Brazil (north-west region)
  ...
  #outlier GBR data point
  kill[data$ihme.loc.id=="GBR" & data$t==2000] <- T
  # outlier Serbia VR 1998-2007
...
indiaFix <- function(data) {
  ...
  #initial expected number of rows
