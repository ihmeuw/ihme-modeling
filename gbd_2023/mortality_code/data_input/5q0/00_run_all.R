...
pre_gbd_year <- gbd_year_previous
...
# Get best old version
best_old_version_id <- mortdb::get_best_versions("5q0 estimate", gbd_year = gbd_year_previous)
best_old_version_id <- best_old_version_id$`5q0 estimate`

# Set filepaths and create directories
output_dir <- paste0("FILEPATH")
subdirs <- c("data", "model", "draws", "summaries", "graphs",
             sapply(0:(num_rakings - 1), function(i) { paste0("raking/", i)}))

for (dir in subdirs) {

  if (!dir.exists(paste0("FILEPATH"))) dir.create(paste0("FILEPATH"), recursive = TRUE)
...
template_comparison <- jobmonr::task_template(
  ...
  op_args = list("script_path", "conda_path", "conda_env", "code_path"),
  task_args = list("gbd_year", "end_year", "best_old_version_id", "gbd_round_id", "pre_gbd_year"),
  node_args = list("version_id", "code_dir")
)
...
task_comparison <- jobmonr::task(
  ...
  end_year = end_year,
  code_dir = code_dir,
  pre_gbd_year = pre_gbd_year,
  gbd_round_id = gbd_round_id,
  best_old_version_id = best_old_version_id,
  compute_resources = list(
    ""
  )
)
