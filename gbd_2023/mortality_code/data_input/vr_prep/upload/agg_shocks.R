...
all_locs <- setDT(get_locations(level = "all", gbd_year = gbd_year))
age_map <- setDT(get_age_map())

output_folder <- paste0("FILEPATH")

disaster_vr_file <- paste0("FILEPATH")
war_vr_file <- paste0("FILEPATH")
...
aggregated_dt <- agg_results(combined_dt, id_vars = id_vars, end_agg_level = 3, loc_scalars = F, value_vars = "numkilled", gbd_year = gbd_year)

write_dta(aggregated_dt, paste0("FILEPATH"), version = 13)
