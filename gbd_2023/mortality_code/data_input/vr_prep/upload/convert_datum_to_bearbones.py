...
def convert_datum_to_bearbones(df):
    ...
    if 'estimate_stage_id' in list(df):
        FINAL_COLS.append('estimate_stage_id')
    df = df[FINAL_COLS]
...
def melt_stub(df, stub, j):
    ...
    i = [x for x in list(df) if x not in value_cols]
...
def _test_datum_to_years_dictionary(datum_dict, datum_vars):
    def test_names():
        ...
        Raises:
            ...
            age_group_id
        """
        age_names = pd.read_csv("FILEPATH")['age_group_name']
        invalid_names = []
