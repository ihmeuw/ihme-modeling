...
## loop through country-source type
  ...
  ## loop through years
      for (yy in min_year:max_year) {
        ...
        preds[[count]] <- data.frame(iso3_sex_source=cc, year=yy, predresid=constant, stringsAsFactors=F)
...
loess_resid <- function(data) {
  for (cc in unique(data$iso3_sex_source)) {
    ...
    data$pred2final[ii] <- predict(loess(pred2raw ~ year, span=0.3, data=data[ii,]))
  }
  data
}