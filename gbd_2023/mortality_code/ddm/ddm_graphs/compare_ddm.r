...
include_comp_point_est <-
subversion <-
comp_version_id <-
comp_subversion <-
prev_version <- get_proc_version("ddm", "estimate", run_id = "best", gbd_year = 2021)
main_dir <- paste0("FILEPATH")
comp_dir <- paste0("FILEPATH")
comp_dir_prev <- paste0("FILEPATH")
...
if(!is.na(comp_version_id)){
  comp <- data.table(haven::read_dta(paste0("FILEPATH"), encoding = "latin1"))
  final_comp_prev <- data.table(haven::read_dta(paste0("FILEPATH")))
}

## GBD prev final completeness
final_comp_prev <- data.table(haven::read_dta("FILEPATH"))

## GBDprev Data
data_prev <- setDT(haven::read_dta(paste0("FILEPATH"), encoding = "latin1"))
...
final_comp_prev <- format_cols(final_comp_prev)
...
data_prev <- merge(data_prev, colors_symbols, by='comp_type', all.x=T)
data_prev <- data_prev[order(region_name, iso3_sex_source, year, comp_type)]
data_prev[, col := makeTransparent(col, alpha=40)]
...
final_comp_prev <- final_comp_prev[order(iso3_sex_source, year)]

# make plots ===========================================================================

#### Plotting function ####
plot_ddm <- function(all, comp, all_prev, final_comp_prev, cc) {
  temp <- all[all$iso3_sex_source == cc,]
  if(!is.na(comp_version_id)) temp2 <- comp[iso3_sex_source == cc,]
  temp3 <- final_comp_prev[final_comp_prev$iso3_sex_source == cc,]
  temp4 <- data_prev[iso3_sex_source== cc,]
  ...
  if (grepl("VR|SRS|DSP", cc)){
    ...
    legend("topleft", c("child", "ggb", "seg", "ggbseg", "ccmp_aplus_no_migration","ccmp_aplus_migration",
                        "child", "child_prev", "1st", "1st_prev", "2nd", "Truncated Completeness", "CI truncated (VR/SRS/DSP/MCCD/CR)", "Final Completeness","GBD Previous", "GBD Previous UI"),
    ...
    legend("topleft", c("child", "ggb", "seg", "ggbseg", "ccmp_aplus_no_migration", "ccmp_aplus_migration",
                        "child", "child_prev", "1st", "1st_prev", "2nd", "Truncated Completeness", "CI untruncated (VR/SRS/DSP/MCCD/CR)", "Final Completeness","GBD Previous", "GBD Previous UI"),
  ...
  # GBD prev lines
  lines(temp3$year, temp3$final_comp, col="green4", lwd=3, lty=1)
  lines(temp4$year, temp4$pred1, col='orange', lwd=1, lty=2)
  lines(temp4$year, temp4$u5_comp_pred, col='red', lwd=1, lty=2)
  ...
  for (sr in sort(unique(all$super_region_name))) {
    ...
    for (rr in sort(unique(tempall$region_name))) {
      ...
      for (cc in sort(c(unique(tempall2$iso3_sex_source[grepl("VR|SRS|DSP|MCCD|CR", tempall2$iso3_sex_source)]),unique(tempall2$iso3_sex_source[!grepl("VR|SRS|DSP|MCCD|CR", tempall2$iso3_sex_source)])))) {
        plot_ddm(all, comp, data_prev, final_comp_prev, cc)
      }
    }
  }
