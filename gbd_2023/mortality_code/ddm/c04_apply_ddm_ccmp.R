...
TMB::compile(paste0("FILEPATH"))  # Compile the C++ file
dyn.load(dynlib(paste0("FILEPATH")))  # Dynamically link the C++ code
TMB::openmp(n = 1)
...
if (with_shock == 1) {
  ...
  c00_file <- paste0("FILEPATH")
} else {
  input_file <- paste0("FILEPATH")
  c00_file <- paste0("FILEPATH")
}
input_03 <- setDT(read_dta(input_file))

# check all locations made it through stata processing
input_00 <- setDT(read_dta(c00_file))
if (!all(unique(input_00$location_id) %in% unique(input_03$location_id))) {
  stop (paste0(
    "Location_ids dropped in stata processing: ",
    setdiff(unique(input_00$location_id), unique(input_03$location_id))
  ))
}

input_03 <- input_03[sex != 'both' & source_type %in% c('VR', 'DSP', 'SRS', 'MCCD', 'VR_2018plus')]
...
fwrite(all_ages_deaths, output_file)
...
# drop data where the terminal age group is below 60
...
# drop locations not in the location hierarchy
...
fwrite(dropped_points, output_file)
...
readstata13::save.dta13(all_completeness, output_file)
