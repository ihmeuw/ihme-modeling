...
parser$add_argument(
  "--with_shock", type = "integer", required = !interactive(),
...
if (with_shock == 1) {
  ...
  d00 <- haven::read_dta(paste0("FILEPATH")) %>% setDT
} else {
  dt <- haven::read_dta(paste0("FILEPATH")) %>% setDT
  d00 <- haven::read_dta(paste0("FILEPATH")) %>% setDT
}
...
if (!all(unique(d00$location_id) %in% unique(dt$location_id))) {
  stop (paste0(
    "Location_ids dropped in stata processing: ",
    setdiff(unique(d00$location_id), unique(dt$location_id))
  ))
}
...
readstata13::save.dta13(all_results, output_file)
