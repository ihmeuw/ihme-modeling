...
cause_meta_data <- rbind(data.table(acause_label = 'mental_schizo', bundle_id = 152, age_pattern_me = 24004, sex_ratio_by_age = F, covariates = ""),
                         ...
                         data.table(acause_label = 'mental_unipolar_dys', bundle_id = 160, age_pattern_me = 23998, sex_ratio_by_age = F, covariates = "cv_recall_1yr"), 
                         ...
                         data.table(acause_label = 'bullying', bundle_id = 3122, age_pattern_me = NA, sex_ratio_by_age = F, covariates = "cv_recall_1yr, cv_no_bullying_def_presented, cv_traditional_only, cv_quant_low_freq_1, cv_quant_low_freq_2, cv_qual_low_freq, cv_qual_opt_freq"),
                         data.table(acause_label = 'mental_anxiety', bundle_id = 162, age_pattern_me = 24000, sex_ratio_by_age = F, covariates = "cv_recall_1yr"),
                         data.table(acause_label = 'imp_id', bundle_id = 291, age_pattern_me = NA, sex_ratio_by_age = F, covariates = ""),
                         data.table(acause_label = 'mental_id', bundle_id = 169, age_pattern_me = NA, sex_ratio_by_age = F, covariates = ""))
...
if(cause_meta_data$covariates != ""){crosswalk_pairs <- paste0("/FILEPATH/gbd2023_crosswalk_pairs_", acause, ".xlsx")}
...
if(sex_ratio_by_age == T){
    cov_list <- c(cov_list, list(mr$LinearCovModel("prev_by_mid_age")))
}
...
## Run MR-BRT ##
sex_data_mrbrt <- mr$MRData()
sex_data_mrbrt$load_df(data = sex_ratios, 
                       col_obs = "log_ratio", 
                       col_obs_se = "log_ratio_se", 
                       col_covs = c(as.list(measure_cvs), list("prev_by_mid_age")), 
                       col_study_id = "nid")

model <- mr$MRBRT(data = sex_data_mrbrt, 
                  cov_models =cov_list, 
                  inlier_pct =0.9)
...
sex_coefs <- sex_coefs[abs((se^2) / ratio) < 100,] # get rid of estimates with extremely high RSE
...
sex_ratio_data <- mr$MRData()
sex_ratio_data$load_df(data = sex_ratio, col_covs=as.list(model$cov_names))
sex_beta_samples <- mr$core$other_sampling$sample_simple_lme_beta(1000L, model)
...
cov_metadata_rows <- data.table(gbd_round = 2023, date_run = Sys.Date(), cause = acause,	parameter	= gsub("cv_", "", model$cov_model_names), covariate = "sex", gamma= model$gamma_soln,	log_or_logit= "log")
...
if(length(covariates) > 0){
  crosswalk_pairs_data <- data.table(read.xlsx(crosswalk_pairs, "Pairs"))
  crosswalk_pairs_data <- crosswalk_pairs_data[pair_review == 1]
  suppressWarnings(crosswalk_pairs_data[, covariate := NULL]) # present in some crosswalk pair datasets and causes issues later
  ## ensure unique estimates
  
 if(acause == "mental_unipolar_dys"){covariates <- covariates[covariates != "cv_lay_interviewer"]}
  
  
  if(unique(covariates %in% names(crosswalk_pairs_data))){
    setnames(crosswalk_pairs_data, covariates, gsub("cv_", "d_", covariates)) 
  }
  ...
  crosswalk_pairs_data[r_nid >= a_nid, `:=` (unique_label = paste0(r_nid, a_nid, r_year_start, a_year_start, r_year_end, a_year_end, r_age_start, a_age_start, r_age_end, a_age_end, sex, r_loc, r_mean, a_mean),
                                             sex_label = paste0(r_nid, a_nid, r_year_start, a_year_start, r_year_end, a_year_end, r_age_start, a_age_start, r_age_end, a_age_end, r_loc))]
  crosswalk_pairs_data[r_nid < a_nid, `:=` (unique_label = paste0(a_nid, r_nid, a_year_start, r_year_start, a_year_end, r_year_end, a_age_start, r_age_start, a_age_end, r_age_end, sex, r_loc, a_mean, r_mean),
                                            sex_label = paste0(a_nid, r_nid, a_year_start, r_year_start, a_year_end, r_year_end, a_age_start, r_age_start, a_age_end, r_age_end, r_loc))]
  ...
  bothsex <- T #if T, will prefer both sex estimates where both and sex-specific are available
  if(bothsex == T){
  ...
  crosswalk_pairs_data[, `:=` (logit_alt = logit(a_mean), logit_alt_se = deltamethod(~log(x1/(1-x1)), a_mean, a_se^2),
                               logit_ref = logit(r_mean), logit_ref_se = deltamethod(~log(x1/(1-x1)), r_mean, r_se^2)), by = "row_id"]
  
  crosswalk_pairs_data[, `:=` (logit_diff = logit_alt - logit_ref, logit_diff_se = sqrt(logit_alt_se^2 + logit_ref_se^2))]

  if(acause %in% c('mental_unipolar_mdd')){
  ...
  crosswalk_pairs_data <- crosswalk_pairs_data[!(ref == 'gold' & alt == "gold"),]
...
crosswalk_data_mrbrt <- cw$CWData(df = crosswalk_pairs_data, obs = "logit_diff", obs_se = "logit_diff_se", dorm_separator = "-", alt_dorms = "alt", ref_dorms = "ref", covs = list(), study_id = "id")
crosswalk_fit <- cw$CWModel(cwdata = crosswalk_data_mrbrt, obs_type = "diff_logit", cov_models = list(cw$CovModel("intercept")), gold_dorm = "gold")
crosswalk_fit$fit(inlier_pct = 0.9)
...
if(acause %in% c('mental_unipolar_mdd')){ # Estimate network without symptom scales first to ensure symptom scales (the noisy crosswalks) does not impact other crosswalks
  ...
  dys_crosswalk_filepath <- paste0("/FILEPATH/", "mental_unipolar_dys", "/crosswalks/")
  
  py_save_object(object = crosswalk_fit, filename = paste0(dys_crosswalk_filepath, "/mdd_crosswalk_model.pkl"), pickle = "dill")
  write.csv(summaries, paste0(dys_crosswalk_filepath, "/mdd_crosswalk_summaries.csv"),row.names=F)
}
...
cov_metadata_rows <- data.table(gbd_round = 2023, date_run = Sys.Date(), cause = acause,	parameter	= "prevalence", covariate = summaries$dorms, gamma = cov_gamma,	log_or_logit = "logit")
...
if(acause == "mental_unipolar_mdd"){
  dys_row <- cov_metadata_rows[covariate == 'lay_interviewer']
  dys_row[["cause"]] <- "mental_unipolar_dys"
  rbind(cov_metadata_rows, dys_row)
}

if(acause == "mental_unipolar_dys"){
  message("As Dysthymia uses crosswalks from MDD, make sure the mr_brt code has also been run for MDD before running data_prep code for Dysthymia")
}
cov_metadata <- rbind(cov_metadata[!(paste0(gbd_round, cause, parameter, covariate) %in% cov_metadata_rows[,(paste0(gbd_round, cause, parameter, covariate))])], cov_metadata_rows)
