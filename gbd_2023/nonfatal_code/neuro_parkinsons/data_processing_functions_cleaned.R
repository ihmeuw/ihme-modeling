...
aggregate_marketscan <- function(mark_dt){
  ...
  marketscan_dt[, `:=` (location_id = ID, location_name = "United States", mean = cases/sample_size,
                        lower = NA, upper = NA)]
  z <- qnorm(0.975)
...
age_sex_split <- function(raw_dt){
  ...
  both_nids <- df_split[df_split$sex=="Both"]
  both_nids <- both_nids$nid
  both_nids <- unique(both_nids)
  df_split <- df_split[df_split$nid %in% both_nids, ]
  has_male <- df_split[df_split$sex=="Male"]
  has_male <- has_male$nid
  has_male <- unique(has_male)
  df_split <- df_split[df_split$nid %in% has_male,]
  df_split$cases <- as.numeric(df_split$cases)
  df_split$sample_size <- as.numeric(df_split$sample_size)
...
## FIND SEX MATCHED DATA (WITHIN STUDY)
find_sex_matches_for_sex_split <- function(dt, measure_vars){
  sex_dt <- copy(dt)
  sex_dt <- sex_dt[sex %in% c("Male", "Female") & measure %in% measure_vars]
  sex_dt <- sex_dt[!(mean == 0 | cases == 0), ]
...
col_order <- function(dt){
  epi_order <- fread(paste0("~/Repos/shared-code/upload_order.csv"), header = F)
  ...
  setnames(dt, c("cv_ukpd_brain_bank", "cv_mds_criteria", "cv_gelb_criteria"),
           c("cv_UKPD_Brain_Bank", "cv_MDS_criteria", "cv_Gelb_criteria")) ## RECORRECT CAPITALS (JUST FOR PD CV'S)
  return(dt)
}
