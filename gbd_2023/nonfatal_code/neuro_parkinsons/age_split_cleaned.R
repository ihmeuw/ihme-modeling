...
expand_age <- function(small_dt, age_dt = ages, age_groups = age){
  ...
  split <- split[age_group_id %in% age_groups | age_group_id == ID] ##don't keep where age group id isn't estimated for cause
  return(split)
}

## GET DISMOD AGE PATTERN
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = id, ## AGE MODEL HAS WIDE TIME WINDOW TO FIT SO YEAR DOESN'T MATTER
                           measure_id = c(ID), location_id = locs, source = "epi",
                           status = "best", sex_id = c(1,2), release_id = 16,
  ...
  us_population <- get_population(location_id = locs, year_id = 2010, sex_id = c(1, 2), 
                                  age_group_id = age_groups, release_id = ID)
  ...
  age_1 <- age_1[age_group_id %in% c(ID), ]
  se <- copy(age_1)
  se <- se[age_group_id==ID, .(measure_id, sex_id, se_dismod, location_id)] 
  ...
  age_1[, age_group_id := ID]
  age_pattern <- age_pattern[!age_group_id %in% c(ID)]
  age_pattern <- rbind(age_pattern, age_1)
  
  ## CASES AND SAMPLE SIZE
  age_pattern[measure_id == ID, sample_size_us := rate_dis * (1-rate_dis)/se_dismod^2]
  age_pattern[measure_id == ID, sample_size_us := rate_dis/se_dismod^2]
  ...
  sex_3[measure_id == ID, se_dismod := sqrt(rate_dis*(1-rate_dis)/sample_size_us)] ##back calculate cases and sample size
  sex_3[measure_id == ID, se_dismod := sqrt(cases_us)/sample_size_us]
...
get_pop_structure <- function(locs, years, age_groups){
  populations <- get_population(location_id = locs, year_id = years, release_id = ID,
                                sex_id = c(ID), age_group_id = age_groups)
  ...
  populations <- populations[!age_group_id %in% c(ID)]
...
format_data_forfinal <- function(unformatted_dt, location_split_id, region, original_dt){
  ...
  original_dt[id %in% ids_over1, note_modeler := paste0(note_modeler, " | note age split because age split went over 1")]
...
age_split <- function(gbd_id, df, age, region_pattern, location_pattern_id){
  ...
  super_region_dt <- get_location_metadata(location_set_id = ID, release_id = ID)
