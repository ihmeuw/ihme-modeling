...
location_split <- function(raw_dt, map){
  map_dt <- copy(map)
  dt <- as.data.table(copy(raw_dt))
  map_dt[, id := .GRP, by = c("nid", "parent_loc")]
  split_dt <- rbindlist(lapply(1:length(map_dt[, unique(id)]), function(x) split_loc(num = x, split_dt = dt, map_dt = map_dt)), use.names = T, fill = T)
...
split_even <- function(manip_dt, n, p, cs){
  ...
  split_dt[, `:=` (lower = NA, upper = NA, cases = sample_size * mean, effective_sample_size = NA, uncertainty_type_value = NA,
                   note_modeler = paste0(note_modeler, " | location split using by number of locs (", num, ")"), ihme_loc_id = NULL)]
...
split_weight <- function(manip_dt, n, p, cs){
  ...
  pops <- get_population(year_id = dt[, unique(midyear)], location_id = cs, release_id = 16)
  pops = merge(pops, age_dt[,c("age_group_id", "age_group_years_start", "age_group_years_end")], by = c("age_group_id"), all.x=T)
  pops[age_group_years_end == 125, age_group_years_end := 99]
  split_dt <- rbindlist(lapply(1:nrow(dt), function(x) split_row(row_id = x, data_dt = dt, pop_dt = pops, cs = cs)))
  return(split_dt)
}
...
split_row <- function(row_id, data_dt, pop_dt, cs){
  ...
  pops_sub <- pop_dt[location_id %in% cs & year_id == row[, midyear] & sex_id == row[, sex] &
  ...
  agg <- agg[, .(location_id, proportion, merge)]
  ...
  split[, `:=` (lower = NA, upper = NA, cases = sample_size * mean, effective_sample_size = NA, uncertainty_type_value = NA, 
                note_modeler = paste0(note_modeler, " | location split using population weights: ", round(proportion, 2)), ihme_loc_id = NULL)]
...
split_map <- as.data.table(read.xlsx(paste0(repo_dir, "gbd2023/loc_splitting_map.xlsx")))
