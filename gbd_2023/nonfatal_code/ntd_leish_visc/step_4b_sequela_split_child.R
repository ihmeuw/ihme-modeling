...
if (loc_id %in% unique_vl_locations){
  ...
  if( loc_id %in% subnat_prop[, location_id ]){
    ...
    for(alpha in full_year_id){
      ...
      population_pull_sex1<-get_population(location_id=loc_id, year_id = alpha, age_group_id = full_age_set, sex_id=1, release_id = release_id)
      population_pull_sex2<-get_population(location_id=loc_id, year_id = alpha, age_group_id = full_age_set, sex_id=2, release_id = release_id)
      population_pull<-rbind(population_pull_sex1, population_pull_sex2)
      full_population_pull<-get_population(location_id=location_id, year_id=alpha, release_id = release_id)
      full_country_pull<-get_population(location_id=parent_id, year_id=alpha, release_id = release_id)
    ...
    for(alpha in full_year_id){
      ...
      population_pull_sex2<-get_population(location_id=loc_id, year_id = alpha, age_group_id = full_age_set, sex_id=2, release_id = release_id)
      population_pull<-rbind(population_pull_sex1, population_pull_sex2)
      full_population_pull<-get_population(location_id=loc_id, year_id=alpha, release_id = release_id)
      total_pop<-full_population_pull$population
      split_pop<-data.frame(population_pull$population)
