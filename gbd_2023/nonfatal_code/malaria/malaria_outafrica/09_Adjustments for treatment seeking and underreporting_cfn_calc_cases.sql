CREATE OR REPLACE FUNCTION rds.calc_cases_gbd2016(
  ...
  p_cases_calc_method TEXT DEFAULT 'LOWER_ESTIMATE',
...
BEGIN
  ...
  -- 'LOWER_ESTIMATE' cases estimate is the only one which requires the 'any treatment
  -- seeking' modifier
  IF p_cases_calc_method != 'LOWER_ESTIMATE'
  THEN
    p_any_ts = 1.0;
  END IF;
  ...
  ELSIF p_public_ts <= 0 OR
    ...
    THEN
      RAISE EXCEPTION
      'The ''public'' and ''any'' treatment seeking values must be >0 when using the ''LOWER_ESTIMATE'' cases formula: received public % and any %',
  ...
  ELSIF p_cases_calc_method = 'UPPER_ESTIMATE'
    THEN
