...
study_dems <- readRDS(paste0(data_root, "FILEPATH", release_id, ".rds"))
year_ids   <- study_dems$year_id
meids_directory     <- paste0(params_dir, "FILEPATH")

## Get locations to parallellize by
...
if (my_loc %in% restrict_loc){
  ...
  data <- gen_zero_draws(model_id= all_cases, location_id = NA, measure_id = 5, release_id = release_id, team = 'FILEPATH')
  data[, metric_id := 3]
...
if (!(my_loc %in% restrict_loc)){
  ...
  ## Grab age data
  ages <- get_demographics("ADDRESS", release_id = release_id)$age_group_id
