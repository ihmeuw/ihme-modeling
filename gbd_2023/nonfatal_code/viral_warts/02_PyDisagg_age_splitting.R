...
nrow(data[data$lower>data$mean, ])

data$lower[data$lower>data$mean]<-0.95*(data$mean[data$lower>data$mean])

nrow(data[data$upper<data$mean, ])
...
pop_df <- inner_join(pop_df, pattern %>% select(age_group_id, age_start, age_end) %>% distinct(), by="age_group_id")
