...
# -*- coding: utf-8 -*-
'''
Description: Calculates prevalence for the cancer nonfatal pipeline
...
def load_durations(acause):
    ...
    sq_df = pd.read_csv('{}/sequelae_durations.csv'.format(utils.get_path(process='nonfatal_model', 
                                                                        key='database_cache')))
...
def calc_mortality(surv_df, acause, location_id, faux_correct):
    ...
    for i in list(range(0,max_draws)):
        df['deaths_{}'.format(i)] = \
            pd.DataFrame(mrg_df['inc_{}'.format(i)] * mrg_df['incr_mort_{}'.format(i)])
...
def calc_increm_mort(surv_df, acause, location_id, faux_correct):
    ...
    def im_draw(df, draw_num, surv_uids, faux_correct):
        ...
        abs_surv_col = 'surv_abs_{}'.format(draw_num) #nd.get_columns("absolute_survival")
        increm_mort_col ='incr_mort_{}'.format(draw_num) # nd.get_columns("incremental_mortality") + '_{}'.format(draw_num)
...
def calc_prevalence(adjusted_sequelae_durations, mort_df, acause, faux_correct):
    ...
    decomp_str = decomp_prefix_cols(faux_correct)
    max_draws = nd.nonfatalDataset().num_draws
    prev_cols = nd.get_columns('{}prevalence'.format(decomp_str))
    mort_cols = nd.get_columns('{}mortality'.format(decomp_str))
...
def generate_estimates(acause, location_id, faux_correct=True, is_estimation_yrs=True):
    ...
    zero_rows =surv_df[surv_df.eq(0).any(axis=1)].index.tolist()
    zero_df = surv_df.iloc[surv_df.index.isin(zero_rows)]
    mort_df = calc_mortality(surv_df, acause, location_id, faux_correct)
    zero_rows =mort_df[mort_df.eq(0).any(axis=1)].index.tolist()
    zero_df = mort_df.iloc[mort_df.index.isin(zero_rows)]
    adjusted_sequelae_durations = load_sequela_framework(surv_df, acause)
    zero_rows =adjusted_sequelae_durations[adjusted_sequelae_durations.eq(0).any(axis=1)].index.tolist()
    zero_df = adjusted_sequelae_durations.iloc[adjusted_sequelae_durations.index.isin(zero_rows)]    
    prev_df = calc_prevalence(adjusted_sequelae_durations, mort_df, acause, faux_correct)
    zero_rows =prev_df[prev_df.eq(0).any(axis=1)].index.tolist()
    zero_df = prev_df.iloc[prev_df.index.isin(zero_rows)]    
    nd.save_outputs("prevalence", prev_df, acause, is_estimation_yrs=is_estimation_yrs)
