#! /ihme/code/cancer/miniconda3/envs/cancer_env/
...
from split_models import split_epi_model
...
def split_estimates(modelable_entity_id, cnf_model_version_id):
    ...
    # only have incidence for diagnosis phase 
...
def save_splits(modelable_entity_id, cnf_model_version_id):
    ...
    def output_file_func(id):
        if(type(id) == int):
            ...
            stripped_id = ''.join(map(str,id))
        return (nd.nonfatalDataset("split", stripped_id).get_output_file('upload'))
    parent_me = modelable_entity_id
    work_dir = get_work_dir(parent_me, cnf_model_version_id)
    ...
    job_dict = cluster_tools.create_jobs(script_path=save_worker,
                                         ...
                                         use_argparse=True,
                                         project_name="cancer",
                                         shell='gbd_py_shell')#utils.get_path(key='gbd_py_shell', process='common'))
    for i in job_dict:
