...
# check and exclude uids with age_start and age_end <=1 years, since dermatitis is age restricted to >1 years.
...
# dermatitis is age restricted to ages>1.
# Thus, our reference data, clinical from Poland, does not have age_start<1.
# so the pattern can not handle age_start<1. when age_start<1, we change the age_start to 1.
...
result_to_merge$lower[result_to_merge$lower<=0] <- half_lowest_non_zero_res_lower
