...
#prob_me <- 20190 # must be changed before running from master
#def_me <- 20191 # must be changed before running from master
#prob_me_v<- 798406 # must be changed before running from master
#def_me_v<- 798394 # must be changed before running from master
...
## READ IN SEVERITY DRAWS FROM .rds-FILE PRODUCED BY headache_severity_calc.R
...
for (a in 1:3) { # ITERATE THROUGH THE 3 AGE GROUPS
  for (t in c("def", "prob")){
    for (s in c("male", "female")) {
      # CREATE COLUMN NAME
      col_name <- paste0("timesym", t, "_mig", "_", s, "_", a)
