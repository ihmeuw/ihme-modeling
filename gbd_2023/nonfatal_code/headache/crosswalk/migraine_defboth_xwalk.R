...
crosswalk_from_matched <- function(dt, matched_dt, ref, alts, measure_name, out_dir){
  ...
  dt <- as.data.table(dt)
  n <- nrow(dt)
  dt$id <- seq(1,n,1) ## set unique identifier -- don't rely on seqs to be present & unique
  pre_data <- copy(dt)
...
write.xlsx(final, paste0(j_root, "WORK/12_bundle/neuro_migraine/", bundle_id, "/03_review/02_upload/definite_xwalk_", date, ".xlsx"), sheetName = "extraction")
upload_epi_data(bundle_id = bundle_id, filepath = paste0(j_root, "WORK/12_bundle/neuro_migraine/", bundle_id, "/03_review/02_upload/definite_xwalk_", date, ".xlsx"))
