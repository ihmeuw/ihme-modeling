...
for(y in years){
  for(s in sexes){
    for(g in groups){
      ...
      incid.dt <- incid.dt[, lapply(.SD, sum), by=.(measure_id, grouping, location_id, year_id, age_group_id, sex_id), .SDcols=paste0("draw_", 0:999)]
      
      # Recalculate custom age groups so that each row represents 1 year (e.g. age_group (2 - 4 years) gets 3 rows)
      ...
      incid.dt <- incid.dt[, lapply(.SD, mean), by=.(measure_id, grouping, location_id, year_id, age_group_id, sex_id), .SDcols=paste0("draw_", 0:999)]
