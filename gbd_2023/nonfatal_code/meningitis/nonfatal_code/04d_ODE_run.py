...
in_dir = args.in_dir + "FILEPATH"
in_dir_data = args.tmp_dir + "FILEPATH"
in_dir_rate = args.tmp_dir + "FILEPATH"
in_dir_value = args.tmp_dir + "FILEPATH"
out_dir_ODE = args.tmp_dir + "FILEPATH"
...
def write_checks(cause, outcome, etiology, location, dm_out_dir):
    ## create a finished.txt for check system 
    finished = []
    checks = os.path.join(out_dir_ODE,"prev_results", "checks")
    if not os.path.exists(checks):
        os.makedirs(checks)
    np.savetxt(os.path.join(checks,"finished_{}_{}_{}_{}.txt".format(cause, outcome, etiology, location)), finished)
...
def format_draws(filename,drawnum):
    """This takes the raw and formats it according 
...
def write_results(final, cause, outcome, etiology, out_dir_ODE, location):
    """Function to write the results that are appended from the DisMod ODE."""
    
    # define filepaths, clean up columns we need
    out_dir = os.path.join(out_dir_ODE, ("save_results_" + outcome + etiology), cause)
...
def main(in_dir, in_dir_data, in_dir_value, in_dir_rate, out_dir_ODE, cause, outcome, etiology, location):
    ...
    for year in year_ids:
        for sex in sex_ids: 
            # begin new data frame to save one cause-outcome-year-sex 
            
            print("Running DisMod ODE for {} {} {} {} {} {}".format(cause, etiology, outcome, location, year, sex))
            
            # define filepaths that already exist
            data_in = os.path.join(in_dir_data, str(location), "FILEPATH".format(
                cause = cause,
                etiology = etiology,
                outcome = outcome, 
                location = location,
                year = year, 
                sex = sex))
            value_in = os.path.join(in_dir_value, "FILEPATH".format(
                cause = cause,
                outcome = outcome,
                etiology = etiology))
            rate_in = os.path.join(in_dir_rate, str(location), str(year), str(sex), "FILEPATH".format(
                location = location,
                year = year,
                sex = sex,
                outcome = outcome))
            draw_in = os.path.join(in_dir, "FILEPATH")
            plain_in = os.path.join(in_dir, "FILEPATH")
            #different effect_in files because epilepsy has remission
            if outcome == "long_modsev":
                effect_in = os.path.join(in_dir, "FILEPATH")
            if outcome == "epilepsy":
                effect_in = os.path.join(in_dir, "FILEPATH")
            
            # define filepaths that will be created here
            sample_out_dir = os.path.join(out_dir_ODE, "FILEPATH".format(cause, outcome, etiology, str(location), str(year), str(sex)))
            sample_out = os.path.join(sample_out_dir, "FILEPATH")
            os.makedirs(sample_out_dir, exist_ok = True)
                
            draw_out_dir = os.path.join(out_dir_ODE, "FILEPATH", cause, outcome, etiology, str(location), str(year), str(sex))
            draw_out = os.path.join(draw_out_dir, "FILEPATH".format(cause, outcome, etiology, str(location), str(year), str(sex)))
            ...
            if drop_temp:
                ...
                temp_dir = os.path.join(out_dir_ODE, "FILEPATH".format(cause, outcome, etiology, str(location), str(year), str(sex)))
            ...
            write_results(final, cause, outcome, etiology, out_dir_ODE, location)

            #write checks file
            write_checks(cause, outcome, etiology, location, out_dir_ODE)

if __name__ == '__main__':
    for outcome in outcomes:
        main(in_dir, in_dir_data, in_dir_value, in_dir_rate, out_dir_ODE, cause, outcome, etiology, location)
