...
ulcerative_colitis_adj <- CUSTOM_MODEL_ID
ulcerative_colitis_unadj <- PARENT_MODEL_ID


##GENERATE 1000 DRAWS OF SCALAR AND CREATE A VECTOR
##values 1+mean/1+lower/1+upper results from Indeterminantibd_scalar.R
mean <- 1.0624 
lower <- 1.0549
upper <- 1.0699
...
unadj_draws <- get_draws(gbd_id_type="modelable_entity_id", 
                         gbd_id=ulcerative_colitis_unadj,
                         source="epi", 
                         status="best",
...
adj_draws[,"modelable_entity_id"] <- ulcerative_colitis_adj
