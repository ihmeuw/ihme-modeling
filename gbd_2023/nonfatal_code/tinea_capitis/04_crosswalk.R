...
df_orig <- df_orig %>% mutate(obs_method = ifelse(cv_not_dr_diagnosis == 1, "not_dr_diagnosis", "dr_diagnosis"))
...
df_matched$altvar <- "not_dr_diagnosis"
df_matched$refvar <- "dr_diagnosis"
...
fit_not_dr_diagnosis <- cw$CWModel(
  ...
  gold_dorm = "dr_diagnosis"   # the level of `alt_dorms` that indicates it's the gold standard
)

fit_not_dr_diagnosis$fit(
  inlier_pct=0.9 # trim 10%
)

df_tmp <- fit_not_dr_diagnosis$create_result_df()

preds_not_dr_diagnosis <- fit_not_dr_diagnosis$adjust_orig_vals( #no need to put data into logit space beforehand!!
  df = df_orig,
...
df_result <- df_orig
df_result$mean_adj <- preds_not_dr_diagnosis$ref_vals_mean
df_result$standard_error_adj <- preds_not_dr_diagnosis$ref_vals_sd
...
df_result <- df_result %>%
  mutate(
    crosswalk_parent_seq = case_when(
      obs_method == "not_dr_diagnosis" & is.na(crosswalk_parent_seq) ~ seq,
      TRUE ~ crosswalk_parent_seq
    ),
    seq = case_when(
      obs_method == "not_dr_diagnosis" & is.na(crosswalk_parent_seq) ~ NA_real_,
      TRUE ~ seq
    )
  )
...
description <- "GBD2023- selected dermatologist diagnisis as reference"
