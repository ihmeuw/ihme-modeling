...
# check and exclude instances when age_start<5 and age_end >9 years, since we should maintain pattern in ref_df.
data_to_split <- data_to_split %>% filter(!(age_start<5 & age_end<9))


# Our reference data, clinical from Poland, does not have age_start<5.
# so the pattern can not handle age_start<5. when age_start<5, we change the age_start to 5.
data_to_split <- data_to_split %>% mutate(age_start = if_else(age_start <5, 5, age_start))
...
# Step 1: Calculate a value to replace 'lower'
...
highest_non_1_upr <- max(result_to_merge$upper[result_to_merge$upper < 1])
new_upper <- 1- (1-highest_non_1_upr)/5
result_to_merge <- result_to_merge %>%
...
# adding uid column to track data:
age_splitted_data$uid <- seq_len(nrow(age_splitted_data))
c('uid') %in% names(age_splitted_data)

#saving the age_splitted_data to csv:
write.csv(age_splitted_data, paste0("FILEPATH"), row.names = FALSE)
write.csv(age_splitted_data,paste0("FILEPATH"))