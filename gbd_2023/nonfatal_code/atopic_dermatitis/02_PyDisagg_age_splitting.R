...
# These rows of data has their age_start and age_end less than 1. Atopic dermatitis is age restricted to ages>1, so we exclude these (also,we dont have pattern for them):
uid_exclude <- c(data_to_split$uid[data_to_split$age_start<1 & data_to_split$age_end<=1])

data_to_split <- data_to_split %>% filter(!uid %in% uid_exclude)

# The atopic dermatitis is age restricted to ages>1. Thus our reference data, clinical from Poland, does not have age_start<1. so the pattern can not handle age_start<1.
# when age_start<1, we change the age_start to 1.
data_to_split <- data_to_split %>% mutate(age_start = if_else(age_start <1, 1, age_start))
...
# check for instances of standard_error>1, and replace it with 1:
...
## replacing values in min(lower) and max(upper), since not in range. 
...
check_se <- age_specific_data %>% filter(standard_error>1)
