...
# Map England UTLA locations to GBD2022 locations
map <- as.data.table(read_xlsx('FILEPATH'))

## check if any UTLA exists in the bundle:
any(map$location_id %in% bv$location_id)

bv <- bv %>% select(-location_name_gbd2022) %>%
  left_join(map, by = c("location_id" = "location_id", "location_name" = "location_name")) %>%
  mutate(
    location_id = coalesce(location_id_gbd2022, location_id),
    location_name = coalesce(location_name_gbd2022, location_name)
  ) %>%
  select(-location_id_gbd2022, -location_name_gbd2022)
...
bv <- bv %>%
  mutate(
    location_name = case_when(
      location_name %in% c("Sweden except Stockholm", "Stockholm") ~ "Sweden",
      TRUE ~ location_name
    ),
    location_id = case_when(
      location_id %in% c(4940, 4944) ~ 93,
      TRUE ~ location_id
    )
  )
...
half_lowest_non_zero_mean <- min(bv$mean[bv$mean > 0]) / 2
...
half_lowest_non_zero_lwr <- min(bv$lower[bv$lower > 0]) / 2
