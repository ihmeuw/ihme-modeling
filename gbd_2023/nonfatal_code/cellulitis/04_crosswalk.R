...
half_lowest_non_zero_lower_adj <- min(df_result$lower_adj[df_result$lower_adj > 0]) / 20
...
df_result <- df_result %>% select(-c(lower, upper, mean, standard_error)) %>% dplyr::rename(lower = lower_adj,
                                                                                            upper = upper_adj,
                                                                                            mean = mean_adj,
                                                                                            standard_error =
                                                                                              standard_error_adj)
...
description <- "GBD2023-dropping data based on locations,measures & source type"
