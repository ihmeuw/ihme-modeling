...
print(paste0("Spliting CSMR for location ", location))
ages <- c(2,3, 6:20, 30, 31, 32, 235, 34, 238, 388, 389 , 28) 
years <- c(seq(1990,2015,5),2019, 2020, 2021,2022, 2023) 
sexes <- c(1,2)

####Load population file

#Population from same version used to produce CSMR, saved in parent script. 
pop <- readRDS(paste0("FILEPATH/pop_",date,".rds"))

############ step 2: Calculate {subtype} proportion within all stroke global deaths  ###########
...
pop_global_under_one <- data.frame(get_population(age_group_id=c(2,3,388,389,28), location_id=1, year_id=years, sex_id=c(1,2),
                                                  status="best", gbd_round_id=8, release_id=16))
...
all_df_global$cause_id[which(all_df_global$modelable_entity_id== 3952 | all_df_global$modelable_entity_id==1832)]   <- 495 
all_df_global$cause_id[which(all_df_global$modelable_entity_id== 3953 | all_df_global$modelable_entity_id==1844)]   <- 496 
all_df_global$cause_id[which(all_df_global$modelable_entity_id== 18730 | all_df_global$modelable_entity_id==18732)] <- 497 

## compute ratio of acute to chronic stroke for isch,ich,sah per year - per age group - per sex - 
all_df_props   <- expand.grid(unique(all_df_global$age_group_id),unique(all_df_global$location_id),unique(all_df_global$sex_id),unique(all_df_global$year_id), unique(all_df_global$cause_id))
...
for(i in 1:nrow(all_df_props)){
  ...
  if(18730 %in% temp$modelable_entity_id){ #if subarachnoid
...
saveRDS(all_df_props,
        file = paste0("FILEPATH/all_df_props_",date,".rds"))

############ step 3: Get CSMR for strokes for recent cod-correct run ###########
...
cod.draws <- data.frame(get_draws("cause_id", cause_ids, location_id=location, year_id=years, source="codcorrect",
                                  age_group_id=ages , measure_id=1, release_id=16, version_id = 422)) 

#Used CoDCorrect v422 for GBD2023
#for GBD 2021 we used version 338 
...
chronic_rates <- data.frame(matrix(NA,nrow = nrow(all_csmr), ncol=draw)) 
