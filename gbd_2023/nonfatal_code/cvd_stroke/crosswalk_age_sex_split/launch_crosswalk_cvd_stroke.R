...
if(length(args)==0){
  ...
  ######################################################
  acause <- 'cvd_stroke_subhem' ## STROKE SUBTYPE
  bundle_id <- 7100 ## BUNDLE_ID
  xw_measure <- 'mtexcess'## MEASURE TO CROSSWALK
  xw_index <- 3 # CROSSWALK INDEX #as.integer(readline(prompt="XW index plz: "))
  ######################################################
} else {
...
## chronic ischemic stroke - mtexcess
if(grepl('cvd_stroke_isch',acause) & xw_measure=='mtexcess' & bundle_id %in% c(786,787)){
  
  data <- data[measure != 'cfr']
  mtwith_nids <- c(103487, 151111, 103470, 103473, 151726, 103507,
                   151728, 151730, 103527, 450487, 450499, 450517, 450523, 450602, 450604, 450610)
  data[nid %in% mtwith_nids, measure:='mtexcess'] ## set this to mtexcess temporarily to sex split with the mtexcess datapoints
}

## chronic intracerebral hemorrhage - mtexcess
if(grepl('cvd_stroke_cerhem',acause) & xw_measure=='mtexcess' & bundle_id %in% c(784,785)){
  
  data <- data[measure != 'cfr']
  data[measure %in% c('mtexcess','mtwith'), measure:='mtexcess'] ## set this to mtexcess temporarily to sex split with the mtexcess datapoints
}

## chronic subarachnoid hemorrhage - mtexcess
if(grepl('cvd_stroke_subhem',acause) & xw_measure=='mtexcess' & bundle_id %in% c(3002,3005)){
...
## ischemic stroke incidence, us_claims alt defs. 
if(grepl('cvd_stroke_isch',acause) & xw_measure=='incidence'){
  

  data <- data[nid != 330721] ## duplicate
  data <- data[nid != 353873] ## duplicate of Mashad stroke study
  data <- data[nid != 331084] ## Iran hospital data implausibly low
  data <- data[nid != 333283] ## Dijon stroke registry duplicate
  data <- data[nid != 337619] ## China hospital data
  
    
  us_claims_nids <- c(471277,471267,471258,470991,470990, ##cms
                      244370,244371, 336847, 336848, 336849, 336850,408680,433114,494351, 494352) ##marketscan
  data[nid %in% us_claims_nids, `:=` (cv_inpatient = 0, cv_usclaims = 1)]
    
  
}

## intracerebral hemorrhage incidence, us_claims alt defs. 
if(grepl('cvd_stroke_cerhem',acause) & xw_measure=='incidence'){

  message('PROCESSING ICH')
  data <- data[nid != 103531] ## duplicate.
  data <- data[nid != 327801] ## duplicate
  data <- data[nid != 333283] ## Dijon stroke registry duplicate
  data <- data[nid != 353873] ## duplicate in Mashad
  data <- data[nid != 331084] ## Iran hospital data implausibly low
  data <- data[nid != 337619] ## China hospital data
  data[age_end > 99, age_end :=99] ## set inpatient data age_end to 99. 
...
## subarachnoid hemorrhage incidence, us_claims alt defs. 
if(grepl('cvd_stroke_subhem',acause) & xw_measure=='incidence'){
