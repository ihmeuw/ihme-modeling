################################################################################################################
################################################################################################################
## Purpose:      CROSSWALKING Appendicitis
################################################################################################################
################################################################################################################
...
active_data = get_bundle_data(OBJECT)
# only run below code once after adding the Dutch GP data to the bundle
# ADD DUTCH GP DATA  ---------------------------------------------------------------------------
dutch <- read.csv(paste0("FILEPATH"))
table(dutch$code_name)
dutch$age_end <- dutch$age_end-1
dutch <- subset(dutch, code_name == "Appendicitis" & (measure == "prevalence" | measure == "incidence"))
dutch$nid <- OBJECT 
dutch$year_start <- dutch$year
dutch$year_end <- dutch$year
dutch$year <- NULL
dutch$seq <- NA
dutch$is_outlier <- 0
dutch$underlying_nid <- NA
dutch$location_id <- OBJECT
dutch$mean <- NA
dutch$source_type <- "Facility - other/unknown"
dutch$sampling_type <- NA
dutch$representative_name <- "Nationally representative only"
dutch$urbanicity_type <- "Mixed/both"
dutch$recall_type <- "Not Set"
dutch$recall_type_value <- NA
dutch$unit_type <- "Person"
dutch$uncertainty_type <- NA
dutch$input_type <- NA
#dutch$measure <- "prevalence"
dutch$mean <- NA
dutch$upper <- NA
dutch$lower<- NA
dutch$standard_error<- NA
dutch$sex[dutch$sex== 1] <- "Male"
dutch$sex[dutch$sex== 2] <- "Female"
dutch$design_effect <- NA
dutch$unit_value_as_published <- 1
dutch$uncertainty_type_value <- NA
dutch$effective_sample_size <- NA
dutch$code_name <- NULL
dutch$age_group_id <- NULL
dutch$cause_code <- NULL
path <- paste0("FILEPATH")
write.xlsx(dutch, path, sheetName="extraction",  rowNames=FALSE)
...
df <- subset(df_all, clinical_data_type != "" | nid %in% c(416752, 552339)) 
...
df_zero <- subset(df, mean ==0 | nid == 416752) # keep mtexcess in df_zero
...
df_nonzero <- subset(df, mean >0 & mean <1 & nid!=416752) #drop all rows of data with mean greater than 1 and mtexcess
...
#original data
df_nonzero_ms2000 <- subset(df_nonzero_65below, cv_ms2000 ==1)
df_nonzero_ms <- subset(df_nonzero_65below, cv_marketscan ==1)
df_nonzero_ref <- subset(df_nonzero_65below, cv_hospital ==1)
...
df_matched_ms <- df_matched_ms[abs(df_matched_ms$mid_year_ref - df_matched_ms$mid_year_alt) <= year_range_ms, ] 

#7. Optional: check the matches
check1 <- df_matched_ms2000 %>% select(location_name, age_start, age_end, nid_ref, mid_year_ref, prev_ref, dorm_ref, mean_logit_ref, se_logit_ref, nid_alt, mid_year_alt, prev_alt, dorm_alt, mean_logit_alt, se_logit_alt)
...
dat1_ms2000 <- cw$CWData(df = df_matched_ms2000, 
                         ...
                         covs = list("age", "sex_id"),               #list of (potential) covariate columns 
                         study_id = "id" )
...
plots$funnel_plot(
  ...
  plot_note = paste0("Funnel plot_appendicitis_", obs_method, "incidence"), 
  plots_dir = paste0("FILEPATH"), 
  file_name = paste0("Funnel_plot_", obs_method,"_incidence_0.1trim", date),
  write_file = TRUE
)
...
pdf(paste0("FILEPATH"),  width=20, height=8) 
...
df_nonzero_ms2000[, `:=` (upper=NA, lower=NA, uncertainty_type_value = NA, effective_sample_size = NA, note_modeler ="crosswalked")]
df_nonzero_ms[, `:=` (upper=NA, lower=NA, uncertainty_type_value = NA, effective_sample_size = NA, note_modeler = "crosswalked")]
...
mad_filepath <- paste0(save_outdir, "FILEPATH_", date, ".xlsx")
