...
# SOURCE
source("/REDACTED/get_ids.R")
source("/REDACTED/get_location_metadata.R")
source("/REDACTED/get_age_metadata.R")
source("/REDACTED/get_population.R")
source('/REDACTED/save_bundle_version.R')
source('/REDACTED/get_bundle_version.R')
source('/REDACTED/save_crosswalk_version.R')
source('/REDACTED/get_crosswalk_version.R')
source("/REDACTED/get_bundle_data.R")
...
graph_predictions <- function(dt) {
  ...
  graph_dt_means[ , variable := ifelse(variable == "f_mean", "Female", "Male")]
  ...
  graph_dt_error[ , variable := ifelse(variable == "f_standard_error", "Female", "Male")]
  setnames(graph_dt_error, "value", "error")
  
  value <- graph_dt_means[, .(value)]
  error <- graph_dt_error[, .(error)]
  columns <- graph_dt_means[, .(age_start, age_end, mean, variable)]
  
  graph_dt <- cbind(columns, value, error)
...
save_crosswalk_pkl <- function(fit, dir) {
  ...
  ## to read fit back in, do the below
...
rm_zeros <- function(dt, 
  ...
  if (offset) {
    ...
    if (nrow(dt[mean==0])>0) {
      ...
      dt[mean==0, `:=`(mean = off)] #, note_modeler= paste(note_modeler, "offset applied to this row's 0 value on mrbrt input."))
    }
    if (fix_ones){
      if (nrow(dt[mean==1])>0){
        ...
        dt[mean==1, `:=` (mean = 1-off)] #, note_modeler= paste(note_modeler, "offset applied to this row's 1 value on mrbrt input.")
      }
    }
  }
...
find_sex_match <- function(dt,
  ...
  test <- dcast.data.table(sex_dt, ...~ sex, value.var = c("mean", "standard_error"))
  sex_dt <- dcast.data.table(sex_dt, ...~ sex, value.var = c("mean", "standard_error"))
...
log_transform_mrbrt <- function(df) {
  ...
  df[, c("log_mean_alt", "se_log_mean_alt") := data.table(crosswalk002::delta_transform(mean = mean_alt, sd = alt_se, transformation = "linear_to_log"))]
  df[, c("log_mean_ref", "se_log_mean_ref") := data.table(crosswalk002::delta_transform(mean = mean_ref, sd = ref_se, transformation = "linear_to_log"))]
...
logit_transform_mrbrt <- function(df) {
  ...
  df[, c("temp_logit_alt", "se_logit_mean_alt") := data.table(crosswalk002::delta_transform(mean = mean_alt, sd = alt_se, transformation="linear_to_logit"))]
  df[, c("temp_logit_ref", "se_logit_mean_ref") := data.table(crosswalk002::delta_transform(mean = mean_ref, sd = ref_se, transformation="linear_to_logit"))]
...
run_sex_split <- function(dt, 
  ...
  sex_matches[, id_var := .GRP, by=.(nid)] # This really assumes we're using "nid" as the id_vars
  ...
  ## CW functions can only access global variables so I'm assigning these to be global variables
  ...
  ## Generate covariate list - inherited from SEB, not currently implementing any covariates
  ...
  if(transformation=="log"){
    both_sex[, c("linear_pred_mean", "linear_pred_se") := data.table(crosswalk002::delta_transform(mean = pred_diff_mean, sd = se_val, transformation="log_to_linear"))]
  
  } else if (transformation=="logit"){
    both_sex[, c("linear_pred_mean", "linear_pred_se") := data.table(crosswalk002::delta_transform(mean = pred_diff_mean, sd = se_val, transformation="logit_to_linear"))]
  ...
  both_sex <- add_population_cols(both_sex, release_id = release_id)
  ...
  both_sex[, m_standard_error := sqrt((linear_pred_se^2 * standard_error^2) + (linear_pred_se^2 * mean^2) + (standard_error^2 * linear_pred_mean^2))]
 ...
 # Correct values over 1
  ...
  #assign parent and clear seq so that we don't have duplicate seq columns -- per save_crosswalk_version documentation, split rows should have NA for seq and old seq as croswalk parent seq
