...
#These configurators build a framework for what your result should look like/include
...
max(data$mean)
max(data$upper)
min(data$standard_error)
max(data$standard_error)
data %>% filter(standard_error>1)
any(is.na(data$mean))
any(is.na(data$upper))
any(is.na(data$lower))
any(is.na(data$standard_error))
...
pattern <- ref_df
#load in draws_df
draws_df <- read.csv(paste0("FILEPATH"))
...
min(result_to_merge$mean)
max(result_to_merge$mean)
# check interval of standard_error:
min(result_to_merge$standard_error)
max(result_to_merge$standard_error)

# max(standard_error)=0.9799, since it could not be>1, we check for instances of this occurance:
result_to_merge %>% filter(standard_error>1)
# there is just one instance, so we replace it with 1:
...
# check if lower,upeer,mean in (0,1) range:
min(result_to_merge$lower)
max(result_to_merge$lower)
min(result_to_merge$upper)
max(result_to_merge$upper)
min(result_to_merge$mean)
max(result_to_merge$mean)


## replacing values in min(lower) and max(upper), since not in rage. as suggested in slide 19 of https://rpubs.com/rsoren/crosswalk_training_aug2023
...
# check if there is any men,lower, upper value less than 0 after splitting and handle that
min(age_specific_data$lower)
min(age_specific_data$mean)
min(age_specific_data$upper)
max(age_specific_data$lower)
max(age_specific_data$mean)
max(age_specific_data$upper)
max(age_specific_data$standard_error)
age_specific_data %>% filter(standard_error>1)

#saving the age_splitted_data to csv:
write.csv(age_splitted_data, paste0("FILEPATH"), row.names = FALSE)
write.csv(age_splitted_data,paste0("FILEPATH"), row.names = FALSE)
