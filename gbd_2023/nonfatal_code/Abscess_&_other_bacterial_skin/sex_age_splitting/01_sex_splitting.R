...
# Map England UTLA locations to GBD2023 locations
map <- as.data.table(read_xlsx('FILEPATH'))

## check if any UTLA exists in the bundle:
any(map$location_id %in% bv$location_id)


# Map Sweden sub-nationals to Sweden
## check if Sweden sub-nationals exist in the bv:
any(c(4940, 4944) %in% bv$location_id)
...
# check bv for less than zero or more than 1 values:
min(bv$lower)
max(bv$lower)
min(bv$mean)
max(bv$mean)
min(bv$upper)
max(bv$upper)
max(bv$standard_error)
bv %>% filter(mean>upper)

# replacing zero values in mean and lower as suggested in slide 19 ofhttps://rpubs.com/rsoren/crosswalk_training_aug2023
## sex_plitting function cannot handle if not(0<mean,upper,lower<=1)

# Step 1: Calculate one fifth of the lowest non-zero value in 'mean'
...
# Step 1: Calculate one fifth of the lowest non-zero value in 'lower'
...
#load in draws_df
draws_df <- read.csv(paste0(modeling_dir,"FILEPATH"))
...
# Correction 1: Make sure that the mean < 1
highest_non_zero_mean <- max(result_to_merge$mean[result_to_merge$mean < 1])
#replacing mean=>1 with mean=>1- (1-highest_non_zero_mean)/2
new_mean <- 1- (1-highest_non_zero_mean)/2
...
#check values:
min(sex_splitted_bv$lower)
min(sex_splitted_bv$mean)
max(sex_splitted_bv$mean)
max(sex_splitted_bv$upper)
min(sex_splitted_bv$standard_error)
sum(sex_splitted_bv$standard_error==0)
max(sex_splitted_bv$standard_error)
sum(sex_splitted_bv$standard_error>1)
### Handling invalid intervals #################################################
# Correction 1: Make sure that the 0 < lower
half_lowest_non_zero_lwr <- min(sex_splitted_bv$lower[sex_splitted_bv$lower > 0]) / 2
sex_splitted_bv <- sex_splitted_bv %>%
  mutate(lower = ifelse(lower <= 0, half_lowest_non_zero_lwr, lower))

# Correction 2: Replacing standard_error==0:
half_lowest_non_zero_se <- min(sex_splitted_bv$standard_error[sex_splitted_bv$standard_error > 0]) / 2
sex_splitted_bv <- sex_splitted_bv %>%
...
# Correction 4: mean,upper<=1
# Step 1: Calculate  the max value in 'mean'
max_mean <- max(sex_splitted_bv$mean[sex_splitted_bv$mean <=1])
# Step 2: Replace greater than 1 with this value in 'max_mean'
sex_splitted_bv <- sex_splitted_bv %>%
  mutate(mean = ifelse(mean >=1 , max_mean, mean))


# Step 1: Calculate  the max value in 'upper'
max_upper <- max(sex_splitted_bv$upper[sex_splitted_bv$upper <=1])
