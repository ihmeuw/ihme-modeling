####################################################
## Description: Crosswalk Data Processing - OTHER MSK
####################################################
...
# LOADING DATA
other <- as.data.table(get_bundle_version(8252, export = F, transform = T))
other <- other[clinical_data_type != "inpatient"]
other[field_citation_value %like% "Truven Health" & year_start == 2000, cv_marketscan_all_2000 := 1]
other[field_citation_value %like% "Truven Health" & year_start != 2000, cv_marketscan := 1]
other[field_citation_value %like% "Taiwan National Health Insurance", cv_taiwan_claims_data := 1]
other[, "crosswalk_parent_seq" := as.numeric(NA)]

other <- other[is_outlier != 1]
other <- other[is.na(drop)]

## FILL OUT MEAN/CASES/SAMPLE SIZE
## CALCULATE STD ERROR BASED ON UPLOADER FORMULAS

other$cases <- as.numeric(other$cases)
other$sample_size <- as.numeric(other$sample_size)
other <- get_cases_sample_size(other)
other <- get_se(other)

orig <- as.data.table(copy(other))

# SEX SPLIT

test <- find_sex_match(other)
test2 <- calc_sex_ratios(test)

model_name <- paste0("other_sexsplit_", date)
...
other <- as.data.table(copy(test)) 
orig <- as.data.table(copy(other))

claims_data <- other[clinical_data_type == "claims"]
other <- other[clinical_data_type != "claims"]
claims_data <- aggregate_marketscan(claims_data)
claims_data <- claims_data[location_name == "United States" | location_name == "Taiwan"]
other <- rbind(other, claims_data)
other <- get_cases_sample_size(other)
other <- get_se(other)

other <- other[, c("nid", "sex", "year_start", "year_end", "age_start", "age_end", "location_id", "location_name", "ihme_loc_id", "country", "region_name", "super_region_name", "measure", "mean", "lower", "upper", "standard_error", "case_def")]

# CREATE CD MATCHES BY DEMOGRPHICS
other[, demographics := paste2(sex, country, measure, sep = "_")]

other2 <- copy(other)

other2 <- other2[!is.na(case_def)]
other2 <- other2[!is.na(standard_error)]

other_cds <- list(unique(other2$case_def))

other2[, year_mean := (year_start + year_end)/2]
other2[, age_mean := (age_start + age_end)/2]

other2$case_def <- as.factor(other2$case_def)
levels(other2$case_def)
other2$case_def <- factor(other2$case_def, levels(other2$case_def)[c(3, 1:2)])
other2 <- other2[order(case_def)]

list2 <- lapply(unique(other2$case_def), function(i) {
  subset(other2, case_def == i)
})
...
other_matched <- copy(paired_reference_marketscan)
other_matched <- other_matched[0,]

for (i in 1:length(pairss)) {
  other_matched <- rbind(other_matched, get(pairss[i]))
}

nrow(other_matched[nid.denom == nid.num])
nrow(other_matched[nid.denom != nid.num])

other_matched <- other_matched[abs(year_mean.denom - year_mean.num) < 6]
other_matched <- other_matched[abs(age_start.denom - age_start.num) < 6 & abs(age_end.denom - age_end.num) <6]

nrow(other_matched[nid.denom == nid.num])
nrow(other_matched[nid.denom != nid.num])

unique(other_matched$case_def.num)
unique(other_matched$case_def.denom)
remaining_cd <- unique(c(unique(as.character(other_matched$case_def.num)), unique(as.character(other_matched$case_def.denom))))
...
other_matched <- other_matched %>%
  mutate(
    ...
    ref_se = standard_error.denom, 
    alt_se = standard_error.num, 
    ratio_se =
      sqrt((mean.num^2 / mean.denom^2) * (alt_se^2/mean.num^2 + ref_se^2/mean.denom^2))
  )

other_matched <- as.data.table(other_matched)

other_matched2 <- copy(other_matched)
other_matched2[, id := paste0(nid.num, " (", country.num, ": ", sex.num, " ", age_start.num, "-", age_end.num, ") - ", nid.denom, " (", country.denom, ": ", sex.denom, " ", age_start.denom, "-", age_end.denom, ")")]
other_matched2[, id_var := paste0(nid.num, ":", nid.denom)]

# WRITING FILES

write.xlsx(other_matched2, "FILEPATH")
...
model_name <- "crosswalk_network_other"
...
test <- ggplot(plotting_data[measure == "prevalence"], aes(x = mean_old, y = mean, color = case_def)) +
  geom_point() +
  theme_bw() +
  labs(x = "Pre-crosswalk", y = "Post-crosswalk", title = "Other MSK, Pre- and Post- Adjustment", color = "Case Definition") +
...
model_name <- "crosswalk_network_other"
final_data <- as.data.table(read.xlsx("FILEPATH"))


# SUBMIT CROSSWALK VERSION TO AGE SPLIT ME
age_split_data <- final_data[sex != "Both"]
age_split_data <- age_split_data[group_review != 0 | is.na(group_review)]
age_split_data[is.na(upper), `:=` (lower = NA, uncertainty_type_value = NA)]
age_split_data[!is.na(upper), uncertainty_type_value := 95]
age_split_data[is.na(upper), upper := NaN]
age_split_data <- age_split_data[location_id != 95]
age_split_data <- age_split_data[(age_end - age_start) < 26]

## SAVE XWALK VERSION
write.xlsx(age_split_data, ""FILEPATH"", sheetName = "extraction")

save_crosswalk_version(8255, "FILEPATH", description = "crosswalked, sex-specific data for age split model")
...
save_crosswalk_version(step2_version, "FILEPATH", description = "sex/age-sex/age split, no MS xwalk")
