...
##  Replace the function below with the appropriate method for data retrieval as per project requirements.
...
split_both_sex <- function(full_dt, sex_results) {
  ...
  pop <- get_population(age_group_id = "all", sex_id = "all", year_id = unique(floor(adjust_both$year_mid)),
                        location_id=unique(adjust_both$location_id), single_year_age = T,release_id = "OBJECT") #will need to be updated to release_id for GBD 2022; as of 12/28/2023 no new GBD 2022 version ready that has single_year_age in it
...
subnat_to_nat <- function(subnat_dt, subnat){
  ...
  if (subnat == T) {
    ...
    dt[level == 6, loc_match := "OBJECT"] ## PAIR UTLAS TO ENGLAND
    dt[level == 5 & grepl("GBR", ihme_loc_id), loc_match := "OBJECT"] ## PAIR ENGLAND REGIONS TO ENGLAND
    dt[level == 5 & location_type %in% c("urbanicity", "admin2"), loc_match := "OBJECT"] ## PAIR INDIA URBAN/RURAL TO INDIA
...
prep_data_matches <- function(raw_dt) {
  ...
  drop_nids3 <- 439918 # Temporary fix: # Temporary Fix: Manually removing the problematic column due to its unintended inclusion -a more permanent solution will involve updating the source data structure.
