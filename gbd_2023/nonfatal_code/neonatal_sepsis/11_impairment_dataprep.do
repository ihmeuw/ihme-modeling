...
use "FILEPATH/qx_results_v2020_loc_set_9_iterative.dta", clear
...
**		- all years 1950-2022 per location
**		- ln_NMR per location-year-sex
**		- developing status of location
********************

get_location_metadata, location_set_id(9) gbd_round_id(7) clear
		...
		********************
		...
		local x = 0
			...
			if `bundle_id' != 353 & `bundle_id' != 352 & `bundle_id' != 351 & `bundle_id' != 90   {
				...
				get_bundle_data, bundle_id(`bundle_id') decomp_step("iterative") clear
		...
		foreach standard_grouping of local standard_grouping_list {
			...
			** Passed: `acause' `grouping' `data_dir' `prepped_dta_dir' `timestamp' 
			if "`covariates'" == "meta"{
				...
				!qsub -N "meta_`acause'_`grouping'" -P PROJECT -l fthread=8 -l m_mem_free=2G -l h_rt=01:00:00 -l archive -q all.q -e FILEPATH -o FILEPATH -cwd "FILEPATH/stata_shell.sh" "FILEPATH/02a_meta_analysis.do" "`acause' `grouping' FILEPATH `prepped_dta_dir' `timestamp'" 
			}
