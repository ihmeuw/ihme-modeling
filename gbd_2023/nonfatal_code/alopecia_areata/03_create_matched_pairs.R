...
df_orig$is_outlier[df_orig$clinical_version_id==5 & df_orig$location_id %ni% c(poland_locs, marketscan_locs) ]<- 1
...
#For male drop Netherland and Japan data and rerun
unique(df_orig$sex)
c("Japan", "Netherlands") %in% df_orig$location_name
df_orig$is_outlier[df_orig$sex=="Male" & df_orig$location_name %in% c("Japan", "Netherlands")]<-1
