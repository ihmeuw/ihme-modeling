...
## Purpose:      CROSSWALKING Vascular Intestinal Disorder IN GBD 2023 USING NEW CLINICAL INFORMATICS DATA
################################################################################################################
################################################################################################################
...
invisible(sapply(list.files("FILEPWTH/", full.names = T), source))
...
#bundle_df_new <- df 
#cols_to_udpate <- setdiff(names(bundle_df_new), "seq")
#bundle_df_new[, cols_to_udpate] <- NA

#bundle_data_filepath <- paste0(save_outdir, "GBD2023_refresh3_bundle_data_", date, ".xlsx")
#write.xlsx(bundle_df_new, bundle_data_filepath, sheetName = "extraction")
#path_to_data <-  bundle_data_filepath
#result <- upload_bundle_data(6938, filepath=path_to_data)
#print(sprintf('Request status: %s', result$request_status))
#print(sprintf('Request ID: %s', result$request_id))   
#check
#bundle_df <- get_bundle_data(bundle_id = 6938)


#save clinical_version_id = 3
#result <- save_bundle_version(bundle_id, include_clinical=c("claims", "inpatient")) #uncomment this when have new fresh
#print(sprintf('Bundle version ID: %s', result$bundle_version_id))
step2_bundle_version <- 43486 #GBD2023 bundle_version_id (clinical_version_id = 3)
...
#Note that the delta_transform()function has be deprecated. Use cw$utils$linear_to_log() and cw$utils$linear_to_logit() instead
...
#6 between-study matches based on 5-year span for ms2000 and ms
...
df_nonCI <- subset(df_all, measure!=measures)
...
if (fine_age_groups=="yes") {
  ...
  all_fine_ages <- as.data.table(get_age_metadata(age_group_set_id=24, release_id = 16)) ##For GBD 2023, works for all GBD rounds/release
...
new_xwalk[as_mean == 0, note_modeler := paste0(note_modeler, " | GBD 2022 refresh3, outliered this location-year-sex-NID age-series because age standardized mean is 0")]
...
new_xwalk[as_mean>((MAD*mad)+median), note_modeler := paste0(note_modeler, " | GBD 2023 refresh, outliered because log age-standardized mean for location-year-sex-NID is higher than ", MAD, " MAD above median")]
new_xwalk[as_mean<(median-(MAD*mad)), is_outlier := 1]
new_xwalk[as_mean<(median-(MAD*mad)), note_modeler := paste0(note_modeler, " | GBD 2023 refresh, outliered because log age-standardized mean for location-year-sex-NID is lower than ", MAD, " MAD below median")]
with(new_xwalk, table(sex, mad))
with(new_xwalk, table(sex, median))
with(new_xwalk, table(sex, exp(median)))

write.xlsx(new_xwalk, output_filepath_mad, sheetName = "extraction", colNames=TRUE)

#Append GBD 2021 best EMR and CSMR
all_final <- rbind.fill(new_xwalk, df_nonCI)
mad_filepath <- paste0("FILEPATH")
write.xlsx(all_final, mad_filepath, rowNames = FALSE, sheetName = "extraction")
