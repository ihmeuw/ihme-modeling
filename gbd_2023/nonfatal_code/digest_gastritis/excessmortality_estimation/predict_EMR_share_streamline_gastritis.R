...
cause_name <- "digest_gastritis" #cause name for saving csv file
cause_path <- paste0(cause_name, "FILEPATH") #location to save files for specific cause of interest

#EMR data decisions
model_id <- mvid   #Dismod model 
remove_hospital = FALSE  #to remove all hospital data
remove_marketscan = FALSE #to remove all marketscan data
remove_taiwan <- FALSE #to remove Taiwan claims
remove_limit <- 0.5 #remove all EMR input data with value above this limit, default should be . or a value greater than we expect modelers to use
...
#Upload specs
uw_name <- "USER_ID" #for uploader validation
crosswalk_version_id <- xwid #this is the crosswalk version to which you want to append the EMR data (step2 best)
bundle_id <- id #this is your new bundle ID for step3 EMR
bundle_version_id <- bvid #this is the bundle version to which you want to save your new crosswalk version after appending EMR - created in previous script!
upload_datapath <- paste0("FILEPATH", date, ".xlsx") #upload path for new crosswalk version
seq_emr <- seq #this is the seq generated for EMR dummy row in new bundle version
...
description <- '2019 step2 crosswalk with predicted EMR - used for step3 modeling' #uploaded gastritis crosswalk with EMR model that used entirely default specs
