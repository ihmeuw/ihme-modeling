...
for(cause in causes) { 
  ...
  for(location in location_ids) {
    ...
    command <- paste0("qsub  -q all.q  -P proj_name -l m_mem_free=4G  -l fthread=5 -l h_rt=24:00:00 -o ",logs," -e ",logs, " -N ", paste0(cause,"_", location, "_adj_child "), shell, " ", "FILEPATH", cause, " ", location, " ", adj_child_files)
    print(command)
    system(command)
