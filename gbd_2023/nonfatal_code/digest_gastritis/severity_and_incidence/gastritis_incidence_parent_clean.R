...
for(cause in causes) { 
  ...
  for(location in location_ids) {
    command <- paste0("qsub -q all.q  -P proj_name -l m_mem_free=4G  -l fthread=5 -l h_rt=24:00:00  -o ",logs," -e ",error, " -N ", paste0(cause,"_", location, " "), shell, " FILEPATH", " ", cause, " ", location, " ", total_incidence_files)
    system(command)
  }
}
...
#save results
save_results_epi(input_dir="FILEPATH", input_file_pattern = "{location_id}.csv", modelable_entity_id =16219, description = 'DESCRIPTION', mark_best='TRUE', measure_id = c(5,6), gbd_round_id=7, decomp_step='iterative', bundle_id = id, crosswalk_version_id = xwid )
