################################################################################################################
################################################################################################################
## Purpose: this script does 5 things, which are 1) retrieves new data, 2) applies GBD 2019 
...
#########################################################################################
##STEP 3: Apply crosswalk coefficients from GBD 2019 
#########################################################################################
...
if (xwalk_type=="logit") {
  ...
  predicted <- as.data.table(subset(xwalk_coef, X_cv_marketscan == 1 & X_cv_ms2000==0 & X_cv_chart_review ==0)) 
...
dt_total$new <- 1 #this will allow us to distinguish later which data is new and which is not
...
description1 <- paste0('crosswalked step 2 new data, updated') #description needed for each crosswalk version; this is what is going to show up in Dismod
...
new_xwalk_not_MAD <- subset(new_xwalk, measure != "prevalence") 
new_xwalk<- subset(new_xwalk, measure == "prevalence") 
...
if (fine_age_groups=="yes") {
  ...
  all_fine_ages <- as.data.table(get_age_metadata(age_group_set_id=19)) ##For GBD 2021, age group set 19 defines most detailed ages
...
new_xwalk <- new_xwalk[, new_weight1 := age_group_weight_value/sum1, by = byvars] #divide each age-group's standard weight by the sum of all the weights in their location-age-sex-nid group 
...
new_xwalk[as_mean == 0, note_modeler := paste0(note_modeler, " | GBD 2021, outliered this location-year-sex-NID age-series because age standardized mean is 0")]
...
new_xwalk[as_mean>((MAD*mad)+median), note_modeler := paste0(note_modeler, " | GBD 2021, outliered because log age-standardized mean for location-year-sex-NID is higher than ", MAD, " MAD above median")]
new_xwalk[as_mean<(median-(MAD*mad)), is_outlier := 1]
new_xwalk[as_mean<(median-(MAD*mad)), note_modeler := paste0(note_modeler, " | GBD 2021, outliered because log age-standardized mean for location-year-sex-NID is lower than ", MAD, " MAD below median")]
...
#########################################################################################
#STEP 7: Updated outlier_status of old data using save_bulkd_outlier
 #########################################################################################
