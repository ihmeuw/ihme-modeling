...
get_emr_data <- function(model_id){
  odbc <- ini::read.ini('FILEPATH/emr_helper/.odbc.ini')
...
remove_clinical_or_location_data <- function(data, locations_to_remove = NULL){
  ...
  clinical_nids_cleaned <- clinical_nids %>%
    ...
    mutate(description =  trimws(description, which = "both"),
           ...
           #use the merged NID where available and if it says NA in that column, then use the single year NID.
  ...
  clinical_nids_labeled <- clinical_nids_reshaped %>%
    mutate(
      ...
      # TODO verify US claims only use MS and not CMS_MDCR
...
map_emr_input <- function(train_data){
  ...
  pdf(file = paste0(plot_output_path, '/all_emr_input_map_subnational.pdf'),
      height = 4.15, width = 7.5,
      pointsize = 6.5)
  gbd_map(
    df_total,
  ...
  pdf(
    file = paste0(plot_output_path, "/all_emr_input_map_national.pdf"),
    height = 4.15, width = 7.5,
    pointsize = 6.5
  )
  gbd_map(
...
"
  ...
  # # TODO is it a correct assumption that the current GBD round is the max release_id?
  # if (model_version_id_metadata$release_id != current_gbd_round_release_id) {
  #   stop(glue::glue("Model version release_id {model_version_id_metadata$release_id} is not the most recent GBD round release_id {current_gbd_round_release_id}.
  #        Please re-run the DisMod model. EMR prediction script depends on having the
  #        most up to date gbd location hierarchy so haq covariate can be merged on accurately."))
  # }
...
identify_missing_locations_from_map <- function(dt, loc_hier, output_path) {
  ...
  if (length(all_missing_locs) == 0) {
    ...
    fp <- file.path(output_path, "missing_locations.csv")
