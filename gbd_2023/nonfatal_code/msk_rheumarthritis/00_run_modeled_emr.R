...
shared_functs <- c("get_bundle_data.R", "get_crosswalk_version.R","save_crosswalk_version.R", "get_bundle_version.R",
                   "get_age_metadata.R", "get_elmo_ids.R", "get_covariate_estimates.R", "get_location_metadata.R", "get_age_metadata.R")
...
cause_name <- "ID" #cause name for saving csvs and for use in plot titles
#EMR data 
model_id <- 519419   #Dismod model version ID from which you want to pull EMR

#Upload specs
user_name <- Sys.info()['user'] #for uploader validation
bundle_id <- 6926 #bundle ID
crosswalk_version_id <- 26861  #xwalk version to which you want to append the predicted EMR - should be iterative 2020 
bundle_version_id <- 29648  #bundle version for crosswalk_version_id specified above 
upload_datapath <- paste0("FILEPATH") #upload path for new xwalk version
seq_emr <- 111370 #seq generated for EMR dummy row in bundle version listed above
...
dt <- as.data.table(get_emr_data(model_id))
setnames(dt, c('source_nid', 'source_underlying_nid'),  c('nid', 'underlying_nid'))

dt[, `:=` (outlier_type_id= NULL, model_version_id= NULL, model_version_dismod_id= NULL)]  
...
write.csv(dt, paste0(date_output_path, cause_name, "FILEPATH"), row.names = F)
