...
process_draws <- function (parent_draws, severe_id, mod_id, mild_id) {
  ...
  severe_draws <- get_draws(gbd_id_type = "model_id", gbd_id = severe_id, source = "ADDRESS",
                            location_id = my_loc, status = "best", release_id = release_id)
...
if (endemic == 0){
  ...
  my_draws <- gen_zero_draws(model_id= 'ADDRESS', location_id = NA, measure_id = c(5), metric_id = 3, release_id = release_id, team = 'ADDRESS')
...
if (endemic == 1){
  ...
  my_parent <- process_draws(my_parent, ADDRESS, ADDRESS, ADDRESS) ## subtract skin diseases
  my_parent <- process_draws(my_parent, ADDRESS, ADDRESS, ADDRESS) ## subtract vision diseases
  my_parent <- process_itch_draws(my_parent, ADDRESS, ADDRESS)  ## subtract itch diseases
