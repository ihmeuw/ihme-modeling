...
## Author: Jaimie Adelson, A.Lu
## MR-BRT code by R. Sorensen
## code/functions/advice incorporated from 
# C. Troeger, N. Roberts, E. Nichols, D. Santomauro, A.Lu, H. Han - thank you!
###############################################################################
...
home_dir <- "/ihme/scratch/projects/yld_gbd/substance_use/opioid/data/process/" #general location to save EMR files
cause_name <- "mental_drugs_opioid" #cause name for saving csvs and for use in plot titles
#EMR data 
model_id <- 543863   #Dismod model version ID from which you want to pull EMR

v#Upload specs
user_name <- Sys.info()['user'] #for uploader validation
bundle_id <- 6917 #bundle ID
crosswalk_version_id <- 31298 #xwalk version to which you want to append the predicted EMR - should be iterative 2020 
bundle_version_id <- 28112 #bundle version for crosswalk_version_id specified above 
upload_datapath <- paste0(j_root, "WORK/12_bundle/", cause_name, "/",  bundle_id, "/03_review/02_upload/2019_modeled_emr_xwalk_", date, ".xlsx") #upload path for new xwalk version
seq_emr <- 9449 #seq generated for EMR dummy row in bundle version listed above
...
remove_locations <- c(142, 160) #fill in location_ids you want to exclude from EMR analysis separated by commas
...
# dt <- read.csv(paste0(date_output_path, cause_name, "_mrbrt_emr_" ,date, ".csv"))
...
#data_dt <- as.data.table(read.csv("/ihme/scratch/projects/yld_gbd/substance_use/opioid/data/process/mental_drugs_opioid/2020-09-21/mental_drugs_opioidage_sex_haq2020-09-21/train_data.csv"))
#model_dt <- as.data.table(read.csv("/ihme/scratch/projects/yld_gbd/substance_use/opioid/data/process/mental_drugs_opioid/2020-09-21/mental_drugs_opioidage_sex_haq2020-09-21/model_summaries.csv"))
...
write_xlsx(list(extraction = final_dt), 
           "/ihme/scratch/projects/yld_gbd/substance_use/opioid/data/process/subnat_emr_opioid.xlsx")
