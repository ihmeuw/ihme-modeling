...
dismod_type <- "" # "mr" or "at"

##### Auto-complete cause meta-data for data prep ------------------------------------------------------------------------

cause_meta_data <- rbind(data.table(acause_label = 'mental_schizo', bundle_id = ifelse(dismod_type == "at", 10516, 152), age_pattern_me = 24004, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = "cv_broad_point, cv_broad_1yr, cv_broad_lifetime, cv_recall_lifetime"),
                         data.table(acause_label = 'mental_unipolar_mdd', bundle_id = ifelse(dismod_type == "at", 10353, 159), age_pattern_me = 23997, age_pattern_location = "super_region", sex_ratio_by_age = T, covariates = "cv_recall_1yr, cv_symptom_scales, cv_whs, cv_lay_interviewer, cv_dsm_5"),
                         data.table(acause_label = 'mental_unipolar_dys', bundle_id = 160, age_pattern_me = 23998, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = "cv_recall_1yr, cv_lay_interviewer"), 
                         ...
                         data.table(acause_label = 'mental_eating_bulimia', bundle_id = 164, age_pattern_me = 24002, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = "cv_recall_1yr, cv_old_dx_criteria"),
                         data.table(acause_label = 'mental_eating_anorexia', bundle_id = 163, age_pattern_me = 24003, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = ""),
                         ...
                         data.table(acause_label = 'mental_adhd', bundle_id = 167, age_pattern_me = NA, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = "cv_recall_1yr, cv_no_impairment"),
                         ...
                         data.table(acause_label = 'mental_pdd', bundle_id = 10352, age_pattern_me = 24001, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = "cv_autism, cv_survey"),
                         ...
                         data.table(acause_label = 'mental_anxiety', bundle_id = ifelse(dismod_type == "at", 10354, 162), age_pattern_me = 24000, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = "cv_recall_1yr"),
                         data.table(acause_label = 'imp_id', bundle_id = ifelse(dismod_type == "at", 10584, 291), age_pattern_me = NA, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = ""),
                         data.table(acause_label = 'mental_id', bundle_id = 169, age_pattern_me = NA, age_pattern_location = "super_region", sex_ratio_by_age = F, covariates = ""))
...
if(dismod_type == "at"){
  v_id <- bundle_metadata[cause == paste0(acause, "_at"), bundle_version]
} else {
...
if(acause != "mental_other"){   ## No need for sex crosswalks for Other Mental Disorders where all data is sex-specific
  ...
  age_sex_split <- function(x){
    #total <- data.table() # if need to debug
    #for(x in unique(review_sheet_bothsex_byage$unique_study_label)){ # if need to debug
    ...
    } else { # If RR or SMR, just assume both-sex estimates are 50% male / 50% female
    ...
    return(by_age)
    #debug <- rbind(total, by_age) # if need to debug
  }
  ...
  population <- get_population(location_id = unique(review_sheet_both$location_id), release_id = 16, age_group_id = c(1, 6:20, 30:32, 235), sex_id = c(1, 2), year_id = seq(min(review_sheet_both$year_start), max(review_sheet_both$year_end)))
  ...
  if(sex_ratio_by_age == T){
    review_sheet_both <- merge(review_sheet_both, sex_ratio, by = c("measure", "mid_age"), all.x = T) # all.x = T for estimates without sex ratio - just duplicate so can still be included)
  } else {
    review_sheet_both <- merge(review_sheet_both, sex_ratio, by = "measure", all.x = T) # all.x = T for estimates without sex ratio - just duplicate so can still be included)
  }
...
if(length(covariates) > 0 & acause != "mental_other"){
  ...
  if(acause == "mental_unipolar_dys"){crosswalk_model_mdd_lay_interview <- py_load_object(filename = paste0("/FILEPATH/", acause, "/FILEPATH/mdd_crosswalk_model.pkl"), pickle = "dill")}
 ...
 if(acause == "mental_unipolar_dys"){
    for(c in covariates[covariates != "cv_lay_interviewer"]){
      c_simple <- gsub("cv_", "", c)
      review_sheet_final[get(c) == 1 & measure == "prevalence", crosswalk_applied := paste0(crosswalk_applied, "-", c_simple)]
    }}
  
  if(!(acause == "mental_unipolar_dys" | acause == "mental_unipolar_mdd")) {
    for(c in covariates){
      c_simple <- gsub("cv_", "", c)
      review_sheet_final[get(c) == 1 & measure == "prevalence", crosswalk_applied := paste0(crosswalk_applied, "-", c_simple)]
  }} 
  ...
  review_sheet_xwalked <- data.table(crosswalk_model$adjust_orig_vals(df = review_sheet_final[crosswalk_applied != "gold" & measure == "prevalence" & mean > 0,],
  ...
  if(acause == "mental_unipolar_mdd"){
    ...
    review_sheet_xwalked_symptoms <- data.table(crosswalk_model_symptoms$adjust_orig_vals(df = review_sheet_final[crosswalk_applied_symptoms != "gold" & measure == "prevalence" & mean > 0,],
  ...
  if(acause == "mental_unipolar_dys"){
    review_sheet_final[, crosswalk_applied_lay_interviewer := 'gold']
    review_sheet_final[cv_lay_interviewer == 1 & measure == "prevalence", crosswalk_applied_lay_interviewer := "lay_interviewer"]
    review_sheet_xwalked_lay <- data.table(crosswalk_model_mdd_lay_interview$adjust_orig_vals(df = review_sheet_final[crosswalk_applied_lay_interviewer != "gold" & measure == "prevalence" & mean > 0,],
                                                                 orig_dorms = "crosswalk_applied_lay_interviewer", orig_vals_mean = "mean", orig_vals_se = "standard_error", data_id = "data_id"))
    review_sheet_xwalked_lay[, gamma_lay := as.numeric(crosswalk_model_mdd_lay_interview$gamma)]
    review_sheet_xwalked <- merge(review_sheet_xwalked, review_sheet_xwalked_lay[,.(data_id, pred_diff_mean_lay = pred_diff_mean, pred_diff_sd_lay = pred_diff_sd, gamma_lay)], by = 'data_id', all = T)
    review_sheet_xwalked[is.na(pred_diff_mean), `:=` (pred_diff_mean = 0, pred_diff_sd = 0, gamma = 0)]
    review_sheet_xwalked[is.na(pred_diff_mean_lay), `:=` (pred_diff_mean_lay = 0, pred_diff_sd_lay = 0, gamma_lay = 0)]
    review_sheet_xwalked <- review_sheet_xwalked[, .(data_id, pred_diff_mean = pred_diff_mean + pred_diff_mean_lay, pred_diff_sd = sqrt(pred_diff_sd^2 + gamma + pred_diff_sd_lay^2 + gamma_lay))]
  }
  ...
  if(acause == "mental_unipolar_dys"){
    review_sheet_final[crosswalk_applied == "gold" & crosswalk_applied_lay_interviewer == "lay_interviewer", crosswalk_applied := "lay_interviewer"]
    review_sheet_final[crosswalk_applied != "gold" & crosswalk_applied_lay_interviewer == "lay_interviewer", crosswalk_applied := paste0(crosswalk_applied, "-lay_interviewer")]
    review_sheet_final[, crosswalk_applied_lay_interviewer := NULL]
  }
  ...
  if(acause == 'mental_anxiety'){
    symptom_xwalk <- fread("/FILEPATH/gad7.csv")
    review_sheet_final[cv_symptom_scale == 1, `:=` (crosswalk_applied = "symptom_scale", pred_diff_mean = symptom_xwalk$coef, pred_diff_sd = symptom_xwalk$se)]
  }
  ...
  if(acause == "mental_bipolar"){
    ...
    review_sheet_final <- rbind(review_sheet_final, review_sheet_mscan, fill = T)
  }
...
if(acause == 'mental_personality'){
  crosswalk_save_folder <- paste0("/FILEPATH/", bundle_id, "/FILEPATH/crosswalk_uploads_2023/")
} else {
crosswalk_save_folder <- paste0("/FILEPATH/", acause, "/", bundle_id, "/FILEPATH/crosswalk_uploads_2023/")
}
...
if(dismod_type == "at"){
  bundle_metadata[cause == paste0(acause, "_at"), `:=` (bundle_version = v_id, crosswalk_version = cw_version)]
} else {
