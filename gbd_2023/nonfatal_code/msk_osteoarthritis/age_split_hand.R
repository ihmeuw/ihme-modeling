####################################################
## Description: AGE SPLIT FUNCTIONS
####################################################
...
col_order <- function(dt){
  epi_order <- fread("FILEPATH", header = F)
...
get_age_pattern <- function(locs, final_data, age_groups){
  final_data <- copy(final_data)
  final_data <- as.data.table(final_data)
  final_data <- final_data[cv_marketscan == 1]
  final_data <- get_cases_sample_size(final_data)
  aggregate_marketscan <- function(mark_dt){
    dt <- copy(mark_dt)
    by_vars <- c("age_start", "age_end", "sex")
    ...
    dt[, `:=` (location_id = 102, location_name = "United States", mean = cases/sample_size,
               lower = NA, upper = NA)]
    z <- qnorm(0.975)
    dt[, standard_error := sqrt(mean*(1-mean)/sample_size + z^2/(4*sample_size^2))]
    dt <- unique(dt, by = by_vars)
    return(dt)
  }
  final_data <- aggregate_marketscan(final_data)
  setnames(final_data, c("standard_error", "mean"), c("se_dismod", "rate_dis"))
  final_data[, sex := ifelse(sex == "Male", 1, ifelse(sex == "Female", 2, 3))]
  setnames(final_data, "sex", "sex_id")
  final_data[, measure_id := 5]
  ages <- get_age_metadata(12)
  setnames(ages, c("age_group_years_start", "age_group_years_end"), c("age_start", "age_end"))
  ages$age_end <- as.integer(ages$age_end)
  ages$age_start <- as.integer(ages$age_start)
  final_data <- merge(final_data, ages, by = "age_start")
  final_data <- final_data[ ,.(sex_id, measure_id, age_group_id, location_id, se_dismod, rate_dis)]
  age_pattern <- copy(final_data)
...
age_split <- function(df, age, region_pattern, location_pattern_id){
  ...
  age_pattern <- get_age_pattern(locs = locations, final_data = original, age_groups = age)
