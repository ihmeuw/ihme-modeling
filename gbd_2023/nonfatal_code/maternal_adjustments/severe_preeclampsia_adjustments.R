###############################################################################################################################################
# Purpose: data processing for severe pre-eclampsia input data
...
# severe_preeclampsia
# xw claims to inpatient by age
severe_preeclampsia <- get_bundle_version(bundle_version_id=bvid)

# calc mean and SE where mean and SE are NA
severe_preeclampsia <- get_cases_sample_size(severe_preeclampsia)
severe_preeclampsia <- get_se(severe_preeclampsia)
severe_preeclampsia <- calculate_cases_fromse(severe_preeclampsia)
severe_preeclampsia <- calc_year(severe_preeclampsia)

severe_preeclampsia$clinical_data_type <- ifelse(severe_preeclampsia$clinical_data_type=="", "literature", severe_preeclampsia$clinical_data_type)
severe_preeclampsia <- subset(severe_preeclampsia, clinical_data_type!="literature")
severe_preeclampsia$is_reference <- ifelse(severe_preeclampsia$clinical_data_type=="claims", 0, 1)
severe_preeclampsia$crosswalk_parent_seq<-""

severe_preeclampsia$age <- (severe_preeclampsia$age_start + severe_preeclampsia$age_end)/2
severe_preeclampsia$age_start <- sapply(1:nrow(severe_preeclampsia), function(i) get_closest_age(i, var = "age_start", start=T, dt = severe_preeclampsia))
severe_preeclampsia$age_end <- sapply(1:nrow(severe_preeclampsia), function(i) get_closest_age(i, var = "age_end", start = F, dt = severe_preeclampsia))

#set column for alt_dorms and ref_dorms for mrbrt
severe_preeclampsia <- severe_preeclampsia[is_reference==0, orig_dorms:="claims"]
severe_preeclampsia$orig_dorms[is.na(severe_preeclampsia$orig_dorms)] <- "inpatient"
severe_preeclampsia <- severe_preeclampsia[measure=="incidence" & mean==0, adj_zero_mean:=1]
severe_preeclampsia <- severe_preeclampsia[, orig_mean:=mean]
severe_preeclampsia <- severe_preeclampsia[measure=="incidence" & mean==0, mean:=(0.5*3.138805e-05)]
write.csv(severe_preeclampsia,"FILEPATH")
severe_preeclampsia <- severe_preeclampsia[, mean:=orig_mean]

####################################################################################################################################################
# label for matchfinding
severe_preeclampsia_ref <- subset(severe_preeclampsia, is_reference==1)
severe_preeclampsia_ref <- severe_preeclampsia_ref[, c("nid", "input_type", "source_type", "location_id", "location_name", "year_start", "year_end", "year_match", "age_start", "age_end",  "measure",
...
severe_preeclampsia_ref <- plyr::rename(severe_preeclampsia_ref, c("mean" = "mean_ref", "standard_error"="std_error_ref"))
severe_preeclampsia_alt <- subset(severe_preeclampsia, is_reference==0)
severe_preeclampsia_alt <- severe_preeclampsia_alt[, c("nid", "input_type", "source_type", "location_id", "location_name", "year_start", "year_end", "year_match", "age_start", "age_end",  "measure",
                         "cv_maternal_direct", "cv_hospital", "mean", "group_review","standard_error", "extractor", "is_outlier", "cv_inpatient", "cv_literature",
                         "cv_anteparthemonly", "cv_postparthhemonly", "cv_severeonly", "cv_severehem_excl", "is_reference", "age", "orig_dorms")]
severe_preeclampsia_alt <- plyr::rename(severe_preeclampsia_alt, c("mean"="mean_alt", "standard_error"="std_error_alt"))

####################################################################################################################################################
# between-study matches

match_severe_preeclampsia <- merge(severe_preeclampsia_ref, severe_preeclampsia_alt, by=c("measure", "location_id", "age_start", "age_end", "age"))
match_severe_preeclampsia$year_diff<- abs(match_severe_preeclampsia$year_match.x-match_severe_preeclampsia$year_match.y)
match_severe_preeclampsia<-subset(match_severe_preeclampsia, match_severe_preeclampsia$year_diff<6)
match_severe_preeclampsia <- subset(match_severe_preeclampsia, measure=="incidence")
match_severe_preeclampsia <- match_severe_preeclampsia[!is.na(match_severe_preeclampsia$mean_ref)]
setnames(match_severe_preeclampsia, "orig_dorms.x", "ref_dorms")
setnames(match_severe_preeclampsia, "orig_dorms.y", "alt_dorms")
# create matched_id for between-study matches
match_severe_preeclampsia <- match_severe_preeclampsia%>%
  mutate(matched_id=paste(nid.x,nid.y,age,year_match.x,year_match.y, sep = " - ") )

write.csv(match_severe_preeclampsia, "FILEPATH")

################################################################################################################################################
# mr-brt script for severe preeclampsia
...
severe_preeclampsia <- read.csv("FILEPATH")
severe_preeclampsia <- as.data.table(severe_preeclampsia)

# here, drop means of zero. matched pairs with zeros can be adjusted @ end but shouldnt be used as matched pairs
severe_preeclampsia <- severe_preeclampsia[!mean_ref==0]
severe_preeclampsia <- severe_preeclampsia[!mean_alt==0]
...
severe_preeclampsia$mean_log_ref <-cw$utils$linear_to_log(mean = array(severe_preeclampsia$mean_ref), sd = array(severe_preeclampsia$std_error_ref))[[1]]
severe_preeclampsia$sd_log_ref <- cw$utils$linear_to_log(mean = array(severe_preeclampsia$mean_ref), sd = array(severe_preeclampsia$std_error_ref))[[2]]

severe_preeclampsia$mean_log_alt <-cw$utils$linear_to_log(mean = array(severe_preeclampsia$mean_alt), sd = array(severe_preeclampsia$std_error_alt))[[1]]
severe_preeclampsia$sd_log_alt <- cw$utils$linear_to_log(mean = array(severe_preeclampsia$mean_alt), sd = array(severe_preeclampsia$std_error_alt))[[2]]

# take difference of log-transformed means and calculate sd of difference in log space
calculate_diff <- function(severe_preeclampsia, alt_mean, alt_sd, ref_mean, ref_sd) {
  dat_diff <- data.table(mean_alt = severe_preeclampsia[, mean_log_alt], mean_se_alt = severe_preeclampsia[, sd_log_alt], 
                         mean_ref = severe_preeclampsia[, mean_log_ref], mean_se_ref = severe_preeclampsia[, sd_log_ref])
  dat_diff[, `:=` (diff = mean_alt-mean_ref, diff_se = sqrt(mean_se_alt^2 + mean_se_ref^2))] 
  return(dat_diff[,c("diff", "diff_se")])
}

diff <- calculate_diff(
  severe_preeclampsia, alt_mean = "mean_log_alt", alt_sd = "sd_log_alt",
  ref_mean = "mean_log_ref", ref_sd = "sd_log_ref"
)

names(diff) <- c("log_diff_mean", "log_diff_se")
severe_preeclampsia <- cbind(severe_preeclampsia, diff)

################################################################################################################################################
# create crosswalk dataframe
dat1 <- cw$CWData(
  df = severe_preeclampsia,
  obs = "log_diff_mean",
  obs_se = "log_diff_se",
  alt_dorms = "alt_dorms",
  ref_dorms = "ref_dorms",
...
fit1 <- cw$CWModel(
  ...
  cov_models = list(
    ...
    cw$CovModel(cov_name = "age",
                spline = cw$XSpline(degree = 2L, knots = c(17, 20, 30, 40, 50, 52), 
                                    l_linear = TRUE,
                                    r_linear = TRUE))),
  gold_dorm = "inpatient")
...
age <- ages[age_start >= 10 & age_end <=54, age_group_id]
...
severe_preeclampsia <- calc_year(severe_preeclampsia)
severe_preeclampsia$clinical_data_type <- ifelse(severe_preeclampsia$clinical_data_type=="", "literature", severe_preeclampsia$clinical_data_type)
severe_preeclampsia_lit <- severe_preeclampsia[clinical_data_type=="literature"]

# pull in file with adjusted means & sds
severe_pre <- read.csv("FILEPATH")
severe_pre <- as.data.table(severe_pre)
severe_pre <- severe_pre[, mean:=meanvar_adjusted]
severe_pre <- severe_pre[, standard_error:=sdvar_adjusted]
severe_pre <- severe_pre[orig_dorms!="surveillance", upper:=NA]
severe_pre <- severe_pre[orig_dorms!="surveillance", lower:=NA]

# merge with lit 
severe_pre <- rbind(severe_preeclampsia_lit, severe_pre, fill=TRUE)

#subset to rows with age grange >5 that need split. Set aside age-specific data. Leave zeroes (cases or mean) alone
severe_pre <- severe_pre[, age_range:=age_end-age_start]
severe_pre_already_split <- severe_pre[age_range<=5 | mean==0 | cases==0] 
severe_pre_already_split <- severe_pre_already_split[, age_end:=age_end-1]
severe_pre_already_split <- severe_pre_already_split[age_range<=5 & !is.na (extractor), age_end:=age_end+1]
severe_pre_to_split <- severe_pre[age_range>5 & mean!=0 & cases!=0]
severe_pre_to_split[age_start>age_end, age_start:=age_start-5]
severe_pre_to_split[, id := 1:.N]


## split 
small_dt <- severe_pre_to_split
age_dt=ages
...
expand_age <- function(small_dt, age_dt = ages){
  ...
  split <- split[age_group_id>6 & age_group_id<15] ##don't keep where age group id isn't estimated for cause
  return(split)
}

split_df <- expand_age(severe_pre_to_split, age_dt=ages)
...
severe_pre <- rbind(severe_pre_already_split, split_df, fill=TRUE)

# label all as non-split
severe_pre <- severe_pre[, split_row:=0]

# label with parent_seqs & save 
split_ids <- unique(severe_pre$id)
severe_pre$crosswalk_parent_seq <- NA
severe_pre$crosswalk_parent_seq <- as.numeric(severe_pre$crosswalk_parent_seq)
severe_pre[is.na(crosswalk_parent_seq), crosswalk_parent_seq := seq]
severe_pre[, seq:=NA]

severe_pre <- subset(severe_pre, !(mean==0 & is.na(lower) & is.na(cases) & is.na(sample_size) & is.na(standard_error) & is.na(effective_sample_size)))
severe_pre[, sex:="Female"]
severe_pre <- severe_pre[group_review==1 | is.na(group_review)]
severe_pre <- severe_pre[measure=="incidence"]
...
severe_pre <- as.data.table(merge(severe_pre, envelope_distinct_new[, c("uses_env", "nid")], by="nid", all.x=TRUE))
severe_pre$note_modeler <- as.character(severe_pre$note_modeler)
severe_pre <- severe_pre[uses_env==1, is_outlier:=1][uses_env==1, note_modeler:="outliered because inpatient source uses envelope"]

# outlier terminal age groups
severe_pre <- severe_pre[age_start==10, is_outlier:=1][age_start==10, note_modeler:="outliered terminal age group"]
severe_pre <- severe_pre[age_start==50, is_outlier:=1][age_start==50, note_modeler:="outliered terminal age group"]

# outlier marketscan
ms_nids <- c(244369, 244370, 244371, 336847, 336848, 336849, 336850, 408680, 433114, 494351, 494352)
severe_pre <- severe_pre[nid%in%ms_nids, is_outlier:=1][nid%in%ms_nids, note_modeler:="outlier marketscan"]

write.xlsx(severe_pre, "FILEPATH")
write.csv(severe_pre, "FILEPATH")
