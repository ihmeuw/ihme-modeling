...
if (refit_agesex==0){
  ...
  ageSexCurve <- readRDS(paste0("FILEPATH"))
  ...
  # Save output
  saveRDS(ageSexCurve, file=paste0("FILEPATH"))
}
...
ages <- get_age_metadata(age_group_set_id=age_group_set_id,
                         release_id=release_id)[#age_group_id %in% age_groups,
                         ,.(age_group_id, age_start = age_group_years_start, age_end = age_group_years_end)]


## (2) Retrieve age-specific data
age_spec_dt <- get_bundle_version(bundle_version_id = ADDRESS)
