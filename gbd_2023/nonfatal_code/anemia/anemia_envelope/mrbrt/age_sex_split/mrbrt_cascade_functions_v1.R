...
.onLoad <- function(libname, pkgname) {
  use_python(python = "FILEPATH/mrtool_0.0.2/bin/python")
...
#
# September 2020
# -- Ported to 'mrbrt002' in January 2020
#
...
#' @param thetas a vector with same length as \code{stage_id_vars}, defining the strength of the prior
#' to be passed to each stage of the cascade; when using uniform priors
#' (i.e. \code{gaussian_prior = FALSE}, the default), theta sets the upper/lower bounds as
#'  \code{[beta +/ 3*sd*theta]}, where beta and sd define the Gaussian posterior distribution
#' from the previous stage; when using Gaussian priors, theta is a multiplier for the
#' standard deviations of the prior, \code{N(beta, (sd*theta)^2)}
...
#' @param gaussian_prior logical; when TRUE, uses a Gaussian prior to pass spline information down
#' the cascade; when FALSE, uses a uniform prior; see \code{thetas} parameter for more info
...
run_spline_cascade <- function(
    stage1_model_object, df, col_obs, col_obs_se, col_study_id, stage_id_vars,
    thetas, output_dir, model_label, gaussian_prior,
  ...
  for (stage_tmp in stage_id_vars) {
    ...
    for (submodel_tmp in unique(df[, stage_tmp])) {
      ...
      if (length(unique(df_submodel[, "y_ranef__cascade"])) == 1) {
        if ("intercept" %in% ob2$cov_model_names) {
          intercept_idx <- which(ob2$cov_model_names == "intercept")
          submodel_cov_list[[intercept_idx]]$prior_gamma_uniform[1:2,] <- 0
        }
        # for (i in 1:length(submodel_cov_list)) { # deprecated
      ...
      if (gaussian_prior) {
        ...
        spline_prior <- do.call("rbind", list(
          prior_dat$betas - prior_dat$stdevs * thetas[theta_idx] * 3,
          prior_dat$betas + prior_dat$stdevs * thetas[theta_idx] * 3
        ))

        submodel_cov_list[[which(cov_is_spline(ob))]]$prior_beta_uniform <- spline_prior
      ...
      try({
        ...
        posterior_out <- try({ get_spline_posteriors(submodel) })
        if (class(posterior_out) == "try_error") {
          spline_posteriors[[model_id]] <- prior_dat
        } else {
      ...
      if (dev) {
        ...
        # list.files(file.path(working_dir, "pickles"))
        ...
        mod_tmp <- py_load_object(file.path(working_dir, "pickles", "location_id__562.pkl"))
        df_pred$pred_loc <- mod_tmp$predict(dat_pred)
        with(df_pred, lines(age_start, pred_loc, lty = 2))
...
predict_spline_cascade <- function(fit, newdata) {
  ...
  for (var in stage_id_vars) newdata[, var] <- as.character(newdata[, var])
