...
#outlier everything except MarketScan & Poland Claims
...
uscb <- df_orig %>%
  filter(grepl("USCB", field_citation_value))
uscb_nids <- as.list(unique(uscb$nid))

df_orig$is_outlier[df_orig$nid %in% uscb_nids]<-1
...
write.csv(df_matched, paste0(modeling_dir, "FILEPATH",bundle_id,"_only_clinical_",date,".csv"), row.names=F)
write.csv(df_orig, paste0(modeling_dir, "FILEPATH",bundle_id,"_only_clinical_",date,".csv"), row.names=F)
