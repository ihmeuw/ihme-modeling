...
# max(standard_error)=0.9799, since it could not be>0.5, we check for instances of this occurance:
result_to_merge %>% filter(standard_error>1)
# there is just one instance, so we replace it with 0.5:
# result_to_merge$standard_error[result_to_merge$standard_error>0.5] <- 0.5
...
# handle mean>upper cases:
condition <- df$mean > df$upper

# Replace upper with mean for those rows
result_to_merge$lower[result_to_merge$mean<result_to_merge$lower] <- result_to_merge$mean[result_to_merge$mean<result_to_merge$lower]
