...
#read in the data
df_orig <- read.csv(paste0(modeling_dir, "FILEPATH",bundle_id,"_only_clinical_",date,".csv"))
df_matched <- read.csv(paste0(modeling_dir, "FILEPATH",bundle_id,"_only_clinical_",date,".csv"))
...
df_result <- df_result %>%
  mutate(lower_adj = ifelse(lower_adj < mean_adj, lower_adj, mean_adj))
...
# I am adding group_review column since this bundle is only clinical data and does not have this column.
df_final$group_review<- NA
...
write.xlsx(df_filtered, file = paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical.xlsx"), sheetName = "extraction", overwrite = TRUE)


# SAVE CROSSWALK VERSION: ---------------------------------------------------------------
source("FILEPATH")

data_filepath <- paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical.xlsx")
description <- "Poland Claims + MarketScan"
