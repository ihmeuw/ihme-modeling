...
if (save_results_vision == 1) {
  ...
  save_results_epi(input_dir = file.path(tmp_dir, "03_outputs", "01_draws", "_vision", "_unsqueezed"),
                   ...
                   gbd_round_id = 7,
                   sex_id = c(1,2),
                   description = paste("Custom", functional, "unsqueezed results from parent model version ID", parent_mvid),
                   decomp_step = ds,
                   bundle_id = bundle,
...
if (save_results_epilepsy == 1) {
  ...
  save_results_epi(input_dir = file.path(root_tmp_dir, "04a_dismod_prep_wmort", "04_ODE", "save_results_epilepsy", "encephalitis"),
                   ...
                   gbd_round_id = 7,
                   sex_id = c(1,2),
                   description = paste("Custom", functional, "epilepsy results from parent model version ID", parent_mvid),
                   decomp_step = ds,
                   bundle_id = bundle,
                   crosswalk_version_id = crosswalk_version_id,
                   mark_best = T)
