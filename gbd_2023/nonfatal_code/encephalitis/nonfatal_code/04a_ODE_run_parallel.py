...
def main(in_dir, in_dir_data, in_dir_value, in_dir_rate, out_dir_ODE, functional, outcome, location):
    ...
    dems = db.get_demographics(gbd_team = "epi", gbd_round_id = 7)
    sex_ids = dems['sex_id']
    year_ids = dems['year_id']
    
    for year in year_ids:
        for sex in sex_ids: 
            # begin new data frame to save one functional-outcome-year-sex (COMMENTED OUT FOR NOW - NOT APPENDING DATA)
