...
# Set encephalitis meids
epilepsy_meid <- 2403

# Inputs -----------------------------------------------------------------------
pull_dir_03b <- file.path(root_tmp_dir, "03b_outcome_split", "03_outputs", "01_draws")

get_epilespy_estimates <- function(gbd_id = 2403, my_id, my_name) {
  draws <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = 2403, measure_id = my_id, age_group_id = ages, source = "epi", location_id = location, gbd_round_id = 7, decomp_step = ds)
  setDT(draws)
...
if (pull_epilepsy) {
  ...
  get_epilespy_estimates(gbd_id = epilepsy_meid, my_id = 9, my_name = "mtexcess")
  # Get estimates from epilepsy (remission) - new for ODE solver 
  get_epilespy_estimates(gbd_id = epilepsy_meid, my_id = 7, my_name = "remission")
}

# Use get_envelope to get run_id for the current run for life tables
envelope <- get_envelope(gbd_round_id = 7, decomp_step = ds, with_shock = 1, with_hiv = 1)
# get life table for current run_id
life.table.dt <- fread(paste0("FILEPATH", envelope$run_id, "FILEPATH", location, ".csv"), header = T)
...
pop.dt <- get_population(year_id = years, location_id = location, sex_id = sexes, age_group_id = ages, gbd_round_id = 7, decomp_step = ds)
...
# generates 1000 samples from a normal distribution with the means and sd incidated in the above CP csv for both age groups
...
dir.create(file.path(tmp_dir, "03_outputs", "01_draws", location), showWarnings = F, recursive = T)
...
for (y in years) {
  for (s in sexes) {
    for (g in groups){
      ...
      incid.dt$modelable_entity_name <- functional
      ...
      age.start.end.dt <- get_age_metadata(age_group_set_id=19, gbd_round_id=7)
      ...
      write.csv(incid.dt, file.path(tmp_dir, "03_outputs", "01_draws", location, paste0(functional, '_', g, '_', location, '_', y, '_', s, '.csv')), row.names=F)
    }
  }
}
# ------------------------------------------------------------------------------
