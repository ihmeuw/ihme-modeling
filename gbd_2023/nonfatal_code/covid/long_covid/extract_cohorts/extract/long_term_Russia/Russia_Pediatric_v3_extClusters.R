...
tabulation <- function(path1, path2, wave_number, outpath, followup_time, typeData){
  ...
  fatg <- mutate(fatg, fatigue = ifelse((flw_fat %in% 1 & flw_fat_dur >=4) | (flw_fat_2 %in% 1 & flw_fat_dur_2 >=4) &
                                          fully_recover <= 5 & eq5d_summary_2 <= 0.9 &
                                          eq5d_summary_2 <= eq5d_summary - 0.1, 1, 0))
  ...
  anx_depres <- mutate(anx_depres, anxiety_depressed = ifelse((flw_diag_anx %in% 1 | flw_diag_depres %in% 1) & fully_recover <= 5 &
                                                                ((eq5d_summary_2 <= 0.9 & eq5d_summary_2 <= eq5d_summary - 0.1) |
                                                                   flw_eq5d_anx_2 > flw_eq5d_anx),
                                                              1,0))
