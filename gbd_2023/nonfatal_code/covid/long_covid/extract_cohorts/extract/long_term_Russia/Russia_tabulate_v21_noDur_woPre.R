...
# 21 add anxiety depression cluster
# 20 add residual columns for new clusters
# 19 fix non-unique IDs in wave 2 data
# 18 add symptom freq + regroup clusters
...
tabulation <- function(path1, path2, wave_number, outpath, followup_time, typeData){
  ...
  if(wave_number=='wave_1'){
    mod1 <-
      read_excel(path1)[,c("PIN","Age","Sex","ICU","COVID PCR")]
    mod1 <- mod1[mod1$`COVID PCR` %in% 1,]
  } else{
    mod1 <-
      read_excel(path1)[,c("PIN","Date of birth","Age","Sex","ICU","Diagnosis (ICD)")]
    mod1 <- mod1[mod1$`Diagnosis (ICD)` %ni% c("NA"),]
    setnames(mod1, "Date of birth", "flw_date_of_birth")
    mod1$flw_date_of_birth <- as.Date(as.POSIXct(strptime(mod1$flw_date_of_birth, format = "%d.%m.%Y")))
    mod1 <- mod1 %>%
      mutate(ICU = ifelse(ICU == "Yes", 1, ifelse(ICU == "No", 0, ICU)))
  }
  ...
  age_mod12 <- merge(mod12, ages, by='age_group_name', all.x=T)

  # Import followup data
  followup_orig <- read_excel(path2)

  if (wave_number == 'wave_2'){
    followup_orig$flw_date_of_birth <- as.Date(followup_orig$flw_date_of_birth)
  }
  ...
  followup <- followup_orig[,c("subjid",'per_fat','per_fat_dur','flw_breathless','flw_breathless_dur','flw_recovered',
                               'flw_loss_taste','flw_loss_taste_dur','flw_loss_smell','flw_loss_smell_dur','beh_ch','beh_ch_dur',
                               ...
                               'flw_sleep','alopecia_cov','flw_headache','flw_dizziness',
  ...
  mod12_fol <- merge(age_mod12, followup_orig, by='subjid')

  # Remove duplicated subjid
  if (wave_number=='wave_2'){
    mod12_fol <- mod12_fol %>%
      group_by(subjid) %>%
      filter(n() == 1 | flw_date_of_birth.x == flw_date_of_birth.y) %>%
      distinct(subjid, flw_date_of_birth.x, .keep_all = TRUE) %>%
      ungroup()
  }
  ...
  sample_n_byICU <- mod12_fol %>%
    group_by(ICU) %>%
    dplyr::summarise(N=n())

  # Getting the number of patients by covid vaccination status
  if (wave_number == "wave_1" & followup_time == "6_month"){
    sample_n_byVac <- NA
  } else {
    setnames(mod12_fol, "iflw_vaccine_cov", "flw_vaccine_cov")
    mod12_fol$flw_vaccine_cov <- as.numeric(mod12_fol$flw_vaccine_cov)
    mod12_fol <- mod12_fol %>%
      mutate(flw_vaccine_cov = ifelse(flw_vaccine_cov %in% c(0, 1), flw_vaccine_cov, "Unknown"))
    sample_n_byVac <- mod12_fol %>%
      group_by(flw_vaccine_cov) %>%
      dplyr::summarise(N=n())
    # sample_n_byVac <- sample_n_byVac[sample_n_byVac$flw_vaccine_cov %in% c(0,1),]
  }
  ...
  # Fatigue sub-threshold cluster (removed depr_mood %in% 1 | anx_cov %in% 1)
  fatg <- mod12_fol[,c('subjid','per_fat','flw_joint_pain','mus_pain','depr_mood','anx_cov','flw_recovered',
                       'eq5d_summary','eq5d_summary_2')]

  fatg <- mutate(fatg, fatigue_sub = ifelse((per_fat %in% 1 | flw_joint_pain %in% 1 | mus_pain %in% 1) &
                                          flw_recovered <= 3 & eq5d_summary_2 <= 0.9,
                                          # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                        1,0))

  message(paste("Average fatigue: "), mean(fatg$fatigue_sub, na.rm=T))

  # Respiratory sub-threshold (breathless, chest pain, cough) cluster
  breath <- mod12_fol[,c('subjid','flw_breathless','flw_chest_pains','flw_pers_cough','flw_breathless_now','flw_breathless_pre_c19',
                         'eq5d_summary','eq5d_summary_2','flw_recovered')]

  breath <- mutate(breath, res_sub = ifelse((flw_breathless %in% 1 | flw_chest_pains %in% 1 | flw_pers_cough %in% 1) &
                                              flw_recovered <= 3 & (eq5d_summary_2 <= 0.9 | flw_breathless_now >=2),
                                            # & ((eq5d_summary_2 <= 0.9 & eq5d_summary_2 <= eq5d_summary - 0.1) |
                                            #     flw_breathless_now > flw_breathless_pre_c19),
                                            1,0))

  message(paste("Average respiratory sub-threshold: "), mean(breath$res_sub, na.rm=T))

  # Cognitive sub-threshold (forgetful, confused, behavior change) cluster
  forg_conf <- mod12_fol[,c('subjid','for_cov','flw_confusion','beh_ch','flw_remember_today','flw_remember_pre_c19','flw_recovered',
                            'eq5d_summary','eq5d_summary_2')]

  forg_conf <- mutate(forg_conf, cog_sub = ifelse((for_cov %in% 1 | flw_confusion %in% 1 | beh_ch %in% 1) &
                                                    flw_recovered <= 3 & (eq5d_summary_2 <= 0.9 | flw_remember_today >=2),
                                                  # & ((eq5d_summary_2 <= 0.9 & eq5d_summary_2 <= eq5d_summary - 0.1) |
                                                  #     flw_remember_today > flw_remember_pre_c19),
                                                  1,0))

  message(paste("Average cognitive sub-threshold: "), mean(forg_conf$cog_sub, na.rm=T))

  # Loss of taste/smell
  ts_loss <- mod12_fol[,c('subjid','flw_loss_smell','flw_loss_taste','flw_recovered',
                          'eq5d_summary','eq5d_summary_2')]

  ts_loss <- mutate(ts_loss, taste_smell_loss = ifelse((flw_loss_smell %in% 1 | flw_loss_taste %in% 1 ) & flw_recovered <=3 &
                                                         eq5d_summary_2 <= 0.9,
                                                       # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                                       1,0))
  ...
  # Sleep problems
  sleep <- mod12_fol[,c('subjid','flw_sleep','flw_recovered',
                          'eq5d_summary','eq5d_summary_2')]

  sleep <- mutate(sleep, sleep_loss = ifelse(flw_sleep %in% 1 & flw_recovered <=3 &
                                               eq5d_summary_2 <= 0.9,
                                             # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                             1,0))

  message(paste("Average sleep problems: "), mean(sleep$sleep_loss, na.rm=T))

  # Dizziness
  dizzy <- mod12_fol[,c('subjid','flw_dizziness','flw_recovered',
                        'eq5d_summary','eq5d_summary_2')]

  dizzy <- mutate(dizzy, dizziness = ifelse(flw_dizziness %in% 1 & flw_recovered <=3 &
                                              eq5d_summary_2 <= 0.9,
                                            # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                            1,0))

  message(paste("Average dizziness: "), mean(dizzy$dizziness, na.rm=T))

  # Alopecia
  alopecia <- mod12_fol[,c('subjid','alopecia_cov','flw_recovered',
                        'eq5d_summary','eq5d_summary_2')]

  alopecia <- mutate(alopecia, alopecia = ifelse(alopecia_cov %in% 1 & flw_recovered <=3 &
                                                   eq5d_summary_2 <= 0.9,
                                                 # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                                 1,0))

  message(paste("Average alopecia: "), mean(alopecia$alopecia, na.rm=T))

  # Headache
  headache <- mod12_fol[,c('subjid','flw_headache','flw_recovered',
                        'eq5d_summary','eq5d_summary_2')]

  headache <- mutate(headache, headache = ifelse(flw_headache %in% 1 & flw_recovered <=3 &
                                                   eq5d_summary_2 <= 0.9,
                                                 # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                                 1,0))
  ...
  # Anxiety depression
  anxiety_depr <- mod12_fol[, c('subjid','flw_recovered','depr_mood','anx_cov','eq5d_summary','eq5d_summary_2')]

  anxiety_depr <- mutate(anxiety_depr, anxiety_depressed = ifelse((depr_mood %in% 1 | anx_cov %in% 1) & flw_recovered <=3 &
                                                                    eq5d_summary_2 <= 0.9,
                                                                  # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                                                  1,0))

  message(paste("Average anxiety_depr: "), mean(anxiety_depr$anxiety_depressed, na.rm=T))
  ...
  a3 <- merge(a3, alopecia[, c('subjid','alopecia')], all.x=T)
  a3 <- merge(a3, headache[, c('subjid','headache')], all.x=T)
  a3 <- merge(a3, anxiety_depr[, c('subjid','anxiety_depressed')], all.x=T)
  ...
  a3 <- mutate(a3, alopecia_residual = ifelse(long_term %in% 1 & alopecia %in% 1, 0, alopecia))
  ...
  a3 <- mutate(a3, long_term_extended = ifelse(long_term %in% 0 & (fatigue_sub_residual %in% 1 | res_sub_residual %in% 1 |
                                                                     cog_sub_residual %in% 1 | taste_smell_loss_residual %in% 1 |
                                                                     sleep_loss_residual %in% 1 | dizziness_residual %in% 1 |
                                                                     alopecia_residual %in% 1 | headache_residual %in% 1 |
  ...
  if (wave_number == "wave_1" & followup_time == "6_month"){
    DT <- merge(DT, mod12_fol[,c("subjid",'Sex','Age','age_group_name','age_group_id',
                                 'ICU','eq5d_summary','eq5d_summary_2')], by = "subjid", all.x=T)
  } else {
    DT <- merge(DT, mod12_fol[,c("subjid",'Sex','Age','age_group_name','age_group_id',
                                 'ICU','flw_vaccine_cov','eq5d_summary','eq5d_summary_2')], by = "subjid", all.x=T)
  }
  ...
  desired_columns <- c('subjid','Sex','Age','age_group_name','age_group_id','ICU','flw_vaccine_cov',
                       'post_acute', 'cog_mild', 'cog_moderate', 'cognitive', 'res_mild', 'res_moderate', 'res_severe', 'res_combine',
                       ...
                       'long_term','long_term_extended','long_term_all',
  ...
  if (wave_number == "wave_1" & followup_time == "6_month"){
    df_all <- mod12_fol[,c('subjid','Sex','age_group_name','age_group_id','ICU',
                           'flw_recovered','eq5d_summary','eq5d_summary_2',bin_vars,bin_vars_dur)]
  } else {
    df_all <- mod12_fol[,c('subjid','Sex','age_group_name','age_group_id','ICU','flw_vaccine_cov',
                           'flw_recovered','eq5d_summary','eq5d_summary_2',bin_vars,bin_vars_dur)]
  }

  DT <- as.data.frame(DT)
  df_all <- merge(df_all, DT[,c('subjid','long_term', 'long_term_extended','long_term_all','follow_up')], by='subjid', all.x=T)
    ...
    df_all <- df_all %>%
      mutate(
        ...
        !!var_longCovid_residual := ifelse(
          long_term_all == 1, 0, .data[[var_longCovid]]),
  ...
  openxlsx::write.xlsx(df_all2, paste0(outpath ,wave_number,"_",followup_time, "_",typeData,"_all_symptoms_v_",version, ".xlsx"),
                       rowNames=F)

  # Export results for long covid residuals
  # Additional symptom clusters to include
  additional_cols <- c('subjid','Sex','ICU','flw_vaccine_cov',
                       'post_acute', 'cog_mild', 'cog_moderate', 'cognitive', 'res_mild', 'res_moderate', 'res_severe',
                       'res_combine', 'Cog_Fat', 'Cog_Res', 'Res_Fat', 'Cog_Res_Fat', 'fatigue_sub',
                       'res_sub', 'cog_sub', 'taste_smell_loss', 'sleep_loss', 'dizziness',
                       'alopecia', 'headache','anxiety_depressed')

  df_noDur_resi <- merge(df_all, DT, by='subjid')

  if (wave_number == "wave_1" & followup_time == "6_month"){
    additional_cols <- setdiff(additional_cols, 'flw_vaccine_cov')
    df_noDur_resi <- merge(df_noDur_resi, mod12_fol[, c('subjid','Age','eq5d_summary','eq5d_summary_2')], by='subjid', all.x=T)
  } else {
    df_noDur_resi <- merge(df_noDur_resi, mod12_fol[, c('subjid','Age','flw_vaccine_cov','eq5d_summary','eq5d_summary_2')], by='subjid', all.x=T)
  }

  additional_cols2 <- additional_cols[!additional_cols %in% c('subjid','Sex','ICU','flw_vaccine_cov')]
  select_cols <- union(additional_cols, c(bin_vars,'Age','flw_recovered','eq5d_summary','eq5d_summary_2'))
  # select_cols <- colnames(df_noDur_resi)[colnames(df_noDur_resi) %in% select_cols]
  # print(additional_cols[!additional_cols %in% colnames(df_noDur_resi)])
  # print(colnames(df_noDur_resi))
  df_noDur_resi <- df_noDur_resi[, select_cols]

  # Create a new workbook to store long covid clusters
  wb <- createWorkbook()
  addWorksheet(wb, "0. All Data")
  writeData(wb, "0. All Data", df_noDur_resi)
  ...
  for (var in additional_cols2){
    df1 <- df_noDur_resi[df_noDur_resi[[var]] == 1,]

    if (wave_number == "wave_1" & followup_time == "6_month"){
      df1 <- df1[c("subjid","Sex","Age","ICU",var, setdiff(names(df1),c("subjid","Sex","Age","ICU",var)))]
    } else {
      df1 <- df1[c("subjid","Sex","Age","ICU",'flw_vaccine_cov',var, setdiff(names(df1),c("subjid","Sex","Age","ICU",'flw_vaccine_cov',var)))]
    }
  ...
  saveWorkbook(wb, paste0(outpath ,wave_number,"_",followup_time, "_",typeData,"_longCovid_clusters_v_",version, ".xlsx"), overwrite = TRUE)
  ...
  openxlsx::write.xlsx(df_both, paste0(outpath ,wave_number,"_",followup_time, "_",typeData,"_individual_symptoms_v_",version, ".xlsx"),
                       rowNames=F)

  # Create intermediary table
  inter <- merge(age_mod12[,c('subjid','Sex','ICU','age_group_name','age_group_id')],DT, by='subjid')
  inter <- inter[,!colnames(inter) %in% c('subjid','Age')]
  ...
  # List of desired columns in specific order
  desired_columns <- c('follow_up','age_group_id','age_group_name','Sex','post_acute', 'cog_mild', 'cog_moderate', 'cognitive',
  ...
  # List of desired columns in specific order
  desired_columns <- c('follow_up','Sex', 'post_acute', 'cog_mild', 'cog_moderate', 'cognitive',
                       'res_mild', 'res_moderate', 'res_severe', 'res_combine',
                       'Cog_Fat', 'Cog_Res', 'Res_Fat', 'Cog_Res_Fat', 'fatigue_sub','fatigue_sub_residual',
                       'res_sub','res_sub_residual', 'cog_sub','cog_sub_residual','taste_smell_loss','taste_smell_loss_residual',
                       'sleep_loss','sleep_loss_residual', 'dizziness','dizziness_residual', 'alopecia','alopecia_residual',
  ...
  cocurr_sex2 <- as.data.frame(t(cocurr_sex))

  # openxlsx::write.xlsx(cocurr_sex, paste0(outpath ,wave_number,"_",followup_time,"_",typeData, "_bySex_v_", version, ".xlsx"),rowNames=F)
  # openxlsx::write.xlsx(cocurr_sex2, paste0(outpath ,wave_number,"_",followup_time,"_",typeData, "_bySex_v_", version+1, ".xlsx"),rowNames=T)

  # Table by ICU only
  cocurr_ICU <- data_long %>%
    group_by(ICU,measure) %>%
    filter(value=='1') %>%
    dplyr::summarise(var=n()) %>%
    spread(measure, var)
  ...
  cocurr_ICU$follow_up <- followup_time

  # List of desired columns in specific order
  desired_columns <- c('follow_up','ICU', 'post_acute', 'cog_mild', 'cog_moderate', 'cognitive',
                       'res_mild', 'res_moderate', 'res_severe', 'res_combine',
                       'Cog_Fat', 'Cog_Res', 'Res_Fat', 'Cog_Res_Fat', 'fatigue_sub','fatigue_sub_residual',
                       'res_sub','res_sub_residual', 'cog_sub','cog_sub_residual','taste_smell_loss','taste_smell_loss_residual',
                       'sleep_loss','sleep_loss_residual', 'dizziness','dizziness_residual', 'alopecia','alopecia_residual',
  ...
  # Filter the list of desired columns based on what actually exists in the dataframe
  existing_columns <- desired_columns[desired_columns %in% colnames(cocurr_ICU)]

  # Subset the dataframe to only include existing columns in the desired order
  cocurr_ICU <- cocurr_ICU[, existing_columns]
  cocurr_ICU2 <- as.data.frame(t(cocurr_ICU))

  # openxlsx::write.xlsx(cocurr_ICU, paste0(outpath ,wave_number,"_",followup_time,"_",typeData, "_byICU_v_", version, ".xlsx"),rowNames=F)
  # openxlsx::write.xlsx(cocurr_ICU2, paste0(outpath ,wave_number,"_",followup_time,"_",typeData, "_byICU_v_", version+1, ".xlsx"),rowNames=T)
  ...
  if (wave_number == "wave_1" & followup_time == "6_month"){
    ...
    cocurr_vac <- data_long %>%
      group_by(flw_vaccine_cov,measure) %>%
      filter(value=='1') %>%
      dplyr::summarise(var=n()) %>%
      spread(measure, var)
    ...
    # List of desired columns in specific order
    desired_columns <- c('follow_up','flw_vaccine_cov', 'post_acute', 'cog_mild', 'cog_moderate', 'cognitive',
                         'res_mild', 'res_moderate', 'res_severe', 'res_combine',
                         'Cog_Fat', 'Cog_Res', 'Res_Fat', 'Cog_Res_Fat', 'fatigue_sub','fatigue_sub_residual',
                         'res_sub','res_sub_residual', 'cog_sub','cog_sub_residual','taste_smell_loss','taste_smell_loss_residual',
                         'sleep_loss','sleep_loss_residual', 'dizziness','dizziness_residual', 'alopecia','alopecia_residual',
    ...
    cocurr_vac <- cocurr_vac[cocurr_vac$flw_vaccine_cov %in% c(0,1),]
    cocurr_vac2 <- as.data.frame(t(cocurr_vac))

    # openxlsx::write.xlsx(cocurr_vac, paste0(outpath ,wave_number,"_",followup_time,"_",typeData, "_byVac_v_", version, ".xlsx"),rowNames=F)
    # openxlsx::write.xlsx(cocurr_vac2, paste0(outpath ,wave_number,"_",followup_time,"_",typeData, "_byVac_v_", version+1, ".xlsx"),rowNames=T)
  }

  # Create a new workbook to save different groups (age-sex, sex, ICU, vac status)
  wb1 <- createWorkbook()

  addWorksheet(wb1, "bySex")
  writeData(wb1, "bySex", cocurr_sex)
  addWorksheet(wb1, "byICU")
  writeData(wb1, "byICU", cocurr_ICU)
  if (wave_number == "wave_1" & followup_time == "6_month"){
    print("No Covid vaccination status")
  } else {
  ...
  # Save the workbook to a file
  saveWorkbook(wb1, paste0(outpath ,wave_number,"_",followup_time, "_",typeData,"_symptoms_byGroups_v_",version, ".xlsx"), overwrite = TRUE)
...
tabulation2 <- function(path1, path2, outpath){
# module 1
# Read in the patient info
  mod1 <- read_excel(path1)[,c("PIN","Age","Sex","ICU","COVID PCR")]
  ...
  bin_vars_select2 <- colnames(clusters_6m_all)[colnames(clusters_6m_all) %like% "_noDur$"]
  bin_vars_select2 <- str_replace(bin_vars_select2, "_longCovid_noDur$", "")
  ...
  names(mod12_fol)[names(mod12_fol) == "flw_headache_9f7a9d"] <- "flw_headache"
  names(mod12_fol)[names(mod12_fol) == "flw_dizziness_56ad3b"] <- "flw_dizziness"
  names(mod12_fol)[names(mod12_fol) == "flw_sleep_715309"] <- "flw_sleep"
  names(mod12_fol)[names(mod12_fol) == "flw_alop"] <- "flw_alopecia"
  names(mod12_fol)[names(mod12_fol) == "flw2_beh_ch"] <- "beh_ch"
  ...
  message(paste("Average post_acute: "), mean(post$post_acute, na.rm=T))
  ...
  # combined cognitive category(if cog_mild =1 | cog_moderate =1, then cognitive =1 )
  cog <- mutate(cog, cognitive = ifelse((cog_mild %in% 1 | cog_moderate %in% 1), 1, 0))
  message(paste("Average cognitive: "), mean(cog$cognitive, na.rm=T))
  ...
  # combined to the respiratory category
  res <- mutate(res, res_combine = ifelse((res_mild %in% 1 | res_moderate %in% 1 | res_severe %in% 1), 1, 0))
  message(paste("Average res_combine: "), mean(res$res_combine, na.rm=T))

  ## NEW CLUSTERS

  # Fatigue sub-threshold cluster (removed depr_mood, anx_cov)
  fatg <- mod12_fol[,c('subjid','per_fat','flw_joint_pain','mus_pain','depr_mood','anx_cov','flw_recovered',
                       'eq5d_summary','eq5d_summary_2')]

  fatg <- mutate(fatg, fatigue_sub = ifelse((per_fat %in% 1 | flw_joint_pain %in% 1 | mus_pain %in% 1) &
                                              flw_recovered <= 3 & eq5d_summary_2 <= 0.9, # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                            1,0))
  ...
  # Alopecia
  alopecia <- mod12_fol[,c('subjid','flw_alopecia','flw_recovered',
                           'eq5d_summary','eq5d_summary_2')]

  alopecia <- mutate(alopecia, alopecia = ifelse(flw_alopecia %in% 1 & flw_recovered <=3 &
                                                   eq5d_summary_2 <= 0.9, # & eq5d_summary_2 <= eq5d_summary - 0.1,
                                                 1,0))
  ...
  a3_cols <- unique(c(colnames(a3),"overlap","res_mild", "res_moderate", "res_severe"))
  a3_dup_names <- a3_cols[!a3_cols %in% c("subjid","age_group_name","age_group_id","Age","Sex",'ICU',"COVID PCR")]
  setnames(clusters_6m, a3_dup_names, paste0(a3_dup_names,"_6mo"), skip_absent = T)

  all_dup_names <- colnames(clusters_6m_all)[!colnames(clusters_6m_all) %in%
                                               c("subjid","age_group_name","age_group_id","Age","Sex","ICU","COVID PCR")]
  ...
  # Update long term with new clusters (exclude long term with old clusters)
  a3 <- mutate(a3, long_term_extended = ifelse(long_term %in% 0 & (fatigue_sub_residual %in% 1 | res_sub_residual %in% 1 |
                                                                     cog_sub_residual %in% 1 | taste_smell_loss_residual %in% 1 |
                                                                     sleep_loss_residual %in% 1 | dizziness_residual %in% 1 |
                                                                     alopecia_residual %in% 1 | headache_residual %in% 1 |
  ...
  # Update long term with both old and new clusters
  a3 <- mutate(a3, long_term_all = ifelse(long_term %in% 1 | long_term_extended %in% 1, 1, 0))
  ...
  # Merge survey responses into output
  DT2 <- merge(DT, mod12_fol, by = c("subjid"), all.x=T)
  DT2$eq5d_pair <- NULL
  ...
  df_all <- mod12_fol[,c('subjid','Sex','Age','age_group_name','age_group_id','ICU','flw_vaccine_cov',
                         'flw_recovered','eq5d_summary','eq5d_summary_2')]
  df_all <- merge(df_all,clusters_6m_all[,colnames(clusters_6m_all)[!colnames(clusters_6m_all)
                                                                    %in% c('Sex','Age','age_group_name','age_group_id','ICU')]],
                  by='subjid', all.x=T)
  df_all <- merge(df_all, followup_12m_orig[,c('subjid',bin_vars,bin_vars_dur)], by='subjid', all.x=T)
  DT <- as.data.frame(DT)
  df_all <- merge(df_all, DT[,c('subjid','long_term', 'long_term_extended','long_term_all','follow_up')], by='subjid', all.x=T)
  ...
  df_all2 <- df_all[, !colnames(df_all) %like% "_longCovid$|_longCovid_residual$|_dur"]
  openxlsx::write.xlsx(df_all2, paste0(outpath ,"wv1_12m_ongoing_all_symptoms_v_",version, ".xlsx"), rowNames=F)
  ...
  # Additional symptom clusters to include (no severity level)
  additional_cols <- c('subjid','Sex','Age','ICU','flw_vaccine_cov',
                       'post_acute', 'cognitive','res_combine',
                       'Cog_Fat', 'Cog_Res', 'Res_Fat', 'Cog_Res_Fat', 'fatigue_sub',
                       'res_sub', 'cog_sub', 'taste_smell_loss', 'sleep_loss', 'dizziness',
                       'alopecia', 'headache','anxiety_depressed')
  additional_cols2 <- additional_cols[!additional_cols %in% c('subjid','Sex','Age','ICU','flw_vaccine_cov')]

  # df_all_resi <- df_all[df_all$any_longCovid == 1, union(additional_cols, grep("long_term$|long_term_all$|_longCovid$|_longCovid_residual$",
  #                                                       colnames(df_all), value = TRUE))]
  df_noDur_resi <- merge(df_all, DT[,!colnames(DT) %like% "long_term|follow_up|vaccine"], by='subjid')
  # df_noDur_resi <- merge(df_noDur_resi, mod12_fol[, c('subjid',"Sex",'ICU',"flw_vaccine_cov")],by=c('subjid',"Sex",'ICU',"flw_vaccine_cov"))

  selected_cols <- union(additional_cols, bin_vars)
  df_noDur_resi <- df_noDur_resi[, c(selected_cols,'flw_recovered','eq5d_summary','eq5d_summary_2')]
  ...
  # Save the workbook to a file
  saveWorkbook(wb, paste0(outpath ,"wv1_12m_ongoing_longCovid_clusters_v_",version, ".xlsx"), overwrite = TRUE)
  ...
  # Calculate total patients with long Covid w/out duration criteria
  # bin_vars_longCovid_noDur <- colnames(df_all)[colnames(df_all) %in% bin_vars_longCovid_noDur]
  df_all_noDur <- df_all %>% select(bin_vars_longCovid_noDur)
  df_all_noDur <- data.frame(colSums(df_all_noDur, na.rm = TRUE))
  colnames(df_all_noDur) <- "yes_longCovid_noDur"
  df_all_noDur$Variable_longCovid_noDur <- rownames(df_all_noDur)
  ...
  # Calculate total patients with long Covid w/out duration criteria, not captured in original clusters
  # bin_vars_longCovid_noDur_residual <- colnames(df_all)[colnames(df_all) %in% bin_vars_longCovid_noDur_residual]
  df_all_noDur_residual <- df_all %>% select(bin_vars_longCovid_noDur_residual)
  df_all_noDur_residual <- data.frame(colSums(df_all_noDur_residual, na.rm = TRUE))
  colnames(df_all_noDur_residual) <- "yes_longCovid_noDur_residual"
  df_all_noDur_residual$Variable_longCovid_noDur_residual <- rownames(df_all_noDur_residual)
  ...
  # Calculate total patients with long Covid w/out duration criteria, not captured in any clusters (both old and new)
  # bin_vars_longCovid_noDur_residual_ext <- colnames(df_all)[colnames(df_all) %in% bin_vars_longCovid_noDur_residual_ext]
  df_all_noDur_residual_ext <- df_all %>% select(bin_vars_longCovid_noDur_residual_ext)
  df_all_noDur_residual_ext <- data.frame(colSums(df_all_noDur_residual_ext, na.rm = TRUE))
  colnames(df_all_noDur_residual_ext) <- "yes_longCovid_noDur_residual_ext"
  df_all_noDur_residual_ext$Variable_longCovid_noDur_residual_ext <- rownames(df_all_noDur_residual_ext)
  ...
  any_cols <- c("any_symptoms","any_longCovid_noDur","any_longCovid_noDur_residual",'any_longCovid_noDur_residual_ext')
  df_all_any <- df_all %>% select(all_of(any_cols))
  df_all_any <- data.frame(t(colSums(df_all_any, na.rm = TRUE)))
  df_all_any$No <- NA
  df_all_any$Unknown <- NA
  ...
  df_all_noDur <- merge(df_all_noDur, df_all_noDur_residual_ext, by="Variable")
  # df_both <- merge(df_all_lc, df_all_noDur, by="Variable")
  df_both <- merge(wide_counts2, df_all_noDur, by="Variable")
  # df_both$Variable_longCovid <- NULL
  # df_both$Variable_longCovid_residual <- NULL
  df_both$Variable_longCovid_noDur <- NULL
  df_both$Variable_longCovid_noDur_residual <- NULL
  ...
  openxlsx::write.xlsx(df_both, paste0(outpath ,"wv1_12m_ongoing_individual_symptoms_v_",version, ".xlsx"),rowNames=F)
  ...
  # openxlsx::write.xlsx(cocurr_sex2, paste0(outpath,"wv1_12m_ongoing_bySex_v_", version+1, ".xlsx"),rowNames=T)
  ...
  cocurr_ICU <- merge(x=cocurr_ICU, y=sample_n_byICU, all.y=TRUE)
  cocurr_ICU[is.na(cocurr_ICU)] <- 0
  cocurr_ICU$follow_up <- "12_months"
  ...
  # openxlsx::write.xlsx(cocurr_ICU, paste0(outpath,"wv1_12m_ongoing_byICU_v_", version, ".xlsx"),rowNames=F)
  # openxlsx::write.xlsx(cocurr_ICU2, paste0(outpath,"wv1_12m_ongoing_byICU_v_", version+1, ".xlsx"),rowNames=T)
  ...
  cocurr_vac <- merge(x=cocurr_vac, y=sample_n_byVac, all.y=TRUE)
  cocurr_vac[is.na(cocurr_vac)] <- 0
  cocurr_vac$follow_up <- "12_months"
  ...
  # openxlsx::write.xlsx(cocurr_vac, paste0(outpath,"wv1_12m_ongoing_byVac_v_", version, ".xlsx"),rowNames=F)
  # openxlsx::write.xlsx(cocurr_vac2, paste0(outpath,"wv1_12m_ongoing_byVac_v_", version+1, ".xlsx"),rowNames=T)
  # Create a new workbook to save different groups (age-sex, sex, ICu, vac status)
  wb1 <- createWorkbook()
  ...
  # Save the workbook to a file
  saveWorkbook(wb1, paste0(outpath,"wv1_12m_ongoing_symptoms_byGroups_v_",version, ".xlsx"), overwrite = TRUE)
}

# Extract wave 1, 12-month, ongoing
path1 <- 'FILEPATH/Part-1-PCR-Upd-5060 copy.xlsx'
path2 <- 'FILEPATH/Ongoing_ 12 months Follow_up.xlsx'
outpath <- "FILEPATH"
tabulation2(path1, path2, outpath)

# ------------------------------------------------------------------------------

# Read in 6-month data by Sex from wave 1 and 2
# outpath<-"FILEPATH"
# df_6mo_wave1 <- read_excel(paste0(outpath,"wave_1_6_month___symptoms_byGroups_v_", version, ".xlsx"), sheet ="bySex")
# df_6mo_wave2 <- read_excel(paste0(outpath,"wave_2_6_month___symptoms_byGroups_v_", version, ".xlsx"), sheet ="bySex")
#
...
outpath<-"FILEPATH"
df_grouped_symp1 <- read_excel(paste0(outpath,"wave_1_6_month___individual_symptoms_v_",version,".xlsx"))
#df_grouped_symp2 <- read_excel(paste0(outpath,"wave_1_12_month_Initial_individual_symptoms_v_",version,".xlsx"))
df_grouped_symp2 <- read_excel(paste0(outpath,"wv1_12m_ongoing_individual_symptoms_v_",version,".xlsx"))
df_grouped_symp3 <- read_excel(paste0(outpath,"wave_2_6_month___individual_symptoms_v_",version,".xlsx"))

df_grouped_symp1$follow_up <- "wave1_6m"
#df_grouped_symp2$follow_up <- "W1_12m_init"
df_grouped_symp2$follow_up <- "wave1_12m"
df_grouped_symp3$follow_up <- "wave2_6m"
...
openxlsx::write.xlsx(df_grouped_symp, paste0(outpath, "all_individual_symptoms_v_", version, ".xlsx"),rowNames=F)

# Merge symptom by groups data
sheet_names <- c("bySex","byICU","byVaccination","byAgeSex")
wb2 <- createWorkbook()

for (sheet in sheet_names){
  if (sheet != "byVaccination"){
    df_bygroups1 <- read_excel(paste0(outpath,"wave_1_6_month___symptoms_byGroups_v_",version,".xlsx"), sheet=sheet)
    df_bygroups1$follow_up <- "wave1_6m"
  }

  df_bygroups2 <- read_excel(paste0(outpath,"wv1_12m_ongoing_symptoms_byGroups_v_",version,".xlsx"), sheet=sheet)
  df_bygroups3 <- read_excel(paste0(outpath,"wave_2_6_month___symptoms_byGroups_v_",version,".xlsx"), sheet=sheet)

  df_bygroups2$follow_up <- "wave1_12m"
  df_bygroups3$follow_up <- "wave2_6m"
  ...
  if (sheet != "byVaccination"){
    ...
    df_bygroups_all <- rbind(df_bygroups2,df_bygroups3)
  }

  # df_bygroups_all$long_term_extended <- NULL

  desired_columns <- c('follow_up','age_group_id','age_group_name','Sex','flw_vaccine_cov','ICU',
...
# Save the workbook to a file
saveWorkbook(wb2, paste0(outpath,"all_clusters_byGroups_v_",version, ".xlsx"), overwrite = TRUE)

# Merge all data
df_indiv1 <- read_excel(paste0(outpath,"wave_1_6_month___longCovid_clusters_v_",version,".xlsx"), sheet="0. All Data")
df_indiv2 <- read_excel(paste0(outpath,"wv1_12m_ongoing_longCovid_clusters_v_",version,".xlsx"), sheet="0. All Data")
df_indiv3 <- read_excel(paste0(outpath,"wave_2_6_month___longCovid_clusters_v_",version,".xlsx"), sheet="0. All Data")

wb3 <- createWorkbook()
addWorksheet(wb3, "wave1_6m")
writeData(wb3, "wave1_6m", df_indiv1)
addWorksheet(wb3, "wave1_12m")
writeData(wb3, "wave1_12m", df_indiv2)
addWorksheet(wb3, "wave2_6m")
writeData(wb3, "wave2_6m", df_indiv3)

# Save the workbook to a file
saveWorkbook(wb3, paste0(outpath,"all_clusters_data_v_",version, ".xlsx"), overwrite = TRUE)
...
#
# df_w1_6m <- na.omit(df_w1_6m[df_w1_6m$fatigue_sub_residual == 1 & df_w1_6m$per_fat != 2,
#                              c('fatigue_sub_residual','flw_fatigue','per_fat')])
# df_w1_12m_init <- na.omit(df_w1_12m_init[df_w1_12m_init$fatigue_sub_residual == 1 & df_w1_12m_init$per_fat != 2,
#                                          c('fatigue_sub_residual','flw_fatigue','per_fat')])
# df_w1_12m_ong <- na.omit(df_w1_12m_ong[df_w1_12m_ong$fatigue_sub_residual == 1 & df_w1_12m_ong$per_fat != 2,
#                                        c('fatigue_sub_residual','flw_fatigue','per_fat')])
# df_w2_6m <- na.omit(df_w2_6m[df_w2_6m$fatigue_sub_residual == 1 & df_w2_6m$per_fat != 2,
#                              c('fatigue_sub_residual','flw_fatigue','per_fat')])
#
...
#          title=paste0("Histogram of flw_fatigue for ",nrow(data)," cases of fatigue_sub_residual")) +  # Change labels as appropriate
