...
tabulation <- function(path1, path2, wave_number, outpath, followup_time, typeData){
  ...
  breath <- mutate(breath, res_sub = ifelse((flw_breathless %in% 1 | flw_chest_pains %in% 1 | flw_pers_cough %in% 1) & flw_recovered <= 3 &
  ...
  forg_conf <- mutate(forg_conf, cog_sub = ifelse((for_cov %in% 1 | flw_confusion %in% 1 | beh_ch %in% 1) & flw_recovered <= 3 &
                                                    ((eq5d_summary_2 <= 0.9 & eq5d_summary_2 <= eq5d_summary - 0.1) |
                                                       flw_remember_today > flw_remember_pre_c19),
                                                  1,0))
