...
## Project: NF COVID
## Script: 
## Description: Italian ISRAIC data without the comparison group
## Date 2/26/2021
## Time stamp: 2/17/2021, we took out all the unknown sex because they are missing all other information. 
##             2/25/2021, Add followup date into the all_mark_dataset
##             2/26/2021, remove the pre-covid variables
## --------------------------------------------------------------------- ----
...
#fatigue
ped <- mutate(ped, post_acute = ifelse((fatigue_past_7days %in% 'yes') & (fully_recovered_from_covid19<10 )& 
...
ped <- mutate(ped, cog = ifelse((confusion_or_lack_of_concentration_past_7days %in% 'si') & (fully_recovered_from_covid19<10 ) &
                                         (classroom_learning_interference_due_to_difficulties_since_covid19>2 ), 1,0))
table(ped$cog)
summary(ped$cog)
...
ped <- mutate(ped, res = ifelse((`difficulty_breathing _chest_tightness_past_7days` %in% 'yes') & (fully_recovered_from_covid19<10 ) &
                                  (pain_on_breathing_past_7days %in% 'yes' |
...
write.csv(a3, paste0("Italy_pediatric_all_data_wo_comparison_v", version, ".csv"))
...
write.csv(cocurr_mu, paste0("Italy_ped_byAgeSex_wo_comparison_v", version, ".csv"))
...
write.csv(cocurr_sex, paste0("Italy_ped_bySex_wo_comparison_v", version, ".csv"))

#create plots
pdf(file= paste0("Italy_pediatric_plot_wo_comparison", version, ".pdf"), height=11, width=8.5)
