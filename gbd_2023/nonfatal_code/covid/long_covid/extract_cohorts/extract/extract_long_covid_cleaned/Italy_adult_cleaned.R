...
## Project: NF COVID
## Script: 
## Description: Italian ISRAIC Adult data
## Date 2/19/2021
## Time Stamp: 2/26/2021 Add follow up days
## --------------------------------------------------------------------- ----
...
adult <- read_excel('Buonsenso_LongCovid-GBD_formatted_v3.xlsx',
                    sheet="Long-COVID")[,c("patient_id","anxiety_before_covid19","follow_up_days",
                                           "anxiety_today",
                                           "daily_activities_before_covid19",
                                           "daily_activities_today",
                                           "breathlessness_today",
                                           "breathlessness_before_covid19",
                                           "muscle_weakness_past_7days",
                                           "cConfusion_past_7days","cough_past_7days","problems_speaking_or_communicating_past_7days",
                                           "lack_of_focus_and_concentration_before_covid19","language_difficulties_before_covid19")]
dt <- merge(age_pattern[,c('patient_id','age','sex','age_group_name')], adult, by='patient_id')
#apply to pediatric patients only
#two patients are actually >19 at the time of the analysis, these two patients would be included
...
#cogntion
dt <- mutate(dt, cog = ifelse((language_difficulties_before_covid19 %in% 1 & problems_speaking_or_communicating_past_7days %in% 'si' ) |
                                  (lack_of_focus_and_concentration_before_covid19 %in% 1 & cConfusion_past_7days %in% 'si' ), 1,0))
table(dt$cog)
summary(dt$cog)

#respiratory
dt <- mutate(dt, res = ifelse(((breathlessness_today>  breathlessness_before_covid19) |
                                (breathlessness_today> 1 & cough_past_7days %in% "si" ))&
                               (daily_activities_today> daily_activities_before_covid19), 1,0))
table(dt$res)
summary(dt$res)
...
write.csv(dt, paste0("Italy_adult_data_marked_v", version, ".csv"))
...
write.csv(cocurr_mu, paste0("Italy_audult_byAgeSex_", version, ".csv"))
...
write.csv(cocurr_sex, paste0("Italy_adult_bySex_v", version, ".csv"))

#create plots
pdf(file= paste0("Italy_adult_plot_", version, ".pdf"), height=11, width=8.5)
...
grid.arrange(Cog, Res, Long_term,Res_cog, ncol=1)
...
grid.arrange( Cog_percent, Res_percent, Long_term_percent,Res_cog_percent, ncol=1)
