...
# 4 re-model community deaths among all deaths
# 3 instead of modeling community deaths among all deaths, model community deaths among infections
# 2 inlier=0.9, correct in normal space
# 1 first pass, normal space, inlier=1, wrong conversions
...
outputfolder <- paste0("FILEPATH", version, "/")
...
######################################################################################
    ...
    # model_dir <- paste0(out,"_",gender,"_v", version, "/")
...
#    predict_matrix$pred_hi <- exp(predict_matrix$pred_hi) / (1 + exp(predict_matrix$pred_hi))
    ...
    ggsave(plot, filename=paste0("FILEPATH","plots/v", version, "_prop_longterm_care_deaths.pdf"), width = 6, height = 8)
