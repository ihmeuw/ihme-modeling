...
#hm <- unique(data[case_name %in% c('history_ami','history_cad'),c('location_name','nid')])

## DIAG MI CROSSWALK
if(grepl('7052',model_abbr) & xw_measure=='incidence'){
  
  ## remove sweden clinical - we have more detailed. 
  data <- data[-which(location_name=='Sweden' & clinical_data_type != "")] ##
  
  ## Remove clinical locations we do not use
  locs_rm <- c(20,37,144,151,155,164,180,193,43887,4856,4862,78)
  data <- data[-which(location_id %in% locs_rm & clinical_data_type != ""), ] ## remove locations where we do not use clinical data
  data <- data[nid %ni% c(337619)] # CHN hospital data
  
  
  ## different labeling over time of troponin category (cv_other & cv_troponin the same)
  data[cv_other==1,cv_diag_blood:=0] 
  data[cv_troponin==1, cv_diag_blood:=1]
  data[is.na(cv_diag_blood), cv_diag_blood:=0]
  data[year_start <= 2001  & clinical_data_type != "", cv_diag_blood := 1] ## any data pre-2001 used CK-MB
  data[clinical_data_type != "", cv_diag_blood := 0] 
  
  ##label the clinical data
  data[clinical_data_type != "", cv_nonfatal := 1] ## label clinical as hospitalized incidence
  
  ##label the remaining columns. 
  data[is.na(cv_nonfatal), cv_nonfatal:=0]   ## assuming if NA it is 0
  data[is.na(cv_firstever), cv_firstever:=0]
  
  ## hospitalized crosswalk. flip the values of cv_nonfatal, 
  ## cv_nonfatal= 0 => hospitalized only. 
  ## cv_nonfatal= 1 => hospitalized + non-hospitalized deaths.
  
  data[,cv_nonfatal_rev:=ifelse(cv_nonfatal==0,1,0)]
  data[,cv_nonfatal := cv_nonfatal_rev]
  
  ## subset to mean & SE less than 1 
  data <- data[mean < 1, ] 
  data <- data[standard_error < 1, ]
  
  data[age_end==124, age_end:=99] ## fix endpoint for CI
  
}


if(grepl('7052',model_abbr) & xw_measure=='mtexcess'){
  
  emr_metadata <- data.table(read.xlsx('FILEPATH/.xlsx'))

  ## Switch definitions so that cv_nonfatal = 0 : hosptialized cfr & cv_nonfatal = 1 : all AMI CFR 
  data[,cv_nonfatal_rev:=ifelse(cv_nonfatal==0,1,0)]
  data[,cv_nonfatal := cv_nonfatal_rev]
  
  data[location_name=='Sweden' & specificity=='hosp', `:=` (group_review = NA, specificity = NA, group = NA)]
  data[location_name %in% c('Sweden except Stockholm','Stockholm'), `:=` (location_name = 'Sweden',location_id = 93,ihme_loc_id='SWE')]
