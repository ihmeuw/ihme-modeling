...
rake <- function(data, measure="case_draw_") {
  ...
  for (LEVEL in 4:max(locations_rake$level)) {
    ...
    if (LEVEL != max(locations_rake$level)) {
      ...
      ### prepare next level for raking by replacing model estimates for parents with raked estimates (for countries with multiple levels of subnationals)
  ...
  ### Keep raked estimates for output
