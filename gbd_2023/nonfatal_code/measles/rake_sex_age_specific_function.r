...
rake <- function(data, measure="case_draw_") {
  ...
  data_rake <- merge(data, parent_rake[, c("parent_id", "year_id", "age_group_id", "sex_id", parent_cols), with=FALSE], 
                     by=c("parent_id", "year_id", "age_group_id", "sex_id"), all.x=TRUE)
  ...
  for (LEVEL in 4:max(locations_rake$level)) {
    ...
    if (LEVEL != max(locations_rake$level)) {
      ...
      data_replace <- data_replace[, c("location_id", "year_id", "sex_id", "age_group_id", replace_cols), with=FALSE]
      setnames(data_replace, "location_id", "parent_id")
      
      # data_rake <- merge(data_rake, data_replace, by=c("parent_id", "year_id"), all.x=TRUE)
      data_rake <- merge(data_rake, data_replace, by=c("parent_id", "year_id", "age_group_id", "sex_id"), all.x=TRUE)
  ...
  data_rake <- data_rake[, c("location_id", "year_id", "sex_id", "age_group_id", rake_cols), with=FALSE]
  ...
  data_parents <- data[level==3, c("location_id", "year_id", "sex_id", "age_group_id", draw_cols), with=FALSE]
