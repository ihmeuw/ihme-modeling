##########################################################################
#GBD 2020 leprosy age crosswalk   -- we use india age pattern model for india data, global age pattern model for all other data sources
##########################################################################
...
pacman::p_load(openxlsx, ggplot2, magrittr, data.table, stringr)
...
col_order <- function(dt){
  ...
  new_epiorder <- c(order, extra_cols)
  setcolorder(dt, new_order)
  return(dt)
}

#!! NOTE: IF USING CROSSWALKED DATA CHECK CASES & SAMPLE SIZE ARE ADJUSTED MAKE SURE THIS LOGIC APPLIES
...
expand_age <- function(small_dt, age_dt = ages){
  ...
  dt[, drop := cases/n.age] 
  #THE NEXT LINE HERE WILL DROP RECORDS FOR WHICH <20 CASES ARE REPORTED, COMMENT IT OUT IF YOU DON'T WANT THAT FEATURE 
  #dt <- dt[!drop<1,]
...
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "model_id", gbd_id = id, ## imposing 2010 as has most age speficic data 
                           measure_id = c(5, 6), location_id = locs, source = "ADDRESS",
                           status = "best", sex_id = c(1,2), gbd_round_id = "ADDRESS", decomp_step = "ADDRESS",
  ...
  us_population <- get_population(location_id = locs, year_id = 2010, sex_id = c(1, 2), 
                                  age_group_id = age_groups, decomp_step = "ADDRESS")
  ...
  se <- copy(age_1)
  se <- se[age_group_id==238, .(measure_id, sex_id, se_dismod, location_id)] ## use standard error from 1-4 age group 
...
get_pop_structure <- function(locs, years, age_groups){
  populations <- get_population(location_id = locs, year_id = years, decomp_step = "ADDRESS",
                                sex_id = c(1, 2, 3), age_group_id = age_groups, gbd_round_id="ADDRESS")
...
#check cases and sample size not missing
dt$sample_size <- dt$effective_sample_size
#round cases up
...
###extract india only data 

dt$ind<-str_extract(dt$ihme_loc_id,"IND")
dt_india<-dt[ind=="IND"]

#subset global dataset (excluding india)
dt_gl<-dt[is.na(ind)]
...
age_split <- function(gbd_id, df, age, region_pattern, location_pattern_id){
  ...
  super_region_dt <- get_location_metadata(location_set_id = 'ADDRESS', decomp_step = "ADDRESS")
...
####-------------THIS Line  EXECUTES THE SPLIT for India --------------:
final_split <- age_split(gbd_id = 'ADDRESS', df = dt_india, age = age_groups, region_pattern = F, location_pattern_id = 'ADDRESS')

final_split_ind<-final_split

#COPY DATASET 
write.csv(final_split_ind,"FILEPATH")


####-------------THIS Line  EXECUTES THE SPLIT for all other locations --------------:

#subset global dataset (excluding india)
dt_gl<-dt[is.na(ind)]

df <- copy(dt_gl)
#global age pattern MEID:
id <- 'ADRESS'


#THIS CODE EXECUTES THE SPLIT for all other locations:
final_split <- age_split(gbd_id = id, df = dt_gl, age = age_groups, region_pattern = F, location_pattern_id = 'ADDRESS')


##append both datasets
age_split_ADDRESS<-rbind(final_split,final_split_ind)

#save copy as csv:
write.csv(age_split_ADDRESS,"FILEPATH")

#upload crosswalk version  
#group, group_review, specificity can be blanked to upload
age_split_ADDRESS[, c("group", "group_review", "specificity", "underlying_nid", "sampling_type", "recall_type_value", "uncertainty_type", "input_type", "design_effect") := NA]


age_split_ADDRESS <- age_split_ADDRESS[location_id != 'ADDRESS']

#Run file paths (needed to save_crosswalk
...
openxlsx::write.xlsx(age_split_ADDRESS, ADDRESS_outfile,  sheetName = "extraction")
