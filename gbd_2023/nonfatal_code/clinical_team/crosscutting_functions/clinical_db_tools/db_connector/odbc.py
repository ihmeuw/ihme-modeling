    ...
    def load(self, path):
        if not isinstance(path, str):
            raise InvalidPathException(
                "Path must be type str, can't be of type {}".format(type(path))
            )
                ...
                if line[0] == "[":
                    ...
                    if server_name in servers:
                        raise DuplicateServerException(
                            "Multiple server settings for {} found in {}. "
                            "Please delete one".format(server_name, path)
                        )
                    ...
                    chunks = line.split("=")
                    if len(chunks) == 2:
                        param_key, param_value = [s.strip() for s in chunks]
