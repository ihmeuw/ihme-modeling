...
class CauseICG:
    ...
    def get_icd_to_icg(self):
        # get cc to icg
        cm = clinical_mapping_db.get_clinical_process_data(
    ...
    def get_icg_to_bundle(self):
        # get icg to bundle
        self.bm = clinical_mapping_db.get_clinical_process_data(
    ...
    def merge_icg_to_cause(self):
        ...
        ic.loc[ic["icg_id"].duplicated(keep=False), "icg_maps_to_multiple_causes"] = "yes"
        self.ic = ic
    ...
    def icg_to_cause_with_missing(self):
        ...
        df_icd = self.cm[["cause_code", "code_system_id"] + keeps].drop_duplicates().copy()
    ...
    def _archive(self, df, fpath):
        ...
        p = f"{self.outdir}/_archive/stored_{n}_{os.path.splitext(fn)[0]}.csv"
        df.to_csv(p, index=False)

    def save(self):
        p1 = f"{self.outdir}/icg_to_cause_map_ci_map{self.mapv}.xlsx"
        p2 = f"{self.outdir}/icd_to_cause_map_ci_map{self.mapv}.xlsx"
