...
inj.drop(
    ["FYEAR", "year_start", "year_end", "AgeBand", "DiagCode3"], axis=1, inplace=True
)
...
df.loc[df["level_7"] == "start_value", "year_end"] = df.loc[
    df["level_7"] == "start_value", "year_start"
]
df.loc[df["level_7"] == "end_value", "year_start"] = df.loc[
    df["level_7"] == "end_value", "year_end"
]
...
val1 = df[
    (df.year_start != df.year_start.min()) & (df.year_end != df.year_end.max())
].value.sum()
...
if df[str_cols].isnull().any().any():
    warnings.warn(
        "There are Nulls in one of the columns {}"
        r" These nulls will be converted to the string nan".format(str_cols),
        Warning,
    )
...
# make all age_ends exlusive to fit our pattern
...
assert round(df_agg.val.sum(), 3) == round(val_sum, 3), "some cases were lost"
#####################################################
# WRITE TO FILE
#####################################################
