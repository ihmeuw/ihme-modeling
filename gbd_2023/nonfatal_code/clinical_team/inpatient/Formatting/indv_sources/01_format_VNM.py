...
# %%
# this was creating negative values so we'll only use the cause totals
...
#         'otp-cases-15-125', 'otp-cases-0-15', 'otp-deaths-0-125',
#         'inp-cases-0-5', 'inp-cases-5-15', 'inp-cases-15-125',
#         'inp-deaths-0-5', 'inp-deaths-5-15', 'inp-deaths-15-125']].copy()

# test extracting male cases
# it looks like the male deaths go negative a lot.
# there are also some very strange results for males related to pregnancies,
# so it looks like there are a lot of women not being identified as women
...
df = df[
    ["disease_code", "disease_name", "otp_total", "inp_total", "inp_total_deaths"]
].copy()
...
df.rename(
    columns={
        "otp_total": "otp_cases",
        "inp_total": "inp_cases",
        "inp_total_deaths": "inp_deaths",
    },
    inplace=True,
)
# %%
# assert inp_total_cases == (df['inp-cases-0-5'] + df['inp-cases-5-15'] +
#                           df['inp-cases-15-125']).sum()
# assert otp_total_cases == (df['otp-cases-0-15'] + df['otp-cases-15-125']).sum()
# %%
df = df.set_index(["disease_code", "disease_name"]).stack().reset_index()
df.rename(columns={"level_2": "facility_outcome", 0: "val"}, inplace=True)
# %%
# split into facility type and age cols
# df['facility_id'], df['outcome_id'], df['age_start'], df['age_end'] =\
#    df['facility_age'].str.split("-", 3).str
...
# we have a code system lookup table in our db now
df["code_system_id"] = 3
...
if df[str_cols].isnull().any().any():
    warnings.warn(
        "There are Nulls in one of the columns {}".format(str_cols),
...
df["facility_id"] = df.facility_id.replace(
    {"otp": "outpatient unknown", "inp": "inpatient unknown"}
)

# NOTE Deaths are already included in 'case'
df = df[df.outcome_id != "death"]

# code below is deprecated, the neg values are created by subtracting one age
# group from another, so for now drop the negatives until we figure a solution
df = df[df.val > 0]
# there are some negative values. These must be typos
# preval = df.val.sum()
# diffval = df[df.val<0].val.sum()*2
# df.val = abs(df.val)  # convert val to all positive ints
# assert preval - df.val.sum() == diffval, \
#    "Making negative cases positive changed the case counts"
# %%
#####################################################
...
assert total_cases == df_agg.val.sum(), "some cases were lost"
assert (
    df[df.facility_id == "inpatient unknown"].val.sum() == inp_total_cases
