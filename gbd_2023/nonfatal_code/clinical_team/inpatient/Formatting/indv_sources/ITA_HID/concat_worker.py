...
def run_concat() -> None:
    ...
    all_files = list(constants_ITA_HID.OUTPATH_DIR.glob("output_by_year/*.parquet"))

    # Concatenate all parquet files
    df = pd.concat([pd.read_parquet(f) for f in all_files], ignore_index=True)

    # Calculate missing_years after df is defined
    missing_years = set(df["year_start"].unique()) - set(constants_ITA_HID.NID_DICT.keys())
    ...
    compare_df = pd.read_hdf(constants_ITA_HID.PREV_DATA)
    # compare_df["cause_code"] = compare_df["cause_code"].str.upper()
    subset_df = df[df["year_start"] <= 2021]
    # not enforcing test_results currently for this source
    msg = formatting.test_case_counts(df=subset_df, compare_df=compare_df, dfs_must_equal_eachother=False)
    if isinstance(msg, list):
        raise RuntimeError(f"Test failed see: {msg}")


    general_purpose.write_hosp_file(df, constants_ITA_HID.FINAL_OUTPATH, backup=True)
