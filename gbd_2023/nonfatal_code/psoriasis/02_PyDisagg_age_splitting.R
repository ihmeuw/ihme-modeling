...
# replacing <=0 or >=1 values in mean and lower, and upper 
# Step 1: Calculate half the lowest non-zero value in 'mean'
half_lowest_non_zero_mean <- min(data$mean[data$mean > 0]) / 2
...
half_lowest_non_zero_lwr <- min(data$lower[data$lower > 0]) / 2
...
# check and exclude uids with age_start and age_end <=1 years, since Psoariasis is age restricted to >2 years.
uid_to_exclude <- data_to_split %>% filter(age_start<2 & age_end<=2) %>% pull(uid)
data_to_split <- data_to_split %>% filter(uid %ni% uid_to_exclude)

# Psoriasis is age restricted to ages>2.
# Thus, our reference data, clinical from Poland, does not have age_start<2.
# so the pattern can not handle age_start<2. when age_start<1, we change the age_start to 1.
...
write.csv(age_splitted_data, "FILEPATH", row.names = FALSE)
write.csv(age_splitted_data,paste0(modeling_dir, "FILEPATH"))
