...
#read in the data
df_orig <- read.csv(paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical_",date,".csv"))
df_matched <- read.csv(paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical_",date,".csv"))
...
write.xlsx(df_filtered, file = paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical_",date,".xlsx"), sheetName = "extraction", overwrite = TRUE)

source("FILEPATH")
data_filepath <- paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical_",date,".xlsx")
description <- "Only marketscan and Poland claims"
