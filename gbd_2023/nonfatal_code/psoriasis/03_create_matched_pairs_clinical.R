...
uscb <- df_orig %>%
  filter(grepl("CMS", field_citation_value))
...
write.csv(df_matched, paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical_",date,".csv"), row.names=F)
write.csv(df_orig, paste0(modeling_dir, "FILEPATH",bundle_id,"_clinical_",date,".csv"), row.names=F)
