...
update_bundles <- c(6749, 276, 370, 264, 265, 274, 356:359)
bundle_df <- bundle_df[bundle_df$bundle_id %in% c(update_bundles),]
...
for (i in 1:length(bundles)){
  ...
  # these models have less data so less knots
    ...
    cov_models <- list(CovModel(
      ...
      spline = XSpline(
        knots = c(0, 40, 100), # outer knots must encompass the data
        degree = 2L, # polynomial order (1=linear, 2=quadratic, 3=cubic)
        r_linear = TRUE # add a linear tail
        ),
    ...
    cov_models <- list(CovModel(
      cov_name = "age_midpoint", 
      spline = XSpline(
        knots = c(0, 30, 60, 100), # outer knots must encompass the data
  ...
  df_pred <- subset(df_matched, select = -c(age_start.x, age_start.y, age_end.x, age_end.y))
  #df_pred <- df_matched
  
  # read in empty df of extra ages needed to predict on (e.g. 85-89, 90-94, etc.)
  df_add <- read.csv("FILEPATH/df_pred_blank.csv")
  df_add <- subset(df_add, select = -c(age_start, age_end))
  
  df_pred_add <- rbind(df_pred, df_add)
  
  df_pred_add[, c("inc2", "inc2_se", "diff", "diff_se", "data_id")] <- adjust_orig_vals(
    fit_object = fit1,       # result of CWModel()
    df = df_pred_add,            # original data with obs to be adjusted
  ...
  df_pred_add <- as.data.table(df_pred_add)
  df_pred_add[!is.na(inc_ref), trim := ifelse(fit1$w==1, "no", "yes")]
  df_pred_add[is.na(inc_ref), trim := "no"]
