# Run crosswalk for self-harm by other means
...
matches <- matches[age_midpoint < 70]
...
# Setup covariates
# # Spline on age using previous model settings
...
cov_models <- list(cw$CovModel(cov_name = "age_midpoint",
                               spline = cw$XSpline(#r_linear = TRUE,
                                                   #l_linear = TRUE,
                                                   knots = c(0, 22, 32, 42, 100), # outer knots must encompass the data
...
ggplot() + 
  ...
  ggtitle(paste0("Self-harm by other means crosswalk model results overlaid on input data, ", Sys.Date()))
...
ggplot(pred) +
  ...
  ggtitle(paste0("Self-harm by other means outpatient coefficients, ", Sys.Date()))
ggsave(paste0("FILEPATH/inj_suicide_other_", Sys.Date(), ".pdf"), width = 8, height = 6)

# Coefficients
coeffs <- pred[,.(age_group_id, age_midpoint, age_start, out_coeff)]
fwrite(coeffs, "FILEPATH/inj_suicide_other.csv")
