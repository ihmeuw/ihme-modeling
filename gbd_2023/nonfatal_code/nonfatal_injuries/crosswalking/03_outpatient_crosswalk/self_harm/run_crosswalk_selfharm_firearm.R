# Run crosswalk for self-harm by firearm
...
quantile(matches$age_midpoint, probs = seq(0, 1, length.out = 5))
...
#                                                    l_linear = TRUE,
#                                                    knots = c(0, 32, 47, 100), # outer knots must encompass the data
#                                                    degree = 2L # polynomial order (1=linear, 2=quadratic, 3=cubic)
#                                )))
# # Spline on age using old settings
# cov_models <- list(cw$CovModel(cov_name = "age_midpoint", 
#                                spline = cw$XSpline(#r_linear = TRUE,
#                                                    #l_linear = TRUE,
...
ggplot() + 
  ...
  ggtitle(paste0("Self-harm by firearm crosswalk model results overlaid on input data, ", Sys.Date()))
...
ggplot(pred) +
  ...
  ggtitle(paste0("Self-harm by firearm outpatient coefficients, ", Sys.Date()))
ggsave(paste0("FILEPATH/inj_suicide_firearm_", Sys.Date(), ".pdf"), width = 8, height = 6)

# Coefficients
coeffs <- pred[,.(age_group_id, age_midpoint, age_start, out_coeff)]
fwrite(coeffs, "FIELPATH/inj_suicide_firearm.csv")
