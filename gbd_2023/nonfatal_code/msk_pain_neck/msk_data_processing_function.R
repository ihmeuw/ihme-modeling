##########################################
##  TITLE MSK data processing functions
##  DATE  DATE  
##########################################
...
age_sex_split <- function(raw_dt){
  ...
  df_split[, ss_total:= sum(sample_size), by = c("nid", "group", "specificity", "measure", "location_id", cvs)] #identifiers
...
split_data <- function(dt, model){
  ...
  # tosplit_dt <- (parallel::mclapply(1:nrow(tosplit_dt), function(x) get_row(n = x, dt = tosplit_dt), mc.cores = 9))
...
take_off_trailing_underscore <- function(x) {
...
adjust_data<- function(dt, fit){
  dt[is.na(mean_orig), mean_orig:= mean]
  dt$case_def<- unlist(dt$case_def)
  dt$case_def<- sapply(dt$case_def, take_off_trailing_underscore)
  
  no_adjust<- dt[case_def== "" | is.na(case_def)] #rows of data I am not adjusting
  adjust<- dt[case_def!= '' & mean!= 0 &!is.na(case_def)]
  zero<- dt[case_def!= '' & mean==0] #zeros are adjusted separately
  adjust[mean== 1, mean:= .999999]
  ...
  adjust[, c("meanvar_adjusted", "sdvar_adjusted", "adjustment_logit", "adjustment_se_logit", "data_id")] <- preds1
  ...
  for (i in 1:nrow(zero)) {
    ...
    # transform unadjusted mean and SE into logit space for adjustment
    zero[case_def==def, c("logit_mean", "logit_se") := data.table(delta_transform(mean = mean + offset, sd = standard_error, transform = "linear_to_logit"))]
    
    #assign adjustment and calculate SE as the prediction SE + gamma
    adj_logit<- unique(adjust[case_def== def, adjustment_logit])
    adj_se_logit<- sqrt(unique(adjust[case_def== def, adjustment_se_logit])^2 + as.numeric(fit$gamma))
    
    # calculate datapoint SE using SE of the datapt + the prediction SE
    zero[case_def==def, logit_standard_error := sqrt(logit_se^2 + adj_se_logit^2)] 
    
    # take mean and SE into real=space to combine and make adjustments
    zero[case_def==def, c("mean_adj", "se_adj") := data.table(delta_transform(mean = logit_mean, sd = logit_standard_error, transform = "logit_to_linear"))]
    
    # assign adjustments
    zero[case_def== def, `:=` (sdvar_adjusted= se_adj,
                               note_modeler= paste0(note_modeler, ' | inflated uncertainty using prediction SE: ', " (", round(adj_se_logit, 2), ")"),
                               adjustment_logit= adj_logit, adjustment_se_logit= adj_se_logit )]
    zero[, meanvar_adjusted:= 0]
  }
  
  # bind zero data back onto dataset
  adjusted<- rbind(adjust, zero, fill= TRUE)
  #adjusted[, mean_orig:= mean]
  
  # make the adjusted values the mean values
  adjusted[, `:=` (note_modeler= paste0(note_modeler, ' | adjusted by logit difference: ', 
                                        round(adjustment_logit, 2), " (", round(adjustment_se_logit, 2), ")"), 
                   ...
                   meanvar_adjusted= NULL, sdvar_adjusted= NULL, adjustment_logit= NULL, adjustment_se_logit= NULL)]
  # bind adjusted and non-adjust data
