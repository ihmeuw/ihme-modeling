#################################################################################
#################################################################################
################# TO ESTIMATE REMISSION FOR BILIARY
#################################################################################
#################################################################################
rm(list=ls())
...
pdf(paste0(plot_output_path, "Total_biliary_raw_proAall", date, ".pdf"),  width=20, height=8) # this will be the name of the file
p <- 1
...
age_dt <- read.csv(paste0(FILEPATH))
...
year_dt <- read.csv(paste0(FILEPATH))
...
graph_rem1
ggsave(graph_rem, filename = paste0(plot_output_path, "biliary_reg4_", date,"_no_dummyzero.pdf"), width = 10, height = 7)
ggsave(graph_rem1, filename = paste0(plot_output_path, "biliary_reg4_", date,"_include_dummyzero.pdf"), width = 10, height = 7)
...
final_rem$sex <- ifelse(final_rem$sex_binary == 1, "Female", "Male")
...
final_rem$note_modeler <- "These data are modeled from the US data using the number of people with procedures codes among those with ICD code of interest,fake zeros at (0, 0) for HAQi"
...
write.csv(final_rem, paste0(plot_output_path, "Total_biliary_modeled_remission_", date, ".csv"))
