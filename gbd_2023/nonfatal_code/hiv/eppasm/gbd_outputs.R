...
split_u1.new_ages <- function(dt, loc, run.name, gbdyear, test_run = NULL, loc_name = loc, inherits = TRUE){
  ...
  pop <- data.table(fread(paste0('FILEPATH/',gbdyear,"/" ,"240906_quokka", "/population_splits/", loc_name, '.csv')))
  ...
  ## pull in incidence proportions from eppasm
    ...
    if(!file.exists(paste0('FILEPATH/', loc, '.csv'))){
      split.dt <- fread(paste0('FILEPATH', loc_name, '_under1_splits.csv'))
      
    }else{
      if(grepl('_', loc_name)){
        split.dt <- fread(paste0('FILEPATH/', gbdyear, '/', run.name ,'/compiled/', loc_name, '_under1_splits.csv'))

      }else{
        split.dt <- fread(paste0('FILEPATH/', gbdyear, '/', run.name ,'/compiled/', loc, '_under1_splits.csv'))
...
split_u1 <- function(dt, loc, run.name, gbdyear="gbd20"){
  ...
  split.dt <- fread(paste0('FILEPATH/', run.name ,'/compiled/', loc, '_under1_splits.csv'))
  split.dt <- melt(split.dt, id.vars = c('year', 'run_num'))
  setnames(split.dt, 'variable', 'age')
  spec_u1 <- merge(spec_u1, split.dt, by = c('year', 'run_num', 'age'))
  pop_weight_inc <- function(x) return(x*spec_u1[['value']])
...
get_summary <- function(loc, run.name, paediatric = FALSE, old.splits, gbdyear){
 output <- fread(paste0('FILEPATH/', gbdyear, '/',run.name, '/compiled/', loc, '.csv'))
 ...
 output[, pop_art:= as.numeric(pop_art)][,pop_gt350:= as.numeric(pop_gt350)][,pop_200to350:= as.numeric(pop_200to350)][, pop_lt200:= as.numeric(pop_lt200)]
  ...
  if(old.splits){
    age.map <- fread(paste0('FILEPATH/', run.name, "/age_map.csv"))
    
  }else{
    age.map <- fread(paste0("FILEPATH/age_map.csv"))
  ...
  dir.create(paste0('FILEPATH/summary_files/'), recursive = T)
  write.csv(out.dt, paste0('FILEPATH/summary_files/', loc, '.csv'), row.names = F)
  return(out.dt)
}
...
save_data <- function(loc, eppd, run.name){
  ...
  if(nrow(eppd$hhs) > 0){
    ...
    if(exists('agegr', where = prevdata)){
      ...
      prevdata.agg2 <- prevdata[agegr == '15-64']
      prevdata.agg2[, age_group_id := 200]
      prevdata.agg2[, age := '15-64']
      prevdata <- prevdata[!agegr == '15-49']
      prevdata <- prevdata[!agegr == '15-64']
      ...
      prevdata <- rbind(prevdata, prevdata.agg2, use.names = T)
      prevdata[, agegr  := NULL]
    }
  }else{
    prevdata <- NULL
