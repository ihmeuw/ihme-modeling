...
if sys.argv[1]=='-f':
    ...
    art_folder = sys.argv[9]
    inc_folder = sys.argv[10]
...
import BeersInterpolation2 as beers

cohort_groups = ['1B', '2A', '2B']

if  group in cohort_groups:
    inc_adj_config = 1
    cohort_output = True
else:
    inc_adj_config = 1
...
result_path = 'FILEPATH' + out_folder + '/'
print(result_path)
    
# Paths
directory = 'FILEPATH/' + folder
demProjFilePath = "FILEPATH/" + folder + "/"
...
for c in range(len(possibleCD4categories)):
...
death_vars = ['d_' + str(i) for i in range(minYear, maxYear+1)]
...
def getPredARTCoverageCounts(popByCD4, predARTCoverage, t, currentYearART, noARTCD4states):
    ...
    for sex in list(ARTCoverageCounts.keys()):
...
def readPredARTCoverage(ISO):
    ...
    for row in caps_data[1:]:
        ...
        if predARTYear in list(predARTCoverage.keys()):
            if predARTAge in list(predARTCoverage[predARTYear].keys()):
                if predARTSex == 1:
...
#TF - part of child update Aug 2017
...
# Epidemic Type
inputEpidemicTypeData = beers.openCSV('FILEPATH/epi_class_2020.csv')
...
# Epidemic Start Year
try:
    inputEpidemicStartYearData = beers.openCSV('FILEPATH/epi_start_year.csv')
...
# HIV-free mortality
inputSRdata = beers.openCSV('FILEPATH/locs/' + ISO + "_life_tables.csv")
...
inputOnARTScalar = pd.read_csv("FILEPATH/artcc_rr.csv")
...
# Adult ART Coverage
if grid == '1':
  inputAdultARTCoverage = beers.openCSV(demProjFilePath  + '/iterate_adultART/' +  art_folder + '/' + ISO + ".csv")
else:
...
if stage != 'stage_1':
    inputAdjInc = beers.openCSV("FILEPATH/" + folder + "_adj" + '/' + ISO + "_SPU_inc_draws.csv")
    
if group[0] == '2' and stage == 'stage_1':
  inputAdjInc = beers.openCSV('FILEPATH/' + ISO + '.csv')

## If grid selection run, the gridFilepath equals the demoProjfilePath
if group[0] == '2' and stage == 'stage_1' and grid == '1':
  inputAdjInc = beers.openCSV(demProjFilePath + '/iterate_incidence/' + inc_folder + "/" + ISO + '.csv')

if group[0] == '2' and stage == 'stage_1' and grid == '0':
    inputAdjInc = beers.openCSV(gridFilePath + '/iterate_incidence/' + inc_folder + "/" + ISO + '.csv')
...
# TF - March 2018 - country - specific inc sex ratio based on prevalence surveys
if group[0] == '1':
    if parent != 'IND':
        inputIncSexRatio = beers.openCSV("FILEPATH/" + ISO[:3] + '.csv')
    else:
        inputIncSexRatio = beers.openCSV("FILEPATH/" + ISO + '.csv')
    sexRatioAdj = 1
else:
...
seed_dir = '140825'
seed_loc = ISO
seed_path = '%sseeds/%s/%s_seeds.csv' % ("FILEPATH/", seed_dir, seed_loc)
try:
    with open(seed_path, 'r') as f:
        f_reader = csv.reader(f)
        seed_data = [row for row in f_reader]
except:
    seed_path = '%sseeds/%s/%s_seeds.csv' % ("FILEPATH/", seed_dir, "DEF")
    with open(seed_path, 'r') as f:
        f_reader = csv.reader(f)
        seed_data = [row for row in f_reader]
...
for i in range(len(inputSRdata)):
...
for age in range(0,15):
    ...
    # run_id = 1
for run_id in range(indiv_run_nums):
    ...
    for obj in [population, deaths, AIDSdeaths, needForART]:
        for i in range(minAge, maxAge + 1):
            obj[i] = {}
            for j in range(minYear, maxYear + 1):
                obj[i][j] = {}
                for k in sexes:
                    if obj == population:
                        obj[i][j][k] = {}
                        if i > childMaxAge:
                            for l in noARTCD4statesNeg:
                                obj[i][j][k][l] = []
                                for m in range(minYear, maxYear + 1):
                        ...
                        obj[i][j][k] = []
                        for l in range(minYear, maxYear + 1):
    ...
    if iteration_number == 0:
        adj_array = [1 + (normBounds(-10,10)/100) for i in range(len(retentionRateCountValues))]
        adjustments['retentionRateCountValues'] = adj_array
    else:
        adj_array = adjustments['retentionRateCountValues']
    retentionRateCountValues = [retentionRateCountValues[i] * adj_array[i] for i in range(len(retentionRateCountValues))]      
    retentionRateCounts = {}

    retentionRatePercentValues = [0.906, 0.759, 0.787, 0.795, 0.785, 0.756, 0.813, 0.700]
    if iteration_number == 0:
        adj_array = [1 + (normBounds(-10,10)/100) for i in range(len(retentionRatePercentValues))]
        adjustments['retentionRatePercentValues'] = adj_array
    else:
        adj_array = adjustments['retentionRatePercentValues']
    retentionRatePercentValues = [retentionRatePercentValues[i] * adj_array[i] for i in range(len(retentionRatePercentValues))]
    retentionRatePercents = {}
    for i in range(len(possibleCD4categories)):
    ...
    if iteration_number == 0:
        adj_array = [1 + (normBounds(-10,10)/100) for i in range(len(noART15CD4distValues))]
        adjustments['noART15CD4distValues'] = adj_array
    else:
        adj_array = adjustments['noART15CD4distValues']
    tmpNoART15CD4distValues = [noART15CD4distValues[i] * adj_array[i] for i in range(len(noART15CD4distValues))]
    noART15CD4distValues = [val / np.sum(tmpNoART15CD4distValues) for val in tmpNoART15CD4distValues]
    noART15CD4dist = {}
    for i in range(len(noART15CD4distValues)):
    ...
    tmpART15CD4distValues = [ART15CD4distValues[i] * adj_array[i] for i in range(len(ART15CD4distValues))]
    ART15CD4distValues = [val / np.sum(tmpART15CD4distValues) for val in tmpART15CD4distValues]
    ART15CD4dist = {}
    for i in range(len(ART15CD4distValues)):
    ...
    if int(inputPopData[2][ageIndexPop]) != int(inputPopData[0][ageIndexPop]) + 2:
        ...
        if iteration_number == 0:
            adj_object = [{k: 1 + (normBounds(-10,10)/100) for k in sexes} for a in range(0,81)]
    ...
    if maxYear > last_sr_year:
        needed_years = list(range(last_sr_year+1, maxYear + 1))
    ...
    for sex in sexes:
        noARTmortality[sex] = []
        for i in range(len(progressionAges)):
    ...
    for sex in sexes:
        ...
        for d in adultARTdurations:
            ...
            for j in range(len(mortalityAgeCategories)):
    ...
    CD4lowerLimits = {}
    for i in range(len(noARTCD4states)):
    ...
    TFRreduction = [float(inputTFRreduction[1:][i][valueIndex]) * adj_array[i] for i in range(len(inputTFRreduction[1:]))]
    for i in range(len(TFRreduction)):
        if TFRreduction[i] == 0:
            TFRreduction[i] = 1.0
    ...
    coverageFillerData = [0 for i in range(minYear, firstYear)]
    ...
    for sex in adultARTCoverage:
        adj = adj_object[sex]
        for i in range(len(adultARTCoverage[sex])):
    ...
    for p in treatTypes:
        treat_data_type[p] = []
        filler = [0 for i in range(minYear, startYear)]
        col_nums = [i for i in range(len(inputPMTCT[0])) if p in inputPMTCT[0][i]]
        pct_col = [i for i in range(len(inputPMTCT[0])) if inputPMTCT[0][i] == p+'_pct'][0]
        num_col = [i for i in range(len(inputPMTCT[0])) if inputPMTCT[0][i] == p+'_num'][0]
    ...
    for p in treat_data_type:
        filler = ['num' for i in range(minYear, startYear)]
    ...
    for treatType in prenatalProph:
        ...
        for y in range(len(prenatalProph[treatType])):
    ...
    if iteration_number == 0:
        adj_array = [1 + normBounds(-10,10)/100 for i in range(len(tmpNoARV))]
        adjustments['percentBFnoART'] = adj_array
    else:
        adj_array = adjustments['percentBFnoART']
    percentBFnoARTvalues = {i+1: min(100, val * adj_array[i]) for i, val in zip(range(len(tmpNoARV)), tmpNoARV)}

    if iteration_number == 0:
        adj_array = [1 + normBounds(-10,10)/100 for i in range(len(tmpOnARV))]
        adjustments['percentBFonART'] = adj_array
    else:
        adj_array = adjustments['percentBFonART']       
    percentBFonARTvalues = {i+1: min(100, val * adj_array[i]) for i, val in zip(range(len(tmpOnARV)), tmpOnARV)}
    percentBFonART = [percentBFonARTvalues for i in range(minYear, maxYear+1)]
    percentBFnoART = [percentBFnoARTvalues for i in range(minYear, maxYear+1)]
    ...
    for i in range(4):
        eligByAge.append({})
        for c in range(len(noARTCD4states)):
    ...
    #If stage 2 or 3, we read in year/sex/age - specific incidence
    #if stage == 'stage_2' or group[0] == '2': 
    if stage != 'stage_1' or group[0] == '2': 
    ...
    # TF - March 2018 - splitting sex ratio of incidence based on prevalence surveys for generalized epidemics
    ...
    if group[0] != '1':
        ...
        incSexRatio = [.24 * sexRatioAdj for i in range(minYear, maxYear + 1)]
    else:
        ...
        incSexRatio = [startYearRatio for i in range(minYear, maxYear + 1)]
    ...
    for y in range(epidemicStartYear, maxYear + 1):
    ...
    if iteration_number == 0:
        adj_array = [1 + normBounds(-10,10)/100 for a in range(len(initCD4dist))]
        adjustments['initCD4dist'] = adj_array
    else:
        adj_array = adjustments['initCD4dist']
    for s in sexes:
        for a in range(len(initCD4dist)):
    ...
    if iteration_number == 0:
        adj_object = [{k: 1 + normBounds(-10,10)/100 for k in ['perinatal', 'BFLT350', 'BFGE350']} for i in range(len(inputMTCtransRates[1:]))]
        adjustments['MTCtransRates'] = adj_object
    else:
        adj_object = adjustments['MTCtransRates']

    for i in range(len(inputMTCtransRates[1:])):
    ...
    for i in range(minYear + 1, maxYear + 1):
        childCD4countDist[i] = []
    for j in range(len(childCD4countDist)):
        for i in range(len(inputChildCD4countDist)):
            childCD4countDist[j + minYear + 1].append({})
            for c in range(len(inputChildCD4countDist[i][1:])):
                if j == 0:
    ...
    if iteration_number == 0:
        adj_object = [{k: 1 + normBounds(-10,10)/100 for k in childCD4countDist[minYear+1][a]} for a in range(len(childCD4countDist[minYear+1]))]
        adjustments['childCD4countDist'] = adj_object
    else:
        adj_object = adjustments['childCD4countDist']

    for a in range(len(childCD4countDist[minYear+1])):
        tmpDist = {}
    ...
    for j in range(len(childCD4percentDist)):
        for i in range(len(inputChildCD4percentDist)):
            childCD4percentDist[j + minYear + 1].append({})
            for c in range(len(inputChildCD4percentDist[i][1:])):
                if j == 0:
    ...
    if iteration_number == 0:
        adj_object = [{k: 1 + normBounds(-10,10)/100 for k in childCD4percentDist[minYear+1][a]} for a in range(len(childCD4percentDist[minYear+1]))]
        adjustments['childCD4percentDist'] = adj_object
    else:
        adj_object = adjustments['childCD4percentDist']

    for a in range(len(childCD4percentDist[minYear+1])):
        tmpDist = {}
    ...
    for i in range(len(orderedAges)):
    ...
    childARTcoverage['CTX'].extend([float(inputChildARTCoverage[1:][i][CTXnumIndex]) * adj_object['num'][i] + min(100,float(inputChildARTCoverage[1:][i][CTXpctIndex]) * adj_object['pct'][i]) for i in range(len(inputChildARTCoverage[1:]))])
    ...
    childARTcoverage['ART'].extend([float(inputChildARTCoverage[1:][i][ARTnumIndex]) * adj_object['num'][i] + min(100,float(inputChildARTCoverage[1:][i][ARTpctIndex]) * adj_object['pct'][i]) for i in range(len(inputChildARTCoverage[1:]))])

    childARTcoverageType = {}
    for i in ['CTX', 'ART']:
        childARTcoverageType[i] = ['num' for x in range(minYear, firstYear)]
    ...
    for k in childCTXeffect:
        for a in range(len(childCTXeffect[k])):
    ...
    for a in range(0, maxAge+1):
        for s in list(sex_nums.values()):
            tmp_row = [individual_id, t, a, s]
            out_deaths = [0.0 for i in range(minYear, maxYear+1)]
            tmp_row.extend(out_deaths)
            cohortData.append(tmp_row)

    tmpPop = {}
    for sex in sexes:
        for age in range(0, 15, 5):
            tmpBirthPrev = 0
            popData = []
            tmpNewHIV = 0
            tmpDeaths = 0
    ...
    #DJ - if stage 2, no child adjustment for 1-4s (already in the file with ratios of 1), Ned could potentially make this extension in the python code                
    if stage == 'stage_2' and parent != 'IND':
        childIncAdjPD = pd.read_csv("FILEPATH/no_adjustment.csv")
    ...
    #DJ - adding third stage child adjustment for 1-4 to avoid double scaling
    if stage == 'stage_3' and parent != 'IND':
        childIncAdjPD = pd.read_csv("FILEPATH/" + folder + "_ratios/"+ISO+"_inc_ratios.csv")
        childIncAdjPD = childIncAdjPD[['sex','single.age','year','ratio_'+str(individual_id)]]
        childIncAdjPD = childIncAdjPD.rename(columns = {'ratio_'+str(individual_id) : 'value'})
        childIncAdj = {}
        for year in range(minYear, maxYear + 1):
    ...
    for t in range(minYear + 1, maxYear + 1):
    #     t = minYear + 1
        print(t)
        newChildHIV = {}
        for sex in sexes:
        ...
        for sex in sexes:
            for age in population: 
                ...
                if age > 0 and age < max(population):
                    ...
                    #TF - 2/23/17 - Haidong wants to cap migration at 80% of pop
        ...
        for sex in sexes:
            for age in range(childMaxAge + 1, maxAge + 1):
                ARTneed = 0
                for c in noARTCD4states:
                ...
                if 'pregnant_women' in eligiblePops:
                    if sex == 'female' and age in range(15, 50) and eligiblePopYears['pregnant_women'] <= t:
                ...
                for age in range(15, maxAge+1):
                    for c in noARTCD4states:
                        if CD4lowerLimits[c] < adultARTeligibility[t-minYear]:
                            ...
                            if sex == 'female' and age in range(15, 50):
        ...
        for t1 in range(1, 11):
            for sex in sexes:
                ...
                for age in range(childMaxAge + 1, maxAge + 1):
                    ...
                    tmp_noARTpop = 0
                    tmp_total_share = 0
                    for c in reversed(range(len(noARTCD4states))):
                        tmp_new_art = 0
                        tmp_noARTpop_2 = 0
                        ## Dirk and Jeff want to decrease off ART mortality based on current treatment levels
                        ## TF April 2018
        ...
        for sex in sexes:
            ...
            if stage == "stage_1":
                tempSum = 0
                pop5 = []
                ageRange = list(range(15,80,5))
                ageRange.append(80)
                for a5 in range(15, 50, 5):
                    pop5.append(0)
                    for age in range(a5, a5 + 5):
                ...
                # I couldn't get the interpolation-first method from the Spectrum source
                # code to work, so I'm interpolating *after* allocating to groups.
        ...
        ## TF - March 2018 - trying to use CIBA to only inform age split, prevalence surveys informing sex split
        if stage != "stage_1" and parent == "IND":
            for sex in sexes:
                pred_inc = 0
                pop_neg = 0
                for age in range(15, 50):
        ...
        for sex in sexes:            
            ...
            for age5 in range(15, maxAge, 5):
                newHIVbyAge5[sex][age5] = 0
                popBy5[sex][age5] = 0
                for a in range(age5, age5 + 5):
        ...
        for age5 in range(15, 49, 5):
            HIVwomen = 0 
            negWomen = 0
            ARTthisYear = 0
            ARTprevYear = 0
            age5c = age5//5 - 3
            for age in range(age5, age5 + 5):
        ...
        # Apply PMTCT
        percent_treatments = [p for p in list(prenatalProph.keys()) if PMTCTtype['prenat'][p][t-minYear] == 'percent']
        num_treatments = [p for p in list(prenatalProph.keys()) if PMTCTtype['prenat'][p][t-minYear] == 'num']
        ...
        percent_treatments = [p for p in list(prenatalProph.keys()) if PMTCTtype['prenat'][p][t-minYear] == 'percent']
        num_treatments = [p for p in list(prenatalProph.keys()) if PMTCTtype['prenat'][p][t-minYear] == 'num']
        percent_treatments_BF = [p for p in list(postnatalProph.keys()) if PMTCTtype['postnat'][p][t-minYear] == 'percent']
        num_treatments_BF = [p for p in list(postnatalProph.keys()) if PMTCTtype['postnat'][p][t-minYear] == 'num']
    ...
    #                     + population[age][t]['both']['asymBFGT12Mo']['all'][i-minYear])
        ...
        for i in range(minYear, t + 1):
            #TF - this code seems incorrect in DPPROJ.PAS? line 4925
            #I wrote this how I think it should be
            #Also should we add CTX to this?
        ...
        if temp > 0:
            adj = newFLART / temp
        #TF - need to revisit this - currently updating population object here to move children from off-ART to on-ART
        for age in range (0, 15):
            age1 = monthConcordance(age)
            for sex in sexes:
                for d in noARTChildHIVStates:
    ...
    #                                         population[age][t][sex][c][d][i - minYear] = 0
        # Get CTX coverage
        #TF - DPPROJ.PAS does this slightly differently (using whether or not infants were tested with PCR)
        #For now, only doing off-ART under-5 plus eligibile for ART 5-15
            ...
            for age in range(0, childMaxAge):
                ...
                for i in range(minYear, t + 1):
                    for c in CD4States:
                        ...
                        #TF - consider changing these variable names later - took these directly from DPPROJ.PAS
...
if grid == '0':
  result_path = '/ihme/hiv/spectrum_draws/' + out_folder + '/'
