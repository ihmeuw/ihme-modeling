...
inc.data <- data.table()
for (tmp.loc in loc) {
	tmp.inc.data <- fread(paste0(in.dir,tmp.loc,'_ART_data.csv'))
...
inc.ratio.dir <- paste0('FILEPATH', base_run, '_ratios/')	##Use the ratios from the base run
...
for (loc.select in unique(source.table$loc)) {
  ...
  if(file.exists(paste0(inc.ratio.dir, loc.select, "_inc_ratios.csv"))){
...
out.dir_ratio <- paste0("/FILEPATH/", run.folder, "_ratios/")
