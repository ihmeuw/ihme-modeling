########################################################
## This script uses an MR-BRT model to sex split the
...
duplicate_sex_rows <- function(df) {
  ...
  output <- rbindlist(list(sex_specific, males, females), fill = TRUE)
...
scalar_sex_rows <- function(df, scalar="", denominator_weights, bounded=T, title="", global_merge = T) {
  ...
  output <- rbind.fill(sex_specific, males, females)
