...
# dmod_global <- get_outputs(topic="cause", cause_id=302, age_group_id = 22, year_id=2017, location_id=1, sex_id=c(1,2), measure_id=5, metric_id=1, gbd_round_id=5)
# dmod_global$number <- dmod_global$val
# denominator_weights <- dmod_global
#
# test <- sex_split_mrbrt_weights(df, denominator_weights, output_dir, title="Adenovirus")
...
sex_split_mrbrt_weights <- function(df, denominator_weights, output_dir, bounded=T, title="", global_merge = T){
  ...
  ## I don't know how to get a funnel plot, produce an approximation of one
  ...
  ## Right now, I don't have age-specific sex ratios for the denominators ##
