...
for(m in unique(eti_meta$modelable_entity_id)){ 
  ...
  # Duplicate to 1000 draws
  # yll_draws[,paste0("draw_",500:999) := lapply(0:499, function(x) {get(paste0("draw_", x))})]
  # for(i in 1:9) {
  #   yll_draws <- yll_draws[, paste0("draw_",(100*i):(100*i+99)) := lapply(0:99, function(x) {get(paste0("draw_", x))})]
  # }
  ...
  # Duplicate due to como having only 100 draws
  # for(i in 1:9) {
  #   yld_draws <- yld_draws[, paste0("draw_",(100*i):(100*i+99)) := lapply(0:99, function(x) {get(paste0("draw_", x))})]
  # }
