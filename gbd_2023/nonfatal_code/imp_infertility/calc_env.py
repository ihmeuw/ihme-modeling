...
class SexEnvelopeMkr(object):
    ...
    def __init__(self, male_prop_id, female_prop_id, exp_id, env_id, year_id,
        ...
        self.male_prop = self.import_sex_prop(male_prop_id, 1, decomp_step)
        self.female_prop = self.import_sex_prop(female_prop_id, 2, decomp_step)
        self.scale_props()

        # import exposure and env
        self.exposure = self.import_exposure(exp_id, decomp_step)
        self.envelope = self.import_couples(env_id, decomp_step)

    def import_sex_prop(self, modelable_entity_id, sex_id, decomp_step):
        ...
        broadcast_idx = pd.MultiIndex.from_product(
            list(broadcast_idx_dmnsns.values()),
            names=list(broadcast_idx_dmnsns.keys()))
        ...
        tmp_df = importer.import_square(
            gopher_what={"modelable_entity_id": modelable_entity_id},
            source="epi",
            decomp_step=decomp_step)

        # broadcast to preferred shape
        broadcast_over = list(broadcast_idx_dmnsns.keys())
        ...
        df = pd.merge(broadcast_df.reset_index(), tmp_df, on=broadcast_over,
                      how="left").set_index(list(broadcast_idx_dmnsns.keys()))
        df.fillna(value=0, inplace=True)
        return df

    def import_exposure(self, modelable_entity_id, decomp_step):
    ...
    def import_couples(self, modelable_entity_id, decomp_step):
...
def calculate_envs(male_prop_id, female_prop_id, exp_id, env_id, male_env_id,
                   female_env_id, year_id, out_dir, decomp_step):
