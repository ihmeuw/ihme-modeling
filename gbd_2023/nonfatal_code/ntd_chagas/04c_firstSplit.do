...
\******************************************************************************/
    ...
    * PULL IN DRAWS FROM DISMOD MODELS FOR INCIDENCE AND PREVALENVCE *
      if `endemic'==0 {
	    ...
	    get_draws, gbd_id_type(ADDRESS) gbd_id(`ADDRESS') source(ADDRESS) location_id(`location') age_group_id(`ages') measure_id(5) release_id(`ADDRESS') decomp_step(`ADDRESS') status(ADDRESS) clear
        rename draw_* prev_*
		capture generate metric_id = 3
		}
  ...
  get_draws, gbd_id_type(ADDRESS) release_id(`ADDRESS') gbd_id(`ADDRESS') source(ADDRESS) location_id(`location') age_group_id(`ages') measure_id(6) status(ADDRESS) clear
