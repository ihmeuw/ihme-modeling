...
*** GET DEMOGRAPHICS AND POPULATION ESTIMATES ***	
	get_demographics, gbd_team("ADDRESS") release_id(`ADDRESS') clear
	local years `r(year_id)'
	local ages `r(age_group_id)'
	local sexes `r(sex_id)'

	get_population, location_id ("all") age_group_id(`ages' 22) sex_id(`sexes' 3) year_id(`years') release_id(`ADDRESS') clear
...
*** PULL IN CHAGAS MORTALITY ESTIMATES & SUM BY LOCATION ***  
	...
	get_outputs, topic("ADDRESS") location_id("all") year_id("all") cause_id(ADDRESS) release_id_id(ADDRESS) clear version(ADDRESS)
	...
	merge 1:1 location_id using `endemic', keep(3) nogenerate
...
*** BRING IN POPULATION AT RISK DATASET ***  
	...
	merge m:1 iso3 year_id using FILEPATH, assert(2 3) keep(3) nogenerate
	drop iso3
...
*** BREAKDOWN US NUMBERS BY STATE ***
	...
	joinby mergeRegion using FILEPATH
