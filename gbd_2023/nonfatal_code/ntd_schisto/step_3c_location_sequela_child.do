...
**********************************************************************************************************************	
	...
	** pull in haematobium and mansoni specific draws if there is coinfection
	if `split' == 1 {
		...
		get_draws, gbd_id_type(model_id) gbd_id(ADDRESS) source(ADDRESS) measure_id(5) location_id(`location') status(best) release_id(`release_id') clear
...
** pull in population data
	** fix age groups 
	numlist "2 3 6/20 30/32 34 235 238 388 389"
	local ages = "`r(numlist)'"
    ** get the population estimates
	get_population, location_id(`location') sex_id("1 2") age_group_id("`ages'") year_id("`years'") release_id(`release_id') clear               
...
**********************************************************************************************************************
	** get draws
	get_draws, gbd_id_type(model_id) gbd_id(ADDRESS) measure_id(18) location_id(1) year_id(2000) source(epi) status(best) release_id(`release_id') clear
	...
	get_covariate_estimates, covariate_id(ADDRESS) status(best) release_id(`release_id') clear
	keep if location_id == `location'
...
** loop through sequelae
	foreach seq in dia bla hep dys hyd hem asc {
		...
		** replace with 0s for younger age groups, if age < 15
		if inlist("`seq'","hep","hem","asc") {
			forvalues i = 0/999 {
				...
				quietly replace draw_`i' = 0 if age_group_id ==34
			}
		}
		...
		**recode age_group_id 34 as 5 to enable logic in age group restrictions to function below
		...
		foreach sex of local sexes {
			...
			if inlist("`seq'", "hep", "hem", "asc") {
				...
				forvalues i = 0/999 {
					qui replace draw_`i' = draw_`i' * (1 - mean_value/5) + draw_`i'[_n+1] * mean_value/5 * .9^5 if age_group_id >= 8 & age_group_id <= 235
					qui replace draw_`i' = 1 if draw_`i' > 1
				}
				...
				keep if year_id == 2005
				forvalues i = 0/999 {
					qui replace draw_`i' = draw_`i' * (1 - mean_value/10) + (4*draw_`i'[_n+2]/5 + 1*draw_`i'[_n+3]/5) * mean_value/10 * .9^10 if age_group_id >= 8 & age_group_id <= 235
					qui replace draw_`i' = 1 if draw_`i' > 1
				}
				replace year_id = 2015
				tempfile `seq'_2015_`sex'
				save ``seq'_2015_`sex'', replace

				restore, preserve
				gsort -age
				keep if year_id == 2005
				forvalues i = 0/999 {
					qui replace draw_`i' = draw_`i' * (1 - mean_value/15) + (4*draw_`i'[_n+2]/5 + 1*draw_`i'[_n+3]/5) * mean_value/15 * .9^15 if age_group_id >= 8 & age_group_id <= 235
					qui replace draw_`i' = 1 if draw_`i' > 1
				}
				replace year_id = 2019
				tempfile `seq'_2019_`sex'
				save ``seq'_2019_`sex'', replace

				restore, preserve
				gsort -age
				keep if year_id == 2005
				forvalues i = 0/999 {
					qui replace draw_`i' = draw_`i' * (1 - mean_value/16) + (4*draw_`i'[_n+2]/5 + 1*draw_`i'[_n+3]/5) * mean_value/16 * .9^16 if age_group_id >= 8 & age_group_id <= 235
					qui replace draw_`i' = 1 if draw_`i' > 1
				}
				replace year_id = 2020
				tempfile `seq'_2020_`sex'
				save ``seq'_2020_`sex'', replace

				restore, preserve
				gsort -age
				keep if year_id == 2005
				forvalues i = 0/999 {
					qui replace draw_`i' = draw_`i' * (1 - mean_value/17) + (4*draw_`i'[_n+2]/5 + 1*draw_`i'[_n+3]/5) * mean_value/17 * .9^17 if age_group_id >= 8 & age_group_id <= 235
					qui replace draw_`i' = 1 if draw_`i' > 1
				}
				replace year_id = 2021
				tempfile `seq'_2021_`sex'
				save ``seq'_2021_`sex'', replace

				restore, preserve
				gsort -age
				keep if year_id == 2005
				forvalues i = 0/999 {
					qui replace draw_`i' = draw_`i' * (1 - mean_value/18) + (4*draw_`i'[_n+2]/5 + 1*draw_`i'[_n+3]/5) * mean_value/18 * .9^18 if age_group_id >= 8 & age_group_id <= 235
					qui replace draw_`i' = 1 if draw_`i' > 1
				}
				replace year_id = 2022
				tempfile `seq'_2022_`sex'
				save ``seq'_2022_`sex'', replace

				restore
				drop if year_id == 2010 | year_id == 2015 | year_id == 2019 | year_id == 2020 | year_id == 2021 | year_id  == 2022
				
				append using ``seq'_2010_`sex''
				append using ``seq'_2015_`sex''
				append using ``seq'_2020_`sex''
				append using ``seq'_2022_`sex''
				append using ``seq'_2023_`sex''
				append using ``seq'_2024_`sex''
			}
			tempfile draws_`sex'
		...
		** fix back the age_group_id fix
		replace age_group_id = 34 if age_group_id == 5
...
foreach seq in dia hep hem asc dys hyd bla {
	merge 1:1 age_group_id year_id sex_id using ``seq'_draws', nogen keep(matched)
	forvalues i = 0/999 {
