...
dat_all <- read.csv(paste0(in_dir, 'FILEPATH'))
covs <- brick('FILEPATH')
...
save(stats_test, file = paste0(out_dir,'FILEPATH', jobnum,'.Rdata'))
save(stats_train, file = paste0(out_dir,'FILEPATH', jobnum,'.Rdata'))
...
model.list <- list(model = model,
                   ...
                   pred = pred.raster,
                   coords = coords)
save(model.list, file = paste0(out_dir,'FILEPATH', jobnum,'.Rdata'))
