...
tabler <- function(matrix, group){
  #takes the matrix (i.e., rnorm output) and forms a table with col names and label
...
make_cfr_draws <- function(scalar_file, epi_demo, draws_dir){
  ...
  cat("cfr skeleton created. drawing 1,000 times from a normal curve.")
  ...
  fwrite(cfr_param, paste0(draws_dir, FILEPATH), row.names = FALSE)
  cat("Created evd_cfr_draws file")
}
...
make_acute_draws <- function(scalar_file, draws_dir){
  ...
  cat("Created acute_duration_draws file!")
}
...
make_chronic_draws <- function(scalar_file, draws_dir){
  ...
  evd_data <- evd_data[study != 'Wendo_2001']    #exclude Wendo 2001 data due to limited metadata
  ...
  #as per spreadsheet work out duration
  years          <- seq(0, 20, 0.05)
  ...
  write.csv(year1, paste0(draws_dir, "FILEPATH","year1.csv"))
  write.csv(year2, paste0(draws_dir, "FILEPATH","year2.csv"))
  
  cat("Created chronic draws!")
}
...
make_underreport_draws <- function(scalar_file, draws_dir){
  ...
  fwrite(under_cases_draw, paste0(draws_dir, "FILEPATH"))
  ...
  fwrite(under_deaths_draw, paste0(draws_dir, "FILEPATH"))
  cat("Created underreporting draws!")
}
...
#' [make draw files with all zero's]
...
calc_as_proportion <- function(data){
  ...
  sex_age  <- data[value_age_group_id != 22]
  tot      <- sum(sex_age[, cases])
  proportions   <- sex_age[, .("proportions" = sum(cases)/tot), by = c("value_age_group_id", "sex_id")]
...
calc_ss_proportion <- function(data, s_id){ 
  ...
  sex_age  <- data[value_age_group_id != 22 & sex_id == s_id]
  tot      <- sum(sex_age[, cases])
  proportions   <- sex_age[, .("proportions" = sum(cases)/tot ), by = c("value_age_group_id")]
...
as_age_split <- function(all_age_data, all_age_proportions){
  ...
  for (obs in 1:nrow(all_age_data)){
    ...
    rows[, c("sex_id", "value_age_group_id") := NULL]
    rows <- cbind(rows, all_age_proportions)
    rows[, cases  := as.numeric(cases) * proportions]
    if (obs == 1){
      split <- rows
    } else {
...
ss_age_split <- function(ss_all_age_data, ss_age_proportions){
  ...
  for (obs in 1:nrow(ss_all_age_data)){
    ...
    rows[, c("value_age_group_id") := NULL]
...
inserter <- function(zero_draw_file, new_data, loc, m_id){
  ...
  for (ind in 1:nrow(rows)){
    ...
    csv[age_group_id == row[, age_group_id] &             #updating csv as go
          sex_id == as.character(row[, sex_id]) & 
...
cases_to_prev <- function(data, duration, pop_decomp_step, round_id){
  ...
  # change year id 1994:2019 to blank
  pop  <- get_population(age_group_id = "all", year_id = unique(data[,year_id]), release_id = release_id, sex_id = c(1,2), location_id = as.character(data[, location_id]))
