...
loc_dt <- get_location_metadata(location_set_id = OBJECT)
super_region_dt <- loc_dt[, .(location_id, super_region_id, super_region_name)]
age_dt <- get_age_metadata(OBJECT)
...
subnat_to_nat <- function(subnat_dt, subnat){
  ...
  if (subnat == T) {
    ...
    dt[level == 6, loc_match := 4749]
    dt[level == 5 & grepl("GBR", ihme_loc_id), loc_match := 4749] 
    dt[level == 5 & location_type %in% c("urbanicity", "admin2"), loc_match := 163] 
