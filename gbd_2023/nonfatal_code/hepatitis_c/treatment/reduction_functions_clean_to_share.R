## #############################################################################
## HEPATITIS C TREATMENT REDUCTION
## #############################################################################
...
apply_reduction <- function(df) {
  ...
  # df[, total_treated:=.I] # for testing purposes to track shift; comment out when running
  
  num_shifts <- length(unique(df$year_id)) - 1
  #df[1:10, 1:10]
  
  if(num_shifts > 35) stop('Need to expand age shift logic')
  # Shift years 
  ...
  # Shift age groups by cohorts; adding 3 years in each group (rationale, we are assuming that in age framework on average everyone spends 3 years of time in each age group; this assumption could be improved)
  ...
  df <- setDT(df)[, paste0("treated_a_", 31:35) := dt.shift(total_treated, 7*num_shifts+20:(8*num_shifts)+20), by = c("sex_id", "location_id")][]
  ...
  df[1:35, .(year_id, age_group_id, total_treated, treated_t_1, treated_t_2, treated_t_3, treated_a_1, treated_a_2, treated_a_3, treated_a_4, treated_a_5)] 
  
  # create sum of treated_a
  df$treated_a_sum <- rowSums(df[, ..treated_a_cols], na.rm = TRUE) #treated_a_sum is a sum of treated_a_ variables for that year
  ...
  df[1:30, .(year_id, age_group_id, total_treated,treated_t_1, sex_id, draw_0, draw_1, n_draw_0, n_draw_1)] 
  ...
  # drop_cols <- drop_cols[!drop_cols == "treated_a_sum"] 
  df[, (drop_cols) := NULL]
  
  # Convert any negative values to zero 
  df[df < 0] <- 0
  # df[1:10, c(total_treated, treated_t_1, treated_t_2, treated_a_sum, draw_0, n_draw_0)]
...
predict_treatment <- function(df, prev_cases, covid_estimation) {
  ...
  df <- rbind(df, p_cases[age_group_id %in% keep_ages], fill = TRUE) #bind unadjusted and adjusted datasets together
  ...
  # df[, total_treated:=.I] # for testing purposes to track shift
  
  for (j in 1:length(fill_years)) {
    num_shifts <- (length(unique(df$year_id)) - 1)
    if(num_shifts > 35) stop(paste0('Need to expand age shift logic. Num shifts is ', num_shifts))
    # Shift years 
    ...
    for (i in -2:max(df$age_shifts)) {
      df[age_shifts == i, paste0("treated_a_",i+1:35)] <- NA 
    }
    df[age_shifts %in% c(-2, -1, 0), treated_a_cols] <- NA  
    ...
    locs_id <- unique(df$location_id)
    
    
    # If countries have their own 2020 and 2021 treatment data, it should be applied; Added on Oct 2022
  ...
  # Clean columns 
  # df <- copy(df)[, (draws) := NULL] # The following lines were temporarily commented out due to issues encountered with discontinuous year data.
  # names(df) <- gsub("n_d", "d", names(df))
...
create_diagnostics <- function(total_reduction, prev_cases, inc_cases, ihme_locid) {
  ...
  write.csv(x = dt_plot, file = paste0(viz_input_dir, "/", ihme_locid, ".csv"), row.names = F)
