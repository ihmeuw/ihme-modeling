...
#load_data <- function(data_dir, ihme_locid) {#updated on 8/1/2022
load_data <- function(ihme_locid) {
  # Load data 
...
create_single_years <- function(full_data) {
  ...
  setDT(full_data)
  var <- c('location_name', 'ihme_loc_id', 'location_id', 'sex', 'year_start', 'year_end', 'age_start', 'age_end', 'number_treated', 'treatment_type', 'year_range', 'incomplete_year_adjustment')
  ...
  # To do: add in conditional statement to break if year start and year end are not equal
  return(full_data)
}
...
sex_split_data <- function(data, age_group_meta, cases) {
  ...
  if (nrow(adjust_both) > 0 ) {
    ...
    # cases <- create_prevalent_cases(adjust_both)
...
format_age_ranges <- function(full_data){
  ...
  # Slight definitional changes to capture intended neonatal categories
  # Shift down observations of "1 month" to fit the GBD late neonatal age group
  full_data[(age_start > 0.0767) & (age_end < 0.084),  `:=` (age_start = 0.0767, age_end = 0.0767) ]
  # Shift down observations of 1 week to fit the GBD early neonatal age group
...
age_split_data <- function(data, age_group_meta, cases, drop_age_groups = F) {
  ...
  # If age group id is NA that is where you know you need to split it 
  ...
  if(nrow(to_split) ==0) {
    ...
    # We passed this in from the parent prepare_data script to cut down the number of times we pull from dismod and convert to cases
    # Originally collapsing data under age 1, but all calculations are done at under 4. - This was fixed
    ## Collapse draws under 4
    cases_1 <- copy(cases)[age_group_id %in% c(2, 3, 388, 389, 238, 34)]
    ...
    cases1 <- rbind(cases[!(age_group_id %in% c(2, 3, 388, 389, 238, 34))], cases_1)
    # Added lines 347-351, 399-400 to calculate the right age ratio based on the correct age range for a data point
    ages1 <- ages[, c("age_group_id", "age_start", "age_end", "age_group_name")]
    ages1[(age_group_id ==2), `:=` (age_end=4)]
    ages1[(age_group_id ==2), `:=` (age_group_id=1)]
    ages1[(age_group_id ==1), `:=` (age_group_name="0 to 4")]
    ages1[(age_end ==125), `:=` (age_end=99)]
    
    ages1 <- subset(ages1, age_group_id !=2 & age_group_id !=3 & age_group_id !=388 & age_group_id !=389 & age_group_id != 238& age_group_id != 34)
    ...
    for (gr_id in unique(to_split$g_id)) {
      ...
      if(unique(to_split1$n_age == 1)) {
        ...
        # Added lines 399-400 to calculate the right age ratio based on the correct age range for a data point
        age_st<- unique(split_dt$age_start_orig)
        age_ed <- unique(split_dt$age_end_orig)
        cases2 <- merge(cases1, ages1, by = "age_group_id")
        cases2 <- subset(cases2, age_start >= age_st & age_end <= age_ed)
        
        
        cases_melt <- melt.data.table(cases2, measure.vars = draws)
...
apply_treatment_efficacy <- function(data){
  # TO DO: Add in a map of the treatment efficacies, for now hard code them 
