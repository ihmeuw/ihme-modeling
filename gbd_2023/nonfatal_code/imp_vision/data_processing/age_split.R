...
# GET OBJECTS -------------------------------------------------------------
b_id <- "ID"  #this is bundle ID
a_cause <- "ID"
id <- "ID" ## this is the meid for where age split Dismod was run  
ver_id <- "ID" ## Dismod version used for age split
step <- 'step1' #step for Dismod version used for age split

data_filepath <- paste0("FILEPATH") 
source("FILEPATH")
source("FILEPATH")

# GET DATA FROM THE SEX SPLIT XW VERSION
dt <- read.csv(data_filepath)
dt <- as.data.table(dt)
...
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = id, 
                           measure_id = 5, location_id = locs, source = "epi", 
                           version_id = ver_id, sex_id = c(1,2), gbd_round_id = 6, decomp_step = step, 
  ...
  us_population <- get_population(location_id = locs, year_id = 2010, sex_id = c(1, 2),
                                  age_group_id = age_groups, gbd_round_id = 6, decomp_step = "iterative")
...
get_pop_structure <- function(locs, years, age_groups){
  populations <- get_population(location_id = locs, year_id = years, gbd_round_id=6, decomp_step = "step4",
...
# RUN THESE CALLS ---------------------------------------------------------------------------
ages <- get_age_metadata(12, release_id = release)
...
super_region_dt <- get_location_metadata(location_set_id = 22, release_id = release)
...
#Check age split properly
final_dt[(final_dt$age_end-final_dt$age_start>26) & final_dt$mean!=0 & final_dt$is_outlier!=1 & (final_dt$measure=="prevalence" | final_dt$measure=="incidence"), ]
