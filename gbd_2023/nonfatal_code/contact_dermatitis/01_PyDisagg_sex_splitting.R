...
ref_df_age_id <- ref_df %>% mutate(age_end = age_end + 1)
...
#load in draws_df
draws_df <- read.csv(paste0(modeling_dir,"PyDisagg_age_sex_splitting/ct_draws_df.csv"))
...
# Correction 2: Make sure that the 0 < lower < mean
half_lowest_non_zero_lwr <- min(result_to_merge$lower[result_to_merge$lower > 0]) / 5
result_to_merge <- result_to_merge %>%
  mutate(lower = ifelse(lower <= 0, half_lowest_non_zero_lwr, lower))

# Correction 3: Make sure that mean < upper < 1
