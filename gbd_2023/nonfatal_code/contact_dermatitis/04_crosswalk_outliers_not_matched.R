...
# df_orig$mean <- preds_not_poland$ref_vals_mean
# df_orig$standard_error <- preds_not_poland$ref_vals_sd
...
df_result <- df_result %>% select(-c("origin_id", "seq_parent", "modelable_entity_name1", "modelable_entity_name.1",
                                 "modelable_entity_id.1", "year_id", "rownum", "obs_method"))
class(df_result$is_outlier)
unique(df_result$is_outlier)

# save_crosswalk_version cant handle group_review==0. we chcek rows with group_review==0 and assign is_outlier==1
df_result %>% filter(!(group_review == 1 | is.na(group_review)))
df_result %>% filter(is.na(group) & is.na(specificity) & !(is.na(group_review)))
df_result %>% filter(group_review==0)
...
# # Exclude outliers from final CW:
# df_filtered <- df_filtered[df_filtered$is_outlier != 1,]
unique(df_filtered$is_outlier)

# Save final data to upload as crosswalk version:
write.xlsx(df_filtered, file = paste0(modeling_dir, "FILEPATH",bundle_id,"_only_us_poland_",date,".xlsx"), sheetName = "extraction", overwrite = TRUE)

source("FILEPATH")
data_filepath <- paste0(modeling_dir, "FILEPATH",bundle_id,"_only_us_poland_",date,".xlsx")
description <- "only US marketscan and poland claims modeled"
