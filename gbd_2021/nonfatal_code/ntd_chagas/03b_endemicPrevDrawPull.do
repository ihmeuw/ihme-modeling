...
* BUILD LIST OF ENDEMIC LOCATIONS * 
  ...
  macro dir
  get_draws, gbd_id_type(ADDRESS) measure_id(5) gbd_id(ADDRESS) gbd_round_id(`ADDRESS') decomp_step(`ADDRESS') location_id(`endemicLocations') age_group_id(`ages') source(ADDRESS) status(ADDRESS) clear
