* NTDs - Chagas disease 
* Description: post processing, adjust for countries wich elimination of transmission hass been confirmed.
/******************************************************************************\
...
* PULL AGE GROUP DATA *
  ...
  local age_group_id `r(ADDRESS)'
  local ages `r(ADDRESS)'
  local year_id `r(ADDRESS)'
  local sex_id `r(ADDRESS)'
  local yearList = subinstr("`year_id'", " ", ",",.)
  
  odbc load, exec("SELECT age_group_id, age_group_years_start AS age_start, age_group_years_end AS age_end FROM age_group WHERE age_group_id IN (`=subinstr("`age_group_id'", " ", ",", .)')") `ADDRESS' clear
...
\******************************************************************************/
    ...
    * PULL IN DRAWS FROM DISMOD MODELS FOR INCIDENCE AND PREVALENVCE *
      get_draws, gbd_id_type(ADDRESS) gbd_id(`ADDRESS') source(ADDRESS) decomp_step(`ADDRESS') gbd_round_id(`ADDRESS') location_id(`location') age_group_id(`ages') measure_id(5 6) status(ADDRESS) clear
...
* BRING IN DATA ON PROPORTION ON NUMBER OF PREGNANCIES BY AGE, YEAR, LOCATION (USING ESTIMATES CREATED FOR HEPATITIS E ESTIMATION) *  
  merge 1:1 location_id age_group_id year_id sex_id using FILEPATH, keep(1 3) nogenerate
