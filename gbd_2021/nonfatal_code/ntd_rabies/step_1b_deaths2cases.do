* NTDS: Rabies
* Purpose: takes in step_1_deaths2cases.do call per location and writes out location estimate
...
*** PULL IN POPULTION ESTIMATES ***
	get_population, location_id(`location') age_group_id(`ages') sex_id(`sexes') year_id(`years') decomp_step(ADDRESS) clear
	save `pop'
	sleep 100
  
  
*** PULL IN DEATH ESTIMATES ***
    get_draws, gbd_id_type(cause_id) gbd_id(ADDRESS) source(ADDRESS) status(ADDRESS) decomp_step(ADDRESS) location_id(`location') age_group_id(`ages') sex_id(`sexes') year_id(`years') measure_id(1) gbd_round_id(ADDRESS) clear 
...
*** WRAP UP ***	
	...
	export delimited location_id year_id age_group_id sex_id measure_id model_iddraw_* using `outDir'/`location'.csv, replace
