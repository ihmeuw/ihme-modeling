...
// WRITE CODE HERE
	...
	*************************************
	...
	//load hearing aid coverage draws for Norway (since it is the country from regression giving coverage by severity, consider using Nord-/Trondelag but currently mapped to norway)
			get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(2411) measure_id(5) age_group_id(`age_ids_list') location_id(90) decomp_step("iterative") gbd_round_id(7) clear
	...
	//load hearing aid coverage draws for country of interest
  		  	get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(2411) measure_id(5) age_group_id(`age_ids_list') location_id(`location_id') decomp_step("iterative") gbd_round_id(7) clear
			...
			*** DIAGNOSTICS
			...
			get_draws, gbd_id_field(modelable_entity_id) source(epi) gbd_id(2411) measure_ids(5) location_ids(90) decomp_step("iterative") gbd_round_id(7) clear
			keep if inlist(age_group_id, `age_ids_list')
			rename draw* Norway_draw*
			save "FILEPATH", replace 
			//CLUSTER: load hearing aid coverage draws for country of interest 
 		  	get_draws, gbd_id_field(modelable_entity_id) source(epi) gbd_id(2411) measure_ids(5) location_ids(58) decomp_step("iterative") gbd_round_id(7) clear
			keep if inlist(age_group_id, `age_ids_list')
			save "FILEPATH", replace 
	...
	** CALCULATE RATIO
	*************************************
		*USERNAME: I think the best way to carry through uncertainty is to calculate at draw level 
