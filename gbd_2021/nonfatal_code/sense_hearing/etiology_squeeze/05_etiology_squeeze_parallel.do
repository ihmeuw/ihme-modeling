...
`*/
		foreach meid in `meids' {
			...
			if `meid' == 1282 get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(`meid') measure_id(5) location_id(`location_id') decomp_step("iterative") gbd_round_id(7) clear
			if `meid' == 2311 get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(`meid') measure_id(18) location_id(`location_id') decomp_step("iterative") gbd_round_id(7) clear
			if `meid' == 24049 get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(`meid') measure_id(5) location_id(`location_id') decomp_step("iterative") gbd_round_id(7) clear
			...
			if inlist(`meid', `meids_best') {
					...
					else get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(`meid') measure_id(5) location_id(`location_id') decomp_step("iterative") gbd_round_id(7) clear
			   		}
			   		*/
			...
			if !inlist(`meid', `meids_best') {
					...
					if `cluster' == 1 get_draws, gbd_id_type(modelable_entity_id) source(epi) gbd_id(2406) measure_id(5) location_id(`location_id') decomp_step("iterative") gbd_round_id(7) clear
					forval i = 0/999 {
...
//Loop over every year and sex for given location 
	...
	//Pull in hearing-aid adjusted severity levels from previous step. Inputs from step 4.
	//use "FILEPATH", clear 
	use "FILEPATH", clear 
	...
	*** MENINGIITS 
		// menigitis is coming in as 26+ db rather than 35+ db estimates.  crosswalk using nhanes data
			**STRATEGY: crosswalk 26+ to 35+, then apply the remainder to 20-34. 
...
**********************************		
		...
		** SQUEEZE!
				...
				** 4) For all ages (remember, <20 has been squeezed, but may not be quite at level of envelope due to second congenital adjustment), assign "age related and other" hearing loss as remainder between sum and envelop 
				**	local draw 0
				**	local sev 95
					quietly {
