...
write.csv(stats, file = paste0(path, 'FILEPATH'))
...
write.csv(relinf, file = paste0(path, 'FILEPATH'))

# plot the risk map
ext <- zoom*extent(preds_sry)
png(paste0(path, 'FILEPATH'),
    width = 3000,
    height = 2000,
    pointsize = 30)
par(oma = rep(0, 4),
...
writeRaster(preds_sry$mean, file = paste0(path, 'FILEPATH'), format = 'GTiff')
writeRaster(preds_sry$uncertainty, file = paste0(path, 'FILEPATH'), format = 'GTiff')
