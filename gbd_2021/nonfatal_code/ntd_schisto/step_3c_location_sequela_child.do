...
*** ======================= BOILERPLATE ======================= ***
	...
	cap log using "FILEPATH/step_3c_`location'.smcl", replace
...
**********************************************************************************************************************	
	...
	** pull in haematobium and mansoni specific draws if there is coinfection
	if `split' == 1 {
		...
		get_draws, gbd_id_type(model_id) gbd_id(ADDRESS) source(ADDRESS) measure_id(5) location_id(`location') status(best) gbd_round_id(`gbd_round_id') decomp_step(`decomp_step') clear
...
**********************************************************************************************************************
	** get draws
	get_draws, gbd_id_type(model_id) gbd_id(ADDRESS) measure_id(18) location_id(1) year_id(2000) source(epi) status(best) gbd_round_id(`gbd_round_id') decomp_step(`decomp_step') clear
		...
		**recode age_group_id 34 as 5 to enable logic in age group restrictions to function below
		...
		foreach sex of local sexes {
			...
			if inlist("`seq'", "hep", "hem", "asc") {
				...
				append using ``seq'_2019_`sex''
				append using ``seq'_2020_`sex''
				append using ``seq'_2021_`sex''
