norway_age_split <- function (intERsplit, age, gbd_round_id = 6, age_pattern_loc = 1, measure = 5, age_pattern_mod_ent, step) {
  ...
  # Exclude 0s, although not really relevant for GERD
  ...
  # Don't split a datum if the average number of case per age-group will be less than 1
  round_age[, drop := cases/n.age]
  round_age <- round_age[!drop<1,]
  ...
  age_pattern <- as.data.table(
    get_draws(gbd_id_type = "modelable_entity_id", gbd_id = 1955,
              measure_id = 5, location_id = 1, source = "epi",
              status = "best", sex_id = c(1,2), gbd_round_id = 6, decomp_step = "step1",
              age_group_id = c(1:20,30,31,32,235), year_id = 2010))
  # Only keep draws for the ages we actually need to split into 
  ...
  population <- as.data.table(get_population(location_id = ppln_locations, gbd_round_id = 7, year_id = ppln_years, sex_id = c(1, 2), decomp_step = "iterative",
  ...
  now_split[, age_group_name := NULL]
  now_split[, most_detailed := NULL]
