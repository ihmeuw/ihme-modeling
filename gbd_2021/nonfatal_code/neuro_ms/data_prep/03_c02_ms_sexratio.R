...
get_sex_ppln <- function(bothsex_dt, decomp_step, gbd_round_id, age_dt = age_dt) {
  ...
  population <- get_population(location_id = unique(dt$location_id), gbd_round_id = gbd_round_id, decomp_step = decomp_step, age_group_id = age_dt$age_group_id, sex_id = c(1, 2), year_id = seq(min(dt$year_start), max(dt$year_end)))
  ...
  #Sum for the both-sex denominator population specific to that observation's demographics
