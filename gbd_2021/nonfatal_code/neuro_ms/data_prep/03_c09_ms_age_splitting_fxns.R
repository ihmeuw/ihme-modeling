dis_age_split <- function (intERsplit, age, gbd_round_id = 6, age_pattern_loc = 1, measure = 5, age_pattern_mod_ent, step) {
  ...
  # Make some IDs, not positive I will need them
  ...
  round_age <- round_age[drop > 1 | is.na(drop),]
  ...
  # Assign age_group_id if the merge wasn't straightforward (0 to 4 in Emma's example)
  ...
  draw_years[draw_years < 1990] <- 1990 # for mean years prior to 1990 take 1990 as the value
  ...
  population <- as.data.table(get_population(location_id = ppln_locations, gbd_round_id = gbd_round_id, year_id = ppln_years, sex_id = c(1, 2), decomp_step = step,
