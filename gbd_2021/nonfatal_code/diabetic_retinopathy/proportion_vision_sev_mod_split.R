##########################################################################
### Author: USERNAME, modified by USERNAME for vision severity
### Date: 03/22/2019
### Project: GBD Nonfatal Estimation
### Purpose: Adjusting non-reference severities for GBD 2019 and GBD 2020
##########################################################################
...
cause_name <- "ID1"
cause_name2 <- "ID2"
covariate_name <- "severity_mod_sev"
bundle_version <- "sex" 
...
loc_dt <- get_location_metadata(location_set_id = 22, gbd_round_id=6) # same output for round 6 and 7 
...
find_sev_match <- function(dt){
  sev_dt <- copy(dt)
  sev_dt <- sev_dt[sev_code %in% c("SEV", "MOD") & measure %in% c("prevalence")]
  ...
  sev_dt <- sev_dt[!mean_SEV == 0 & !mean_MOD == 0]
...
calc_sev_ratios <- function(dt){
  ...
  ratio_dt[, `:=` (ratio = mean_MOD/mean_SEV,
                   ratio_se = sqrt((mean_MOD^2 / mean_SEV^2) * (standard_error_MOD^2/mean_MOD^2 + standard_error_SEV^2/mean_SEV^2)))]
...
model_name <- paste0(cause_name, "_mod_sevsplit", date)
...
dt <- dt[!(nid %in% nid_bc_p$nid & diagcode == "ENV-MOD_SEV-b"),]

model <- sev_model

tosplit_dt <- copy(dt)
nosplit_dt <- tosplit_dt[sev_code %in% c("SEV", "MOD")]
tosplit_dt <- tosplit_dt[sev_code == "MOD_SEV"] 
...
split_dt[, paste0("MOD_", 0:999) := lapply(0:999, function(x) mean*get(paste0("draw_", x))/(get(paste0("draw_", x))+1))]
...
split_dt[, MOD_mean := rowMeans(.SD), .SDcols = paste0("MOD_", 0:999)]
split_dt[, MOD_standard_error := apply(.SD, 1, sd), .SDcols = paste0("MOD_", 0:999)]
split_dt[, sev_standard_error := apply(.SD, 1, sd), .SDcols = paste0("SEV_", 0:999)]
split_dt[, c(draws, paste0("SEV_", 0:999), paste0("MOD_", 0:999)) := NULL]
...
sev_dt[, `:=` (mean = sev_mean, standard_error = sev_standard_error, upper = NA, lower = NA, 
               ...
               note_modeler = paste0(note_modeler, " | SEV sev split with MOD/SEV ratio: ", ratio_mean, " (", 
                                     ratio_se, ")"))]
sev_dt <- dplyr::select(sev_dt, names(dt))
sev_dt[mean<0, mean:= 0] 
MOD_dt <- copy(split_dt)
MOD_dt[, `:=` (mean = MOD_mean, standard_error = MOD_standard_error, upper = NA, lower = NA, 
               cases = NA, sample_size = NA, uncertainty_type_value = NA, sev_code = "MOD",
               note_modeler = paste0(note_modeler, " | MOD sev split with MOD/SEV ratio: ", ratio_mean, " (", 
                                     ratio_se, ")"))]
MOD_dt <- dplyr::select(MOD_dt, names(dt))
total_dt <- rbindlist(list(nosplit_dt, MOD_dt, sev_dt))

predict_sev <- list(final = total_dt, graph = split_dt)


graph_predictions <- function(dt){
  graph_dt <- copy(dt[measure == "prevalence", .(age_start, age_end, mean, sev_mean, sev_standard_error, MOD_mean, MOD_standard_error)])
  graph_dt_means <- melt(graph_dt, id.vars = c("age_start", "age_end", "mean"), measure.vars = c("sev_mean", "MOD_mean"))
  graph_dt_means[variable == "MOD_mean", variable := "MOD"][variable == "sev_mean", variable := "SEV"]
  graph_dt_error <- melt(graph_dt, id.vars = c("age_start", "age_end", "mean"), measure.vars = c("sev_standard_error", "MOD_standard_error"))
  graph_dt_error[variable == "MOD_standard_error", variable := "MOD"][variable == "sev_standard_error", variable := "SEV"]
  ...
  gg_sex <- ggplot(graph_dt, aes(x = mean, y = value, color = variable)) + 
    ...
    scale_color_manual(name = "Severity", values = c("SEV" = "midnightblue", "MOD" = "purple")) +
    theme_classic()
  return(gg_sex)
}
...
total_dt <- total_dt[sev_code!="SEV",]
