...
\******************************************************************************/
    ...
    * PULL IN DRAWS FROM DISMOD MODELS FOR anti-HCV INCIDENCE AND PREVALENVCE *
      interpolate, gbd_id_type("modelable_entity_id") gbd_id(`meid') source("epi") location_id(`location') age_group_id(`ages') measure_id(5) status("best") reporting_year_start(1990) reporting_year_end(2022) gbd_round_id(7) decomp_step("iterative") clear
      rename draw_* prev_*
      save `mergeTemp'
	  
      interpolate, gbd_id_type("modelable_entity_id") gbd_id(`meid') source("epi") location_id(`location') age_group_id(`ages') measure_id(6) status("best") reporting_year_start(1990) reporting_year_end(2022) gbd_round_id(7) decomp_step("iterative") clear
      rename draw_* inc_*
      *drop n_draws
...
\******************************************************************************/		
    ...
    destring year_id, replace
 	...
 	keep if (mod(year_id,5)==0 | year_id==2019 | year_id==2020 | year_id == 2021 | year_id == 2022) & year_id>=1990
