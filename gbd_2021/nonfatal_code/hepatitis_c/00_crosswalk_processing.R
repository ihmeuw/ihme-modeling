...
orig_dt <- get_bundle_version(bundle_version_id = bv_id, fetch = "all", export = TRUE)
...
# BLOOD DONOR CROSSWALK ----------------------------------------------------------
cv_drop <- c("cv_antibody", "cv_rna", "cv_double_anti", "cv_healthcare_worker", "cv_pregnant")
hep_dt <- copy(orig_dt)
...
epidb <- epidb[cv_rna != 1, ]
epidb[cv_pregnant == 1, is_outlier := 1]

cw_file_path <- FILEPATH
write.csv(epidb, cw_file_path, row.names = FALSE)