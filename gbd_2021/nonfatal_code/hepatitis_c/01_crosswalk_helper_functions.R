...
get_matches <- function(n, pair_dt, year_span = 10, age_span = 10){
  ...
  unmatched <- unmatched[!age_start >= 100] ## GET RID OF CENTENIALS BECAUSE CAUSING ISSUES WITH AGE ROUNDING AND LIKELY WON'T MATCH
  ## AGE ROUNDING
...
save_mrbrt <- function(model, mrbrt_directory, mod_name) {
  ...
  write.csv(df_result, paste0(mrbrt_dir, mod_name, "/df_result_crosswalk.csv"), row.names = FALSE)
  py_save_object(object = model, filename = paste0(mrbrt_dir, mod_name, "/results.pkl"), pickle = "dill")
}
...
plot_sex_by_age <- function(dt) {
  ...
  ggsave(paste0(mrbrt_dir, model_name, "prevalence_by_sex_ratios_age.pdf"), gg_sex_age)
  return(gg_sex_age)
}
...
prediction_plot <- function(vetting_data, by_state = F){
  ...
  for (cv in cvs){
    ...
    gg_pred <- ggplot(graph_dt, aes(x = mean, y = mean_adj, color = as.factor(super_region_name))) +
      ...
      theme_classic()
    ggsave(gg_pred, filename = paste0(mrbrt_dir, model_name, "/adjust_graph", cv, ".pdf"), width = 12)
  }
  return(gg_pred)
}
...
adjustment_plot <- function(vetting_data) {
  ...
  ggsave(gg_orig, filename = paste0(mrbrt_dir, model_name, "/original_mean.pdf"), width = 12)
  ggsave(gg_adj, filename = paste0(mrbrt_dir, model_name, "/adj_mean.pdf"), width = 12)
}
...
prep_data_matches <- function(raw_dt) {
  ...
  drop_nids3 <- 439918 # this one keeps sneaking in so hard coding in dropping until I can create a column in sheet 
