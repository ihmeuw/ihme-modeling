# Purpose: split grade <2 into grade 1 and 0
# Notes: run logistic regression using SINAN data to determine proportions for split
#
#' [ Set-Up ]
data_root <- 'FILEPATH'
...
library(viridis)
library(dplyr)
library(Zelig, lib.loc = paste0(code_root, "FILEPATH"))
...
# data
data  <- fread(paste0(params_dir, "FILEPATH"))
...
# (denominator year_start age_start) uniquely separates grade 1 and grade 2 except for below data where four rows present and not possible to decipher which grade 1 matches with grade 2
...
# Get ages needed
age_md <- get_age_metadata(ADDRESS, gbd_round_id = gbd_round_id)
...
write.csv(props, paste0(params_dir, "FILEPATH", row.names = FALSE))
...
write.csv(draws, paste0(params_dir, "FILEPATH"), row.names = FALSE)
