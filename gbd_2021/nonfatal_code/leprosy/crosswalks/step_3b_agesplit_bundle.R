...
get_age_pattern <- function(locs, id, age_groups){
  ...
  se <- copy(age_1)
  se <- se[age_group_id==238, .(measure_id, sex_id, se_dismod, location_id)] ##just use standard error from 1-4 age group 
...
age_split <- function(gbd_id, df, age, region_pattern, location_pattern_id){
  ...
  super_region_dt <- get_location_metadata(location_set_id = 22,decomp_step = "ADDRESS")
...
final_split <- age_split(gbd_id = 'ADDRESS', df = dt_india, age = age_groups, region_pattern = F, location_pattern_id = 1)
...
final_split <- age_split(gbd_id = id, df = dt_gl, age = age_groups, region_pattern = F, location_pattern_id = 1)
...
#deleting GBR because exclude anyway
age_split_ADDRESS <- age_split_ADDRESS[location_id != 95]
...
openxlsx::write.xlsx(age_split_ADDRESS, bADDRESS_outfile,  sheetName = "extraction")
