...
# Purpose: Apply sex crosswalk to bundle ADDRESS -- Global(non-India), age-specific
# Description: ADDRESS - non-India, age-specific  data sources: ICMR, WER disaggregated incidence-WHO, SINAN, sci-lit
...
#############################################################################################
#NOTE: this bundle contains India data - can delete in future rounds
...
bADDRESS <- bADDRESS[field_citation_value != "ADDRESS"]
bADDRESS <- bADDRESS[age_end - age_start < 25]
...
# repeat for females
females<-bADDRESS_b %>% 
  select(nid, location_id, underlying_nid, sampling_type, recall_type_value, uncertainty_type, input_type, design_effect, year_start, year_end, age_start, unit_value_as_published, measure, recall_type, representative_name, source_type, unit_type, urbanicity_type, uncertainty_type_value, upper, lower, age_end, seq, is_outlier, case_name, bundle_id, origin_seq, cause_id, fem_cases, inc2, se_fem, fem_ss) %>%
...
females_ss<-females_ss %>%
  select(nid, location_id, underlying_nid, sampling_type, recall_type_value, uncertainty_type, input_type, design_effect, year_start, year_end, age_start, unit_value_as_published, measure, age_end, recall_type, representative_name, source_type, unit_type, urbanicity_type, uncertainty_type_value, upper, lower, seq, is_outlier, case_name, bundle_id, origin_seq, cause_id, cases, mean, standard_error, effective_sample_size)
females_ss <- rbind(females,females_ss)
