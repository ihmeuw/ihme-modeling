...
*--------------------4.1: Get results of sequeale models from database
	...
	foreach meid in `meids'{
		...
		get_model_results, gbd_team("ADDRESS") gbd_round_id("ADDRESS") gbd_id("`ID'") location_id("1") year_id("`gbdyears' 2015") age_group_id("`gbdages'") sex_id("-1") clear
	...
	foreach meid in `meids'{
		...
		foreach location in `ameronchoids'{
			...
			*Hard code younger ages to zero 
			...
			keep age_group_id location_id year_id sex_id model_iddraw* measure_id
			order model_idmeasure_id location_id year_id age_group_id sex_id  draw* 
