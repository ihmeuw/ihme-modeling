...
locs <- no_file
param_map <- data.table(location_id = locs, decomp_step = decomp_step, gbd_round_id = gbd_round_id)

# grs
location_md <- get_location_metadata(location_set_id=35, gbd_round_id=gbd_round_id)
nig_sn <- location_md[parent_id == 214, location_id]
eth_sn <- location_md[parent_id == 179, location_id]
