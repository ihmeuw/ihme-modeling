...
# TEST simple model lymphedema prevalence as a function of adjusted infection
...
#drop outlier on lf prevalence, drop 0 prevalence 
...
lf_prev<-get_model_results(gbd_team='team_name', gbd_id='ADDRESS', gbd_round_id='ADDRESS',age_group_id=ages, sex_id=c(1,2), location_id=unique_lf_locations, year_id=c(1990,1995,2000, 2005, 2010,2015, 2019,2020,2021,2022), decomp_step="ADDRESS")
...
for(i in unique_lf_locations){
  ...
  upload_file$model_id<-1492
  upload_file$measure_id<-5
    s1<-setDT(upload_file)
  write.csv(s1,(paste0("FILEPATH", i,".csv")))
}

locs<-get_location_metadata(location_set_id = 35, gbd_round_id=7, decomp_step="ADDRESS")
...
source("FILEPATH/save_results.R")
save_results(input_dir =paste0("FILEPATH"),
             input_file_pattern = "{location_id}.csv", 
             model_id= ADDRESS,
             description = "collaborator review",
             measure_id = 5,
