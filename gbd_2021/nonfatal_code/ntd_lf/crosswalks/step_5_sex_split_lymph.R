...
source(sprintf("%sFILEPATH/get_location_metadata.R",prefix))
source(sprintf("%sFILEPATH/get_demographics.R",prefix))
source(sprintf("%sFILEPATH/get_population.R",prefix))
source(sprintf("%sFILEPATH/get_ids.R",prefix))
source(sprintf("%sFILEPATH/get_covariate_estimates.R",prefix))
source(sprintf("%sFILEPATH/save_results.R",prefix))
...
#if sample size missing from records - drop
...
apply_sex_crosswalk <- function(mr_brt_fit_obj, all_data, decomp_step){
  ...
  cat("\nStarting sex crosswalk. Populations are aggregated by single-year ages but not by year.\n")
  ...
  unadj_df <- add_population_cols(data = unadj_df, gbd_round = ADDRESS, decomp_step = decomp_step)
  ...
  unadj_df[, log_mean := log(mean)]
  unadj_df$log_se <- sapply(1:nrow(unadj_df), function(i) {
    mean_i <- unadj_df[i, mean]
     se_i   <- unadj_df[i, standard_error]
  ...
  unadj_df[, paste0("both_prev_", 0:999) := lapply(0:999, function(x) exp(get(paste0("both_prev_", x))) * 0.995)]
