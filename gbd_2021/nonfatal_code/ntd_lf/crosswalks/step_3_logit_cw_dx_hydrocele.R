...
#dat_original will be hydrocele model inputs 

dat_original <-read.csv("FILEPATH") 

#need to recode variables for dx and hydrocele

dat_original$hydro_mean<-dat_original$mean
dat_original$hydro_ss<-dat_original$sample_size
dat_original$hydro_cases<-dat_original$cases
...
#drop anything missing both prevalence measures
dt<-dt[!is.na(dt$prev_ict) | !is.na(dt$prev_mf),]
...
dt<-dt[, diagnostic := ifelse(!is.na(prev_mf), "mf","ICT")]
...
mean_var <- "mean"
se_var <- "se"
#cov_names <- "age_mid" # can be a vector of names
...
#create ref variable
dat_original$ref<-if_else(dat_original$diagnostic=="ICT",1,0)
#dat_original$ref<-1
...
fit1 <- run_mr_brt(
  output_dir = paste0("FILEPATH", Sys.info()["user"], "/"), # user home directory
  model_label = "crosswalk_hyrdo",
  data = tmp_metareg,
  mean_var = "diff_logit",
 ...
 # covs = list(cov_info("age_mid", "X",
                      # bspline_gprior_mean = "0, 0, 0, 0",
                       ...
                       #degree = 3,
                       #n_i_knots = 3, #3 knots would be four priors
                       #r_linear = TRUE,
                       #l_linear = TRUE )),
...
df_pred <- as.data.frame(tmp_orig) # cov_names from line 55
...
####inv.logit with deltamethod was giving me an error so i used the following instead -- is that okay?
