...
#CHANGE REPO HERE for H drive where upload_order.csv file is stored
repo_dir <- paste0(h_root, "FILEPATH")
functions_dir <- paste0(ADDRESS, "ADDRESS")
date <- gsub("-", "_", date)
...
col_order <- function(dt){
  e_order <- fread(paste0(repo_dir, "FILEPATH"), header = F)
  ...
  new_order <- c(e_order, extra_cols)
...
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "model_id", gbd_id = id, ## USING 2010 AGE PATTERN BECAUSE LIKELY HAVE MORE DATA FOR 2010
...
#clean Mean and Sample size, # cases where missing

#drop remaining rows with mean only data
dt<-dt[!is.na(dt$sample_size)]

dt$cases<-round(dt$cases)
dt$group_review<-1
dt$sample_size<-round(dt$sample_size)
...
dt[year_start <1980, year_start := 1980]
dt[year_end <1980, year_end := 1980]


dt$seq <- seq.int(nrow(dt))
setnames(dt, "seq", "crosswalk_parent_seq")
dt$crosswalk_parent_seq<-NULL
...
#CHANGE YOUR FILE PATH TO WHEREVER THE OUTPUT SHOULD GO
