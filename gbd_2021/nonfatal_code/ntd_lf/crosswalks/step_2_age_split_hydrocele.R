...
col_order <- function(dt){
  e_order <- fread(paste0(repo_dir, "upload_order.csv"), header = F)
...
format_data <- function(unformatted_dt, sex_dt){
  ...
  dt[, year_id := round((year_start + year_end)/2, 0)] ##merge on year later
  return(dt)
}
...
get_age_pattern <- function(locs, id, age_groups){
  age_pattern <- get_draws(gbd_id_type = "model_id", gbd_id = id, 
...
dt<-dt[,cases:=ifelse((!is.na(cases)& !is.na(mean) & !is.na(sample_size)), (mean*sample_size), cases)] #impute sample sizes

dt<-dt[,sample_size:=ifelse((!is.na(cases) & (!is.na(mean)) & is.na(sample_size)),(cases/mean),sample_size)] 

dt<-dt[,cases:=ifelse((is.na(cases) & (!is.na(mean)) & !is.na(sample_size)),mean*sample_size, cases)]

dt<-dt[,mean:=ifelse((is.na(mean)),(cases/sample_size), mean)] 
