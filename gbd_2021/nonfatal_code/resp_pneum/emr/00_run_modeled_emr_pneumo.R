...
cause_name <- "FILEPATH" #cause name for saving csvs and for use in plot titles
#EMR data 
model_id <- 578801  #Dismod model version ID from which you want to pull EMR

#Upload specs
user_name <- Sys.info()['user'] #for uploader validation
bundle_id <- 7532 #bundle ID
crosswalk_version_id <- 32780 #xwalk version to which you want to append the predicted EMR - should be iterative 2020 
bundle_version_id <- 35204 #bundle version for crosswalk_version_id specified above 
upload_datapath <- paste0("FILEPATH/2019_modeled_emr_xwalk_", date, ".xlsx") #upload path for new xwalk version
seq_emr <- 1 #seq generated for EMR dummy row in bundle version listed above
...
age_predict <- c(15,35,45,55,65,70,75,80,85,90,100)
...
locs <- as.data.table(get_location_metadata(9, gbd_round_id=7))
...
# final_combined[final_combined$location_id == 95, ]


write_xlsx(list(extraction = final_combined), "xwalk_pneumo.xlsx")
...
result <- save_crosswalk_version(bundle_version_id=35204,
                                 data_filepath="xwalk_pneumo.xlsx",
                                 description= "EMR modeled rnd 2, age start 15, haqi and sex, outlier 0 values")
