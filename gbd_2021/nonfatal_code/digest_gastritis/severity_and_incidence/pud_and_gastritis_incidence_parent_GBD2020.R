...
epi_locations <- get_location_metadata(location_set_id=35, gbd_round_id=7, decomp_step="step3") #most detailed locations in GBD 2020 step 2 is location set id 35 (per Joe Wagner)
...
for(cause in causes) { 
  ...
  for(location in location_ids) {
    command <- paste0("qsub -q all.q  -P proj_rgud -l m_mem_free=4G  -l fthread=5 -l h_rt=24:00:00  -o ",logs," -e ",error, " -N ", paste0(cause,"_", location, " "), shell, " /homes/hhan5/R/GBD2020_code/pud_and_gastritis_incidence_child.R", " ", cause, " ", location, " ", total_incidence_files)
    system(command)
  }
}


print("Petra and Pascale rock")

#save results
save_results_epi(input_dir="FILEPATH", input_file_pattern = "{location_id}.csv", modelable_entity_id =16219, description = 'hhan5: GBD202020 custom upload of adjusted from total pud with prevalence of asymptomatic no anemia sequela, 4/26/2021', mark_best='TRUE', measure_id = c(5,6), gbd_round_id=7, decomp_step='iterative', bundle_id = 6998, crosswalk_version_id = 20357 )
