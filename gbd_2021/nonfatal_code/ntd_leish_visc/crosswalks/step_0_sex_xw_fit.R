...
ADDRESS1 <- fread("FILEPATH")
# age_spec VL prevalence
ADDRESS2 <- fread("FILEPATH")
# all_age VL incidence
ADDRESS3 <- fread("FILEPATH")
# age_spec VL incidence
ADDRESS4 <- fread("FILEPATH")

col_names <- Reduce(intersect, list(names(ADDRESS1), names(ADDRESS2), names(ADDRESS3), names(ADDRESS4)))
all_data  <- rbind(ADDRESS3[, ..col_names], ADDRESS4[, ..col_names])
