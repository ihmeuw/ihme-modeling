...
locs <- get_location_metadata(ADDRESS, gbd_round_id = gbd_round_id, decomp_step = decomp_step)
...
param_map <- data.table(location_id = est_locs, gbd_round_id = gbd_round_id, decomp_step = decomp_step)
...
qsub(job_name = paste0("VL_Fatal_Draws_", stgpr_run_id),
     shell    = my_shell,
