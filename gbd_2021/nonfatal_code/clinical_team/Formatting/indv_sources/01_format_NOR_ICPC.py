...
assert (
    set(df.new_location_name.unique()) - set(loc_subnats.location_ascii_name.unique())
    == set()
)
assert (
    set(loc_subnats.location_ascii_name.unique()) - set(df.new_location_name.unique())
    == set()
)
...
df = df.groupby(
    by=[col for col in df.columns if col != "val"], as_index=False
).val.sum()
