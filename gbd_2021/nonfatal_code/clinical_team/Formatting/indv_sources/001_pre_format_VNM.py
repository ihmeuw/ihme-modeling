...
datlist = []
for sheet in sheets:
    dat = pd.read_excel(fpath, sheetname=sheet)
    dat["sheetname"] = sheet
    datlist.append(dat)
...
for disease in cdf.disease_code.unique():
    ...
    if ctq[ctq.disease_code == disease].shape[0] == 0:
        continue
    if (
        cdf[cdf.disease_code == disease].inp_total_cases.sum()
        != ctq[ctq.disease_code == disease].inp_total_cases.sum()
    ):
