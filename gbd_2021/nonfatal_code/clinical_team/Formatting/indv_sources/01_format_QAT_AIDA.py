...
def reshape_qatar(df):
    ...
    df.drop(
        axis=1,
        labels=["DISHCARGE DIAGNOSIS", "SUM", "sum_calc", "AVG LOS", "NO. OF DEATHS"],
        inplace=True,
    )

    df.rename(
        columns={"ICD 9 CODE": "cause_code", "NAT": "nat", "SEX": "sex_id"},
        inplace=True,
    )

    df = (
        df.set_index(["cause_code", "year_start", "nat", "sex_id"])
        .stack()
        .reset_index()
    )
    ...
    df.loc[df.age == "85+", "age"] = "85-124"  # we'll add 1 to every age end
...
# add 1 to every age end to fit the age_start(inclusive) age_end(exclusive)
# pattern we use and the shared age groups use
