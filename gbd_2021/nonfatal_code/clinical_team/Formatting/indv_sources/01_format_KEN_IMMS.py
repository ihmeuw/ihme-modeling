...
df = pd.read_excel(fpath, sheetname="original", header=2)

df.reset_index(drop=True, inplace=True)

# manually add column names
col_names = [
    "groups_and_disease_name",
    "icd_code",
    "grand_total",
    "total_alive",
    "total_dead",
...
df["sex_id"] = 3
# df = df[['sex_id', 'year_start', 'year_end', 'icd_code'] + list(age_cols)]
...
def reshape_long(df):
    ...
    df = (
        df.set_index(["sex_id", "year_start", "year_end", "icd_code"])
        .stack()
        .reset_index()
    )
    df.rename(columns={"level_4": "age_outcome", 0: "val"}, inplace=True)

    df["age_start"], df["age_end"], df["outcome"] = (
        df["age_outcome"].str.split("-", 2).str
    )
...
assert (
    df.query("outcome_id == 'discharge'").val.sum()
