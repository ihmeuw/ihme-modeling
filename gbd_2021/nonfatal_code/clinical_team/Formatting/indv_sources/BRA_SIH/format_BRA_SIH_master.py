...
def read_finished_files():
    """
    Reads in results of qsubbed worker scripts.

    Returns
    -------
    Pandas DataFrame made from the outputs of all worker scripts
    """
    single_year_processed_dir = "FILEPATH"
    ...
    df = df.replace(to_replace="NULL", value=pd.np.nan)
...
def finalize(df):
    ...
    # get 2015 and 2016
    bra_15_16 = pd.read_hdf("FILEPATH")
