...
def make_location_id(df, munic_col):
    ...
    missing_location_mask = df.MUNIC_RES_substr.isin(["00", "02", "03", "60", "61"])
...
def make_age_groups(df):
    ...
    df.loc[df.age_unit == "ignored", "age"] = pd.np.nan
    ...
    df.loc[df.age_unit == "unknown", "age"] = pd.np.nan
...
def main(year):
    ...
    df = make_location_id(df, munic_col)
