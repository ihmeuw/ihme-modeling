...
def get_sample_size_icpc(df, gbd_round_id, decomp_step):
    ...
    df = df.merge(pop, how='left', on=['location_id', 'year_start',
                                       'year_end', 'age_group_id',
                                       'sex_id'])
    assert_msg = """
...
def check(df):
    ...
    ref = pd.read_csv("/ihme/hospital/clinical_runs/run_3/estimates/"
                      "for_upload/final/bundle_csv/outpatient.csv", nrows=100)
...
def get_icpc_map():
    ...
    imap = pd.read_csv("/ihme/hospital/scratch/clean_icpc_map.csv")
...
def save_icpc(df, run_id):
    ...
    filepath = "/ihme/hospital/clinical_runs/run_{}/estimates/outpatient/final/icpc.csv".format(
        run_id)
...
def main_icpc(run_id, gbd_round_id, decomp_step):
    ...
    df = pd.read_hdf("/ihme/hospital/clinical_runs/run_{}/estimates/"
                     "inpatient/master_data/formatted/NOR_ICPC.H5".format(run_id))
