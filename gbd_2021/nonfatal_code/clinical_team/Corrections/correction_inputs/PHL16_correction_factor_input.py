...
def align_values(df):
    ...
    df['sex'] = df['sex_id']
    assert set(['M', 'F']) == set(df.sex.unique()
                                  ), "we did not expect other sex_id values"
    # all ages are years per codebook
    assert set(['Years']) == set(df.age_group_unit.unique()
                                 ), "Some age units aren't years?"
...
loc_merge.rename(
    columns={'location_ascii_name': 'location_name'}, inplace=True)
...
df.loc[df['location_name'] == 'metro manila',
       'location_name'] = 'national capital region'  # official name
df.loc[df['location_name'] == 'western samar',
       'location_name'] = 'samar (western samar)'
...
shared_locs = locs.query(
    "parent_id == 16").location_ascii_name.str.lower().unique()
...
df['dx_1_len'] = np.nan
df.loc[df['dx_1'].notnull(), 'dx_1_len'] = df.loc[df['dx_1'].notnull(),
                                                  'dx_1'].apply(len)
