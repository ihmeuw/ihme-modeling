...
# load our functions
for hosp_path in [FILEPATH.format(user),
                  FILEPATH.format(user)]:
...
df['adm_date_str'] = df['ayear'].astype(
    str) + "/" + df['amonth'].astype(str) + "/" + df['aday'].astype(str)
...
for cause_type in ['bundle', 'icg']:
    ...
    write_path = FILEPATH.format(
        r=run_id, y=year, l=loc, c=cause_type)
