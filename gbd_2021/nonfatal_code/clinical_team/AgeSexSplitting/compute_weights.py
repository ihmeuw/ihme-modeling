...
# load our functions
from clinical_info.Functions import hosp_prep, gbd_hosp_prep, demographic_utils
...
def test_weight_sources(df):
    ...
    if bad_sources:
        assert False, (
            "source(s) {} don't have min age 0 and source(s)"
...
def _check_for_missing_icgs(df, map_version="current"):
    ...
    full_icg_list = clinical_mapping.get_clinical_process_data(
...
def sqr_by_group(df, groups, level, clinical_age_group_set_id):
    ...
    tq = (
        df[["source", "age_group_id", "product"]]
        .groupby(["source", "age_group_id"])
        .nunique()
    )
    if tq["product"].min() == 1:
        res = tq[tq["product"] == 1]
...
def prep_weights(
    ...
    if squaring_method == "broad":
        template = hosp_prep.make_square(df, clinical_age_group_set_id)
        df = template.merge(
            df,
            how="left",
            on=[
    ...
    df = (
        df.groupby(by=group_cols)
        .agg({"counts": "sum", "population": "sum"})
        .reset_index()
    )
...
def compute_weights(
    ...
    Args:
        ...
        gbd_round_id: int
            (deprecated) indicates which gbd round to use when pulling in central population
            estimates
        decomp_step: str
            (deprecated) indicates which decomp step to use within the gbd round above
        squaring_method: str
        	'broad' or 'bundle_source_specific'. Indicates when a zero should be added. if broad
            will add zeros for all sources, if bundle_source_specific will only add a zero when
            a given disease was coded by that source
    ...
    assert round_id == 1 or round_id == 2, "round_id isn't 1 or 2, it's {}".format(
        round_id
    )
