...
def db_connection():
    ...
    mariadb_connection = mariadb.connect(
        user=user, password=password, database="FILEPATH", host=host
    )
...
def get_creds():
    ...
    Per the documentation on FILEPATH of the hospital repo, store your
    ...
    try:
        ...
        assert False, (
            "You must store your clinical data sandbox credentials in "
            "FILEPATH/.db_credentials.json. Please see the FILEPATH "
...
def update_table(df, tbl_name, merge="append", map_version=-1, dev=True):
    ...
    df.to_sql(tbl_name, con=engine, if_exists=merge, schema="FILEPATH", index=False)
    # con.close()
...
def test_df(df, tbl_name):
    ...
    # Replace Null values with -1. We needs this conversion to avoid errors with the
    # call to cast_to_int
    ...
    for i, r in df.iterrows():
        col = r["col_name"]
        tbl.loc[
            (tbl.icg_id == r["icg_id"]) & (tbl[col] == r["old_val"]), [col, "update"]
        ] = [r["new_val"], 1]

    # Since the data is stored long we need to cast certain cols from obj to int
    tbl = cast_to_int(tbl, tbl_name)

    # Cast back to Nan value. Cant drop yet since we need to retain these changes
    # for the update_df
...
def insert_icg_helper(tbl_name, df):
    ...
    df_sub = pd.pivot_table(
        df_sub, index="icg_id", values="value", columns="col_name", aggfunc=np.sum
    ).reset_index()
...
def insert_icg(df, dev=True, update_version=False):
    ...
    # Check for a complete update for each icg entry
    for id in ids:
    ...
    assert tbl[
        tbl.icg_id.isin(icg_new)
    ].empty, "You are adding ICGs that already exisit"
    ...
    # We want to add the new icg / cc to each table all at once, to mimic a SQL transaction,
    # to avoid an incomplete insert.
    if count == 3:
...
def update_duration(df, dev=True, update_version=False):
    ...
    if "icg_measure" in df.col_name.unique():
        ...
        assert (
            len(r) == 0
        ), "There is an incomplete update for icg(s) switching measures"
...
def update_age_sex_restriction(df, dev=True, update_version=False):
    """
    There really isn't a way to test the age_sex restrictions, outside of logical data 
    validations since these decisions are made by USER and company. 
    ...
    assert tbl.loc[
        tbl.yld_age_end > 95, "yld_age_end"
    ].any(), "There are ages that exceede 95"
    assert tbl[
        tbl.yld_age_start > tbl.yld_age_end
    ].empty, "Start ages greater than End ages"
...
def update_map_version(dev=True, tables=None):
    ...
    tables = ["cause_code_icg", "icg_bundle", "icg_durations", "age_sex_restrictions"]
