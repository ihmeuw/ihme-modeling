...
class Uncertainty(object):
    ...
    def _check_input(self) -> None:
        ...
        missing_columns = [
            col for col in Uncertainty._required_columns if col not in self.df.columns
        ]
        if missing_columns:
        ...
        can_calc_mean = self.df["mean"].notnull() | (
    ...
    def fill_ess_ss(self) -> None:
        ...
        """
        self.df.loc[
            self.df.effective_sample_size.isnull(), "effective_sample_size"
    ...
    def fill_uncertainty_type(self) -> None:
        ...
        self.df.loc[
            (self.df.uncertainty_type.isnull()) & (self.df.standard_error.notnull()),
            "uncertainty_type_id",
        ] = 1
        ...
        self.df.loc[
            (self.df.uncertainty_type.isnull()) & (self.df.sample_size.notnull()),
            "uncertainty_type_id",
        ] = 4
...
def ui_from_ess(
    ...
    if param_type == "rate":
        ...
        use_se_mask = cases <= 5
    if use_se_mask.any():
        ui = ui_from_se(p=p[use_se_mask], se=se[use_se_mask], param_type=param_type,)
...
def fill_single_measure(
    ...
    if not df_se_from_ui.empty:
        df_se_from_ui["standard_error"] = se_from_ui(
            ...
            method=ratio_type,
            confidence=(
                df_se_from_ui["uncertainty_type_value"].astype(float).values / 100
            ),
        )
