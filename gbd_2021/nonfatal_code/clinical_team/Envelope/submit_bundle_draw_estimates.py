...
def get_demos(all_years, clinical_age_group_set_id):
    """
    Returns 3 lists which we'll use to send jobs in parallel and check our outputs
    """
    age_groups = (
        hosp_prep.get_hospital_age_groups(clinical_age_group_set_id)
        .age_group_id.unique()
        .tolist()
    )
    sexes = [1, 2]
    if all_years:
        years = np.arange(1988, 2020, 5)
    else:
        years = np.arange(2003, 2017, 5)
        warnings.warn(
            "This is running on a subset of years, specifically {}".format(years)
        )
    warnings.warn("update this function if 2023 or later data is added")
...
def check_files(run_id, age_groups, sexes, years):
    ...
    """
    exp_prep_files = [
        "{}.H5".format(f) for f in exp_file_getter(age_groups, sexes, years)
    ]
    exp_final_files = [
        "{}.csv".format(f) for f in exp_file_getter(age_groups, sexes, years)
    ]
...
def main_bundle_draw_submit(
    run_id, draws, gbd_round_id, decomp_step, clinical_age_group_set_id
):
    """
    ...
    age_groups, sexes, years = get_demos(
        all_years=True, clinical_age_group_set_id=clinical_age_group_set_id
    )
