...
def create_draws(df, draws, draw_name="draw_"):
    ...
    msg = "Why is this function being used? the data should already have draws"
    warnings.warn(f"\n\n\n{msg}\n\n\n\n")
    ...
    for draw in range(0, draws, 1):
        df["{}{}".format(draw_name, draw)] = np.exp(
            np.random.normal(
                loc=np.log(df["mean"]), scale=df["est_se"], size=df.shape[0]
            )
        )
...
def get_est_se(df):
    ...
    """
    df["est_se"] = (
        ((np.log(df["mean"]) - np.log(df["lower"])) / 1.96)
        + ((np.log(df["upper"]) - np.log(df["mean"])) / 1.96)
    ) / 2
    return df
...
def make_square(df, use_draws, run_id, try_to_break=False):
    ...
    assert "icg_id" not in df.columns, ("Data must not be at the", " icg_id level")
    if use_draws:
        chk_col = "draw_0"
    else:
        chk_col = "mean"
    ...
    check_mean = df.loc[df[chk_col] > 0, chk_col].sort_values().reset_index(drop=True)
    if try_to_break:
        check_mean.iloc[3] = 0.030214
    ...
    df = make_zeros(
        ...
        use_draws=use_draws,
        etiology="bundle_id",
        cols_to_square=[chk_col],
    )

    # assert the sorted means are identical
    assert (
        check_mean
        == df.loc[df[chk_col] > 0, chk_col].sort_values().reset_index(drop=True)
    ).all()
...
def merge_population(df, run_id, gbd_round_id, decomp_step, use_draws):
    """
    ...
    hosp_prep.report_if_merge_fail(
        df,
    ...
    for col in cols_to_count:
        if use_draws:
            ...
            assert (
                ...
                .sum()
                == 0
            ), "We expect all maternal counts to be null here"
            ...
            assert (
                ...
                .sum()
                == 0
            ), "We expect Zero maternal counts to be null here"
...
def pooled_square_builder(
    loc, est_types, ages, sexes, loc_bundle, loc_years, eti_est_df, etiology
):
...
def make_zeros(df, cols_to_square, run_id, use_draws, etiology="bundle_id"):
    """
    ...
    # location years based on full coverage locations, ie we don't want to create a
    # bunch of zero draws for UTLA data
    if use_draws:
        loc_years = DRAWS_LOC_YEARS
    else:
        loc_years = FC_LOC_YEARS
    ...
    locs = loc_years.location_id.unique()
    ...
    assert pre == sqr_df.shape[0], "Should cases have increased from {} to {}".format(
        pre, sqr_df.shape[0]
    )
    ...
    assert pre == sqr_df.shape[0], "Rows should not have changed from {} to {}".format(
        pre, sqr_df.shape[0]
    )
...
def merge_denominator(df, run_id, use_draws, full_coverage_sources):
    ...
    pre = df.shape[0]
    df = df.merge(
        denoms, how="left", on=["age_group_id", "sex_id", "year_id", "location_id"]
    )
...
def aggregate_to_dismod_years(df, use_draws):
    ...
    if df[chk].max() > df["population"].max():
        warnings.warn(
            "Some {} are insanely large so we're matching them to 5 times population".format(
                chk
            )
        )
    ...
    # for aggregated data that the Data Indexers (USER for GBD 2016)
    # makes for us.
    ...
    assert lost_cases < accepted_loss, (
        ...
        "From {} to {}. A difference of {}".format(
...
def create_source_map(df):
    ...
    source_replace = {
        "CHN_SHD": "CHN_NHSIRS",
        "GEO_Discharge_16_17": "GEO_COL_14",  # shouldn't matter to leave it in
...
def run_helper(
    ...
    df = merge_denominator(
        df, run_id, use_draws=use_draws, full_coverage_sources=full_coverage_sources
    )

    df = aggregate_to_dismod_years(df, use_draws=use_draws)
    df = count_to_rate(
        df, use_draws=use_draws, review=False, maternal_ests=maternal_ests
    )
...
if __name__ == "__main__":
    ...
    base = f"FILEPATH"
    pic = pd.read_pickle(f"FILEPATH")
    # read and subset input for bundle data
    read_path = f"FILEPATH"

    df = read_split_icg(
        read_path=read_path, age_group=age_group, sex=sex, year_bin_start=year_bin_start
    )

    # source map needs ALL data
    source_map = create_source_map(df)
    # location years do too!
    ...
    no_draw_df = df[
        ((df["source"].isin(full_coverage_sources)) & (df["estimate_id"] == 1))
        | (df["estimate_id"] == 6)
    ].copy()
    ...
    draw_df = df[
        (df["estimate_id"] == 1) & ~(df["source"].isin(full_coverage_sources))
    ].copy()

    assert df.shape[0] == (
        draw_df.shape[0] + no_draw_df.shape[0]
    ), "something went wrong in the data split"
    del df

    if no_draw_df.shape[0] == 0:
        warnings.warn(
            "There is no data for this age/sex/year group. Mostly likely because "
            "it's for males in an early gbd year"
        )
        no_draw_df = pd.DataFrame()
    else:
        no_draw_df = run_helper(
            df=no_draw_df,
            run_id=run_id,
            use_draws=False,
            gbd_round_id=gbd_round_id,
            decomp_step=decomp_step,
            maternal_ests=[6, 7, 8, 9],
            full_coverage_sources=full_coverage_sources,
        )
    if draw_df.shape[0] == 0:
        warnings.warn(
            "There is no data for this age/sex/year group. Mostly likely because "
            "it's in an early gbd year"
        )
    ...
    # should be fine to concat empty df with populated df
    ...
    df = write_draws(
        df, run_id=run_id, age_group=age_group, sex=sex, year_bin_start=year_bin_start
    )
    runm = (time.time() - start) / 60
    print(
        "prepping to apply bundle data w/env uncertainty ran in {} minutes".format(runm)
    )
