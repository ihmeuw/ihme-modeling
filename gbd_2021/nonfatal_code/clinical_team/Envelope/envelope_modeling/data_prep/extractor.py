...
def get_roots():
    if os.name=='posix':
        ...
        return 'FILEPATH', 'FILEPATH'
root_j, root_h = get_roots()

def change_paths(path, new_root="FILEPATH", old_root= "FILEPATH"):
...
class extractor(object):
    ...
    def check_number_of_responses_2(self):
        """ When multiple variables apply to one field, make sure there is a matching number of items"""
    ...
    def __yes_no__(self, reader_var):
        ...
        for t in np.arange(len(self.reader[b])):
            ...
            if not is_nan(yes_opt) and not is_nan(no_opt) :               
    ...
    def __cont__(self, reader_val):
        ...
        self.collect[b] = self.all_data[self.reader[b]].sum(axis=1, skipna=False).values
    ...
    def get_gender_5(self):
        ...
        if 'nan' not in list(map(str,self.reader['sex'])):
            ...
            if (self.collect.sex=='Both').sum()/len(self.collect)> .4:
                print('No sex was assigned to over 40 percent of the respondents....')
    ...
    def get_age_6(self):
        ...
        if 'nan' not in list(map(str, self.reader['age_yr'])):
            ...
            if 'age_yr' in self.collect.columns:
                ...
                self.collect.loc[(self.collect['age_hold']<2) & (self.collect['age_hold']>=1), "age_yr"] = 1
                self.collect.loc[(self.collect['age_hold']<2) & (self.collect['age_hold']>=1), "age_end"] = 2
                self.collect.loc[(self.collect['age_hold']<5) & (self.collect['age_hold']>=2), "age_yr"] = 2
                self.collect.loc[(self.collect['age_hold']<5) & (self.collect['age_hold']>=2), "age_end"] = 5

                del self.collect['age_hold']
                

            if (self.collect.age_yr==999).sum()/len(self.collect)> .4:
                print('No age was assigned to over 40 percent of the respondents....')
                print((self.reader['nid']))
    ...
    def __group_bin__(self):
        """ all bins have to have at least thirty observations and 2 cases. probably an easier way to do this but...hey"""
    ...
    def get_numbers_12(self, miss_labs=[np.nan, '', '.', ],
        ...
        if zero_check/sample_check>.2:
            ...
            if self.reader['nid'][0] not in ['138595', '66763','60405', '31770', '111486', '111487', '238523', '111488', '95306']: 
...
#########################################################################
## gdoc
## Re-formats the google doc to an xlsx that can be extracted

def gdoc_query(url='FILEPATH'):

    response = requests.get(url)
    df_reader =pd.read_excel(BytesIO(response.content)).T 
