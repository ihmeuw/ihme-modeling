...
cat('IHME ACT 5km WITH drug resitance\n')
...
for(xxx in 1:length(time_points)){
  ...
  for(counter in 6){
    ...
    cn<-raster('FILEPATH.tif') # main country layer
    NAvalue(cn)=-9999
    pred_val<-getValues(cn)#get values again
    w<-is.na(pred_val) #find NAs again
    index<-1:length(w) 
    ...
    irs_r<-raster(paste0('/FILEPATH',pred_year_IRS,'.IRS.tif')) # main country layer
    irs_r2<-raster(paste0('/FILEPATH',pred_year_IRS-1,'.IRS.tif')) # main country layer
    NAvalue(irs_r)=-9999
    NAvalue(irs_r2)=-9999
    ...
    if(counter==4 | counter==5) {
      #counter factual
      act_r<-raster(paste0('/FILEPATH',pred_year_ACT,'.tif'))# main country layer
      act_r2<-raster(paste0('FILEPATH',pred_year_ACT-1,'.tif')) # main country layer
      # factual
      ...
      #	act_r<-raster(paste0('/FILEPATH,'.ACT.tif')) # main country layer
      #	act_r2<-raster(paste0('FILEPATH'.ACT.tif'))# main country layer
    ...
    cn<-raster('FILEPATH') # main country layer
    NAvalue(cn)=-9999
