...
apply_age_split <- function(data, 
  ...
  #repo_dir <- paste0(root, "age_split/")
  ...
  expand_age <- function(small_dt, age_dt = ages){
    ...
    if (!(is.na(min_cases))){
      cat(paste0("\n Dropping cases that are less than ", min_cases, ". Splitting low case numbers into ~40 bins (sex split and age split) produces odd results at times. Control this with the min_cases argument.\n"))
      dt <- dt[cases >= min_cases,]
      dt[cases >= min_cases, drop := 0.5]
      } else {
      cat(paste0("\n Not dropping cases, be careful since splitting low case numbers into ~40 bins (sex split and age split) produces odd results at times. Control this with the min_cases argument. First try is to drop where cases less than bins (40).\n"))
      }
    ...
    split <- split[age_group_id %in% age | age_group_id == 1] ## age group id isn't estimated for cause
    return(split)
  }
  ...
  get_age_pattern <- function(locs, id, age_groups){    
    ...
    if (is.na(prop_smm)){
    age_pattern <- get_draws(source = ADDRESS,
    ...
    se <- copy(age_1)
    se <- se[age_group_id==5, .(measure_id, sex_id, se_dismod, location_id)] ##use standard error from 1-4 age group
  ...
  age_split <- function(gbd_id, df, age, region_pattern, location_pattern_id){
    ...
    ## BREAK IF NO ROWS
    if (nrow(final_dt) == 0){
      print("nothing to age-sex split")
      break
    }
    return(final_dt)
  }
  ...
  cat(paste0("\n\n Splitting into ", start_split, " until ", end_split, " You can control these paremeters with the start_split and end_split arguments. 
                                                                          They are should correspond to gbd age group starts and ends.\n"))
