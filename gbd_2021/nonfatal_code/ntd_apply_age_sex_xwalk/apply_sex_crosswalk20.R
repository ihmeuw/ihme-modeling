...
add_population_cols <- function(data, gbd_round, decomp_step){
  ...
  data_ss2<- data_ss %>% 
    filter(!is.na(male_population)) %>%
    group_by(split_seq) %>% 
    summarise(male_population = sum(male_population), female_population=sum(female_population), both_population=sum(both_population), na.rm=TRUE)
  
  data <- merge(data, data_ss2, by="split_seq", all=T)
...
apply_sex_crosswalk <- function(mr_brt_fit_obj, all_data, decomp_step){
  ...
  cat("\nStarting sex crosswalk. Populations are aggregated by single-year ages but not by year. Will add this functionality soon.\n")
  ...
  unadj_df <- add_population_cols(data = unadj_df, gbd_round = round_id, decomp_step = decomp_step)
