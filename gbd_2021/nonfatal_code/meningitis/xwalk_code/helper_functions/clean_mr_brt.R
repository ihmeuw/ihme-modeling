...
##
## Updates: 5/12/2020 using delta_transform() from Reed's crosswalk package
##
...
#suppressPackageStartupMessages(library(combinat, lib='/share/code/cvd/minh_rlibs'))
library(msm)
...
match_mr_brt <- function(data,
  ...
  ## If toggled, this will match subnational data points to national data points and vice versa. 
  if (subnat_to_nat) {
  ...
  for (n in 1:ncol(comb)) {
    ...
    if (nrow(df) > 0) {
      ...
      ## Needlessly complicated ifelses -- matches on mean +/- overlap if the range is not large(r than specified), exact matches if it's a large range.
...
log_transform_mrbrt <- function(df) {
  ...
  df[, c("log_mean_alt", "se_log_mean_alt") := data.table(delta_transform(mean = mean_alt, sd = alt_se, transformation = "linear_to_log"))]
  df[, c("log_mean_ref", "se_log_mean_ref") := data.table(delta_transform(mean = mean_ref, sd = ref_se, transformation = "linear_to_log"))]
...
sex_match <- function(data,
  ...
  if (drop_zeros) {
    ...
    if (!quiet) message("Dirty fix for mean=0 or 1, just dropping those data points. Working on a better fix.")
  }
...
##sy: identify rows where mean is contained inside adjusted SEWhat is saved when you make a "commit"?
...
get_con <- function(dbname, host) {
  ...
  credentials <- fread(paste0(j, "temp/central_comp/credentials/default.csv"))
