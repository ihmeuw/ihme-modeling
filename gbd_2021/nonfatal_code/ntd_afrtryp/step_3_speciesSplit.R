...
# for by each species in all endemic locations for years 1980-2019            #
###############################################################################
...
gr_specie_r <- fread(paste0(params_dir, "FILEPATH"))
gr_specie_r <- gr_specie_r[! (type %in% c("citation", "quality")), .SD, .SDcols = c("loc_id", "pre1980", 1980:2019)]
setnames(gr_specie_r, old='pre1980', new='1979')
# Reshape from wide to long
gr_specie_r_long <- melt(data=gr_specie_r, id.vars="loc_id", variable.name="year_id")
# Convert status into binary var
gr_specie_r_long[, `:=` (status_r = ifelse(value %in% c('p','pp'), 1,0), value = NULL)]


## (2) Process geographic restrictions - species G ##
gr_specie_g <- fread(paste0(params_dir, "FILEPATH"))
gr_specie_g <- gr_specie_g[! (type %in% c("citation", "quality")), .SD, .SDcols = c("loc_id", "pre1980", 1980:2019)]
setnames(gr_specie_g, old='pre1980', new='1979')
# Reshape from wide to long
gr_specie_g_long <- melt(data=gr_specie_g, id.vars="loc_id", variable.name="year_id")
# Convert status into binary var
gr_specie_g_long[, `:=` (status_g = ifelse(value %in% c('p','pp'), 1,0), value = NULL)]
...
# Rename loc_id
setnames(specie_split_dt, 'loc_id', 'location_id') 

# Convert year_id to numeric
specie_split_dt[, year_id := as.numeric(as.character(year_id))]
...
saveRDS(speciesSplit, file = paste0(interms_dir,"FILEPATH"))
