...
## (1) Import case data
Data2Model <- readRDS(file = paste0(interms_dir,"FILEPATH"))
...
#179, 193, and 213 need rows for 1990-2019
rows <- Data2Model[location_id %in% c(179, 193, 213) & year_id==1989][rep(seq_len(3), each=30),]
rows$year_id <- rep(1990:2019, times=3)
...
leave_out <- Data2Model[year_id %in% 2019:2022 | location_id %in% c(179, 185, 193, 209, 213)]
Data2Model <- Data2Model[year_id < 2019 & ! is.na(value_ppl_risk)]
...
# NOTE 2: Special handling required for location_ids 208, 168, 173
# These countries experienced a recent increase in cases causing the roc to be too large

#2019
Data2Model[, `:=` (lag_inc_risk = shift(incidence_risk, n=1, type="lag"), lag_roc = shift(roc, n=1, type="lag")), by=location_id]
Data2Model[year_id==2019, incidence_risk := lag_inc_risk * lag_roc]
Data2Model[location_id %in% c(208, 168, 173) & year_id==2019, incidence_risk := lag_inc_risk]
#2020
Data2Model[, `:=` (lag_inc_risk = shift(incidence_risk, n=1, type="lag"), lag_roc = shift(roc, n=1, type="lag")), by=location_id]
Data2Model[year_id==2020, incidence_risk := lag_inc_risk * lag_roc]
Data2Model[location_id %in% c(208, 168, 173) & year_id==2020, incidence_risk := lag_inc_risk]
#2021
Data2Model[, `:=` (lag_inc_risk = shift(incidence_risk, n=1, type="lag"), lag_roc = shift(roc, n=1, type="lag")), by=location_id]
Data2Model[year_id==2021, incidence_risk := lag_inc_risk * lag_roc]
Data2Model[location_id %in% c(208, 168, 173) & year_id==2021, incidence_risk := lag_inc_risk]
#2022
Data2Model[, `:=` (lag_inc_risk = shift(incidence_risk, n=1, type="lag"), lag_roc = shift(roc, n=1, type="lag")), by=location_id]
Data2Model[year_id==2022, incidence_risk := lag_inc_risk * lag_roc]
Data2Model[location_id %in% c(208, 168, 173) & year_id==2022, incidence_risk := lag_inc_risk]
...
saveRDS(all_ages, file = paste0(interms_dir,"FILEPATH"))
...
pops <- get_population(age_group_id=age_groups , location_id=locations,
                       year_id=(years), sex_id=c(1,2), gbd_round_id=gbd_round_id,
...
saveRDS(hat, file=paste0(interms_dir,"FILEPATH"))
...
A1 <- 1884       ## number of cases with sleeping disorder (Blum et al 2006)
A2 <- 2533 - A1  ## number of cases without sleeping disorder 
