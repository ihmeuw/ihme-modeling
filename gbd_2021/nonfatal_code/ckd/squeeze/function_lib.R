...
construct_qsub<-function(mem_free, threads, runtime, script, pass, description, submit=FALSE,
                         shell = "FILEPATH", 
                         project = "FILEPATH", q = "all.q",
    ...
    sys_sub<- paste0('qsub -cwd -P ', project, " -N ",description ," -e ", errors, ' -o ',output, ' -l m_mem_free=', mem_free, 'G', " -l fthread=", threads,
                     " -l h_rt=", runtime, " -q ", q)
}
...
job.array.master <- function(tester, paramlist, username, project, threads, mem_free, 
                             runtime, q, description, errors, childscript, shell, args = NULL) {
  ...
  sys_sub <- paste0("qsub -P ", project, " -N ", description ," -e ", errors, " -t 1:", n, " -tc 500 -l m_mem_free=", mem_free, 
...
check_missing_locs<-function(indir, filepattern, team, round = 6, step = 'step4'){
  ...
  filepattern <- gsub(pattern = "\\..*", replacement = "", x = filepattern)
...
# structure
  get_epi_path<-function(bundle_id,acause,upload){
    if (upload==T){folder<-"FILEPATH"}else{folder<-"FILEPATH"}
    path<-paste0("FILEPATH")
    return(path)
  }

# Read in a given request number from a specified bundle/acuase combination from the "FILEPATH" folder
...
# Read in a given upload file (via filename) from a specified bundle/acause combiation from the "FILEPATH"
...
# Write a file to the FILEPATH in the FILEPATH structure for a given
...
# Calculate standard error using Wilson formula
...
are_there_duplicates <- function(dataframe, return_dataframe = FALSE, n = 20) {
  ...
  # Bind columns to see which ones are duplicated
