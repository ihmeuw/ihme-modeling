...
# Estimating equation comparisons only apply to data age 18+ (all under 18 sources use data table
# Schwartz)
dt<-dt[age_start>=18]
...
if (apply_offset){
  message(paste0("applying offset to data where both numerator and denominator are 0. offset is calculated to be ", offset_pct,"% of the median of all non-zero datapoints.
  ...
  # Recalculate standard error after applying the offset
...
# Transform alternate and reference into appropriate space
...
if (mod_type=="logit_diff"){
  ...
  # Assume additive variance to calculate SE of logit difference
