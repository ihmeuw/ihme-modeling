...
source("FILEPATH/data_processing_functions.R"))
...
# deal with backslashes in albuminuria cv_cols
...
if (nrow(data[is.na(mean)])>0){
  ...
  # fill in missing means as cases/sample size
...
# make sure every row has standard error(SE)
if (nrow(data[is.na(standard_error)])>0){
  ...
  # fill in missing standard error values
  ...
  if(nrow(data[is.na(standard_error)&measure%in%c("prevalence","proportion")])>0){
    ...
    if(nrow(data[is.na(standard_error)&measure%in%c("prevalence","proportion")])>0){
      ...
      stop(paste("the following prev/prop data seqs are missing standard error and mean, sample size, lower, and upper. 
  ...
  if (nrow(data[is.na(standard_error)&measure%in%c("incidence")])>0){
    ...
    message(paste(missing_se,"rows of inc data are missing a standard error value. filling in based on mean and sample size"))
...
# aggregate cases and sample size for each study-sex-year-measure-covariate combo
...
if (mod_type=="logit_diff"){
  ...
  if (apply_offset==F){
    message("dropping rows where ratio and ratio SE are NA or infinite because offsetting was not implemented")
