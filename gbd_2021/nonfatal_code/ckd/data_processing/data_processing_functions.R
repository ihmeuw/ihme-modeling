...
age_sex_split_lit<-function(dt,split_indicator,return_full=T){
  ...
  # Create data table with just sex specific data
  ...
  # Mark which nids have 0 total cases
...
apply_mrbrt_sex_split <- function(input_dt, acause, bid, bvid, ds, gbd_rnd, trim_pct=0.1){
  ...
  #' @param input_dt a data.table containing data in the epi database dismod shape which 
  ...
  #' @param ds decomp step to pull population from
  ...
  # merge together and tack onto data table
  ...
  # drop male and female population values
  split_dt[,c("Male_population","Female_population"):=NULL]
  
  # melt sex-specific columns long
...
apply_mrbrt_xwalk <- function(input_dt, bid, alt_def, cv_ref_map_path){
  ...
  if (nrow(cv_ref_map)>1) stop("the bundle_id-alt_def combination supplied identifies more than one crosswalk model in the cv_ref_map")
