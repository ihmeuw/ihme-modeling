  ...
  df <- dataset[,c("author", "cases", "sample_size", "mean", "standard_error", "cv_nonrandom")]
  ...
  model <- MRBRT(
    ...
    cov_models =list(
      ...
      LinearCovModel("cv_nonrandom", use_re = FALSE)
    ),
    inlier_pct = 1)
  ...
  ggsave(plot, filename=paste0(outputfolder,"FILEPATH", version, "_prop_asymptomatic.pdf"), width = 6, height = 4)
  ...
  if (age_pattern == 1) {
    ...
    ggsave(plot, filename=paste0(outputfolder,"FILEPATH", version, "_prop_asymptomatic_age_pattern.pdf"), width = 6, height = 4)
    ...
    final_draws <- reshape(draws, idvar = c("intercept"), timevar = "draw", direction = "wide")
  ...
  if(save_final==1) {
    write.csv(final_draws, file =paste0(outputfolder, "final_asymptomatic_proportion_draws.csv"))
  }
