...
## Project: NF COVID
## Script: 
## Description: Compile Iranian Data with both Pre and Post Covid Information
## Date 2/16/2021
## --------------------------------------------------------------------- ----
...
dt <- age_pattern[,c('id.2',"Dm3","age_group_name","Dm5","Hp19a6","Hp19a8","Hp19a9","Hp19a10","mha12","mha25",
                     "mha92","mha91","mha94","mha111","mha112","mha113","Hp19a4","Hp19b4","mhb25","mhb261","mhb251","mha1021",
                     "mha103")]

#Fatigue/pain/emotional problem cluster:
#Rule 1: a case must have answered Hp19a.6 (‘reduced ability for daily functions prior to COVID’)= 2 (no) and 
#at least two out of Hp19a.8 (‘feeling sad most of them time prior to COVID’), Hp19a.9 (‘frustration and no hope prior to COVID’), and Hp19a.10 (‘dissatisfaction and not enjoying life prior to COVID’)= 2; I also see questions on first page of Q3 asking history before admission of fatigue which could also apply.
dt <- mutate(dt, fatigue_rule1 = ifelse((Hp19a6 %in% "no") & ((Hp19a8 %in% "no" & Hp19a9 %in% "no") | (Hp19a8 %in% "no" & Hp19a10 %in% "no") | 
                                                               (Hp19a9 %in% "no" & Hp19a10 %in% "no")), 1,0))

table(dt$fatigue_rule1)
summary(dt$fatigue_rule1)
...
dt <- mutate(dt, fatigue_rule2 = ifelse((mha12 %in% "yes" | mha25 %in% "yes" | mha92 %in% "yes") & ((mha91 %in% "yes" | mha94 %in% "yes") | (mha111 %in% "yes" | mha112 %in% "yes")), 1,0))
table(dt$fatigue_rule2)
summary(dt$fatigue_rule2)

#combine the fatigue category
dt <- mutate(dt, post_acute = ifelse((fatigue_rule1 %in% 1 & fatigue_rule2 %in% 1), 1, 0))
table(dt$post_acute)
...
dt <- mutate(dt, cognitive = ifelse((mha113 %in% "yes" & Hp19a4 %in% "no" & Hp19b4 %in% "yes"), 1,0))
table(dt$cognitive)
...
#res mild
dt <- mutate(dt, res_mild = ifelse(((mha1021 %in% "intense activity" & mha103 %in% 'no' ))& (((mhb25 %in% "no") & (mhb261 %in% "0"))|
                                     (mhb25 %in% "yes" & mhb251 %in% "exercise" & mha1021 %in% c("rest","normal activity"))), 1,0))
table(dt$res_mild)
summary(dt$res_mild)

#res_moderate
dt <- mutate(dt, res_moderate = ifelse(((mha1021 %in% "normal activity" & mha103 %in% 'no'))& (((mhb25 %in% "no") & (mhb261 %in% "0"))|
                                                                         (mhb25 %in% "yes" & mhb251 %in% "exercise" & mha1021 %in% c("rest","normal activity"))), 1,0))
table(dt$res_moderate)
summary(dt$res_moderate)

#res_severe
dt <- mutate(dt, res_severe = ifelse((mha103 %in% "yes"| mha1021 %in% "rest")& (((mhb25 %in% "no") & (mhb261 %in% "0"))|
                                                                            (mhb25 %in% "yes" & mhb251 %in% "exercise" & mha1021 %in% c("rest","normal activity"))), 1,0))
table(dt$res_severe)
...
#28 overlaps in the respiratory category. need to round them one level up
...
write.csv(DT, paste0("Iran_all_data_marked_v", version, ".csv"))
...
write.csv(cocurr_mu, paste0("Iran_byAgeSex_", version, ".csv"))
...
write.csv(cocurr_sex, paste0("Iran_bySex_v", version, ".csv"))
...
pdf(file= 'Iran_data_plots.pdf', height=11, width=8.5)
#combined categories
