...
main <- function(version_tag, loc_id) {
  ...
  # Infections
  inf <- readRDS(paste0('FILEPATH', loc_id, '.rds'))[measure_name == 'infections',
...
#  inf$draw_orig <- NULL
  ...
  # Hospitalizations
  hsp <- readRDS(paste0('FILEPATH', loc_id, '.rds'))[measure_name == 'hospitalizations',
                                                  c('location_id', 'age_group_id', 'sex_id', 
                                                    'date', roots$draws), with=F]
...
#  hsp$draw_orig <- NULL
  ...
  # ICU Admissions
  icu <- readRDS(paste0('FILEPATH', loc_id, '.rds'))[measure_name == 'icu',
                                                  c('location_id', 'age_group_id', 'sex_id', 
                                                    'date', roots$draws), with=F]
...
#  icu$draw_orig <- NULL
  ...
  # Deaths
  dth <- readRDS(paste0('FILEPATH', loc_id, '.rds'))[measure_name == 'deaths',
                                                  c('location_id', 'age_group_id', 'sex_id', 
                                                    'date', roots$draws), with=F]
...
#  dth$draw_orig <- NULL
  ...
  hsp$hsp_admit_too_large_mask <- 0
...
#         paste0('FILEPATH', /hsp_admit_adjustments_', loc_id, '.csv'))
  ...
  adj_factors <- merge(adj_factors, shift_inf, 
                       by=c('location_id', 'age_group_id', 'sex_id', 'date', 'variable'), all.y=TRUE)
  adj_factors[is.na(deaths), deaths := 0]
  rm(shift_inf)
  ...
  prop_asymp$V1 <- NULL
  prop_asymp$age_group_id <- 1
  prop_asymp <- melt(prop_asymp[, c('age_group_id', roots$draws), with=F], 
                     measure.vars = roots$draws)
  setnames(prop_asymp, 'value', 'prop_asymp')
  prop_asymp$age_group_id <- NULL
  adj_factors <- merge(adj_factors, prop_asymp, by=c('variable'))
  rm(prop_asymp)
  
  
  
  # for rows where deaths are greater than infections
...
#  table(adj_checks$adjust_asymp_prop_mask, useNA='always')
  ...
  write_feather(prop_asymp, paste0('FILEPATH', loc_id, '_prop_asymp.feather'))
  ...
  write_feather(inf, paste0('FILEPATH', '_infecs.feather'))
  rm(inf)
  ...
  rm(hsp)
  write_feather(new_hsp, paste0('FILEPATH', 'hospital_admit_', loc_id, '.feather'))
  rm(new_hsp)
  ...
  rm(icu)
  write_feather(new_icu, paste0('FILEPATH', 'icu_admit_', loc_id, '.feather'))
  rm(new_icu)
  ...
  dth <- dcast(dth, formula = 'location_id + age_group_id + sex_id + date ~ variable',
               value.var='deaths')
  write_feather(dth, paste0('FILEPATH', loc_id, '_deaths.feather'))
  rm(dth)
  ...
  write_feather(adj_factors, paste0('FILEPATH', 
  ...
  # Write data
#  fwrite(df, paste0('FILEPATH', '/adjustment_factors_', loc_id, '.csv'))
