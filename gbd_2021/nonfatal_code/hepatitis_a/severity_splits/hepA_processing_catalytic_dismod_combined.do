...
\******************************************************************************/
    ...
    * PULL IN DRAWS FROM DISMOD MODELS FOR ANTI-HAV PREVALENVCE *
      interpolate, gbd_id_type(modelable_entity_id) gbd_id(`meid') location_id(`location') age_group_id(`ages') measure_id(5) source("epi") status(best) reporting_year_start(1990) reporting_year_end(2022) gbd_round_id(7) decomp_step("iterative") clear
      rename draw_* prev_*

      gen cause_id = CAUSE_ID
...
run FILEPATH
get_population, age_group_id(-1) location_id(`location') year_id(-1) sex_id(-1) gbd_round_id(7) decomp_step("step2") clear
