...
means <- get_draws(location_id = loc, gbd_id = 10487, measure_id = 19, year_id = years, age_group_id=ages, gbd_id_type = "modelable_entity_id", source="epi", status = 'best', decomp_step='iterative', gbd_round_id=7)
stdev <- get_draws(location_id = loc, gbd_id = 10488, measure_id = 19, year_id = years, age_group_id=ages, gbd_id_type = "modelable_entity_id", source="epi", status = 'best', decomp_step='iterative', gbd_round_id=7)
  
#age-specific fertility
print("LOADING ASFR COVARIATE ESTIMATES")
source("FILEPATH/get_covariate_estimates.R")
asfr <- get_covariate_estimates(13,location_id=loc,decomp_step='iterative',gbd_round_id=7)
...
#stillbirths 
still <- get_covariate_estimates(2267,location_id=loc,decomp_step='iterative',gbd_round_id=7)[,.(location_id,year_id,mean_value)]
