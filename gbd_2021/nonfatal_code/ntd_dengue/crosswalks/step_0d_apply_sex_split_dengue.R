...
data<-read.csv("FILEPATH/input_data_sex.csv")
...
write.csv(data,"FILEPATH/prelim_sex_walk.csv")
...
df1 <- CWData(
  ...
  #covs = list("year"),     # names of columns to be used as covariates later
...
dat2<-as.data.table(read.csv("FILEPATH/dengue_URxwalk.csv"))
...
dat2_b <- merge(dat2, malepop, by=c("location_id","year_start"))

# rename population variable
dat2_b <- dat2_b %>% 
  rename(male_pop = population)

dat2_b$sex_id.x<-NULL
dat2_b$sex_id.y<-NULL
dat2_c <- merge(dat2_b, fempop, by=c("location_id","year_start")) 

dat2_c<-dat2_c %>% 
  rename(fem_pop = population)

# drop variables for clean up
dat2_c <- select(dat2_c,-c(sex_id.x,sex_id.y,year_id.x,year_id.y, run_id.y,run_id.x, age_group_id.y,age_group_id.x,upper, lower, standard_error, mean, effective_sample_size ))
...
write.csv(split_dat2,"FILEPATH/sex_split6757.csv")

table(split_dat2$sex)
