...
save_results_description <- paste0("DESCRIPTION", " - custom version ", custom_version)
mark_model_best <- TRUE
gbd_round <- 7
year_end  <- 2022
decomp <- "iterative"
xw_version_id <- 29423 # best crosswalk version for bundle_id 291 (ID envelope)
...
file.path("FILEPATH/get_draws.R") %>% source
#***********************************************************************************************************************
...
if (action=="init") {
  ...
  ### launch jobs
  JOBS <- NULL
  pop_locs <- pop_locs[!pop_locs %in% gsub(".csv", "", list.files("FILEPATH"))]
  for (loc in pop_locs) {
  ...
  upload_id_splits <- function(severity_split) {
    ...
    job <- paste0("qsub -N save_id_", severity_split, " -P proj_erf -l m_mem_free=70G -l fthread=30 -l h_rt=24:00:00 -l archive=TRUE -q all.q ",
                  shell, " ", script,
                  " --args",
                  " --type epi",
                  " --me_id ", me_id,
