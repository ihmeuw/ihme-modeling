...
calculate_incidence <- function(this_acause, this_location_id, cnf_model_version_id,
  ...
  test_incidence_draws(incidence_draws, uid_cols, is_estimation_yrs)
...
## Define Tests
################################################################################
test_incidence_draws <- function(df, uid_columns, is_estimation_yrs){
  ##
  ##
...
test_expected_sex <- function(df, acause) { 
  ...
  # load registry_entity_input table 
  sex_metadata <- fread(paste0(get_path(process='nonfatal_model', key='database_cache'), '/registry_input_entity.csv'))
  
  # subset to single acause 
  this_cause <- sex_metadata[acause == acause, ]
