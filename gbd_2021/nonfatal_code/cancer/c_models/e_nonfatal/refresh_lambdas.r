...
## age_groups <- c(6:20, 28, 30:33, 44:45, 148)
...
## See additional Mortality team documentation at http://mort-app-p01.ihme.washington.edu:1003/reference/index.html
...
source('FILEPATH/r_utils/utilities.r') # Loads utilities functions, eg. get_path
...
mortality_funcs <- "FILEPATH/functions"
...
get_req_age_groups <- function(){
  # Returns a list of required age groups
  return(c(6:20, 28, 30:33, 44:45, 148))
}

convert_lifetable <- function(dt, id_vars = get_uid_cols()) {
    dt <- copy(dt)
    print('converting lifetable...')
    req_age_groups <- get_req_age_groups()
...
calc_lambda <- function(age_id, calc_dt, age_col){
    # NOTE: log() function returns natural log (ln)
...
replace_young_ages <- function(dt, young_ages) {
    # There is no easy way to calculate the young age groups <10yo, 
    # so the '10-14' age group value is used as a substitute
    '%ni%' <- Negate('%in%')
    tmp <- dt[age_group_id == 7,]
    replace_ages <- c(1, 6, 28, young_ages)
...
generate_lambda_values <- function(dt, young_ages) {
...
# Generate Lambdas
nLx <-generate_nLx(gbd_round_id, d_step)
lambda_values <- generate_lambda_values(nLx, young_ages)

# re-assign age_group_id 7 for younger age groups 
lambda_values <- replace_young_ages(lambda_values, young_ages) 
