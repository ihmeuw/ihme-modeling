...
#ratio of females to males by population
...
split_data <- function(dt, model, rr = F){
  ...
  preds <- read.csv(paste0(mrbrt_dir, model_name, "/model_draws.csv"))
...
plot_mr_brt_custom <- function(model_object, continuous_vars = NULL, dose_vars = NULL, print_cmd = FALSE) {
  ...
  job <- paste("qsub", m_mem_free, fthread, runtime_flag,
               ...
               job_name, "-P NAME", outputs_flag, errors_flag, shell_script, script)
...
dem_sex_dt <- add_replace_data(dem_sex_dt)
...
dem_sex_dt <- age_sex_split(dem_sex_dt) 

model_name <- paste0("dementia_sexsplit_", date)

if (file.exists(paste0(mrbrt_dir, model_name, "FILEPATH"))){
  sex_model <- readr::read_rds(paste0(mrbrt_dir, model_name, "FILEPATH"))
} else {
  sex_model <- run_mr_brt(
    output_dir = mrbrt_dir,
    model_label = paste0(model_name),
    data = mrbrt_sex_dt[cv_marketscan == 0],
    mean_var = "log_ratio",
    se_var = "log_se",
    study_id = "id",
    method = "trim_maxL",
  ...
  readr::write_rds(sex_model, paste0(mrbrt_dir, model_name, "FILEPATH"))
...
ggsave(predict_graph, filename = paste0(mrbrt_dir, model_name, "FILEPATH"))
