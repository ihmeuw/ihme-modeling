...
## FUNCTION TO GET MORTALITY DATA (TO CALCULATE ATTRIBUTABLE RISK)
get_mort_data <- function(i){
  ...
  mort_path <- paste0("FILEPATH", arlit_dt[, midyear][i], ".h5")
...
lit_dt <- lit_dt[!effect_size == 0] ## GET RID OF THIS POINT BECAUSE IS EXTREME OUTLIER
...
# ATTRIBUTABLE RISK MR-BRT MODEL ---------------------------------------------------------
