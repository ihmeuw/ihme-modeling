...
prev_dt <- get_outputs(topic = "cause", measure_id = ID, year_id = pd_dt[, unique(year_id)], location_id = pd_dt[, unique(location_id)],
                       age_group_id = pd_dt[, unique(age_group_id)], sex_id = pd_dt[, unique(sex_id)], cause_id = dem_cid,
                       gbd_round_id = ID, metric_id = ID)
...
graph_mrbrt_results <- function(results, predicts, sex = T){
  ...
  gg_subset <- ggplot() +
    ...
    scale_size_continuous(name = "Inverse Variance") +
    labs(x = "Age") +
    ggtitle(paste0("Meta-Analysis Results for Relative Risk of Parkinson's Disease")) +
    theme_classic() +
