...
folder_dir <- paste0(j_root, "FILEPATH") 
save_dir <- paste0()
...
icd9 <- fread(paste0(folder_dir, "icd9_map.csv"))

## This loop progressively tries to merge on the full code provided, and then truncates for less
## precision until everything merges - at least with three truncation attempts. This logic could be 
## extended if needed
...
write.csv(dif_dt, paste0(save_dir, "differenceDemNoDem.csv"), row.names = F)
