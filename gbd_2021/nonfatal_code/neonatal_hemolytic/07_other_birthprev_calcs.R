## ******************************************************************************
##
## Purpose: Calculate EHB and kernicterus from other
## Input:   
## Output:
## Last Update: 9/9/19
##
...
pacman::p_load(data.table, dplyr, ggplot2, stats, boot, msm, foreign)
...
#import EHB prevalence numbers for each etiology (not birth prevalence)
out_dir <- 'PATHNAME'
rh <- fread(file = paste0(out_dir,'PATHNAME/rh_disease_ehb_all_draws.csv'))
g6pd <- fread(file = paste0(out_dir,'PATHNAME/g6pd_ehb_all_draws.csv'))
preterm <- fread(file = paste0(out_dir,'PATHNAME/preterm_ehb_all_draws.csv'))

#restrict Rh data down to just DisMod estimation years and specified sexes
rh <- rh[year %in% c(1990,1995,2000,2005,2010,2015,2019,2020,2021,2022)]
rh <- rh[sex %in% c(1,2)]

#melt everything into long format, and then merge the three datasets together
rh_long <- melt.data.table(rh, id.vars = names(rh)[!grepl("draw", names(rh))], 
                             measure.vars = patterns("draw"),
                             variable.name = 'draw.id')
setnames(rh_long, 'value', 'rh')
g6pd_long <- melt.data.table(g6pd, id.vars = names(g6pd)[!grepl("draw", names(g6pd))], 
                             measure.vars = patterns("draw"),
                             variable.name = 'draw.id')
setnames(g6pd_long, 'value', 'g6pd')
preterm_long <- melt.data.table(g6pd, id.vars = names(g6pd)[!grepl("draw", names(g6pd))], 
                             measure.vars = patterns("draw"),
                             variable.name = 'draw.id')
setnames(preterm_long, 'value', 'preterm')
other_long <- merge(rh_long, g6pd_long, by = c('location_id', 'sex', 'year','draw.id'), all.x = TRUE)
other_long <- merge(other_long, preterm_long, by = c('location_id', 'sex', 'year','draw.id'), all.x = TRUE)

#calculate other as 1 - (Rh prev + G6PD prev + preterm prev)
other_long[, other := 1 - (rh + g6pd + preterm)]
...
#multiply every prev draw by a draw of the ehb scalar
draw_cols <- paste0("draw_", 0:999)
ehb_scalar_table <- data.table(draw.id = draw_cols, scalar = ehb_scalar)

other_long <- merge(other_long, ehb_scalar_table, by = 'draw.id')
other_long[, value.ehb := other * scalar]


#add haqi to other prev
...
kern_long <- melt.data.table(kern_scalars, id.vars = names(kern_scalars)[!grepl("draw", names(kern_scalars))], 
                             measure.vars = patterns("draw"),
                             variable.name = 'draw.id')
setnames(kern_long, 'value', 'value.kern')

#merge on haqi and draw_id
prev_long <- merge(other_long, kern_long, by = c('haqi','draw.id'), all.x = TRUE)
...
write.csv(ehb_prev_wide, file = paste0(out_dir, 'PATHNAME/other_ehb_all_draws.csv'),
          row.names = FALSE)
write.dta(ehb_prev_wide, file = paste0(out_dir, 'PATHNAME/other_ehb_all_draws.dta'))
...
write.csv(kern_prev_wide, file = paste0(out_dir, 'PATHNAME/other_kernicterus_all_draws.csv'),
          row.names = FALSE)
write.dta(kern_prev_wide, file = paste0(out_dir, 'PATHNAME/other_kernicterus_all_draws.dta'))
...
write.csv(ehb_prev_wide, file = paste0(out_dir, 'PATHNAME/other_ehb_summary_stats.csv'),
          row.names = FALSE)
write.dta(ehb_prev_wide, file = paste0(out_dir, 'PATHNAME/other_ehb_summary_stats.dta'))



# scatter current ehb and kernicterus proportions against previous run
prev_ehb <- fread('PATHNAME/other_ehb_summary_stats.csv')
prev_kern <- fread('PATHNAME/other_kernicterus_summary_stats.csv')
...
pdf(file = paste0(out_dir, "PATHNAME/scatter_7_29_kern_model_vs_gbd19.pdf"),
    width = 12, height = 8)
gg1 <- ggplot() +
