...
if (my_loc %in% restrict_loc){
  ...
  data <- gen_zero_draws(model_id= meid, location_id = NA, measure_id = 5, gbd_round_id = gbd_round_id, team = 'ADDRESS')
  data[, metric_id := 3]
  data[, model_id:= meid]
...
if (!(my_loc %in% restrict_loc)){
  ...
  worm_draws <- get_draws(gbd_id_type = "model_id", gbd_id = all_cases, source = "ADDRESS",
                          location_id = my_loc, decomp_step = decomp_step,  status = "best", 
