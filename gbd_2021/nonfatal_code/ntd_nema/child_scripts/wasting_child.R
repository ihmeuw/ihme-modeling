...
if (restrict == 1){
  ...
  draws <- gen_zero_draws(model_id= NA, location_id = NA, measure_id = 5, gbd_round_id = gbd_round_id, team = ADDRESS)
...
if (restrict == 0){
  ...
  # add ages not 2,3,388,389,238,34 as 0 
  draws <- gen_zero_draws(model_id= NA, location_id = NA, measure_id = 5, gbd_round_id = gbd_round_id, team = 'FILEPATH')
  draws[, location_id := loc_id]  
  draws[, model_id:= wasting]
