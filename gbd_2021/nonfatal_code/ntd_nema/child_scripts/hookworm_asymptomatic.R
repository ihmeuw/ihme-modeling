# Purpose: child script for calculation of hookworm asymptomatic
#
...
draws_dir <- paste0(draws_dir, 'ADDRESS/FILEPATH')
...
if (my_loc %in% restrict_loc){
  ...
  worm_draws <- get_draws(gbd_id_type = "model_id", gbd_id = ADDRESS, source = ADDRESS,
                          location_id = 101, decomp_step = ADDRESS,  status = 'best', gbd_round_id = gbd_round_id)
  # ^^ do not edit -- need static generic zero draw file
  
  worm_draws[, c("model_id", "metric_id", "version_id") := NULL]
...
if (!(my_loc %in% restrict_loc)){
  ...
  parent_draws <- get_draws(gbd_id_type = "model_id", gbd_id = ADDRESS, source = ADDRESS,
                            location_id = my_loc, status = "best", decomp_step = ADDRESS, year_id = c(1990, 1995, 2000, 2005, 2010, 2015, 2017, 2019), gbd_round_id = gbd_round_id)

  cat("\tGetting severe anemia draws\n")
  severe_draws <- get_draws(gbd_id_type = "model_id", gbd_id = ADDRESS, source = ADDRESS,
                            year_id = c(1990, 1995, 2000, 2005, 2010, 2015, 2017, 2019),
                            location_id = my_loc, status = "best",
                            decomp_step = ADDRESS, gbd_round_id = gbd_round_id) 

  cat("\tGetting moderate anemia draws\n")
  mod_draws    <- get_draws(gbd_id_type = "model_id", gbd_id = ADDRESS, source = ADDRESS,
                            location_id = my_loc, status = "best", decomp_step = ADDRESS, year_id = c(1990, 1995, 2000, 2005,2010, 2015, 2017, 2019), gbd_round_id = gbd_round_id)

  cat("\tGetting mild anemia draws\n")
  mild_draws   <- get_draws(gbd_id_type = "model_id", gbd_id = ADDRESS, source = ADDRESS,
                            location_id = my_loc, status = "best", decomp_step = ADDRESS, year_id = c(1990, 1995, 2000, 2005, 2010, 2015, 2017, 2019), gbd_round_id = gbd_round_id)
  ...
  if (length(anynegs) != 0){
    ...
    write.csv(df, paste0(draws_dir, my_loc, "FILEPATH"), row.names = F)
