...
##
## Adapted from "stroke_dementia_splits_processing.R", which contained code for chronic severity splits for stroke (with only stroke + dementia, no HF) sequelae 
...
output_folder <- as.character(args[4])
input_folder <- as.character(args[5])
...
# pull in draws from chronic ischemic stroke model
...
# pull in draws from chronic ich stroke model
chronic_ich <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = 10836, source="epi",
                         gbd_round_id = gbd_round_id, decomp_step = decomp_step, status="best",
                         measure_id = c(5, 6), location_id = location, age_group_id = unique(ages$age_group_id), sex_id = c(1, 2))

# pull in draws from chronic sah stroke model
...
dementia_split = get_severity_split(source_meid=24352, gbd_round_id=gbd_round_id, decomp_step=decomp_step)
...
# reshape data
dementia_split = melt(dementia_split, measure.vars = names(dementia_split)[grepl("draw", names(dementia_split))], variable.name = "draw", value.name = "dem_split")

##################################################################

# pull HF severity splits (asymptomatic, mild, moderate, severe HF) (using HF due to COPD, which has the same splits, since these are no longer centrally available for stroke)
HF_split = get_severity_split(source_meid=9569, gbd_round_id=gbd_round_id, decomp_step=decomp_step)
...
# reshape data
HF_split = melt(HF_split, measure.vars = names(HF_split)[grepl("draw", names(HF_split))], variable.name = "draw", value.name = "HF_split")
...
for(parent in c(10837, 10836, 18733)){
  ...
  # pull in stroke severity splits -- use GBD 2019 as they were not moved over for GBD 2020
  ...
  # merge stroke severity split data with age metadata to get age_group_id -- young age groups do not match as stroke severity splits still using GBD 2019 data, need to reassign
  ...
  # reshape data
  stroke_split = melt(stroke_split, measure.vars = names(stroke_split)[grepl("draw", names(stroke_split))], variable.name = "draw", value.name = "split_prop")
  ...
  stroke_split_rescaled = dcast(stroke_split, age_group_id + sex_id + draw ~ target_meid, value.var = "split_prop")
  ...
  stroke_split_rescaled = melt(stroke_split_rescaled, measure.vars = c(sev_2, sev_3, sev_4, sev_5), variable.name = "target_meid", value.name = "split_prop_rescaled")
  
  ##################################################################
  
  # merge hf due to stroke model data with stroke severity splits (allow.cartesian=T used as merge results in more than nrow(x)+nrow(i) rows -- hf_due_stroke has multiple years, stroke_split has multiple severities)
  hf_due_stroke_split = merge(hf_due_stroke, stroke_split_rescaled, by=c("age_group_id", "sex_id", "draw"), allow.cartesian = TRUE)
  
  # multiply hf due to stroke data by the severity split value
  hf_due_stroke_split[, hf_split_draw := hf_due_stroke_draw * split_prop_rescaled]
  
  hf_due_stroke_split = hf_due_stroke_split[, c("age_group_id", "sex_id", "draw", 
                                                "location_id", "measure_id", "metric_id", 
                                                "year_id", "target_meid", "hf_split_draw")]
  
  hf_due_stroke_split[, target_meid := as.numeric(as.character(target_meid))]
  
  ##################################################################
  
  # merge chronic stroke model data with stroke severity splits (allow.cartesian=T used as merge results in more than nrow(x)+nrow(i) rows -- chronic_<subtype> has multiple years, stroke_split has multiple severities)
  chronic_subtype_split = merge(get(paste0("chronic_", abbr)), stroke_split, by=c("age_group_id", "sex_id", "draw"), allow.cartesian = TRUE)
  
  # multiply chronic stroke data by the severity split value
  chronic_subtype_split[, split_draw := get(paste0(abbr, "_draw")) * split_prop]
  ...
  chronic_subtype_split[is.na(hf_split_draw), hf_split_draw := 0]
  
  # calculate chronic stroke without heart failure data by subtracting hf due to stroke from stroke
  chronic_subtype_split[, stroke_no_hf := split_draw - hf_split_draw]
  ...
  for (severity in c(paste0("Asymptomatic ", tolower(full)), paste0(full, " severity level ", c(1,2,4)))){
    ...
    if (severity %in% c(paste0("Asymptomatic ", tolower(full)), paste0(full, " severity level 1"))){
      ...
      severity_subset <- dcast(severity_subset, location_id + age_group_id + sex_id + year_id + measure_id + metric_id + target_meid + target_meid_name ~ draw, value.var = "split_draw")
      ...
      severity_subset[stroke_no_hf<0, stroke_no_hf:=0]
      
      # save stroke with no HF
      stroke_without_HF <- dcast(severity_subset, location_id + age_group_id + sex_id + year_id + measure_id + metric_id ~ draw, value.var = "stroke_no_hf")
      ...
      severity_subset = severity_subset[, c("age_group_id", "sex_id", "draw", "measure_id", "location_id", "year_id", "metric_id", "hf_split_draw")]
      severity_subset = merge(severity_subset, HF_split, by=c("measure_id", "draw"), allow.cartesian = TRUE)
      
      # multiply chronic stroke with hf data by severity split
      severity_subset[, hf_sev_split_draw := hf_split_draw * HF_split]
      ...
      for (meid in unique(severity_subset$target_meid)){
        ...
        final = dcast(final, location_id + age_group_id + sex_id + year_id + measure_id + metric_id ~ draw, value.var = "hf_sev_split_draw")
  ...
  severities_3_and_5[, perc_hf := ifelse(split_draw==0, 0, hf_split_draw/split_draw)]
  severities_3_and_5[, perc_no_hf := ifelse(split_draw==0, 0, stroke_no_hf/split_draw)]
  
  # reshape stroke data
  severities_3_and_5 = dcast(severities_3_and_5, age_group_id + sex_id + measure_id + metric_id + location_id + year_id + draw ~ target_meid_name, value.var=c("split_draw", "perc_hf", "perc_no_hf"))
  ...
  severities_3_and_5[, severe_w_cogn_imp := ifelse(age_group_id%in%dementia_due_to_stroke$age_group_id, get(paste0("split_draw_", full, " severity level 5"))*.1, get(paste0("split_draw_", full, " severity level 5")))]
  ...
  severities_3_and_5[, severe_w_dem := ifelse(age_group_id%in%dementia_due_to_stroke$age_group_id, get(paste0("split_draw_", full, " severity level 5"))*.9, 0)]
  ...
  severities_3_and_5[ , moderate_w_cogn_imp := get(paste0("split_draw_", full, " severity level 3")) - moderate_w_dem]
  ...
  for(severity in c(paste0(full, " severity level ", c(3,5)))){
    ...
    # save for without HF
    severities_3_and_5[, paste0(column, "_no_hf") := get(column)*get(paste0("perc_no_hf_", severity))]
    ...
    # save for with HF
    severities_3_and_5[, paste0(column, "_with_hf") := get(column)*get(paste0("perc_hf_", severity))]
    ...
    # multiply chronic stroke with hf data by severity split
    severities_3_and_5_with_hf[, hf_sev_split_draw := get(paste0(column, "_with_hf")) * HF_split]
    ...
    for (meid in unique(severities_3_and_5_with_hf$target_meid)){
      ...
      # account for negative values
      final[hf_sev_split_draw<0, hf_sev_split_draw:=0]
  ...
  for(severity in c("Moderate", "Severe")){
    ...
    cols_to_keep = c("age_group_id", "sex_id", "location_id", "year_id", "measure_id", "metric_id", "draw", paste0(tolower(severity), "_w_dem"), paste0("perc_hf_", full, " severity level ", sev_level), paste0("perc_no_hf_", full, " severity level ", sev_level))
    ...
    setnames(severity_subset_young_ages, paste0(tolower(severity), "_w_dem"), "final_draw")
    
    # merge split stroke data with dementia severity splits -- youngest age groups will be dropped (allow.cartesian=T used as merge results in more than nrow(x)+nrow(i) rows -- stroke data has multiple years, dementia split data has multiple severities)
    severity_subset = merge(severity_subset, dementia_split, by=c("age_group_id", "sex_id", "measure_id", "draw"), allow.cartesian = TRUE)
    
    severity_subset[, final_draw := get(paste0(tolower(severity), "_w_dem")) * dem_split]
    severity_subset[, paste0(tolower(severity), "_w_dem") := NULL]
    severity_subset[, dem_split := NULL]
    ...
    for(dem_sev in c(1944, 1945, 1946)){
      ...
      # save for without HF
      severity_subset_dementia[, no_hf_draw := final_draw*get(paste0("perc_no_hf_", full, " severity level ", sev_level))]
      
      # account for negative values
      severity_subset_dementia[no_hf_draw<0, no_hf_draw:=0]
      ...
      # fix for mispelling of subarachnoid in ME ID name
      if (length(meid)==0){
      ...
      # save for with HF
      severity_subset_dementia[, hf_draw := final_draw*get(paste0("perc_hf_", full, " severity level ", sev_level))]
      
      severity_subset_dementia_with_hf = severity_subset_dementia[, c("age_group_id", "sex_id", "year_id", "location_id", "measure_id", "metric_id", "draw", "hf_draw")]
      ...
      # multiply chronic stroke with hf data by severity split
      severity_subset_dementia_with_hf[, hf_sev_split_draw := hf_draw * HF_split]
