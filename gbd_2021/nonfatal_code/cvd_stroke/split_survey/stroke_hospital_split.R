##############################
# Stroke code for spliting clinical data I64 and I62 into subtypes 
# Author: USERNAME
# Date  : 9/9/2020
##############################
...
## Get age & location information for graphing later
locations <- data.frame(get_location_metadata(location_set_id=35))
age_meta <- get_age_metadata(age_group_set_id=19)

## Drop Ecuador, Georgia, Mexico, Kenya, Turkey, India, Philippines, Indonesia, Chile, low SDI areas of China, Brazil, UTLAs
...
## Get clinical informatics correction factors
cf.map <- data.table(read.csv("FILEPATH/stroke_cfs_refresh3.csv")) 
setnames(cf.map, c('cf_median','upper','lower'),c('Y_mean','Y_mean_lo','Y_mean_hi'))
cf.map <- cf.map[cf_type=='cf2']
...
## Get I62
new_I62 <- save_bundle_version(bundle_id=116 ,decomp_step= 'iterative',gbd_round_id=7,include_clinical = c('claims, inpatient only','inpatient')) 

i62 <- get_bundle_version(bundle_version_id = new_I62$bundle_version_id, export=FALSE, fetch='all') #all the new clinical data. 
...
## Get i64
new_i64 <- save_bundle_version(bundle_id=371,decomp_step= 'iterative',gbd_round_id=7,include_clinical = c('claims, inpatient only','inpatient'))
...
####### STEP 2 #######
# Grab SUBTYPE SPECIFIC data from bundle 454 (ischemic), 455 (intracerebral), and 2996 (subarachnoid)
######################

## Ischemic stroke

new_fe.is <- save_bundle_version(bundle_id=454, decomp_step='iterative',gbd_round_id=7, include_clinical=TRUE) 

fe.is <- get_bundle_version(bundle_version_id= new_fe.is , export=FALSE, fetch='all')     
...
write.xlsx(fe.is[age_start >= 40], file='FILEPATH/unsplit_isch_hospital.xlsx')

## Intracerebral hemorrhage
new_fe.ich <- save_bundle_version(bundle_id=455, decomp_step='iterative',gbd_round_id=7, include_clinical=TRUE) 
...
fe.cerhem <- subset(fe.cerhem, location_id %ni% unlist(todrop$location_id, use.names=F)) 
fe.cerhem$parent_id <- NULL
setnames(fe.cerhem, "mean", "mean.cerhem")
setnames(fe.cerhem, "standard_error", "se.cerhem")
write.xlsx(fe.cerhem[age_start >= 40], file='FILEPATH/unsplit_ich.xlsx')

## Subarachnoid hemorrhage
new_fe.subhem <- save_bundle_version(bundle_id=2996, decomp_step='iterative',gbd_round_id=7, include_clinical=TRUE) 
...
write.xlsx(fe.subhem[age_start >= 40], file='FILEPATH/unsplit_sah.xlsx')

####### STEP 3 #######
## Merge all together and calculate proportion splits
######################
...
#if all other strokes are 0, then assign proportion to 0, else leave alone
...
## IS
fe.is.upload <- merge(fe.is, merged[,c("location_id", "year_start", "year_end", "age_start", "age_end", "sex", "mean.is.new", "se.is.new")],  
...
filename.is <- paste0(jpath, "FILEPATH/7112_hospital_split_refresh_", date, ".xlsx")
...
filename.ich <- paste0(jpath, "FILEPATH/7088_hospital_split_refresh_", date, ".xlsx")
...
filename.subhem <- paste0(jpath, "FILEPATH/7100_hospital_split_refresh_", date, ".xlsx")
write.xlsx(fe.subhem.upload, filename.subhem, sheetName="extraction", na="")
