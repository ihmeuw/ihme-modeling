...
sourceDirectory('FILEPATH/r/') ## source central functions. 
'%ni%' <- Negate('%in%')

# testing variables
#prop_date <- '2020_08_30'
#stage <- 'stage2'
#subtype <- 'ich'

# Get all stroke prevalence

split_all_stroke_prev <- function(prop_date, stage, subtype){
  
# Get age-meta data: 
age_map <- get_age_metadata(age_group_set_id = 19, gbd_round_id =7)
...
## Get all stroke prevalence surveys
all_stroke <- get_bundle_data(bundle_id = 456, decomp_step='iterative', gbd_round_id = 7)
...
data_to_split <- data_to_split[nid!= 125159] #remove source

## Get most recent files that contain proportion split - created in stroke_survivors_with_csmr.R
...
if(subtype=='is'){
with_props[,standard_error_new := sqrt(standard_error_isch^2 * standard_error^2 + standard_error_isch^2*mean^2 + standard_error^2*mean_isch^2 )]

}
if(subtype=='ich'){
with_props[,standard_error_new := sqrt(standard_error_ich^2 * standard_error^2 + standard_error_ich^2*mean^2 + standard_error^2*mean_ich^2 )]

}
if(subtype=='sah'){
with_props[,standard_error_new := sqrt(standard_error_sah^2 * standard_error^2 + standard_error_sah^2*mean^2 + standard_error^2*mean_sah^2 )]

}

with_props[,`:=` (mean=mean_new,cases=cases_new,standard_error=standard_error_new)] ## set mean and cases to be recalculated values. 
