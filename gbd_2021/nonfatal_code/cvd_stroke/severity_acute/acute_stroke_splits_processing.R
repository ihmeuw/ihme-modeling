...
# pull HF severity splits (asymptomatic, mild, moderate, severe HF) (using HF due to COPD, which has the same severity splits, since these are no longer centrally available for stroke)
...
for(parent in c(24714, 24706, 24710)){
  ...
  # pull in draws from acute stroke model
  ...
  # merge acute stroke model data with stroke severity splits (allow.cartesian=T used as merge results in more than nrow(x)+nrow(i) rows -- acute_stroke has multiple years, stroke_split has multiple severities)
  acute_stroke_split = merge(acute_stroke, stroke_split, by=c("age_group_id", "sex_id", "draw", "measure_id"), allow.cartesian = TRUE)
  
  # multiply acute stroke data by the severity split value
  acute_stroke_split[, stroke_split_draw := acute_stroke_draw * split_prop]
  ...
  stroke_split = dcast(stroke_split, measure_id + age_group_id + sex_id + draw ~ target_meid, value.var = "split_prop")
  ...
  stroke_split = melt(stroke_split, measure.vars = c(sev_2, sev_3, sev_4, sev_5), variable.name = "target_meid", value.name = "split_prop")
  ...
  hf_due_stroke_split[, hf_split_draw := hf_due_stroke_draw * split_prop]
  ...
  hf_and_stroke[is.na(hf_split_draw), hf_split_draw := 0]
  
  # calculate acute stroke without heart failure data by subtracting hf due to stroke from stroke
  hf_and_stroke[, stroke_no_hf := stroke_split_draw - hf_split_draw]
  ...
  for (sev in c(2:5)){
    ...
    acute_stroke_no_hf = dcast(subset, location_id + age_group_id + sex_id + year_id + measure_id + metric_id ~ draw, value.var = "stroke_no_hf")
    ...
    subset = subset[, c("age_group_id", "sex_id", "draw", "measure_id", "location_id", "year_id", "metric_id", "hf_split_draw")]
    subset = merge(subset, HF_split, by=c("measure_id", "draw"), allow.cartesian = TRUE)
    
    # multiply acute stroke with hf data by severity split
    subset[, hf_sev_split_draw := hf_split_draw * HF_split]
