#################################################################################################
#' Name: APPLY CLINICAL & NETWORK RATIOS TO PCOS
################################################################################################
...
nrow(dat_original[reference %in% c(NA)])
nrow(dat_original[cv_literature == 1])
...
cov_names <- c("X_cv_clinical", "X_cv_self_report", "X_cv_Rotterdam", "X_cv_aes")
...
nrow(network_dt[cv_literature == 1])
...
final_adjusted[ ,c("mean_logit","se_logit","beta0", "beta0_se_tau", "var_logit_tmp",
                   ...
                   "mean_adjusted", "se_adjusted", "lo_adjusted", "hi_adjusted") := NULL]
#ADD A USEFUL NOTE
final_adjusted[ ,note_modeler := paste0(note_modeler," | crosswalked all clinical to NIH reference")]

dim(final_adjusted)
names(final_adjusted)
unique(final_adjusted$seq)

#INFLATE SE ONLY OF MEAN = 0, CLAIMS DATA (bc it has never been adjusted)
inflate_dt <- copy(noadjust_dt)
inflate_coefs <- copy(coefs_network)
...
final_adjusted[ ,c("X_cv_self_report", "X_cv_Rotterdam", "X_cv_aes") := NULL]
...
#FIX CIs
nrow(adjusted_network[upper > 1 ])
nrow(adjusted_network[age_start >= 40 & mean >= 0.3])

adjusted_network[mean > 0.8, is_outlier := 1]
adjusted_network[age_start >= 40 & mean >= 0.3, is_outlier := 1]

#EPI UPLOADER VALIDATION FIXES
adjusted_network[is.na(unit_value_as_published), unit_value_as_published := 1]
unique(adjusted_network$unit_value_as_published)

adjusted_network[is.na(upper) & is.na(lower), uncertainty_type_value := NA]

nrow(adjusted_network[standard_error > 1 & is.na(lower) & is.na(upper)])
adjusted_network[standard_error > 1, standard_error := NA]
