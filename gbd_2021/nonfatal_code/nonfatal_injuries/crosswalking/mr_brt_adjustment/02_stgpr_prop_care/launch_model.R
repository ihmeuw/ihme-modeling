# team: GBD Injuries
# script: launch ST-GPR model to get proportion of people who received care estimates
...
countries <-
  countries[, c('location_id', 'location_name', 'ihme_loc_id')]

data <-
  fread('FILEPATH/st_gpr_proportions_age_specific_year_agg.csv')
...
write.csv(
  data,
  'FILEPATH/st_gpr_proportions_age_specific_year_agg_culled.csv'
)
...
path_to_config <-
  'FILEPATH/stgpr_model_settings.csv'
my_model_id <- 14
