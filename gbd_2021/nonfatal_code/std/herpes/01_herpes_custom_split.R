...
prev_draws <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = parent_meid, source = "epi",
                        measure_id = 5, location_id = loc_id, status = "best", gbd_round_id = 7, decomp_step = "iterative")
inc_draws <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = parent_meid, source = "epi",
                       measure_id = 6, location_id = loc_id, status = "best", gbd_round_id = 7, decomp_step = "iterative")
...
#ME 3090 (INITIAL SYMP GH - CONTINUE USING GBD SCALAR METHOD
#prep props
