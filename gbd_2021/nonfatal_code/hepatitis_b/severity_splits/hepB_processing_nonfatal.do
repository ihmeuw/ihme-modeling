...
\******************************************************************************/
  ...
  run FILEPATH
    ...
    * PULL IN DRAWS FROM DISMOD MODELS FOR HBsAG PREVALENVCE AND INCIDENCE *
      get_draws, gbd_id_type("modelable_entity_id") gbd_id(MEID) source("epi") location_id(`location') age_group_id(`ages') measure_id(5) status("best") gbd_round_id(7) decomp_step("iterative") clear
      rename draw_* prev_*
      save `mergeTemp'
    
    * Incidence, stick prev and incidence together
      get_draws, gbd_id_type("modelable_entity_id") gbd_id(MEID) source("epi") location_id(`location') measure_id(6) status("best") gbd_round_id(7) decomp_step("iterative") clear
  ...
  * PERFORM DRAW-LEVEL CALCULATIONS *
    forvalues i = 0/999 {
      quietly {
        ...
        *prCarrierAlpha and prCarrierBeta are created in file
