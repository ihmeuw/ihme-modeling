...
get_definitions <- function(ref_dt){
  ...
  cvs <- cvs[!cvs %in% cv_drop] ##DROP UNNEEDED COVARIATES USING THE CV_DROP OBJECT
  dt[, definition := ""]
  for (cv in cvs){
...
## MAPPING SUBNATIONALS TO NATIONALS
...
get_matches <- function(n, pair_dt, year_span = 10, age_span = 5){
  ...
  ## EXACT LOCATIONS FOR MARKETSCAN
...
park_dt[cv_nursing_home_excl == 0.5, cv_nursing_home_excl := 1] 
