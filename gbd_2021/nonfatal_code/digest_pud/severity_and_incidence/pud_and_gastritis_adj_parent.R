...
for(cause in causes) { 
  ...
  for(location in location_ids) {
    ...
    # GBD 2020
    command <- paste0("qsub  -q all.q  -P proj_rgud -l m_mem_free=4G  -l fthread=5 -l h_rt=24:00:00 -o ",logs," -e ",logs, " -N ", paste0(cause,"_", location, "_adj_child "), shell, " ", "FILEPATH/pud_and_gastritis_adj_child.R ", cause, " ", location, " ", adj_child_files)
    print(command)
    system(command)
