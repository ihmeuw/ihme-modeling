...
calculate_oos_error <- function(preds.dir, input.data.dir, output.dir, folds){
  ...
  input_data <- fread(file.path("FILEPATH/case_regression_input.csv"))
  
  ## read in in sample predictions
  in_sample <- fread("/FILEPATH/01_case_predictions_from_model.csv")
  
  out_of_sample <- lapply(1:n_folds, function(i) {
    fread(file.path("/FILEPATH/01_case_predictions_from_model_", i, ".csv"))
  }) %>% rbindlist
  
  # Note that there is one out of sample prediction per row of original input data!
  nrow(input_data) == nrow(out_of_sample)
  
  #drop draw cols to drop because only want to look and IS and OOS point prediction generated using the REs rather than the IS draws generated using the std RE
  draw_cols <- subset(colnames(in_sample), colnames(in_sample) %like% "_draw_")
  in_sample_summ <- in_sample[, !draw_cols, with = FALSE]
  
  # merge the OS predictions on to the input data for comparison
