...
"/FILEPATH/collapse_point.R" %>% source
file.path("/FILEPATH/age_sex_split.R") %>% source
#***********************************************************************************************************************
...
#----PREP---------------------------------------------------------------------------------------------------------------
### get location data
locations <- get_location_metadata(gbd_round_id=gbd_round, location_set_id=22, decomp_step=decomp_step)[,
...
if (gbd_round == 7) {
  ...
  population <- get_population(location_id=pop_locs, year_id=(1980-a):year_end, age_group_id=c(2:20, 6:20, 30:32, 235), status="best",
...
if(is.null(custom_coverage)){
  if (use_lagged_covs) {
    covar <- get_covariate_estimates(covariate_id=2308, year_id=1980:year_end, location_id=pop_locs, gbd_round_id=gbd_round, decomp_step=decomp_step, gbd_round_id= gbd_round)[, .(location_id, year_id, mean_value, model_version_id)]
  } else {
    if (decomp) {
      covar <- get_covariate_estimates(covariate_id=32, year_id=1980:year_end, location_id=pop_locs, gbd_round_id=gbd_round, decomp_step=decomp_step, gbd_round_id = gbd_round)[, .(location_id, year_id, mean_value, model_version_id)]
    } else {
  ...
  if (use_lagged_covs) {
    covar <- get_covariate_estimates(covariate_id=2308, model_version_id = custom_coverage, year_id=1980:year_end, location_id=pop_locs, gbd_round_id=gbd_round, decomp_step=decomp_step)[, .(location_id, year_id, mean_value, model_version_id)]
  } else {
    if (decomp) {
      covar <- get_covariate_estimates(covariate_id=32, model_version_id = custom_coverage, year_id=1980:year_end, location_id=pop_locs, gbd_round_id=gbd_round, decomp_step=decomp_step)[, .(location_id, year_id, mean_value, model_version_id)]
    } else {
      covar <- get_covariate_estimates(covariate_id=32, model_version_id = custom_coverage, year_id=1980:year_end, location_id=pop_locs)[, .(location_id, year_id, mean_value, model_version_id)]
    }
  }
...
prep_case_notifs <- function(cause="Pertussis") {
  ...
  case_notif <- read_excel(file.path("FILEPATH/incidence_series_2020.xls"), sheet=cause) %>% data.table
...
prep_regression <- function(...) {
  ...
  ### prep pertussis historical data, converted to .csv from
  ...
  pne <- fread(file.path("FILEPATH/Pertussis_Notification_Extraction.csv"))[iso3=="XEW"]
  names(pne) <- tolower(names(pne))
  setnames(pne, c("iso3", "year"), c("ihme_loc_id", "year_id"))
  pne[, notifications := as.integer(notifications)]
  # GBR 1940+
  pne1940 <- read.dta(file.path("FILEPATH/DTP3_pre_1940.dta")) %>% as.data.table
  ...
  regr[is.infinite(ln_inc), ln_inc := NA] # replace Inf with NA
...
if(run_for_rrs){
  ...
  if(custom_version == "09.17.20"){
    # on this run date wanted to calculate RR's using GBD2019 best covariate versions, so read in data and covars as prepped for GBD2019 best model
    regr <- fread("/FILEPATH/09.05.19/model_inputs/incidence_regression_input.csv")
  }
...
if (CHE_RE=="gbd2017") {
  reffect_CHE <- 4.639529  # GBD 2019 Step 4: 4.971355;  GBD 2017: 4.639529
} else {
  reffect_CHE <- reffect["CHE", ]
}
cat(paste0("CHE random effect used (NF) =", reffect_CHE), file=file.path(j.version.dir.logs, "log_incidence_mereg.txt"), sep="\n", append=TRUE)
...
if(covid_adjustment){
  # change me_id so that are saving NF to covid adjusted me
  me_id <- 26945

  # replace 2020 draws with covid inclusive 2020 draws
  covid_inc_draws <- fread(adjusted_inc_draws_path)[, cause_id := NULL][, year_id := 2020]
  # switch from draws of inc rate to counts and rename cols
  covid_inc_draws <- merge(covid_inc_draws, population, by = c("location_id", "year_id", "sex_id", "age_group_id"))
  covid_inc_draws[, (paste0("inc_draw", 0:999)) := lapply(.SD, function(x) x * population)]
  setnames(covid_inc_draws, grep("draw", names(covid_case_draws), value = T), paste0("split_case_draw", 0:999))
  # drop and replace all 2020 est
  # covid_inc_draws has only most detailed, split cases has most detailed and national because we raked
  adjusted_split_cases <- split_cases[!(year_id == unique(covid_inc_draws$year_id))]
  adjusted_split_cases <- rbind(split_cases, covid_inc_draws)
  split_cases <- copy(adjusted_split_cases)
...
save_nonfatal <- save_nonfatal[age_group_id %in% c(age_start:age_end, 388, 389, 238, 34), ]
...
# save whooping incidence draws, covidfree_modelable_entity_id=1424
# FAIR:
job_nf <- paste0("qsub -N s_epi_", acause, " -l m_mem_free=150G -l fthread=5 -l archive -l h_rt=8:00:00 -P ", cluster_proj, " -q all.q -o /FILEPATH/",
              username, " -e /share/temp/sgeoutput/", username,
...
if(covid_adjustment) {
  ...
  message("qsubbed save_results_epi with covid adjusted cases. Stopping 01_natural_history_model script -- CoD team handles fatal COVID adjustment from the scalars produced separately.")
  stop()
...
prep_covariates <- function(...) {
  ...
  if (decomp) {
    ldi <- get_covariate_estimates(covariate_id=57, year_id=1980:year_end, location_id=pop_locs, gbd_round_id=gbd_round, decomp_step=fatal_decomp_step)
  } else {
  ...
  # covariate: HSA, covariate_id=208, covariate_name_short="health_system_access_capped" --> actually Maternal Care & Immunization covariate (MCI)
  if (decomp) hsa <- get_covariate_estimates(covariate_id=208, year_id=1980:year_end, location_id=pop_locs, decomp_step=fatal_decomp_step, gbd_round_id = gbd_round) else hsa <- get_covariate_estimates(covariate_id=208, year_id=1980:year_end, location_id=pop_locs) # NG: removed model_version_id because "No model versions match model_version_id=18665" 2.11.19
  ...
  if (decomp) mal_cov <- get_covariate_estimates(covariate_id=1230, year_id=1980:year_end, location_id=pop_locs, decomp_step=fatal_decomp_step, gbd_round_id = gbd_round) else mal_cov <- get_covariate_estimates(covariate_id=1230, year_id=1980:year_end, location_id=pop_locs)
  ...
  if (decomp) haqi <- get_covariate_estimates(covariate_id=1099, year_id=1980:year_end, location_id=pop_locs, decomp_step=fatal_decomp_step, gbd_round_id = gbd_round) else haqi <- get_covariate_estimates(covariate_id=1099, year_id=1980:year_end, location_id=pop_locs) # NG: removed model_version_id because "No model versions match model_version_id=19760" 2.11.19
...
prep_cfr_data <- function(...) {
  ...
  cfr_expert <- fread(file.path(home, "data/cfr_data_gbd2013.csv"))
  ...
  cfr_2016 <- fread(file.path(home, "data/sys_review_annatorre/cfr_GBD2016_extraction.csv"))
...
if (HAQI_or_HSA == "use_HSA") {
  ...
  saveRDS(nb_model, file = file.path(j.version.dir.logs, "log_cfr_negbin.RDS"))
...
### save_results
job <- paste0("qsub -N s_cod_", acause, " -l m_mem_free=100G -l fthread=5 -l archive -l h_rt=8:00:00 -P proj_cov_vpd -q all.q -o /share/temp/sgeoutput/",
              username, " -e FILEPATH", username,
              " FILEPATH -i FILEPATH -s /FILEPATH/save_results_wrapper.r",
              " --type cod",
              " --me_id ", cause_id,
