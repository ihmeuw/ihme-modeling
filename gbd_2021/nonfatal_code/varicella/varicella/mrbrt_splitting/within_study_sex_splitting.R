## FOR STUDIES WITH AGE AND SEX SPLIT DATA SAVED SEPARATELY, APPLY THE SEX RATIO FROM WITHIN THE STUDY TO SPLIT THE AGE SPECIFIC ROWS
...
age_sex_split <- function(raw_dt){
  ...
  # define which rows are the 'age' rows and which are the 'sex' rows... (drops any differently specific rows ie year, ICU/nonICU)
  df_split <- copy(within_study)
  df_split <- df_split[specificity %in% c("age", "sex") & split == 0,]

  ## CALC CASES MALE/FEMALE AND CASES THAT ARE AGE SPLIT
  df_split[, cases_total:= sum(cases), by = c("nid", "group", "specificity", "measure", "location_id", cvs)]
  df_split[, prop_cases := cases / cases_total]

  ## CALC PROP MALE/FEMALE AND PROP IN EACH AGE GROUP
  ...
  total <- rbind(original, age.sex, fill = T) ## DON'T RETURN PARENTS, but will return orig rows of data from that nid that has specificity other than sex, age, or total
