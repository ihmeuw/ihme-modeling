...
# get envelope
if (decomp) {
  envelope <- get_envelope(location_id=pop_locs, sex_id=1:2, age_group_id=ages, year_id=1980:year_end, with_hiv=0, gbd_round_id=gbd_round, decomp_step=fatal_decomp_step)
} else {
...
if (decomp) {
  if (pop_run_id==145) {
    ...
    population <- get_population(location_id=pop_locs, sex_id=1:2, age_group_id=ages, year_id=1980:year_end, gbd_round_id=gbd_round, decomp_step=fatal_decomp_step)
  }
} else {
...
if (decomp) {
  cod <- get_cod_data(cause_id=cause_id, gbd_round_id=gbd_round, decomp_step=fatal_decomp_step)[, .(location_id, year, age_group_id, sex, sample_size, cf, nid, description)] %>% setnames(., "sex", "sex_id") %>% setnames(., "year", "year_id")
} else {
...
if (HAQI_or_HSA == "HSA") {
  ...
  if (decomp) covariates <- get_covariate_estimates(covariate_id=208, year_id=1980:year_end, location_id=pop_locs, status="best", gbd_round_id=gbd_round, decomp_step=fatal_decomp_step)[, .(location_id, year_id, mean_value, model_version_id)]
  ...
  if (decomp) covariates <- get_covariate_estimates(covariate_id=1099, year_id=1980:year_end, location_id=pop_locs, status="best", gbd_round_id=gbd_round, decomp_step=fatal_decomp_step)[, .(location_id, year_id, mean_value, model_version_id)]
...
theta = 1 / 4.42503
GLM <- glm.nb(deaths ~ health + age_group_id + offset(log(population)), init.theta=theta, data=regress) #log link is default in this function

# save log
capture.output(summary(GLM), file = file.path(j.version.dir.logs, "log_deaths_nbreg_CoD.txt"), type="output")
saveRDS(GLM, file = file.path(j.version.dir.logs, "log_deaths_nbreg_CoD.rds"))
#***********************************************************************************************************************
...
if(gbd_round < 7){
  ...
  coefmat <- matrix(unlist(coefmat), ncol=26, byrow=TRUE, dimnames=list(c("coef"), c(as.vector(names(coefmat)))))
...
### save_results
job <- paste0("qsub -N s_cod_", acause, " -l m_mem_free=100G -l fthread=4 -l archive -l h_rt=8:00:00 -P ", cluster_proj, " -q all.q -o /FILEPATH/",
...
if (decomp) {
  if (gbd_round==6 & decomp_step=="step2") {
    ...
    var_nf   <- data.table(python_draws(gbd_id_type="modelable_entity_id", gbd_id=1439, status="best", gbd_round_id=gbd_round, source="epi", decomp_step=decomp_step))
  } else if (gbd_round==6 &decomp_step=="step4") {
  ...
  } else if (dismod_status == "latest") {
    var_nf   <- get_draws(gbd_id_type="modelable_entity_id", gbd_id=1439, status="latest", gbd_round_id=gbd_round, source="epi", measure_id=c(5, 6), decomp_step=decomp_step)
  }
} else {
...
### upload results
job <- paste0("qsub -N s_epi_", acause, " -l m_mem_free=125G -l fthread=10 -l archive -l h_rt=12:00:00 -P ", cluster_proj, " -q all.q -o /FILEPATH/",
              username, " -e /FILEPATH/", username,
              " /FILEPATH -i /FILEPATH -s /FILEPATH",
              " --type epi",
              " --me_id ", me_id,
