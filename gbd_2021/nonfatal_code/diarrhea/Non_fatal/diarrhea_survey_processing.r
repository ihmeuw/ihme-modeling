...
keep.names <- c("ihme_loc_id","year_start","year_end","psu","nid","survey_name","strata","pweight","sex_id","age_year","age_month","int_month", # This is missing from the extractions: "urban",
...
write.csv(d.input, "/FILEPATH/", row.names=F)
...
write.csv(pred.df, paste0("/FILEPATH/"))
...
########################################################
## Part 4. Apply month/country scalar to raw data
########################################################
	diarrhea <- full
	diarrhea$ihme_loc_id <- diarrhea$iso3
...
## Use that file ("/FILEPATH/")
## to identify subnational units from the survey, map them to GBD locations ##
## then save it as "/FILEPATH/" ##
## This file ("/FILEPATH/") was created in
## Excel manually. The purpose of
