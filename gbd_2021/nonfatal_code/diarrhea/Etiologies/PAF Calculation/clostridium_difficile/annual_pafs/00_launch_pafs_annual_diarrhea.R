...
create_scalar <- F
upload_rei <- 183 # "all" or specify a subset of REI ids
...
# store qsub command
qsub = paste0('qsub -e /PATH/sgeoutput/', user, '/errors/ -o /PATH/sgeoutput/', user, '/output/ -cwd -N diarrhea_paf_array -P proj_tb ' ,
              # new arguments
              '-l fthread=10 -l m_mem_free=50G -q all.q -l archive=TRUE -t ', my_tasks, ' ',
              shell_file, ' -s ', script_path, ' ', args)
# submit job
system(qsub)
