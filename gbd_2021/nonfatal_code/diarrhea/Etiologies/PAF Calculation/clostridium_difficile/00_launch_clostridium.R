...
version_id <- 244 # Update to latest version of codcorrect
...
for(l in locations) {
  ...
  qsub = paste0('qsub -e /FILEPATH/errors/ -o /FILEPATH/output/ -cwd -N cdifficile_paf_', l, ' -P ihme_general ',
                ' -l fthread=1 -l m_mem_free=20G -q all.q -l archive=TRUE ',
                '/FILEPATH/execRscript.sh -s /FILEPATH/01_clostridium_codcorrect_2020.r ', args)
