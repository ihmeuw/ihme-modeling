//// Create cholera files for draws runs ////
...
//use "$/FILEPATH/", clear
local years 1990 1991 1992 1993 1994 1995 1996 1997 1998 1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020 2021 2022
get_population, year_id(`years') location_id(all) age_group_id(all) sex_id(1 2) release_id(9) tool_type_id(3) clear
...
use age_group_id sex_id location_id measure_id year_id draw_`iter2' if measure_id==6 using "$/FILEPATH/", clear
...
use age_group_id sex_id location_id measure_id year_id draw_`iter2' if measure_id==5 using "$/FILEPATH/", clear
...
/// Load age pattern from DisMod proportion model at global level
	use age_group_id sex_id draw_`iter2' using "$/FILEPATH/", clear
	keep if sex_id==2
	merge m:1 age_group_id using "$/FILEPATH/", nogen keep(3)
...
/// Load Odds Ratios from GEMS, apply to Cholera Data ///
	use modelable_entity_id age_group_id odds_`iter' using "$/FILEPATH/", clear
...
// Make sure that a PAF is calculated for the proportion data 
// using odds ratios from GEMS. Account for how frequently 
// cholera is detected in inpatient populations (more frequently than non-inpatient)
// and crosswalk data to the non-inpatient reference category. Account for the age groups from the source data.
...
//use nid-sample_size extractor is_outlier cv_inpatient cv_community draw_`iter' using "$/FILEPATH/", clear
	use nid-cv_inpatient draw_`iter' using "$/FILEPATH/", clear
...
merge m:1 location_id year_id using "$/FILEPATH/", nogen force
...
// None of the predictors are significant, use random effects //	
...
// Create a "true_count" for where cases = reported cases to fix issue 
...
// Predict the overall proportion of diarrhea cases due to cholera //
		mixed lnprop lnsdi if prop < 0.025 || super_region_id: || region_id: || location_id:  
		predict ltr*,reffect
		gen l_re = ltr3
...
// Fix for where cholera isn't endemic //	
		...
		table region_name year_id if level==3 & inlist(year_id, 1990, 1995, 2000,2005, 2010, 2015, 2019, 2020, 2021, 2022), contents(sum overall)
...
//use age_group_id location_id year_id sex_id draw_`iter2' using "/FILEPATH/", clear

// From CODCorrect
 use age_group_id location_id measure_id year_id sex_id draw_`iter2' using "/FILEPATH/", clear
	...
	table year_id if inlist(year_id, 1990, 2010,  2019, 2020, 2021, 2022), c(sum d_diarrhea) row
tempfile mortality
save `mortality'
saveold "$/FILEPATH/", replace

use age_group_id location_id year_id sex_id cf_`iter' using "/FILEPATH/", clear
	keep if inlist(year_id, 1990, 1995, 2000, 2005, 2010, 2015,  2019, 2020, 2021, 2022)
	...
	table region_name year_id if inlist(year_id, 1990, 2010, 2019, 2020, 2021, 2022), c(sum n_diarrhea sum d_diarrhea sum s_ncases  sum s_ndeaths) row
