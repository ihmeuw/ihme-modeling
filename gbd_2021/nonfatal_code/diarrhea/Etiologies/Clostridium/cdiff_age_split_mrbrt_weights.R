...
## Define age groups
age_info <- read.csv("/FILEPATH/age_mapping_current.csv")
...
age_split_mrbrt_weights <- function(df, denominator_weights, output_dir, bounded=T, title="", global_merge = T){
  ...
  # Using 999999 for new data NIDs
  input_data$nid <- paste0(input_data$nid,"_",input_data$location_id)

  age_anchor <- input_data[1:4,]
  age_anchor$logit_mean <- c(quantile(input_data$logit_mean[input_data$age_mid > 5], 0.25, na.rm=T),
                             quantile(input_data$logit_mean[input_data$age_mid > 5], 0.15, na.rm=T),
                             quantile(input_data$logit_mean[input_data$age_mid > 5], 0.1, na.rm=T),
                             quantile(input_data$logit_mean[input_data$age_mid > 5], 0.05, na.rm=T))
  age_anchor$delta_logit_se <- c(quantile(input_data$delta_logit_se, 0.05, na.rm=T),
                             quantile(input_data$delta_logit_se, 0.05, na.rm=T),
                             quantile(input_data$delta_logit_se, 0.05, na.rm=T),
                             quantile(input_data$delta_logit_se, 0.05, na.rm=T))
  age_anchor$mean <- inv.logit(age_anchor$logit_mean)
  age_anchor$age_mid <- c(70,80,90,99)
  age_anchor$age_end <- c(70,80,90,99)
  age_anchor$nid <- 5:8

  #input_data <- rbind(input_data, age_anchor)

  # Test
    #input_data$age_mid <- input_data$age_end

  # Run an MR-BRT model #
  fit1 <- run_mr_brt(
    output_dir = output_dir,
    model_label = "age_curve_name",
    data = input_data,
    mean_var = "logit_mean",
    se_var = "delta_logit_se",
    #covs = list(cov_info("age_mid","X", degree = 3, i_knots = "2,5,30")),
    covs = list(cov_info("age_mid","X", degree = 2, n_i_knots = 3, bspline_gprior_mean = "0,0,0,0", bspline_gprior_var = "inf,inf,inf,inf")),
    trim_pct = 0.1,
    method = "trim_maxL",
    #study_id = "nid",
  ...
  #preds$pred_mean <- exp(preds$Y_mean)
...
##----------------------------------------------------------------------------------------------------
  ...
  age_non_specific$note_modeler <- paste0(age_non_specific$note_modeler," | these data were split from original age 0-99 using an age curve from MR-BRT. Numerator weights from the MR-BRT model,
