...
master_mrbrt_crosswalk <- function(
  ...
  ## Naming the log files. These should take the format: FILEPATH/etc.
  ...
  if (nrow(data[sex=="Both",]) > 0) {
    ...
    ## Notify how many sex-specific data points you have to XW.
    ...
    sex_matches[, id_var := 1:nrow(sex_matches)] # This assumes we're all using "id_var" as the id_vars (USERNAME)
  ...
  if (num_both_sex > 0) {
    if (sex_split_only) {
      ...
      sex_split_documentation <- list(arguments = arguments,
                                    ...
                                    ratio_se = ratio_se)
      #saveRDS(sex_split_documentation, file = paste0(logs_root, log_name, ".RDS"))
      ...
      sex_split_crosswalk_documentation <- list(arguments = arguments,
                                                ...
                                                ratio_mean = ratio_mean,
                                                ratio_se = ratio_se)
      #saveRDS(sex_split_crosswalk_documentation, file = paste0(logs_root, log_name, ".RDS"))
    ...
    crosswalk_documentation <- list(arguments = arguments,
                                    ...
                                    xw_ratios=matches,
                                    data_for_comp=data_for_comparison)
    #saveRDS(crosswalk_documentation, file = paste0(logs_root, log_name, ".RDS"))
