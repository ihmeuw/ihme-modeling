...
def logit_rake(dataset, p_i, p_N, N_i, rake_locations, non_rake_locations, tolerance, max_jump=12):
    ...
    if(chk > 0):
        ...
        drops = ['a', 'b', 'c', 'guess_a', 'guess_b', 'guess_c',
                 'F_a', 'F_b', 'F_c', 'new_est', 'population']
...
def set_to_0_1(raked: xr.Dataset, dropped: xr.Dataset) -> xr.Dataset:
    ...
    merged = merged.update({"est": merged.where(merged.est_parent <= 10, 1).est})
    merged = merged.update({"est": merged.where(merged.est_parent >= -10, 0).est})
    merged = merged.update({"est": merged.where(merged.est <= 10, 1).est})
    merged = merged.update({"est": merged.where(merged.est >= -10, 0).est})
    return merged
...
if __name__ == "__main__":
    ...
    if run_type in ['in_sample_selection', 'oos_selection']:
        ...
        param_set = fit.loc[fit.best == 1, 'parameter_set'].iat[0]
    else:
        param_set = 0
        ...
        if(status == 'rake'):
            ...
            for parent in parents:
                ...
                if rake_logit == 1:
                    ...
                    dropped_1.append(xrake.where(xrake.est_parent > 10, drop=True))
                    dropped_0.append(xrake.where(xrake.est_parent < -10, drop=True))
                    xrake_kept = xrake.where(xrake.est_parent <= 10, drop=True)
                    xrake_kept = xrake_kept.where(xrake_kept.est_parent >= -10, drop=True)
                    xdf = logit_rake(xrake_kept, p_i='est', p_N='est_parent', N_i='population',
