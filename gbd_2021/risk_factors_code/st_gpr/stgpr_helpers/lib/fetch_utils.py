...
from db_stgpr.api.enums import DataStage, TransformType
...
def get_results(
    ...
    if not model_iteration_id and model_type_utils.is_selection_model(
        ...
        raise exc.NoModelIterationProvided(
            f"Selection model requires model iteration ID to pull {entity.lower()}"
        )
    ...
    results_df = db_stgpr.get_results(model_iteration_id, table, session, filters).drop(
        columns=columns.MODEL_ITERATION_ID
    )
    ...
    if level_space and entity == "Spacetime estimates":
        transform = db_stgpr.get_transform_type(stgpr_version_id, session)
        results_df[columns.VAL] = transform_utils.transform_data(
            results_df[columns.VAL], transform.name, reverse=True
        )
    elif not level_space and entity in ["GPR estimates", "Final estimates"]:
        transform = db_stgpr.get_transform_type(stgpr_version_id, session)
        results_df[
            [columns.VAL, columns.UPPER, columns.LOWER]
        ] = transform_utils.transform_data(
            results_df[[columns.VAL, columns.UPPER, columns.LOWER]], transform.name
        )
...
def get_data(
    ...
    data_stage_enum = (
        DataStage.prepped if data_stage == DataStage.with_nsv.name else DataStage[data_stage]
    )
    data_df = db_stgpr.get_data(
        stgpr_version_id, data_stage_enum, session, model_iteration_id, filters
    )
    if data_stage == DataStage.with_nsv.name:
...
def get_stage_1_estimates(
    ...
    if level_space:
        ...
        stage_1_estimates[columns.VAL] = transform_utils.transform_data(
            stage_1_estimates[columns.VAL], transform.name, reverse=True
        )
...
def _add_nsv_to_variance(df: pd.DataFrame, transform: TransformType) -> pd.DataFrame:
    mean_level_space = transform_utils.transform_data(
        df[columns.VAL], transform.name, reverse=True
    )
    variance_level_space = transform_utils.transform_variance(
        df[columns.VAL], df[columns.VARIANCE], transform.name, reverse=True
    )
    ...
    return df.assign(
        **{
            columns.VARIANCE: transform_utils.transform_variance(
                mean_level_space, variance_plus_nsv, transform.name
            )
        }
    )
