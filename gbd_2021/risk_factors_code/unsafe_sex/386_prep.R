...
get_age_pattern <- function(locs){
  age_pattern <- as.data.table(get_outputs(topic = "rei", rei_id = c(103), cause_id = 402,
                                           ...
                                           year_id = 2010)) ##imposing age pattern (switch to status=best when there is one)
...
final_dt<-final_dt[location_id%in%c(95), step2_location_year:="needed to switch UK to a dismod location, changed to england"]
final_dt<-final_dt[location_id%in%c(95), location_id:=4749]
...
#### Outliers (AND MOVING TO STEP 4 FROM ITERATIVE)
source("FILEPATH")
source("FILEPATH")
source("FILEPATH")

df<-get_bundle_data(bundle_id = bid, decomp_step = "step2", export = T)
upload_bundle_data(bundle_id = bid, decomp_step = "step4", filepath = "FILEPATH")
bund<-save_bundle_version(bundle_id = bid, decomp_step = "step4", include_clinical = F)
save_crosswalk_version(bundle_version_id = bund$bundle_version_id, data_filepath = "FILEPATH", description = "Step 2 data in 1-p space")

df<-get_crosswalk_version(crosswalk_version_id = 13016)
...
save_bulk_outlier(crosswalk_version_id = 13016, decomp_step = "step4",
                  filepath = "FILEPATH",
                  description = "St Lucia outliers dropped, 1-p")
