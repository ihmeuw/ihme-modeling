################################################################################
## DESCRIPTION ##  Do network diet analysis; called by launch_crosswalk. 
## Three parts to this script. 1- prep data, 2-do the network analysis in mr-brt, 3- adjust the data
###############################################################################
...
do_everything <- function(risk, mrbrt_version, output_dir="FILEPATH", do_data_prep=TRUE, run_mrbrt=TRUE, adjust_data=TRUE, bundle_prep=TRUE){
  ...
  # using the predict_data_prep function pulls in data from the "new compiled" set. 
  if(adjust_data){
