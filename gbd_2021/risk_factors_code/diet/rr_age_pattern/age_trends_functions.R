...
weighted_average <- function(causeid, reid, plot = T){
  ...
  # Will error if no mediators in matrix, or if mediators in matrix are not among those we have age patterns for (bmi, sbp, fgp, ldl)
  #
  ...
  # Read in the mediation matrix from GBD 2019 (still used in GBD 2020 PAF code)
...
apply_age_pattern <- function(causeid, reid, risk_curve_draws_df, age_pattern_draws_df, draws_in_log = F, return_draws_log = F, plot = T){
  ...
  # 6/29 test: RR_age_specific = AF*(RR - 1) + 1, where RR is in normal space
