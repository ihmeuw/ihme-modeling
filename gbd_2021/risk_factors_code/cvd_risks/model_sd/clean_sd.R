...
new_data <- rbindlist(lapply(new_files, function(x){
  message(x)
  extraction <- as.data.table(openxlsx::read.xlsx(x, sheet="literature_full"))[-1,]   ##USERNAME: drop the first row, useless info
...
subnat_data <- rbindlist(lapply(subnat_reextract_paths, function(x){
  message(x)
  extraction <- as.data.table(openxlsx::read.xlsx(x))[-1,]   ##USERNAME: drop the first row, useless info
  extraction[, source_file:=x]
}), fill=T)
