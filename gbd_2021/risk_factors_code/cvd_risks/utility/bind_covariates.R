...
drp_ages <- F ##USERNAME: default to keep ages, only drop if created below
  if(length(multi_age_covs)>0 | length(multi_agesex_covs)>0){
    if(!"age_group_id" %in% names(df) | all(df[["age_group_id"]]==22)){
      ...
      if("age_start" %in% names(df) & "age_end" %in% names(df)){
        ...
        ## create 'expanded' again, this time with column 'sex_split_id', with repetitions=split$n.sex
    ...
    if(length(multi_age_covs)!=0 | length(multi_agesex_covs)>0 | length(multi_sex_covs)!=0){
      ...
      check_agg <- function(temp_cov, check_vars, agg_covs, u, covar){
        ...
        if(nrow(miss_splits)>0){
          ...
          invisible(lapply(c(check_vars), function(x){  ##USERNAME: merge vars gets inherited from agg()
      ...
      agg <- function(agg_covs, merge_vars, check_vars){
        ...
        for(u in 1:length(agg_covs)){
          ...
          ##USERNAME: aggregate, weighted by population size
      ...
      pops <- get_population(location_id=unique(df$location_id), 
                           age_group_id=c(unique(df.exp$age_group_id), 22), ##USERNAME: need all age group ids, and for multi_sex_covs 
      ...
      if(3 %in% unique(df$sex_id) & length(multi_sex_covs)!=0){
        ...
        ##USERNAME: create age group id for merging w/ pops
        ...
        multi_sex_covs <- list() ##USERNAME: empty this list
