...
################### EXPAND DATA #########################################
## Arguments
#   - starts_ends: a named list of character vectors of length 2. 
...
expand_data <- function(df, starts_ends, var_values, value_weights=NULL){
  ...
  ################### CHECKS #########################################
    ## make sure certain cols don't already exist
      ...
      lapply(new_cols, df=df, check_exists, not_exist=T) ##USERNAME: check_exists function comes from data_tests.R script
  ...
  ################### ROUND VALUES TO NEAREST VALUE #########################################
    ## create temporary rounded start/end cols
  ...
  ## reduce weights
    if(missing(value_weights)){
      df[, split_wt:=split_wt/get(paste0("n_", varname))] ##USERNAME: if each value given equal weight, just divide by number of splits performed
    } else {
...
make_fixef_matrix <- function(df, fixefs=NULL, add_intercept=F){
  ...
  if(length(fixefs)>0){
    ...
    ##USERNAME: drop first column, the intercept created by model.matrix. This gets recreated in toggle below
    X <- as.matrix(X[, -1])
...
make_tridiag_matrix <- function(n_sites, rho=NULL){
  ...
  if(!is.null(rho)){
    m0[col(m0)==row(m0)] <- 1+rho^2 ## get diagonals
    m1[col(m1)==row(m1)-1 | col(m1)==row(m1)+1]<- -rho ## get off-diagonals
    m0<-as(m0, "dgTMatrix")
    
  } else {
    m0[col(m0)==row(m0)]<-1 ## get diagonals
...
predict_draws <- function(prediction_math, draw_list, data_list=NULL,
  ...
  ################### SUB FUNCTIONS #########################################
    ...
    #   n[x]
    # })
    ##s add draw vals to temporary environment
  ...
  if(return_draws==T){
    ...
    if(upper_lower==T){
      ...
      ## calculate mean and SD of draws if not returNing draws and want SE
