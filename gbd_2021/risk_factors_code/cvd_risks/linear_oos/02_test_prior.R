...
test_prior <- function(crosswalk_version_id=NA, path_to_data=NA, decomp_step, cov_list, data_transform,
                       ...
                       drop_nids=FALSE, seed=32594, location_set_id=22, proj="proj_cvd", 
                       m_mem_free=2, forms_per_job=10){
  
  if(F){# interactively test SBP data
    crosswalk_version_id=17183 
    decomp_step="iterative"
    cov_list= c('Socio-demographic Index', 'Liters of alcohol consumed per capita','Age- and sex-specific SEV for Low vegetables',
                'Mean BMI', 'Prevalence of obesit', 'Healthcare access and quality index','Age- and sex-specific SEV for Low omega-3',
  ...
  output_folder <- paste0("/FILEPATH/",crosswalk_version_id,"/") #path to temporarily save model outputs--this folder gets created and deleted during the function
  ...
  for(sexchar in sex_list){
    ...
    if(length(unique(data.s$age_group_id))==1){
      if(unique(data.s$age_group_id)==22  | unique(data.s$age_group_id)==27){
        ...
        by_age<-0
      }else if(!(unique(data.s$age_group_id) %in% c(10:20, 30:33))){
