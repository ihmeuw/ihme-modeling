...
# otherwise we will use the midpoint
...
#------------------Format covariates--------------------------------------------
# remove covariates that only have 1 level for all observations
...
# note: cv_confounding uncontrolled only has the values of 0 and 1 for these extractions
...
# for studies with >1 observations, we weight the SEs to represent smaller sample sizes
for (n in dt$nid){
  if (nrow(dt[nid==n])>1){
