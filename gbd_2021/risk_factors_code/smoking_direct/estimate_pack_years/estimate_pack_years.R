...
for (a in c(7:20, 30, 31, 32, 235)) {
  for (y in c(seq(1960, 2010, 5), seq(1962, 2017, 5))) {
    ...
    assign(paste0("vec_amt_", a, "_", s, "_", y), mapply(fit_dist_v2, location = l, year = y, age = a, sex = s, me_name = "total_amt_smoked", mean = temp$mean, sd = temp$sd, minval = 0.01, maxval = 60, n_samples = 5000, subtract_age = F, return_fxn = F))
  }
}
...
for (y in year_list) {
  ...
  for (a in c(9:20, 30, 31, 32, 235)) {
    ...
    years_smoked_vec<-round(as.vector(mapply(fit_dist_v2, location = l, year = y, age = a, sex = s, me_name = "init_age_smoking", mean = temp$mean, sd = temp$sd, minval = 5, maxval = max_return(a), n_samples = 100, subtract_age = T, return_fxn = F)),0)
