#-------------------------------------------------
# Purpose: modeling age trends of smoking-CVDs RRs
#-------------------------------------------------
...
if(grepl('peripheral', ro_pair)){
  pred_data[, cv_outcome_pad := 1 * age_midpoint]
}
...
plot_age_rr <- ggplot(pred_data, aes(x = age_start, y = exp(Y_mean_re)))+geom_line()+
  geom_ribbon(aes(ymin = exp(Y_mean_lo_re), ymax = exp(Y_mean_hi_re)), alpha = 0.3) + 
...
p <- ggplot()+
  geom_ribbon(data = pred_data, aes(x = age_start, ymin = exp(Y_mean_lo_re), ymax = exp(Y_mean_hi_re), fill ="95% CI"), alpha = 1/5)+
  geom_line(data = pred_data, aes(x = age_start, y = exp(Y_mean_re), color = "mean estimate")) +
  geom_point(data = mod1_data, aes(x=(age_start+age_end)/2, y=exp(obs), alpha = 1/obs_se, color = as.factor(w)))+
  geom_errorbarh(data = mod1_data, aes(xmin = age_start, xmax = age_end, y = exp(obs), alpha = 1/obs_se, color = as.factor(w)))+
  geom_abline(slope = 0, intercept = 1, color = "blue", alpha = 1/2)+
  theme_classic()+
...
save_data[, `:=` (Y_mean_re = exp(Y_mean_re),
                  Y_mean_lo_re = exp(Y_mean_lo_re),
                  Y_mean_hi_re = exp(Y_mean_hi_re))]
...
y_draws[,  paste0("draw_", 0:999) := lapply(.SD, exp), .SDcols= paste0("draw_", 0:999)] 
## NOTE: I exponentiate log_rr, and the AF is based on the RR.

#write.csv(y_draws, paste0(save_dir, risk, "_", out, "_y_draws.csv"), row.names = F)
...
# Attenuation factor = percent_change + 1 (AF is based on RR)
draws_long[, atten_factor := (value- ref_age_group_val)/ref_age_group_val + 1]
...
# Visualize summary
plot_attenuation <- ggplot(summary, aes(x = age_start, y = pct_mean))+geom_line()+
