# Dichotomous VAD
# 
# Final GBD 2020 risk curves for VAD
...
# --------------------------------------------------------------------------------
outcome <- c("Measles","Diarrhea", "LRTI")
...
for(out in outcome){
  for(bias_cov in test_covs){
    ...
    # load data from whereever it belongs - convert to harmful
    outdata <- fread(paste0(my_data_dir, out, "_data_file.csv"))
    outdata[, log_effect_size:=log_effect_size*-1]
    
    # trimming for LRI, not for Measles & Diarrhea
    pct_trim <- ifelse(out == "LRTI", 0.9, 1)
    ...
    # make draws with and without mean gamma
    ...
    pub_bias_results <- rbind(pub_bias_results, summary)
    ...
    a <- plot_dich_simple_forest(dat1, mod1, pred_data_plot, rr_lab = "log(RR)", plot_title = paste0("VAD - ",out,", ", trim_lab, ", ", cov_lab))
    b <- plot_dich_simple_funnel(dat1, mod1, pred_data_plot, rr_lab = "log(RR)", plot_title = paste0("VAD - ",out,", ", trim_lab, ", ", cov_lab))
