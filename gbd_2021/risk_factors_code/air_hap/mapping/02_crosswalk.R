...
crosswalk <- fread(file.path(home_dir, "FILEPATH/personal_exp_factor_DATE.csv"))
...
crosswalk[location_name=="Republic of the Gambia", location_name:="Gambia"] # this location didn't match a GBD2020 location
...
# if the value is negative, replace with 1
...
ratio_summary <- data.table(mean = c(mean(betas_sum$groupchild),mean(betas_sum$groupmale)),
