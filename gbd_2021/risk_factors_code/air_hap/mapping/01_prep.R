...
# Set outcome value to be arithmetic mean, geometric mean, or median in that order. Also label
...
# man/woman/child
# for  S no 137, only keep aggregate for women and male/female for children
...
# 182B doesn't specify, but I looked at database and discovered this
...
# 191 only reports heating fuel. problem?
...
#------------------Add extractions from meta-analysis for GBD 2019 ----------------------
...
#------------------Add new extractions for GBD 2020 -------------------------------------

gbd <- fread(file.path(in_dir,"hap_map_gbd_2020.csv"))

gbd[,S.No:=509] # continuing from where 2019 mapping data left off
...
gbd[coal==1 | charcoal==1, coal:=1] # recoding charcoal as coal because that is how we categorize it for GBD
...
# Received concentrations for the PURE-HAP study, so I'll use those preferentially because they are more specific
...
# replace negatives with 1 (we are going to log-transform this)
