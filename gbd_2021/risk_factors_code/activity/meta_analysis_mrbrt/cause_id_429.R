...
unique(df$cv_lpa_domain)
df[,custom_leisure_pa := ifelse(cv_lpa_domain %in% c("sports", "recreation"), 1, 0)]
unique(df$cv_lpa_unit)
df[,custom_met_conversion := ifelse(cv_lpa_unit %like% "MET", 0, 1)]

# Remove cv_lpa_ from column names
setnames(df, names(df), gsub("cv_lpa_", "", names(df)))
...
bias_cols <- bias_cols[!bias_cols %in% c("reverse_causality_score",
                                         "custom_domain", "exposure_idv_score", "exposure_obj_score", "outcomes_blind_score",
                                         "control_rct_other_score", "selection_bias_score")]
bias_cols <- c(bias_cols, "control_for_bmi")
...
test_cols <- e_m_cols[!e_m_cols %in% c("e_m_custom_incidence", "e_m_custom_mortality", "e_m_custom_male", "e_m_custom_female", 
                                       "e_m_custom_met_conversion", "e_m_custom_leisure_pa")]#, "e_m_control_for_bmi")] # testing out fewer columns
# CovFinder covs
...
if(fit_model){
  ...
  py_save_object(final_model, paste0(save_dir, "FILEPATH"), pickle = "dill")
  
} else {
  final_model <- py_load_object(paste0(save_dir, "FILEPATH"), pickle = "dill")
}
