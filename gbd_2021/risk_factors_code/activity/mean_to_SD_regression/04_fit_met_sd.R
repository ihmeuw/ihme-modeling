...
code_dir <- if (os == "Linux") paste0("FILEPATH", user, "/") else if (os == "FILEPATH") ""
...
source("FILEPATH")
library(dplyr)
age_data <- get_age_metadata(age_group_set_id = 19)
...
data_df <- data.table(data_df)                                                                                                     # END OF BLOCK

data_df[, outlier := (st_dev / mean > 1) | (st_dev / mean < .1)]

## Run mean-sd regressions
sd_mean_mod_mets <- lm(log(st_dev)~log(mean) + age_group_years_start + super_region_name + female, data = data_df[outlier == F])   # THIS LINE IS FOR TESTING
