...
# Description: Read in raw copula results and carry out aging (parallelized by location)
...
if(run_interactively == 1){
  ...
  apply_air_pollution_shifting <- param_map[task_id, apply_air_pollution_shifting]
  jointdistr_dir <- param_map[task_id, jointdistr_dir]
  agedcohorts_dir <- param_map[task_id, agedcohorts_dir]
  version <- param_map[task_id, version]
  joint_distr_type <- param_map[task_id, joint_distr_type]
  
}

copula_dir <- file.path(jointdistr_dir, version, joint_distr_type)

# Create dirs if needed
dir.create(file.path(agedcohorts_dir, version, joint_distr_type), showWarnings = F, recursive = T)
dir.create(file.path(agedcohorts_dir, version, joint_distr_type, "/warnings/"), showWarnings = F, recursive = T)
dir.create(file.path(agedcohorts_dir, version, joint_distr_type, "/cohorts/"), showWarnings = F, recursive = T)


print(paste("FP:", jointdistr_fp, "applying air pollution shifts", apply_air_pollution_shifting))
...
get_other_cols <- function(data, toMatch = c("ga", "bw")){
  # should return c("location_id", "sex_id", "age_group_id", "year_id", "draw")
...
get_data <- function(loc){ #sex, year
  
  input_file <- file.path(copula_dir, jointdistr_fp)
...
merge_on_me_map <- function(data){
  ...
  ## Fill missing modelable_entity_id values
...
fill_missing_id_values <- function(data){
  ...
  ## change this to be any column that has either NA or ONLY 1 value 
...
move_draw_num_to_end <- function(x, age){
  ...
  prefix <- strtrim(no_age_id, width = final_underscore_position) # pop_survive_
...
get_mortality_information <- function(loc_id){
  ...
  mort <- fread(paste0("FILEPATH", loc_id, ".csv"))  
  ...
  # Change pop_baseline for age_group 3 to be pop_surv of age_group_2
...
merge_on_rr_information <- function(data, age_group_id = NULL){
  ...
  rr_id_vars <- find_non_draw_cols(rrs, "draw") # "age_group_id" "cause_id"     "sex_id"       "year_id"      "mortality"    "morbidity"    "parameter" 
  ...
  rrs <- rrs[year_id == 1990]
  
  adj.year <- lapply(c(1990:2022), function(yid){
...
distribute_impossible_exposure_to_bins <- function(data){
  ...
  # GA data that is unassigned beacuse it is over 42 is ressigned to 4500
...
age_cohort <- function(data, age_group_id = NULL){
  ...
  if(age_group_id == 2){
    ...
    #change_final_exposure_to_baseline(data, age_group_id)
...
calculate_survivors_per_category <- function(data) {
  ...
  if(apply_air_pollution_shifting){
...
vet_age_cohort <- function(data){
  ...
  # Allow 1.01 to not be an inconsistency
  ...
  if (num_inconsistencies != 0){
    ...
    write.csv(vet_file, paste0(agedcohorts_dir, "/", version, "/", joint_distr_type, "/warnings/inconsistencies_", gsub(jointdistr_fp, pattern = ".rds", replacement = ""), ".csv"), row.names = F, na = "")
...
vet_dimensions <- function(dt){
  ...
  if(num_ages != 2 | 
    ...
    write.csv(vet_file, paste0(agedcohorts_dir, "/", version,  "/", joint_distr_type, "/warnings/incorrect_dimensions_", gsub(jointdistr_fp, pattern = ".rds", replacement = ""), ".csv"), row.names = F, na = "")
...
location_id = as.integer(unlist(tstrsplit(jointdistr_fp, "_", keep = 1)))
...
saveRDS(aged_cohort, file.path(agedcohorts_dir, version, joint_distr_type, "cohorts", jointdistr_fp))
