...
ids <- fread("FILEPATH/lbwsg_ids.csv")
...
ggplot(files_plot[me_name == "ga_37"]) + 
  geom_point(aes(x = mean_lmp, y = mean_oe), color = "blue") + 
  geom_abline(slope = 1, intercept = 0) + theme_bw() + 
  ggtitle("GA37 LMP-OE Matches") + xlim(0,0.2) + ylim(0,0.2) 

dev.off()


files <- files[me_name == "ga_37" & ga_method == "oe"]
...
# ----- GA_37 Prevalence Crosswalk

ga_model <- py_load_object(filename = paste0("FILEPATH"), pickle = "dill")

data <- merge(data, ga_37_map[,.(nid, ga_estimate_method_tag)], all.x = T, by = "nid")

data[microdata_lmp_tag == 1, ga_estimate_method_tag := "LMP"]


data[is.na(ga_estimate_method_tag) & is.na(cv_ga_28), ga_estimate_method_tag := "UNKNOWN"]
data[cv_ga_28 == 1 | cv_lbw == 1, ga_estimate_method_tag := "NO_XWALK"]

noxwalk_data <- data[mean == 0 | mean == 1 | cv_lbw == 1 | cv_ga_28 == 1 |
                       ga_estimate_method_tag %in% c("NO_XWALK", "OTHER", "UNKNOWN") | 
                       !is.na(mean_adj_LMP), ]

data <- data[!(mean == 0 | mean == 1 | cv_lbw == 1 | !is.na(cv_ga_28) |
                 ga_estimate_method_tag %in% c("NO_XWALK", "OTHER", "UNKNOWN") | 
                 !is.na(mean_adj_LMP)), ]
...
plot_data <- data[is.na(cv_ga_28) & cv_lbw == 0]
...
ggplot(plot_data) + 
  ...
  theme_bw() + ggtitle("GA37 LMP Xwalk", subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
for(ihme_loc in unique(substr(plot_data[ga_estimate_method_tag %in% c("LMP") , ihme_loc_id], 1,3))){
  ...
  gg <- ggplot(plot_data[ihme_loc_id %like% ihme_loc]) + 
    ...
    theme_bw()  + ggtitle(paste0("GA37 LMP Xwalk - ", ihme_loc), subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
# ----- GA_37 GA_OTHER Prevalence Crosswalk
...
noxwalk_data <- data[cv_ga_28 == 1 | cv_lbw ==1 | ga_estimate_method_tag %in% c("NO_XWALK", "LMP") | mean == 0 | mean == 1]
data <- data[is.na(cv_ga_28) & !(cv_lbw == 1 | ga_estimate_method_tag %in% c("NO_XWALK", "LMP")  | mean == 0 | mean == 1) ]
...
data[, mid_year_cat := cut(mid_year, breaks = seq(1990, 2025, 5))]
...
ggplot(matches[]) + 
  ...
  facet_wrap(~ihme_loc_id) + theme_bw() + xlim(0,0.3) + ylim(0,0.3) + 
  ggtitle("GA37 OtherGA-OE Matches")
...
ggplot(plot_data) + 
  ...
  facet_grid(~sex) + ylim(0,ylimend) + ylab("Unadjusted") + 
  theme_bw() + ggtitle("GA37 OtherGA Xwalk", subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
for(ihme_loc in unique(substr(plot_data[ga_estimate_method_tag %in% c("OtherGA"), ihme_loc_id], 1,3))){
  ...
  gg <- ggplot(plot_data[ihme_loc_id %like% ihme_loc]) + 
    ...
    facet_grid(sex~location_name) + ylim(0,ylimend) + ylab("Unadjusted") + 
    theme_bw()  + ggtitle(paste0("GA37 OtherGA Xwalk - ", ihme_loc), subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
# Remove from pool of crosswalk matches ga_28 data, inpatient, or MarketScan 2000 (issues in data) 
noxwalk_data <- data[cv_ga_28 == 1 | cv_lbw == 1| clinical_data_type == "inpatient" | nid == 244369 | mean == 0 | mean == 1]
data <- data[is.na(cv_ga_28) & !(cv_lbw == 1 | clinical_data_type == "inpatient" | nid == 244369 | mean == 0 | mean == 1) ]
...
ggplot(matches[]) + 
  ...
  geom_abline(slope = 1, intercept = 0) + 
  facet_wrap(~location_name) + theme_bw() + xlim(0,0.3) + ylim(0,0.3) + 
  ggtitle("ga_37 Claims-VR Matches")
...
plots$funnel_plot(
  ...
  plots_dir = "FILEPATH", 
  file_name = "ga_37_claims_vr_xwalk_funnel",
  write_file = TRUE
)
...
ggplot(plot_data) + 
  geom_point(aes(x = year_id, y = mean_adj_OtherGA, color = clinical_data_type, shape = ga_estimate_method_tag), alpha = 0.2) + 
  facet_grid(~sex) + ylim(0,ylimend) + ylab("Unadjusted") + 
  theme_bw() + ggtitle("GA37 Claims Xwalk", subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
for(ihme_loc in unique(substr(plot_data[clinical_data_type == "claims", ihme_loc_id], 1,3))){
  ...
  gg <- ggplot(plot_data[ihme_loc_id %like% ihme_loc]) + 
    geom_point(aes(x = year_id, y = mean_adj_OtherGA, color = clinical_data_type, shape = ga_estimate_method_tag) ) + 
    facet_grid(sex~location_name) + ylim(0,ylimend) + ylab("Unadjusted") + 
    theme_bw()  + ggtitle(paste0("GA37 Claims Xwalk - ", ihme_loc), subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
# Remove from pool of crosswalk matches ga_28 data, inpatient, or MarketScan 2000 (issues in data) 
noxwalk_data <- data[cv_ga_28 == 1 | cv_lbw == 1| clinical_data_type == "claims" | mean == 0 | mean == 1]
data <- data[is.na(cv_ga_28) & !(cv_lbw == 1 | clinical_data_type == "claims"  | mean == 0 | mean == 1) ]
...
for_matches <- merge_on_location_metadata(data)

# Remove BRA & GBR from pool of matches

matches_nat <- merge(for_matches[clinical_data_type == "non_clinical" & !ihme_loc_id %like% c("BRA|USA|GBR"), .(ihme_loc_id = substr(ihme_loc_id, 1,3), sex, sex_id, mid_year_cat, index.denom = temp_index, mean_other = mean, se_other = standard_error, nid.denom = nid)], 
                     for_matches[clinical_data_type == "inpatient" & !ihme_loc_id %like% c("BRA|USA|GBR"), .(ihme_loc_id = substr(ihme_loc_id, 1,3), sex, sex_id, mid_year_cat, index.num = temp_index, mean_inpatient = mean, se_inpatient = standard_error, nid.num = nid)], allow.cartesian = T)

matches_sub <- merge(for_matches[clinical_data_type == "non_clinical" & ihme_loc_id %like% c("USA"), .(ihme_loc_id, sex, sex_id, mid_year_cat, index.denom = temp_index, mean_other = mean, se_other = standard_error, nid.denom = nid)], 
                     for_matches[clinical_data_type == "inpatient" & ihme_loc_id %like% c("USA"), .(ihme_loc_id, sex, sex_id, mid_year_cat, index.num = temp_index, mean_inpatient = mean, se_inpatient = standard_error, nid.num = nid)], allow.cartesian = T)

matches <- rbind(matches_nat, matches_sub)

dat_diff <- as.data.frame(cbind(
  delta_transform(
    mean = matches$mean_inpatient, 
    sd = matches$se_inpatient,
    transformation = "linear_to_logit" ),
  delta_transform(
    mean = matches$mean_other, 
    sd = matches$se_other,
...
ggplot(matches[]) + 
  geom_point(aes(x = mean_inpatient, y = mean_other), color = "blue", alpha = 0.2) + 
  geom_abline(slope = 1, intercept = 0) + 
  facet_wrap(~ihme_loc_id) + theme_bw() + xlim(0,0.25) + ylim(0,0.25) + 
  ggtitle("ga_37 Inpatient-VR Matches")
...
data[, c("mean_adj_inpatient", "se_adj_inpatient", "diff", "diff_se", "data_id")] <- adjust_orig_vals(
  fit_object = matches_CWModel,       # result of CWModel()
  df = data,            # original data with obs to be adjusted
  orig_dorms = "clinical_data_type",  # name of column with (all) def/method levels
  orig_vals_mean = "mean",  # original mean
...
data[, mean := mean_adj_inpatient]
data[, standard_error := se_adj_inpatient]

noxwalk_data[, mean_adj_inpatient := mean]
noxwalk_data[, se_adj_inpatient := standard_error]
...
plots$funnel_plot(
  ...
  plot_note = "inpatient_vr_xwalk", 
  plots_dir = "FILEPATH", 
  file_name = "ga_37_inpatient_vr_xwalk_funnel",
  write_file = TRUE
)
...
ggplot(plot_data) + 
  geom_point(aes(x = year_id, y = mean_adj_claims, color = clinical_data_type, shape = ga_estimate_method_tag), alpha = 0.2) + 
  facet_grid(~sex) + ylim(0,ylimend) + ylab("Unadjusted") + 
  theme_bw() + ggtitle("GA37 Inpatient Xwalk", subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
for(ihme_loc in unique(substr(plot_data[clinical_data_type == "inpatient", ihme_loc_id], 1,3))){
  
  gg <- ggplot(plot_data[ihme_loc_id %like% ihme_loc]) + 
    geom_point(aes(x = year_id, y = mean_adj_claims, color = clinical_data_type, shape = ga_estimate_method_tag) ) + 
    facet_grid(sex~location_name) + ylim(0,ylimend) + ylab("Unadjusted") + 
    theme_bw()  + ggtitle(paste0("GA37 Inpatient Xwalk - ", ihme_loc), subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
for(l in c(0:5)) { 
  ...
  for(page_loc in unique(na.omit(data$page_loc_id))){
    ...
    gg <- ggplot(data=page_data_est, aes(x=year_id, y=mean, color = interaction(ga_estimate_method_tag, clinical_data_type))) + 
      geom_point() + theme_bw() + ggtitle(unique(locs[location_id == page_loc, location_name])) + ylim(0,0.5)
...
# REMOVE
data <- data[cv_ga_32_weeks == 0 | is.na(cv_ga_32_weeks)]

# Claims missing cv-lbw
data[is.na(cv_lbw), cv_lbw := 0 ]

noxwalk_data <- data[mean == 0 | mean == 1 | !is.na(cv_ga_28)]
data <- data[!(mean == 0 | mean == 1 | !is.na(cv_ga_28))]
...
data[cv_lbw == 0, cov_lbw := "preterm"]
data[cv_lbw == 1, cov_lbw := "lbw"]
...
matches_CWModel <- CWModel(
  ...
  cov_models = list(CovModel("intercept"),
                    CovModel("sex_id")),
  gold_dorm = "preterm", # level of 'ref_dorms' that's the gold standard
  inlier_pct = 0.95,
  use_random_intercept = TRUE
)
# 
# py_save_object(object = matches_CWModel, filename = paste0("/ihme/mnch/lbwsg/data/processing/processed_data/ga_37/", "ga_37_lbw_ga37_matches_CWModel", "_model.pkl"), pickle = "dill")

matches_CWModel <- py_load_object(filename = paste0("FILEPATH"), pickle = "dill")


data[, c("mean_adj_lbwga37", "se_adj_lbwga37", "diff", "diff_se", "data_id")] <- adjust_orig_vals(
  fit_object = matches_CWModel,       # result of CWModel()
  df = data,            # original data with obs to be adjusted
  orig_dorms = "cov_lbw",  # name of column with (all) def/method levels
  orig_vals_mean = "mean",  # original mean
  orig_vals_se = "standard_error"  # standard error of original mean
)

data[, mean := mean_adj_lbwga37]
data[, standard_error := se_adj_lbwga37]

noxwalk_data[, mean_adj_lbwga37 := mean]
noxwalk_data[, se_adj_lbwga37 := standard_error]
...
plots$funnel_plot(
  ...
  continuous_variables = list("sex_id"),
  obs_method = 'lbw',
  plot_note = "lbw_ga37_xwalk", 
  plots_dir = "FILEPATH", 
  file_name = "lbw_ga37_xwalk_funnel",
  write_file = TRUE
)
...
ggplot(plot_data) + 
  geom_point(aes(x = year_id, y = mean_adj_inpatient, color = as.factor(cv_lbw), shape = clinical_data_type), alpha = 0.2) + 
  facet_grid(~sex) + ylim(0,ylimend) + ylab("Unadjusted") + 
  theme_bw() + ggtitle("LBW-GA37 Xwalk", subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))

ggplot(plot_data) + 
  geom_point(aes(x = year_id, y = mean, color = as.factor(cv_lbw), shape = clinical_data_type), alpha = 0.2) + 
  facet_grid(~sex) + ylim(0,ylimend) + ylab("Adjusted") + 
  theme_bw()  + ggtitle("LBW-GA37 Xwalk", subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))

for(ihme_loc in unique(substr(plot_data[cv_lbw == 1, ihme_loc_id], 1,3))){
  
  gg <- ggplot(plot_data[ihme_loc_id %like% ihme_loc]) + 
    geom_point(aes(x = year_id, y = mean_adj_inpatient, color = as.factor(cv_lbw), shape = clinical_data_type)) + 
    facet_grid(sex~location_name) + ylim(0,ylimend) + ylab("Unadjusted") + 
    theme_bw()  + ggtitle(paste0("LBW-GA37 Xwalk - ", ihme_loc), subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
  
  print(gg)
  
  
  gg <- ggplot(plot_data[ihme_loc_id %like% ihme_loc]) + 
    geom_point(aes(x = year_id, y = mean, color = as.factor(cv_lbw), shape = clinical_data_type)) + 
    facet_grid(sex~location_name) + ylim(0,ylimend) + ylab("Adjusted") + 
    theme_bw()  + ggtitle(paste0("LBW-GA37 Xwalk - ", ihme_loc), subtitle = subtitle)#+ theme(axis.text.x=element_blank()) #+ theme(scale_size_manual(values = c(0.0005,0.001, 0.002,0.006)))
...
for(l in c(0:5)) { 
  ...
  data <- melt(data, id.vars = c("clinical_data_type", "path_to_top_parent", "ga_estimate_method_tag", "location_id", "year_id", "sex_id", "nid"), 
               measure.vars = c("mean_adj_agesex", "mean_adj_LMP", "mean_adj_OtherGA", "mean_adj_claims", "mean_adj_inpatient", "mean_adj_lbwga37"))
  ...
  for(plot_loc in unique(na.omit(data$plot_loc_id))){
    ...
    gg <- ggplot(data=page_data_est, aes(x=plot_loc_name, y=value, fill = variable)) + 
      geom_boxplot() + theme_bw() + ylim(0,0.5)
