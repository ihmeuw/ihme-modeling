...
#
# Author: AUTHOR
# Purpose: Logit rake level 5 and 6 subnats to raked leve 4 parent subnats
#
#####################################################################################################################################################################################################################################
rm(list=ls())
...
file.list <- paste0(root, "exposure/csa_new/05_holt_data/forecasted_draws/", risk, '/', run_id, '/', version, '/', c(as.character(unique(locs[parent_id==l]$location_id))), '.csv')
...
lvl4 <- fread(paste0(root, "exposure/csa_new/05_holt_data/raked_draws/", risk, '/', run_id, '/', version, '/', l, '.csv'))
...
dir.create(paste0(root,"exposure/csa_new/05_holt_data/raked_draws/",risk, '/', run_id,"/", version, "/"), recursive=T)
for(c.loc in unique(raked$location_id)){
  print(c.loc)
  write.csv(raked[location_id == c.loc], paste0(root,"exposure/csa_new/05_holt_data/raked_draws/",risk, '/', run_id,"/", version, "/", c.loc,".csv") )
}
