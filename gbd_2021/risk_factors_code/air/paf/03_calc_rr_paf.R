...
old.cols <- paste0("draw_", 0:(draws.required-1)) # comes in 0-999 format
...
ambient.grid.in <- file.path(home.dir,"FILEPATH",exp.grid.version)
...
# read in ambient draws (using collapsed gridcells by ug/m3)
...
# Read in RR data ----------------------------------------------------------------
  ...
  if(row$group=="adults"){
    ...
    mrbrt <- exp(mrbrt)
    mrbrt[,exposure:=NULL]
    mrbrt[,exposure_spline:=c(seq(0,10,by=0.1),seq(11,1000,by=1))] # predicted out new exposures
  ...
  # Recalculate hap RR: excess risk for the combined exposure minus the excess risk due to ambient plus 1
  ...
  exp <- merge(exp[,lapply(.SD,weighted.mean,w=pop_weight),
...
saveRiskCause <- function(this.risk,this.cause, this.age,file){
  ...
  write.csv(temp, paste0(home.dir,"/",this.risk,"/rr/draws/",output.version,"/",
                        this.age,"_",
                        this.cause, "_", 
                        this.country, "_",
                        this.year, ".csv"),
...
saveSex <- function(this.sex, this.cause, this.measure, this.risk, this.age, file){
    ...
    write.csv(temp, paste0(home.dir,"/",this.risk,"/paf/draws/",output.version, "/",
