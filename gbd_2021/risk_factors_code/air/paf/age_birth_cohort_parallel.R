...
dir.create(file.path(agedcohorts_dir, version, joint_distr_type, "/FILEPATH/"), showWarnings = F, recursive = T)
dir.create(file.path(agedcohorts_dir, version, joint_distr_type, "/FILEPATH/"), showWarnings = F, recursive = T)
...
collapse_microdata_to_prevalence <- function(microdata){
  ...
  ### ADD ASSERT STATEMENT ABOUT exposure summing to 1
  
  #lapply(ga_bw_prevalence.list, function(x) { sum(x$exposure)})
...
vet_age_cohort <- function(data){
  ...
  if (num_inconsistencies != 0){
    ...
    write.csv(vet_file, paste0(agedcohorts_dir, "/", version, "/", joint_distr_type, "/FILEPATH", gsub(jointdistr_fp, pattern = ".rds", replacement = ""), ".csv"), row.names = F, na = "")
