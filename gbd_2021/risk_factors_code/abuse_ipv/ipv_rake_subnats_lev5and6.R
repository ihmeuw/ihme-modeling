...
#Arguments
if (interactive()) {
  version <- 'super_regional_data_range_phi0.9'
  run_id <- 186296
  l <- 25318
} else {
...
file.list <- paste0(root, "exposure/ipv/xwalk/05_holt_data/forecasted_draws/", run_id, '/', version, '/', c(as.character(unique(locs[parent_id==l]$location_id))), '.csv')
...
lvl4 <- fread(paste0(root, "exposure/ipv/xwalk/05_holt_data/raked_draws/", run_id, '/', version, '/', l, '.csv'))
...
dir.create(paste0(root,"exposure/ipv/xwalk/05_holt_data/raked_draws/",run_id,"/", version, "/"))
