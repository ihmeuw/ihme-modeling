...
# read in dataset
water.cw.orig <- rbindlist(lapply(c("FILEPATH/cw_water_2.csv", # LBD dataset
...
# [8/17/20] there are two countries, Saudi Arabia & Solomon Islands, that have *cw measures that need to be split but are not in the water.cw dataset
...
sani_reports <- wash_reports[risk == "t_type", .(nid, underlying_nid, file_path, study, location_id, ihme_loc_id, location_name, year_start, year_end,
                                                 risk, measure, val, sample_size, is_outlier, cv_HH, underlying_field_citation_value)]
# looks like every NID has two entries for measure == latrine_unimp. need to clean
...
sani.cw.reg[reg == "oceania", reg := "se_asia"] # Oceania has only 3 countries with 1 source each, so i'm going to include in SE Asia since they are both in the same GBD super region
