...
education_run_id <- 146882
rei_id <- 363
me_id <- 20949
cause_ids <- c(568, 571)
location <- args[1]
years <- c(1990:2022)
ages <- c(6:20, 30:32, 235) # ages younger than 5 are have 0 exposure and are manually added at the end
gbd_round <- 7
decomp <- 'iterative'
...
rr_draws <- fread(paste0(j_root, "/FILEPATH/rr_mrt_brt_GBD2020.csv"))
rr_draws[, `:=` (rr = exp(rr), mean_rr = exp(mean_rr))]

### Load persistence estimates from meta-regression ####
persistence_draws <- fread(paste0(j_root, "/FILEPATH/persistence_draws.csv"))
...
#### Pull Exposure ####
if(me_id == 18740){
  exposure <- interpolate(gbd_id_type = "modelable_entity_id", gbd_id = me_id, measure_id = 18, source = "epi", location_id = location, reporting_year_start=min(years), reporting_year_end=max(years), age_group_id = ages, sex_id = c(1,2), status = "best", decomp_step=decomp, gbd_round_id = gbd_round)
} else {
  exposure <- get_draws(gbd_id_type = "modelable_entity_id", gbd_id = me_id, measure_id = 18, source = "epi", location_id = location, year_id = years, age_group_id = ages, sex_id = c(1,2), status = "best", decomp_step=decomp, gbd_round_id = gbd_round)
}
...
population <- get_population(age_group_id = ages, location_id=location, year_id=c(1950:2022), sex_id=c(1, 2), decomp_step = decomp, gbd_round_id = gbd_round)
...
in_school <- fread(paste0("/FILEPATH/", education_run_id,"/draws_temp_0/", location, ".csv"))
for(y in c(1920:1949)){
  in_school_temp <- in_school[year_id==1950,]
  in_school_temp[, year_id:=y]
  in_school <- rbind(in_school, in_school_temp)
}
in_school[,location_id := as.numeric(location_id)] # To resolve error on cluster where location_id becomes character
