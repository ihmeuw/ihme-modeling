...
attributable_risk <- function(d, a, exposure, relative_risk, l=0, u=100){
  ...
  attribute <- exposure$current_drinkers * integrate(attribute_f, lower=l, upper=u)$value
...
construct_tmrel <- function(d, a, tmrel, relative_risk){
  
  rr <- relative_risk[draw == d & age_group_id == a,]
  t <- construct_rr(rr$tmrel, rr)
  return(t)
