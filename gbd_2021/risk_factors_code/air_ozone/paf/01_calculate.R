...
yearWrapper <- function(this.year) {
  ...
  exp[exp$ozone <= 0, "ozone"] <- 0.1 # Set ozone values of 0 or smaller to be 0.1
  
  # For countries with 0 population, set all pop to be 0.1
  ...
  #extrapolated years use mean and variance from extrapolation
  if(this.year %in% c(2018,2019,2020)){
...
# sample draws of the ozone exposure using predefined SD
...
#----FORMAT/SAVE-----------------------------------------------------------------------------------------------------------------
# Set up variables
# we estimate one cause/age group for ozone
