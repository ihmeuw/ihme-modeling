...
if(run_map){
  ...
  # Some countries are too small to pick up any grids. To correct for this,
...
# predict out for each gridcell for 2018, 2019, 2020 using average trend from last 10 years
...
keep_years <-  setdiff(c(years,extrap_years),c(1990:1999,2001:2004,2006:2009,2021:2020))

pop <- pop[, c("longitude","latitude","pop_1990","pop_1995",
               paste0("pop",keep_years,"v4r10")),
           with=F]

setnames(pop,c("latitude","longitude",paste0("pop",keep_years,"v4r10")),c("Latitude","Longitude",paste0("pop_",keep_years)))
