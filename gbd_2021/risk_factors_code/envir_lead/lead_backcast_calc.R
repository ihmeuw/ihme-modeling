...
coeff.dir <- file.path(j_root, "WORK/05_risk/risks/envir_lead/data/rr")  # where the draws of the blood to bone lead conversion factor is
setwd(coeff.dir)
...
draw <- arg[1]
run_id <- arg[2]
output_version <- arg[3]
draws_required <- as.numeric(arg[4])
threads <- as.numeric(arg[5])
...
in.dir <- file.path("/share/epi/risk/temp/envir_lead/products/gpr_draws",run_id)
blood_out <- file.path("/share/epi/risk/temp/envir_lead/products/blood_backcast",output_version,run_id,"output_by_draw")
dir.create(blood_out,recursive = T,showWarnings = FALSE)
bone_out <- file.path("/share/epi/risk/temp/envir_lead/products/bone_backcast",output_version,run_id,"output_by_draw")
...
# oldtime[,data := 2]
oldtime[, data := 0.016] # TMREL is 0.016 ug/dL, as per Flegal & Smith 1992 (Lead Levels in Preindustrial Humans)
...
linear <- ddply(df[year_id == 1970],c("location_id","sex_id","age_group_id"),summarise,data = seq(0.016,data,(data-0.016)/(1970-1920))[2:50]) %>% data.table # this relies on the plyr package; if dplyr is loaded after plyr, it will mask 'summarise' and it won't work
linear$year_id <- NA
...
dt[age_group_id %in% c(2,3,388,389), babymean := sum(data) , by=c("location_id","year_id","sex_id")]
dt[age_group_id == 388, data := babymean]
dt[age_group_id == 389, data := babymean]
...
for (loc in unique(output$location_id)){
  bone <- output[location_id == loc & age_group_id %in% c(10:20,30:32,235), -c("iq_exp"), with=F]
