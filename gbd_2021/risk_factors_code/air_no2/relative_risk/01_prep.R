...
#------------------Prep RR data-------------------------------------------------
data <- read.xlsx(file.path("FILEPATH/extraction_NO2_GBD2020.xlsm"),startRow=3, colNames=F, skipEmptyRows = T, skipEmptyCols = F, sheet = 1) %>% as.data.table
names(data) <- read.xlsx(paste0("FILEPATH/extraction_NO2_GBD2020.xlsm"), sheet="extraction", colNames=T, skipEmptyCols = F, rows=1) %>% names

# remove observations flagged for exclusion
data <- data[is_outlier!=1]

# drop observations with an undefined NO2_conc_increment
...
# estimate median if missing (is there a better way to do this than just using the mean?)
...
##### cv_duration_fup #####
# for cross-sectional studies, we count them as <5 years fup
...
data[,cv_exp_prenatal:=cv_postnatal] # we already have a covariate for prenatal exposure (fix the name here)
...
# remove "current" from the exposure windows covariates to make it the default
