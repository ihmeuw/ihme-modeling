...
divide_data <- function(xwalked, round.ages = T){
  ...
  #' Only keep above 10 year olds in data. Drop all children from the dataset.
...
save_stgpr_train <- function(data, save.path){
  ...
  col.stgpr <- c("nid","location_id","year_id","age_group_id","sex_id","data","variance","sample_size","measure","is_outlier")
...
get_stgpr_draws <- function(run_id, location_ids=0){
  ...
  if(location_ids==0){
    ...
    for(i in location_ids){
      prev_files_chew <- paste0(file, i,".csv")
      prev_files_chew <- fread(paste0(prev_files_chew))
      setDT(prev_files_chew)
      data <- rbindlist(list(data, prev_files_chew), use.names = TRUE)
    }
  }
  data
}
...
split_points <- function(agg.test, run_id, p.sample = "reflect"){
  ...
  if (region){
    ...
    message("Finished pulling draws")
    ...
    drawvars <- paste0("draw_",0:99)
    new_drawvars <- paste0("draw_smk_",0:99)
    # get draws of the number of smokers in each age, location, year, and sex
    ...
    # now we have number of smokers in each region, year, location, age
    st.draw[,(new_drawvars) := lapply(.SD, function(x) sum(x)), .SDcols=new_drawvars, by=c("region_id", "age_group_id", "sex_id")]
    # sum population by region
    #st.draw[,region_pop := sum(population),by=c("region_id", "age_group_id", "sex_id")]
    # then divide to get new prevalence draws for each age, sex, year, and region:
    st.draw[,(drawvars) := lapply(.SD, function(x) x/sum(population)), .SDcols=new_drawvars, by=c("region_id", "age_group_id", "sex_id")]
    ...
    message("Regional/time period draws finished!")
