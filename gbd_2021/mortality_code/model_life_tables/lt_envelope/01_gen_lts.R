...
## Parse arguments
  ...
  parser$add_argument("--loc", type="character", required=TRUE,
                      help="location to run (ihme_loc_id)")
...
## Get parents
  if(interactive()){
    lt_run_id <- "VERSION"
    current_ihme <- "LOCATION"
    gbd_year <- "YEAR"
    end_year <- "YEAR"

    spectrum_name <- mortdb::get_external_input_map("mlt life table estimate", lt_run_id)[external_input_name == "hiv", external_input_version]
    parents <- mortdb::get_proc_lineage("mlt life table", "estimate", run_id = lt_run_id, lineage_type = "parent", gbd_year = gbd_year)
    vers_45q15 <- parents[parent_process_name == "45q15 estimate", parent_run_id]
    vers_5q0 <- parents[parent_process_name == "5q0 estimate", parent_run_id]
    vers_age_sex <- parents[parent_process_name == "age sex estimate", parent_run_id]
    vers_u5_env <- parents[parent_process_name == "u5 envelope estimate", parent_run_id]
    vers_lt_empir <- parents[parent_process_name == "life table empirical data", parent_run_id]
  }

## Set primary working directory
  data_dir  <- paste0("FILEPATH",lt_run_id)
  input_dir <- paste0(data_dir, "/inputs")
  out_dir   <- paste0("FILEPATH",lt_run_id)
...
## source process-specific functions
  devtools::load_all(paste0(code_dir, "/mltgeneration"))
...
## All input files
  ...
  lt_match_map    <- fread(paste0(input_dir, "/lt_match_map.csv")) ## all values = 100, number of ELTs to include in standard LT
...
# Because of the impact of HIV on age-pattern of mortality, we treat ZAF different from non-ZAF.
# non-ZAF: calculate and adjust standard lt in HIV-free space, later iterate to add HIV back
# ZAF: calculate and adjust standard lt in with-HIV space but only including ZAF ELTs
# These toggles are used to help operationalize this distinction in this script.
# See HUB page documentation for a lengthier description.
...
## Child and adult sims
  ...
  if(gbd_year > 2019){
    u5ages <- c("enn", "lnn", "pna", "pnb", 1:4)
    env_output_ages <- c("enn", "lnn", "pna", "pnb",
                         as.character(c(1, 2, seq(5, 95, 5))))
  } else {
    u5ages <- c("enn", "lnn", "pnn", 1:4)
    env_output_ages <- c("enn", "lnn", "pnn",
...
## 5q0 and 45q15 sims, Adult HIV CDR
    ...
    ## Import data
    child_sims <- fread(paste0(input_age_sex_dir, "/draws/", current_location_id, ".csv"),
    ...
    ## Output these failures to flat files for diagnostics (don't break the code, but warn strongly)
    ...
    entry_sims <- merge(adult_sims, child_sims, by=c("ihme_loc_id", "year", "sex", "simulation"))
    setnames(entry_sims, c("simulation", "v45q15", "noshock_q5"), c("sim", "entry_45q15", "entry_5q0"))
...
## Child HIV Crude Death Rate
  ...
  assert_ids(child_hiv, list(ihme_loc_id = current_ihme, year = run_years))
...
## Import LT Database
  ...
  empir_lts <- select_lt_empirical(master_lts = master_lts,
                                   lt_type = lt_option,
                                   hiv_cdr = hiv_draws,
                                   location = current_ihme,
                                   loc_map = loc_map)

  ## Generate empirical qx values long by age
  empir_lts[, lx0 := as.double(lx0)] # all vals are 1 (default to integer)
...
## Under-5 envelope results
  ...
  death_cols <- paste0("deaths", u5ages)
...
## Columns: ihme_loc_id, sex, ctry_adult_scalar, ctry_child_scalar
  hiv_country_cdr_scalars <- fread(paste0(input_dir, "/hiv_cdr_scalars.csv"))
  hiv_country_cdr_scalars <- hiv_country_cdr_scalars[ihme_loc_id == current_ihme]
  assertthat::assert_that(nrow(hiv_country_cdr_scalars) > 0)
  assert_ids(hiv_country_cdr_scalars, list(sex = sexes))
...
## This represents an upper bound of HIV-to-all-cause deaths that HIV should encompass
  ...
  if(!current_ihme %in% unique(hiv_death_prop_range$ihme_loc_id)){
    stop(paste0("hiv_deaths_prop_range_update.csv does not contain ", current_ihme))
  }
...
## No-HIV Country List
## This list contains all countries for which we assume 0 HIV -- not square by any measure
...
## HIV Age-specific Relative Risks (age 40 as reference category)
    ...
    spectrum_name <- gsub("_hiv_rr", "", spectrum_name) # just in case spectrum name is specified with suffix
    rr_filepath <- paste0("FILEPATH", name, '_s1_hiv_rr')
    hiv_rrs <- fread(paste0(rr_filepath, "/locations/", current_ihme, "_hiv_rr.csv"))
    hiv_rrs[age == 40, hivrr := 1] # Age 40 is the reference age group for relative risks
    hiv_rrs <- hiv_rrs[year <= end_year, list(year, sex, sim, age, hivrr)]
...
## TESTING 95+ fix, do not recalculate (gbd 2019)
## For future GBD, test the impact/necessity of this recalculation
  is_current_hmd <- T
...
## Calculate standard lifetable using Mahalanobis distance matching, exclusion criteria, and weighted collapsing
  ...
  # convert 5q0 and 45q15 to logit space
  hiv_free_qx[, logit_calc_45q15 := logit_qx(calc_45q15)]
  hiv_free_qx[, logit_calc_5q0 := logit_qx(calc_5q0)]

  # merge on 'lt_match_map' to indicate the number of ELTs for each standard lt by location
  hiv_free_qx <- merge(hiv_free_qx, lt_match_map, by = c("ihme_loc_id"))
  setnames(hiv_free_qx, "match", "n_match")
  
  # set key on 'lt_weights' (standard lt is a weighted mean of ELTs. this table contains the relevant weights.)
  setkey(lt_weights, sex, lag, region_cat)

  # run 'calc_stan_lt' function to calculate a standard life table for each draw
  stan_results <- mclapply(0:999, calc_stan_lt, mc.cores = 5,
                           empir_lt = empir_lts,
                           weights = lt_weights,
                           loc_map = loc_map)
  ...
  summary_weights <- entry_weights[, list(mean_weight = mean(stan_weight),
                                 ...
                                 by = c("ref_ihme", "ref_year", "ihme_loc_id", "year", "sex",
                                        "life_table_category_id", "source_type_id")]
...
## Write out standard lifetable summaries before it's adjusted based on differences
  summary_stan_qx <- copy(stan_qx)
  summary_stan_qx <- mortcore::summarize_data(summary_stan_qx,
...
## Draw-level life tables and envelopes in HDF files indexed by year
  ...
  save_lt_env <- function(data, file_prefix) {
    ...
    # use mortcore::save_hdf
    lapply(run_years, save_hdf,
                  data = data,
                  filepath = filepath,
                  by_var = "year",
                  level = 2)
...
## Summary-level csv files
  save_summary_lt_env <- function(data, data_type, file_prefix) {
    ...
    if(data_type == "envelope") {
      summary <- mortcore::summarize_data(summary,
                                          id_vars = id_vars,
                                          outcome_var = "deaths",
                                          metrics = c("mean", "lower", "upper"),
                                          na.rm = T)
    } else if(data_type == "life_table") {
      summary <- data[, list(mean_mx = mean(mx),
                             lower_mx = quantile(mx, probs = 0.025, na.rm=T),
                             upper_mx = quantile(mx, probs = 0.975, na.rm=T),
                             mean_ax = mean(ax),
                             lower_ax = quantile(ax, probs = 0.025, na.rm=T),
                             upper_ax = quantile(ax, probs = 0.975, na.rm=T),
                             mean_qx = mean(qx),
                             lower_qx = quantile(qx, probs = 0.025, na.rm=T),
                             upper_qx = quantile(qx, probs = 0.975, na.rm=T)),
                      by = id_vars]
    } else {
      stop("'data_type' must be 'envelope' or 'life_table'")
    }
