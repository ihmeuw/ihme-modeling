...
#' @param qx_target data.table with variables ihme_loc_id, year, sex, sim, target_45q15, target_5q0. Original target qx values, used as inputs into calc_stan_lt 
#' @param modelpar_sims data.table with variables sex, age, sim, sim_difflogit5, sim_difflogit45. Regression coefficients based on logit differences between standard LT and collapsed comparator LTs
...
adjust_stan_qx <- function(qx_stan, qx_target, modelpar_sims, id_vars) {
    ...
    ## Apply betas to all draws (note: modelpar_sims only goes up through age 100)
    ...
    ## Create 5q0 and 45q15 for the adjusted lifetable -- add on as a general variable
