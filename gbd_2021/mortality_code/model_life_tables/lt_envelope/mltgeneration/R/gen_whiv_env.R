...
#' @param gbd_year to be passed into agg_u5_env function (helps select age groups to split)
#'
#' @return data.table with variables: sim, ihme_loc_id, sex, year age, deaths
#'         With-HIV lifetable results for all demographics, up to age 95+ (also modifies whiv_lt in-place?)
#' @export
#'
#' @examples
#' @import data.table
...
gen_whiv_env <- function(whiv_lt, population, u5_envelope, mx_params, hmd_indic, env_ids, gbd_year) {
	...
	## Add on the under-5 envelope results
		u5_envelope <- agg_u5_env(u5_envelope, gbd_year = gbd_year)
