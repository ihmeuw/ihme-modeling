...
calc_stan_lt <- function(lt_draw, empir_lt, weights, loc_map) {
	...
	## After matching empirical LTs, we'll do ranking etc. on those id_vars
		est_id_vars_nosex <- c("est_ihme_loc_id", "est_year", "est_sim")
		est_id_vars <- c(est_id_vars_nosex, "sex")
	...
	## Pull out metadata associated with the given LT observation
		...
		lt_parent <- loc_map[ihme_loc_id == lt_country, substr(parent_ihme, 1, 3)] # for subnational locs
	...
	## Sum distance for males and females, by est_id_vars minus sex
		empir_lt[, distance := sum(distance), by = c("ihme_loc_id", "year", "source_type_id",
		                                             "est_year", "est_ihme_loc_id")]
		
	## If only male or only female, multiply distance by 2 to put on same scale as pairs
		empir_lt[, n_sexes := .N, by = c("ihme_loc_id", "year", "source_type_id",
		                                 "est_year", "est_ihme_loc_id")]
		empir_lt[n_sexes == 1, distance := distance * 2]
		
	## Drop if only male or only female and lag > 5
		empir_lt[, abs_lag := abs(year - est_year)]
		empir_lt <- empir_lt[!(n_sexes == 1 & abs_lag > 5)]
		empir_lt[, n_sexes := NULL]
		
	## For category 6 (location-year specfic) LTs, in years where they are present,
	## remove other life tables for that year and remove cat 6 LTs from other years
	
	# remove location year_specific life tables from est_years they are not from
	empir_lt <- empir_lt[(life_table_category_id == 6 & est_year == year) | (life_table_category_id != 6)]
	
	# determine which years have cat 6 tables
	cat_6_years <- unique(empir_lt[life_table_category_id == 6, est_year, by = sex])
	
	# merge and delete all other life tables from years where cat 6 tables are present
	empir_lt[cat_6_years, is_year_specific := TRUE, on = .(sex, est_year)]
	empir_lt <- empir_lt[((is_year_specific == TRUE) & (life_table_category_id == 6)) | is.na(is_year_specific)]
	empir_lt[, is_year_specific := NULL]
	...
	## Perform first subset: Keep n_match observations, as well as all location- or location-parent-specific observations
		...
		empir_lt[, dist_rank := frank(distance), by = est_id_vars]
		assertthat::are_equal(min(empir_lt$dist_rank), 1) # catch any wonkiness with frank
	...
	## Collapse observation weights by age/sex/country etc. (re-scaled to 1)
    	...
    	empir_lt <- lx_to_qx_wide(empir_lt, keep_lx = F, assert_na = F) # lx for older ages is NA so assert_na = F
	...
	## Final assertions
		# We only need age-specific qx through age 80 because 85+ granular is recalculated using recalc_85plus_qx
