...
#'
#' Format empirical life tables appropriately for processing in main lifetable process. Throw it wide by lx* and empir variables, and carry out assertions on values.
#'
...
download_lt_empirical <- function(run_id = "best", gbd_year) {

  age_map <- setDT(get_age_map(type = "lifetable", gbd_year = gbd_year))[, .(age_group_id, age_group_years_start)]

  empir_lts <- get_mort_outputs("life table empirical", "data",
                                life_table_parameter_id = c(3, 4),
                                run_id = run_id,
                                gbd_year = gbd_year)
  empir_lts <- empir_lts[outlier_type_id == 1] ## Not outliered data

  summary_5q0_45q15 <- get_mort_outputs("life table empirical", "data",
                                        life_table_parameter_id = 3,
                                        age_group_ids = c(1, 199),
                                        run_id = run_id,
                                        gbd_year = gbd_year)
  ...
  empir_lts <- merge(empir_lts, summary_5q0_45q15, by = id_vars, all.x = T)
