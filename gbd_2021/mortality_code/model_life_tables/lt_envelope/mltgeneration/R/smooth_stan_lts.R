...
#'
#' To adjust for high-variance standard life table results over time, smooth the output qx values over years (but within id_vars such as age, country, sex, etc.)
#'
...
smooth_stan_lts <- function(stan_qx, id_vars) {
    ...
    collapse_stan_lts <- function(ref_year, dt, id_vars) {
        dt <- copy(dt)
        # If the first year (generally 1950), use 10 years' worth of data; otherwise, use 6 years on either end (13 total years, except for early years that may use fewer)
