...
#'  * ZAF: South Africa and all subnational locations. Will only use ZAF-specific lifetable. Note: not used as of GBD2016 -- switched to using with-HIV ZAF lifetables
...
import_lt_empirical <- function(lt_type = "", hiv_cdr, location, loc_map) {
  ...
  if(lt_type != "USA") {  
    ## Drop lifetables from high-HIV country/year/sex results -- only if it's in the universal LT database
  ...
  ## Initialize the lx_na columns if they don't already exist (USA and ZAF)
