...
#' 
#' @param master_lts data.table with the variables ihme_loc_id, year, sex, source_type_id, life_table_category_id, lx* (0, 1, 5 [5] 110), empir_5q0, empir_45q15, empir_logit_5q0, empir_logit_45q15
...
#'  * ZAF: South Africa and all subnational locations. Will only use ZAF-specific lifetables, currently coming from location-specific life_table_category_id = 2.
...
select_lt_empirical <- function(master_lts, lt_type = "", hiv_cdr, location, loc_map) {
  ...
  if(lt_type == "location-specific") empir_lts <- master_lts[life_table_category_id %in% c(2, 6) & ihme_loc_id == location]
  if(lt_type == "ZAF") empir_lts <- master_lts[life_table_category_id %in% c(2, 6) & grepl("ZAF", ihme_loc_id)] ## ZAF-category LTs are 3, but now ZAF locations use with-HIV lifetables in the location-specific lifetable db file
  if(lt_type == "USA") empir_lts <- master_lts[life_table_category_id == 4]

  ## Drop lifetables from high-HIV country/year/sex results -- only if it's coming from the universal LT database
  ## Adopt a less restrictive bar for country-specific locations
  ...
  ## Double check ZAF LTs are dropped for non-ZAF locations
  if(lt_type != "ZAF") master_lts <- master_lts[!(ihme_loc_id %like% "ZAF")]
  ...
  ## Drop if both male and female ELTs are not included for a location-year-source, unless from this location
  ## This is to facilitate selection in pairs by sex in standard life table generation
  empir_lts[, n_sexes := .N, by = c("ihme_loc_id", "year", "source_type_id")]
  empir_lts <- empir_lts[n_sexes == 2 | ihme_loc_id == location]
  empir_lts[, n_sexes := NULL]
  ...
  ## Merge on region and super region IDs, along with parent_ihme values here
