...
#'                    GBD2015 MortCoD Appendix: Part 3.2 in 3.3.A
...
gen_stage_1_life_table <- function(iter_qx, mx_110, betas_qx_85, ax_params) {
  
  ## if qx>=1 for age 95, 100, or 105, set to 0.999
  iter_qx[age %in% c(95, 100, 105) & qx >= 1, qx := 0.999]
	...
	## Also print diagnostics to try to figure out why jobs will randomly error out with Error in shift(lx, 1, type = "lead") : Unsupported type 'NULL'
	## Calls: gen_stage_1_life_table -> lx_to_dx -> [ -> [.data.table -> shift -> .Call
		print(iter_qx[1:10])
		qx_to_lx(iter_qx)
		print(iter_qx[1:10])
		lx_to_dx(iter_qx)
	...
	## Initialize ax values
		...
		iter_qx <- gen_80plus_ax(iter_qx, ax_params) # Returns the dataset here rather than doing in-line
		setkeyv(iter_qx, id_vars)
		
	## Create an age_length variable -- capturing the length in years of the age group
		gen_age_length(iter_qx)

	## Ages 10-105: Adjust mx, nLx, ax, mx, and nLx (again) 
		...
		# This is also weird -- why is it only age <= 100 rather than <= 105?
	...
	## Ages 0-10: Using k1 values and lx values, recalculate nLx and then rollw that calculation through the rest of life-table parameters
