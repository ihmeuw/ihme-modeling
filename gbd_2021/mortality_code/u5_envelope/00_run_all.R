...
if (interactive()) {
  ...
  with_shocks <- ""
  code_dir <- "FILEPATH"
  births_draws_version <- x
  version_age_sex_id <- x
...
template_u5 <- task_template(
  ...
  command_template = paste(
    ...
    "{end_year} {location_id} {ihme_loc_id} {version_id}",
...
template_summarizing_u5 <- task_template(
  ...
  command_template = paste(
    "PYTHONPATH= PATH=FILEPATH:$PATH OMP_NUM_THREADS=10",
    "{script_path} {code_path} {conda_path} {conda_env}",
...
tasks_u5 <- lapply(1:nrow(location_hierarchy), function(i) {
  ...
  task_u5 <- task(
    ...
    upstream_tasks = list(),
    max_attempts = 2,
    code_path = glue::glue("{code_dir}/01_u5.py"),
    conda_path = conda_path,
    conda_env = conda_env,
    script_path = glue::glue("{code_dir}/01_u5.sh"),
...
tasks_scaling_u5 <- lapply(1:nrow(parent_locations), function(i) {
  ...
  task_scaling_u5 <- task(
    ...
    script_path = glue::glue("{code_dir}/02_u5_scaling.R"),
    version_id = version_id,
    location_id = as.integer(location_hierarchy[i]$location_id),
    gbd_year = gbd_year,
    compute_resources = list(
...
tasks_summarizing_u5 <- lapply(1:nrow(location_hierarchy), function(i) {
  ...
  task_summarizing_u5 <- task(
    ...
    max_attempts = 2,
    code_path = glue::glue("{code_dir}/03_generate_summary_files.py"),
    conda_path = conda_path,
    conda_env = conda_env,
    script_path = glue::glue("{code_dir}/03_generate_summary_files.sh"),
    version_id = version_id,
    location_id = as.integer(location_hierarchy[i]$location_id),
    compute_resources = list(
      "memory" = "100G",
...
if (wfr == "D") {
  ...
  mortdb::send_slack_message(
    message = paste0("U5 envelope ", version_id, " failed."),
    channel = paste0("@", username),
    icon = ":thinking_face:",
    botname = "U5EnvelopeBot"
  )
