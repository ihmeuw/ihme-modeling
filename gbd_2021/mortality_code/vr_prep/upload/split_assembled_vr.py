...
NID_MAP_PATH = "".format(NEW_RUN_ID)
...
def remove_overlapping_ages(vr_data):
    ...
    df = df.groupby(["location_id", "sex_id", "year_id",
                     "age_group_days_start", "source_type_id"]).apply(mark_dups_to_keep)
    df = df[df.keep == 1]
    ...
    vr_data[vr_data.age_group_id != 22].duplicated(
        subset=['age_group_days_start', 'year_id', 'sex_id', 'location_id', 'source_type_id'],
        keep=False).sum()
...
def drop_missing_terminal_ages(df):
    ...
    groupby_cols = ["location_id", "sex_id", "year_id", "source_type_id"]
...
def age_binning(df, age_column):
    ...
    # make age_start_binned and age_end_binned numeric, for some reason pd.cut makes them
    # categoricals
...
def collapse_small_age_groups(df):
    ...
    good_age_group_ids = get_demographics(gbd_team='cod', gbd_round_id=GBD_ROUND_ID)['age_group_id']
...
def re_collapse(df):
    ...
    variables.  It filles nulls in underlying_nid before collapse to avoid
...
def drop_splits(df):
    ...
    age_1_to_4 = df.age_group_id == 5  # 5 is 1 to 4
...
def cast_agesex_split_wide(df):
    ...
    output = pd.pivot_table(df, index=['iso3', 'location_id', 'year', 'sex', 'source', 'NID', 'underlying_nid', 'source_type_id'], columns='age_group_id', values='deaths').reset_index()
...
def save_outputs(df, output_file):
    ...
    for col in ['sex', 'year', 'location_id', 'source_type_id']:
...
def prep_population_input(vr_input_object, is_cod_vr):
    ...
    pop = pd.read_csv("".format(NEW_RUN_ID))
    ...
    if is_cod_vr != "TRUE":
        ...
        location_metadata = get_location_metadata(location_set_id=82, gbd_round_id=GBD_ROUND_ID)
...
def main(input_vr_file, input_distribution_file, is_cod_vr, out_dir):
    ...
    if is_cod_vr == "TRUE":
        vr_data = vr_data[["sex_id","nid","year_id","age_group_id","location_id","source","underlying_nid","deaths","source_type_id"]]
    ...
    asp_dist = asp.AgeSexDistribution(
        distribution_data=pd.read_csv(input_distribution_file),
        index_columns=["location_id", "year_id", "sex_id", "age_group_id"],
    ...
    # do not split terminal age group for IND SRS     
    split.loc[(split.source.str.contains("SRS")) & (split.age_group_id_aggregate == 160), "age_group_id"] = 160

    # need to drop these columns. Kept because they were useful for vetting
...
if __name__ == "__main__":
    ...
    print("WITH_SHOCK is set to {}".format(IS_COD_VR))
    if IS_COD_VR == "TRUE":
        if WITH_SHOCK == "TRUE":
            in_filename = ""
        else:
    ...
    # Filepath needs to be updated MANUALLY
    input_distribution_file = ("".format(NEW_RUN_ID))

    # set up output locations
    input_vr_file = ("".format(NEW_RUN_ID, in_filename))
    out_dir = (""
               .format(NEW_RUN_ID))
    out_file = "".format(out_dir, out_filename)
