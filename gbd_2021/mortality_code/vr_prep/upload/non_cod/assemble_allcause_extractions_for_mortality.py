...
Claude system. The data contained there is all cause vr data that was
prepared during GBD 2017 (and beyond).
"""
...
def get_all_cause_vr(location_set_version_id):
    ...
    # Force rerun should be True to make sure that the latest results are
    # pulled, instead of pulling from a cached potentially out of date copy
    all_cause_filters = {'extract_type_id': [167], 'is_mort_active': True,
                         'force_rerun': True, 'block_rerun': False, 'project_id':14}
...
def aggregate_all_ages_groups(df):
    ...
    NOTE to future D(E)As: I'm not sure this function is needed.  I believe we
    may drop age group id 22 later. If that is the case, then this function is
    harmless to keep, as long as it doesn't cause problems.
    """

    # if a source only has age_group_id 22, we don't want to make another age_group_id 22
    # That would lead to doubling death counts.
    # so separate out those sources
    ...
    only_has_age_group_id_22_df = df[df.only_has_age_group_id_22 == 1].copy()  # keep for later
    ...
    df = df[df.age_group_id != 22] # don't want to re aggregate all ages that are already present. That would lead to doubling death counts.
...
def hotfix_drop_sources(df):
    """
    This function drops two sources: Italy_VR_all_cause in 2015
    and Poland_VR_all_cause in 2016
    """
...
if __name__ == "__main__":
    ...
    # rely on default arg of gbd round id
    location_metadata = get_location_metadata(location_set_id=35, gbd_round_id=GBD_ROUND_ID)
