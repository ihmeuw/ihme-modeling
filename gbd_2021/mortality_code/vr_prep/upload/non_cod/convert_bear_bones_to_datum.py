...
def _test_contiguous_age_group(ac):
    ...
    def is_contiguous(age_group):
        ...
        Returns boolean
        """
        # Wonder if we can use agetree for this
        ...
        if set([238, 34, 5]) < set(age_group):
            ...
            age_group.remove(5)
        if 1 in age_group:
            # A couple exceptions to handle, go from most granular to least
            ...
            if set([2,3,388,389]) < set(age_group):
                if (set([238, 34]) < set(age_group) | 5 in age_group):
                    age_group.remove(1)
            # 2) ENN, LNN, PNN present, same logic
...
def format_for_reshape(ac):
    ...
    pre = ac.shape[0]
    # Lebanon VR was formatted incorrectly
    ...
    # 2 problematic sources with overlapping <1 age groups, remove duplicate group for now
...
def main(ac, input_folder, output_folder, save=True):
    ...
    # Use 70plus, that's the standard one. 70+years is deprecated
...
if __name__ == '__main__':
    ...
    ac = pd.read_csv("".format(output_folder))

    irn = pd.read_csv("".format(input_folder))

    # insert China data because it became necessary to bypass CoD functions
    # because CoD functions want to aggregate up to "China" but we want
    # "China without hong kong and macao"

    # China CDC 1979 - 2016
    include_chn_cdc = True
    if include_chn_cdc:
        china_cdc = pd.read_csv("")
        # add columns
    ...
    nc.to_csv("".format(output_folder), index=False)
