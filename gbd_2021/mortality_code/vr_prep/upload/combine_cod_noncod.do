...
** ***********************************************************************
    ...
    ** For 2016, aggregate Scotland, Wales, England from COD VR with N Ireland from all cause VR
	...
	preserve
		...
		** aggregating set missing DATUM values to 0, so set them back to missing
...
** ***********************************************************************
	...
	** drop new MNG points from COD
	replace outlier = 1 if CO == "MNG" & VR_SOURCE == "WHO_causesofdeath" & !inlist(YEAR, 1994, 2010, 2019, 2020)
...
** ***********************************************************************
	...
	** Cyprus will be used for all rather than Cyprus death scalars
	...
	** Drop MDA until 1996 as data is representative of all Moldova (MDA plus Transnistria) before then
...
** We want ZAF data analyzed separately before and after 2002
	...
	replace SUBDIV = "VR" if VR_SOURCE == "stats_south_africa" & COUNTRY == "ZAF" // Adjusted VR source?
...
** ***********************************************************************
	...
	** Additional drops to get consistency
	replace outlier = 1 if COUNTRY == "IRN" & VR_SOURCE == "WHO_causesofdeath" & YEAR == 2001
...
** ***********************************************************************	
	...
	** competing sources in MDA year 2016; prefer the all cause source
	replace outlier = 1 if ihme_loc_id == "MDA" & year == 2016 & deaths_source == "WHO_causesofdeath"

	** competing sources in Greece year 2015; prefer the other source Greece_ICD10
	replace outlier = 1 if ihme_loc_id == "GRC" & year == 2015 & deaths_source != "WHO_causesofdeath"

	// outlier NLD 2016 all cause source because it doesn't have deaths for under-1 and 95+ in raw data
	replace outlier = 1 if ihme_loc_id == "NLD" & year == 2016 & deaths_source == "Netherlands_VR_all_cause"

	** competing sources in TUR year 2014-2016; prefer the all cause source
	replace outlier = 1 if ihme_loc_id == "TUR" & (year == 2014 | year == 2015 | year == 2016) & deaths_source == "WHO_causesofdeath"

	** competing sources in SYC year 2015; prefer the all cause source
	replace outlier = 1 if ihme_loc_id == "SYC" & year == 2015 & deaths_source == "WHO_causesofdeath"

	** competing sources in BRA subnationals in 2016; prefer the all cause source
	...
	** competing sources in USA (national) in 2010, prefer WHO_causesofdeath to keep with the rest of the time trend
	replace outlier = 1 if ihme_loc_id == "USA" & year == 2010 & deaths_source == "WHO"

	** competing sources in USA (national) in 2015, prefer WHO_causesofdeath to keep with the rest of the time trend
...
** HONG KONG
	** 1979-2000 want WHO_causesofdeath
    ...
    // THEN replace un-outlier the stuff we want
	replace outlier = 0 if ihme_loc_id == "CHN_354" & deaths_source == "WHO_causesofdeath" & year >= 1979 & year <= 2000

	** 2001 and onwards want everything outliered EXCEPT the WHO source (which currently has NID 287600, 
	// FIRST outlier everything for hong kong for the time period
...
** MACAO
	...
	** Use non-cod CHL 2019 data rather than prelim 2020 for 2019 
    replace outlier = 1 if ihme_loc_id == "CHL" & year == 2019 & deaths_source == "WHO_causesofdeath"
	...
	** Nat Capital Region pre-1980 VR
...
drop dup
		...
		** add nids from sourcing
		...
		// use original nid/underlying nid if there's deaths_underlying_nid_orig available
...
// 1. Unknown/all age and unknown sex deaths are split to pna/pnb and not pnn. Same with 1-4 and the new 1, 2-4 ages
