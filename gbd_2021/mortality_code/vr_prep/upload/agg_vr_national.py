...
def aggregate_to_country_level(orig_df, nid_map, gbd_round_id, release_id):
    ...
    output_cols = ["age_group_id", "deaths", "location_id", "nid",
                   ...
                   "source_type_id", "is_agg"]

    # Copy just the subnationals to a separate dataframe
    locs_df = get_current_location_hierarchy(location_set_id=82, location_set_version_id=975, release_id=release_id)  
    ...
    def agg_children(parent):
        ...
        while agg_level > national_level:
            ...
            children_to_agg["subnat_count"] = children_to_agg.groupby(by=["year_id", "source_type_id"])['location_id'].transform("nunique")
            ...
            children_to_agg = children_to_agg.groupby(["age_group_id", "source_type_id","sex_id", "year_id", "parent_id"], as_index=False).agg({"deaths": sum, "nid": safe_agg, "source": safe_agg})
    ...
    duplicate_source.to_csv("".format(new_run_id), index = False)
...
if __name__ == '__main__':
    ...
    input_data.underlying_nid = input_data.underlying_nid.replace(np.nan, 99999) 
    nid_map = pd.read_csv("").format(new_run_id)

    output_data = aggregate_to_country_level(input_data, nid_map)
    output_data.underlying_nid = output_data.underlying_nid.replace(99999, np.nan)
