# Purpose: 1) Prep sex ratios to create males/females for VR in which we only have both sex data
...
# Pull with shock with-HIV GBD2016 envelope
envelope <- get_mort_outputs(model_name = "with shock death number", model_type = "estimate", gbd_year = gbd_year)
...
# Merge on both_sex_only indicators
both_sex_only_indicators <- fread(paste0("FILEPATH"))
if (nrow(both_sex_only_indicators) > 0) {
	...
	# Need to expand these to single-year proportionally
	...
	write_csv(final_sex_ratios, paste0("FILEPATH"))
}
