...
if (!interactive()) {
  ...
  version_id <- x
  pop_version_id <- x
  population_estimate_version_id <- x
  gbd_year <- x
  code_dir <- "FILEPATH"
...
source(paste0(code_dir, "/compute_scalars_function.R"))
...
# Run population prep
# for cases we want to run with non-best gbd2017 inputs
if((gbd_year==2017) & (pop_version_id!="best")){
  
  population <- get_mort_outputs(
    model_name = "population empirical", 
    model_type = "data", 
    location_set_id = 82, 
    run_id = pop_version_id, 
    gbd_year = prev_gbd_year
  )
...
population <- population[outlier == 0, list(
  location_id, year = year_id, 
  precise_year, 
  sex_id, 
  age_group_id, 
  source_type_id, 
  pop_source = detailed_source, 
  pop_nid = nid, 
  underlying_pop_nid = underlying_nid, 
  mean
)]
