...
if (`pop_count' > 0 & `death_count' > 0) { 
	...
	* Everything except for DSP gets paired with censuses. All non-VR sources that haven't been scaled to population-representativeness would have been dropped in d02_reshaped_deaths
	...
	** 'dsp_use_ddm' is an variable that indicates whethe we want to use it for DDM estiamtion. This is because we are treating DSP pop registry and only using in 5-year increments.
	...
	// first save blank file and exit if no rows qualify
	count if (year2_num - year1_num == 1) & (year2 - year1 <= 20)
	if r(N) == 0 {
		drop _all
		save "`saveas'", emptyok replace // save blank file to pass validations
	}
	else {
		...
		forvalues j = 0/100 {
			...
			// This interpolates forwards from the beg year if VR is closer to that, and backwards from the end year if VR is closer to that.
				// Although should it really matter? Since either way, it's basically drawing a straight line between the two. Two ways to get the same thing. To do: make a test dataset and test the two approaches, see if they are different at all.
