...
** reshape_deaths, popdata("pop_allages_reshaped") data("deaths_allages") saveas("deaths_allages_reshaped") iso3("all")
** reshape_deaths, popdata("pop_allages_reshaped") data("deaths_allages") saveas("deaths_allages_USA_reshaped") iso3("USA")
**
********************************************************
** Set up Stata
...
local test = 0
if `test' == 1 {
	local popdata "FILEPATH"
	local data "FILEPATH"
	local iso3 ""
	local saveas "FILEPATH"
...
if (`deaths_count' != 0){
	** For DDM estimation, do not use non-VR and non-CENSUS sources that haven't been scaled to population-representativeness
...
qui if (`pop_count' != 0 & `deaths_count' != 0) { 
	...
	** Keep census, pop registry, and select DSP data to be used in DDM. All other source types will be brought in at c10 to be used as denominators for calculating mortality rates. (everything should be VR or scaled from survey denominator-space to gbd pop denominator-space)
	** 'dsp_use_ddm' is a variable that indicates whether we want to use it for DDM estiamtion. This is because we are treating DSP pop registry and only using in 5-year increments.
	keep if source_type == "CENSUS" | source_type == "pop_registry" | (source_type == "DSP" & dsp_use_ddm == 1)
	replace ihme_loc_id = ihme_loc_id + "&&" + sex + "@@" + source_type
	...
	foreach c of local ihme_loc_id {
		...
		foreach cy of local c_years {
			...
			if(_N == 0) {
				...
				* Check if there are death years that have too few distinct age groups. 
				* If there are VR years that have too few distinct age groups, mixed with ones that have enough distinct age groups for GGB, the this function will collapse to find the lowest common age group among the data.
				* This will prevent DDM from being able to select proper age trims to get GGB estimates.
				* To address this, we drop VR years that have too few age groups IF there are other VR years in between the census years that have the necessary number of age groups
				* If the entire collection of VR years have too few distinct age groups, we'll leave as is because SEG estimates aren't as reliant on number of distinct age groups.
				...
				* Drop if there are less than 11 distinct age groups and there are more than two different numbers of distinct age groups
				* Drop if less than 11 & there are more than one number of age group counts and at least one of those is 11 or more
