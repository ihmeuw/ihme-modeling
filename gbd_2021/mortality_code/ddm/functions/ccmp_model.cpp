...
Type objective_function<Type>::operator() () {
  ...
  for (int y = 0; y < n_periods; y++) {
    for (int a = 0; a < n_ages; a++) {
      ...
      ax_female(a, y) = 0.5;
      ax_male(a, y) = 0.5;
      // assign ax for under1 age group according to Table 3.3 on page 48 of Preston Demography Book
      if (a == 0) {
  ...
  // Type abs_diff_logit_c_female = (logit_c_female - input_logit_c_female);
  // Type abs_diff_logit_c_male = (logit_c_male - input_logit_c_male);
  // PARALLEL_REGION nll -= dnorm(abs_diff_logit_c_female, Type(0), fixed_sigma_c, true);
  // PARALLEL_REGION nll -= dnorm(abs_diff_logit_c_male, Type(0), fixed_sigma_c, true);
  ...
  // PARALLEL_REGION nll -= dnorm(vector<Type>(abs_diff_logit_c_female), Type(0), fixed_sigma_c, true).sum();
  // PARALLEL_REGION nll -= dnorm(vector<Type>(abs_diff_logit_c_male), Type(0), fixed_sigma_c, true).sum();
