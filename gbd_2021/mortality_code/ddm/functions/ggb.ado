...
/*
forvalues j = 0/`maxnum' {
	replace c1_`j' = 1 if c1_`j' == 0
	replace c2_`j' = 1 if c2_`j' == 0
	replace vr_`j' = 1 if vr_`j' == 0
}
*/
...
forvalues i = 1/`maxnum' {
	...
	qui {
		...
		// Condition 1: Is the width between age groups larger than or equal to time between censuses (before) or less than (after)
		// Condition 2a: For back, is the width between the age groups smaller than or equal to the width between the agegroup and the open interval (since if it's less than that, we want to truncate our lower bound to match)
		// Condition 2b: For ahead, is the width between the age groups smaller than or equal to the width between the agegroup and the bottom?
		forvalues j = 0/`iminus' {
			...
			// If the space between the upper age limit and age is the limiting factor, go until the first observation (therefore, the largest spacing) that qualifies to hit that mark.
...
** ********** LHS and RHS match that in Equation 4, Hill et al. Page 238.
