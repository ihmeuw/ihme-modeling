...
format_value_matrix <- function(data, use_age_int, use_terminal_age, id_vars, year_start, baseline = F) {
  ...
  data[age_group_years_end == age_group_table_terminal_age, age_middle_floor := floor((use_terminal_age - age_group_years_start) / 2)]
...
agg_age_data <- function(data, id_vars, value_vars = "mean", age_grouping_var = NULL) {
  ...
  # if not loaded, source `mortcore` for aggregation
  if (!"mortcore" %in% (.packages())) {
    library(mortcore)
  }
  ...
  agg_data <- lapply(1:nrow(unique_age_groupings), function(i) {
    ...
    same_age_grouping_data <- mortcore::agg_results(
      same_age_grouping_data,
      id_vars = c(id_vars, "sex_id", "age_group_id"),
      value_vars = value_vars,
      age_aggs = target_age_groups$age_group_id,
      agg_hierarchy = F,
      id_assertion = F
    )
