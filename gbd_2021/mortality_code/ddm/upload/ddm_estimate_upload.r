...
parser$add_argument('--code_dir', type='character', required=TRUE,
                    help='directory used to run code')
...
d08_data_filepath <- paste0(main_dir, "data/d08_smoothed_completeness.dta")
d08_data_filepath_old <- paste0("FILEPATH/d08_smoothed_completeness.dta")

d00_data_filepath <- paste0(main_dir, "data/d00_compiled_deaths.dta")
...
est[grepl("VR", source) & is.na(source_type_id), source_type_id := 1] # Anything that is VR and doesn't have a source type gets assigned to source_type_id = 1 (VR)
...
upload_results(filepath = paste0(main_dir, "upload/ddm_estimates.csv"),
               model_name = "ddm",
               model_type = "estimate",
