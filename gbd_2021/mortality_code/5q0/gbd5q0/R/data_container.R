...
DataContainer <- setRefClass("DataContainer",
  ...
  methods = list(
    initialize = function(gbd_round_id = NULL, start_year = 1950,
                          end_year = 2019, output_dir = NULL,
                          version_id = NULL,
    ...
    get = function(key) {
      # Make sure location data is in data_cache
      if (!('location' %in% names(data_cache))) {
        ...
        location_data <- get_location_metadata(location_set_id = 82, gbd_round_id = .self$gbd_round_id)
        ...
        if (key %in% names(file_paths)) {
          if (file.exists(.self$file_paths[[key]])) {
            ...
            if (key == 'population') {
              ...
              data <- get_covariate_estimates(covariate_id = 57, model_version_id  = .self$external_inputs[['ldi_pc']],gbd_round_id = .self$gbd_round_id, decomp_step = "iterative")
              ...
              data <- get_covariate_estimates(covariate_id = 463, model_version_id = .self$external_inputs[['maternal_edu']],gbd_round_id = .self$gbd_round_id, decomp_step = "iterative")
              ...
              data <- get_covariate_estimates(covariate_id = 214, model_version_id = .self$external_inputs[['hiv_child_cdr']], gbd_round_id = .self$gbd_round_id, decomp_step = "iterative")
    ...
    get_submodel = function(submodel_id, key) {
      file_path <- paste0(.self$output_dir, "/submodel/", submodel_id,
                          .self$submodel_file_paths[[key]])
    ...
    save_submodel = function(data, submodel_id, key) {
      if (key %in% names(submodel_file_paths)) {
        dir.create(paste0(.self$output_dir, "FILEPATH"),
                   showWarnings = FALSE)
        dir.create(paste0(.self$output_dir, "FILEPATH"),
                   showWarnings = FALSE)
        file_path <- paste0(.self$output_dir, "FILEPATH",
                            .self$submodel_file_paths[[key]])
        if (key == 'stage_1_model') {
          return(file_path)
        } else {
