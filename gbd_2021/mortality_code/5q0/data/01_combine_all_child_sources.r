...
if (Sys.info()[1] == 'Windows') {
  ...
  j <<- "FILEPATH"
  h <<- Sys.getenv("FILEPATH") #repo directory
...
gbr_replacement <- fread("FILEPATH")

data <- data[!(grepl("GBR", ihme.loc.id) & t %in% c(2018, 2020))]

data <- rbind(data, gbr_replacement)
