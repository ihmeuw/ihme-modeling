...
data <- fread(paste0("FILEPATH"), colClasses = list("character" = "source.date"))
data[, dupvar := 0]
...
#If source date is not an integer, it is NA
...
#Find source date in source name if source date and nid are missing
...
#If source date is a year range take the last year
...
#########################################################           other      VR             ######################
# correct NIDs to whats in the prepped data 
...
add.nids <-fread(paste0("FILEPATH"))
...
#Bring in missing India VR NIDs
add.nids <- fread(paste0("FILEPATH"))
...
#Bring in missing Brazil subnats VR NIDs
add.nids <- fread(paste0("FILEPATH"))
...
#Bring in rest of the missing VR NIDs
add.nids <- fread(paste0("FILEPATH"))
...
#remaining VR
add.nids <- setDT(read.dta(paste0("FILEPATH")))
...
########################################################## done with VR ################################################################
###These Census were given wrong NID in the prepping
...
#FLS without sourcedate
merged[ source == 'idn fls' & is.na(nid), nid := 264956 ]

#Bring in missing SUSENAS NIDs
...
# If there any any underlying_NIDs from original CaC, copy them over to the new underlying_nid column
# Standardize "NA" rows
merged[underlying_NID == ".", underlying_NID := NA]
merged[underlying_NID == "", underlying_NID := NA]

# Replace values from CaC nid when the nid wasn't assigned (and one exists in the CaC data)
merged[is.na(nid) & !is.na(nid_from_c_all_child), nid := nid_from_c_all_child]
# Replace values from CaC underlying_NID when the underlying_nid wasn't assigned (and one exists in the CaC data)
...
write_csv(merged, paste0("FILEPATH"))
