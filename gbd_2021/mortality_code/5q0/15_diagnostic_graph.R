...
# 1. 5q0 scatter plot compared with the most recent run and the best 
# version of previous gbd year
...
compare_id <- get_proc_version("5q0", "estimate", gbd_year = gbd_year, run_id = "best")
...
pc_to_label = 0.005 # top x% changes to label
...
merge_covariates <- function(cov_id) {
  ...
  output[, bin_comp := findInterval(pct_diff_comp, brks)] # NOTE: bins become 1, 2, ... 5 (corresponding to above comment)
...
readr::write_csv(ldi, "FILEPATH")
readr::write_csv(maternal, "FILEPATH")
readr::write_csv(hiv, "FILEPATH")
