...
pre_gbd_year <- get_gbd_year( gbd_round_id-1)
...
send_slack <- T # if you want to send message to slack channel once graphs are done
...
upload_estimate_file <- "FILEPATH"
spacetime_output_file <- "FILEPATH"
spacetime_hyperparameters_file <- "FILEPATH"


# Comparison file paths
comparison_dir <- "FILEPATH"
comparison_stage_1_2_file <- "FILEPATH"
comparison_gpr_file <- "FILEPATH"
old_spacetime_hyperparameters_file <- "FILEPATH"
unraked_prevround <- "FILEPATH"

# Comparison file paths
comparison_dir2 <- "FILEPATH"
comparison_stage_1_2_file2 <-"FILEPATH"
comparison_gpr_file2 <- "FILEPATH"
old_spacetime_hyperparameters_file2 <- "FILEPATH"
unraked_prev <- "FILEPATH"
...
mexonly <- F
plot_outliers <- T
comp.other.version <- T # plot gbd 2017
plot.unrake <- T
location.plot <- T
...
if(comp.other.version){
  # Read in comparison GPR2 i.e. gbd 2017
...
# Get UNICEF data (always pull bests)
...
plot_data <- function(d,d3, prerake, compare.gpr = NULL,compare.gpr2 = NULL, level = 3,compare.gpr.unicef=NULL, compare.gpr.wpp=NULL, dorig = NULL,renums=NULL,compare.old = NULL,compare.old.stages = NULL, source_comparison = NULL, outliers = NULL, urban = NULL) {
  ...
  if(!is.null(compare.old)){
    ...
    if(comp.other.version){
      legendtext = c(legendtext, "GPR 2017")
      legendcol = c(legendcol, "coral")
      legendlty = c(legendlty,1)
    }
    
    if(plot.unrake){
      legendtext = c(legendtext, paste0("Pre-rake ", pre_gbd_year),paste0("Pre-rake ",gbd_year))
      legendcol = c(legendcol, "cyan","cyan")
      legendlty = c(legendlty, 1,4)
    }
    
    legendtext = c(legendtext,"UNICEF", "WPP", "Stage 1 Current", paste0("Stage 1 GBD ",pre_gbd_year),"Stage 2 Current: Space Time", paste0("Stage 2 GBD ",pre_gbd_year),
                   "VR/SRS/DSP", "CBH", "SBH", "HH", "Transparent points are", "original data points", "")
    legendcol = c(legendcol,"purple", "gold", "green","green", "blue", "blue",
                  "black", "black", "black", "black", NA, NA, NA)
    legendlty = c(legendlty, 1,1,2,1,2,rep(NA,7))
    if (!is.null(urban)) { # If plotting India urban/rural
...
for (ss in start_year) {
  ...
  for (rr in unique(d$gbd_region)) {
    ...
    for (cc in unique(d$ihme_loc_id[d$gbd_region==rr])) {
      ...
      output_graph_file <- paste0("FILEPATH", ss, "_", cc, "_v",
                                  version_id, ".pdf")
...
#send slack messsage
if(send_slack){
  message <- paste0("5q0 Run ", version_id, " graphs are ready: ", "FILEPATH")
  send_slack_message(message,channel = "#mortality", icon = ":earth_asia:", botname = "5q0Bot")
}
