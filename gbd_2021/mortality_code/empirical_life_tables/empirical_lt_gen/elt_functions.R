...
# get life table data from database with some formatting
# ==================================================================================
get_life_table <- function(version='best', parameter_ids=c(3), smooth_widths=c(1), gbd_year = 2019){
  message(paste0(Sys.time()," : getting life tables from database"))
  lt <- get_mort_outputs(model_name = "life table empirical", 
                         model_type = "data", 
                         run_id = version,
...
ids_to_names <- function(lt, age=T, sex=T, outlier_type=T, parameter=T, category=T){
  ...
  if(age==T){
    age_map <- get_age_map(type = "lifetable")
  ...
  if(category==T){
    lt_category_names <- get_mort_ids(type = "life_table_category")
...
# add plot numbers by unique combinations of source, location, year, sex, smooth_width
# ================================================================================
...
# plot compiled pdfs separated by outlier status, one life table per page
# =================================================================================
...
# merge plot number map and shiny table output for decisions
# =============================================================
...
# save all smoothing levels to a smooth directory for machine vision
# ===================================================================
...
# =============================================
smoothing_plots <- function(version_id, num_cores = 1){
  ...
  if(version_id == "custom"){
    ...
    lt <- mortdb::ids_to_names(lt)

    lt[, c("start_date", "end_date") := NULL]
    setnames(lt,'age_group_years_start','age_start')
  ...
  # Drop outliered lifetables
  lt <- lt[smooth_to_use != 0, ]
  ...
  # Tests for proper `plot_num` sequence:
  # max(unique(diff(lt$plot_num))) < 2
  # length(unique(lt[, .(.N), by = .(plot_num)])) == 1
  n <- max(lt$plot_num)
  n_groups <- floor(n/1000)

  cols <- c("blue", "green", "red")
  names(cols) <- c("universal", "country specific", "outlier")
  y_limits <- c(-10, 0)
  y_breaks <- c(-10, -5, -2.5, -1, -.5)

  if(version_id == "custom"){
    dir <- "/homes/krpaul/eltplots"
  } else {
    dir <- paste0("FILEPATH", "/diagnostics")
  }
  dir.create(dir)
  ...
  # plot
  .make_plot <- function(lt, start_i, n, dir) {
    ...
    for(i in start_i:end_i){
      ...
      subtitle_dt <- setorder(
        unique(lt_plot[, c("smooth_width", "outlier_type")]),
        smooth_width
      )

      subtitle_text <- paste0("sw ", subtitle_dt$smooth_width, ": ", subtitle_dt$outlier_type, collapse = "\n")
  ...
  parallel::mclapply(
    seq(0, n_groups * 1000, 1000),
    function(x) .make_plot(lt = lt, start_i = x, n = n, dir = dir),
    mc.cores = min(num_cores, n_groups)
  )
...
save_for_shiny <- function(){
  # read in all decisions
  files <- list.files("FILEPATH", pattern='elt_predictions', full.names=T)
  ...
  write.csv(lt, "FILEPATH", row.names=F)
}


# workflow to outlier based on csv and save to files for mv (05/10/19)
# ======================================================================
wf_05_10_19 <- function(){
  ...
  # apply agreement KP & HW
  lt <- merge_plot_num_map(lt, "FILEPATH")
  lt <- outlier_based_on_csv(lt, "FILEPATH")
  lt[,plot_num:=NULL]
  # apply kp 4000 review
  lt <- merge_plot_num_map(lt, "FILEPATH")
  lt <- outlier_based_on_csv(lt, "FILEPATH")
  lt[,plot_num:=NULL]
}
