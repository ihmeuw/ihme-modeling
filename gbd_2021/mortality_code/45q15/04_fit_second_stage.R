...
transform <- "logit" # toggle for transformation type
...
# Amplitude:
#     (1) logit-difference between 1st and 2nd stage predictions in high-data-density loc-years
#     (2) variance in this logit-difference by location
#     (3) mean of these location-specific variances --> amplitude = mse for ALL location-years
...
# Replace mse with location-specific mse for complete vr locations to
#     reduce uncertainty (similar to fertility method)
data <- as.data.table(data)

      # Only use high-income national locations (with some exceptions - as per asfr)
      ...
      # Subset data to complete vr locations and use data from 1990+ vr sources in mse calculation (as per asfr)
...
# Done differently based on "transform" toggle at the top
...
# Use MAD estimator of census, household, and sibs data to add nonsampling variance
