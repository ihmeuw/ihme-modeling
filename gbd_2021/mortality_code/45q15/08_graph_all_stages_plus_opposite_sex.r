...
current_file <- list(est=paste0("FILEPATH"),
                     pred=paste0("FILEPATH"),
                     data=raw)

# UN WPP 2022 
unpop <- fread("FILEPATH")
unpop <- unpop[, .(country_ihme_loc_id, date_start, sex_id, value)]
unpop <- unpop[, list(ihme_loc_id = country_ihme_loc_id, year_id = date_start + 0.5, sex_id, value, value_raw = value * 1000)]
unpop <- unpop[sex_id %in% 1:2]
unpop[, sex := ifelse(sex_id == 1, "male", "female")]
...
prev_gbd_path <- paste0("FILEPATH/", prev_gbd_45q15_est_version)

prev_gbd_extension <- ".csv"
if(prev_gbd_year < 2019) {
  prev_gbd_extension <- ".txt"
}

comparison_file <- list(est=paste0("FILEPATH"),
                        pred=paste0("FILEPATH"),
                        data=raw_prev)
...
graph_order$sex <- rep(c("male","female"), nrow(graph_order)/2)
...
prep_data <- function(file1, file2, file3, start_year) {
  ...
  d2 <- as.data.frame(d2)
  d2 <- unique(d2[,c("ihme_loc_id","sex","year","LDI_id","mean_yrs_educ","hiv","type")])
  d <- merge(d1,d2)
  d$stderr <- NA
...
current_amplitude <- fread(paste0("FILEPATH"))
previous_amplitude <- fread(paste0("FILEPATH"))
current_amplitude[, mse := round(mse, 5)]
previous_amplitude[, mse := round(mse, 5)]
...
old_params <- fread(paste0("FILEPATH"))
...
params <- as.data.frame(fread(paste0("FILEPATH")))
...
fwrite(d, paste0("FILEPATH"), row.names = F)
...
plot_preds_data_compare <- function(d, c, c2 = NULL, start_year,comp_gbd_type, unpop, opp_sex) {
  ...
  plot(0,0,xlim=c(start_year, end_year),ylim=ylim,
  ...
  if(plot.unrake){
    legend("bottomleft",
           legend=c("1st stage",paste0("GBD", prev_gbd_year, " 1st stage"), "2nd stage", paste0("GBD", prev_gbd_year, " 2nd stage"), "GPR Unscaled" ,"GPR", paste0("GBD", prev_gbd_year), "Opposite sex GPR",
    ...
    legend("bottomleft",
           legend=c("1st stage",paste0("GBD", prev_gbd_year, " 1st stage"), "2nd stage", paste0("GBD", prev_gbd_year, " 2nd stage"), "GPR", paste0("GBD", prev_gbd_year), "Opposite sex GPR",
                    "DDM Complete", "DDM Adjusted", "Unadjusted Rates", "Sibs", "DSS"),
           cex=0.5,
  ...
  if(unique(d$ihme_loc_id) %in% unpop$ihme_loc_id){
    legend("top", legend=c("UN WPP 2022",names(colors)[colors %in% d$color]), fill=c("hotpink",colors[colors %in% d$color]), border=c("hotpink",colors[colors %in% d$color]), horiz=T, bg="white")
  }
  ...
  for (rr in sort(unique(graph_order$region_name))) {
    ...
    for(row in which(graph_order[,region_name] == rr)){
      ...
      if (nrow(d[ii,]) != 0) {
        ...
        amplitude <- unique(current_amplitude[ihme_loc_id == cc & sex == ss,]$mse)
        ...
        amplitude_old <- unique(previous_amplitude[ihme_loc_id == cc & sex == ss,]$mse)
        ...
        mtext(paste(gsub("_", " ", rr), "\n",
                    ...
                    "amplitude: ", amplitude, " (", amplitude_old, "), ",
                    "zeta: ", zeta, " (", zeta_old, "), ",
...
if(send_slack){
  message <- paste0("45q15 Run ", version_id, " graphs saved: ", "FILEPATH")
  send_slack_message(message,channel = "#mortality", icon = ":earth_asia:", botname = "45q15Bot")
