...
qsub(
  ...
  code = paste0(code_dir, '/09b_prep_graph_age_sex.R'),
...
write.csv(job_map, paste0("FILEPATH/graphing_job_map.csv"))

array_qsub(
  paste0('agesex_', version_id, '_graphing'),
  code = paste0(code_dir, '/09c_graph_age_sex.R'),
...
qsub(
  ...
  code = paste0(code_dir, '/09d_append_age_sex.py'),
