...
template_compile_hiv_free_ratios <- task_template(
  ...
  command_template = paste(
    "PYTHONPATH= PATH=FILEPATH:$PATH OMP_NUM_THREADS=1",
    "{script_path} {code_path} {conda_path} {conda_env}",
    "{version_id} {hiv_version}"
  ),
...
template_upload <- task_template(
  ...
  command_template = paste(
    ...
    "{shell_path} -i {image_path} -s {script_path}",
    "--version_id {version_id}",
    "--age_sex_data_version {age_sex_data_version}",
...
task_compile_hiv_free_ratios <- task(
  ...
  max_attempts = 3,
  code_path = glue::glue("{code_dir}/01_compile_hiv_free_ratios.py"),
  conda_path = conda_path,
  conda_env = conda_env,
  script_path = glue::glue("{code_dir}/01_compile_hiv_free_ratios.sh"),
  version_id = version_id,
...
task_fit_models <- task(
  ...
  shell_path = shell_path_r,
  image_path = image_path,
  script_path = glue::glue("{code_dir}/02_fit_models.R"),
  version_id = version_id,
  version_5q0_id = version_5q0_id,
  compute_resources = list(
    "memory" = "10G",
...
task_sex_stage_1 <- task(
  ...
  shell_path = shell_path_r,
  image_path = image_path,
  script_path = glue::glue("{code_dir}/03_predict_sex_model_stage1.R"),
  version_id = version_id,
  version_5q0_id = version_5q0_id,
  gbd_year = gbd_year,
  end_year = end_year,
...
task_sex_stage_2 <- task(
  ...
  shell_path = shell_path_r,
  image_path = image_path,
  script_path = glue::glue("{code_dir}/04_predict_sex_model_stage2.R"),
  version_id = version_id,
  version_5q0_id = version_5q0_id,
  version_ddm_id = version_ddm_id,
  gbd_year = gbd_year,
...
tasks_sex_gpr <- lapply(unique(location_data$ihme_loc_id), function(i) {
  ...
  task_sex_gpr <- task(
    ...
    max_attempts = 3,
    code_path = glue::glue("{code_dir}/05_predict_sex_model_gpr.py"),
    conda_path = conda_path,
    conda_env = conda_env,
    script_path = glue::glue("{code_dir}/05_predict_sex_model_gpr.sh"),
    version_id = version_id,
    ihme_loc_id = i,
    code_dir = code_dir,
    compute_resources = list(
...
tasks_age_model_1 <- lapply(c("male", "female"), function(i) {
  ...
  task_age_model_1 <- task(
    ...
    shell_path = shell_path_r,
    image_path = image_path,
    script_path = glue::glue("{code_dir}/06a_predict_age_model_stage1.R"),
    version_id = version_id,
    sex = i,
    version_5q0_id = version_5q0_id,
    version_ddm_id = version_ddm_id,
...
tasks_age_model_2 <- lapply(1:nrow(ages_and_sexes), function(i) {
  ...
  task_age_model_2 <- task(
    ...
    shell_path = shell_path_r,
    image_path = image_path,
    script_path = glue::glue("{code_dir}/06b_predict_age_model_stage2.R"),
    version_id = version_id,
...
task_age_model_format <- task(
  ...
  shell_path = shell_path_r,
  image_path = image_path,
  script_path = glue::glue("{code_dir}/06c_predict_age_model_format_outputs.R"),
  version_id = version_id,
  compute_resources = list(
    "memory" = "5G",
    "cores" = 2L,
...
tasks_age_gpr <- lapply(unique(location_data$ihme_loc_id), function(i) {
  ...
  task_age_gpr <- task(
    ...
    max_attempts = 3,
    code_path = glue::glue("{code_dir}/07_predict_age_model_gpr.py"),
    conda_path = conda_path,
    conda_env = conda_env,
    script_path = glue::glue("{code_dir}/07_predict_age_model_gpr.sh"),
    version_id = version_id,
    ihme_loc_id = i,
    code_dir = code_dir,
    compute_resources = list(
...
tasks_scale <- lapply(unique(location_data$ihme_loc_id), function(i) {
  ...
  task_scale <- task(
    ...
    shell_path = shell_path_r,
    image_path = image_path,
    script_path = glue::glue("{code_dir}/08_scale_age_sex.R"),
    version_id = version_id,
    version_5q0_id = version_5q0_id,
...
task_graph <- task(
  ...
  script_path = glue::glue("{code_dir}/09a_graph_age_sex_wrapper.R"),
  version_id = version_id,
  gbd_year = gbd_year,
  code_dir = code_dir,
  compute_resources = list(
...
if (wfr == "D") {
  ...
  mortdb::send_slack_message(
    message = paste0("Age-sex model ", version_id, " failed."),
    channel = paste0("@", username),
    icon = ":thinking_face:",
    botname = "AgeSexModelBot"
  )
