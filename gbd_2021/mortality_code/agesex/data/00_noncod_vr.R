...
# (1) compiles VR data from COD for all sexes
# (2) compiles VR data from Mortality for all sexes (and SRS and DSP)
# (3) combine VR from COD and Mortality and prioritize COD over Mortality (with some exceptions)
...
subnat_map <- fread(paste0(input_dir, "/subnat_loc_map.csv"))
...
## Get scaled IND SRS data from DDM 
...
## remove non-VR (and set aside SRS, DSP, etc.)
...
for(lev in c(6, 5, 4)){
  ...
  if(lev == 4){
    ...
    cod_subnat <- cod_subnat[level != lev | (grepl("CHN_", ihme_loc_id) & level == lev)] # prevent loss of CHN aggs
  }else{
...
# replace aggregates with sum smaller age groups to create as complete
# source as possible
...
vr_both <- copy(vr)
vr_both[, n_sexes := .N, by = byvars] # don't add both-sex if we don't have both male and female
...
# save for input to next script (01_compile)
write.csv(vr,
          paste0("FILEPATH"),
          row.names = F)
