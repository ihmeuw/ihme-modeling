...
readr::write_csv(subnat_loc_map, fs::path(input_folder, "subnat_loc_map.csv"))
...
readr::write_csv(births_cov, fs::path(input_folder, "births_cov.csv"))
...
readr::write_csv(pop, fs::path(input_folder, "gbd_population.csv"))
...
readr::write_csv(est_5q0, fs::path(input_folder, "5q0_estimates.csv"))

# Copy star data
file.copy(
  "FILEPATH/stars_by_iso3_time_window.csv",
  fs::path(input_folder, "stars_by_iso3_time_window.csv"),
  overwrite = TRUE
)
...
outliering_template <- task_template(
  ...
  command_template = paste(
    ...
    "--version_id {version_id}",
    "--ddm_version_id {ddm_version_id}",
    "--model_5q0_version_id {model_5q0_version_id}",
...
cbh_task <- task(
  ...
  script_path = fs::path(code_dir, "00_cbh.R"),
...
noncod_vr_task <- task(
  ...
  shell_path = shell_path_r,
  image_path = image_path,
  script_path = fs::path(code_dir, "00_noncod_vr.R"),
  version_id = as.integer(version_id),
...
compile_task <- task(
  ...
  shell_path = shell_path_r,
  image_path = image_path,
  script_path = fs::path(code_dir, "01_compile_agesex_data.R"),
  version_id = as.integer(version_id),
  gbd_year = as.integer(gbd_year),
  ddm_version_id = as.integer(ddm_version_id),
  compute_resources = list(
...
outliering_task <- task(
  ...
  shell_path = shell_path_r,
  image_path = image_path,
  script_path = fs::path(code_dir, "02_outliering.R"),
  version_id = as.integer(version_id),
  gbd_year = as.integer(gbd_year),
  ddm_version_id = as.integer(ddm_version_id),
...
if(wfr != "D") {
  ...
  send_slack_message(
    message = paste0("Age sex data ", version_id, " failed."),
