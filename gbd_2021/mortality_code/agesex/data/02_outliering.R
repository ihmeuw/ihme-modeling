...
# Description:
# (1) marks outliers - systematic and manual
...
# 2 = exclude from gpr
# 3 = incomplete
# 4 = implausible sex ratio
# 5 = all zero
# 6 = vr available
# 7 = too few deaths
# 8 = manual
# 9 = implausible deaths/births
# 10 = inadequate sex data
# 11 = CBH before range
# 12 = under 0.0001
...
if(interactive()){
  ...
  version_id <- x
  version_5q0_est_id <- x
  version_5q0_data_id <- x
  version_ddm_id <- x
  gbd_year <- x
  mark_best <- F
...
# remove duplicates for merging - prioritize higher ptid
...
raw_vr[, year := floor(year)] # to make years match
raw_vr[, year:= as.double(year)] # to preserve data type
...
## exclude any VR/SRS that is incomplete (calculate directly by comparing to 5q0 GPR estimates) -------
...
est5q0[, year:= as.double(year)] # to preserve data type
...
# create age-specific exclude (for cases when excluding overall i.e. non-zero exclude variable value)
...
# merge all 5q0 obs for interpolation
...
dt[, s_comp := q_u5/q5med] # used later in manual outliering
...
ddm45q15[, year := as.double(year)] # to preserve data type
...
# Uncomment for no 2020+ data run
# dt <- dt[year < 2020]
