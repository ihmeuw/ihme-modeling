...
parser$add_argument(
  "--hiv_deaths",
  action = "store_true",
  help = "Calculate and upload final HIV deaths"
)
parser$add_argument(
  "--best_hiv_deaths",
  action = "store_true",
  help = "Only if `hiv_deaths`, toggle besting of uploaded HIV deaths"
)
parser$add_argument(
  "--special_aggregates",
  action = "store_true",
...
map_proc <- c(
  ...
  "hiv adjustment estimate" = "hiv_adj",
  "mlt life table estimate" = "mlt",
  "mlt death number estimate" = "mlt_dn",
  "u5 envelope estimate" = "u5",
...
list_versions <- list(
  ...
  parent =
    ...
    )[parent_process_name %like% "^pop|^u5|^mlt", .(parent_process_name, parent_run_id)] |>
    tibble::deframe() |>
...
subdir_logs <- c(
  "partial_shocks_redistribution",
  "absorbed_shocks_hiv",
  "negative_mx_with_shock",
...
if (args$hiv_deaths) {
  # Create sex-specific directories for HIV deaths
  fs::dir_create(fs::path(dir$main, "upload", "hiv_deaths", 1:2), mode = "775")
}
...
if (args$hiv_deaths) {
  ...
  task_array_hiv_deaths <- gen_task_array_hiv_deaths(
    wf_tool = wf_tool,
    loc_ids = map_loc[is_estimate == 1, location_id],
    upstream = task_array_abr_lt
  )

  jobmonr::add_tasks(wf, task_array_hiv_deaths)
