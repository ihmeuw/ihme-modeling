...
map_age_detailed <- arrow::read_ipc_file(fs::path(dir$cache, "age_map_single_year_mdu1.arrow"))
...
dt_pop_detailed <- fs::path(dir$cache, "pop_detailed.arrow") |>
  arrow::open_dataset(format = "arrow") |>
  dplyr::filter(location_id == args$loc_id) |>
  dplyr::left_join(map_age_detailed, by = c("age_group_id", "age_start")) |>
...
dt_lt[age_start < 5, qx := demCore::mx_ax_to_qx(mx, ax, age_end - age_start)]
dt_lt[age_start >= 5, qx := demCore::mx_to_qx(mx, age_end - age_start)]
...
if (nrow(check_full_qx) > 0) {
  ...
  arrow::write_ipc_file(
    check_full_qx,
    fs::path(dir$main, "logs/qx_ax_change", args$loc_id, ext = "arrow")
  )
...
# NOTE: avoid using `demCore:lifetable()` because we don't want to use or
...
# NOTE: `abridge_lt()` can't produce multiple aggregate age groups covering
...
dt_lt[, pop := NULL]
dt_lt[map_age_detailed, age_group_id := i.age_group_id, on = .(age_start, age_end)]
stopifnot(!anyNA(dt_lt))
...
if (nrow(check_big_ax) > 0) {
  ...
  arrow::write_ipc_file(
    check_big_ax,
    fs::path(dir$main, "logs/big_abridged_ax", args$loc_id, ext = "arrow")
  )
...
if (nrow(check_bad_qx) > 0) {
  ...
  check_bad_qx[, ax_new := demCore::mx_qx_to_ax(mx, 0.99999, age_end - age_start)]

  arrow::write_ipc_file(
    check_bad_qx,
    fs::path(dir$main, "logs/qx_ax_change_abridged", args$loc_id, ext = "arrow")
  )
