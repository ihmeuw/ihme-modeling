...
map_age<- arrow::read_ipc_file(fs::path(dir$cache, "age_map_single_year_mdu1.arrow"))
...
path_canon_lt <- fs::path(dir$main, "output/most_detailed_lt") |>
...
stopifnot(
  ...
  all(sort(unique(dt_canon$age_group_id)) == sort(map_age$age_group_id))
)
...
cols_mx_male <- grep("^mx_.*_male$", names(dt_canon), value = TRUE)
cols_mx_female <- grep("^mx_.*_female$", names(dt_canon), value = TRUE)
...
dt_canon[, `:=`(
  axdeaths_male = ax_male * mx_hiv_free_male,
  axdeaths_female = ax_female * mx_hiv_free_female
)]
...
dt_agg_shocks <- dt_agg_nonscaled[
  ...
  .(
    ...
    shocks_male = deaths_with_shock_male - deaths_with_hiv_male,
    shocks_female = deaths_with_shock_female - deaths_with_hiv_female
  )
]
...
dt_agg_scaled[
  dt_agg_shocks,
  `:=`(
    deaths_with_shock_male = deaths_with_hiv_male + i.shocks_male,
    deaths_with_shock_female = deaths_with_hiv_female + i.shocks_female
  ),
  on = id_cols
]
...
if (isTRUE(args$special_aggregates)) {
  ...
  stopifnot(
    !anyNA(dt_canon_agg),
    all(sort(locs_all) == sort(unique(dt_canon_agg$location_id))),
    dt_canon_agg[, .N, by = .(location_id, age_group_id)][N != length(draw_range)] == 0
  )
...
c("deaths_hiv_free", "deaths_with_hiv", "deaths_with_shock", "axdeaths") |>
  purrr::walk(\(x) {
...
purrr::walk(c("male", "female", "all"), \(sex) {
  ...
  dt_canon_agg[, paste0("ax_", sex) := get(paste0("axdeaths_", sex)) / get(paste0("deaths_hiv_free_", sex))]
  dt_canon_agg[, paste0("axdeaths_", sex) := NULL]

  col_deaths <- grep(paste0("^deaths_.*_", sex, "$"), names(dt_canon_agg), value = TRUE)
...
dir_out <- fs::path(dir$main, "output/agg_full_lt")

dt_canon_agg |>
  split(by = "location_id", keep.by = FALSE) |>
  purrr::iwalk(\(x, y) arrow::write_ipc_file(
    x,
    fs::path(dir_out, paste0("location_id=", y), args$agg_year, ext = "arrow")
  ))
