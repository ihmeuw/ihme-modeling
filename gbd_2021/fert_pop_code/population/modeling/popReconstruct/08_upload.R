...
pop_reporting <- parallel::mclapply(location_hierarchy[, location_id], function(loc_id) {
  print(loc_id)
...
pop <- parallel::mclapply(location_hierarchy[, location_id], function(loc_id) {
  print(loc_id)
...
net_migration <- parallel::mclapply(location_hierarchy[is_estimate == 1, location_id], function(loc_id) {
  print(loc_id)
...
pop_id_vars <- list(location_id = location_hierarchy[, location_id], year_id = years,
...
pop_reporting_id_vars <- list(location_id = location_hierarchy[, location_id], year_id = years,
...
baby_diagnostics <- net_migration[age_group_id == 28 & measure_id == 18 & sex_id == 3]
baby_diagnostics <- merge(baby_diagnostics, location_hierarchy[, list(location_id, ihme_loc_id, location_name)], by = "location_id", all.x = T)
