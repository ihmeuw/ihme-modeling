...
# Drop both sex data when sex-specific is available
census_data[
  ,
  remove_bothsex := all(1:3 %in% sex_id),
  by = .(location_id, year_id, nid, source_name, outlier_type)
]

census_data <- census_data[!(sex_id == 3 & (remove_bothsex))]
census_data[, remove_bothsex := NULL]
...
readr::write_csv(census_data_age, file = paste0(output_dir, "/outputs/02_distribute_unknown_sex_age.csv"))
