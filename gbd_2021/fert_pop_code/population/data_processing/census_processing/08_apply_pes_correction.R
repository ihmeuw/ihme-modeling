...
pes_data <- pes_data[age_group_id == 22 & sex_id == 3] # TODO: make sure the age specific data has an all age aggregate included
...
# TODO: reevaluate if we should be using the gbd location sdi value for the historical location and merge these on cleaner
...
# TODO: confirm whether we want to be shifting or scaling the pes age pattern, would need to think about change in underenumeration to overenumeration that occurs at older ages
...
# don't adjust already pes-adjusted data
census_data[pes_adjustment_type == "adjusted", pes_adjust := FALSE]
...
readr::write_csv(census_data, file = paste0(output_dir, "/outputs/08_apply_pes_correction.csv"))
readr::write_csv(corrections, file = paste0(output_dir, "/outputs/08_pes_correction_variance.csv"))
