...
align_censuses <- function(censuses, hierarchy) {
  ...
  # TODO: update NID for these
  censuses[, c("underlying_nid", "data_stage") := NA]
  censuses[, nid := 375237] # TODO: update this to the actual nid for national location adjustments and not for location splitting
...
# Transnistria not included in moldova censuses
...
# Yemen split into north and south pre unification
# yem_loc_hierarchy <- gen_hierarchy(parent_ihme_loc_id = "YEM", ihme_loc_id = c("YEM_north", "YEM_south"),
...
census_data[location_id == 150 &
              record_type == "population registry",
            outlier_type := "outliered"]
...
sgp_deaths <- fread("/mnt/team/fertilitypop/pub/population/data_processing/census/SGP/res_vs_nonres_singapore_distributed_unknown.csv")
...
## To fix this processing run for SGP:
#  - "Fake" five year age group data in census years (we want to keep)
#  - Broad age group data for the total population (we want to drop after we've aggregated)
#  - All-ages data original from #2 (that we want to keep in non-census years)
...
sgp_data <- lapply(sgp_census_years, function(year) {
  ...
  scaled_census_data[, nid := 375237] # TODO: update this to the actual nid for national location adjustments and not for location splitting
...
location_split_data <- function(data, gbd_pop) {
  ...
  split_data <- lapply(census_years, function(year) {
    ...
    census_year[, nid := 375237] # TODO: update this to the actual nid for national location adjustments and not for location splitting
...
# TODO: check whether we need to do this in China for Tibet. And other subnationals
...
# Adjust NHGIS to US intercensal estimates ---------------------------------     

# At request of USCT, we want our priority data source for the USA
# to be a mid-year estimates (for 1960-2010). However, these recommended
# data sources (intercensal estimates) often had less granular age groups
# or a lower open age interval the code below allows us to use this new data
# and extend it using the age pattern of a more granular source (NHGIS)

usa_loc_hierarchy <- location_hierarchy[parent_id == 102]
usa_data <- census_data[location_id %in% usa_loc_hierarchy[, location_id] & 
                          year_id >= 1970 & year_id < 2020]

# create 5 year age bins from the single year data 
usa_data <- merge(
  usa_data, 
  age_groups[, c("age_group_id","age_group_years_start","age_group_years_end")],
  by = "age_group_id",
  all.x = TRUE
)
usa_data[age_group_years_start < 85, ':='(age_bin_start = ((age_group_years_start %/% 5) * 5),
                                          age_bin_end = (((age_group_years_start %/% 5) * 5) + 5))]
usa_data[age_group_years_start >= 85, ':='(age_bin_start = 85, age_bin_end = 125)]

# create scalars
scalars <- usa_data[source_name == "NHGIS"]
setnames(scalars, "mean", "mean_single_year")
scalars <- scalars[, .(mean_single_year = sum(mean_single_year)),
                   by = c("location_id","year_id","sex_id","age_bin_start","age_bin_end")]

scalars_5yr <- usa_data[!source_name == "NHGIS" & outlier_type == "not outliered"]
setnames(scalars_5yr, "mean", "mean_5year")
scalars_5yr <- scalars_5yr[, .(mean_5year = sum(mean_5year)),
                           by = c("location_id","year_id","sex_id","age_bin_start","age_bin_end")]
...
scalars[, scalar := mean_5year / mean_single_year]
...
usa_data[source_name == "NHGIS", mean := mean * scalar]
# TODO: Should we be outliering or dropping duplicate sources?
usa_data[!source_name == "NHGIS", outlier_type := "duplicated"]
usa_data[source_name == "NHGIS", outlier_type := "not outliered"]
usa_data <- usa_data[, -c("mean_single_year","mean_5year","scalar")]

# aggregate subnational scaled NHGIS to national
usa_mapping <- data.table(child = usa_loc_hierarchy$location_id, parent = 102)
usa_nat <- hierarchyUtils::agg(
  dt = usa_data[source_name == "NHGIS"],
  id_cols = setdiff(names(usa_data), c("mean")),
  value_cols = "mean",
...
usa_data <- rbind(usa_data, usa_nat)

# quick diagnostic
# TODO: delete or make permanent
library(ggplot2)
options(scipen=999)

usa_data[, sex_id := as.factor(sex_id)]

pdf(paste0(output_dir,"diagnostics/USA_rescaled_means.pdf"),
    width = 10, height = 20)

for (ll in unique(usa_data[source_name=="NHGIS"]$location_id)) {
  loc_name <- location_hierarchy[location_id == ll, location_name]
  
  fig <- ggplot(data = usa_data[outlier_type == "not outliered" & location_id == ll]) + 
    geom_point(aes(x = age_group_years_start, y = mean, color = sex_id)) + 
    geom_line(aes(x = age_group_years_start, y = mean, color = sex_id)) + 
    theme_bw() + 
    facet_grid(vars(year_id), scales = "free_y") +
    labs(x = "Age Start", y = "Population Mean", 
        title = paste0(loc_name, "(",ll,")"))
...
# subset
keep_vars <- c(census_id_vars,"sex_id","age_group_id","mean")
usa_data <- usa_data[, ..keep_vars]

census_data[location_id %in% c(102, usa_loc_hierarchy[, location_id]) &
              year_id >= 1970 &
              year_id < 2020,
            outlier_type := "duplicated"]


# Combine and Save --------------------------------------------------------

split_data <- rbind(deu_data, cyp_data, srb_data, mys_data, mda_data, yug_data, 
                    sgp_data, ind_data, usa_data, use.names = T)
...
readr::write_csv(census_data, file = paste0(output_dir, "/outputs/07_make_national_location_adjustments.csv"))
