...
readr::write_csv(age_groups, file = paste0(output_dir, "/inputs/age_groups.csv"))
...
readr::write_csv(gbd_pop, file = paste0(output_dir, "/inputs/gbd_pop_current_round_best.csv"))
...
setnames(gbd_pop, "population", "mean")
setkeyv(gbd_pop, c("run_id", "location_id", "year_id", "sex_id", "age_group_id"))
readr::write_csv(gbd_pop, file = paste0(output_dir, "/inputs/gbd_pop_previous_round_best.csv"))
...
readr::write_csv(comparators, file = paste0(output_dir, "/inputs/comparators.csv"))
...
census_data <- census_data[between(year_id, year_start, year_end)] # TODO: once we start using censuses before 1950, need to figure out split for the old subnationals first
setkeyv(census_data, c(census_id_vars, "sex_id", "age_group_id"))
readr::write_csv(census_data, file = paste0(output_dir, "/outputs/01_raw.csv"))
...
readr::write_csv(subnational_mappings, file = paste0(output_dir, "/inputs/subnational_mappings.csv"))


## save national mappings
national_mapping <- fread(paste0(subnational_mappings_dir, "national.csv"))
readr::write_csv(national_mapping, file = paste0(output_dir, "/inputs/national_mapping.csv"))
...
readr::write_csv(dismod_fit, file = paste0(output_dir, "/inputs/pes_dismod_age_pattern.csv"))


## pull compiled pes data
pes_data <- mortdb::get_mort_outputs(model_name = "pes", model_type = "data", run_id = pes_data_run_id)
# drop outliers
# TODO: move these outliers to the database and in pes prep. Shouldn't be hardcoded here. May need to add a column to the database
...
readr::write_csv(pes_data, file = paste0(output_dir, "/inputs/pes_data.csv"))
...
readr::write_csv(sdi_data, file = paste0(output_dir, "/inputs/sdi.csv"))
...
readr::write_csv(full_lifetables, file = paste0(output_dir, "/inputs/full_lt.csv"))
...
readr::write_csv(census_specific_settings, file = paste0(output_dir, "/inputs/census_specific_settings.csv"))
...
readr::write_csv(location_specific_settings, file = paste0(output_dir, "/inputs/location_specific_settings.csv"))
