...
##           - For rows without an nid: (SHOULD ENSURE ALL ROWS ARE ASSIGNED)
...
calculate_hyperparameters <- function(data, use_5q0_density_parameters = F, use_fertility_density_parameters = F, gbd_year, test_lambda = NA) {
  
  births <- get_covariate_estimates(covariate_id = 60, gbd_round_id = gbd_round_id, decomp_step = "iterative")
  ...
  if (use_fertility_density_parameters) {
    ...
    # these correspond to gbd 2017 hyperparameter selection
  ...
  readr::write_csv(data_density_table,"FILEPATH")
