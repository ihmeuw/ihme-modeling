...
gpr_old <- gpr_old[year_id >= year_start]

gpr_old[, stage := "GPR Final 2016"]
gpr_old[, year := year_id]

setnames(gpr_old, "sbr_mean", "sbr")
...
colors <- rev(colors)
colnames <- c("stage 1 estimates", "stage 2 estimates", "GPR Initial",
              paste0("GPR Final Current (V", version_estimate, ")"),
              paste0("GPR Final Previous (V", test_version, ")"), "GPR Final 2016")
...
## Add hyperparameters (new and old)
...
params_old <- fread("FILEPATH")
colnames(params_old) <- c("ihme_loc_id", "scale_old", "amp2x_old", "lambda_old", "zeta_old", "best_old")
...
source_list_filepath <- "FILEPATH"

if (!file.exists(source_list_filepath)) {

  source(paste0("FILEPATH", "/source_list.R"))
...
source_list <- source_list[(source_type %like% "Statistical Report" | source_type %like% "VR") &
...
if (length(unique(combined$ihme_loc_id)) > 1 & !graph_nat_est_only & !graph_ind_only) {
  ...
  pdf("FILEPATH", width = 42, height = 10)
...
for (loc in locsdf$ihme_loc_id[order(locsdf$sort_order)]) {
  ...
  p1 = ggplot() +
       ...
       geom_ribbon(data = tmp[ihme_loc_id == loc & stage == "GPR Final 2016",],
       ...
       geom_line(data = tmp[stage == "stage 1 estimates" | stage == "stage 2 estimates" |
                            stage == paste0("GPR Final Current (V", version_estimate, ")") |
                            stage == paste0("GPR Final Previous (V", test_version, ")") |
                            stage == "GPR Initial" | stage == "GPR Final 2016",],
                 aes(x = year_id, y = sbr, colour = stage), size = 1.25) +
       ...
       labs(title = paste(loc_name, "  ", loc, "  ", locs$region_name[locs$ihme_loc_id == loc], "  Stillbirth Rate (SBR) \n",
                          ...
                          "; lambda2016 =", unique(tmp$lambda_old), "; zeta2016 =",
                          unique(tmp$zeta_old), "; scale2016 =", unique(tmp$scale_old))) +
       xlab("Year") +
       ylab("SBR") +
       labs(color = " ", fill = " ", alpha = " ", shape = " ") +
  ...
  p2 = ggplot(data = tmp) +
       geom_line(data = tmp[stage == "stage 1 estimates" | stage == "stage 2 estimates" |
                            stage == paste0("GPR Final Current (V", version_estimate, ")") |
                            stage == "GPR Initial",],
                 aes(x = year_id, y = ratio, colour = stage), size = 1.5) +
  ...
  source_list_loc <- source_list_loc[order(source_list_loc$outlier, source_list_loc$std_def,
                                           source_list_loc$years),]
