...
readr::write_csv(shock_compiled, "FILEPATH")
...
if (drop_non_rep_sci_lit) {
  ...
  lit_to_outlier <- fread(paste0(data_dir, version_data, "FILEPATH"))
...
if (nrow(dup_test) > 0) readr::write_csv(dups_to_review2, "FILEPATH")
...
# Drop data points that should remain unoutliered

data_to_review <- data_to_review[!(outlier == 0 & outlier_new == 0)]

# Drop data points that should remain outliered for the same reason
