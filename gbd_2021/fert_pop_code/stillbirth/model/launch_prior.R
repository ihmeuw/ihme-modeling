...
launch_prior <- function(me, decomp_step, crosswalk_version_id = NA, path_to_data = NA, test_mods = TRUE, average = TRUE, pred_ages,
  ...
  rmse_output <- "FILEPATH" # where to save object returned by test_prior
  rmse_output_archive <- "FILEPATH" # where to save object returned by test_prior

  data_output <- "FILEPATH"
  crosswalk_data_output <- "FILEPATH"
  ...
  if (test_mods == T) {
    ...
    rmses_and_data <- test_prior(crosswalk_version_id = crosswalk_version_id, path_to_data = path_to_data, decomp_step = decomp_step,
  ...
  if (average == T) {
    ...
    ave_models <- assemble_prior(data, rmses[drop == 0,], cov_list, data_transform, pred_ages = pred_ages, me = me,
