...
test_prior <- function(crosswalk_version_id = NA, path_to_data = NA, decomp_step, cov_list, data_transform,
  ...
  central <- "FILEPATH" # path to central functions
  ...
  locs <- get_location_metadata(location_set_id = location_set_id, gbd_round_id = gbd_round_id)[, .(location_id, ihme_loc_id, super_region_name, region_name, location_name, level)]
  ...
  if (length(unique(data$age_group_id)) <= 1 & any(grepl("age_group_id", c(fixed_covs, random_effects)))) {
    ...
    stop("You specified age_group_id as a predictor, but you have less than 2 levels of this variable")
  ...
  for (sexchar in sex_list) {
    ...
    if (length(unique(data.s$age_group_id)) == 1) {
      ...
      if (unique(data.s$age_group_id) == 22 | unique(data.s$age_group_id) == 27) {
        ...
        stop("Error: look into creating a custom age group")
    ...
    ko_items <- prep_ko(data.s, location_set_id = location_set_id, by_age = by_age, by_sex = ifelse(by_sex, 1, 0), gbd_round_id = gbd_round_id)
    message("Done")
  ...
  for (sexchar in sex_list) {
    ...
    for (i in 1:max(seq$id)) {
      ...
      command <- paste0("sbatch -p all.q -t 00:15:00 --mem=", m_mem_free, "G -c 2 -A ",
                        proj, " -J ", paste0("oos_", sexchar, "_", i),
                        "FILEPATH", "linear_oos_process.R ",
                        start, " ", end, " ", sexchar, " ",  data_path, " ",
                        forms_path, " ", output_folder, " ", date, " ", data_transform, " ",
                        modtype, " ", kos)
      system(command)
