...
## read in model object (directories etc saved in previos 07a script, read in to workspace here)
...
## differentially save based on if it's going to be raked or not

if(loc %in% parents_and_childs){
  write.csv(pred_old_draws, paste0("FILEPATH"), row.names = F)
  write.csv(pred_old_sum, paste0("FILEPATH"), row.names = F)
  
  write.csv(pred_young_draws, paste0("FILEPATH"), row.names = F)
  write.csv(pred_young_sum, paste0("FILEPATH"), row.names = F)
