...
input_model_dir = "FILEPATH /" + model_version_id + "/loc_specific"

# Get the fertility location_hierarchy and subset to estimated locations
location_hierarchy = pd.read_csv("FILEPATH /" + model_version_id + "/loop1/inputs/model_locs.csv")
...
for ihme_loc_id in location_hierarchy['ihme_loc_id']:
  fp = "FILEPATH /" + model_version_id + '/loc_specific/' + ihme_loc_id + '_' + model_version_id + '.pdf'
...
m.write(input_model_dir + '/../plots_' + model_version_id + '_appended.pdf')
...
s.write(input_model_dir + '/../plots_' + model_version_id + '_' + parent + '.pdf')
