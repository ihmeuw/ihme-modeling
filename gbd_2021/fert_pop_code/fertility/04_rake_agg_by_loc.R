...
##          Aggregate subnational to national for specificied locations
###############################
...
pop <- fread(paste0("FILEPATH"))[sex_id == 2]
model_locs <- fread(paste0("FILEPATH"))

## Decide which to rake and which to aggregate
...
for(loc in unique(raked$ihme_loc_id)){
  ...
  readr::write_csv(temp, paste0("FILEPATH"))
}
