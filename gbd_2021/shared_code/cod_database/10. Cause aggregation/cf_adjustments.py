...
from cod_prep.utils import report_if_merge_fail, expand_to_u5_age_detail, CodSchema
...
class SampleSizeCauseRemover(CodProcess):
    ...
    def adjust_sample_size(self, df):
        ...
        df['sample_size_remove'] = df.groupby(demo_cols, as_index=False)['deaths_remove'].transform('sum')
...
class Raker(CodProcess):
    ...
    def cleanup(self, df):
        ...
        df = df.drop(self.sub_cols + self.agg_cols + self.death_prop_cols +
