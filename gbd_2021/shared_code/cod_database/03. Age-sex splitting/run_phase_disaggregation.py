...
from cod_prep.claude.cause_reallocation import adjust_leukemia_subtypes, adjust_dsp_liver_cancer
...
def run_pipeline(nid, extract_type_id, project_id, launch_set_id,
    ...
    source = get_value_from_nid(nid, "source", project_id, extract_type_id)
    data_type_id = get_value_from_nid(nid, "data_type_id", project_id, extract_type_id)
    iso3 = get_value_from_nid(nid, "data_type_id", project_id, extract_type_id)
    ...
    if len(df) > 0:
        ...
        # run age and age/sex remapping for unmodeled age/sex/causes
        age_remapper = AgeRemap(
        ...
        if source in ['Iran_maternal_surveillance', 'Iran_forensic', 'US_police_conflict']:
        ...
        if (iso3 == 'CHN') and (code_system_id in [40, 6]):
            print_log_message("Applying Liver cancer adjustment for China ICD9 DSP, and HongKong/Macao ICD9 detail.")
            df = adjust_dsp_liver_cancer(df, code_system_id, code_map_version_id)
...
def main(nid, extract_type_id, project_id, code_system_id, launch_set_id):
    ...
    if get_value_from_nid(nid, 'iso3', project_id=project_id, extract_type_id=extract_type_id) == 'NOR':
    ...
    assert len(df) != 0, ("Dataframe is empty."
                          ...
                          "the input database?")
    # run the pipeline
    df = run_pipeline(nid, extract_type_id, project_id, launch_set_id,
                      df, code_system_id, code_map_version_id, cause_set_version_id,
                      location_set_version_id, pop_run_id, env_run_id,
                      distribution_set_version_id, diagnostic=False)
    # upload to database
