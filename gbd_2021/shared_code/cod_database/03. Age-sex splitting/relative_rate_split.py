...
def relative_rate_split(split_df, pop_df, dist_df, detail_map_dict,
    ...
    split_df = split_df.rename(
        ...
        if x in list(split_col_renames.keys()) else x
    )
    ...
    dist_df = dist_df.rename(
        columns=lambda x: dist_names[x] if x in list(dist_names.keys()) else x
    )
    ...
    split_df = split_df.rename(
        ...
        if x in list(agg_val_names.keys()) else x
    )
    ...
    # this might happen when we only have unmodeled age/sex/causes
    # maybe for male maternal deaths; then unknown-age male-maternal deaths
    # would be lost in the calculation. So split with uninformed distribution
