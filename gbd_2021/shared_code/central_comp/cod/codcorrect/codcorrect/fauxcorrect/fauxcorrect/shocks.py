...
def append_shocks(
    ...
    if Measures.Ids.YLLS in measure_ids:
        ...
        new_scaled_ylls = _append_shocks(
            scaled_ylls, shock_ylls, version.draw_cols)
    else:
...
def _append_shocks(
    ...
    Since indirect COVID shocks can be negative, if adding shocks
    results in a negative value, set value to 0.
    """
...
def _get_input_filepaths(
    ...
    if machine_process == GBD.Process.Name.CODCORRECT:
        ...
        if not most_detailed_location:
            shocks_dir = os.path.join(
                shocks_dir, FilePaths.LOCATION_AGGREGATES)
    else:
        raise ValueError(
