...
class AggregateCauses:
    ...
    def __init__(self, parent_dir: str,
        ...
        Arguments:
            ...
            version (CodCorrectParameters): machinery parameters for the given
                 codcorrect run.
        """
    ...
    def shocks_data(self):
        if not isinstance(self._shocks_data, pd.DataFrame):
            shocks_data = io.read_unaggregated_shocks_draws(
                ...
                self.location_id,
                self.sex_id)
            self._shocks_data = shocks_data[
                Columns.INDEX + self.version.draw_cols]
    ...
    def _aggregate_rescaled(self):
        ...
        data = (
            data[Columns.INDEX + self.version.draw_cols]
    ...
    def _aggregate_shocks(self):
        ...
        data = data.loc[
            data[Columns.CAUSE_ID].isin(self.most_detailed_cause_list)]
        # Collapse draws for cause with multiple models (shock and indirect COVID)
