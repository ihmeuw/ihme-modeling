...
from fauxcorrect.queries import pred_ex as pred_ex_mod
from fauxcorrect.utils.constants import (
    Columns, FilePaths, GBD, Jobmon, Keys, Measures
)
...
def cache_pred_ex(
    ...
    """
    logging.info("Inside cache_pred_ex!")
    pred_ex_path = _get_pred_ex_path(parent_dir)
    pred_ex = pred_ex_mod.get_pred_ex(
        life_table_run_id=version.life_table_run_id,
        tmrlt_run_id=version.tmrlt_run_id,
        expected_age_group_ids=version.most_detailed_age_group_ids
    )
...
def calculate_ylls(
    ...
    ylls = _perform_calculation(
        pred_ex, draws, yll_index_cols, version.draw_cols)

    logging.info("Calculating YLLs for shocks")
    yll_shocks = _perform_calculation(
        pred_ex, shocks, yll_index_cols, version.draw_cols)
    save_map = {
    ...
    save_map[machine_process](
        ylls, yll_shocks, parent_dir, location_id, sex_id)
...
def _read_unaggregated_shock_draws(
    ...
    return read_from_draw_source(
        draw_dir,
        file_pattern,
        num_workers=task_templates.CalculateYlls.executor_parameters[Jobmon.NUM_CORES],
        filters=filters
    )
