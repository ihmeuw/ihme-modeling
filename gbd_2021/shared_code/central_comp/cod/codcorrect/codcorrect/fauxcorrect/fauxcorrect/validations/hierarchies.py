...
def validate_cause_hierarchies(
    ...
    Arguments:
        version (MachineParameters): machinery parameters for
            the given machine run.
    Raises:
        ValueError: If an aggregate cause is found in more
            than one of the cause hierarchies passed into the
            machinery parameter set.
    """
    super_set = set()
    for cause_set_id in version.aggregation_cause_set_ids:
        cause_hierarchy = version._cause_parameters[cause_set_id].hierarchy
        aggregate_causes = set(
            cause_hierarchy.loc[
                cause_hierarchy[Columns.MOST_DETAILED] != 1
            ]['cause_id'].tolist())
        overlap = super_set.intersection(aggregate_causes)
        if overlap:
            other_cause_sets = [cs for cs in version.aggregation_cause_set_ids
                                if cs != cause_set_id]
            raise ValueError(
                f"Cause_set_id {cause_set_id} contains aggregate causes {overlap}"
