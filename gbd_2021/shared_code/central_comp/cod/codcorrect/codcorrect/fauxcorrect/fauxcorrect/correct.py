...
def cache_spacetime_restrictions(
        parent_dir: str,
        gbd_round_id: int,
        decomp_step: str
) -> None:
    ...
    Arguments:
        ...
        decomp_step: decomp step of the run
    """
    restrictions_path = _get_spacetime_restrictions_path(parent_dir)
    restrictions = get_all_spacetime_restrictions(gbd_round_id, decomp_step)
    data_cols = [
...
def _read_envelope_draws(
    ...
    envelope_columns = (
        constants.Columns.DEMOGRAPHIC_INDEX + draw_cols
    )
...
def _read_envelope_summary(
    ...
    envelope_columns = (
        constants.Columns.DEMOGRAPHIC_INDEX + envelope_draw_cols
    )
...
def _read_unscaled_draws(
    ...
    try:
        draws = DrawSource({
            ...
            'num_workers': task_templates.ApplyCorrection.executor_parameters[
...
def _add_envelope_summary(df, env):
    ...
    if constants.Columns.ENVELOPE in df:
        if any(np.isnan(df[constants.Columns.ENVELOPE])):
            logging.info("Nans found in envelope column. Dropping.")
            df = df.drop(constants.Columns.ENVELOPE, axis=1)
        else:
            logging.info("Using envelope column from draws file.")
            return df
    ...
    assert merged[constants.Columns.ENVELOPE].notnull(
...
def _filter_and_expand_expected_data(
    ...
    # expected metadata contains HIV/IC/Shocks, which we don't need, so
    ...
    expanded_expected_data = (
        filtered_expected_data.explode(constants.Columns.CAUSE_AGES).rename(
            columns={
...
def _format_for_rescale(
    ...
    # filter data to just causes in the codcorrect correction hierarchy
...
def _rescale_data(data: pd.DataFrame, draw_cols: List[str]):
    ...
    for level in (range(data[constants.Columns.LEVEL].min() +
        ...
        data = _rescale_to_parent(data, level, draw_cols)
...
def _rescale_to_parent(data: pd.DataFrame, level: int, draw_cols: List[str]):
    ...
    """
    parent_keep_columns = constants.Columns.INDEX + draw_cols
    merge_columns = (
...
def _convert_to_deaths(data, envelope, draw_cols):
    ...
    envelope_data = (
        envelope[constants.Columns.DEMOGRAPHIC_INDEX + draw_cols]
    )
...
def _add_zeros_back(data, zeros, draw_cols):
    ...
    df = df[constants.Columns.INDEX + draw_cols].groupby(
