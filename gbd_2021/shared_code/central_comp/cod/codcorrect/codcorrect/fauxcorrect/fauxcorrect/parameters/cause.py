...
class CauseParameters:
    ...
    def __init__(
        ...
        Arguments:
            cause_set_id (int): informs which group of causes this parameter
                details.
            gbd_round_id (int): The GBD round ID for the run
            decomp_step (str): the specific step in GBD decomp that
    ...
    def _get_set_version_id(self) -> int:
        ...
        params: Dict[str, int] = {
            ...
            'gbd_round_id': self.gbd_round_id,
            'decomp_step_id': decomp_step.decomp_step_id_from_decomp_step(
        ...
        if results[constants.Columns.CAUSE_SET_VERSION_ID].iat[0] is None:
            raise RuntimeError(
                f"No active cause set version for cause set {self.set_id}, "
                f"GBD round {self.gbd_round_id}, {self.decomp_step}. "
                "Result was None.")
