...
from codem.joblaunch.run_utils import change_model_status, log_branch
...
def main():
    ...
    wf_1 = CODEmWorkflow(name=f'cod_{model_version_id}_cov_ko',
    ...
    try:
        ...
        if exit_status:
            logger.info(f"Input data workflow returned exit status {exit_status}, see workflow logs {wf_0.stderr}.")
    ...
    try:
        ...
        if exit_status:
            logger.info(f"Covariate selection + knockouts returned exit status {exit_status}, see workflow logs {wf_1.stderr}.")
    ...
    if exit_status:
        logger.info(f"The ensemble workflow failed, returning exit status {exit_status}, see workflow logs {wf_2.stderr}.")
        ...
        cleanup_files(model_version_id=model_version_id, db_connection=db_connection)
