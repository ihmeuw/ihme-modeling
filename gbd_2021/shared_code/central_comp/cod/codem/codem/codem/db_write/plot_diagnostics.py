...
def create_draw_plots(beta_draws, cause_name, date, model_version_id, sex, filepath):
    ...
    df = beta_draws.copy()
    title = f"{cause_name} \n Date: {date} \n Model Version ID: {model_version_id}, Sex: {sex.capitalize()}"
...
def create_covar_ndraws_plot(covariate_summary_df, cause, date, model_version_id, sex, filepath):
    """
    ...
    plt.title(f"{cause} \n Date: {date} \n Model Version ID: {model_version_id}, Sex: {sex.capitalize()}", fontsize=22)
    sns.set_style("darkgrid")
    plt.tight_layout()
    pylab.savefig(filepath)


def create_covar_submodel_plot(covariate_summary_df, cause, date, model_version_id, sex, filepath):
    """
