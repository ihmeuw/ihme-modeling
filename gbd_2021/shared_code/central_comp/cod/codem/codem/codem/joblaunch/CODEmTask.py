...
from codem.joblaunch.profiling import START_DATE, get_parameters
...
class CODEmBaseTask(BashTask):
    ...
    def model_version_metadata(self):
        ...
        if self.db_connection is not None:
            ...
            raise RuntimeError("Must pass either a connection definition to look in .odbc.ini, or db connection.")
        ...
        self.model_version_type = self.metadata_df['model_version_type'].iloc[0].lower().replace(" ", "_")
    ...
    def run_parameters(self, hybridizer, start_date, end_date, max_cores=56,
        ...
        if hybridizer:
            self.parameters['ram_gb'] = self.parameters['ram_gb'] * 1.2 + 10
            self.parameters['runtime_min'] = self.parameters['runtime_min'] * 1.2 + 60
