...
def generate_severity_split_config(gbd_round_id=GBD_ROUND_ID) -> None:
    ...
    for key in df["parent_meid"].unique():
        ...
        for out in df.loc[df["parent_meid"] == key, "child_meid"].unique():
...
def generate_super_squeeze_config() -> None:
    ...
    pre_pos_path = os.path.join(
        code_dir, "..", "..", "lib", "data", "map_pre_pos_mes.csv")
    pre_pos_df = pd.read_csv(pre_pos_path)
    pre_pos_inputs = pre_pos_df.modelable_entity_id_source.unique().tolist()

    source_target_path = os.path.join(
        code_dir, "..", "..", "lib", "data", "source_target_maps.csv")
...
def generate_maps(decomp_step: str, gbd_round_id: int) -> None:
