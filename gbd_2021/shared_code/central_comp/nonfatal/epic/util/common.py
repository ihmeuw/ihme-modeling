...
def validate_decomp_step(obj_name, decomp_step, gbd_round_id):
    ...
    # validate decomp step with Rules
    rules_manager = RulesManager(
        ResearchAreas.EPI, Tools.SAVE_RESULTS, decomp_step_id=decomp_step_id
    )
    validate_step_viewable(
        obj_name, decomp_step, rules_manager
    )
...
def pull_mvid(
    ...
    with open(json_path) as json_file:
        try:
            metadata = json.load(json_file, parse_int=True)
        except TypeError:
            with open(json_path) as json_file:
...
def compile_all_mvid(
    parent_dir: str
) -> None:
    meid_mvid_dict = {}
    # collect and read all ME json files from disk
    json_dir = os.path.join(
        parent_dir, FilePaths.INPUT_FILES_DIR
    )
    meid_files = [
        os.path.join(json_dir, file) for file in os.listdir(json_dir) if \
        file.endswith(".json")
    ]
    ...
    meid_mvid_map = pd.DataFrame.from_dict(
        data=meid_mvid_dict, orient='index',
        columns=['model_version_id']
    )
    meid_mvid_map = meid_mvid_map.reset_index().rename(columns={'index': 'modelable_entity_id'})
    output_path = os.path.join(
        parent_dir, "meid_mvid_map.csv"
    )
