...
class SaveFactory(object):
    ...
    def get_task(self, parent_dir, input_dir, input_file_pattern,
                 ...
                 gbd_round_id, decomp_step, n_draws, best, upload_summaries
    ) -> PythonTask:
        # make task
        ...
        task = PythonTask(
            ...
            args=[
                ...
                "--n_draws", str(n_draws),
                "--best", int(best),
                "--upload_summaries", int(upload_summaries)
            ],
            name=name,
            num_cores=17,
            m_mem_free="80.0G",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gbd_round_id", required=True, type=int)
    ...
    parser.add_argument("--upload_summaries", required=True, type=int)
    ...
    elif args.modelable_entity_id in MEIDS.INCIDENCE_TO_PREVALENCE:
        measure_id = [gbd.measures.PREVALENCE]
    else:
    ...
    for arg in [Params.BEST, Params.UPLOAD_SUMMARIES]:
        if vars(args)[arg] not in [0, 1]:
            raise ValueError(f"{arg} must be one of (0, 1), got {vars(args)[arg]} instead.")
    ...
    mvid_df = save_results_epi(
        ...
        mark_best=bool(args.best),
        birth_prevalence=birth_prev,
        gbd_round_id=args.gbd_round_id,
        decomp_step=args.decomp_step,
        n_draws=args.n_draws,
        upload_summaries=bool(args.upload_summaries)
    )
