...
def compile_dp_files(mvid):
    ...
    with ProcessPoolExecutor(40) as pool:
...
def write_fit_stats(mvid, outdir, joutdir) -> pd.DataFrame:
    """Writes file to outdir/model_version_fit_stat.csv
    ...
    Returns:
        ...
        https://hub.ihme.washington.edu/x/CHchAQ
    """
    log = logging.getLogger(__name__)
    ...
    if cov_df.empty:
        log.warning(
            ...
            'Note fit_stats.csv will not contain coverage information.')
    else:
