...
def get_model_version(mvid):
    ...
    query = f"""
     ...
     mv.gbd_round_id, mv.crosswalk_version_id
...
class Importer(object):
    ...
    def query_t3_dismod_data(crosswalk_version_id):
        ...
        '''
        columns_to_return = [
            'data_id', 'nid', 'location_id', 'integrand',
            'mean', 'year_start', 'year_end', 'age_start',
            'age_end', 'lower', 'upper', 'sample_size',
            'standard_error', 'sex_id', 'study_covariates',
            'age_demographer']
        rename_columns = {
            'seq': 'data_id',
            'measure': 'integrand'}
        ...
        df = rename_potential_overwrite_columns(
            df, list(rename_columns.values()))
    ...
    def correct_demographer_notation(df, gbd_round_id):
        ...
        mutation. For gbd2020 and beyond, we reduce the set of rows we
        transform to rows:
        ...
        if gbd_round_id >= 7:
            mutate_mask = (
                mutate_mask &
                (dmdf.a_data_id != IHME_COD_DB) &
                (dmdf.age_demographer != 0))
        dmdf.loc[mutate_mask, 'age_upper'] = (
            dmdf.loc[mutate_mask, 'age_upper'] + 1)
        return dmdf
    ...
    def get_t3_input_data(self, decomp_method):
        ...
        data = self.query_t3_dismod_data(
            self.model_version_meta.crosswalk_version_id.iat[0])
    ...
    def attach_country_covariate(self, covariate_id, integrand,
        ...
        if asdr_cause_id is not None:
            ...
            covdata = ln_asdr.get_lnasdr_from_db(
                int(asdr_cause_id), cc_vid, self.gbd_round_id, self.mvid
    ...
    def get_simple_prior(self):
        ...
        overrides. 'prevalence' integrand is defined as 'p_zero'.
        in this simple_prior. The rest map to xi_<<greek_letter>> as expected.
        """
...
def rename_potential_overwrite_columns(
    ...
    for overwrite_col in rename_columns:
        if overwrite_col in df.columns:
            df = df.rename(
                columns={
                    overwrite_col: avoiding_overwriting_prefix + overwrite_col
                }
            )
    return df
