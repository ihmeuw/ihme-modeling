...
def qstat_details(jids):
    ...
    for key, group in itertools.groupby(deets, group_separator):
        for line in group:
            ...
            ws = line.split(":")
            k = ws[0].strip()
            k = re.sub(r'\s*1', '', k)  # remove __1s in qstat keys
            v = ":".join(ws[1:]).strip()
            if k == 'job_number':
                v = int(v)
                jobdict[v] = {}
    ...
    Args:
        ...
        jobtype (string, optional): joint purpose argument for specifying what
            ...
            below options. default is 'python'
                ...
                'stata':
                    FILEPATH -b do run_file
                'R':
                    FILEPATH < runfile --no-save --args
                'shell':
        ...
        prepend_to_path (string, optional): Copies the current shell's
            ...
            the PATH is typically very short (FILEPATH).
