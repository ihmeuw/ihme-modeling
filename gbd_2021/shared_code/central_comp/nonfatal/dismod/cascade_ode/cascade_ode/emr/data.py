...
from cascade_ode.crosswalk import (
...
class DataMetadata(Data):
    ...
    def get_data(self, model_version_id):
        columns_to_return = [
            self._data_key, 'location_id', 'sex_id', 'year_start',
            'year_end', 'age_start', 'age_end', 'measure_id',
            'nid', 'underlying_nid', 'outlier_type_id']
        rename_columns = {
            'seq': self._data_key,
            'is_outlier': 'outlier_type_id'}

        crosswalk_version_id = crosswalk_version_id_from_model_version_id(
            model_version_id)
        df = get_crosswalk_version(crosswalk_version_id)

        # get sex_id from sex, drop both-sex data
        df = df.merge(ezfuncs.query(
            'SELECT sex, sex_id FROM shared.sex', conn_def='epi'))
        df = df.loc[df["sex_id"] != 3]

        # get measure_id
        df = df.merge(ezfuncs.query(
            'SELECT measure, measure_id FROM shared.measure', conn_def='epi'))

        # subset out terminal ages and ages with width greater than 20
        df = df.loc[
            ((df["age_end"] - df["age_start"]) < 20) |
            (df["age_start"] >= 95)]
