...
class Cascade(object):
    ...
    def __init__(
        ...
        Args:
            ...
            version(str, 'version_str'): version_str
...
class Cascade_loc(object):
    ...
    def gen_data(self, sex, drop_emr=False):
        ...
        subdata = self.subset_data_by_location_and_sex(
            data, sex, self.loclvl, self.loc, self.decomp_method, self.fix_sex())
        ...
        if self.loclvl != 'world':
            ...
            parent_data = self.prior_data_for_this_location(
                loclvl=self.loclvl, at_fix_sex_lvl=self.at_fix_sex_lvl(),
                year=self.year, sex=self.sex, fix_sex=self.fix_sex(), loc=self.loc,
    ...
    def average_data_by_integrand(self, data, sex):
        ...
        if not self.decomp_method[Methods.ENABLE_CSMR_AVERAGE]:
            ...
            # we're in step 2, use original method below
            pass
    ...
    @staticmethod
    def subset_data_by_location_and_sex(data, sex, loclvl, loc,
                                        decomp_method, fix_sex):
        """
        Subset the locations. This modifies the data DataFrame.
        """
        if loclvl != 'world':
            subdata = data[data[loclvl] == loc]
            if not decomp_method[Methods.FIX_CSMR_RE] or \
                    loclvl == 'atom':
    ...
    @staticmethod
    def prior_data_for_this_location(loclvl, at_fix_sex_lvl, year, sex, fix_sex, loc, parent_data):
    ...
    def shift_priors(data, adj_data, loc, use_median=False, ignore_small_values=False):
        ...
        for ig in integrands:
            ...
            if len(ig_prior) == 0:
                ...
                Cascade_loc.adjust_prior_by_mean_of_fit(ig_data, ig_prior, use_median=use_median, ignore_small_values=ignore_small_values)
                shifted_priors = shifted_priors.append(ig_prior)
    ...
    @staticmethod
    def adjust_prior_by_mean_of_fit(parent_predict, child_prior, use_median, ignore_small_values):
        r"""
...
def _average_data_by_integrand(
    ...
    Arguments:
        ...
        decomp_step (str): decomp_step from the keys of gbd.constants.decomp_step
...
def _pop_weight_average(df, popdf, location_id):
    """
    if data age range spans multiple age groups, assume uniform
...
def _get_age_group_span(gbd_round_id):
    ...
    """
    age_df = shared_functions.get_age_metadata(gbd_round_id=gbd_round_id)
