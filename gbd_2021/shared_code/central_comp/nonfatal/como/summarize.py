...
from como.legacy.common import get_population
...
class _FinalizeComponent:
    ...
    def __init__(self, io_mock, dimensions, population, std_age_weights, gbd_round_id):
    ...
    def _agg_pop_wtd_ages_birth(self, age_group_id):
        age_tree = agetree(age_group_id, gbd_round_id=self.gbd_round_id)
    ...
    def _agg_age_std_ages_birth(self):
        age_tree = agetree(age.AGE_STANDARDIZED, gbd_round_id=self.gbd_round_id)
...
class ComoSummaries:
    ...
    def std_age_weights(self):
        if self._std_age_weights is None:
            self._std_age_weights = get_age_weights(gbd_round_id=self.gbd_round_id)
    ...
    def _import_static_draws_by_component(self, component):
        ...
        real_source = DrawSource(
            {"draw_dir": draw_dir, "file_pattern": "{measure_id}_{year_id}_{sex_id}.h5"}
        )
