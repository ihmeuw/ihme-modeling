...
def make_file_pattern(out_dir, table_type, prefix):
    ...
    file_pattern = "{o}/draws/*/upload/*/{tt}/{p}*.csv".format(
...
def find_missing_input_files(file_df, table_type, out_dir, prefix, cols,
                             validate_path_pattern):
    """Returns a dataframe with all the missing files, assuming that it
    should be square in hyperspace.
    The 'expected' range for each index is the set of unique values in that
    column in file_df. So if an entire index value is missing then it won't
    know. It only finds missing 'holes', not missing 'slices.'
    ...
    # Use in_place=True for memory efficiency
...
def output_null_inf_count(read_dir, write_dir, table_type, prefix):
    ...
    inf_row_count = count_rows_from_file_pattern(inf_file_pattern,
                                                 inf_row_count)
    fn = os.path.join(write_dir, "null_inf_{}_{}.json".format(
        table_type, prefix))
...
class NatorUpload:
    ...
    def get_file_pattern(self, location_id):
        ...
        return (f"{self.upload_root}/draws/{location_id}"
                f"/upload/*/{self.table_type}/upload_{file_prefix}*.csv")
    ...
    def run_all_uploads_mp(self,
                           file_dict: Dict,
                           n_processes: Dict={'risk': {'single_year': 8,
                                                       'multi_year': 4},
                                              'etiology': {'single_year': 2,
                                                           'multi_year': 2},
                                              'summary': {'single_year': 6,
                                                          'multi_year': 2}
                                             }):
