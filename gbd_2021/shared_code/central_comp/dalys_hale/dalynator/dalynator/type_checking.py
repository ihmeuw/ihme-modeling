# A set of dynamic type validation functions. These generally check that a value
# is not None and of the right type, and raise an appropriate error message if the type is
# not correct.
...
def is_best_or_positive_int(var, name):
    ...
    if (var is not None):
        ...
        if isinstance(var, int) and var > 0:
            failed = False
    if failed:
        raise ValueError("{n} must be a positive int or 'best'. "
