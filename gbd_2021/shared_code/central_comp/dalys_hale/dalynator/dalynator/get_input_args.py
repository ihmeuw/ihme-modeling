...
def set_folder_permissions(path, logger):
    ...
    will throw ValueError if they are not. Sets group_id to same.
...
def construct_args_upload(parser, cli_args=None):
    ...
    log_filename = "upload_{}_{}.log".format(
...
def construct_parser_pct_change():
    ...
    parser = arg_pool.add_input_data_root(parser)
    parser = arg_pool.add_out_dir(parser)  # wasn't storing before
    parser = arg_pool.add_loc_id(parser)  # wasn't storing before here either
