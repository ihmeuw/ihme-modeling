...
from db_queries.core.age import get_age_group_set
...
class Cache:
    ...
    def _cache_regional_scalars(self) -> None:
        ...
        scalars = query(sql_q, conn_def='MORTALITY_DATABASE')
        cache_file = "{}/scalars.h5".format(
            self.cache_dir)
    ...
    def _cache_age_spans(self) -> None:
        ...
        age_group_set_id = get_age_group_set(self.gbd_round_id)
        age_spans_df = get_age_spans(age_group_set_id)
    ...
    def _get_causes_from_como_output(self) -> pd.DataFrame:
        if gbd.measures.YLD in self.measure_ids:
            epi_dir = ("{}/como/{}/summaries/cause/3/single_year/7"
