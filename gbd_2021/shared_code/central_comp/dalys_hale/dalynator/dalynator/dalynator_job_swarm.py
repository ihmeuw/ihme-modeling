...
class DalynatorJobSwarm(object):
    ...
    def __init__(
        ...
        if 'pct_change' in self.phases_to_run and start_year_ids:
        ...
        try:
            ...
            if self.do_not_execute:
                ...
                if not self.upload_to_test:
                    self._post_to_slack(self.success, 'test',
                                        null_inf_message,
    ...
    def _get_region_locs(self) -> List[int]:
        ...
        for loc_set_id in self.location_set_ids:
            regions.extend(list(get_location_metadata(
                ...
                decomp_step=self.decomp_step).query(
        ...
        for measure_id in measure_ids:
            ...
            for start_year, end_year in zip(self.start_year_ids,
                ...
                for location_id in location_ids:
                    ...
                    self.cs_upstream_filter.task_loc_map[task.hash] = location_id
                    # categorized the pct_change task according to the end year
                    # because then when it is queue behind
                    # other tasks it will be forced to wait for the prior most
                    # detailed for earlier years that it
                    # depends upon before being submitted
            ...
            for storage_engine in DalynatorJobSwarm.STORAGE_ENGINES:
                if storage_engine == "INNODB" or self.dual_upload:
                    ...
                    task = BashTask(command=("run_upload"" {params}"
                                    ...
                                    m_mem_free='40G',
                                    max_attempts=3,
                                    max_runtime_seconds=(60 * 60 * 24 * 11),
                                    queue='long.q')
