...
def best_version(input_machine, gbd_round_id, decomp_step):
    ...
    return int(ezfuncs.query(q, conn_def='GBD_DATABASE').squeeze())
...
def validate_multi_mode_years(year_n_draws_map, n_draws_years_dict,
    ...
    if len(n_draws_years_dict.keys()) == 2:
        ...
        # solution to checks checking for year set 1 and year set 2, set year
        # set 2 to an empty list to minimize conditional statements needed
        years_2 = []
...
def location_set_to_location_set_version(location_set_id=None,
    ...
    Args:
        ...
        gbd_round: year of gbd round (not the id) e.g. 2016, not 4
...
def expand_and_validate_location_lists(tool_name, location_set_ids,
    ...
    Returns:
     most_detailed_locs - the leaf locations with no children;
    ...
    for loc_set in location_set_ids:
        tree_list = hdb.loctree(
            ...
            decomp_step=decomp_step, return_many=True)
        for lt in tree_list:
