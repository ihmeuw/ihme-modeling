...
def parse_args():
    ...
    parser.add_argument('--db-env', type=str, default='prod')
...
def main(
    ...
    for location_set_id in mmr_constants.AGGREGATE_LOCATION_SET_IDS:
        try:
            ...
            logging.warn(
                f'location_set_id: {location_set_id} doesn\'t exist for gbd_round_id: '
                f'{gbd_round_id}, decomp_step: {decomp_step}')
...
def location_aggregate_birth_counts(
    ...
    logging.info(f'aggregating for location_set_id {location_set_id}')
    multiple_tree_flag = (
...
def collate_and_save_birth_counts(
        cov_estimate_filename_list: List[pathlib.PosixPath],
        constants_path: pathlib.PosixPath
):
    index_cols = ['location_id', 'year_id', 'age_group_id', 'sex_id']
...
def save_birth_count_estimates(
    ...
    assert len(df) and len(df['model_version_id'].unique()) == 1
...
if __name__ == '__main__':
    ...
    args = parse_args()
    main(args.gbd_round_id, args.decomp_step, args.process_vers)
