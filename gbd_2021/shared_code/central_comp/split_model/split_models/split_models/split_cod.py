...
@profile
def _launch_cod_splits(source_cause_id, target_cause_ids, meid_decomp_step_map,
    ...
    meid_cause_map = dict(zip(
        list(meid_decomp_step_map.keys()), target_cause_ids))
    ...
    swarm = SplitCoDSwarm(
        ...
        proportion_id_decomp_step_map=meid_decomp_step_map,
...
def split_cod_model(source_cause_id,
    ...
    Arguments:
        ...
        output_dir (str): place where you want new draws to be saved.
            Defaults to "FILEPATH/{given gbd_round}"
    ...
    """
    # Validate decomp args
    validate_decomp_step_input(decomp_step, gbd_round_id)
    if target_meid_decomp_steps:
        validate_target_meid_decomp_steps(target_meid_decomp_steps, gbd_round_id, 'cod')
    else:
        warnings.warn(
            f'No target_me_id_decomp_step argument supplied, looking '
            f'for best proportion models in same decomp_step as source '
            f'model: {decomp_step}'
        )
        target_meid_decomp_steps = decomp_step
    ...
    meid_decomp_step_map: Dict[int, str] = validate_meid_decomp_step_mapping(
        target_meids, gbd_round_id, target_meid_decomp_steps, 'cod'
    )
