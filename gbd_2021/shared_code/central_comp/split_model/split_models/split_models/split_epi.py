...
def filet(source_meid, target_prop_map, location_id, split_meas_ids,
          ...
          n_draws, downsample, proportion_meid_decomp_map):
    """
    ...
    for target_me_id in target_prop_map:
        if mvid_map is not None:
            version_id = mvid_map[target_prop_map[target_me_id]]
        else:
            version_id = None
        this_props = get_draws(
            gbd_id_type='modelable_entity_id',
            gbd_id=target_prop_map[target_me_id],
            source='epi',
            measure_id=prop_meas_id,
            location_id=location_id,
            version_id=version_id,
            gbd_round_id=gbd_round_id,
            decomp_step=proportion_meid_decomp_map[target_prop_map[target_me_id]],
            n_draws=n_draws,
...
def split_n_write(args):
    ...
    (source, targets, loc, split_meas_ids, prop_meas_id, output_dir,
     ...
     downsample, proportion_meid_decomp_map) = args
    try:
        res = filet(
            source,
            targets,
...
def launch_epi_splits(
    ...
    mvid_map = {}
    for prop in proportion_meid_decomp_map:
        try:
            mvid = best_version(
                modelable_entity_id=prop,
                gbd_round_id=gbd_round_id,
                decomp_step=proportion_meid_decomp_map[prop]
            )[0]
        except Exception as e:
            raise ValueError(
                "No best model for modelable_entity_id: {me} in gbd_round_id: "
    ...
    leaf_ids = [leaf.id for leaf in lt.leaves()]
...
def split_epi_model(
        ...
        decomp_step=None,
        output_dir=None,
        proportion_meid_decomp_steps=None,
):
    """
    ...
    if proportion_meid_decomp_steps:
        validate_target_meid_decomp_steps(proportion_meid_decomp_steps, gbd_round_id, 'epi')
    else:
        warnings.warn(
            f'No proportion_me_id_decomp_step argument supplied, looking '
            f'for best proportion models in same decomp_step as target '
            f'model: {decomp_step}'
        )
        proportion_meid_decomp_steps = decomp_step
    proportion_meid_decomp_map = validate_meid_decomp_step_mapping(
        prop_meids, gbd_round_id, proportion_meid_decomp_steps, 'epi'
    )
    ...
    if output_dir is None:
        output_dir = 'FILEPATH/{}'.format(
            gbd_round_from_gbd_round_id(gbd_round_id)
        )
