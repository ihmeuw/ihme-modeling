...
split_u1.new_ages <- function(dt, loc, run.name, gbdyear="gbd20", test_run = NULL, loc_name = loc, inherits = TRUE){
  ...
  pop <- data.table(fread(paste0("/FILEPATH/", loc_name, '.csv')))
  ...
  split.dt <- fread(paste0('FILEPATH', loc, '_under1_splits.csv'))
...
get_summary <- function(loc, run.name, paediatric = FALSE, gbdyear){
  output <- fread(paste0('FILEPATH', loc, '.csv'))
    loc_name = loc
  ...
  output.count <- melt(output, id.vars = c('age', 'sex', 'year', 'pop', 'run_num'))
    ...
    age.map <- fread(paste0('/ihme/hiv/epp_input/gbd20', '/', run.name, "FILEPATH/age_map.csv"))
  ...
  write.csv(out.dt, paste0('FILEPATH/', loc, '.csv'), row.names = F)
  return(out.dt)
}
