...
use "FILEPATH/uhc.dta", clear
keep if location_id==`location'
save `uhc', replace
	
use "FILEPATH/incidence_draws/`location'.dta", clear
	...
	*** ESTIMATE PREVALENCE FROM INCIDENCE 
	drop run_id v1 metric_id
		...
		*drop population variable here, switch to age_start instead of age_group_id
		...
		reshape wide draw_*__  uhcMean, i(age_start location_id sex_id model_id measure_id) j(year_id)
		...
		* Reshape from wide back to long *
		forvalues y = 1990 / 2018 {
			preserve
			keep age_group_id age_start location_id sex_id model_id measure_id *`y'
			rename *`y' *
			generate year_id = `y'
			tempfile y`y'
			save `y`y''
		...
		* Add cases with acute sequelae (i.e., all cases except long-term cases), assuming 6 month duration
		forvalues i = 0 / 999 {
		...
		*make sure no negative draws
        ...
        keep measure_id location_id year_id age_group_id sex_id model_id draw_* age_start age_end
		...
		*replace measure_id for prevalence
		...
		export delimited using "`interms_dir'/ADDRESS_prevalence/`location'.csv", replace
		clear
