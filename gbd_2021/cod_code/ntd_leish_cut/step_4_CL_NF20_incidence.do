...
*** ======================= BOILERPLATE ======================= ***
	...
	set more off
	set maxvar 32767
	local in_dir FILEPATH/ntd_leish/inputs
    local code_dir FILEPATH/ntd_leish/code
	...
	if "`exec_from_args'" == "True" {
		...
		local params_dir "FILEPATHparams"
...
tempfile inc draws pop ageMeta ageCross appendTemp mergeTemp pops hsa
	...
	import delimited using "FILEPATH/stgpr_draws/`location'.csv", clear
...
*** PULL THE DRAWS FROM THE INCDIENCE AGE/SEX CURVE MODEL (DISMOD USED ONLY FOR AGE-SEX SPLIT) ***  
	get_draws, gbd_id_type(model_id) gbd_id("ADDRESS") source("ADDRESS") location_id(`location') measure_id(6) age_group_id(`ages') decomp_step("ADDRESS") version_id("ADDRESS") clear
	drop measure_id model_id version_id
