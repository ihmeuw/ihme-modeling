...
# Cols to keep: "location_name", "location_id", "region_id", "year_id"
#               "total_reported", "undetected_0" - "undetected_999"  
...
all_ages <- rbind(setDT(all_ages),
              all_ages[year_id==2022,][, year_id := year_id + 1],
              all_ages[year_id==2022,][, year_id := year_id + 2],
              all_ages[year_id==2022,][, year_id := year_id + 3],
              all_ages[year_id==2022,][, year_id := year_id + 4],
              all_ages[year_id==2022,][, year_id := year_id + 5],
              all_ages[year_id==2022,][, year_id := year_id + 6],
              all_ages[year_id==2022,][, year_id := year_id + 7]
...
# Drop years
all_ages <- all_ages[year_id <= 2022]
...
## (10) Clean up
# Remove prev_ prefix from variable names
names(prev) <- sub("prev_", "", names(prev))

# Create measure id variable (5 = prevalence)
prev[, measure_id := 5]
...
nf_draws <- nf_draws[year_id %in% c(1990, 1995, 2000, 2005, 2010, 2015, 2017, 2019, 2020, 2021, 2022)]
