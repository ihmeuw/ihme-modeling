...
prev_dt <- get_outputs(topic = "cause", measure_id = ID, year_id = ds_dt[, unique(year_id)], location_id = ds_dt[, unique(location_id)],
...
graph_mrbrt_results <- function(results, predicts){
  ...
  gg_subset <- ggplot() +
    ...
    scale_size_continuous(name = "Inverse Variance") +
    labs(x = "Age") +
    ggtitle(paste0("Meta-Analysis Results for Relative Risk of Down's Syndrome")) +
    theme_classic() +
