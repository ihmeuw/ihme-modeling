...
## REMOVE INDIVIDUALS IN ADAMS (Ageing, Demographics and Memory Study) SUBSET FROM HRS
...
get_missing <- function(i){
  percent_missing_adams <- test_dt[group == "adams", sum(is.na(get(i)))/nrow(test_dt[group == "adams"])] #Adams - Ageing, Demographics and Memory study
...
epi_data[,`:=` (nid = ID, location_id = ID, location_name = "United States", year_start = 2000, year_end = 2000,
                sex = firstup(as.character(sex_id)), group = 1, specificity = "age,sex", group_review = 0, cv_marketscan = 0,
                exclude_xwalk = 0, extractor = "USER", is_outlier = 0, cv_niaaa = 0, cv_1066 = 0, cv_gp_data = 0,
