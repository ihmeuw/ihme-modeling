...
## MAPPING SUBNATIONAL TO NATIONAL LOCATIONS
  ...
  fit_graph <- function(n){
    ...
    gg <- ggplot() +
      ...
      theme(axis.title.y = element_blank(), axis.ticks.y = element_blank(), axis.line.y = element_blank(),
            axis.text.y = element_text(size = 5), axis.text.x = element_text(size = 14))
    return(gg)
  }
...
get_preds_adjustment <- function(raw_data, model){
  ...
  dt <- dt[!sum == 0]
  dt[, sum := NULL]
  pred_dt <- read.csv(paste0("FILEPATH"))
...
if (file.exists(paste0(mrbrt_dir, model_name, "FILEPATH"))){
  dem_model <- readr::read_rds(paste0(mrbrt_dir, model_name, "FILEPATH"))
} else {
  ...
  readr::write_rds(dem_model, paste0(mrbrt_dir, model_name, "FILEPATH"))
}

if (file.exists(paste0(mrbrt_dir, model_name, "FILEPATH"))){
  dem_predicts <- list(model_draws = fread(paste0(mrbrt_dir, model_name, "FILEPATH")))
} else {
