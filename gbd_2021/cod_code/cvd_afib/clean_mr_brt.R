...
source("FILEPATH") ## helper functions
...
match_mr_brt <- function(data,
  ...
  ## Make sure all of same class
  for (def in defs) data[, paste0(def) := as.numeric(paste0(def))]
  ...
  for (n in 1:ncol(comb)) {
    ...
    ## if (length(setdiff...)) is a necessary check.
    ##
