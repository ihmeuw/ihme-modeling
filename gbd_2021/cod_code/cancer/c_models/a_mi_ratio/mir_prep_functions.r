...
mir_prep.aggData <- function(input_df, var_to_aggregate, mor_type){
    ...
    if (mor_type == "CR") {
        # to carry over the seq column from the bundle version, need to maintain at 
        # least 1 seq value for rows that are being aggregated. Group by uid columns 
...
mir_prep.applyRestrictions <- function(mi_df, restriction_type, mor_type) {
    ## Applies restrictions by type, then applies restrictions by HAQ value
    ##
    print("Applying input restrictions...")
    restricted_data <- mir_prep.restrictByConfigSettings(mi_df, restriction_type, mor_type)
    restricted_data <- mir_prep.restrictByHAQ(restricted_data)
    return(restricted_data)
}


mir_prep.restrictByConfigSettings <- function(mi_df, config_type, mor_type) {
    ...
    min_cases_map$min_cases[min_cases_map$acause %in% VR_causes$acause] <- 0
...
mir_prep.restrictByHAQ <- function(mi_df){
    ...
    ##  NOTE: quartile_three variable here is actually the upper whisker of a 
...
mir_prep.generateCaps <- function(type, mir_model_version_id, input_data, pctile) {
    ...
    if (type == "upper") {
        ...
        # Apply all cause caps to correct causes based on cdb
        for(cause in unique(cause_config$acause)){
            if(cause_config[acause==cause]$use_cause_specific_upper_cap==0){
        ...
        # Make caps for age_groups with children under 20 at least 1 for all causes
        these_caps[age_group_id<9&mi_ratio<1, mi_ratio :=1]
    } else if (type == "lower") {
