...
How To Use: "python upload_to_cod.py <database_name>" 
    database_name : string - acceptable values: (testcod or prodcod)
...
from cancer_estimation.c_models.b_cod_mortality import tests as tt
...
def clear_prev_data_version_status(database, table):
    ...
    engine = get_engine(conn_def=database)
    conn = engine.connect()
...
def prep_data(uid_cols, data_version_id, db):
    ...
    # temporarily fill columns to 0 until post-age-sex 
...
def upload_to_db(table, db):
    ...
    #determine schema (uploading to cod or canncer)
...
def run_cod_upload(database='testcod'):
    ...
    df = prep_data(get_uid_cols(), data_version_id, database) 
