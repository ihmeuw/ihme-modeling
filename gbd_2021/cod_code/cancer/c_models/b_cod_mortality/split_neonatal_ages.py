...
def get_split_pairs():
    ''' Gets the split ages involved in each stage step and avoid hard coding
        throughout split script
...
def split_neo_age(dataset, wgt_df, uid_cols, metric, stage_step):
    ...
    neo_age = dataset.loc[dataset['age']
                              == list(split_pairs[stage_step].keys())[0], uids_noAge + [metric]].copy()
...
def apply_recode(df):
    ''' Apply recode to split mortality estimates (different than incidence recode),
