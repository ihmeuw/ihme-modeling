...
'''
Description: * Redistributes metric values for observations to which multiple
...
def main(dataset_id, data_type_id):
    ...
    pt.verify_metric_total(input_data, this_dataset.load_input(), metric, 
                    "Dropping unused age groups at beginning of cause disagg")

    # Format and add observation numbers
    formatted_input = prep_for_disagg(input_data.copy(), uid_cols, metric)
    # Disaggregate
    #   Note: from this point forward, the 'cause' column is no longer necessary for analysis
    ...
    pt.verify_metric_total(input_data, final_df,
                                metric, "cause disaggregation module")
    # save
    md.complete_prep_step(final_df, input_data, this_dataset)
