# -*- coding: utf-8 -*-
'''
Name of Module: age_sex_core.py
...
def add_missing_ages(df, uid_cols, metric):
    ...
    for frmat_type in ['frmat_id', 'im_frmat_id']:
        ...
        # quick fix for removing non ped ages in ped datasets 
        for frmat_id in df[frmat_type].unique():
            ...
            # cancer ages jump from 2 (0-4 yr) to 7 (5-9 yr) so this code handles that
...
def remove_wrong_ages(df):
    ...
    # using frmat map to determine missing ages to add:
    frmat_type = "frmat_id"
    this_fmt_map = pd.read_stata("FILEPATH")
    long_fmt_map = dft.wide_to_long(this_fmt_map,
                                  stubnames='frmat',
                                  i=["index"],
                                  j='age')
...
def split_age(dataset, wgt_df, metric, uid_cols, replacement_col='gbd_age'):
    ...
    standard_age_frmats = [0]  # NOTE: deprecated in near future
    standard_age_im_frmats = [1, 2, 8, 9]  # NOTE: deprecated in near future
