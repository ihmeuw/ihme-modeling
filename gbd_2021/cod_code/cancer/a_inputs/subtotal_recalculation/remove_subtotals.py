...
def set_subcauses(recalc_data, subcause_issue_file):
    ...
    # c1 is each orig_cause/code in our input data
    count = 0
    for c1 in recalc_data:
        ...
        for c2 in recalc_data:
            ...
            if (set(recalc_data[c2]['codes']) <= set(recalc_data[c1]['subcodes'])
                ...
                #recalc_data[c1]['subcauses_remaining'].append(c2)

                # NOTE: Add in c2's codes if C2 is part of C1
            ...
            # Alert of overlaps, saves overlaps and kills script
    ...
    for parent_cause in recalc_data:
        ...
        for child_cause in recalc_data:
            ...
            #   subtract the child data from the parent
                ...
                # NOTE: adding main child cause to main causes remaining
...
def convert_to_range(code_range):
    ''' Re-formats individual codes and returns them as a range of codes to
        ...
        NOTE: first and last variables become 
    ...
    if len(decimal_code_range):
        ...
        for letter in ['C', 'D']:
            ...
            # NOTE: shifted this one indent forward because it wasn't 
...
def run(dataset_id, data_type_id, uid):
    ...
    exception_file = '{}/negative_data/{}_{}_uid_{}.csv'.format(
        error_folder, dataset_name, data_type_id, uid)
    leftover_vals_file = '{}/leftover_values/{}_{}_uid_{}.txt'.format(
        error_folder, dataset_name, data_type_id, uid)
    
    # clean our exception file folders
    ...
    for i in sorted(df['orig_cause'].unique().tolist()):
        ...
        if "-" not in i and "," not in i:
            ...
            # NOTE: Setting codes to subcodes for even integer numbers
            ...
            # we want to avoid adding in orig_cause
        ...
        # NOTE: Used to keep track of orig_cause of subcauses before being 
        # broken into decimals subcodes
    ...
    # NOTE: MAIN RUN to remove subtotals
    ...
    # NOTE: This section converting decimals back to integers
    ...
    for i in unique_df.index:
        these_components = code_components.determine_components(unique_df.ix[i],
                                                ICD10_code_index)
    ...
    for u in uid_set:
        ...
        # if nothing was removed, or if nothing was removed and it was a single cause, 
        #   or all of the input codes are still present, set the codes remaining as the
        #   cause
    ...
    # NOTE: conditions for handling: if orig metric value = current value, leave alone
