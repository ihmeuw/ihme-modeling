...
** NOTE: Should run for both VR and CR before running any final MIR models 
** Input(s): N/A
...
program add_merge_name
    // generate merge_name
        ...
        replace merge_name = "usa_seer_update" if regexm(dataset_name, "usa_seer_1973_2013")
        replace merge_name = "IND_PBCR_inc" if regexm(dataset_name, "IND_PBCR_2009_2011_inc")
...
// // Get INC data and Reformat to enable merge
    ...
    if "`type'" == "VR" {
        save "$temp_folder/incDataVR.dta", replace
    }
    else {
        save "$temp_folder/incData.dta", replace
    }
...
// // Get MOR data
    ...
    if "`type'" == "VR" {
        ...
        save "$temp_folder/morData.dta", replace
    }

// Merge data from the same dataset
    ** NOTE: cases1/deaths1 with missing values are dropped during standardize_format.do after all cases/deaths values are verified (as of 5/15/2015). 
	** If data appears to be missing from a dataset after the merge, verify missing entries in the formatted file
    if "`type'" == "VR" {
    ...
    if "`type'" == "VR" {
        save "$temp_folder/mergedVR.dta", replace
    }
    else {
...
** *****************************************************************************
if "`type'" == "VR" {
    use "$temp_folder/mergedVR.dta", clear
}
else {
    use "$temp_folder/merged.dta", clear
}
...
// Keep relevant variables
    ...
    if "`type'" == "VR" {
        save "$temp_folder/refinedVR.dta", replace
    }
    else {
...
// Load Merged Registry Data
    if "`type'" == "VR" {
        use "$temp_folder/refinedVR.dta", clear
    }
    else {
...
// // Keep National Data where possible
    ...
    compress
    if "`type'" == "VR" {
        save "$temp_folder/first_MI_drop_tempfileVR.dta", replace
    }
    else {
...
// // Separate INC-only: check for redundancy and rename identifiers
    // Keep INC-only
        if "`type'" == "VR" {
            use "$temp_folder/first_MI_drop_tempfileVR.dta", clear
        }
        else {
    ...
    // Save file for use with leukemia data in Part 6 (below)
        preserve
            ...
            if r(N) > 0    {
                if "`type'" == "VR" {
                    save "$temp_folder/leukemia_dataVR.dta", replace
                }
                else {
    ...
    preserve 
        ...
        save "$temp_folder/inc_only_pop.dta", replace
    restore

    // Run KeepBest
        reshape long cases, i(registry_index year* sex_id acause dataset_id dataset_name*) j(age)
        gen deaths = .
        save "$temp_folder/inc_only_keepbest_input`type'.dta", replace   

        !python "FILEPATH" "$temp_folder/inc_only_keepbest_input`type'.dta" "mi_ratio"

        import delimited using "$temp_folder/keepbest_output_mirs.csv", clear 
    ...
    // merge back population 
        merge m:1 registry_index year_id sex_id acause dataset_name using "$temp_folder/inc_only_pop.dta", assert(2 3) keep(3) nogen 
    ...
    // Finalize INC-only
       capture drop dataType dataset_name dataset_id
        if "`type'" == "VR" {
            save "$temp_folder/INC_onlyVR.dta", replace
        }
        else {
...
// // MOR-only: check for redundancy and rename identifiers
    ...
    preserve 
        ...
        save "$temp_folder/mor_only_pop.dta", replace
    restore

    // Run KeepBest
        reshape long deaths, i(registry_index year* sex_id acause dataset_id dataset_name*) j(age)
        gen cases = .
        save "$temp_folder/mor_only_keepbest_input`type'.dta", replace 
        !python "FILEPATH" "$temp_folder/mor_only_keepbest_input`type'.dta" "mi_ratio" "MOR"
        import delimited using "$temp_folder/keepbest_output_mirs_MOR.csv", clear 
        drop if deaths == .
        drop cases
    ...
    // merge back population 
        merge m:1 registry_index year_id sex_id acause dataset_name using "$temp_folder/mor_only_pop.dta", assert(2 3) keep(3) nogen 
    ...
    // Finalize MOR-only
        capture drop dataType dataset_name dataset_id
        if "`type'" == "VR" {
            save "$temp_folder/MORonlyVR.dta", replace
        }
        else {
...
// // Merge INC-only/MOR-only and keep only matching registries
    if "`type'" == "VR" {
        use "$temp_folder/MORonlyVR.dta", clear
    }
    else {
    ...
    // merge
        ...
        if "`type'" == "VR" {
            merge 1:1 registry_index sex_id year_id* acause age using "$temp_folder/INC_onlyVR.dta"
        }
        else {
            merge 1:1 registry_index sex_id year_id* acause age using "$temp_folder/INC_only.dta"
        }
...
// // save combined data
    ...
    // save
        compress
        if "`type'" == "VR" {
            save "$temp_folder/second_MI_drop_tempfileVR.dta", replace
        }
        else {
...
// Append Merged INC/MOR Data to data of which INC/MOR are from the same dataset
    ...
    if "`type'" == "VR" {
        append using "$temp_folder/second_MI_drop_tempfileVR.dta"
    ...
    if "`type'" == "VR" {
        save "$temp_folder/third_MI_drop_tempfileVR.dta", replace
    }
    else {
...
// preserve population values 
    preserve 
        ...
        save "$temp_folder/pop_inc_mor.dta", replace 
    restore

// Run KeepNational and KeepBest on the recombined data to eliminate redundancies
    !python "FILEPATH" "$temp_folder/third_MI_drop_tempfile`type'.dta" "mi_ratio" "INC_MOR"
    import delimited using  "$temp_folder/keepbest_output_mirs_INC_MOR.csv", clear 
...
// Save a copy of the final data selections with full information
    ...
    if "`type'" == "VR" {
        save_staging_data, output_file("selected_MI_data_VR_$today.dta")
    }
    else {
...
// Combine sexes for GBD2019 new causes
    preserve
        keep if inlist(acause, "neo_bone","neo_eye","neo_eye_other","neo_eye_rb","neo_liver_hbl","neo_lymphoma_burkitt","neo_lymphoma_other","neo_neuro","neo_tissue_sarcoma")
        replace sex_id = 3
...
// Save
    if "`type'" == "VR" {
        export delimited "$mi_ratio_staging_storage/MI_ratio_model_input_VR_$today.csv", replace
    }
    else {
