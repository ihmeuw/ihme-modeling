...
cod_data <- get_cod_data(cause_id = cause_id, age_group_id = age_groups_ids, decomp_step = "step3", gbd_round_id = 7)

#Use 4&5 star data only to calculate ratios 
vr_star <- data.table(read.xlsx("FILEPATH"))  
loc_meta <- get_location_metadata(location_set_id = 22,gbd_round_id = 7,decomp_step = "step3")
