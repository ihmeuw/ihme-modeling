...
source_shared_functions(functions =  c("get_covariate_estimates", "get_population", "interpolate"))
# Read in parameters ------------------------------------------------------
args <- commandArgs(trailingOnly = TRUE)
out_dir <- as.character(args[1])
root_dir <- as.character(args[2])
location_id <- as.numeric(args[3])
model_version_id <- as.numeric(args[4])
...
# and grab data for women of child-bearing age
asfr_id       <- 13
female_id     <- 2
...
asfr <- get_covariate_estimates(covariate_id = asfr_id, location_id = location_id, sex_id = female_id,
                                age_group_id = age_group_ids, decomp_step = decomp, gbd_round_id = 7) %>% 
  filter(year_id >= 1980) 


years <- unique(asfr$year_id)
pop   <- get_population(year_id = years, location_id = location_id, sex_id = female_id, age_group_id = age_group_ids, decomp_step = decomp, gbd_round_id = 7)
...
readr::write_csv(live_births, "FILEPATH")
...
draws <- interpolate_safe(limit = limit, dir = "FILEPATH", unique_id = location_id,
                          ...
                          location_id = location_id, age_group_id = age_group_ids, version_id = model_version_id, decomp_step = "iterative")
...
readr::write_csv(unadjusted_births, "FILEPATH")
