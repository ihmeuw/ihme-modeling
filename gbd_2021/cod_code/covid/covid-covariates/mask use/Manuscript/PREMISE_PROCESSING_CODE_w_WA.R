...
metadata_path <- file.path(output_dir, "PREMISE_PROCESSING_CODE.metadata.yaml")
...
ST<-readShapePoly("FILEPATH/lsvid_723/best/covid.shp")
...
states <- ST[ST$parent_id==102 & ST$loc_id!=102 & ST$loc_id!=570,]
wa_ctys <- ST[ST$loc_id %in% c(60886, 60887, 3539),]
states <- rbind(states, wa_ctys)
...
us_premise_impact$loc_id <- as.numeric(us_premise_impact$loc_id)
...
counties <- data.frame(state=c("Spokane County","King and Snohomish Counties", "Washington except for King, Snohomish, and Spokane Counties"),
                       Division = "Pacific", Region = "West")
state_mapping <- rbind(state_mapping, counties)
...
weeks_all<-read.csv(paste0(input_dir,"/weeks.csv"))
...
for(v in vars_to_plot){
  ...
  for(s in unique(prepped_data_state[variable==v]$state)){
    ...
    unique_ss$proportion <- 1.005
    p_char <- ggplot()+
      geom_bar(data=prepped_data_state[state == s & type == "character" & variable == v],
...
write.csv(weekly_data_all, file.path(output_dir, "Weekly_Premise_Mask_Use_WA.csv"), row.names=FALSE)

#unique states that have at least one week <30 counts
states_less_30<-weekly_data_all[weekly_data_all$question_count<30,]
#set of those states
