...
estimate_available_doses <- function(vaccine_output_root) {
  ...
  final_dt[, effective_stockpile_doses := ifelse(secured_daily_doses > 0, 
                                                 daily_stockpile_doses * (secured_effective_daily_doses / secured_daily_doses), 
                                                 0)]
  
  ## We only expect Pfizer, Moderna to be available in 2020
  # (except in Russia, China)
  final_dt[!(location_id %in% c(62, children_of_parents(6, hierarchy, "loc_ids"))), # including all nested China subnats using path_to_top_parent
           secured_daily_doses := ifelse(date < "2021-01-01", 0, secured_daily_doses)]
  final_dt[!(location_id %in% c(62, children_of_parents(6, hierarchy, "loc_ids"))),
  ...
  # pre-agg dx
  agg_cols <-   # borrow from below
    c("secured_daily_doses","daily_stockpile_doses","effective_stockpile_doses","secured_effective_daily_doses",
  ...
  plot_cols <- c("secured_daily_doses","daily_stockpile_doses","effective_stockpile_doses","secured_effective_daily_doses",
                 "location_id", "location_name", "date", "population")
  plot_cols_piv <- plot_cols[!plot_cols %in% c("location_id", "location_name", "date", "population")]
  
  tmp <- aggregate_up_to_parents(
    dt = tmp, 
    hierarchy = hierarchy,
    interpolate = FALSE, 
    agg_cols = agg_cols
  ...
  # Recalculate columns
  tmp[, weighted_efficacy_location := secured_effective_daily_doses / secured_daily_doses]
  tmp[, weighted_variant_efficacy_location := secured_variant_effective_daily_doses / secured_daily_doses]
  tmp[, weighted_protected_location := effective_protected_daily_doses / secured_daily_doses]
  tmp[, weighted_efficacy_residual := effective_daily_residual / residual_secured_daily_doses]
  tmp[, weighted_efficacy_global := global_effective_daily_residual / global_daily_residual]
  ...
  #   theme_minimal() +
  #   ggtitle(unique(tmp[location_id==loc, location_name]))
  
  # Reassign looped dataframe back to original when diagnostics are passed
  final_dt <- copy(tmp) # un-comment when diagnostics show this is ready
  ...
  ##-------------------------------------------------------------------------
  message(paste('Writing available doses to file:', file.path(vaccine_output_root, 'final_doses_by_location.csv')))
  ...
  vaccine_data$write_final_doses(final_dt, vaccine_output_root)
