...
version_1 <- args$current_version
version_2 <- args$compare_version
vaccine_output_root <- file.path(DATA_ROOTS$VACCINE_OUTPUT_ROOT, version_1)
...
# Identify latest date at which vax can be compared to vax+willing

tmp <- do.call(rbind,
                     lapply(split(hes1, by='location_id'), 
                            FUN=function(x) {
                              
                              sel <- max(which(complete.cases(x[,.(pct_vaccinated, smooth_combined_yes)])))
                              return(x[sel,])
...
tmp <- ggplot(data=tmp) + 
  ...
  ylab('pct_vaccinted') +
  ggtitle(glue('Version {version_1}: pct_vaccianted vs pct_vaccinted + willing up to {hes1$max_date[1]}')) +
  theme_bw()
