#booster dose baseline effectiveness
...
ve1 <- subset(basve, select = c(exclude, study_id, study_id2, author, location_id, location_name, vac_before_booster, vaccine_developer, 
...
#Subset by vaccine_developer == "Pfizer & BioNTech"  "Moderna" "AstraZeneca" "Johnson & Johnson"

#inf <- subset(ve2, vac_before_booster == "Pfizer & BioNTech" & sev_severity == "infection"
#                 & vaccine_developer == "Moderna") #Moderna #AstraZeneca #Pfizer & BioNTech #Johnson & Johnson

#sym <- subset(ve2, vac_before_booster == "Pfizer & BioNTech" & sev_severity == "symptomatic"
#                 & vaccine_developer == "Moderna") #Moderna #AstraZeneca #Pfizer & BioNTech #Johnson & Johnson

#sev <- subset(ve2, vac_before_booster == "Pfizer & BioNTech" & sev_severity == "severe"
#                 & vaccine_developer == "Moderna") #Moderna #AstraZeneca #Pfizer & BioNTech #Johnson & Johnson

#inf <- subset(ve2, sev_severity == "infection" & vaccine_developer == "Johnson & Johnson")
...
fp_dat <- data.frame(
  mean = c(NA, NA, sev$efficacy_mean, NA, ve1$ve),
  lower = c(NA, NA, sev$efficacy_lower, NA, ve1$ve_lower),
  upper = c(NA, NA, sev$efficacy_upper, NA, ve1$ve_upper)
)

fp_text <- cbind(
  c("", "Author", as.character(sev$author), NA, "Mean estimate"),
  c("", "Country", as.character(sev$location_name), NA, NA),
  c("", "Booster dose effectiveness (95% UI)", paste0(
