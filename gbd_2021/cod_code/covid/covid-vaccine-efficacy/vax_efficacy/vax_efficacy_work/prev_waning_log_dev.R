...
inf <- subset(infec, group_analysis == current_variant)

#write.csv(inf, "/ihme/homes/cstein87/vaceff/pinf_curves/past_infection.csv", row.names = F)
#write.csv(inf, "/ihme/homes/cstein87/vaceff/pinf_curves/past_infection_o.csv", row.names = F)
...
#3.2.3 Ã¢ÂÂ Uncertainty from fixed effects and between-study heterogeneity
...
past_inf_ancest_omi <- copy(df_pred3)
past_inf_ancest_omi <- as.data.table(past_inf_ancest_omi)

# set pinf values >1 to 1
# past_inf_ancest_omi_adjusted$pinf[past_inf_ancest_omi_adjusted$pinf > 1] <- 1

fwrite(past_inf_ancest_omi, file.path(output_dir, "past_inf_ancest_omi.csv"))

# saving inf dt
inf_ancest_omi <- copy(inf)
fwrite(inf_ancest_omi, file.path(output_dir, "inf_ancest_omi.csv"))
...
#ancestral, Alpha, Delta
#past_inf <- df_pred3
#write.csv(past_inf, "/ihme/homes/cstein87/vaceff/pinf_curves/past_inf.csv", row.names = F)

#Omicron
#past_inf_o <- df_pred3
#write.csv(past_inf_o, "/ihme/homes/cstein87/vaceff/pinf_curves/past_inf_o.csv", row.names = F)
...
previnfplot <- ggplot() + 
  ...
  labs(y="Previous infection protection", x = "Week after infection", size = "Inverse of variance") + #colour = "Primary and second variant"
  ...
  #Panel B: Protection against Omicron reinfection
  ggtitle("Panel B: Protection against Omicron reinfection") + 
  #guides(size = TRUE)
  guides(color = FALSE)
...
outfile <- "FILEPATH"
ggsave(file = paste0(outfile, "Fig3PanelB", ".pdf"), previnfplot, device = cairo_pdf, #Fig3PanelA #Fig3PanelB
