...
# defining eff offset eff scale
efficacy_offset <- 0.1
efficacy_scale <- 0.9
minimum_efficacy <- efficacy_offset + 0.01

# create e_mean column of copied efficacy mean data
ve2 <- mutate(ve2, e_mean = efficacy_mean)
ve2[e_mean < minimum_efficacy, e_mean := minimum_efficacy]

# calculating e_mean from efficacy_mean (James fix)
ve2 <- mutate(ve2, e_mean = (e_mean - efficacy_offset)/efficacy_scale)
...
ve2 <- mutate(ve2, se = ((efficacy_upper - efficacy_lower)/3.92)/(efficacy_scale^2))

#subsetting sev_severity
...
inf <- subset(ve2, group_analysis == current_variant)
...
mod1 <- MRBRT(
  data = ve6,
  cov_models = list(
    LinearCovModel("intercept", use_re = FALSE),  #TRUE #FALSE: Reduce the variation of the random effect #use_re  = use random effect
    LinearCovModel(
      alt_cov = "mid_point",
      use_spline = TRUE,
      ...
      spline_knots = array(seq(0, 1, length.out = 5)),       # 1.0  9.5 18.0 26.5    #(seq(0, 1, by = 0.2)
      spline_degree = 2L,
...
#Transforming back to VE (original code)
...
df_pred3 <- mutate(df_pred3, pinf = ((plogis(pred5)*efficacy_scale) + efficacy_offset)) # James fix
...
past_inf_non_omi_5knots_lt_true_adjusted <- copy(df_pred3)
# past_inf_ancest_omi_adjusted <- as.data.table(past_inf_ancest_omi_adjusted)

# set pinf values >1 to 1
# past_inf_ancest_omi_adjusted$pinf[past_inf_ancest_omi_adjusted$pinf > 1] <- 1

fwrite(past_inf_non_omi_5knots_lt_true_adjusted, file.path(output_dir, "past_inf_non_omi_5knots_lt_true_adjusted.csv"))

# saving inf dt
inf_non_omi_5knots_lt_true_adjusted <- copy(inf)
fwrite(inf_non_omi_5knots_lt_true_adjusted, file.path(output_dir, "inf_non_omi_5knots_lt_true_adjusted.csv"))

#Omicron
# past_sev_o <- df_pred3
# fwrite(past_sev_o, file.path(output_dir, "past_sev_o.csv"))

######
#plot waning prev_inf

# inf <- mutate(inf, insesqua = (1/sqrt(se))/10)
# inf <- mutate(inf, insesqua_smaller = ((1/sqrt(se))/10)*0.75)
...
previnfplot <- ggplot() + 
  ...
  geom_point(data = inf, mapping = aes(x = mid_point, y = e_mean, color = study_id2, size = insesqua_reduced), 
             show.legend = FALSE) +
