...
ve2 <- mutate(ve2, prim_variant_name = if_else(prim_inf_var == "Ancestral", "Alpha",
                                               if_else(prim_inf_var == "all variant", "Alpha",         #original: "all variant", "all variant",
                                                       if_else(prim_inf_var == "Ancestral, B.1.1.7", "Alpha",  #original: "Ancestral, B.1.1.7", "mixed",
                                                               if_else(prim_inf_var == "mixed variant", "Alpha",       #original: "mixed variant", "mixed",
                                                                       if_else(prim_inf_var == "mixed", "Alpha",               #original: "mixed", "mixed",
                                                                               if_else(prim_inf_var == "B.1.1.7", "Alpha",             #changed to "Ancestral" for this category be considered in the data availability plot
                                                                                       if_else(prim_inf_var == "B.1.351", "Beta",
                                                                                               if_else(prim_inf_var == "B.1.617.2", "Alpha",           #changed to "Ancestral" for this category be considered in the data availability plot
...
ve2 <- mutate(ve2, variant_name = if_else(variant == "Ancestral", "Alpha",
                                          if_else(variant == "all variant", "Alpha",                 #original: "all variant", "all variant",
                                                  if_else(variant == "mixed variant", "Alpha",               #original: "mixed variant", "mixed",
                                                          if_else(variant == "mixed", "Alpha",                       #original: "mixed", "mixed",
                                                                  if_else(variant == "mixed ", "Alpha",                      #original: "mixed ", "mixed",
                                                                          if_else(variant == "Ancestral, B.1.1.7", "Alpha",          #original: "Ancestral, B.1.1.7", "mixed", 
...
# subsetting variant
ve2 <- subset(ve2, variant == variant_type)

# subsetting symptom severity
...
#Previous infection
infec <- subset(ve2, sev_severity == severity_type)
...
inf <- subset(infec, variant_name == variant_type)
...
past_inf_alpha <- copy(df_pred3)
fwrite(past_inf_alpha, file.path(output_dir, "past_inf_alpha.csv"))

# saving inf dt
inf_alpha <- copy(inf)
fwrite(inf_alpha, file.path(output_dir, "inf_alpha.csv"))

#Omicron
past_inf_o <- df_pred3
fwrite(past_inf_o, file.path(output_dir, "past_inf_o.csv"))
