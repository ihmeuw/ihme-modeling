...
*** CREATE EMPTY ROWS FOR INTERPOLATION (NOT NEEDED IF PULLING DR COD MODEL) ***
	if "`codModel'"=="global" {
		...
		generate year_id = _n + 1989
		drop if inlist(year_id, `yearList') | year_id>`lastModelYear'
...
*** MERGE IN CASE FATALITY DATA *** 
	if "`codModel'"=="global" merge m:1 age_group_id using `rootDir'/FILEPATH.dta, assert(3) nogenerate
	...
	*/
	...
	forvalues i = 0/999 {
		quietly {
			...
			replace inc_`i' = inc_`i'/2 if age_group_id==3  //  given incubation period only have of this age group is at risk
...
\******************************************************************************/			
	...
	drop if year_id > `lastModelYear'
	
	expand 11 if year_id==1990, gen(newObs)
	bysort newObs location_id age_group_id sex_id (year_id): replace year_id = _n + 1979 if newObs==1
	drop newObs
	
	expand `=1 + max(`yearList') - `lastModelYear'' if year_id==`lastModelYear', gen(newObs)
	bysort newObs location_id age_group_id sex_id (year_id): replace year_id = _n + `lastModelYear' if newObs==1
	drop newObs
	
	merge 1:1 location_id year_id age_group_id sex_id using `shiftFile', keep(1 3) nogenerate
	
	
	forvalues i = 0/999 {
		quietly {
			replace typh_`i' = exp(ln(typh_`i') + shift) if year<1990 | year>`lastModelYear'
			replace para_`i' = exp(ln(para_`i') + shift) if year<1990 | year>`lastModelYear'
			replace typh_`i' = 0 if age_group_id==2 & (year<1990 | year>`lastModelYear')
			replace para_`i' = 0 if age_group_id==2 & (year<1990 | year>`lastModelYear')
			}
		di "." _continue
		}
...
if "`codModel'"=="global" {
	...
	foreach type in typh para {
		...
		forvalues year = 1990/`lastModelYear' {
	...
	if "`codModel'"=="global" {
		...
		foreach x in typh para {
			...
			foreach location_id of local location_ids { 
				outsheet location_id year_id sex_id age_group_id cause_id measure_id draw_* using `outDir_`x''/FILEPATH/`location_id'.csv if location_id==`location_id', comma replace
				}
...
*** EXPORT DEATHS IF USING DATA RICH CODEM MODEL ***
		...
		foreach x in typh para {
			...
			outsheet location_id year_id sex_id age_group_id cause_id measure_id draw_* using `outDir_`x''/FILEPATH/`location'.csv, comma replace
			}
...
*** EXPORT TOTAL INCIDENCE ***		
	...
	foreach location_id of local location_ids { 
		outsheet location_id year_id sex_id age_group_id modelable_entity_id measure_id draw_* using `rootDir'/FILEPATH/`location_id'.csv if location_id==`location_id', comma replace
		}
...
*** EXPORT SQUEEZED PROPORTION MODELS ***		
	...
	foreach x in para typh {
		...
		foreach location_id of local location_ids { 
			outsheet location_id year_id sex_id age_group_id modelable_entity_id measure_id draw_* using `outDir_`x''/FILEPATH/`location_id'.csv if location_id==`location_id', comma replace
			}
