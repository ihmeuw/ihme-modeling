...
*** CREATE SEVERITY SPLIT DRAWS FOR TYPHOID ***
	...
	local gastric_bleeding = 0.05 * (2/365) * (52/6) // we assume that 5% of people will have this for 2-days
	local inf_mod    = 0.35 
	local inf_sev    = 0.43 + 0.05 - `gastric_bleeding'  // for the remainder of duration, those with gastric bleeding are assumed to have inf_sev
...
*** CREATE DURATION DISTRIBUTION PARAMETERS ***	
	...
	for the duration of typhoid/paratyphoid symptoms.
	...
	The second block was used for moderate illness with a mean duration of 14 days and 95%CI of 7-21 (1-3 weeks)
	*/	
	...
	/* testing code
	local w:             4.258416742493178
	local v:             1.741583257506825
	replace test = rbeta(`v', `w') * `range' + `min'
	sum test, d
	centile test, centile(2.5 97.5)
	*/
	...
	save `rootDir'/FILEPATH/sequela_splits.dta, replace
