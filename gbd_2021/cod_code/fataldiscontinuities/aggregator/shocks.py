...
def read_all_model_draws(required_columns, 
    ...
    shocks_causes_rate = [729,945,387,699,707,695,725, 726, 727,854,842,711,703,
                          302,332,341,408,345,843,357, 337, 338]
    data = []
    def pull_draws(cause_id, location_id, required_columns):
        ...
        if cause_id == 843:
            ...
            data_sub = pd.read_csv(read_file,encoding='utf8')
        data_sub = data_sub.query("year_id <= 2020")
        assert max(data_sub.year_id.unique()) == 2020
    ...
    for cause in shocks_causes_rate:
        if int(location_id) in [44533] + list(range(44793,44801)):
            meta = lm(gbd_round_id=7, location_set_id=21)
