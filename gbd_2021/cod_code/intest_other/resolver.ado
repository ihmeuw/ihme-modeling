...
*** DETERMINE IF WE ARE PULLING MODEL RESULTS OR COVARIATES AND SET UP ENVIRONMENT ACCORDINGLY ***
	if "`estimate_type'" == "results" {
		...
		get_model_results, `command' location_set_id(35) decomp_step("iterative") gbd_round_id(7) clear
		...
		get_covariate_estimates, covariate_id(`command') decomp_step("iterative") gbd_round_id(7)  clear
...
*** CROSS ALL COMBINATIONS OF MEASURE, LOCATION, AGE, YEAR & SEX FROM BOTH MASTER & RESULTS ***	
	...
	if "`estimate_type'" == "results" {
		...
		keep location_id year_id age_group_id sex_id mean parent_id masterMerge measure
		duplicates drop 
		}
