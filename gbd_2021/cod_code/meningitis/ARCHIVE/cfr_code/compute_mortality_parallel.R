...
if(n_draws == 1000) n_draws <- NULL # for use in shared function calls
...
meninigitis_shock_version_ids <- c(652571, 652646) # change every ds

# directories where fatal and nonfatal PAF csvs will be written
fatal_out_dir <- paste0(out_dir, "fatal/")
...
nonfatal_out_dir <- paste0(out_dir, "nonfatal/")
...
if(is.null(n_draws)) codcorrect_version <- 239 else codcorrect_version <- 239
...
# the above already includes shocks
...
if(is.null(n_draws)){
  if (length(grep("draw_", colnames(other_save_results_mort_dt))) < 1000) {
    file.create(paste0(h, "failed_", loc, ".txt"), overwrite=T)
    stop(paste("You need 1000 draws in your input file for location", loc))
  }
} 
