...
uncertainty <- TRUE # Indicator if you need uncertainty. This will increase runtime. 
...
mob_nat <- mob_nat[date %in% last_days] #keep 1 row of covariate per location-month (mob_avg_month is same value for all days(ie rows) in month) and we have only data that is assigned to last day of month (monthly data for meas, monthly avg for flu)
...
# Set Jan and Feb to be zero except China and Taiwan
mask[date < as.Date("2020-03-01") & !(location_id %in% c(6,8)), mask_avg := 0] 
...
gg_meas_trim <- ggplot(meas_outlier, aes(x = mob_avg_month, y = plotting_ratio, color = as.factor(w))) + geom_point() +
  ggtitle(paste0("Measles data outliering in global spline cascade with ", 100*(1-inlier_pct), " percent trimming")) +
...
### HELPER FUNCTION FOR MAKING VETTING PDF
sr_flu_plot <- function(data, my_super_region_name) {
  gg_sr_flu <- ggplot(data[grepl("sr_cause__", cascade_prediction_id) & super_region_name == my_super_region_name & cause == "flu" & is.na(month)], 
...
sr_measles_plot <- function(data, my_super_region_name) {
  gg_sr_measles <- ggplot(data[grepl("sr_cause__", cascade_prediction_id) & super_region_name == my_super_region_name & is.na(month)], 
                          aes(x = mob_avg_month, y = pred, color = as.factor(mask_use_bin)))+
...
if(uncertainty){
  ...
  for(my_cause in c("measles", "flu")){
    ...
    message("wrote out summarized draws")
  }
