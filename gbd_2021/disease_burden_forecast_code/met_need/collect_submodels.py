...
def entity_specific_collection(
    ...
    if intrinsic:
        # Per research decision, we set all intrinsic scenarios to reference
        non_ref_scenarios = [
            s
...
def fix_scenario_crossing(years: YearRange, future_da: xr.DataArray) -> xr.DataArray:
    ...
    # It's also decided that non-ref scenarios should not have uncertainty
    dim_order = ["draw"] + [x for x in future_da.dims if x != "draw"]
    future_da = future_da.transpose(*dim_order)  # draw-dim to 1st to broadcast

    # per RT meeting 20211201, we'll no longer save computed past SEVs
...
def intercept_shift_processing(
    ...
    if transform != "no-transform":
        ...
        # Perhaps the right thing to do is to follow the scalars pipeline
